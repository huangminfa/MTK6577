...
public class MessagingController implements Runnable {
    ...
    private final PriorityBlockingQueue<Command> mHighPriorityCommands = new PriorityBlockingQueue<Command>(
            11, new Comparator<Command>() {
                @Override
    ...
    static private int mPriorityNumber = 100;
    ...
    public void run() {
        ...
        while (true) {
            ...
            if (command.listener == null || isActiveListener(command.listener)) {
                ...
                Logging.d("Command [" + command.description +"] time [" +(System.currentTimeMillis() - time)+"ms], left " + mCommands.size() +" tasks");
        ...
        put("synchronizeMailbox", listener, new Runnable() {
            public void run() {
                try {
                    ...
                    if (0 == mSendStatus) {
                        try {
                            callback.updateCallback(EmailExternalConstants.RESULT_FAIL, account.mId, folder.mId);
                        } catch (RemoteException e) {
                            Logging.w(Logging.LOG_TAG, "synchronizeMailbox Failed");
                        }
                    } else {
                        try {
                            callback.updateCallback(EmailExternalConstants.RESULT_SUCCESS, account.mId, folder.mId);
                        } catch (RemoteException e) {
     ...
     */
    private boolean imapPartialFetchMessage(Message message, Folder remoteFolder) throws MessagingException{
        ...
        for (Message message : unsyncedMessages) {
            ...
            if (message.getSize() > (MAX_SMALL_MESSAGE_SIZE) || message.getSize() == 0) {
                ...
                if (message.getBody() == null) {
                    ...
                    copyOneMessageToProvider(message, account, toMailbox,
                            isPartialDownload ? EmailContent.Message.FLAG_LOADED_PARTIAL : EmailContent.Message.FLAG_LOADED_COMPLETE);
                }
            } else {
...
//
    ...
    private int searchMailboxImpl(long accountId, SearchParams searchParams,
        ...
        for (Message message : searchingMessages) {
            try {
                ...
                if (message.getSize() > (MAX_SMALL_MESSAGE_SIZE) || message.getSize() == 0) {
                    if (message.getBody() == null) {
                        // POP doesn't support STRUCTURE mode, but it should not go into search mode.
    ...
    private SyncResults synchronizeMailboxGeneric(final Account account, final Mailbox mailbox)
        ...
        // Only sync DRAFTS for Orange IMAP server
    ...
    private void processPendingUploadsSynchronous(Account account,
            ContentResolver resolver, String[] accountIdArgs) {
        HostAuth authRecv = HostAuth.restoreHostAuthWithId(mContext, account.mHostAuthKeyRecv);
        StringBuilder folderSelection = new StringBuilder(""); 
        if (authRecv != null && Email.isDraftSyncAccount(authRecv.mAddress)) {
            folderSelection.append(MailboxColumns.ACCOUNT_KEY + "=?"
                + " and " + (MailboxColumns.TYPE + "=" + Mailbox.TYPE_SENT + " or " + MailboxColumns.TYPE + "=" + Mailbox.TYPE_DRAFTS));
        } else {
            folderSelection.append(MailboxColumns.ACCOUNT_KEY + "=?"
    ...
    public void loadMessageForView(final long messageId, MessagingListener listener) {
        ...
        try {
            Command command = new Command();
            synchronized (this) {
                mPriorityNumber = mPriorityNumber - 1;
                command.priorityNumber = mPriorityNumber;
            }
            command.listener = listener;
            command.runnable = runnable;
            ...
            mHighPriorityThread = new Thread(new Runnable() {
                @Override
                 public void run() {
                    ...
                    // can get the newest command every time. As well as the efficiency will be better.
//                    synchronized (this) {
...
//                }
    ...
    public void cancelLoadAttachment(final long attachmentId) {
        synchronized(mCancelledAttachments) {
            ...
            // Try to break the downloading course if the data had already transferring from the remote side. 
            // i.e. close the network connection.
            if (mRunningCommand != null) {
                if (mRunningCommand.runnable instanceof CancelableRunnable && mDownloadingAttachmentId == attachmentId) {
            ...
            public void run() {
                try {
                    ...
                    if (mCancelledAttachments.contains(attachmentId)) {
                        Logging.d(ATTACHMENT_TAG, "Attachment downloading canceled before really starting");
                        return;
                    }
                    ...
                    synchronized(mCancelledAttachments) {
                        if (mCancelledAttachments.contains(attachmentId)) {
                            Logging.d(ATTACHMENT_TAG, "Attachment downloading canceled before opening remote folder");
                            return;
                        }
                    ...
                    mListeners.loadAttachmentFailed(
                            ...
                            new MessagingException(MessagingException.IOERROR, ioe.getMessage()), background);
