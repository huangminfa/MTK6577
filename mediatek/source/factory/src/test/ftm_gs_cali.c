...
static void *gs_cali_update_iv_thread(void *priv)
{
	...
	while(1)
	{
		...
		if(op == GS_OP_CLEAR)
		{
			...
			if(err)
			{
				snprintf(dat->gsc.status, sizeof(dat->gsc.status), "Clear fail(%d)\n", err);
				//dat->mod->test_result = FTM_TEST_FAIL;
			}
			else
			{
				snprintf(dat->gsc.status, sizeof(dat->gsc.status), "Clear okay\n");
		...
		else if(op == GS_OP_CALI)
		{
			...
			{
				len = snprintf(dat->gsc.status, sizeof(dat->gsc.status), "Calibrating: okay\n");
				dat->mod->test_result = FTM_TEST_PASS;
			}
