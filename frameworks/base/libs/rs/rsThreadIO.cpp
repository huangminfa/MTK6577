...
bool ThreadIO::playCoreCommands(Context *con, bool waitForCommand, uint64_t timeToWait) {
    ...
    while (!mToCore.isEmpty() || waitForCommand) {
        ...
        // Fix clock_gettime vsyscall time wrap issue
        uint64_t delay = 0;
        if (waitForCommand) {
        	uint64_t endTime = con->getTime();
            delay = timeToWait - (endTime - startTime);
            if (delay > timeToWait) {
            	// avoid timing issue
            	if (timeToWait > 0) {
            		LOGE("clock_gettime vsyscall time wrap error. timeToWait=%llu, "
            				"delay=%llu, startTime=%llu, endTime=%llu",
            				timeToWait, delay, startTime, endTime);
