...
public class PolarClockWallpaper extends WallpaperService {
    ...
    class ClockEngine extends Engine implements SharedPreferences.OnSharedPreferenceChangeListener {
        private float SMALL_RING_THICKNESS = 8.0f;
        private float MEDIUM_RING_THICKNESS = 16.0f;
        private float LARGE_RING_THICKNESS = 32.0f;

        private float DEFAULT_RING_THICKNESS = 24.0f;

        private float SMALL_GAP = 14.0f;
        private float LARGE_GAP = 38.0f;
        ...
        private void scaleThicknessAndTap(){
            float ratio = 1.0f;
            WindowManager mWM = (WindowManager) getSystemService(Context.WINDOW_SERVICE);
            if (mWM != null){
            	int width = mWM.getDefaultDisplay().getWidth();
            	int height = mWM.getDefaultDisplay().getHeight();
            	ratio = (float)(width<height ? width:height) / 480.0f;
            	Log.v("PolarClock","width = "+width+"height = "+height);
            }
            SMALL_RING_THICKNESS *= ratio;
            MEDIUM_RING_THICKNESS *= ratio;
            LARGE_RING_THICKNESS *= ratio;
            DEFAULT_RING_THICKNESS *= ratio;
            SMALL_GAP *= ratio;
            LARGE_GAP *= ratio;
        }
