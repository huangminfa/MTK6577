...
#ifndef ANDROID_DEFAULT_CODE
status_t SampleTable::setStartTimeOffset(uint64_t time_offset)//added by hai.li to support track time offset
{
	mStartTimeOffset = time_offset;
	return OK;
}

uint64_t SampleTable::getStartTimeOffset()
{
    ...
    switch (flags) {
        ...
        default:
        {
            ...
            if (closestIndex > 0) {
                ...
                uint32_t absdiff1 =
                    abs_difference(
                            (uint32_t)mSampleTimeEntries[closestIndex].mCompositionTime,
                            req_time);

                uint32_t absdiff2 =
                    abs_difference(
                            (uint32_t)mSampleTimeEntries[closestIndex - 1].mCompositionTime,
                            req_time);
#else
                uint32_t absdiff1 =
                    abs_difference(
...
#ifndef ANDROID_DEFAULT_CODE
        uint64_t sample_time = mSampleIterator->getSampleTime(true);
#else
...
#ifndef ANDROID_DEFAULT_CODE
        uint64_t x_time = mSampleIterator->getSampleTime(true);
#else
...
#ifndef ANDROID_DEFAULT_CODE
        uint64_t y_time = mSampleIterator->getSampleTime(true);
#else
...
status_t SampleTable::getMetaDataForSample(
    ...
    if (compositionTime) {
        *compositionTime = mSampleIterator->getSampleTime(true);
    }
