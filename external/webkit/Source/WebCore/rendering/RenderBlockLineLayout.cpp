    ...
    if (firstChild()) {
        ...
        if (doTextWrap && hasTextToWrap) {
            ...
            while (obj) {
                ...
                /// M: workaround for CMCC TestCase6, don't doTextWrap inside that table. @{
                if (obj->isTable() && obj->style()->width().isPercent()
                        && obj->style()->width().percent() == 50.0 && obj->style()->hasBorder()) {
                    isConstrained = true;
                    break;
                }
                /// @}
