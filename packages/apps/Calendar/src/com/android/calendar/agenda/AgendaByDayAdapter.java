...
public class AgendaByDayAdapter extends BaseAdapter {
    ...
    public void calculateDays(DayAdapterInfo dayAdapterInfo) {
        ...
        if (prevStartDay > 0) {
            ///M:If the repeate event's end julian Day out of range,don't add it
            ///to the adapter.@{
            int totalEnd = Utils.getValidJuLianDay(dayAdapterInfo.end);
            ///@}
            for (int currentDay = prevStartDay + 1; currentDay <= totalEnd;
                    currentDay++) {
                boolean dayHeaderAdded = false;
                Iterator<MultipleDayInfo> iter = multipleDayList.iterator();
                while (iter.hasNext()) {
        ...
        ///M:Filter all the events that out of range.
        ///To Ensure that no any event out of range@{
        Iterator<RowInfo> iteratorRowInfo = rowInfo.iterator();
        while (iteratorRowInfo.hasNext()) {
            RowInfo row = iteratorRowInfo.next();
            if (row.mDay < Utils.FIRST_CALENDAR_DAY || row.mDay > Utils.LAST_CALENDAR_DAY) {
                iteratorRowInfo.remove();
                LogUtil.i(TAG, "out of range,remove..row.mDay=" + row.mDay);
            }
        }
        ///@}
        mRowInfo = rowInfo;
