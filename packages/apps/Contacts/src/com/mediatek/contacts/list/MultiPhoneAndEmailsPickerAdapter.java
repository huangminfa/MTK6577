...
import com.mediatek.contacts.util.AnrUtil;
...
public class MultiPhoneAndEmailsPickerAdapter extends DataKindPickerBaseAdapter {
    ...
    protected void bindData(ContactListItemView view, Cursor cursor) {
        ...
        if (!cursor.isNull(getDataTypeColumnIndex())) {
            ...
            int simId = -1;
            if (indicateIndex != -1) {
                simId = cursor.getInt(indicateIndex);
                accountType = AnrUtil.getAccountTypeById(simId);
            }
            ...
            if (mimeType.equals(Email.CONTENT_ITEM_TYPE)) {
                ...
                if (OperatorUtils.isAasEnabled(accountType)) {
                    label = AnrUtil.getAasTypeLabel(type, simId);
                }
            }
        }
        Log.i(TAG, "DataKindPoickerBaseAdapter, label=" + label);
        /*
