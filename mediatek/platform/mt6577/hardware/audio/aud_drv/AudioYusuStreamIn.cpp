...
ssize_t AudioYusuStreamIn::read(void* buffer, ssize_t bytes)
{
   ...
   if(i4Standby == true)
   {
       ...
       if(mHw->pLad->pCCCI->CheckMDSideSphCoefArrival())
       {
            ...
            LOGD("read, MD Speech parameter is not ready and sleep(10ms)\n");
            usleep(10*1000);
            memset(buf,0,bytes);
            return bytes;
        }
...
void AudioYusuStreamIn::getVoiceRecordDataFromExtModem(uint8 *pBuf, int s4Size)
{
       ...
       rb    rbBuf_ExtMD;
       if (pBuf==NULL){
           LOGE("getVoiceRecordDataFromExtModem invalid pBuf\n");
           return;
       }

       if (s4Size<=0){
           LOGD("getVoiceRecordDataFromExtModem invalid s4Size\n");
           return;
       }
       //lock for buffer access
       InBuffLock();
       ...
       if ( mstandby == true ) {
          LOGD("getVoiceRecordDataFromExtModem::StreamIn has standby\n");
          InBuffUnLock();
          return;
       }
