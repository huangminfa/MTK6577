...
void update_Text_Info(ap_info * pApInfo, char* output_buf, int buf_len)
{
    ...
    ptr += sprintf(ptr, "Status : %s\r\n", 
        pApInfo->media_status==media__disconnect?"Disconnect": 
        (pApInfo->media_status==media_connecting?"Connecting":
        (pApInfo->media_status==media_connected?"Connected":"Unknown")));
    ...
    ptr += sprintf(ptr, "Mode : %s\r\n", pApInfo->mode==2?"Infrastructure":
            	(pApInfo->mode==1?"Ad-hoc":"unknown"));	      	
    ptr += sprintf(ptr, "Channel : %d\r\n", pApInfo->channel);           
    ptr += sprintf(ptr, "RSSI : %d dBm\r\n", pApInfo->rssi);    
    ptr += sprintf(ptr, "RATE : %d M\r\n", pApInfo->rate);    
