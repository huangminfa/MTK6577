...
void bb_verror_msg(const char *s, va_list p, const char* strerr)
{
	...
	applet_len = strlen(applet_name_hello) + 2; /* "applet: " */
...
#define msgptr (&(iov[2].iov_base))
	...
	if (logmode & LOGMODE_STDIO) {
		iov[0].iov_base = (char*)applet_name_hello;
		iov[0].iov_len = strlen(applet_name_hello);
		iov[1].iov_base = (char*)": ";
		iov[1].iov_len = 2;
		/*iov[2].iov_base = msgc;*/
		/*iov[2].iov_len = used;*/
