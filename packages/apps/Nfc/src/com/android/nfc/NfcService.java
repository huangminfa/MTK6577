...
/// M: import NfcAddon packages @{
import com.android.nfc.addon.NfcAddon;
/// @}
...
public class NfcService extends Application implements DeviceHostListener {
    ...
    public void onRemoteFieldActivated() {
        /// M: bypass EVT_FIELD_ON as a quick work-around for NXP solution. @{
	    Log.d(TAG, "bypass FIELD_ON");
    ...
    public void onRemoteFieldDeactivated() {
        /// M: bypass EVT_FIELD_OFF as a quick work-around for NXP solution. @{
	    Log.d(TAG, "bypass FIELD_OFF");
    ...
    /// M: @{
	private NfcAddon.INfcModeController mP2pRecvEnabler;
	private NfcAddon.INfcModeController mTagRwEnabler;
	private NfcAddon.INfcModeController mCardEmuController;
	private NfcAddon.INfcConfigurationObserver mP2pRecvSwitchCallback;
	private NfcAddon.INfcConfigurationObserver mTagRwSwitchCallback;
	private NfcAddon.INfcConfigurationObserver mCardEmuSwitchCallback;
	private NfcAddon.INfcConfigurationObserver mCardEmuConfigChangeCallback;
	private NfcAddon.INfcStatusBar mNfcStatusBar;
	private NfcAddon.INfcConfigurationObserver mAirplaneModeAutoSwitchChangeCallback;
	private NfcAddon.INfcModeController mAirplaneModeAutoSwitchController;
	private NfcAddon.INfcConfigurationObserver mP2pBitrate106SwitchCallback;
	private NfcAddon.INfcModeController mP2pBitrate106Enabler;
	private NfcAddon.INfcModeController mVibrationEnabler;
	private NfcAddon.INfcModeController mSoundEnabler;
	private NfcAddon.INdefSignatureParser mNdefSignatureParser;

	class P2pRecvSwitchCallback implements NfcAddon.INfcConfigurationObserver {
		public void onStateChanged() {
			Log.d(TAG, "[op customization]: P2p mode switch to " + mP2pRecvEnabler.isEnabled());
			if (isNfcEnabled()) {
				mP2pLinkManager.enableDisable(mIsNdefPushEnabled, mP2pRecvEnabler.isEnabled());
			}
			mNfcStatusBar.updateStatusBar(NfcService.this, isNfcEnabled(), mIsNdefPushEnabled, checkScreenState());
		}
	}
	
	class TagRwSwitchCallback implements NfcAddon.INfcConfigurationObserver {
		public void onStateChanged() {
			Log.d(TAG, "[op customization]: tag rw switch to " + mTagRwEnabler.isEnabled());
			mNfcStatusBar.updateStatusBar(NfcService.this, isNfcEnabled(), mIsNdefPushEnabled, checkScreenState());
		}
	}
	
	class CardEmuSwitchCallback implements NfcAddon.INfcConfigurationObserver {
		public void onStateChanged() {
			Log.d(TAG, "[op customization]: card emu switch to " + mCardEmuController.isEnabled());			
			if (isNfcEnabled()) {
				if (mCardEmuController.isEnabled()) {
					mDeviceHost.selectUiccSecureElement(DeviceHost.SELECT_UICC);
				} else {
					mDeviceHost.selectUiccSecureElement(DeviceHost.DESELECT_UICC);
				}
				
				if (mCardEmuController.getEnabledWhen() == NfcAddon.INfcModeController.ENABLED_WHEN_ALWAYS) {
					mDeviceHost.selectUiccSecureElementInPBTF(DeviceHost.SELECT_UICC);
				} else {
					mDeviceHost.selectUiccSecureElementInPBTF(DeviceHost.DESELECT_UICC);
				}
			}			
			mNfcStatusBar.updateStatusBar(NfcService.this, isNfcEnabled(), mIsNdefPushEnabled, checkScreenState());
		}
	...
	class CardEmuConfigureChangeCallback implements NfcAddon.INfcConfigurationObserver {
		public void onStateChanged() {
			if (isNfcEnabled()) {
				if (mCardEmuController.isEnabled()) {
					if (mCardEmuController.getEnabledWhen() == NfcAddon.INfcModeController.ENABLED_WHEN_ALWAYS) {
	...
	class AirplaneModeAutoSwitchChangeCallback implements NfcAddon.INfcConfigurationObserver {
		public void onStateChanged() {
			if (mPrefs.getBoolean(PREF_NFC_ON, NFC_ON_DEFAULT) && isAirplaneModeOn()) {
				if (mAirplaneModeAutoSwitchController.isEnabled() && isNfcEnabled()) 
					new EnableDisableTask().execute(TASK_DISABLE);
				else if (!mAirplaneModeAutoSwitchController.isEnabled() && !isNfcEnabled())
					new EnableDisableTask().execute(TASK_ENABLE);
        	}
		}
	}

	class P2pBitrate106SwitchCallback implements NfcAddon.INfcConfigurationObserver {
		public void onStateChanged() {
			if (mP2pBitrate106Enabler.isEnabled()) {
				mDeviceHost.doSelectP2pBitrate106(DeviceHost.ENABLE_P2P_BITRATE106);
			} else {
				mDeviceHost.doSelectP2pBitrate106(DeviceHost.DISABLE_P2P_BITRATE106);			
			}
		}
	}
    /// @}
    ...
    public void onCreate() {
        ...
        /// M: modified @{
		mP2pRecvSwitchCallback = new P2pRecvSwitchCallback();
		mTagRwSwitchCallback = new TagRwSwitchCallback();
		mCardEmuSwitchCallback = new CardEmuSwitchCallback();
		mCardEmuConfigChangeCallback = new CardEmuConfigureChangeCallback();
		mAirplaneModeAutoSwitchChangeCallback = new AirplaneModeAutoSwitchChangeCallback();
		mP2pBitrate106SwitchCallback = new P2pBitrate106SwitchCallback();
		mP2pRecvEnabler = NfcAddon.createBasicNfcModeController("nfc_p2p_enabled", true, mPrefs, mP2pRecvSwitchCallback);
		mTagRwEnabler = NfcAddon.createBasicNfcModeController("nfc_rw_enabled", true, mPrefs, mTagRwSwitchCallback);
		mCardEmuController = NfcAddon.createCardModeController("nfc_card_enabled", true, mPrefs, mCardEmuSwitchCallback, mCardEmuConfigChangeCallback);
		mAirplaneModeAutoSwitchController = NfcAddon.createBasicNfcModeController("nfc_airplane_mode_auto_switch", true, mPrefs, mAirplaneModeAutoSwitchChangeCallback);
		mP2pBitrate106Enabler = NfcAddon.createBasicNfcModeController("nfc_p2p_bitrate106_enabled", true, mPrefs, mP2pBitrate106SwitchCallback);
		mVibrationEnabler = NfcAddon.createBasicNfcModeController("nfc_vibration_enabled", true, mPrefs, new NfcAddon.INfcConfigurationObserver(){public void onStateChanged(){}});
		mSoundEnabler = NfcAddon.createBasicNfcModeController("nfc_sound_enabled", true, mPrefs, new NfcAddon.INfcConfigurationObserver(){public void onStateChanged(){}});
        String[] resStrings = new String[3];
        resStrings[0] = getString(R.string.mtk_str_nottrusted);
        resStrings[1] = getString(R.string.mtk_str_yes);
        resStrings[2] = getString(R.string.mtk_str_no);
		mNdefSignatureParser = NfcAddon.createNdefSignatureParser("prompt_untrusted_tag", false, mPrefs, resStrings);
        //String[] notifStrings = new String[6];
        //notifStrings[0] = getString(R.string.mtk_str_beam);
        //notifStrings[1] = getString(R.string.mtk_str_cardemu);
        //notifStrings[2] = getString(R.string.mtk_str_tagrw);
        //notifStrings[3] = getString(R.string.mtk_str_none);
        //notifStrings[4] = getString(R.string.mtk_str_enabled);
        //notifStrings[5] = getString(R.string.mtk_str_nfcactive);
		//mNfcStatusBar = NfcAddon.createBasicNfcStatusBar(R.drawable.nfc, R.layout.custom_notification, R.id.image, 
        //                R.drawable.ic_launcher_nfc, R.id.title, R.id.text, notifStrings, 
        //                mTagRwEnabler, mP2pRecvEnabler, mCardEmuController);
        mNfcStatusBar = NfcAddon.createDefaultNfcStatusBar();
        /// @}
    ...
    class EnableDisableTask extends AsyncTask<Integer, Void, Void> {
        ...
        boolean enableInternal() {
            ...
            /// M: modified @{
			if (!mCardEmuController.isDefault()) {
				if (mCardEmuController.isEnabled()) {
					mDeviceHost.selectUiccSecureElement(DeviceHost.SELECT_UICC);
					if (mCardEmuController.getEnabledWhen() == NfcAddon.INfcModeController.ENABLED_WHEN_ALWAYS) {
						mDeviceHost.selectUiccSecureElementInPBTF(DeviceHost.SELECT_UICC);
			...
			mNfcStatusBar.updateStatusBar(NfcService.this, true, mIsNdefPushEnabled, checkScreenState());
            /// @}
        ...
        boolean disableInternal() {
            ...
            /// M: modified @{ 
			if (!mCardEmuController.isDefault()) {
				// disableInternal is called when the client wants to totally shutdown NFC
				// so we just deselect all configures related to SE
				// we do this after applyRouting because() the card mode may be on now
				mDeviceHost.selectUiccSecureElement(DeviceHost.DESELECT_UICC);
				mDeviceHost.selectUiccSecureElementInPBTF(DeviceHost.DESELECT_UICC);
			}			
			mNfcStatusBar.updateStatusBar(NfcService.this, false, mIsNdefPushEnabled, checkScreenState());
            /// @}
		...
		void executeEeWipe() {
			Log.d(TAG, "NFC related. executeEeWipe() is disabled by MediaTek.");
			return;
		}

        void executeEeWipeOriginal() {
    ...
    final class NfcAdapterService extends INfcAdapter.Stub {
        ...
        /// M: modified @{
		@Override
        public int getCardEmulationConfig() {
			synchronized(NfcService.this) {
				if (mCardEmuController.isDefault() ||
                    !mCardEmuController.isEnabled()) {
					return NfcAddon.INfcModeController.DISABLED;
                }
				return mCardEmuController.getEnabledWhen();
			}
        }

        @Override
        public boolean setCardEmulationConfig(int config) {
			synchronized(NfcService.this) {
                if (config == NfcAddon.INfcModeController.DISABLED) {
				    return mCardEmuController.disable(mPrefsEditor);
                } else {
                    if (config > NfcAddon.INfcModeController.ENABLED_WHEN_SCREEN_UNLOCK ||
                        config < NfcAddon.INfcModeController.DISABLED) {
                        return false;
                    }
                    if (!mCardEmuController.enable(mPrefsEditor)) {
                        return false;
                    }
                    if (!mCardEmuController.setEnabledWhen(mPrefsEditor, config)) {
                        return false;
                    }
                    return true;
                }
		...
		@Override
		public boolean isP2pRecvEnabled() {
			synchronized(NfcService.this) {
				return mP2pRecvEnabler.isEnabled();
			}			
		}

		@Override
		public boolean enableP2pRecv() {
			synchronized(NfcService.this) {
				return mP2pRecvEnabler.enable(mPrefsEditor);
			}			
		}

		@Override
		public boolean disableP2pRecv() {
			synchronized(NfcService.this) {
				return mP2pRecvEnabler.disable(mPrefsEditor);
			}			
		}
		...
		public boolean isTagRwEnabled() {
			synchronized(NfcService.this) {
				return mTagRwEnabler.isEnabled();
			}			
		}
		
		@Override
		public boolean enableTagRw() {
			synchronized(NfcService.this) {
				return mTagRwEnabler.enable(mPrefsEditor);
			}			
		}

		@Override
		public boolean disableTagRw() {
			synchronized(NfcService.this) {
				return mTagRwEnabler.disable(mPrefsEditor);
			}			
		}

		@Override
		public boolean isAirplaneModeAutoSwitchEnabled() {
			synchronized(NfcService.this) {
				return mAirplaneModeAutoSwitchController.isEnabled();
			}			
		}		
		
		@Override
		public boolean enableAirplaneModeAutoSwitch() {
			synchronized(NfcService.this) {
				return mAirplaneModeAutoSwitchController.enable(mPrefsEditor);
			}			
		}

		@Override
		public boolean disableAirplaneModeAutoSwitch() {
			synchronized(NfcService.this) {
				return mAirplaneModeAutoSwitchController.disable(mPrefsEditor);
			}			
		}

		@Override
		public boolean isP2pBitrate106Enabled() {
			synchronized(NfcService.this) {
				return mP2pBitrate106Enabler.isEnabled();
			}			
		}

		@Override
		public boolean enableP2pBitrate106() {
			synchronized(NfcService.this) {
				return mP2pBitrate106Enabler.enable(mPrefsEditor);
			}
		}

		@Override
		public boolean disableP2pBitrate106() {
			synchronized(NfcService.this) {
				return mP2pBitrate106Enabler.disable(mPrefsEditor);
			}
		}
		...
		public boolean isVibrationEnabled() {
			synchronized(NfcService.this) {
				return mVibrationEnabler.isEnabled();
			}		
		}
		...
		public boolean enableVibration() {
			synchronized(NfcService.this) {
				return mVibrationEnabler.enable(mPrefsEditor);
			}
		}
		...
		public boolean disableVibration() {
			synchronized(NfcService.this) {
				return mVibrationEnabler.disable(mPrefsEditor);
			}
		}
		...
		public boolean isSoundEnabled() {
			synchronized(NfcService.this) {
				return mSoundEnabler.isEnabled();
			}		
		}
		...
		public boolean enableSound() {
			synchronized(NfcService.this) {
				return mSoundEnabler.enable(mPrefsEditor);
			}
		}
		...
		public boolean disableSound() {
			synchronized(NfcService.this) {
				return mSoundEnabler.disable(mPrefsEditor);
			}
		}
		...
		public boolean isPromptEnabled() {
			synchronized(NfcService.this) {
				return mNdefSignatureParser.isPromptEnabled();
			}
		}
		...
		public boolean enablePrompt() {
			synchronized(NfcService.this) {
				return mNdefSignatureParser.enablePrompt(mPrefsEditor);
			}
		}
		...
		public boolean disablePrompt() {
			synchronized(NfcService.this) {
				return mNdefSignatureParser.disablePrompt(mPrefsEditor);
			}
		}
        /// @}
        ...
        public boolean enableNdefPush() throws RemoteException {
            ...
            synchronized(NfcService.this) {
                ...
                if (isNfcEnabled()) {
                    /// M: modified
                    mP2pLinkManager.enableDisable(true, mP2pRecvEnabler.isEnabled());
                }
                /// M: modified
				mNfcStatusBar.updateStatusBar(NfcService.this, isNfcEnabled(), mIsNdefPushEnabled, checkScreenState());
            }
        ...
        public boolean disableNdefPush() throws RemoteException {
            ...
            synchronized(NfcService.this) {
                ...
                if (isNfcEnabled()) {
                    /// M: modified
                    mP2pLinkManager.enableDisable(false, mP2pRecvEnabler.isEnabled());
                }
                /// M: modified
				mNfcStatusBar.updateStatusBar(NfcService.this, isNfcEnabled(), mIsNdefPushEnabled, checkScreenState());
            }
        ...
        public INfcAdapterExtras getNfcAdapterExtrasInterface(String pkg) {
			Log.d(TAG, "NFC related. getNfcAdapterExtrasInterface() is disabled by MediaTek.");
			return null;
		}

        public INfcAdapterExtras getNfcAdapterExtrasInterfaceOriginal(String pkg) {
    ...
    void applyRouting(boolean force) {
        synchronized (this) {
            ...
            if (PN544_QUIRK_DISCONNECT_BEFORE_RECONFIGURE && mScreenState == SCREEN_STATE_OFF) {
				...
				mNfcStatusBar.updateStatusBar(this, true, mIsNdefPushEnabled, mScreenState);
                /// @}
                return;
            }
			...
			if (mCardEmuController.isDefault()) {
				...
				if (mCardEmuController.isEnabled() && (mScreenState >= mCardEmuController.getEnabledWhen())) {
					Log.d(TAG, "NfcAddon card mode on");
					mDeviceHost.selectUiccSecureElement(DeviceHost.SELECT_UICC);// how about control the state inside the JNI layer!?
				} else {
					Log.d(TAG, "NfcAddon card mode is not enabled or screen state not allowed, deselect UICC");
					mDeviceHost.selectUiccSecureElement(DeviceHost.DESELECT_UICC);
				}				
			}	
			mNfcStatusBar.updateStatusBar(this, true, mIsNdefPushEnabled, mScreenState);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_NDEF_TAG:
                    ...
                    /// M: modified @{
					if (mNdefSignatureParser.isParsing()) {//<-- return false for no add-on case
						Log.d(TAG, "bypass MSG_NDEF_TAG because previous one is still under signature parsing");
						break;
					}
                    /// @}
                    ...
                    /// M: modified @{
					if (!mTagRwEnabler.isEnabled()) {
						Log.d(TAG, "[op customize] tag read/write is disabled, need to disconnect the tag!?");
						tag.disconnect();
						break;
						}
                    /// @}
                    ...
                    if (ndefMsgs != null) {
					    /// M: modified @{ 
                        Log.d(TAG, "doStartPresenceChecking()");
                        tag.startPresenceChecking();
						if (mNdefSignatureParser.parse(tag, ndefMsgs, mHandler, mContext, 3000))//<-- return false for no add-on case 
							break;//<-- true means we can parse ndef signature, so do the dispatch after the async parsing is finished
                        /// @}
                ...
                /// M: modified @{
				case NfcAddon.INdefSignatureParser.MSG_NDEF_DISPATCH_AFTER_PARSING_SIGNATURE:
					{
					TagEndpoint tagToDispatch = (TagEndpoint) mNdefSignatureParser.getCachedTag();
					NdefMessage[] ndefMsgsToDispatch = mNdefSignatureParser.getCachedNdefMessages();
					mNdefSignatureParser.clearCachedObjects();
					//tagToDispatch.startPresenceChecking();
					dispatchTagEndpoint(tagToDispatch, ndefMsgsToDispatch);					
					break;
					}

				case NfcAddon.INdefSignatureParser.MSG_NDEF_PROMPT_AFTER_PARSING_SIGNATURE:
					mNdefSignatureParser.prompt(3000);
					break;					

				case NfcAddon.INdefSignatureParser.MSG_NDEF_DISCONNECT_AFTER_PROMPT:
					{
					TagEndpoint tagToDisconnect = (TagEndpoint) mNdefSignatureParser.getCachedTag();
					mNdefSignatureParser.clearCachedObjects();
					//tagToDisconnect.disconnect();
					break;
					}
                /// @}
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(
                ...
                /// M: modified @{
				synchronized (NfcService.this) {
					if (mAirplaneModeAutoSwitchController.isEnabled()) {
						// ----- Google begins ----- //
                        Log.d(TAG, "AirplanModeOn = " + isAirplaneModeOn);
                        Log.d(TAG, "mPrefs.getBoolean(PREF_NFC_ON, NFC_ON_DEFAULT) = " + mPrefs.getBoolean(PREF_NFC_ON, NFC_ON_DEFAULT));
		                if (isAirplaneModeOn) {
		                    new EnableDisableTask().execute(TASK_DISABLE);
