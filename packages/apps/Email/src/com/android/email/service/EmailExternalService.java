...
//    private static final String EXTRA_RESULT = "com.android.email.extra.RESULT";
    ...
    private ControllerCallbacks mControllerCallback = null;
    ...
    private void handleUpdateMail(Intent intent) {
        ...
        if (accountIdArray == null || accountIdArray.length == 0) {
            Logging.d(TAG, "UpdateInbox-->extra accout array is null, needCallback:" + needCallback);
            if (needCallback) {
    ...
    private boolean saveMessage(com.android.emailcommon.mail.Message mimeMessage, long accountId,
        ...
        */
        ...
        saveOrUpdate(body);
        Logging.d(TAG, "Update Message to local database end. accountId = " + accountId + " mailboxId = " + mailboxId );
        Logging.d(TAG, "MessageInfor " + (null ==localMessage? "NUll": localMessage.mId));
        // process (and save) attachments
        if (hasAttachments) {
            try {
                LegacyConversions.updateAttachments(this, localMessage, attachments);
    ...
    private void handleSendCallback(int result, long accountId, int resultType) {
        /**
         * Only send success and need to save, do save operation.
         * TODO: Only save message for SEND result and the DELIVER result need be check. if not this
         * message may be saved twice.
         */
        if (mSavetoSent && (EmailExternalConstants.RESULT_SUCCESS == result)
            ...
            Logging.d(TAG, "Not send message success or not need save . result = " + result + " mSavetoSent = " +mSavetoSent );
        }
        //Send result
        Logging.d(TAG, "*****Handler sendCallback: result = " + result + ", account = " + accountId + ",resultType = " + resultType);
    ...
    public void sendResult(String action, long accountId, int result) {
        Logging.d(TAG, ">>> sendResult accountID = " + accountId + " result = " + result + "action = " + action);
