...
public class BluetoothTethering extends INetworkManagementEventObserver.Stub{
  	...
  	private Context mContext;
    	private final String TAG = "BluetoothTethering";

	private static BluetoothTethering mBTtethering=null;

	private INetworkManagementService mNetworkManagementService;

	private String[] mTetherableBTRegexs;

	private static boolean  BtTetheringRegistered = false;
	...
	public static BluetoothTethering getBluetoothTetheringInstance(){
		if(mBTtethering == null){
			mBTtethering = new BluetoothTethering();
			return mBTtethering;
		}
		else {
			return mBTtethering;
	...
	public void interfaceAdded(String iface) {
		Log.d(TAG, "interfaceAdded, iface: " + iface);	
	        if (!isBT(iface)) {
	          	 //Xlog.d(TAG, iface + " is not a BT tetherable iface, ignoring");
	           	 return;
	        }
