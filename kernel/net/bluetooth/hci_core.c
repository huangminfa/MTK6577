...
#ifdef RDA_BT_SUPPORT
        {
                #define HCI_OP_WRITE_LOCAL_ADDRESS 0xfc1a
                __u8 address[] = {0x00, 0x00, 0x00, 0x00, 0x90,0x59};
                //printk("hci_init_req read address\n");
                if(rda5890_read_bt_addr(address) != 6)
                {
                        get_random_bytes(address, 4);
                        rda5890_write_bt_addr(address);
                }
                hci_send_cmd(hdev, HCI_OP_WRITE_LOCAL_ADDRESS, sizeof(address), address);
