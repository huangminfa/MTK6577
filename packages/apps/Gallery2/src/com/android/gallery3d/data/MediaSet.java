...
public abstract class MediaSet extends MediaObject {
    ...
    protected int enumerateMediaItems(ItemConsumer consumer, int startIndex) {
        int total = getMediaItemCount();
        int start = 0;
        // check to see if current code is in a Job and Job is cancelled
        final Thread t = Thread.currentThread();
        final CancelableThread ct = (t != null && t instanceof CancelableThread) ? (CancelableThread)t : null;
        
        while (start < total) {
            int count = Math.min(MEDIAITEM_BATCH_FETCH_COUNT, total - start);
            ArrayList<MediaItem> items = getMediaItem(start, count);
            for (int i = 0, n = items.size(); i < n; i++) {
                if (ct != null && ct.isThreadCancelled()) {
                    Log.w(TAG, "enum items: cancelled and return: enumed count=" + (start + i + 1));
                    return start + i;
                }
