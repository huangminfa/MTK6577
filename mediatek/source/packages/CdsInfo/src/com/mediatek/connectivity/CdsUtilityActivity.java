...
public class CdsUtilityActivity extends Activity implements View.OnClickListener {
    private final String TAG = "CdsUtilityActivity";

    private final String HTTPSTRING = "http://";
    private final String PINGSTRING = "ping -c 5 www.google.com";
    private final String PINGV6STRING = "ping6 -c 5 www.google.com";
    ...
    public void onCreate(Bundle icicle) {
        ...
        cmdLineList = (AutoCompleteTextView) findViewById(R.id.cmdLine);
        ...
        spinner.setOnItemSelectedListener(new Spinner.OnItemSelectedListener() {
            ...
            int position, long arg3) {
                ...
                if(position == PING) {
                    cmdLineList.setText(PINGSTRING);
                } else if(position == PINGV6) {
                    cmdLineList.setText(PINGV6STRING);
                }else if(position == HTTPRESPONSNE) {
                    cmdLineList.setText(HTTPSTRING);
                } else if(position == RUN) {
                    cmdLineList.setText("");
                } else {
                    cmdLineList.setText("www.google.com");
                }
                mCmdOption = position;
                cmdLineList.requestFocus();
                cmdLineList.setSelection(cmdLineList.getText().length());
            }
            @Override
    ...
    private void handleRunCmd() {
        ...
        String cmdStr = cmdLineList.getText().toString();
        Xlog.d(TAG, "" + cmdStr);
        ...
        if(mCmdOption == PING || mCmdOption == RUN || mCmdOption == PINGV6) {
            ...
            new Thread( new Runnable() {
                public void run() {
                    ...
                    try {
                        String cmdLineStr = cmdLineList.getText().toString();
