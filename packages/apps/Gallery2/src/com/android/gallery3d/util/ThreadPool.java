...
import com.android.gallery3d.util.PriorityThreadFactory.CancelableThread;
...
public class ThreadPool {
    ...
    ResourceCounter mCpuCounter = new ResourceCounter(MediatekFeature.getCpuCoreNum());
    ...
    public ThreadPool() {
        mExecutor = new ThreadPoolExecutor(
                ...
                //we change the thread priority to less favorable to let
    ...
    public <T> Future<T> submit(Job<T> job, FutureListener<T> listener) {
        ...
        mExecutor.execute(w);
        if (MtkLog.DBG) {
            MtkLog.d(TAG, "Job [" + job + "] submitted to threadpool!");
        }
        return w;
    }
    ...
    private class Worker<T> implements Runnable, Future<T>, JobContext {
        ...
        public void run() {
            ...
            synchronized(this) {
                // put this in sychronized block to avoid exception caused by concurrent access
                if (mExecThread != null && (mExecThread instanceof CancelableThread)) {
            ...
            if (MtkLog.DBG) {
                MtkLog.d(TAG, "[" + mJob + "].run: job is DONE");
            }
        }
        ...
        public synchronized void cancel() {
            ...
            if (MtkLog.DBG) {
                MtkLog.w(TAG, "[" + mJob + "] is cancelled!");
            }
        }
