...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
    ...
    public final static String DEFAULT_DOWNLOAD_DIRECTORY_OP01 = "/mnt/sdcard/MyFavorite";
    public final static String DEFAULT_DOWNLOAD_SDCARD2_DIRECTORY_OP01= "/mnt/sdcard2/MyFavorite";
    public final static String DEFAULT_MY_FAVORITE_FOLDER_OP01 = "/MyFavorite";
    ...
    private void syncSetting(WebSettings settings) {
        ...
        if (optr != null && optr.equals("OP01")) {
            settings.setStandardFontFamily(getFontFamily());
        }
        ...
        if (ua != null) {
            ...
            // M: Add this to support operator UA string
            ...
            if (optr != null && optr.equals("OP01")) {
                if (Build.MODEL.contains("v3")) {
                    ...
                    model = "athens15/v2";
                    strTime = new SimpleDateFormat("MM.dd.yyyy").format(date);
                } else if (Build.MODEL.contains("6517")&& Build.MODEL.contains("v2")) {
                    model = "mt6517/v2";
                    strTime = new SimpleDateFormat("MM.dd.yyyy").format(date);
                } else if (Build.MODEL.contains("6517")) {
                    model = "mt6517/v1";
                    strTime = new SimpleDateFormat("MM.dd.yyyy").format(date);
                ...
                String op01UA = model + " Linux/3.0.13 Android/" + 
                ...
                Xlog.v(XLOGTAG, "OP01 UA string is: " + op01UA + "Build.Model is: " + Build.MODEL);
                
                if (getUserAgent() == 0) {
                    settings.setUserAgentString("Athens15_TD/V2 Linux/3.0.13 Android/4.0 Release/02.15.2012 " +
            ...
            if (optr != null && optr.equals("OP02")) {
                if (Build.MODEL.contains("v3")) {          
                    ...
                    model = "lenovo75/v1";
                    strTime = new SimpleDateFormat("MM.dd.yyyy").format(date);
                } else if (Build.MODEL.contains("6577")&& Build.MODEL.contains("v2")) {
                    model = "mt6577/v2";
                    strTime = new SimpleDateFormat("MM.dd.yyyy").format(date);
                } else if (Build.MODEL.contains("6577")) {
                    model = "mt6577/v1";
                    strTime = new SimpleDateFormat("MM.dd.yyyy").format(date);
                } else {
                    model = Build.MODEL;
                ...
                String op02UA = model + " Linux/3.0.13 Android/" + 
                        ...
                        " Browser/AppleWebKit534.30 Profile/MIDP-2.0 Configuration/CLDC-1.1" +
                        " Mobile Safari/534.30" + " Android 4.0.1;";
                Xlog.v(XLOGTAG, "OP02 UA string is: " + op02UA + "Build.Model is: " + Build.MODEL);
                
                if (getUserAgent() == 0) {
                    settings.setUserAgentString(op02UA);
                } else {
                    settings.setUserAgentString(USER_AGENTS[getUserAgent()]);
                }
            } else
            ...
            if (optr != null && optr.equals("OP03")) {
                ...
                if (getUserAgent() == 0) {
                    settings.setUserAgentString(op03UA);
                    Xlog.v(XLOGTAG, "OP03 UA string is: " + op03UA);
                } else {
                    settings.setUserAgentString(USER_AGENTS[getUserAgent()]);
                }
            }
	...
	//M+
	public void clearManagedSettings(WebSettings settings){
		Iterator<WeakReference<WebSettings>> iter = mManagedSettings.iterator();
        while (iter.hasNext()) {
            WeakReference<WebSettings> ref = iter.next();
            WebSettings webSettings = ref.get();
    ...
    public void onSharedPreferenceChanged(
        ...
        // M: add for OP01 landscape browser mode.
        	...
        	if (mController.getActivity() != null) {
                ...
                if (optr !=null && optr.equals("OP01")) {
	            	Xlog.d(XLOGTAG, "Activity.setRequestedOrientation()");
	        		mController.getActivity().setRequestedOrientation(
	        				sharedPreferences.getBoolean(key, false) ? ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE
	                        : ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED);
                }
        	}
        }
        // M: end add for OP01 landscape browser mode.
    }
    ...
    private void updateSearchEngine(boolean force) {
        ...
        if (force || mSearchEngine == null ||
            ...
            if (mSearchEngine != null) {
                if (mSearchEngine.supportsVoiceSearch() && mController != null && mController.getTabControl() != null) {
    ...
    //M: Don't need set function because set is done in AdvancedPreferencesFragment onActivityResult
    ...
    public String getDefaultDownloadPathWithMultiSDcard() {
        ...
        String defaultStorage = mStorageManager.getDefaultPath();
