...
public class CalendarAppWidgetService extends RemoteViewsService {
    ...
    public static class CalendarFactory extends BroadcastReceiver implements
        ...
        ///M:Use Temp Model to save the model that build in onLoadCompleter().It will be assigned 
        ///to mModel in onDataSetChanged()@{
        private static CalendarAppWidgetModel sTempModel;
        ///@}
        ...
        public RemoteViews getViewAt(int position) {
            ...
            if (position < 0 || position >= getCount()) {
                LogUtil.w(TAG,
                        "getViewAt,position is not right,return null.position:"
            ...
            if (mModel == null) {
                RemoteViews views = new RemoteViews(mContext.getPackageName(),
                        R.layout.appwidget_loading);
                ///M:set the start & end time -1,it is invalid time,so it will be set as the current
                ///system time in AllInOneActivity.@{
				final Intent intent = CalendarAppWidgetProvider
						.getLaunchFillInIntent(mContext, 0, -1, -1);
                ///@}
        ...
        public int getViewTypeCount() {
            ///M: the possible number of view types is 5 
            return 5;
        }
        ...
        public long getItemId(int position) {
            ///M:@{
            if(position < 0 || position >= getCount()){
                LogUtil.w(TAG, "getItemId(),position is not right,return 0,position:"
                        + position + ",getCount():" + getCount());
                return 0;
            }
            ///@}
            ...
            if (mModel == null ||  mModel.mRowInfos.isEmpty()) {
                LogUtil.w(TAG, "getItemId(),mModel is Null or Empty,return 0.mModel:"
                        + mModel);
                return 0;
            }
            RowInfo rowInfo = mModel.mRowInfos.get(position);
        ...
        public void onLoadComplete(Loader<Cursor> loader, Cursor cursor) {
            ...
            synchronized (mLock) {
                ...
                ///M:Save the build Model in mTempModel@{
                sTempModel = buildAppWidgetModel(mContext, mCursor, tz);
                ///@}
                ...
                // against extras.
                long triggerTime = calculateUpdateTime(sTempModel, now, tz);
