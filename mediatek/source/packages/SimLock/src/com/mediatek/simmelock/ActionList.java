...
import com.mediatek.featureoption.FeatureOption;//To find whether the project support Gemini
...
public class ActionList extends PreferenceActivity 
{
	...
	protected void onResume() {
      ...
      if(false == FeatureOption.MTK_GEMINI_SUPPORT)//if it single SIM, no need to pass the card number
      {
      	...
      	phone.getIccCard().QueryIccNetworkLock(lockCategory,4,null,null,null,null,callback);
      }
      else//Gemini SIM: put the SIM  number to the bundle
      {
    	Bundle bundle = new Bundle();
      	...
      	if (0 == intSIMNumber)
      	{
          	...
          	Message callback = Message.obtain(mHandler, QUERY_ICC_SML_COMPLETE);
          	mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_1).QueryIccNetworkLock(lockCategory,4,null,null,null,null,callback);        		
      	}
      	if (1 == intSIMNumber)
      	{
          	//whether some lock category is disabled?
          	Message callback = Message.obtain(mHandler, QUERY_ICC_SML_COMPLETE);
          	mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_2).QueryIccNetworkLock(lockCategory,4,null,null,null,null,callback);        		
      	}
      }
    ...
    private void getUnlockEnableState(int category){
    	...
    	for(int tempCate = 0;tempCate < category;tempCate++){
    		Log.i("SIMMELOCK","[getUnlockEnableState] Queried lock category is "+tempCate+" || unlock_enable is "+unlock_enable);
    		if(unlock_enable == false)
    			break;
    		...
    		{
    		   	Phone phone = PhoneFactory.getDefaultPhone();
    		   	phone.getIccCard().QueryIccNetworkLock(tempCate,4,null,null,null,null,unlockCallBack);
    		}
    		else//Gemini SIM: put the SIM  number to the bundle
    		{
    		   	GeminiPhone mGeminiPhone = (GeminiPhone)PhoneFactory.getDefaultPhone();
    		   	if (0 == intSIMNumber)
    		   	{
    		       	...
    		       	mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_1).QueryIccNetworkLock(tempCate,4,null,null,null,null,unlockCallBack);        		
    		   	}
    		   	if (1 == intSIMNumber)
    		   	{
    		       	//whether some lock category is disabled?
    		       	Message callback = Message.obtain(mHandler, QUERY_ICC_SML_LOCK_STATE);
    		       	mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_2).QueryIccNetworkLock(tempCate,4,null,null,null,null,unlockCallBack);        		
    		   	}
    		}
