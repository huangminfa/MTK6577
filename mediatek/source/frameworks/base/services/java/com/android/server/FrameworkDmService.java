...
import com.android.server.MountService.VolumeState;
...
public class FrameworkDmService {
	...
	BroadcastReceiver dmReceiver = null;
    ...
    public FrameworkDmService(Context context, AlarmManagerService ams, StatusBarManagerService sbs, NotificationManagerService nms) {
    	mAMS = ams;
    	mSBS = sbs;
    	mNMS = nms;
    	mContext = context;
	...
	public int dmEnable (boolean enable){
		...
		if (!enable) {
			int net = 0;
			net = StatusBarManager.DISABLE_EXPAND | StatusBarManager.DISABLE_NOTIFICATION_ALERTS | StatusBarManager.DISABLE_NOTIFICATION_ICONS | StatusBarManager.DISABLE_NOTIFICATION_TICKER;
