...
#ifdef ENABLE_LOG_AUDIO_MIXER
    #define LOG_AUDIO_MIXER LOGD
#else
    #define LOG_AUDIO_MIXER LOGV
#endif
...
{
    LOG_AUDIO_MIXER("AudioFlinger::MixerThread:  constructor mSampleRate = %d",mSampleRate);
 ...
 void AudioMixer::deleteTrackName(int name)
 {
    ...
    if (uint32_t(name) < MAX_NUM_TRACKS) {
        LOG_AUDIO_MIXER("deleteTrackName(%d)", name);
    ...
    switch (name) {
        case MIXING: {
            if (mState.tracks[ mActiveTrack ].enabled != 1) {
                ...
                LOG_AUDIO_MIXER("enable(%d)", mActiveTrack);
    ...
    switch (name) {
        case MIXING: {
            if (mState.tracks[ mActiveTrack ].enabled != 0) {
                ...
                LOG_AUDIO_MIXER("disable(%d)", mActiveTrack);
                invalidateState(1<<mActiveTrack);
            }
        } break;
        default:
    ...
    case RESAMPLE:
        ...
        if (name == RESET) {
            ...
            if(track.resampler != NULL){
				track.resampler->setOutputSampleRate(mSampleRate);
            }
#endif
            invalidateState(1<<mActiveTrack);
            return NO_ERROR;
...
#ifndef ANDROID_DEFAULT_CODE
        // add by chipeng for resampler
        if(name == MIXER_SAMPLE_RATE){
            LOG_AUDIO_MIXER("AudioMixer::setParamete name == MIXER_SAMPLE_RATE value = %d",*valueBuf);
            if (*valueBuf > 0) {
                track_t& track = mState.tracks[ mActiveTrack ];
                this->mSampleRate = *valueBuf;
                track.setResamplerRate(uint32_t(*valueBuf));
	        track_t* t = mState.tracks;
	        for (int i=0 ; i<32 ; i++) {
	            t->sampleRate = *valueBuf;
	            t++;
	        }
	        return NO_ERROR;
	    }
...
{
    //LOG_AUDIO_MIXER("AudioMixer::track_t::setResampler sampleRate = %d value = %d devSampleRate =%d",sampleRate,value,devSampleRate);
    if (value!=devSampleRate || resampler) {
        if (sampleRate != value) {
            sampleRate = value;
#ifndef ANDROID_DEFAULT_CODE
            if(resampler != NULL && resampler->mSampleRate != devSampleRate){
                delete resampler;
                resampler = NULL;
            }
 #endif
            if (resampler == 0) {
#ifndef ANROID_DEFAULT_CODE
                int quality =AudioResampler::DEFAULT;
                ...
                resampler = AudioResampler::create(   // here create resampler
                        format, channelCount, devSampleRate,quality);
#else
...
#ifndef ANDROID_DEFAULT_CODE
bool AudioMixer::track_t::setResamplerRate(uint32_t devSampleRate)
{
    LOG_AUDIO_MIXER("AudioMixer::track_t::setResamplerRate devSampleRate = %d",devSampleRate);
    if(resampler != NULL){
 	delete resampler;
	resampler = NULL;
    }
...
void AudioMixer::process__validate(state_t* state)
{
    ...
    LOG_AUDIO_MIXER("mixer configuration change: %d activeTracks (%08x) "
...
{
    LOG_AUDIO_MIXER("AudioMixer::process__genericNoResampling");
