...
public class PlainSocketImpl extends SocketImpl {
    ...
    private int read(byte[] buffer, int offset, int byteCount) throws IOException {
        ...
        if (shutdownInput) {
            System.out.println("[CDS]shutdownInput");
            return -1;
        }
        ...
        if (readCount == 0) {
            System.out.println("[CDS]read is 0");
        ...
        if (streaming) {
            while (byteCount > 0) {
                ...
                if(bytesWritten == 0){//Add by MTK03594 for ALPS00076216
                   System.out.println("[CDS]write is:" + bytesWritten + "/" + byteCount);
                   throw new SocketTimeoutException("write timeout");
                } else if (bytesWritten != byteCount) {//Add by MTK04247 for ALPS00239330
                   System.out.println("[CDS]write is:" + bytesWritten + "/" + byteCount);
