...
static void log_print(int level, char *format, ...)
{
    if (level >= 0 && level <= LOG_MAX) {
        ...
        va_list ap;
        va_start(ap, format);
        __android_log_vprint(levels[level], "mtk_route", format, ap);
        va_end(ap);
    }
}
...
void  bb_displayroutes(int noresolve, int netstatfmt)
{
	...
	log_print(INFO, "Kernel IP routing table");	
	log_print(INFO,  "Destination     Gateway         Genmask         Flags %s Iface",
			netstatfmt ? "  MSS Window  irtt" : "Metric Ref    Use");
 ...
 ERROR:
		...
		if (netstatfmt) {
			printf("%-15.15s %-15.15s %-16s%-6s%5d %-5d %6d %s\n", 
				sdest, sgw, inet_ntoa(mask), flags, mtu, win, ir, devname);
			log_print(INFO,"%-15.15s %-15.15s %-16s%-6s%5d %-5d %6d %s", 
				sdest, sgw, inet_ntoa(mask), flags, mtu, win, ir, devname);
		} else {
			printf("%-15.15s %-15.15s %-16s%-6s%5d %-5d %6d %s\n", 
				sdest, sgw, inet_ntoa(mask), flags, mtu, win, ir, devname);
			log_print(INFO,"%-15.15s %-15.15s %-16s%-6s%-6d %-2d %7d %s", 
				sdest, sgw, inet_ntoa(mask), flags, metric, ref, use, devname);
		}
		free(sdest);
		free(sgw);
	}
