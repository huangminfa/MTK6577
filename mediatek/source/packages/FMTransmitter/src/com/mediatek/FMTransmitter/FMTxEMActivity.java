...
public class FMTxEMActivity extends Activity implements OnClickListener{
    ...
    private static final String gsPreferenceName = "userData";
    /**
    ...
    ProgressDialog mSearchDialog = null;
        ...
        /**
         * record target frequency, each time user click on the "power up", this frequency should be used to power tx
         * when system clicked one frequency in the listView,  targetFrequency should be updated
        ...
        private static int CHANNELS_TO_SEARCH = 0;
         ...
         */
        private boolean gbIsPlaying = false;
        ...
        public void onCreate(Bundle savedInstanceState) {
            ...
            //thisActivity = this; 
            SharedPreferences preferences = getSharedPreferences(gsPreferenceName, MODE_PRIVATE);
            ...
            */
            ...
            mMainHandler = new Handler()
            {
                ...
                public void handleMessage(Message msg)
                {
                    ...
                    case SEARCH_IN_PROCESS:
                        ...
                        /*
                        mSearchDialog.setOnCancelListener(
                            new DialogInterface.OnCancelListener() {
                                
                                //@Override
                                public void onCancel(DialogInterface dialog) {
                                    // TODO Auto-generated method stub
                                    FMTxLogUtils.e(TAG, "search cancelled.");
                                    if(null != mSearchThreadHandler )
                                    {
                                        mSearchThreadHandler.sendEmptyMessage(SEARCH_INTERRUPTED);
                                        try {
                                            //mFMTxService.stopSearchProcess();
                                        } catch (RemoteException e) {
                                            // TODO Auto-generated catch block
                                            e.printStackTrace();
                                            FMTxLogUtils.e(TAG, "service call stopSearchProcess() failed.");
                                        }
                                    }
                                    mSearchDialog = null;
                                }
            ...
            /*test code
             * mChannelsAdapter.add("88.9");
             * mChannelsAdapter.add("107.5");
             * mChannelsAdapter.add("107.8");
             */
            ...
            mSpinnerList.setOnItemSelectedListener(new OnItemSelectedListener(){
                ...
                public void onItemSelected(AdapterView<?> arg0, View arg1,
                    ...
                    {
                        FMTxLogUtils.e(TAG, "index = " + arg2 + " is not a valid index.");
                    }
                }
        ...
        private void enableAllView()
        {
            ...
            //mInputRdsText.setEnabled(true);//this should not be enabled here, this EditText can and should only be enabled when RDS is turned off
                ...
                case TXPOWERUP:
                    ...
                    if(getRDSTxStatus() == RDSStateEnum.RDSENABLED)
                    {
                        try {
                            ...
                            {
                                FMTxLogUtils.d(TAG, "service call - setRDSEnabled(false) return true.");
                            }
                            else
                            {
                                FMTxLogUtils.e(TAG, "service call - setRDSEnabled(false) return false.");
                            }
                        } catch (RemoteException e)
                        {
                            e.printStackTrace();
                            FMTxLogUtils.e(TAG, "service call - setRDSEnabled(false) failed.");
                        }
                        try {
                    ...
                    break;
                case TXCLOSED:
                    FMTxLogUtils.i(TAG, "FM Tx in close state.");
                    break;
                default:
                    break;
...
*/            super.onStop();
        ...
        protected void onDestroy ()
        {
                ...
                case TXPOWERUP:
                    ...
                    if(getRDSTxStatus() == RDSStateEnum.RDSENABLED)
                    {
                        try {
                            if(mFMTxService.setRDSTxEnabled(false))
                            {
                                FMTxLogUtils.v(TAG, "service call - setRDSEnabled(false) return true.");
                            }
                            else
                            {
                                FMTxLogUtils.e(TAG, "service call - setRDSEnabled(false) return false.");
            ...
            SharedPreferences preferences = getSharedPreferences(gsPreferenceName, MODE_WORLD_READABLE);
        ...
        private boolean bindFMTransmitterService() {
            ...
            {
                //Toast.makeText(this, "bindService call succeed", Toast.LENGTH_LONG).show();
                showIndication("bindService call succeed" , false);
                return true;
            }
            else
            {
                //Toast.makeText(this, "bindService call failed", Toast.LENGTH_LONG).show();
        ...
        public void onClick(View v) {
            // TODO Auto-generated method stub
            /*
             * int max = arrClickViews.length;
             * int objIndex = 0;
             * for(; objIndex < max; objIndex++)
             * {
             * if(v.equals(arrClickViews[objIndex])){
             * break;
             * }
             * }
            */
            ...
            /*
            else if(v.equals(gCheckFrequency))
            {
                //check frequency button is clicked
                handleCheckBtn();
            }
            */
        ...
        //check whether max channel is ok or not first
            ...
            {
                e.printStackTrace();
                FMTxLogUtils.e(TAG, "mFMTxService.sendPowerdownFMRxMsg expception in handleSearchBtn()");
            }
            ...
            {
                FMTxLogUtils.e(TAG, "search already in process.");
            }
            */
            ...
            int channels = new Integer (s); 
             if (channels > MAX_CHANNELS || channels < 1)
             {
                 //Toast.makeText(this, "invalid parameter", Toast.LENGTH_LONG).show();
                 showIndication("invalid parameter" + "\nmin channel numbers = 1" + "\nmax channel numbers = " + MAX_CHANNELS  , true);
             }
             else
             {
        ...
        //check whether is in the specified range of FM frequency first
            ...
            float fre = new Float(mInputFrequency.getText().toString());
            //FMTxLogUtils.d(TAG, "before plus 0.1 fre = " + fre);    
            fre = fre + SEARCH_GAP;
            fre = Math.round(fre * BASE_NUMBER);
            //FMTxLogUtils.d(TAG, "after  * 10 fre = " + fre);    
            fre = fre / BASE_NUMBER;
            //FMTxLogUtils.d(TAG, "after  plus 0.1 fre = " + fre);    
            //double x = new Float(fre).doubleValue(); 
            if(fre > MIN_FREQUENCY && fre < MAX_FREQUENCY )
            {
                FMTxLogUtils.v(TAG, "fre ok.");    
                mInputFrequency.setText(new Float(fre).toString());
                //mInputFrequency.setText(new Double(x).toString());
            }
            else
            {
        ...
        //check whether is in the specified range of FM frequency first
            ...
            {
                showIndication("Please input an Frequency between " + MIN_FREQUENCY + " and " + MAX_FREQUENCY, true);
                return;
            }
            /*
            try {
                
                    showIndication("" + frequency + " is " + 
                            (mFMTxService.isFreAvailable(frequency) ? "" : "not ")
                            + "available for FM Transmitter", true);
                            
            } catch (RemoteException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
                FMTxLogUtils.e(TAG, "service call FMTxService.isFreAvailable(xxx) failed.");
            }
            */
            //gCheckFrequency.setEnabled(true);
        }
        ...
        //check whether target mp3 file exist or not
            ...
            {
                //Toast.makeText(this, "Please insert an SD card", Toast.LENGTH_LONG).show();
                showIndication("Please insert an SD card", true);
                gbIsPlaying = false;
                return ;
            }
            */
            ...
            {
                //Toast.makeText(this, "input a valid music name", Toast.LENGTH_LONG).show();
            ...
            else
            {
                ...
                if(null == mMediaPlayer)
                {
                    ...
                    if (null == mMediaPlayer)
                    {
                        //Toast.makeText(this, "MediaPlayer creatation failed", Toast.LENGTH_LONG).show();
                ...
                try {
                    mMediaPlayer.setDataSource("/sdcard/" + mMp3Path.getText().toString());
                    mMediaPlayer.prepare();
                    mMediaPlayer.start();
                    //mMediaPlayer.setLooping(true);//here we make the player to play without stop 
                    gbIsPlaying = true;    
                    ...
                    {
                        FMTxLogUtils.e(TAG, "PlayerStateMonitorThread creation failed");
                    }                
                    ...
                    //Toast.makeText(this, "please make sure file /sdcard/"+ mMp3Path.getText().toString() + " exist", Toast.LENGTH_LONG).show();
                    showIndication("please make sure file /sdcard/"+ mMp3Path.getText().toString() + " exist" , true);
                } 
            }
            updatePlayerUI(false);
        } 
        ...
        //check whether target RDS text in the range of
            ...
            try {
                ...
                // TODO Auto-generated catch block
                e1.printStackTrace();
                FMTxLogUtils.e(TAG, "service call - mFMTxService.isRDSSupport() failed.");
            }
            ...
            case RDSENABLED:
                ...
                catch (RemoteException eEnable)
                {
                    ...
                    FMTxLogUtils.e(TAG, "service call - setRDSEnabled(true) failed.");
                }
                break;
            case RDSDISABLED:
                ...
                {
                    showIndication("invalid RDS text length, please input an ASCII string whose length is between 1 ~ 8" , false);
                    return;
                }
                ...
                for(int i = 0 ; i < e.length() ; i++)
                {
                    ...
                    //if(s[0] >= 0x20 && s[0] <= 0x7B)
                    if(!(ps[i] >= 0 && ps[i] <= 0x7F))
                    {
                        FMTxLogUtils.e(TAG, "error code " + new String(ps));
                        showIndication("invalid RDS text content, please input an ASCII string" , false);
                        return;
                    }
                }
                ...
                /*
                char[] s = new char[1];
                int[] rdsText = new int[4];
                for(int i = 0; i < rdsText.length; i++)
                {
                    rdsText[i] = 0; 
                }
                ...
                for(int i = 0 ; i < e.length() ; i++)
                {
                    e.getChars(i, i + 1, s, 0);
                    //if(s[0] >= 0x20 && s[0] <= 0x7B)
                    if(!(s[0] >= 0 && s[0] <= 0x7F))
                    {
                        FMTxLogUtils.e(TAG, "error code " + new String(s));
                        showIndication("invalid RDS text content, please input an ASCII string" , false);
                        return;
                    }
                    else
                    {
                        FMTxLogUtils.e(TAG, "" + new String(s));
                    }
                    rdsText[3 - i / 2] = (int) (rdsText[3 - i / 2] + (s[0] << (8 * ((i + 1) % 2))));
                }
                if(e.length() < 8)
                {
                    for(int i = e.length(); i < 8; i++)
                    {
                        rdsText[3 - i / 2] = (int) (rdsText[3 - i / 2] + (0x20 << (8 * ((i + 1) % 2))));
                    }
                }
                */
                try
                {
                    ...
                    //test code, make ps = 100, rds = rdsText, rdsCnt = 4
                    //
                    /*
                    int[] iRdsPs = new int[12];
                    iRdsPs[0] = 'm';
                    iRdsPs[1] = 't';
                    iRdsPs[2] = 'k';
                    iRdsPs[3] = ' ';
                    ...
                    iRdsPs[10] = 'o';
                    iRdsPs[11] = 'n';
                    mFMTxService.setRDSText(pi, iRdsPs, rdsText, 4);
                    */
                    PI += 1;
                    mFMTxService.setRDSText(PI, ps, null, 0);
                }
        ...
        //check current power on/off status first    
            ...
            serviceValidCheck();
            //first disable this button, to prevent multi-click on this button
            mButtonPowerOn.setEnabled(false);
            //disable other input menu for a moment
            //*************************************
            //********do necessary jobs to disable views to reduce user's event in this process
            //********pop up a ProgressDialog if necessary
            //*************************************
            ...
            case TXOPENED:
                ...
                try {
                    ...
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                    FMTxLogUtils.e(TAG, "service call - powerUpTx(xxx) failed.");
                }
            ...
            case TXPOWERUP:
                ...
                try {
                    ...
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                    FMTxLogUtils.e(TAG, "service call - powerDownTx() failed.");
                }
                break;
            case TXCLOSED:
                earphone = false;
                ...
                try {
                    ...
                    {
                        FMTxLogUtils.e(TAG, "mServiceConnection - Tx is not supported in this device");
                        mFMTxService.closeTxDevice();
                    }
                    else
                    {
                        FMTxLogUtils.d(TAG, "mServiceConnection - Tx is supported in this device");
                    }
                        mFMTxService.powerUpTx(mTargetFrequency );
                    }
                    else
                    {
                        ...
                        FMTxLogUtils.e(TAG, "openTxDevice(xxx) return false.");
                    }
                } catch (RemoteException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                    FMTxLogUtils.e(TAG, "service call - openTxDevice()/powerUpTx(xxx) failed.");
                }
        ...
        public TxDeviceStateEnum getTxStatus()
        {
          serviceValidCheck();
            try {
                ...
                // TODO Auto-generated catch block
                e.printStackTrace();
                FMTxLogUtils.e(TAG, "service call - isTxPowerUp()failed.");
            }
            return TxDeviceStateEnum.TXCLOSED;
        }
        ...
        public RDSStateEnum getRDSTxStatus()
        {
             ...
             {
                 e.printStackTrace();
                 FMTxLogUtils.e(TAG, "service call - FMTxService.isRDSOn()failed.");
             }
        ...
        private boolean setTarFrequency(float frequency)
        {
            if(frequency >= MIN_FREQUENCY && frequency <= MAX_FREQUENCY )
            {
                ...
                if (mTargetFrequency != frequency)
                {
                    ...
                    if(getTxStatus() == TxDeviceStateEnum.TXPOWERUP){
                        ...
                        try {
                            ...
                            FMTxLogUtils.e(TAG, "service call - repowerTx(xxx)failed.");
                        }
                ...
                {
                    FMTxLogUtils.i(TAG, "Tx frequency has already been set to " + frequency + " MHz");
                    //showIndication("Tx frequency has already been set to " + frequency + " MHz", true);
                }
            ...
            {
                //Toast.makeText(this, "Please input an Frequency between " + MIN_FREQUENCY + " and " + MAX_FREQUENCY, Toast.LENGTH_LONG).show();
                showIndication("Please input an Frequency between " + MIN_FREQUENCY + " and " + MAX_FREQUENCY, true);
                FMTxLogUtils.e(TAG, "target frequency " + frequency + "out of range supported.");
                return false;
            }
        ...
        private void turnTogfTargetFrequency()
        {
            ...
            try {
                if(mFMTxService.isTxPowerUp())
                {
                    ...
                    {
                        FMTxLogUtils.i(TAG, "turnTogfTargetFrequency succeed.");
                    }
                    else
                    {
                        //Toast.makeText(this, "turn to " + mTargetFrequency + " failed", Toast.LENGTH_LONG).show();
                        showIndication("Set current frequency to " + mTargetFrequency + " MHz failed" , true);
                        FMTxLogUtils.e(TAG, "turnTogfTargetFrequency failed.");
                    }
                }
                else
                {
                    FMTxLogUtils.e(TAG, "turnTogfTargetFrequency: FM tx is not powered up yet");
                }
            } catch (RemoteException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
                FMTxLogUtils.e(TAG, "service call - turnToFrequency(xxx)failed.");
            }
        }
        ...
        {
            //FMTxLogUtils.d(TAG, "-->serviceValidCheck");
            if(mFMTxService == null)
            {
                FMTxLogUtils.e(TAG, "service not connected yet, activity will exit.");
                finish();
            }
        }
         ...
         * 
         * @param frequency
         * @param onoff - current Tx state - true means Tx is power up, otherwise power down
         */
            ...
            public void run () 
            {
                ...
                mSearchThreadHandler = new Handler()
                {
                    ...
                    public void handleMessage(Message msg)
                    {
                        ...
                        {
                        case SEARCH_INTERRUPTED:
                            FMTxLogUtils.e(TAG, "SearchThread - receive the interrupted message.");
                            mSearchThreadHandler = null;
                            break;
                        default:
                            break;
                ...
                //TODO:do search job here
                try {
/*                        int[] iArrFrequencyList =  mFMTxService.searchChannelsForTx(MIN_FREQUENCY, CHANNELS_TO_SEARCH);
                        if(iArrFrequencyList.length() > 0)
                        {
                            mFrequencyList = new float[iArrFrequencyList.length()];
                            for(int i =0; i < iArrayFrequencyList.length(); i++)
                            {
                                mFrequencyList[i] = iArrFrequencyList[i] / 10;
                            }
                             //mFrequencyList = mFMTxService.searchChannelsForTx(MIN_FREQUENCY, CHANNELS_TO_SEARCH);
                             Thread.sleep(2000);
                        }
...
*/                        int direction = 0;
                        ...
                        mFrequencyList =  mFMTxService.searchChannelsForTx(mTargetFrequency, direction, CHANNELS_TO_SEARCH);
                } catch (Exception e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                    FMTxLogUtils.e(TAG, "service call - searchChannelsForTx(xxx)failed.");
                    mFrequencyList = null;
                }
        ...
        private class PlayerStateMonitorThread extends Thread
        {
            ...
            public void run () 
            {
                FMTxLogUtils.d(TAG, "i am running, my Id = " + getId() + "myName = " + getName());
                super.run();
                try
                {
                    do{
                        FMTxLogUtils.d(TAG, "PlayerStateMonitorThread - receive PLAY_STATE_QUERY message.");
                        Thread.sleep(MONITOR_PRECISION);
                        if(mMediaPlayer != null)
                        {
                            if(mMediaPlayer.isPlaying())
                            {
                                FMTxLogUtils.d(TAG, "MediaPlayer is playing.");
                    ...
                    } while(FMTxEMActivity.PlayerStateMonitorThread.interrupted() == false );
                }
                catch (Exception e)
                {
                    e.printStackTrace();
