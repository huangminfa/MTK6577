...
public class PlayerActivity extends Activity implements View.OnClickListener
	...
	//MTK_MBBMS_V30_SUPPORT start
    private static final int TUNE_FAILED = -1;
	private boolean mFirstTimeEnter = true;	
	private IMDControl mIMDControl;

	//MTK_MBBMS_V30_SUPPORT end
    ...
    private static final int MSG_HIDE_IMD_UNREAD = 11;	
	...
	private Bitmap videoChannelIcon;
	private Bitmap audioChannelIcon;
	private Bitmap unsubscribedLogo;	
	private Bitmap subscribedIndicator;	
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
                ...
                case MSG_OPEN_DONE: {
					...
					if (mState != STATE_OPENING) return;
					
					//MTK_MBBMS_V30_SUPPORT start
					if (MBBMSService.isV3() && mFirstTimeEnter) { 
						mFirstTimeEnter = false;
						if (msg.arg2 == TUNE_FAILED && !mStopByUser 
								&& mModeSwitchManager.isMbbmsMode() && !isFinishing()) {
							gotoLocationWarningAndFinish();
							return;
						}
					}
					//MTK_MBBMS_V30_SUPPORT end
				...
				case MSG_HIDE_CHANNEL_NUMBER: {
				    if (msg.arg1 == 0) {
				        ...
				        mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_HIDE_CHANNEL_NUMBER,1,0,null),CHANNEL_NUMBER_ON_DURATION); 				        
				    } else {
				...
				case MSG_HIDE_IMD_UNREAD: {
					mIMDControl.hideIMDUnread();
					break;
				}
                case MSG_MSK_FAIL: {
					onMskFail();
    ...
    private class CallStateListener extends PhoneStateListener{
        ...
        public void onCallStateChanged(int state, String incomingNumber) {
			...
			if (mStarted && mMuted && state == TelephonyManager.CALL_STATE_IDLE) {
				if (!mMuteByUser) {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		if (state == null) {
			...
			if (mChannelList.size() <= 0) {//if it is restart from OOM murder.
			...
			if (mCurrentChannel < 0 || mCurrentChannel >= mChannelList.size()) {
                Log.e(TAG, "Saved mCurrentChannel = " + mCurrentChannel + " is not in between 0 and (size:)" + mChannelList.size());
				finish();
			}
		...
		defaultLoadingIcon = BitmapFactory.decodeResource(getResources(),R.drawable.loading_icon);
	...
	if (mMuted) {//do not interfere other music app since CMMB use same stream as Music.
		setMute(false);
	}
    ...
    protected void onNewIntent(Intent intent) {
		...
		if (mCurrentChannel != ch) {
			...
			if (half) {
				String groupId = intent.getStringExtra(Utils.EXTRA_INTERACTIVITY_GROUP_ID);
				int groupPosition = intent.getIntExtra(Utils.EXTRA_INTERACTIVITY_GROUP_POSITION, -1);
				mIMDControl.showIMD(groupId,groupPosition);
			}
		}
    }		
	...
	private void showWelcomeInfo() {		
		...
		if (welcome == null) {
			...
			if (mWelcomeTextArray[mCurrentChannel] != null) {
				...
				mWelcomeTextArray[mCurrentChannel] = welcome = NO_WELCOME_TEXT;
			}
		} 

		if (welcome == NO_WELCOME_TEXT/*not equals*/) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
	    ...
	    if (ev.getAction() == MotionEvent.ACTION_DOWN) {
			if (mIMDControl.hide(false,true)) {
				return true;
    ...
    private void addMenuItems(Menu menu) {
		...
		if (MBBMSService.isV3()) {
        	menu.add(Menu.NONE, POSITION_SHOW_IMD_AMOUNT,
                POSITION_SHOW_IMD_AMOUNT, R.string.imd_amount).setIcon(R.drawable.menu_imd_icon);		
		}
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
		...
		case POSITION_FULL_SCREEN:
            toggleFullScreen();
            break;  			
		case POSITION_SHOW_IMD_AMOUNT:
            mIMDControl.showIMDUnread();
            break;  			
        }
        return true;
    }
    ...
    public void onBackPressed() {
        ...
        if (mState == STATE_CAPTURING) {
            ...
            return;
        } else if (mIMDControl.onBackPressed()) {
			return;
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        switch (id) {
	        case DIALOG_EMERGENCY_BROADCAST_3_4: {
                ...
                final View new_ebm_title = factory.inflate(R.layout.new_ebm_title, null);
				dlg = new AlertDialog.Builder(this)
				.setView(new_ebm_title)
				...
				.create();
				 ...
				 dlg.setOnShowListener(new DialogInterface.OnShowListener() {
				 	public void onShow(DialogInterface dialog) {
						//the android doc is not clear,AnimationDrawable.start could only work when its associated view is attached.
    ...
    public void registerImage() {
        ...
        if (uri != null/*null means we got an error*/) {
			...
			String text = String.format(getStringById(R.string.saved_to),mSnapFilename+".jpg",CMMBMediaPlayer.getDefaultStoragePath());
			Toast.makeText(this,text,500).show();			
        }		
    }
	...
	private void snapDone(String err_msg) {
		if (mState == STATE_CAPTURING) {	
			if (err_msg != null) {
				Toast.makeText(this,err_msg, 500).show();			
			} else {
				registerImage();
			}			
			mState = STATE_STARTED;
...
/*
		...
		if (ev == CMMBServiceClient.CMMB_EVENT_CLIENT_INFO) {
			switch (type) {
				case CMMBServiceClient.CMMB_INFO_BITSTREAM_ARRIVAL:
					//because this function is called in binder thread,we don't want it to execute openTrue outside main thread.
				...
				case CMMBServiceClient.CMMB_INFO_RETRIEVE_MSK_START:
					...
					if(mState == STATE_OPENING && !mHasStartRetrieveKey){
						...
						mHandler.removeMessages(MSG_OPEN_DONE);
						mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_OPEN_DONE,0,-1,getStringById(R.string.cmcc_net_error)),OPEN_TIME_OUT_INTERVAL2);
    ...
    private void gotoProgram() {
		...
		//PlaybillActivity and this activity is designed in single-instance model,so we bring it to front if it is already in the stack. 
    ...
    private void gotoLocationWarningAndFinish() {
        Intent intent = new Intent();
        intent.setClass(this,LocationAdviceActivity.class);
        startActivity(intent);
		//end this activity.
		finish();
	}				
			...
			while(true) {
				...
				if (!mResult) {
					mHandler.sendMessage(mHandler.obtainMessage(MSG_OPEN_DONE,0
						,result.code.equals(MBBMSService.STATUS_FAIL_TUNE_FAILED) ? TUNE_FAILED : 0
	...
	private void openChannel() {
		...
		mIMDControl.resetShowForbidden();
		mIMDControl.hide(true,false);
		resetSignalState();
	...
	private int getSignal() {		
		Log.d(TAG, "getSignal()");
		if (mService == null) {//service is not ready yet.
	...
	private void onSignalUpdate(int level) {
		...
		//level:0~50
		if (level >= LEVEL_SIGNAL_MIN && level <= LEVEL_SIGNAL_UNAVAILABLE_MAX && mSignalState != SIGNAL_STATE_UNAVAILABLE) {
			stopByNoSignal();			
		...
		} else if (level > LEVEL_SIGNAL_TRANSIENT_MAX && level <= LEVEL_SIGNAL_WEAK_MAX && mSignalState != SIGNAL_STATE_WEAK) {	
		...
		} else if (level > LEVEL_SIGNAL_WEAK_MAX && level <= LEVEL_SIGNAL_MEDIUM_MAX && mSignalState != SIGNAL_STATE_MEDIUM) {
			int oldState = mSignalState;
		...
		} else if (level > LEVEL_SIGNAL_MEDIUM_MAX && level <= LEVEL_SIGNAL_MAX && mSignalState != SIGNAL_STATE_NORMAL) {
			int oldState = mSignalState;
	...
	private Bitmap getLoadingIcon() {
		...
		Log.d(TAG, "bp = "+bp+" defaultLoadingIcon = "+defaultLoadingIcon);
		if (bp == null) bp = defaultLoadingIcon;
		return bp;
	}
    ...
    private void initControlPanel() {
		...
		if (isAnyActiveCall()) setMuteButtonEnabled(false);
    ...
    private void initChannelListPopup() {
		...
		videoChannelIcon = BitmapFactory.decodeResource(getResources(),R.drawable.video_channel_small);
		audioChannelIcon = BitmapFactory.decodeResource(getResources(),R.drawable.audio_channel_small);		
		unsubscribedLogo = BitmapFactory.decodeResource(getResources(),R.drawable.unsubscribed_indicator_small);		
		subscribedIndicator = BitmapFactory.decodeResource(getResources(),R.drawable.subscribed_indicator_small);		
    }	  
	
    private void showChannelList() {
        Log.d(TAG, "showChannelList()");
		mControlPanel.setVisibility(View.INVISIBLE);
		mIMDControl.hide(false,false);
    ...
    private void togglePanelVisibility(){
		...
		if (mControlPanel == null) return;//not initialize yet.
	    if (mControlPanel.getVisibility() == View.VISIBLE) {
			mHandler.removeMessages(MSG_CONTROL_PANEL_OFF); 			
	    	fadeOut(mControlPanel);				
	    } else {
	...
	private void showPlayFailedViews(String other_error) {
		displayOSDViews(false,other_error != null ? other_error : getStringById(R.string.open_channel_fail));
	}	
	...
	private void stopByError(String other_error) {	
	    Log.d(TAG, "stopByError() other_error = "+other_error); 
		
		mPlayFailed = true;	
		stop();		
		if (!mStopByUser) {
			showPlayFailedViews(other_error);
		}
	}

	private void stopByValidationFailure(String other_error) {	
		Log.d(TAG, "stopByValidationFailure() other_error="+other_error); 
		mPlayFailed = true;		
		showPlayFailedViews(other_error);
		stop();		
	}	
	
	private void stopByNeedSubscrib(String other_error) {
	...
	private void playOnResume() {
		...
		if (mStopByUser || mPlayFailed) return;
	...
	private void setStatusBarExpanded(boolean expanded) {
        Log.d(TAG, "toggleStatusBarExpanded mDiableStatusBarExpanded = "+mDiableStatusBarExpanded
			+" expanded = "+expanded);
		if (mDiableStatusBarExpanded == !expanded) return;
    ...
    public void onLowMemory() {
		...
		mIMDControl.onLowMemory();
        super.onLowMemory();
    }
	//MTK_MBBMS_V30_SUPPORT end

	//not used at present.
	private boolean canShowIMD() {
		return mState == STATE_STARTED || mState == STATE_CAPTURING;
	}

	private class IMDControl implements IMDDirector.IMDActor{
		private View mIMDDescriptionView;
		private View mIMDView;		
		private IMDDescriptionTextView mIMDDescriptionTextView;
		private WebView mWebView;
		private TextView mIMDUnread;				
		private IMDDirector mIMDDirector;
		private int mUnreadCount;	
		private IMD mIMD;
		private boolean mShowForbidden;
			
		public IMDControl () {
			mIMDDirector = new IMDDirector(PlayerActivity.this,this,null);			
		}

		
		public boolean onBackPressed() {
			Log.d(TAG, "IMDControl.onBackPressed");
		...
		//below 3 functions implement IMDDirector.IMDActor interface.
		public void onValidationExpired() {
			Log.d(TAG, "onValidationExpired"); 		
		}

		public void onNewIMD(IMD imd,boolean show) {
			Log.d(TAG, "onNewIMD show = "+show+" mShowForbidden = "+mShowForbidden);
			Log.d(TAG, "onNewIMD imd = "+imd+" serviceId = "+imd.serviceId);
			//currently if there's already one IMD being showed,do not intterupt it.
			if (show && !mShowForbidden && !hideView(mIMDDescriptionView,false)) {
				//during channel switching an IMD of a previous channel being watch may receive,just ignore it.
				if (!imd.serviceId.equals(getCurrentChannelID())) return;
				
				String auto_pop = DB.Setting.getSettingValue(PlayerActivity.this.getContentResolver(),
					DB.Setting.NAME_ENUM_AUTO_POP_INTERACTIVITY);
				if (auto_pop.equals("1")) {
					showIMD(imd);
				} else {				
					showIMDDescription(imd);
				}
			}

			//IMDDirector.showIMD will modify unread number,so place it after showIMD.
			showIMDUnread();			
		}				

		public void switchChannel(String serviceID) {
			Log.d(TAG, "switchChannel:"+serviceID); 
			checkChannelAndPlay(getChannelIndex(serviceID));
		}	
		...
		public boolean hide(boolean hideUnread,boolean forbidden) {
			Log.d(TAG, "hide hideUnread = "+hideUnread+" forbidden = "+forbidden);
			if (hideUnread) {
				hideIMDUnread();
			}
			boolean doHideIMD = hideIMD();
			boolean doHideDescription = hideIMDDescription();
			Log.v(TAG, "hide doHideIMD = "+doHideIMD+" doHideDescription = "+doHideDescription);
			if (doHideDescription && forbidden) {
				mShowForbidden = true;
			}
			return doHideIMD || doHideDescription;			
		}

		public void resetShowForbidden() {
	    	Log.d(TAG, "resetShowForbidden");
			mShowForbidden = false;
		}
		
	    private boolean hideView(View view,boolean hide) {
	    	Log.d(TAG, "hideView");
		...
		public void showIMD(String groupId,int groupPosition) {
			IMD imd = IMDManager.getInstance(PlayerActivity.this.getContentResolver()).get(groupId,groupPosition);
			if (imd == null) {
				Log.e(TAG, "showIMD no IMD with group id = "+ groupId 
					+" and groupPosition = "+groupPosition);
			} else {
				showIMD(imd);
			}
		}
		
	    private void showIMD(IMD imd) {
	    	if (mIMDView == null) {
	    		mIMDView = LayoutInflater.from(PlayerActivity.this).inflate(R.layout.imd_webview,null);
	    		RelativeLayout.LayoutParams rl = new RelativeLayout.LayoutParams(
	    				ViewGroup.LayoutParams.MATCH_PARENT,
	    				ViewGroup.LayoutParams.WRAP_CONTENT);	 
				rl.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
				((ViewGroup) findViewById(R.id.root)).addView(mIMDView,rl); 
	    		mWebView = (WebView)mIMDView.findViewById(R.id.webview);		
				mWebView.setBackgroundColor(0);//make it transparent
				mWebView.setBackgroundResource(R.color.player_control_bg);			
				mIMDDirector.setWebView(mWebView);

	    		ImageView fullControl = (ImageView)mIMDView.findViewById(R.id.full_control);
	    		fullControl.setOnClickListener(new View.OnClickListener(){
					public void onClick(View v) {	
						hide(false,false);
						gotoIMDActivity(true,true);
					}    			
	    		});					
	    	}   
	        mIMDView.setVisibility(View.VISIBLE);
			Log.d(TAG, "showIMD canGoBack = "+mWebView.canGoBack());		
			
			mIMDDirector.showIMD(imd);
	    }

	    private boolean hideIMD() {
	    	Log.d(TAG, "hideIMD");
			mIMDDirector.closeIMD();
	    	return hideView(mIMDView,true);   	
	    }
	    	    

	    private void showIMDDescription(IMD imd) {
	    	Log.d(TAG, "showIMDDescription");
	    	if (mIMDDescriptionView == null) {
	    		mIMDDescriptionView = ((ViewStub)findViewById(R.id.imd_description)).inflate();
	    		mIMDDescriptionTextView = (IMDDescriptionTextView)mIMDDescriptionView.findViewById(R.id.description);
	    		mIMDDescriptionView.setClickable(true);
	    		mIMDDescriptionView.setOnClickListener(new View.OnClickListener(){
					public void onClick(View v) {					
						//to comsume the click action.
					}    			
	    		});				
				mIMDDescriptionTextView.setTextAppearance(PlayerActivity.this, android.R.style.TextAppearance_Large);		
	    		mIMDDescriptionTextView.setSingleLine(true);
	    		mIMDDescriptionTextView.setDisplayCompleteListener(new IMDDescriptionTextView.DisplayCompleteListener() {
					public void onDisplayComplete() {
						hideIMDDescription();						
						IMD next = getNextUnreadIMD(mIMD);
						Log.d(TAG, "onDisplayComplete next = "+next);						
						if (next != null) {
							onNewIMD(next,true);
						} 
					}    			
	    		...
	    		ImageView halfControl = (ImageView)mIMDDescriptionView.findViewById(R.id.half_control);
	    		halfControl.setOnClickListener(new View.OnClickListener(){
					public void onClick(View v) {
						hideIMDDescription();
						showIMD(mIMD);
					}    			
	    		...
	    		/*RelativeLayout.LayoutParams rl = new RelativeLayout.LayoutParams(
	    				...
	    				ViewGroup.LayoutParams.WRAP_CONTENT);
	    		rl.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
	    		((ViewGroup) findViewById(R.id.root)).addView(mIMDDescriptionView,rl);*/
	    	}
	    	mIMDDescriptionTextView.setText(imd.description);
	    	mIMDDescriptionTextView.requestLayout();
	    	mIMDDescriptionView.setVisibility(View.VISIBLE); 
			mIMD = imd;
	    }
	    
	    private boolean hideIMDDescription() {
	    	Log.d(TAG, "hideIMDDescription");
			if (mIMDDescriptionTextView != null) {
				mIMDDescriptionTextView.cancelEffect();
			}
	    	return hideView(mIMDDescriptionView,true);   	  	
	    }
		
	    public void showIMDUnread() {
			if (mIMDUnread == null) {
				mIMDUnread = (TextView)((ViewStub)findViewById(R.id.unread_imd)).inflate();
				mIMDUnread.setClickable(true);
				mIMDUnread.setOnClickListener(new View.OnClickListener(){
					public void onClick(View v) {					
						//if (mUnreadCount >= 1) {							
							hide(true,false);				
							gotoIMDActivity(mUnreadCount == 1,false);
						//}
					}    			
	    		});
			}
			
			updateUnreadIMDCount();
			
	    	mIMDUnread.setText(String.valueOf(mUnreadCount));    		
	    	mIMDUnread.setVisibility(View.VISIBLE);
	    	mHandler.removeMessages(MSG_HIDE_IMD_UNREAD);
	    	mHandler.sendEmptyMessageDelayed(MSG_HIDE_IMD_UNREAD, 6000);
	    }

	    public boolean hideIMDUnread() {
	    	Log.d(TAG, "hideIMDUnread");
			
	    	boolean doHide = hideView(mIMDUnread,true);   	
			if (doHide) {
				mHandler.removeMessages(MSG_HIDE_IMD_UNREAD);
			}
			return doHide;
	    ...
	    private void updateUnreadIMDCount() {
			Cursor cursor = getContentResolver().query(SG.InteractivityData.CONTENT_URI, 
                            new String[]{"count(_id)"}, 
                            SG.InteractivityData.HAS_READ + "= 0" + " AND " +
							SG.InteractivityData.FROM_SG + "= 0" + " AND " +
                            SG.InteractivityData.SERVICE_ID_REF + " = "+getCurrentChannelID(), 
                            null, null);
			if (cursor != null) {
				if (cursor.moveToFirst()) {
					mUnreadCount = cursor.getInt(0);
				}
				cursor.close();
			} 
	    	Log.i(TAG, "updateUnreadIMDCount mCurrentChannel = "+ mCurrentChannel 
				+ " mUnreadCount =" + mUnreadCount);
	    }		

		private List<IMD> getIMDList() {
			return IMDManager.getInstance(PlayerActivity.this.getContentResolver()).getList();
		}

	    private String getUnreadIMDGroupId() {			
			String gid = null;			
			String sid = getCurrentChannelID();
			List<IMD> list = getIMDList();
			for(IMD imd:list) {
				if (!imd.hasRead && imd.serviceId.equals(sid)) {
					gid = imd.groupID;
					break;
				}
			}
			
	    	Log.i(TAG, "getUnreadIMDGroupId mCurrentChannel = "+ mCurrentChannel 
				+ " gid =" + gid);
			return gid;
	    }			

		private IMD getNextUnreadIMD(IMD imd) {			
			if (imd != null && mUnreadCount > 1) {
				String sid = getCurrentChannelID();
				List<IMD> list = getIMDList();
				int pos = list.indexOf(imd);
				if (pos == -1) {
					Log.w(TAG, "getNextUnreadIMD pass-in imd is not found !"); 
				} else {
					pos++;
					for(int size = list.size(); pos < size; pos++) {
						IMD item = list.get(pos); 
						if (!item.hasRead && item.serviceId.equals(sid)) {
							return item;
						}
					}
				}
		...
		private void gotoIMDActivity(boolean detail,boolean fullScreen) {
			Intent intent;
			if (detail) {
				intent = new Intent(PlayerActivity.this, InteractivityDetailActivity.class);
				if (fullScreen) {
					IMD imd = mIMDDirector.getCurrentIMD();
					if (imd != null) {
						intent.putExtra(Utils.EXTRA_INTERACTIVITY_GROUP_ID,imd.groupID);	
					}
				} else {
					intent.putExtra(Utils.EXTRA_INTERACTIVITY_GROUP_ID,getUnreadIMDGroupId());	
				}
			} else {
				intent = new Intent(PlayerActivity.this, InteractivityListActivity.class);
			}
			intent.putExtra(Utils.EXTRA_SERVICE_ROW_ID,getCurrentChannelRowID());		
			intent.putExtra(Utils.EXTRA_SERVICE_NAME,getCurrentChannelName());		
			intent.putExtra(Utils.EXTRA_SERVICE_ID,getCurrentChannelID());	
			startActivity(intent);
		}
	}
	...
	private class ChannelListAdapter extends BaseAdapter {
        ...
        class ViewHolder{
            ...
            private ImageView encryped_indicator;
        }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {    
            ...
            if (convertView == null) {      
                ...
                holder.channelName =(TextView)convertView.findViewById(R.id.player_channel_name);   
                holder.programName =(TextView)convertView.findViewById(R.id.player_program_name);
                holder.logo =(ImageView)convertView.findViewById(R.id.player_logo_image);   
                holder.encryped_indicator =(ImageView)convertView.findViewById(R.id.player_encryped_indicator);   
            ...
            holder.programName.setText(pr.program);
            
            if (ch.lock_stat == Channel.LOCK_STATE_NONE) {
				holder.encryped_indicator.setVisibility(View.INVISIBLE);
	        	holder.logo.setImageBitmap(ch.logo == null ? (ch.isAudio() ? audioChannelIcon : videoChannelIcon) : ch.logo);
            } else if (ch.lock_stat == Channel.LOCK_STATE_UNLOCK){
		holder.logo.setImageBitmap(ch.logo == null ? (ch.isAudio() ? audioChannelIcon : videoChannelIcon) : ch.logo);
				holder.encryped_indicator.setVisibility(View.VISIBLE);
				holder.encryped_indicator.setImageBitmap(subscribedIndicator);
			} else {
				holder.encryped_indicator.setVisibility(View.INVISIBLE);
				holder.logo.setImageBitmap(unsubscribedLogo);	
				//holder.logo.setImageBitmap(ch.logo == null ? unsubscribedLogo: ch.logo);					
			}
