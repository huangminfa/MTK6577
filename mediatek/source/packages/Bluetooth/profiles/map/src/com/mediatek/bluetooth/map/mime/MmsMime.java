...
public class MmsMime extends MimeBase {
	...
	//	req.setDate(mHeaders.mTimeStamp);
		...
		if (mHeaders.mMiltipartType != null) {
			req.setContentType(mHeaders.mMiltipartType.getBytes());
		} else {
			req.setContentType(MimeContent.TEXT_PLAIN.getBytes()); 
		}
	...
	public PduBody generatePduBody(){
		...
		for (MimeAttachment attachement : mAttachment) {
			...
			if (attachement.mContentBytes != null) {
				part.setData(attachement.mContentBytes);
			...
			if (attachement.mMimeType != null) {
				part.setContentType(attachement.mMimeType.getBytes());
			} else {
				part.setContentType(MimeContent.TEXT_PLAIN.getBytes()); 
			}
