...
extern void  fputc_printable(int ch, FILE *file);
extern void  bb_displayroutes(int noresolve, int netstatfmt);
extern char*  xmalloc_sockaddr2dotted_noport(const struct sockaddr *sa);
...
enum {
	...
	OPT_route = (1 << OPTBIT_ROUTE),
	OPT_wide = (1 << OPTBIT_WIDE ),
	OPT_prg = (1 << OPTBIT_PRG  ),
};
...
static int  add_to_prg_cache_if_socket(const char *fileName,
...
static int  dir_act(const char *fileName,
		struct stat *statbuf UNUSED_PARAM,
		...
		const char *state_str, const char *proto, int is_connected)
{
	 ...
	 || (!is_connected && (flags & NETSTAT_LISTENING)))
	{
		...
		//printf(FMT_NET_CONN_DATA , proto , param->rxq , param->txq , l ,r ,state_str);
...
static int  udp_do_one(char *line)
{
	int have_remaddr;
...
static int  raw_do_one(char *line)
{
	int have_remaddr;
	struct inet_params param;
...
static int  unix_do_one(char *line)
{
