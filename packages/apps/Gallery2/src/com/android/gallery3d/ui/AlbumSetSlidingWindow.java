...
public class AlbumSetSlidingWindow implements AlbumSetView.ModelListener {
    ...
    private static final int PLACEHOLDER_COLOR = 0xFF444444;

    public boolean mIsStereoDisplaySupported = 
    ...
    private void notifySlotChanged(int slotIndex) {
        ...
        if (mActiveRequestCount == 0 || isActiveSlot) {
            for (DisplayItem item : mData[slotIndex % mData.length].covers) {
                ...
                if (MtkLog.DBG) {
                    MtkLog.i(TAG, "notifySlotChanged: #" + slotIndex + ", cover image requested");
                }
    ...
    private void updateAllImageRequests() {
        if (MtkLog.DBG) {
            MtkLog.i(TAG, "updateAllImageRequests: start=" + mActiveStart + ", end=" + mActiveEnd);
        }
        ...
        for (int i = mActiveStart, n = mActiveEnd; i < n; ++i) {
            for (DisplayItem item : mData[i % mData.length].covers) {
                ...
                if (MtkLog.DBG) {
                    MtkLog.i(TAG, " updateAllImageRequests: cover image for slot[" + i + "] requested!");
                }
    ...
    private class GalleryDisplayItem extends AbstractDisplayItem
        ...
        private  int mSubType;//nooooooooooooo final?
        ...
        public GalleryDisplayItem(int slotIndex, int coverIndex, MediaItem item) {
            ...
            mSubType = item.getSubType() | (mIsPanorama ? MediaObject.SUBTYPE_PANORAMA : 0);
            if (mIsStereoDisplaySupported) {
                // we do NOT draw stereo overlay for AlbumSetPage
                int stereoSubType = MediaObject.SUBTYPE_MPO_3D | 
                                    MediaObject.SUBTYPE_MPO_3D_PAN | 
                                    MediaObject.SUBTYPE_STEREO_VIDEO |
                                    MediaObject.SUBTYPE_STEREO_JPS;
                mSubType &= ~stereoSubType;
            }
        ...
        public int render(GLCanvas canvas, int pass) {
            ...
            mSelectionDrawer.draw(canvas, mContent, width, height,
                    ...
                    /* mIsPanorama */ mSubType, mLabelSpec.labelBackgroundHeight,
        ...
        public void startLoadBitmap() {
            if (MtkLog.DBG) {
                MtkLog.d(TAG, "startLoadBitmap: slot=" + mSlotIndex + ", item=" + (mMediaItem != null ? mMediaItem.getName() : "null"));
            }
        ...
        private void onLoadBitmapDone() {
            if (MtkLog.DBG) {
                MtkLog.i(TAG, "GDI.onLoadBitmapDone, slot=" + mSlotIndex + ", item=" + (mMediaItem != null ? mMediaItem.getName() : "null") + ", bitmap=" + mFuture.get());
            }
    ...
    private static int identifySourceType(MediaSet set) {
        ...
        if ((set instanceof LocalAlbum || set instanceof LocalMergeAlbum) && 
            path.toString().endsWith(StereoHelper.INVALID_LOCAL_PATH_END)) {
            type = SelectionDrawer.DATASOURCE_VIRTUAL_STEREO;
        }
    ...
    public void onWindowContentChanged(int index) {
        if (MtkLog.DBG) {
            MtkLog.d(TAG, "onWindowContentChanged: index=" + index + ", isactive=" + mIsActive);
        }
        if (!mIsActive) {
