...
public class TodosListAdapter extends BaseAdapter implements QueryListener,
    ...
    private TextView mHeaderNumberText = null;
    ...
    public void addItem(TodoInfo info) {
        if (info == null || info.getId() == null || mIsLoadingData) {
            LogUtils.e(TAG, "addItem() failed. mFinishLoadingItems=" + mIsLoadingData
                    + " Or Ill-legeal TodoInfo.");
            return;
        }
        int index = -1;
        TodoInfo temp = null;
        if (TodoInfo.STATUS_DONE.equals(info.getStatus())) {
            ...
            if (mEditType == EDIT_DONES) { // update checkbox's value.
                for (int i = mDonesDataSource.size() - 2; i >= index; i--) {
                    if (mDonesCheckStates.get(i)) {
                        mDonesCheckStates.put(i + 1, true);
                        mDonesCheckStates.delete(i);
                    }
                }
            }
        } else {
            ...
            if (mEditType == EDIT_TODOS) { // update checkbox's value.
                for (int i = mTodosDataSource.size() - 2; i >= index; i--) {
                    if (mTodosCheckStates.get(i)) {
                        mTodosCheckStates.put(i + 1, true);
                        mTodosCheckStates.delete(i);
                    }
                }
            }
        }
    ...
    private void updateDonesHeaderView() {
        ...
        if (mDonesExpand) {
            donesHolder.mBtnExpand.setImageResource(R.drawable.ic_expand_open);
        } else {
            donesHolder.mBtnExpand.setImageResource(R.drawable.ic_expand_close);
        }
    }
    ...
    private void updateDonesFooterView() {
        ...
        donesFooterHolder.mFooterHelper.setEnabled(false);
    }
    ...
    private View getItemView(int position, View convertView, ViewGroup parent) {
        ...
        if (checkBoxEnable) {
            holder.mChangeInfoState.setVisibility(View.INVISIBLE);
            holder.mTodoInfoCheckBox.setVisibility(View.VISIBLE);
            holder.mTodoInfoCheckBox.setEnabled(checkBoxEnable);
            holder.mTodoInfoCheckBox.setChecked(checked);
        } else {
            holder.mTodoInfoCheckBox.setVisibility(View.INVISIBLE);
            holder.mChangeInfoState.setVisibility(View.VISIBLE);
    ...
    private void updateHeaderNumberText() {
        if (mTodosDataSource.isEmpty() && mDonesDataSource.isEmpty()) {
            mHeaderNumberText.setText(EMPTY_TEXT);
        } else {
            mHeaderNumberText.setText(String.valueOf(mTodosDataSource.size()));
        }
    }
}
