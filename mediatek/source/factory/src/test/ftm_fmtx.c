...
static item_t fmtx_items[] = {
    ...
    item(ITEM_FREQ4,  "Test Freq 4"),
    item(ITEM_FREQ5,  "Test Freq 5"),
    item(ITEM_FREQ6,  "Test Freq 6"),
    item(ITEM_FREQ7,  "Test Freq 7"),
    item(ITEM_FREQ8,  "Test Freq 8"),
    item(ITEM_FREQ9,  "Test Freq 9"),
...
static void *fmtx_update_thread(void *priv)
{
    ...
    if (ret){
        sprintf(fmtx->info, "%s", "FM TX open failed\n"); 
        iv->redraw(iv);
        return NULL;
    }
    ...
    if(ret){
        ...
        g_fmtx_fd = -1;
        sprintf(fmtx->info, "%s", "FM TX power on failed\n"); 
        iv->redraw(iv);
        return NULL;
    }
    sprintf(fmtx->info, "%s", "FM TX power on successfully\n");
    iv->redraw(iv);
    //set audio path
#ifdef FM_AUDIO_TX
    Audio_FMTX_Play(true, FMTX_1K_TONE);
    sprintf(fmtx->info+strlen(fmtx->info), "1K Hz tone output\n");
    iv->redraw(iv);
#endif
    ...
    while(1){
        ...
        if(g_txsetfreq){
            ...
            if (ret){
                ...
                if(ret){
                    sprintf(fmtx->info+strlen(fmtx->info), "Enable RDS TX Failed\n");
                    iv->redraw(iv);
                }
                ...
                if(ret){
                    sprintf(fmtx->info+strlen(fmtx->info), "set rds faield\n");
                    iv->redraw(iv);
                }else{
                    sprintf(fmtx->info+strlen(fmtx->info), "RDS TX Data:");
