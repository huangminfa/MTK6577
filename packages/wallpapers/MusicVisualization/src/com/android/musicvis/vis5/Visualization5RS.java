...
class Visualization5RS extends RenderScriptScene {
    ...
    Visualization5RS(int width, int height) {
        ...
        for(int i = 0; i < outlen; i++) {
            ...
            {
                mPointData[i*8] *= (4.5f/9.f);
                mPointData[i*8+4] *= (4.5f/9.f); 
            }
            else
            if ((mWidth == 600 && mHeight == 976))
            {
                mPointData[i*8] *= (7.2f/9.f);
                mPointData[i*8+4] *= (7.2f/9.f); 
            }
        }
    }
    ...
    public void resize(int width, int height) {
        ...
        for(int i = 0; i < outlen; i++) {
            ...
            if ((mWidth == 1024 && mHeight == 552))
            {
                mPointData[i*8] *= (7.8f/9.f);
                mPointData[i*8+4] *= (7.8f/9.f); 
            }
            else
            if ((mWidth == 600 && mHeight == 976))
            {
