...
int BootTimesCheck()
{
	...
	if(!fp)
	{
		...
		ret = ioctl(dev_open_fp, PRINT_GENERAL_INFO, (unsigned long)print_buff);
#endif		
		LOGE("MVG_TEST BootTimesCheck Create powerlost_data.dat !\n");
		...
		ret = ioctl(dev_open_fp, PRINT_GENERAL_INFO, (unsigned long)print_buff);
#endif	
				LOGE("MVG_TEST BootTimesCheck Create powerlost_data.dat Failed!\n");
				return -2;
			}
			for(i=0;i<32;i++)
			{
			...
			for(i=0;i<TEST_DATA_FILE_SIZE;i++)
			{
				...
				LOGE("MVG_TEST BootTimesCheck Write powerlost_data.dat, data num is:%d",fopCnt);
			}
		}
		fclose(fp);
		memset(print_buff,0,32);
		...
		LOGE("MVG_TEST BootTimesCheck Create powerlost_times.txt !\n");
		...
		ret = ioctl(dev_open_fp, PRINT_GENERAL_INFO, (unsigned long)print_buff);
#endif
			LOGE("MVG_TEST BootTimesCheck Create powerlost_times.txt Failed!\n");
			return -1;
		}
		poweroff_times = 0;
		sprintf(Inbuffer, "%5d\n",poweroff_times);
		fopCnt = fwrite(Inbuffer, TEST_TIMES_LENGTH+1, 1, fp);
		LOGE("MVG_TEST BootTimesCheck Write '0' to powerlost_times.txt char number is: %d!\n", fopCnt);
		fclose(fp);
		memset(print_buff,0,32);
	...
	else
	{
		...
		LOGE("MVG_TEST BootTimesCheck Read powerlost_times.txt char number is: %d!\n", fopCnt);
		fclose(fp);
		Outbuffer[TEST_TIMES_LENGTH] = '\0';
		poweroff_times = atoi(Outbuffer);
		g_PowerOffTimes = poweroff_times;
		LOGE("MVG_TEST ----------Test has passed %d times!-------\n", poweroff_times);
		ret = ioctl(dev_open_fp, PRINT_REBOOT_TIMES, (unsigned long)(&g_PowerOffTimes));
		if(ret)
		{
			LOGE("MVG_TEST dev file ioctl PRINT_REBOOT_TIMES operation  --------Error!");
		}
		if(poweroff_times > 2000)
		{
			LOGE("MVG_TEST--------------MVG_TEST Test passed!---------------\n");
		}
		poweroff_times ++;
		sprintf(Inbuffer, "%5d\n",poweroff_times);
		fp = fopen(TEST_TIMES_COUNTER_FILE, "wt+");
		...
		ret = ioctl(dev_open_fp, PRINT_GENERAL_INFO, (unsigned long)print_buff);
			...
			LOGE("MVG_TEST BootTimesCheck Open powerlost_times.txt Failed!\n");
			return -1;
		}
		fopCnt = fwrite(Inbuffer, TEST_TIMES_LENGTH+1, 1, fp);
		LOGE("MVG_TEST BootTimesCheck Write powerlost_times.txt char number is: %d!\n", fopCnt);
		fclose(fp);
	}
	return 0;
}
...
int DataFileVerification()
{
	...
	if(!fp)
	{
		...
		if(ret)
		{
			LOGE("MVG_TEST dev file ioctl PRINT_FILE_OPERATION_ERR operation  --------Error!");
		}
		LOGE("MVG_TEST DataFileVerification Open powerlost_data.dat Failed!\n");
		return -1;
	}
	for(i=0; i<TEST_DATA_FILE_SIZE; i++ )
	{
		...
		//LOGE("MVG_TEST DataFileVerification Read powerlost_data.dat, data num is:%d",fopCnt);
		for(j=0; j<32; j++)
		{
			if(Outbuffer[j] != j)
			{
				...
				if(ret == -1)
				{
					LOGE("MVG_TEST dev file ioctl PRINT_DATA_COMPARE_ERR operation  --------Error!");
				}
				LOGE("MVG_TEST DadaFileVerification File Compare Error! i=%d, j=%d",i,j);
				return -1;
			}		
		}	
	}
...
int Test_1()
{
	...
	if(!fp)
	{
		...
		LOGE("MVG_TEST Test_1 Create Test.txt Failed!\n");
		return -1;
	}
	for(i=0; i<20000; i++)
	{
	...
	LOGE("MVG_TEST Test_1 Write Test.txt char number is: %d!\n", fopCnt*20000);
	fclose(fp);
	...
	if(!fp)
	{
		...
		LOGE("MVG_TEST Test_1 Open Test.txt Failed!\n");
		return -1;
	}
	for(i=0; i<20000; i++)
	{
		...
		for(j=0; j<fopCnt; j++)
		{
			if(Inbuffer[j] != Outbuffer[j])
			{
				...
				ret = ioctl(dev_open_fp, PRINT_GENERAL_INFO, (unsigned long)print_buff);
				LOGE("MVG_TEST Test_1 data%d compare ERROR!\n", j);
				return -1;
			}
		}
	}
	LOGE("MVG_TEST Test_1 data compare Successful!\n");
	fclose(fp);

	ret = remove(TEST_FILE);
	if(ret != 0)
	{
		LOGE("MVG_TEST Test_1 Delete Test file Failed, Err Code: %d\n",ret);
		return ret;
	}
	ret = rmdir(TEST_PATH);
	if(ret != 0)
	{
		LOGE("MVG_TEST Test_1 Delete Test Folder Failed, Err Code: %d\n",ret);
		return ret;
	}
	ret = 0;
	return ret;
...
int Test_2()				//need more test case
{
	...
	if(!fp)
	{
		...
		ret = ioctl(dev_open_fp, PRINT_GENERAL_INFO, (unsigned long)print_buff);
#endif
		LOGE("MVG_TEST Test_2 Open pwrloststdata.txt Failed!\n");
		return -1;
	}
	for(i=0;i<100;i++)
	{
	...
	LOGE("MVG_TEST Test_2 Write pwrloststdata.txt char number is: %d!\n", fopCnt*100);
	//fclose(fp);
	return ret;
}
...
int main(void)
{
	...
	if(dev_open_fp < 0)
	{
		LOGE("MVG_TEST Open nand_mvp dev file Failed!dev_open_fp=%d !\n",dev_open_fp);
		return -1;
	}
	LOGE("MVG_TEST dev file open, dev_open_fp=%d\n",dev_open_fp);
	sprintf(print_buff, "MVG_TEST Dev File Open OK!\n\0");
	ret = ioctl(dev_open_fp, PRINT_GENERAL_INFO, (unsigned long)print_buff);
	if(ret == -1)
	{
		LOGE("MVG_TEST dev file ioctl PRINT_GENERAL_INFO operation  --------Error!ret=%d\n",ret);
	}
	ret = BootTimesCheck();
	if(ret == 0)
	{
		LOGE("MVG_TEST BootTimesCheck ------------OK!\n");
	}
	else
	{
		LOGE("MVG_TEST BootTimesCheck ------------Error!\n");
	}
	ret = DataFileVerification();
	if(ret == 0)
	{	
		LOGE("MVG_TEST DataFileVerification -----------OK!\n");
	}
	else
	{
		LOGE("MVG_TEST DataFileVerification -----------Error!\n");
	}
	while(1)
	{
		i = 1000;
		while(i--)
		{
			...
			if(ret != 0)
			{
				LOGE("MVG_TEST Test_1 ------------- Failed!\n");
			}
			ret = Test_2();
			if(ret != 0)
			{
				LOGE("MVG_TEST Test_2 ------------ Failed!\n");
			}
		}
		sleep(1);
	}
