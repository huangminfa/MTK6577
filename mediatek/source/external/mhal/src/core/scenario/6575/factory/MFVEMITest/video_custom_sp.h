...
#include "vcodec_if.h"
#ifdef ANDROID_LOAD
#define ASSERT(expr) do {if(!(expr)) rVCODEC_OAL_Function.VCodecAssertFail(__FILE__, __LINE__, 0); } while (0)
#else
#define ASSERT(expr)                                            \
    do {                                                        \
        if (!(expr))                                            \
...
VIDEO_DEC_API_T     *GetDecoderAPI(VIDEO_DECODER_T);
...
VIDEO_ENC_API_T     *GetEncoderAPI(VIDEO_ENCODER_T);
VCODEC_ENC_API_T *GetMPEG4EncoderAPI(void);
//VIDEO_ENCODER_API_T *GetH264EncoderAPI(void);
