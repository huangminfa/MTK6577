...
public class RecentsVerticalScrollView extends ScrollView implements SwipeHelper.Callback {
    ...
    private void update() {
        ...
        int themeMainColor = 0;
        if (FeatureOption.MTK_THEMEMANAGER_APP) {
            themeMainColor = getContext().getResources().getThemeMainColor();
        }
        Log.d(TAG, "update: mAdapter = " + mAdapter + ",itemCount = " +  mAdapter.getCount());
        ...
        for (int i = 0; i < mAdapter.getCount(); i++) {
            ...
            if (old == null) {
                ...
                if (FeatureOption.MTK_THEMEMANAGER_APP) {
                    if (themeMainColor != 0) {
                        calloutLine.setBackgroundColor(themeMainColor);
                    }
                }
