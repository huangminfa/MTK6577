...
import com.mediatek.MediatekDM.DmCommomFun;
...
public class DmDataConnection {
    ...
    public int startDmDataConnectivity() throws IOException {
        ...
        if (simId == -1) {
            Log.e(TAG.Connection,
                  "Get Register SIM ID error in start data connection");
            return result;
        }
        // for gemini
        ...
        if (result == MTKPhone.APN_TYPE_NOT_AVAILABLE
                || result == MTKPhone.APN_REQUEST_FAILED) {
    ...
    public void stopDmDataConnectivity() {
        ...
        try {
            ...
            if (simId == -1) {
                Log.e(TAG.Connection,
                      "Get Register SIM ID error in stop data connection");
                return;
            }
            if (MTKOptions.MTK_GEMINI_SUPPORT == true) {
                // begin for connectity gemini
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(
                ...
                if (mBdl != null) {
                    ...
                    int intentSimId = intent.getIntExtra(
                                          MTKConnectivity.EXTRA_SIM_ID, 0);
                    int networkType = info.getType();

                    if (intentSimId == simId
                            && networkType == MTKConnectivity.TYPE_MOBILE_DM) {
                        Log.i(TAG.Connection,
                              "[dm-conn]->type == " + info.getTypeName()+"("+networkType+")");
                        Log.i(TAG.Connection,
                              "[dm-conn]->intent_sim_Id == " + intentSimId);
                        Log.i(TAG.Connection,
                              "[dm-conn]->network_sim_Id == " + networkSimId);
                        Log.i(TAG.Connection,
                              "[dm-conn]->registered_sim_Id == " + simId);
                        ...
                        if (state == State.CONNECTED) {
                            ...
                            try {
                                ...
                                Log.e(TAG.Connection,
                                      "[dm-conn]->ensureRouteToHost() failed:", ex);
                            }
                           ...
                           DmConst.intentAction.NET_DETECT_TIMEOUT)) {
                Log.i(TAG.Connection,
    ...
    private int beginDmDataConnectivity() throws IOException {
        ...
        int result = mConnMgr.startUsingNetworkFeature(
                         ConnectivityManager.TYPE_MOBILE, MTKPhone.FEATURE_ENABLE_DM);

        Log.i(TAG.Connection, "[dm-conn]->startUsingNetworkFeature: result=" + result);

        if (result == MTKPhone.APN_ALREADY_ACTIVE) {
            Log.i(TAG.Connection,
                  "[dm-conn]->APN_ALREADY_ACTIVE");
            ScreenLock.releaseWakeLock(mContext);
    ...
    private int beginDmDataConnectivityGemini(int simId) throws IOException {
        ...
        Log.i(TAG.Connection, "startDmDataConnectivityGemini: simId = " + simId
              + "\t result=" + result);

        if (result == MTKPhone.APN_ALREADY_ACTIVE) {
            Log.w(TAG.Connection,
                  "The data connection is already exist, go ahead");
            ScreenLock.releaseWakeLock(mContext);
            ScreenLock.acquirePartialWakelock(mContext);
            ensureRouteToHost();
        }
    ...
    private void endDmConnectivityGemini(int simId) {
        try {
            Log.i(TAG.Connection, "endDmDataConnectivityGemini: simId = "
                  + simId);
    ...
    private void ensureRouteToHost() throws IOException {
        ...
        if (!mConnMgr.requestRouteToHost(MTKConnectivity.TYPE_MOBILE_DM,
                                         inetAddr)) {
