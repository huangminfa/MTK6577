...
public class CalendarEventObserver extends ContentObserver implements ISelfChangeObserver {
	// ==============================================================
	// Constants
	// ==============================================================
	...
	public void onChange(boolean selfChange) {
		...
		Debugger.logW(new Object[] { selfChange }, "synchronized maxCalendarEventId = " + maxCalendarEventId);
		synchronized (this) {
			if (mObservering) {
				if (!isSelfChangingContent()) {
					...
					if (maxCalendarEventId > mCalendarEventId) {
						Debugger.logD(new Object[] { selfChange }, "New event, id=" + maxCalendarEventId);
						CalendarEvent event = mProxy.getEvent(maxCalendarEventId, true, true);

						EventDispatcher.dispatchCalendarEventChangedEvent(new Event().put(ICalendarEventListener.BY_SELF,
								false).put(ICalendarEventListener.CALENDAREVENT, event));
					}
				} else {
					Debugger.logI(new Object[] { selfChange }, "Event content is changed by self.");
					this.notify();
				}
			}
