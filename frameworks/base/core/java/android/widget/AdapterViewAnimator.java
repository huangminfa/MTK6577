...
public abstract class AdapterViewAnimator extends AdapterView<Adapter>
    ...
    //Item count of adapter when handle data change last time.
    ...
    int getWindowSize() {
        if (mAdapter != null) {
            ...
            if (adapterCount == 1) {
                return 1;
            } else if (adapterCount <= getNumActiveViews() && mLoopViews) {
    ...
    void showOnly(int childIndex, boolean animate) {
        ...
        for (int i = 0; i < mPreviousViews.size(); i++) {
            ...
            if (DBG_SHOW) {
                Xlog.d(TAG, "showOnly 2: i = " + i + ", viewToRemove = " + viewToRemove);
            }
        ...
        if (DBG_SHOW) {
            Xlog.d(TAG, "showOnly 5: newWindowStartUnbounded = " + newWindowStartUnbounded
                    ...
                    + newWindowStartUnbounded + ",mDataHasChanged = " + mDataHasChanged);
        }
        // If the window has changed or data has changed.
                ...
                public void run() {
                    if (DBG) {
                        Xlog.d(TAG, "Handle data change start: mCurrentWindowStart = "
                                + mCurrentWindowStart + ",mCurrentWindowEnd = " + mCurrentWindowEnd
                                + ",mWhichChild = " + mWhichChild + ", item count = " + getCount() + ", mOldItemCount = " + mOldItemCount
                                + ",child count = " + getChildCount() + ",getWindowSize()= "
                                + getWindowSize() + ",mViewsMap = " + mViewsMap
                                + ",mPreviousViews = " + mPreviousViews + ",this = " + this + ",mPreviousItemCount = " + mPreviousItemCount);
                    }
                    handleDataChanged();

                    //when item count changed clear mViewsMap and mPreivousViews
    ...
    public void setAdapter(Adapter adapter) {
        ...
        if (DBG) {
            Xlog.d(TAG, "setAdapter: adapter count = " + getCount() + ",mWhichChild = "
                    + mWhichChild + ",this = " + this + ",mPreviousItemCount = " + mPreviousItemCount);
        }
