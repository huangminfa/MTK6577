...
static void *HeadsetIoctl_function(void *ptr)
{
   //assigned pointer
    ...
    int ret =0;
    int AudioMode;
    LOGD("HeadsetIoctl_function into whileloop");
    ...
    while(1){
        ...
        if(ret == 0){
            ...
            LOGD("HeadsetIoctl_function ret == 0 AudioMode = %d",AudioMode);
            ...
            LOGD("HeadsetIoctl_function ret == 0");
            Lad->LAD_SwitchMicBias(1);
            //delay for suer micbias is open by modem side
            usleep(50*1000);
        }
        else{
            LOGE("HeadsetFd HOOK_SWITCH_CHECK error");
        }
    }
    return 0;
}
