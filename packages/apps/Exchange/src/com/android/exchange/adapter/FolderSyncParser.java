    ...
    public void changesParser(final ArrayList<ContentProviderOperation> ops,
        ...
        EmailAsyncTask<?, ?, ?> task =
                EmailAsyncTask.runAsyncParallel(new Runnable() {
                    @Override
                    public void run() {
                        EmailAsyncTask.printStartLog("SynchronizeOnFolderParser");
                        ...
                        synchronized (FolderSyncParser.this) {
                            ...
                            try {
                                ExchangeService.callback().syncMailboxListStatus(mAccount.mId, 0, 0);
                            } catch (RemoteException exception) {
                                // ignore it
                            }
                            if (Logging.LOG_PERFORMANCE) {
                                Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] EasSyncService:FolderSync:commit ["
                                        + mCommitElapse + "]");
                            }
                        ...
                        EmailAsyncTask.printStopLog("SynchronizeOnFolderParser");
                    }});
