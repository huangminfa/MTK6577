...
//        }
    ...
    public void onScomoUpdated() {
        ...
        if (!scomoState.mVerbose||scomoState.mState==DmScomoState.IDLE||scomoState.mState==DmScomoState.ABORTED) {
            showEmpty();
        } else if (scomoState.mState==DmScomoState.DOWNLOADING) {
            showDownloading(getString(R.string.downloading_scomo));
        } else if (scomoState.mState==DmScomoState.DOWNLOADING_STARTED
                   ||scomoState.mState==DmScomoState.RESUMED
                  ) {
            showDownloading(getString(R.string.connecting));
        } else if (scomoState.mState==DmScomoState.INSTALLING) {
            showInstalling(getString(R.string.installing_scomo));
        } else if (scomoState.mState==DmScomoState.UPDATING) {
            showInstalling(getString(R.string.upgrading));
        } else if (scomoState.mState==DmScomoState.PAUSED) {
            showDownloading(getString(R.string.paused));
        } else if (
            scomoState.mState==DmScomoState.CONFIRM_DOWNLOAD
            /*|| scomoState.mState==DmScomoState.CONFIRM_INSTALL*/
            || scomoState.mState==DmScomoState.INSTALL_FAILED
            || scomoState.mState==DmScomoState.DOWNLOAD_FAILED
            || scomoState.mState==DmScomoState.INSTALL_OK
            || scomoState.mState==DmScomoState.GENERIC_ERROR
        )
        {
    ...
    private void showInstalling(String state) {
        ...
        ((ImageView)mInstallingLayout.findViewById(R.id.icon)).setImageDrawable(scomoState.getIcon());
        ((TextView)mInstallingLayout.findViewById(R.id.name)).setText(scomoState.getName());
        ((TextView)mInstallingLayout.findViewById(R.id.version)).setText(scomoState.getVersion()+"  "+scomoState.getSize()/1024+"KB");
    ...
    private void showDownloading(String state) {
        ...
        ((ImageView)mDownloadingLayout.findViewById(R.id.downloadingIcon)).setImageDrawable(scomoState.getIcon());
        ((TextView)mDownloadingLayout.findViewById(R.id.TextViewName)).setText(scomoState.getName());
        ...
        ((ProgressBar)mDownloadingLayout.findViewById(R.id.ProgressBarProgress)).setMax(scomoState.totalSize);
        ((ProgressBar)mDownloadingLayout.findViewById(R.id.ProgressBarProgress)).setProgress(scomoState.currentSize);
    }
