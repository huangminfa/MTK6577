    ...
    */
    ...
    private void createGroup(Intent intent) {
        ...
        if (!checkGroupNameExist(label, accountName, accountType, true)) {
            ...
            Intent callbackIntent = intent.getParcelableExtra(EXTRA_CALLBACK_INTENT);
            callbackIntent.putExtra(GroupEditorFragment.SAVE_MODE_EXTRA_KEY, SaveMode.RELOAD);
            deliverCallback(callbackIntent);
            return;
        }
    ...
    private void deleteGroup(Intent intent) {
        ...
        if(slotId >=0){
            ...
            Log.i(TAG, "[deleteGroup]simId:" + simId);
            int ugrpId = -1;
            ...
            Log.i(TAG, "[deleteGroup]c.getCount(): " + c.getCount());
            ...
            Log.i(TAG, "[deleteGroup]simIndexArray:" + simIndexArray);
            if (slotId >=0 && !TextUtils.isEmpty(groupLabel) ) {   
                if (simIndexArray != null) {
                    ...
                    for (int simIndex : simIndexArray) {
                        boolean ret = ContactsGroupUtils.USIMGroup.deleteUSIMGroupMember(slotId, simIndexArray[i], ugrpId);
                        Log.i(TAG, "[deleteGroup]slotId:" + slotId + "ugrpId:"
                                + ugrpId + "|simIndex:" + simIndexArray[i] + "|Result:" + ret);
                        i++;
                    }
                }
                //Delete USIM group
