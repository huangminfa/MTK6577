...
/*#ifndef ANDROID_DEFAULT_CODE
	if (bytes <= 64*1024)
	{
		mMaxFileSizeBytes = bytes;//Modified by hai.li for CTS test pass
		LOGW("Max file size is a bit small: %lld bytes", bytes);
	}
	else
	{
		mMaxFileSizeBytes = bytes - 64*1024;  //Modified by hai.li @2010-12-25 to reserve larger space for larger bitrate support
    	LOGD("Max file size is %lld-65536 = %lld" ,bytes ,bytes - 64*1024);
	}
#else*/
...
// 
		...
		LOGD(" key =  rtp-target-addresses");
        if (mOutputFormat != OUTPUT_FORMAT_RTP_AVP) {
            LOGE("Bad parameter!!! %s for non-rtp writer %d", value.string(), mOutputFormat);
            return BAD_VALUE;
        }
        LOGD("set rtp-target-addresses = %s success!!!", value.string());

        mRTPTarget.setTo(value.string());
        return OK;
	} else if(key == "media-param-pause") {
		int64_t isPause = 0;
		LOGD(" key =  media-param-pause \n");
		if (safe_strtoi64(value.string(), &isPause) && (1 == isPause)) {
			if (pause() != OK) {
				LOGD("Pause return error");
		...
		{
			LOGE("Bad parameter!!! isPause = %lld", isPause);
			return BAD_VALUE;
		} 
#ifdef MTK_S3D_SUPPORT
	} else if (key == "video-param-stereo-mode") {
	    LOGD(" key = video-param-stereo-mode");
		if ((safe_strtoi32(value.string(), (int32_t *)&mVideoStereoMode)) &&
			...
			LOGD("video-param-stereo-mode = %d", mVideoStereoMode);
			return OK;
		}
		else {
			LOGE("Bad parameter!!! mVideoStereoMode = %d", mVideoStereoMode);
			return BAD_VALUE;
		} 
#endif
	} else if (key == "media-param-tag-artist") {
		LOGD(" key = media-param-tag-artist");
		LOGD(" set media-param-tag-artist = %s success!!!", value.string());

		mArtistTag.setTo(value.string());
		return OK;
	} else if (key == "media-param-tag-album") {
		LOGD(" key = media-param-tag-album");
		LOGD(" set media-param-tag-album = %s success!!!", value.string());
...
status_t StagefrightRecorder::startPCMRecording() {
    ...
    if (mOutputFormat == OUTPUT_FORMAT_WAV) {
        if (mAudioEncoder != AUDIO_ENCODER_PCM) {
            LOGE("Invalid encoder %d used for PCM recording",mAudioEncoder);
            return BAD_VALUE;
        }
        //AUDIO_SOURCE_MIC 8K,16K;AUDIO_SOURCE_I2S:8K~48K
        if (mSampleRate < 8000 || mSampleRate > 48000) {
            LOGE("Invalid sampling rate %d used for PCM recording",mSampleRate);
            return BAD_VALUE;
        }
    } 
    if (mAudioChannels != 1 && mAudioChannels != 2) {
        LOGE("Invalid number of audio channels %d used for PCM recording",
                mAudioChannels);
        return BAD_VALUE;
    }
    ...
    LOGE("StagefrightRecorder::startPCMRecording");
...
status_t StagefrightRecorder::startOGGRecording()
{
    ...
    {
            LOGE("Invalid encoder %d used for OGG recording", mAudioEncoder);
            return BAD_VALUE;
    }
    if (mSampleRate < 8000 || mSampleRate > 48000) 
    {
            LOGE("Invalid sampling rate %d used for OGG vorbis recording",mSampleRate);
            return BAD_VALUE;
    }
   
    if (mAudioChannels != 1 && mAudioChannels != 2) 
    {
        LOGE("Invalid number of audio channels %d used for ogg recording",mAudioChannels);
        return BAD_VALUE;
    }
...
//MTK80721 HDRecord 2011-12-23
    ...
    if (mParams.find("HDREC_SET_VOICE_MODE") > -1 || mParams.find("HDREC_SET_VIDEO_MODE") > -1)
    {
        audioSource = new AudioSource(mAudioSource, mSampleRate, mParams, mAudioChannels);
    }
    else
    {
        audioSource = new AudioSource(mAudioSource, mSampleRate, mAudioChannels);
    }
#else
...
#ifndef ANDROID_DEFAULT_CODE
    LOGV("mFrameRate = %d, minFrameRate = %d, maxFrameRate = %d", mFrameRate, minFrameRate, maxFrameRate);
#endif  //#ifndef ANDROID_DEFAULT_CODE
}
...
#ifndef ANDROID_DEFAULT_CODE
    LOGV("mVideoBitRate = %d, minBitRate = %d, maxBitRate = %d", mVideoBitRate, minBitRate, maxBitRate);
#endif  //#ifndef ANDROID_DEFAULT_CODE
}
...
#ifndef ANDROID_DEFAULT_CODE
    LOGV("mVideoWidth = %d, minFrameWidth = %d, maxFrameWidth = %d", mVideoWidth, minFrameWidth, maxFrameWidth);
#endif  //#ifndef ANDROID_DEFAULT_CODE
}
...
//-->Check video codec valid
	...
	if(VIDEO_ENCODER_H263 == mVideoEncoder) {//h263 encode only support these
		if (((mVideoWidth == 128) && (mVideoHeight == 96)) ||
			...
			LOGD("h263 size is OK, %dx%d", mVideoWidth, mVideoHeight);
		}
		else {
			mVideoEncoder = VIDEO_ENCODER_MPEG_4_SP;
			LOGW("Unsupport h263 size, switch to MPEG4, %dx%d", mVideoWidth, mVideoHeight);
		}
	}
	...
	if (VIDEO_ENCODER_H264 == mVideoEncoder)
	{
		...
		if ((mVideoWidth <= maxWidth) && (mVideoHeight <= maxHeight)
			...
			LOGD("H264 size is ok, %dx%d", mVideoWidth, mVideoHeight);
		}
		else {
			mVideoEncoder = VIDEO_ENCODER_MPEG_4_SP;
			LOGW("Unsupport h264 size, switch to MPEG4, %dx%d", mVideoWidth, mVideoHeight);
		}
	}
//<--
#endif
...
#ifndef ANDROID_DEFAULT_CODE
    LOGV("mVideoHeight = %d, minFrameHeight = %d, maxFrameHeight = %d", mVideoHeight, minFrameHeight, maxFrameHeight);
#endif  //#ifndef ANDROID_DEFAULT_CODE
}
...
status_t StagefrightRecorder::setupVideoEncoder(
	...
	int32_t camMemVa, camMemSize, camMemCount;
	...
	LOGD("camMemInfo.u4VdoBufCount=%d, camMemInfo.u4VdoBufSize=%d", camMemInfo->u4VdoBufCount, camMemInfo->u4VdoBufSize);
	for (int32_t i = 0; i < camMemInfo->u4VdoBufCount; i++)
	{
		camMemInfo->u4VdoBufVA[i] = camMemVa + camMemSize * i;//VA is continous
		LOGD("camMemInfo.u4VdoBufVA[%d]=%d", i, camMemInfo->u4VdoBufVA[i]);
	}
