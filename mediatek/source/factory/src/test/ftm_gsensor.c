...
static void *gsensor_update_iv_thread(void *priv)
{
    ...
    while (1) {
        ...
        len = snprintf(dat->info+len, sizeof(dat->info)-len, "%+6.3f %+6.3f %+6.3f\n%s (%s)\n%s: %s %s: %s\n%s: %s %s: %s\n%s: %s %s: %s\n", 
                 ...
                 (acc->tilt != TILT_UNKNOWN) ? ("OK") : ("NG"), gsensor_pos[acc->tilt],
                 gsensor_pos[TILT_X_POS], (acc->pos_chk & (1 << TILT_X_POS)) ? ("OK") : ("Testing"),
                 gsensor_pos[TILT_X_NEG], (acc->pos_chk & (1 << TILT_X_NEG)) ? ("OK") : ("Testing"),
                 gsensor_pos[TILT_Y_POS], (acc->pos_chk & (1 << TILT_Y_POS)) ? ("OK") : ("Testing"),
                 gsensor_pos[TILT_Y_NEG], (acc->pos_chk & (1 << TILT_Y_NEG)) ? ("OK") : ("Testing"),
                 gsensor_pos[TILT_Z_POS], (acc->pos_chk & (1 << TILT_Z_POS)) ? ("OK") : ("Testing"),
                 gsensor_pos[TILT_Z_NEG], (acc->pos_chk & (1 << TILT_Z_NEG)) ? ("OK") : ("Testing"));
		...
		{
            len += snprintf(dat->info+len, sizeof(dat->info)-len, "SelfTest: %s\n", status);   
        }
		if(len < 0)
		{
		   LOGE(TAG "%s: snprintf error \n", __FUNCTION__); 
        ...
        len += snprintf(dat->info+len, sizeof(dat->info)-len, "Statistic: %s\nMAX: %+6.3f %+6.3f %+6.3f\nMIN: %+6.3f %+6.3f %+6.3f\nMax-Min: %+6.3f %+6.3f %+6.3f\nAVG: %+6.3f %+6.3f %+6.3f\nSTD: %+6.3f %+6.3f %+6.3f\n",
                 (!acc->statistics) ? ("DOING") : ("DONE"),
        ...
        len += snprintf(dat->info+len, sizeof(dat->info)-len, "Max-Min should be in the range of 0 ~ 0.2\n");   
