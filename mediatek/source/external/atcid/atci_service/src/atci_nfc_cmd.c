...
void ATCMD_NFC_Read_CNF(void)
{
    ...
    int retry_count = 0;
    // Read resonse
    LOGI("ATCMD_NFC_Read_CNF:NFC read start");
    while(retry_count < 5)
    {
        ...
        LOGI("retry_count=%d,rec_bytes=%d",retry_count, rec_bytes);
        if (rec_bytes > 0)
        {
            // check msg id
            LOGI("ATCMD_NFC_Read_CNF:NFC read (msg_id,dest_mod_id) = (%d,%d)",nfc_ilm_rec.msg_id, nfc_ilm_rec.dest_mod_id);
            ...
            if ((nfc_ilm_rec.msg_id == MSG_ID_NFC_TEST_RSP) && (nfc_ilm_rec.dest_mod_id == MOD_NFC_APP))
            {
                ...
                LOGI("ATCMD_NFC_Read_CNF:NFC read msg_type=%d,length=%d", nfc_msg.msg_type,nfc_msg_length);
                ...
                case MSG_ID_NFC_SETTING_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_SETTING =%d/%d/%d/%d/%d/%d/%d/%d/%d/",
                ...
                case MSG_ID_NFC_NOTIFICATION_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_DISCOVERY =%d/",
                ...
                case MSG_ID_NFC_SE_SET_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_SECURE_ELEMENT =%d/",
                ...
                case MSG_ID_NFC_DISCOVERY_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_DISCOVERY =%d/%d/",
                ...
                case MSG_ID_NFC_TAG_READ_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_TAG_READ =%d/%d/",
                ...
                case MSG_ID_NFC_TAG_WRITE_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_TAG_WRITE =%d/%d/",
                ...
                case MSG_ID_NFC_TAG_DISCONN_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_TAG_DISCONN =%d/",
                ...
                case MSG_ID_NFC_TAG_F2NDEF_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_TAG_FORMAT_NDEF =%d/",
                ...
                case MSG_ID_NFC_TAG_RAWCOM_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_TAG_RAW_COMM =%d/%d/",
                ...
                case MSG_ID_NFC_P2P_COMMUNICATION_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_P2P_COMM =%d/%d/",
                ...
                case MSG_ID_NFC_RD_COMMUNICATION_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_RD_COMM =%d/%d/",
                ...
                case MSG_ID_NFC_TX_ALWAYSON_TEST_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_TX_ALWAYSON_TEST =%d/",
                ...
                case MSG_ID_NFC_TX_ALWAYSON_WO_ACK_TEST_RSP:
                {
                    ...
                    memcpy(&nfc_cnf.result.m_script_cnf, (nfc_script_response*)(nfc_ilm_rec.local_para_ptr + nfc_msg_length), sizeof(nfc_script_response));
                    LOGI("AT_NFC_CMD:NFC NFC_OP_TX_ALWAYSON_WO_ACK_TEST =%d/",
                         nfc_cnf.result.m_script_cnf.result);
                    break;
                }
                case MSG_ID_NFC_CARD_EMULATION_MODE_TEST_RSP:
                {
                    ...
                    memcpy(&nfc_cnf.result.m_script_cnf, (nfc_script_response*)(nfc_ilm_rec.local_para_ptr + nfc_msg_length), sizeof(nfc_script_response));
                    LOGI("AT_NFC_CMD:NFC NFC_OP_CARD_MODE_TEST =%d/",
                         nfc_cnf.result.m_script_cnf.result);
                    break;
                }
                case MSG_ID_NFC_READER_MODE_TEST_RSP:
                {
                    ...
                    memcpy(&nfc_cnf.result.m_script_cnf, (nfc_script_response*)(nfc_ilm_rec.local_para_ptr + nfc_msg_length), sizeof(nfc_script_response));
                    LOGI("AT_NFC_CMD:NFC NFC_OP_READER_MODE_TEST =%d/",
                         nfc_cnf.result.m_script_cnf.result);
                    break;
                }
                case MSG_ID_NFC_P2P_MODE_TEST_RSP:
                {
                    ...
                    memcpy(&nfc_cnf.result.m_script_cnf, (nfc_script_response*)(nfc_ilm_rec.local_para_ptr + nfc_msg_length), sizeof(nfc_script_response));
                    LOGI("AT_NFC_CMD:NFC NFC_OP_P2P_MODE_TEST =%d/",
                         nfc_cnf.result.m_script_cnf.result);
                    break;
                }
                ...
                case MSG_ID_NFC_SWP_SELF_TEST_RSP:
                {
                    ...
                    memcpy(&nfc_cnf.result.m_script_cnf, (nfc_script_response*)(nfc_ilm_rec.local_para_ptr + nfc_msg_length), sizeof(nfc_script_response));
                    LOGI("AT_NFC_CMD:NFC NFC_OP_SWP_SELF_TEST =%d/",
                         nfc_cnf.result.m_script_cnf.result);
                    break;
                }
                case MSG_ID_NFC_ANTENNA_SELF_TEST_RSP:
                {
                    ...
                    memcpy(&nfc_cnf.result.m_script_cnf, (nfc_script_response*)(nfc_ilm_rec.local_para_ptr + nfc_msg_length), sizeof(nfc_script_response));
                    LOGI("AT_NFC_CMD:NFC NFC_OP_ANTENNA_SELF_TEST =%d/",
                         nfc_cnf.result.m_script_cnf.result);
                    break;
                }
                case MSG_ID_NFC_TAG_UID_RW_RSP:
                {
                    ...
                    LOGI("AT_NFC_CMD:NFC NFC_OP_TAG_UID_RW =%d/%d/%X/%X/%X/%X/",
                ...
                {
                    fgSupport = 0;
                    LOGI("AT_NFC_CMD:Don't support CNF CMD %d",nfc_msg.msg_type);
                    break;
                }
                }
                if (fgSupport == 1)
                {
                    LOGI("AT_NFC_CMD:NFC read nfc_cnf.op=%d,nfc_msg.msg_type=%d", nfc_cnf.op,nfc_msg.msg_type);
                    nfc_cnf.status = 0;
                    break;
                }
                else
                {
                    LOGI("AT_NFC_CMD:Don't Write to PC MSGID,%d,",nfc_msg.msg_type);
                }
            }
            else
            {
                LOGI("AT_NFC_CMD:Don't support MSGID,%d,DestID,%d",nfc_ilm_rec.msg_id, nfc_ilm_rec.dest_mod_id);
            }
        }
        retry_count++;
    }
...
int NFC_Socket_Open(void)
{
    ...
    if ((pid = fork()) < 0)
    {
        LOGE("NFC_Socket_Open: fork fails: %d (%s)\n", errno, strerror(errno));
        return (-2);
    }
    ...
    {
        LOGE("META_NFC_init: ERROR opening socket");
        return (-4);
    }
    server = gethostbyname("127.0.0.1");
    if (server == NULL) {
        LOGE("META_NFC_init: ERROR, no such host\n");
        return (-5);
    }
    ...
    {
        LOGE("META_NFC_init: ERROR connecting");
        return (-6);
    }

#if 0
    LOGI("META_NFC_init: create read command thread\n");
...
void NFC_Socket_Close()
{
    ...
    LOGI("NFC_Socket_Close: kill: %s\n", "/system/xbin/nfcservice");
    err = execl("kill /system/xbin/nfcservice", "nfcservice", NULL);
    if (err == -1)
    {
        LOGE("META_NFC_init: kill error: %s\n", strerror(errno));
    }
#endif
    return;
}
...
void AT_NFC_CMD(ilm_struct* nfc_ilm_req_ptr)
{
    ...
    if ( ret <= 0)
    {
        LOGE("AT_NFC_CMD:write failure,%d",ret);
        return;
    }
    else
    {
        LOGI("AT_NFC_CMD:write CMD done,%d",ret);
    }
    return;
}
...
void ATCMD_NFC_OP(NFC_REQ *req)
{
    ...
    switch(req->op)
    {
        LOGI("ATCMD_NFC_OP:NFC request op=%d", req->op);
#if 0
    case NFC_OP_SETTING:
    {
        ...
        LOGI("META_NFC_OP:NFC msg_type,msg_length = (%d,%d)", nfc_msg.msg_type, nfc_msg.msg_length);
...
int nfc_cmd_handler(char* cmdline, ATOP_t at_op, char* response)
{
    ...
    switch(at_op)
    {
        ...
        case AT_SET_OP:
        {
            ...
            LOGD("nfc_cmd_handler action ID:%d", actinoID);
