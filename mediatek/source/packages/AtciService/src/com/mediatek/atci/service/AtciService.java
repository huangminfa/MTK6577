...
public class AtciService extends Service {
    ...
    static final String SOCKET_NAME_ATCI = "atci-service-framework";
 ...
 // frequency is float,in database as int, use to convert
	...
	protected void initializeAtOSVer() {
		...
		mAtParser.register("%OSVER", new AtCommandHandler() {
            private AtCommandResult checkOSVer() {
				...
				try {
					OSVersion = SystemProperties.get("ro.build.version.release", "ERROR");
    ...
    protected void initializeAtTelephony() {
        ...
        mAtParser.register("%FLIGHT", new AtCommandHandler() {
            private AtCommandResult isAirplaneModeOn() {
                int mode = Settings.System.getInt(mContext.getContentResolver(), Settings.System.AIRPLANE_MODE_ON, 0);
        ...
        mAtParser.register("%ECALL", new AtCommandHandler() {
                        ...
                        if (FeatureOption.MTK_GEMINI_SUPPORT){
                            try{
                                if(callId == 0){
                                    ...
                                    if(null != iTelephony && !iTelephony.isIdleGemini(Phone.GEMINI_SIM_1)){
                                        iTelephony.endCallGemini(Phone.GEMINI_SIM_1);
                                    }else if(null != iTelephony && !iTelephony.isIdleGemini(Phone.GEMINI_SIM_2)){
                                        iTelephony.endCallGemini(Phone.GEMINI_SIM_2);
                                    }
                                }else if(callId == 1 || callId == 2){
                                    int simId = (callId == 1) ? Phone.GEMINI_SIM_1 : Phone.GEMINI_SIM_2;
            ...
            mAtParser.register("%BTAD", new AtCommandHandler() {
                ...
                @Override
                public AtCommandResult handleTestCommand() {
                    return new AtCommandResult(formatResponse("AT%BTAD=[BD ADDR : 12 HEX nibble => 6 Bytes]"));
                }
                ...
                public AtCommandResult handleSetCommand(Object[] args) {
                    ...
                    if (args.length == 12) {
                        StringBuilder addr = new StringBuilder(12);
                        String str_addr;
                        for(Object obj : args){
                            addr.append(obj);
                        }
                        str_addr = addr.toString().substring(0, 12);
                        Log.d(LOG_TAG, "Addr="+str_addr);
                        if( writeBTAddressNative(str_addr) == false ){
                            ret = -1;
                        }
                    }else{
                        ret = -1;
            ...
            mAtParser.register("%BTTM", new AtCommandHandler() {
                ...
                @Override
                public AtCommandResult handleReadCommand() {
                    return new AtCommandResult(formatResponse((queryBTTestModeNative()==false) ? "1" : "2"));
                }
    ...
    protected void initializeAtFMRadio() {
        mAtParser.register("%FMR", new AtCommandHandler() {
            ...
            private int getFMRadioState() {
                if ((null == mFMRadioService) || (false == mIsFMRadioServiceBinded) ) {
                    ...
                    if (!isPowerUp()) {
                        ...
                        if ((true == mIsFMRadioDoSeek) || (true == mIsFMRadioSeeking)) {
                            if (mIsFMRadioSeekUp) {
            ...
            public AtCommandResult handleSetCommand(Object[] args) {
                ...
                if (args.length == 1) {
                    ...
                    if (fmradioId >= 0 && fmradioId <= 4) {
                        Log.d(LOG_TAG, "Make ECC call with " + fmradioId);
                         ComponentName componentName = startService(new Intent(AtciService.this,FMRadioService.class));                        
                              ...
                              Log.e(LOG_TAG, "FMRadio: can't start FMRadio service"); 
                                  ...
                                  if (ret) {
                                      ...
                                      case 1:
                                          ...
                                          mIsFMRadioDoSeek = false;
                                          if (!isPowerUp()) {
                                              powerUp((float) FIRST_FREQUENCY / FREQUENCY_CONVERT_RATE);
                                          } else {
                                              int frequency = getFrequency();
                                              Log.i(LOG_TAG, "FMRadio frequency=" + frequency);
                                              if (FIRST_FREQUENCY != frequency) {
                                                  tune((float) FIRST_FREQUENCY / FREQUENCY_CONVERT_RATE);
                                              } else {
                                      ...
                                      case 2:
                                          ...
                                          mIsFMRadioDoSeek = false;
                                          if (!isPowerUp()) {
                                              powerUp((float) SECOND_FREQUENCY / FREQUENCY_CONVERT_RATE);
                                          } else {
                                              int frequency = getFrequency();
                                              Log.i(LOG_TAG, "FMRadio frequency=" + frequency);
                                              if (SECOND_FREQUENCY != frequency) {
                                                  tune((float) SECOND_FREQUENCY / FREQUENCY_CONVERT_RATE);
                                              } else {
    ...
    protected void initializeAtLanguage() {
        ...
        mAtParser.register("%LANG", new AtCommandHandler() {
            /*
                       ...
                       * -   In case of the model in which AT%LANG command is not implemented :  ERROR 
                       */            
            @Override
            public AtCommandResult handleActionCommand() {
            ...
            /*
                       ...
                       * You should obey the standard of Language name managed by GSM.(Specify loaded Language to the PDM-requesting report.)
    ...
    protected void initializeAtMp3Play() {
        //private static final String EMERGENCY_CALL_ACTION="android.location.agps.EMERGENCY_CALL";
        //private static final boolean DBG = true;
        //AT% Commands Name : MPT Description : Measuring of MP3 output level & quality through Phone MP3 DSP(including Stereo)
        ...
        mAtParser.register("%MPT", new AtCommandHandler() {
            private AtCommandResult QueryTestMode() {                
                ...
                String response = "" + mMp3PlayerMode;//formatResponse("" + mMp3PlayerMode + "\nOK!");
                return new AtCommandResult(response);
            }
    ...
    protected void initializeAtNoSleep() {
        ...
        mAtParser.register("%NOSLEEP", new AtCommandHandler() {
			...
			/*   Action Commands :
			  ...
			  *   If it is possible to enter Sleep Mode, returns"0", If it is No Sleep Mode, returns"1".
			  */
			@Override
            public AtCommandResult handleActionCommand() {
	...
	protected void initializeAtSystemCall() {
        mAtParser.register("+SN", new AtCommandHandler() {
            ...
            public AtCommandResult handleActionCommand() {
            	...
            	String response = "";
            	try{ 
            		sn = ((TelephonyManager)mContext.getSystemService(Context.TELEPHONY_SERVICE)).getSN();
                    response = "SN: " + sn + "\r\nOK\r\n";
            	} catch (Exception e) {
            		response = "ERROR\r\n";
            	}
...
//                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
            ...
            public AtCommandResult handleSetCommand(Object[] args) {
                ...
                if (1 == setNumber) {
                    mCameraOn = 1;
                    if (true == startCameraCapturePicture()) {
                        return cameraOk();
                    } else {
...
//                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
            ...
            public AtCommandResult handleSetCommand(Object[] args) {
                ...
                if (setNumber < 0 || setNumber > 7) {
                    Log.e("AtciService", "Error set number " + setNumber);
                    return new AtCommandResult(formatResponse("AVR ERROR"));
                }

                if (setNumber >= 6 && setNumber <= 7) {
                    Log.w("AtciService", "not supported function tested!");
                    return notSupport();
                }
                ...
                if (1 == setNumber) {
                    mCameraAVROn = 1;
                    if (true == startCameraCaptureVideo()) {
                        return cameraOk();
                    } else {
                        return new AtCommandResult(formatResponse("CAMERA INITIAL FAILED"));
                    }
    ...
    protected void handleInput(String input) {
        ...
        if (DBG) Log.d(LOG_TAG, "Processing " + input + " took " +
                           (System.currentTimeMillis() - timestamp) + " ms");

        if (result.getResultCode() == AtCommandResult.ERROR) {
            Log.i(LOG_TAG, "Error processing <" + input + "> with result <" + result.toString() + ">");
        }
    ...
    class AtciReceiver implements Runnable {
        ...
        AtciReceiver() {
            buffer = new byte[ATCI_MAX_BUFFER_BYTES];
        }
            ...
            for(;;) {
                ...
                try {
                    ...
                    for(;;) {
                        ...
                        if(countRead > 0) {
                            handleInput(new String(buffer, 0, countRead));
                        }
                    }
    ...
    private BroadcastReceiver mEccStateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (EMERGENCY_CALL_ACTION.equals(action)) {                                               
                ...
                String call_number= intent.getStringExtra("Call_Number");
                
                Log.i(LOG_TAG, "Receive ECC intent call state:" + state + " with number:" + call_number);
                if (call_number!=null ){
    ...
    class SeekThread extends Thread {
        ...
        public boolean seekDirection = false;
        ...
        public void run() {
            ...
            float fStation = seek((float) currentStation / FREQUENCY_CONVERT_RATE, seekDirection);
            ...
            if (iStation >= HIGHEST_STATION || iStation <= LOWEST_STATION) {
                ...
                if (!seekDirection) {
                    fStation = seek((float) (HIGHEST_STATION) / FREQUENCY_CONVERT_RATE, seekDirection);
                    iStation = (int) (fStation * FREQUENCY_CONVERT_RATE);
                } else {
                    fStation = seek((float) (LOWEST_STATION) / FREQUENCY_CONVERT_RATE, seekDirection);
                    iStation = (int) (fStation * FREQUENCY_CONVERT_RATE);
                }
            }
            if (iStation >= HIGHEST_STATION || iStation <= LOWEST_STATION) {
