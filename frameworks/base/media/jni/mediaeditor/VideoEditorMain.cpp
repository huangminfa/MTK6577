...
// commands to enable memory leak check for videoeditor (zygote)
// adb shell setprop libc.debug.malloc 1
// adb shell setprop media.videoeditor.leak 1
// adb shell stop
// adb shell start
// or use ve.py startleak
#include <MemoryDumper.h>
#include <cutils/properties.h>
#define VIDEO_EDITOR_LEAK_DIRECTORY "/sdcard/videoeditor/leak"
using android::MemoryDumper;
static bool gVideoEditorLeakCheck = false;

static MemoryDumper* getMemoryDumper() {
    static MemoryDumper* dumper = NULL;
    if (dumper == NULL) {
        const size_t FILENAME_SIZE = 64;
        char dumpFileName[FILENAME_SIZE];
        memset(dumpFileName, 0, FILENAME_SIZE);
        snprintf(dumpFileName, FILENAME_SIZE-1, VIDEO_EDITOR_LEAK_DIRECTORY "/memstatus_%d", getpid());
        dumper = new MemoryDumper(dumpFileName);
    }
    return dumper;
}

static void doMemoryDumper() {
    if (gVideoEditorLeakCheck) {
        MemoryDumper* dumper = getMemoryDumper();
        if (dumper == NULL) {
            LOGE("cannot get MemoryDumper");
        } else {
            dumper->dumpHeap();
        }
    }
...
static void checkMemoryDumper() {
    char value[PROPERTY_VALUE_MAX];
    if (property_get("media.videoeditor.leak", value, NULL)
            && (!strcmp(value, "1") || !strcasecmp(value, "true"))) {
        gVideoEditorLeakCheck = true;
        system("/system/bin/mkdir -p " VIDEO_EDITOR_LEAK_DIRECTORY);
        LOGI("memory leak check is enabled");
    } else {
        gVideoEditorLeakCheck = false;
    }
}
#endif
                                    ...
                                    M4OSA_Char* pcmfilePath )
{
	...
	if(pContext->bgstate != ManualMCSState_STOPPING){
		...
		LOGV("videoEditor_generateAudio bgstate %d",pContext->bgstate);
	}
#endif
	...
	if(pContext->bgstate != ManualMCSState_STOPPING){
		...
		LOGV("videoEditor_generateAudio change to bgstate %d",pContext->bgstate);
	}
#endif
    ...
    LOGD("LVME_generateAudio Current progress is =%d", curProgress);
    pEnv->CallVoidMethod(pContext->engine,
    ...
    do {
        ...
        if (result != M4NO_ERROR) {
            LOGD("LVME_generateAudio M4MCS_step returned 0x%x",result);
    ...
    /*- PROGRESS CB */
#ifdef STOPBGAUDIO
	LOGV("videoEditor_generateAudio change to bgstate %d  result %d",pContext->bgstate,result);
	if((result == M4NO_ERROR)&&(pContext->bgstate==ManualMCSState_STOPPING)){
		...
		pContext->bgstate = ManualMCSState_INITIALIZED;
		LOGD("videoEditor_generateAudio stop return %d",result);
		return result;	
	}
#endif
    ...
    if (audioSettingObject != M4OSA_NULL) {
        ...
        if (regenerateAudio) {
...
#ifdef STOPBGAUDIO			
			LOGD("Calling Generate Audio now");
#endif
			...
			if(M4NO_ERROR == result){
				LOGD("videoEditor_populateSettings bgstate %d",pContext->bgstate);
				if(pContext->bgstate == ManualMCSState_FINISHED){
					pContext->bgstate = ManualMCSState_INITIALIZED;
				}else{
				    LOGD("videoEditor_populateSettings bg audio no finished!");
					needToBeLoaded == false;
    ...
    if (initialized)
    {
        ...
        if (pContext != NULL)
        {
            ...
            pContext->initParams.pTempPath = (M4OSA_Char *)
                 M4OSA_32bitAlignedMalloc(strlen((const char *)tmpString) + 2, 0x0,
                                                 (M4OSA_Char *)"tempPath");
#else
            pContext->initParams.pTempPath = (M4OSA_Char *)
...
videoEditor_generateClip(
    ...
    if (pContext->pEditSettings != NULL 
        ...
        LOGW("H264 export is unsupported now");
                ...
                jobject                             thiz)
{
    ...
    LOGD("videoEditor_stopEncoding START");
...
#ifdef STOPBGAUDIO
	if (stopped) {
		LOGD("videoEditor_stopEncoding pContext->bgstate %d",pContext->bgstate);
		if(pContext->bgstate != ManualMCSState_INITIALIZED){
	        // Set the state to initialized.
	        pContext->bgstate = ManualMCSState_INITIALIZED;			
		}else{
			pContext->state = state;
			LOGD("videoEditor_stopEncoding BGAudio Stop!return pContext->state %d",pContext->state);
			return ;
		}
	}
#endif
	...
	LOGD("videoEditor_stopEncoding  End");
}
                ...
                jobject                             thiz)
{
    ...
    if (released)
    {
		...
		if (pContext->bgstate != ManualMCSState_INITIALIZED){	
			pContext->bgstate = ManualMCSState_STOPPING;
			LOGD("videoEditor_release ManualMCSState_STOPPING");
			Mutex::Autolock autoLock(pContext->mLock);
		}		
#endif
        // Reset the context.
