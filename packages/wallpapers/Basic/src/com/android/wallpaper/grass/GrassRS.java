...
class GrassRS extends RenderScriptScene {
	...
	private void GetDensityMode()
	{
		...
		case DisplayMetrics.DENSITY_LOW:
			DensityMode = DENSITY_MODE_LOW;
			break;
			
		case DisplayMetrics.DENSITY_MEDIUM:
			DensityMode = DENSITY_MODE_MEDIUM;
			break;
		case DisplayMetrics.DENSITY_HIGH:
			DensityMode = DENSITY_MODE_HIGH;
			break;
		}
		...
		if (DensityMode == -1)
		{
			...
			{
				DensityMode = DENSITY_MODE_LOWER;
			}
			else
			{
				DensityMode = DENSITY_MODE_HIGHER;
			}
		}

		Log.d(LOG_TAG, "*******************************************");
		Log.d(LOG_TAG, String.format("**** DPI(%d) Mode(%d) ****", _dpi, DensityMode));
		Log.d(LOG_TAG, "*******************************************");
	}
    ...
    private void createBlade(ScriptField_Blade.Item blades) {
        ...
        blades.lengthX = (random(BladeBaseX) + 3.0f) * TESSELATION * size;
        blades.lengthY = (random(BladeBaseY) + 2.0f) * TESSELATION * size;
    ...
    private class TimezoneTracker extends BroadcastReceiver {
        public void onReceive(Context context, Intent intent) {
            ScriptC script = getScript();
            if (script != null) {
                script.setTimeZone(Time.getCurrentTimezone());
            }
            synchronized (mLock) {
            	RenderScript rs = getRS();
        		if (rs != null && rs.isAlive()) {
