...
bool SniffDRM(
    ...
    if (decryptHandle != NULL) {
        if (decryptHandle->decryptApiType == DecryptApiType::CONTAINER_BASED) {
            //*mimeType = String8("drm+container_based+") + decryptHandle->mimeType; // original code
            *mimeType =
                String8("drm+container_based+")
                + DrmMtkUtil::toCommonMime(decryptHandle->mimeType);
            // for OMA DRM v1 implementation, the confidence is set to 0.01f
            //   so that it's smaller that any other confidence value
            LOGD("SniffDRM: this is an OMA DRM v1 implementation file");
            *confidence = 0.01f;
            return true;
