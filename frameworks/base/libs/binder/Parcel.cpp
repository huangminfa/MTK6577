...
status_t Parcel::writeDupFileDescriptor(int fd)
{
    int dupFd = dup(fd);
    if (dupFd < 0) {
        LOGE("writeDupFileDescriptor: error %d dup fd %d\n", errno, fd);
    }
    return writeFileDescriptor(dupFd, true /*takeOwnership*/);
}
...
int32_t Parcel::readExceptionCode() const
{
  ...
  if (exception_code == EX_HAS_REPLY_HEADER) {
    const int header_pos = dataPosition(); // Google bug. We need to keep this position, since the remaining data includes "header" and "payload"
    int32_t header_size = readAligned<int32_t>();
    // Skip over fat responses headers.  Not used (or propagated) in
    // native code
    setDataPosition(header_pos + header_size); // skip data including "header" (length = sizeof(int32_t)) and "payload" (length = header_size)
