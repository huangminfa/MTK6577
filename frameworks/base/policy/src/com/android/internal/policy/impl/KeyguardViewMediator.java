...
public class KeyguardViewMediator implements KeyguardViewCallback,
        KeyguardUpdateMonitor.deviceInfoCallback, KeyguardUpdateMonitor.SimStateCallback {
 ...
 // IPO INTENT
    ...
    public void onScreenTurnedOff(int why) {
        synchronized (this) {
            mScreenOn = false;
            if (DBG_WAKE) Log.d(TAG, "onScreenTurnedOff(" + why + ")");
            ...
            if (mExitSecureCallback != null) {
                ...
                Xlog.d(TAG, "on screen turned off, we should show keyguard immediately, because it doesn't destroyed, isReady="+isReady);
                if (isReady) {
    ...
    public void onScreenTurnedOn(KeyguardViewManager.ShowListener showListener) {
        synchronized (this) {
            ...
            mKeyguardViewManager.setScreenStatus(mScreenOn);
            mDelayedShowingSequence++;
            if (DBG_WAKE) Log.d(TAG, "onScreenTurnedOn, seq = " + mDelayedShowingSequence);
    ...
    public void setKeyguardEnabled(boolean enabled) {
        synchronized (this) {
            if (DBG_WAKE) Log.d(TAG, "setKeyguardEnabled(" + enabled + ")");
    ...
    //used for PowerManagerService.java
    public static boolean isKeyguardNotShowing(){
        if(DEBUG) Xlog.d(TAG, " mExternallyEnabled=" + mExternallyEnabled + " mShowing=" + mShowing);
    ...
    private void resetStateLocked() {
        if (DEBUG) Xlog.d(TAG, "resetStateLocked");
    ...
    private void verifyUnlockLocked() {
        if (DEBUG) Xlog.d(TAG, "verifyUnlockLocked");
    ...
    private void notifyScreenOnLocked(KeyguardViewManager.ShowListener showListener) {
        if (DEBUG) Xlog.d(TAG, "notifyScreenOnLocked");
    ...
    private void wakeWhenReadyLocked(int keyCode) {
        if (true || DBG_WAKE) Xlog.d(TAG, "wakeWhenReadyLocked(" + keyCode + ")");
    ...
    private void hideLocked() {
        if (DEBUG) Xlog.d(TAG, "hideLocked");
    ...
    public void onSimStateChanged(IccCard.State simState) {
        if (DEBUG) Xlog.d(TAG, "onSimStateChanged: " + simState);
        ...
        switch (simState) {
            ...
            case NOT_READY:
                synchronized (this) {
                    if (isShowing() && IccCard.State.PIN_REQUIRED == mUpdateMonitor.getLastSimState(Phone.GEMINI_SIM_1)) {
                        resetStateLocked();
                    }
                }
                break;
    ...
    public void onSimStateChangedGemini(IccCard.State simState, int simId) {
        if (DEBUG) Xlog.d(TAG, "onSimStateChangedGemini: " + simState + ",simId = " + simId);
        ...
        switch (simState) {
            case ABSENT:
                ...
                if (!mUpdateMonitor.isDeviceProvisioned()) {
                    if (!isShowing()) {
                        if (DEBUG) Xlog.d(TAG, "INTENT_VALUE_ICC_ABSENT and keygaurd isn't showing, we need "
            ...
            case PIN_REQUIRED:
            case PUK_REQUIRED:{
                if (IccCard.State.PIN_REQUIRED == simState && mUpdateMonitor.getPINDismissFlag(simId, true)){
                   Xlog.i(TAG, "We have dismissed PIN, so, we exit from PINPUK_REQUIRED");
                   //return ; 
                }
                if (IccCard.State.PUK_REQUIRED == simState && mUpdateMonitor.getPINDismissFlag(simId, false)){
                   Xlog.i(TAG, "We have dismissed PUK, so, we exit from PINPUK_REQUIRED");
                   //return ; 
                }
                if (!isShowing()) {
                    if (DEBUG) Xlog.d(TAG, "INTENT_VALUE_ICC_LOCKED and keygaurd isn't showing, we need "
                            + "to show the keyguard so the user can enter their sim pin");
                    doKeyguardLocked();
                } else {
                    resetStateLocked();
            ...
            case NOT_READY:
                if (isShowing() && IccCard.State.PIN_REQUIRED == mUpdateMonitor.getLastSimState(simId)) {
                    resetStateLocked();
                }
                break;
        }
    ...
    private BroadcastReceiver mBroadCastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(DELAYED_KEYGUARD_ACTION)) {
                ...
                if (DEBUG) Xlog.d(TAG, "received DELAYED_KEYGUARD_ACTION with seq = "
    ...
    public void pokeWakelock(int holdMs) {
        synchronized (this) {
            if (DBG_WAKE) Xlog.d(TAG, "pokeWakelock(" + holdMs + ")");
            mWakeLock.acquire();
    ...
    public void keyguardDone(boolean authenticated, boolean wakeup) {
        synchronized (this) {
            ...
            Xlog.d(TAG, "keyguardDone(" + authenticated + ")");
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case KEYGUARD_TIMEOUT:
                    synchronized (KeyguardViewMediator.this) {
                        Log.i(TAG, "doKeyguardLocked, because:KEYGUARD_TIMEOUT");
                        doKeyguardLocked();
                    }
                    break;
                case KEYGUARD_RELAYOUT:
                    handleKeyguardReLayout(msg.arg1 != 0);
                    break;
            }
        }
    };
    ...
    private void handleKeyguardDone(boolean wakeup) {
        Xlog.d(TAG, "handleKeyguardDone, wakeup="+wakeup);
        handleHide();
        if (wakeup) {
        ...
        if (mContext != null && ActivityManagerNative.isSystemReady()) {
            //If calling sendBroadcast() when ActivityManagerService is not yet "system ready", a Java Exception will pops up.
    ...
    private void handleKeyguardDoneDrawing() {
        synchronized(this) {
            if (DEBUG) Xlog.d(TAG, "handleKeyguardDoneDrawing");
            if (mWaitingUntilKeyguardVisible) {
                if (DEBUG) Xlog.d(TAG, "handleKeyguardDoneDrawing: notifying mWaitingUntilKeyguardVisible");
    ...
    private void handleTimeout(int seq) {
        synchronized (KeyguardViewMediator.this) {
            if (DBG_WAKE) Xlog.d(TAG, "handleTimeout, seq="+seq+", mWakelockSequence="+mWakelockSequence);
    ...
    private void adjustUserActivityLocked() {
        ...
        if (DEBUG) Xlog.d(TAG, "adjustUserActivityLocked mShowing: " + mShowing + " mHidden: " + mHidden);
    ...
    private void adjustUserActivityLockedByShow() {
        ...
        boolean enabled = !mShowing || mHidden;
        if (1 == mShowCount){
            if (DEBUG)Xlog.d(TAG, "we firstset enableUserActivity,enabled="+enabled);
            //need framework support
            //mRealPowerManager.enableUserActivityForce(enabled, false);
        }else{
            if (DEBUG)Xlog.d(TAG, "we will set the enableUserActivity enabled="+enabled);
            mRealPowerManager.enableUserActivity(enabled);
        }
        if (!enabled && mScreenOn) {
            // reinstate our short screen timeout policy
    ...
    private void handleWakeWhenReady(int keyCode) {
        synchronized (KeyguardViewMediator.this) {
            if (DBG_WAKE) Xlog.d(TAG, "handleWakeWhenReady(" + keyCode + ")");
    ...
    private void handleReset() {
        synchronized (KeyguardViewMediator.this) {
            if (DEBUG) Xlog.d(TAG, "handleReset");
    ...
    private void handleVerifyUnlock() {
        synchronized (KeyguardViewMediator.this) {
            if (DEBUG) Xlog.d(TAG, "handleVerifyUnlock");
    ...
    private void handleNotifyScreenOff() {
        synchronized (KeyguardViewMediator.this) {
            if (DEBUG) Xlog.d(TAG, "handleNotifyScreenOff");
    ...
    private void handleNotifyScreenOn(KeyguardViewManager.ShowListener showListener) {
        synchronized (KeyguardViewMediator.this) {
            if (DEBUG) Xlog.d(TAG, "handleNotifyScreenOn");
    ...
    private void handleKeyguardReLayout(boolean dmLock) {
        synchronized (KeyguardViewMediator.this) {
            if (DEBUG) Xlog.d(TAG, "handleKeyguardReLayout");
            mKeyguardViewManager.reLayoutScreen(dmLock);
        }
    }

    public void onDMKeyguardUpdate() {
        boolean dmLock = mUpdateMonitor.DM_IsLocked();
        Log.d(TAG, "onDMKeyguardUpdate():" + dmLock);
        if (dmLock) {
            mPM.goToSleep(SystemClock.uptimeMillis() + 1);
        }
