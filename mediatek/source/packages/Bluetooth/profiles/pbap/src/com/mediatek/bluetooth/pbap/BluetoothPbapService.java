...
import com.mediatek.bluetooth.pbap.BluetoothPbapServer;
import com.mediatek.bluetooth.pbap.BluetoothServerAuthorize;
import com.mediatek.bluetooth.pbap.BluetoothAuthenticating;
...
public class BluetoothPbapService extends Service {
	...
	public void onReceive(Context context, Intent intent) {
	    printLog("onReceive("+intent.getAction()+")");
            ...
            if(action.equals(PBAP_AUTHORIZE_RETURN_ACTION)) {
                ...
                if(res == BluetoothServerAuthorize.RESULT_USER_ACCEPT && 
                    mRemoteDevice != null){
	...
	public void handleMessage(Message msg) {
	    ...
	    switch (msg.what) {
            case BluetoothPbapServer.PBAP_AUTHORIZE_IND:
                ...
                if(mRemoteDevice == null || mRemoteDevice.getTrustState() != true){
            ...
            case PBAP_AUTH_TIMEOUT_IND:
                ...
                if(mPid == NotificationFactory.getProfileNotificationId(BluetoothProfile.ID_PBAP, PBAP_NOTIFICATION_AUTHORIZE)) {
                    mPbapServer.accept(false);
                }else if(mPid == NotificationFactory.getProfileNotificationId(BluetoothProfile.ID_PBAP, PBAP_NOTIFICATION_AUTH_CHALL)) {
                    mPbapServer.authChallRsp(true, null, null);
                }
                cancelServerNotification();
                break;
    ...
    private final IBluetoothPbap.Stub mServer = new IBluetoothPbap.Stub() {
        ...
        {
            // TODO: disconnect ongoing session
        }
    ...
    private void setServerNotification(int mode) {
        ...
        if(mode == PBAP_NOTIFICATION_AUTHORIZE) {
            ...
            getString(R.string.bluetooth_pbap_server_authorize_notify_ticker),
            System.currentTimeMillis());
            intent.setClassName(getPackageName(), BluetoothServerAuthorize.class.getName())
            .setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            printLog("Remote device name is "+mRemoteDevice.getName());
          intent.putExtra(BluetoothServerAuthorize.DEVICE_NAME, mRemoteDevice.getName())
          .putExtra(BluetoothServerAuthorize.ACTION_CANCEL, PBAP_AUTHORIZE_CANCEL_ACTION)
          .putExtra(BluetoothServerAuthorize.ACTION_RETURN, PBAP_AUTHORIZE_RETURN_ACTION);
          //.putExtra(BluetoothServerAuthorize.TIMEOUT_VALUE, PBAP_AUTHORIZE_TIMEOUT);
          returnAction = PBAP_AUTHORIZE_RETURN_ACTION;
            ...
            getString(R.string.bluetooth_pbap_server_authorize_notify_title),
            getString(R.string.bluetooth_pbap_server_authorize_notify_message),
            contentIntent);
            timeout = PBAP_AUTHORIZE_TIMEOUT;
        }
        else if(mode == PBAP_NOTIFICATION_AUTH_CHALL) {
            noti = new Notification(android.R.drawable.stat_sys_data_bluetooth,
            getString(R.string.bluetooth_pbap_server_auth_chall_notify_ticker),
            System.currentTimeMillis());
            intent.setClassName(getPackageName(), BluetoothAuthenticating.class.getName())
            .setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
          intent.putExtra(BluetoothAuthenticating.DEVICE_NAME, mRemoteDevice.getName())
          .putExtra(BluetoothAuthenticating.ACTION_CANCEL, PBAP_AUTHENTICATE_CANCEL_ACTION)
          .putExtra(BluetoothAuthenticating.ACTION_RETURN, PBAP_AUTHENTICATE_RETURN_ACTION);
          //.putExtra(BluetoothAuthenticating.TIMEOUT_VALUE, PBAP_AUTHENTICATE_TIMEOUT);
          returnAction = PBAP_AUTHENTICATE_RETURN_ACTION;
            contentIntent = PendingIntent.getActivity(BluetoothPbapService.this, 0, intent, 0);
            noti.setLatestEventInfo(context,
                getString(R.string.bluetooth_pbap_server_auth_chall_notify_title),
                getString(R.string.bluetooth_pbap_server_auth_chall_message, mRemoteDevice.getName()),
                contentIntent);
