...
void SetRegisterNotifResToJava(nfc_reg_notif_response& res_c,
	...
	LOGD("RES: status %d , se %d, se_status %d, se_type %d, length %d",
...
void SetDiscoveryNotifResToJava(nfc_dis_notif_response& res_c, JNIEnv*& env,
	...
	if (status_val.int_val != 0 /*which_val.int_val == 0TYPE_DET_NONE*/) {
		LOGE("REQ: SetDiscoveryNotifResToJava status %d",
				status_val.int_val);
		JVALUE target_val;
	...
	if (which_val.int_val == 1/*TYPE_DET_TAG*/) {
		LOGD("REQ: SetDiscoveryNotifResToJava which_val %d  TYPE_DET_TAG",
				which_val.int_val);
		JVALUE target_val;
		...
		LOGD("REQ: SetDiscoveryNotifResToJava which_val %d  TYPE_DET_P2P",
						which_val.int_val);
		JVALUE target_val;
...
JNIEXPORT jobject JNICALL Java_com_mediatek_engineermode_nfc_NfcNativeCallClass_getSettings(
	...
	LOGD("JNI-REQ: nfc_enable, debug_enable, sw_protocol, %d,%d,%d",
...
JNIEXPORT jobject JNICALL Java_com_mediatek_engineermode_nfc_NfcNativeCallClass_getRegisterNotif(
	...
	LOGD("REQ: bitmap %d", req_c.cmd.m_reg_notify_req.reg_type);
...
JNIEXPORT jobject JNICALL Java_com_mediatek_engineermode_nfc_NfcNativeCallClass_setSEOption(
	...
	LOGD("REQ: set_SEtype %d", req_c.cmd.m_se_set_req.set_SEtype);
...
(JNIEnv * env, jclass jc, jobject req_j)
{
	...
	LOGD("enter testEntry");
	GetTestReqFromJava(req_c, env, req_j);
	LOGD("testEntry GetTestReqFromJava over");
	...
	LOGD("testEntry SendCommand over");

	jobject res_j = XNewElement(env, CLASS_TAG_TEST_RES);

	LOGD("testEntry new CLASS_TAG_TEST_RES over");
	SetTestResToJava(resp, env, res_j);

	LOGD("testEntry SetTestResToJava over");
	return res_j;
}
...
{
	LOGD("META_NFC_init was called!");
	if(oncerInit == 0)
	{
		oncerInit = 1;
		LOGD("First call initNfcDriver");
		CMetaNfc::Init();
		int ret = META_NFC_init();
		if(ret != 0)
		{
			oncerInit = 0;
			LOGD("initNfcDriver Failed.");
		}
		return ret;
	}
	else
	{
		LOGD("DUP call initNfcDriver, Nothing will be done.");
		return 0;
	}
...
JNIEXPORT void JNICALL Java_com_mediatek_engineermode_nfc_NfcNativeCallClass_deinitNfcDriver(
		JNIEnv * env, jclass jc) {
	LOGD("META_NFC_deinit was called!");

	if (oncerInit == 1) {
		oncerInit = 0;
		LOGD("First call DEinitNfcDriver");

		NFC_REQ req_c;
		LOGD("enter deinitNfcDriver");
		...
		LOGD("DEinitNfcDriver SendCommand over");

		CMetaNfc::DeInit();
		META_NFC_deinit();
		return ;
	} else {
		LOGD("DUP call DE initNfcDriver, Nothing will be done.");
		return;
	}
}
