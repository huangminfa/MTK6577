...
public class NetworkInfo extends Activity {
	...
	static final String LOG_TAG = "NetworkInfo";
	...
	private static int mModemType = MODEM_NONE;
	
	public static int GetModemType()
	{
	    if(mModemType == MODEM_NONE){
	       mModemType = GetModemTypeFromSysProperty();
	    }
	    
	    Xlog.i(LOG_TAG, "modem type:" + mModemType);
	    return mModemType;
	}
	static int GetModemTypeFromSysProperty()
	{
		String mt = SystemProperties.get("gsm.baseband.capability");
		Xlog.v(LOG_TAG, "gsm.baseband.capability " + mt);
		int mode = MODEM_NO3G;
		if (mt != null) {
			try {
	...
	public boolean onOptionsItemSelected (MenuItem aMenuItem) {   
        ...
        switch (aMenuItem.getItemId()) {   
            case CheckInfomation_ID:
                ...
                Intent intent = new Intent(this, NetworkInfo_Info.class);
