                           ...
                           fm_u16 *ScanTBLsize, fm_u16 scandir, fm_u16 space)
{
	...
	if(fm_true != scan_abort)//sometime abort will come even scan not start
	{
	    // normal scan
	    if (FM_LOCK(cmd_buf_lock)) return (-FM_ELOCK);
	...
	if(fm_true != scan_abort)//scan abort need to do nothing
	{
	    if (!ret && res) {
	...
	{
		cqi_abort = fm_false;//clear cqi abort to avoid next cqi get after scan abnormal, see mt6628_CQI_Get: cqi abort.
#ifndef MTK_FM_50KHZ_SUPPORT//50 k scan will reset in mt6628_Scan_50KHz
		scan_abort = fm_false;
#endif	
	}
