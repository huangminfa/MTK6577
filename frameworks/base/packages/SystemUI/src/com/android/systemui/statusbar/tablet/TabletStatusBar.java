...
public class TabletStatusBar extends StatusBar implements
	...
	private int mNumOfNotificationIconsShow = 0;

    TabletStatusBarPolicy mIconPolicy;
    ...
    LinearLayout mNotificationTrigger;
    ...
    View mRecentButton;    
    View mMoreIcon;
    boolean mIsShowMenuButton = false;

	LinearLayout.LayoutParams mBackPortLayoutParams;
	LinearLayout.LayoutParams mHomePortLayoutParams;
	LinearLayout.LayoutParams mMenuPortLayoutParams;
	LinearLayout.LayoutParams mRecentPortLayoutParams;	
    ...
    protected void addPanelWindows() {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (mobileRSSI != null) {
                mNetworkControllerGemini.addPhoneSignalIconView(mobileRSSI);
            }
            final ImageView wifiRSSI = 
                    (ImageView)mNotificationPanel.findViewById(R.id.wifi_signal);
            if (wifiRSSI != null) {
                mNetworkControllerGemini.addWifiIconView(wifiRSSI);
            }

			final ImageView mobileRSSI_gemini = 
                    (ImageView)mNotificationPanel.findViewById(R.id.mobile_signal_gemini);
            if (mobileRSSI_gemini != null) {
                mNetworkControllerGemini.addPhoneSignalIconView(mobileRSSI_gemini);
            }
        } else {
            final ImageView mobileRSSI = 
                    (ImageView)mNotificationPanel.findViewById(R.id.mobile_signal);
        ...
        // [SystemUI] Support "Dual SIM". {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            mNetworkControllerGemini.addDataTypeIconView(
                    (ImageView)mNotificationPanel.findViewById(R.id.mobile_type));
            mNetworkControllerGemini.addCombinedLabelView(
                    (TextView)mBarContents.findViewById(R.id.network_text));
            mNetworkControllerGemini.addWifiLabelView(
                (TextView)mNotificationPanel.findViewById(R.id.wifi_text));
            mNetworkControllerGemini.addMobileLabelView(
                (TextView)mNotificationPanel.findViewById(R.id.mobile_text));

			mNetworkControllerGemini.addDataTypeIconView(
								(ImageView)mNotificationPanel.findViewById(R.id.mobile_type_gemini));
			mNetworkControllerGemini.addMobileLabelView(
                (TextView)mNotificationPanel.findViewById(R.id.mobile_text_gemini));
        } else {
...
//        lp.windowAnimations = com.android.internal.R.style.Animation_ZoomButtons; // simple fade
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
		if (DEBUG)
		{
			Slog.d(TAG, "newConfig : " + newConfig.orientation);
		}
		...
		{
			mBackButton.setLayoutParams(mBackPortLayoutParams);
			mHomeButton.setLayoutParams(mHomePortLayoutParams);
			mRecentButton.setLayoutParams(mRecentPortLayoutParams);
			mMenuButton.setLayoutParams(mMenuPortLayoutParams);
		}
...
//            Slog.d(TAG, "size=" + newIconSize + " padding=" + newIconHPadding);
		...
		//reloadAllNotificationIcons();//force to reload all views
    }
    ...
    protected View makeStatusBarView() {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            if (IS_CU) {
                ...
                sb = (TabletStatusBarView)View.inflate(context, R.layout.zzz_status_bar_gemini, null);
            }
        } else {
        ...
        mNotificationArea = (NotificationArea)sb.findViewById(R.id.notificationArea);
        if (!NOTIFICATION_PEEK_ENABLED) {
            mNotificationArea.setOnTouchListener(new NotificationTriggerTouchListener());
        }
		mMoreIcon = sb.findViewById(R.id.moreIcon);;
		mNotificationArea.setOverflowIndicator(mMoreIcon);
		mNotificationArea.setTabletStatusBar(this);		

        // the button to open the notification area
        //mNotificationTrigger = sb.findViewById(R.id.notificationTrigger);
        mNotificationTrigger = (LinearLayout) sb.findViewById(R.id.notificationTrigger);
        if (NOTIFICATION_PEEK_ENABLED) {
...
//        final int LIGHTS_COMING_UP_SYSBAR_DELAY    = 50;
		...
		//save layout params of navigation buttons in portrait
		mBackPortLayoutParams 	= (LinearLayout.LayoutParams)mBackButton.getLayoutParams();
		mHomePortLayoutParams 	= (LinearLayout.LayoutParams)mHomeButton.getLayoutParams();
		mRecentPortLayoutParams = (LinearLayout.LayoutParams)mRecentButton.getLayoutParams();
		mMenuPortLayoutParams 	= (LinearLayout.LayoutParams)mMenuButton.getLayoutParams();
...
//                                  : mNotificationPeekScrubRight);
    ...
    public void addIcon(String slot, int index, int viewIndex, StatusBarIcon icon) {
        if (DEBUG) Slog.d(TAG, "addIcon(" + slot + ") -> " + icon);
         ...
         view.set(icon);
         mNotificationTrigger.addView(view, viewIndex, new LinearLayout.LayoutParams(mIconSize, mIconSize));
    }
    ...
    public void updateIcon(String slot, int index, int viewIndex, StatusBarIcon old, StatusBarIcon icon) {
        ...
        StatusBarIconView view = (StatusBarIconView)mNotificationTrigger.getChildAt(viewIndex);
		if (view != null)
			view.set(icon);
    }
    ...
    public void removeIcon(String slot, int index, int viewIndex) {
        ...
        mNotificationTrigger.removeViewAt(viewIndex);
    }
    ...
    private void setNavigationVisibility(int visibility) {
        ...
        if (mIsShowMenuButton)
        {
            if (DEBUG) {
                Slog.d(TAG, (disableHome?"hide2":"show2") + " the MENU button");
            }
            mMenuButton.setVisibility(disableHome ? View.GONE : View.VISIBLE);
        }
    ...
    public void topAppWindowChanged(boolean showMenu) {
        if (DEBUG) {
            Slog.d(TAG, (showMenu?"showing1":"hiding1") + " the MENU button");
        }

        //add check condition for Lock screen to hide menu button.
        if (mHomeButton!= null && mHomeButton.getVisibility() == View.VISIBLE)
        {
...
//                        mInitialTouchY));
    ...
    private void loadNotificationPanel() {
        ...
        for (int i=0; i<toShow.size(); i++) {
            View v = toShow.get(i);
            if (v.getParent() == null) {
		   int pile_cnt= mPile.getChildCount();    
                mPile.addView(v, pile_cnt); // the notification panel has newest at the bottom
            }
        }
...
//        content.setOnFocusChangeListener(mFocusChangeListener);
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_CLOSE_SYSTEM_DIALOGS.equals(action)
				...
				KeyButtonView mTempKB = (KeyButtonView) mBackButton;
				if (mTempKB != null) {
					mTempKB.setGlowBackgroud(mContext.getResources().getDrawable(R.drawable.ic_sysbar_highlight));
				}
				
				mTempKB = (KeyButtonView) mHomeButton;
				if (mTempKB != null) {
					mTempKB.setGlowBackgroud(mContext.getResources().getDrawable(R.drawable.ic_sysbar_highlight));
				}
				
				mTempKB = (KeyButtonView) mRecentButton;
				if (mTempKB != null) {
					mTempKB.setGlowBackgroud(mContext.getResources().getDrawable(R.drawable.ic_sysbar_highlight));
				}
				
				mTempKB = (KeyButtonView) mMenuButton;
				if (mTempKB != null) {
					mTempKB.setGlowBackgroud(mContext.getResources().getDrawable(R.drawable.ic_sysbar_highlight));
				}
				
				for (int i = 0, n = mNotificationData.size(); i < n; i++) {
    ...
    public boolean isExpanded() {
        return mNotificationPanel.isShowing();
    }
	...
	public int getShowingNotificationIcons()
    {
    	return mNumOfNotificationIconsShow;
    }

	public void setMaxNotificationIcons(int num)
	{
		mMaxNotificationIcons = num;
		updateNotificationIcons();
	}
}
