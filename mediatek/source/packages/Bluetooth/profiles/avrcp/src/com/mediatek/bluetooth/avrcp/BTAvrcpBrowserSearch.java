...
public class BTAvrcpBrowserSearch {
	...
	private EditText toAddrET;
	static public int num  = 0;
	public final static String TAG = "AVRCP_UTIL";
    ...
    public static String getSongData(Context context, long id) {
        ...
        String album = "";
        int albumIndex, titleIndex, artistIndex;
		...
		if (cursor != null) {
        	...
        	for (int j = 0; j < cursor.getCount(); j++ ){
        		Log.v(TAG, String.format( "[UTIL] found: album:'%s' artist:'%s' title:'%s' " , cursor.getString(albumIndex)  , cursor.getString(artistIndex) , cursor.getString(albumIndex))  );
        		cursor.moveToNext();
        	}
        	cursor.close();
        	cursor = null;
    ...
    public static long [] getAllSongs(Context context) {
        ...
        final String[] ccols = new String[] { 
        		...
        		MediaStore.Audio.Media.ARTIST, 
        		MediaStore.Audio.Media.TITLE  };  
        int albumIndex = 1, titleIndex = 2 , artistIndex =3;
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
            	...
            	if( printDetail == true ){
            		cursor = c;
            		Log.v(TAG, String.format( "[UTIL] id:%d album:'%s' artist:'%s' title:'%s' " , cursor.getLong(0), cursor.getString(titleIndex)  , cursor.getString(artistIndex) , cursor.getString(albumIndex))  );
            	}                
            }
    ...
    public long [] getSearchSongs(Context context, String sSearch) {
        ...
        final String[] ccols = new String[] { 
        		...
        		MediaStore.Audio.Media.ARTIST, 
        		MediaStore.Audio.Media.TITLE  };  
        int albumIndex = 1, titleIndex = 3 , artistIndex = 2;
        Cursor c = query(context, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
        		ccols, MediaStore.Audio.Media.IS_MUSIC + "=1",
                null, null);
        Cursor cursor;
