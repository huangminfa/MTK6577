...
//    static public final String NOTIFICATION_TITLE = "notification_title";
    ...
    public void onCreate() {
        ...
        ((GeminiPhone)mPhone).registerForSuppServiceFailedGemini(mCallHandler, SUPP_SERVICE_FAILED, null, miSIMid);
        initNotify();
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case OP_LAUNCH_APP:
                ...
                CatLog.d(LOGTAG, "handleMessage OP_LAUNCH_APP - mCmdInProgress[" + mCmdInProgress + "]");
...
//                if(bpkgNotInstalled){
//                  StkLog.d(LOGTAG, "[prepare to start STKAPPSERVICE]");                
//               switch (cmdMsg.getCmdType()){
//               case SET_UP_MENU:
//                   StkLog.d(LOGTAG, "[OP_CMD][SETUPMENU delayed]");  
//                   mFirstCmdSetupMenu = true;
//                   if(cmdMsg.getMenu() != null){
//                       mstrbackDisplayTitle = cmdMsg.getMenu().title;
//                       StkLog.d(LOGTAG, "[OP_CMD][SETUPMENU delayed][Menu Title] : " + mstrbackDisplayTitle);
//                   }                   
//                   break;
//                   }
...
//              }
                break;
            case OP_RESPONSE:
                CatLog.d(LOGTAG, " [OP_RESPONSE][responseNeeded] : " + responseNeeded);
                if (responseNeeded) {
...
//                if (mMainCmd == null) {
//                    StkLog.d(LOGTAG, "OP_BOOT_COMPLETED - unInstall");
...
//              msg.title = mstrbackDisplayTitle;
        ...
        case SELECT_ITEM:
            ...
            if (iTel != null){
                try {
                    if(iTel.isRadioOnGemini(Phone.GEMINI_SIM_1) == true) {
            ...
            break;
        case SET_UP_MENU:
            CatLog.d(LOGTAG, "[handleCmd][SET_UP_MENU] +");
    ...
    private void processNormalResponse() {
        ...
        if(mCurrentCmd.getCmdType() != null){
            CatLog.d(LOGTAG, "Normal Response PROCESS end! cmdName[" + mCurrentCmd.getCmdType().name() + "]");
        }       
        CatResponseMessage resMsg = new CatResponseMessage(mCurrentCmd);
        resMsg.setResultCode(ResultCode.OK);
    ...
    private void processAbnormalResponse() {
        ...
        if(mCurrentCmd.getCmdType() != null){
            CatLog.d(LOGTAG, "Abnormal Response PROCESS end! cmdName[" + mCurrentCmd.getCmdType().name() + "]");
        }       
        CatResponseMessage resMsg = new CatResponseMessage(mCurrentCmd);
    ...
    private void processAbnormalNoCallResponse() {
        ...
        if(mCurrentCmd.getCmdType() != null){
            CatLog.d(LOGTAG, "Abnormal No Call Response PROCESS end! cmdName[" + mCurrentCmd.getCmdType().name() + "]");
        }
        CatResponseMessage resMsg = new CatResponseMessage(mCurrentCmd);
        resMsg.setResultCode(ResultCode.TERMINAL_CRNTLY_UNABLE_TO_PROCESS);
        mStkService.onCmdResponse(resMsg);
    ...
    private void processNormalOpenChannelResponse() {
        ...
        TextMessage txtMsg = mCurrentCmd.geTextMessage();
        if (mCurrentCmd.getCmdType() != null){
            CatLog.d(LOGTAG, "Normal OpenChannel Response PROCESS end! cmdName[" + mCurrentCmd.getCmdType().name() + "]");
        }        
        CatResponseMessage resMsg = new CatResponseMessage(mCurrentCmd);
        resMsg.setResultCode(ResultCode.OK);
        resMsg.setConfirmation(true);
    ...
    private void processAbnormalOpenChannelResponse() {
        ...
        if (mCurrentCmd.getCmdType() != null){
            CatLog.d(LOGTAG, "Abnormal OpenChannel Response PROCESS end! cmdName[" + mCurrentCmd.getCmdType().name() + "]");
        }        
        CatResponseMessage resMsg = new CatResponseMessage(mCurrentCmd);
        resMsg.setResultCode(ResultCode.TERMINAL_CRNTLY_UNABLE_TO_PROCESS);
        mStkService.onCmdResponse(resMsg);
    ...
    private void processSetupCall() {
        if (Call.State.IDLE != isPhoneIdle(1)) {
            processAbnormalPhone2BusyResponse();
        } else {
            // get callback.
    ...
    private void handleCmdResponse(Bundle args) {
        ...
        CatResponseMessage resMsg = new CatResponseMessage(mCurrentCmd);
        if (null != mCurrentCmd && null != mCurrentCmd.getCmdType()) {
        ...
        case RES_ID_INPUT:
            ...
            if (input != null && (null != mCurrentCmd.geInput())&&(mCurrentCmd.geInput().yesNo)) {
        ...
        case RES_ID_TIMEOUT:
            ...
            if ((mCurrentCmd.getCmdType().value() == AppInterface.CommandType.DISPLAY_TEXT
                    .value())
                    && (mCurrentCmd.geTextMessage().responseNeeded == true)) {
    ...
    private boolean canShowTextDialog(TextMessage msg) {
        ...
        CatLog.d(LOGTAG, "canShowTextDialog? mMenuIsVisible = " + mMenuIsVisible + " mInputIsVisible = " + " mDialogIsVisible = " + mDialogIsVisible); 
    ...
    private final BroadcastReceiver mSIMStateChangeReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (TelephonyIntents.ACTION_SIM_STATE_CHANGED.equals(intent.getAction())) {
                ...
                int simId = intent.getIntExtra(com.android.internal.telephony.Phone.GEMINI_SIM_ID_KEY, -1);
