...
public class Preferences {
    ...
    public void checkLowStorage() {
        ...
        long remaining = stat.getAvailableBlocks() * stat.getBlockSize();
        if (Email.DEBUG) {
            Log.d(Logging.LOG_TAG, "AvailableBlocks: "
                    + stat.getAvailableBlocks() + " BolckSize: "
                    + stat.getBlockSize() + " Data file system remaining size is "
                    + remaining / 1024 + " KBytes");
        }
    ...
    public boolean getAutoDownloadRemaining() {
        return mSharedPreferences.getBoolean(AUTO_DOWNLOAD_REMAINING, AUTO_DOWNLOAD_REMAINING_DEFAULT);
    }
