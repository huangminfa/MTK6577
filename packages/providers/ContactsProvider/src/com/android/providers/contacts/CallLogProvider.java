...
//        sCallsProjectionMap.put(Calls._ID, Calls._ID);
    ...
    */
	...
	private void updateDialerSearchCallLogInfo(SQLiteDatabase db,
			int recordType, long nameRawContactID, long dataId,
			long toBeDeletedCallLogRowID, String toBeDeletedCallLogNum,
			String toBeDeletedCallLogIDSet) {
		
		log("[updateDialerSearchCallLogInfo]recordType:" + recordType
				+ " || nameRawContactID:" + nameRawContactID
				+ " || toBeDeletedCallLogRowID: " + toBeDeletedCallLogRowID
				+ " || toBeDeletedCallLogNum: " + toBeDeletedCallLogNum);
		
		String selectionStmt = null, updateStmt = null;
		if (DS_NAMERECORD == recordType) {
			selectionStmt = Calls.RAW_CONTACT_ID + " = " + nameRawContactID
					+ " AND " + Calls._ID + " NOT IN (" + toBeDeletedCallLogIDSet + ")";
			updateStmt = DialerSearchLookupColumns.RAW_CONTACT_ID + " = " + nameRawContactID + " AND "
					+ DialerSearchLookupColumns.CALL_LOG_ID + " = " + toBeDeletedCallLogRowID + " AND "
					+ DialerSearchLookupColumns.NAME_TYPE + " = " + DialerSearchLookupType.NAME_EXACT;
		} else if (DS_NUMBERRECORD == recordType) {
			selectionStmt = Calls.RAW_CONTACT_ID + " = " + nameRawContactID
					+ " AND " + Calls.DATA_ID + " = " + dataId 
					+ " AND " + Calls._ID + " NOT IN (" + toBeDeletedCallLogIDSet + ")";
			updateStmt = DialerSearchLookupColumns.RAW_CONTACT_ID + " = " + nameRawContactID + " AND "
		...
		Cursor callLogCursorOfRawContact = db.query(Tables.CALLS, new String[] {
				Calls._ID, Calls.DATE }, selectionStmt, null, null, null,
				"_id DESC", "2");
		ContentValues updateNameLookupValue = new ContentValues();
		updateNameLookupValue.put(DialerSearchLookupColumns.CALL_LOG_ID, 0);
		if (callLogCursorOfRawContact != null) {
			int count = callLogCursorOfRawContact.getCount();
			log("[delete] query Calls table. Result count:" + count);
			if (callLogCursorOfRawContact.moveToFirst()) {
				// There exists another call log record corresponding to the same raw_contact_id
				if (count == 2) {
					callLogCursorOfRawContact.moveToNext();
					long newCallLogId = callLogCursorOfRawContact.getLong(0);
					updateNameLookupValue.clear();
					updateNameLookupValue.put(DialerSearchLookupColumns.CALL_LOG_ID, newCallLogId);
					int updateCount = db.update(Tables.DIALER_SEARCH,
							updateNameLookupValue, updateStmt, null);
					log("[delete] 2 count: " + count + " || updateCount: " + updateCount);
				} else {
					int updateCount = db.update(Tables.DIALER_SEARCH,
							updateNameLookupValue, updateStmt, null);
					log("[delete] 1 count: " + count + " || updateCount: " + updateCount);
				}
			} else {
				int updateCount = db.update(Tables.DIALER_SEARCH, updateNameLookupValue, updateStmt, null);
				log("[delete]callLogCursorOfRawContact empty. updateCount: " + updateCount);
			}
			callLogCursorOfRawContact.close();
		} else {
			int updateCount = db.update(Tables.DIALER_SEARCH,
					updateNameLookupValue, updateStmt, null);
			log("[delete]callLogCursorOfRawContact null. update dialer search table." +
