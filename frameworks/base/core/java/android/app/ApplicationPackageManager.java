...
final class ApplicationPackageManager extends PackageManager {
    ...
    @Override public Drawable getActivityIcon(Intent intent)
        ...
        throw new NameNotFoundException(intent.toUri(0));//ALPS00106316
    }
    ...
    private Drawable getCachedIcon(ResourceName name) {
        synchronized (sSync) {
            WeakReference<Drawable.ConstantState> wr = sIconCache.get(name);//ALPS00106316
    ...
    private void putCachedIcon(ResourceName name, Drawable dr) {
        synchronized (sSync) {
            sIconCache.put(name, new WeakReference<Drawable.ConstantState>(dr.getConstantState()));//ALPS00106316
            if (DEBUG_ICONS) Log.v(TAG, "Added cached drawable state for " + name + ": " + dr);
        }
    }

    static void handlePackageBroadcast(int cmd, String[] pkgList, boolean hasPkgInfo) {//ALPS00106316
    ...
    private static HashMap<ResourceName, WeakReference<Drawable.ConstantState>> sIconCache
             = new HashMap<ResourceName, WeakReference<Drawable.ConstantState>>();//ALPS00106316
