...
int rawbulk_tty_alloc_request(struct rawbulk_function *fn) {
	...
	//req->buffer = kmalloc(MAX_TTY_RX_PACKAGE, GFP_KERNEL);
        req->buffer = __get_free_page(GFP_KERNEL);
        if (!req->buffer) {
            kfree(req);
            break;
        }
	...
	req->buffer = kmalloc(MAX_TTY_TX_PACKAGE, GFP_KERNEL);
        if (!req->buffer) {
            kfree(req);
            break;
        }
...
static struct _function_init_stuff {
    ...
    {"rawbulk-ets", "ets", "ETS Port", 512, 512, 4, 4, PAGE_SIZE, PAGE_SIZE, PAGE_SIZE, true, false },
