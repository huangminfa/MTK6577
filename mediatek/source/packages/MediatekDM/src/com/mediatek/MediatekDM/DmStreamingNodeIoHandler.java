...
//	final private String cfgFilePath = "/data/data/Streaming.cfg";
    final private String[] item = { "Name", "To-Proxy", "To-NapID", "NetInfo",
                                    "MIN-UDP-PORT", "MAX-UDP-PORT"
                                  };
    final private String[] mContentItem = new String[item.length];
    ...
    String[] projection = { "mtk_rtsp_name", "mtk_rtsp_to_proxy",
                            "mtk_rtsp_to_napid", "mtk_rtsp_netinfo", "mtk_rtsp_min_udp_port",
    ...
    public int read(int arg0, byte[] arg1) throws MdmException {
        ...
        if (DmService.mCCStoredParams.containsKey(uriKey)) {
            ...
            Log.d(TAG.NodeIOHandler,"get valueToRead from mCCStoredParams, the value is "+recordToRead);
        } else {
                ...
                if (uri.getPath().contains(getItem()[i])) {
                    if ((String) map.get(getItem()[i]) != null) {
                        if (recordToRead.length() == 0) {
                            recordToRead = Settings.System.getString(mContext.getContentResolver(), (String)map.get(getItem()[i]));
                        }
                    } else {
            ...
            Log.d(TAG.NodeIOHandler,"put valueToRead to mCCStoredParams, the value is "+recordToRead);
        }
        ...
        if (recordToWrite.length() == arg2) {
            for (int i = 0; i < getItem().length; i++) {
                if (uri.getPath().contains(getItem()[i])) {
                    ...
                    if ((String) map.get(getItem()[i]) != null) {
                        if (!specificHandlingForWrite(recordToWrite, values,
                                                      getItem()[i])) {
                            Settings.System.putString(mContext
                                                      .getContentResolver(), (String)map.get(getItem()[i]),
                                                      recordToWrite);
                        }
                    }
                    recordToWrite = null;
