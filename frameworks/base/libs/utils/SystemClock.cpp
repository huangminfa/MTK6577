...
extern "C" {
    extern int deltat_write(long diff_sec);
}
#endif
...
int setCurrentTimeMillis(int64_t millis)
{
    ...
    if (res < 0) {
        ...
        if (ts.tv_sec != tv.tv_sec)     /* use the time read by Alarm driver */
            prev_time = ts.tv_sec;
        diff_sec = tv.tv_sec - prev_time;
        LOGD("prev_time = %ld, diff_sec = %ld\n", prev_time, diff_sec);
        deltat_write(diff_sec);
    }
    close(fd);
#else
