    ...
    Segment*& pSegment)
{
    ...
    while (pos < end)
    {
		...
		if (!CheckUIntInvalid(size, len)) {
			size = total - pos;
			LOGW("ID=0x%llx, size reserved, set size according to file size->%lld", id, size);
		}
#endif
...
Cluster* Segment::GetPrev(const Cluster* pCurr)
{
	...
	LOGE("cannot reach here...(%s):(%d)", __func__, __LINE__);
	return &m_eos;
}
#endif
...
void Track::Info::Clear()
{
    ...
    codecNameAsUTF8 = NULL;
#ifndef ANDROID_DEFAULT_CODE
	LOGD("Clear: contentAddTotalSize=%d, contentAddData=(%p)", contentAddTotalSize, contentAddData);
...
void Track::Info::ParseContentEncodings(IMkvReader* pReader, long long pos, long long size)
{
		...
		if (id == 0x2240) {
			...
			while (pos < _stop) {
				if (Match(pReader, pos, 0x1031, ce.order))
					LOGD("encoding order=%lld", ce.order);
				else if(Match(pReader, pos, 0x1032, ce.scope))
					LOGD("encoding scope=%lld", ce.scope);
				else if(Match(pReader, pos, 0x1033, ce.type))
					LOGD("encoding type=%lld", ce.type);
				else {
					...
					if (_id == 0x1034) {//ContentCompression
						...
						while (pos < __stop) {
							if (Match(pReader, pos, 0x0254, ce.compression.algo))
								LOGD("compression algo=%lld", ce.compression.algo);
			...
			if ((ce.scope != 1) ||//all frame
				...
				LOGW("Unsupport content encoding element, scope(%lld), type(%lld), algo(%d), setting size(%d)", 
						ce.scope, ce.type, ce.compression.algo, ce.compression.settingsSize);
			} else {
				LOGD("content encoding element, encoding scope(%lld), encoding type(%lld), \
	...
	if (contentAddTotalSize != 0)
	{
		...
		LOGD("contentAddData=0x%p, contentAddTotalSize=%d", contentAddData, contentAddTotalSize);
		...
		if ((offset > contentAddTotalSize) || (compressions.size() != 0))
			LOGE("Parse content encoding error");
		LOGD("contentAddData=0x%x, contentAddTotalSize=%d", *contentAddData, contentAddTotalSize);
	}
...
void Track::GetContentAddInfo(unsigned char** data, size_t* size)
{
	if (m_info.contentAddTotalSize != 0) {
		...
		*data = m_info.contentAddData;
		LOGD("contentAddTotalSize=%d, contentAddData=0x%x(%p)", m_info.contentAddTotalSize, *(m_info.contentAddData), m_info.contentAddData);
	}
	else {
		*size = 0;
		*data = NULL;
    ...
    m_bitDepth(-1)
{
    ...
    while (pos < stop)
    {
		...
		if (!strcmp("A_VORBIS",i.codecId) && i.codecPrivateSize != 0) {
			LOGV("Vorbis audio has codec private EBML");
			if (m_rate == 0.0/*-1*/) {
				LOGV("Error handle for no audio sample rate EBML header, get header from codec private data");
				long rate = 0;
				rate = *((long *)(i.codecPrivate + 15)); // (4 bytes header + sizeof("vorbis")+ 4 bytes version + one byte channels)
				m_rate = (double) rate;
				LOGV("rate from codec private -> %d",(int)rate);
			}
			if (m_channels == 0/*-1*/) {
				LOGV("Error handle for no audio channels EBML header, get header from codec private data");
				char channels;
				channels = *(i.codecPrivate + 14); // (4 bytes header + sizeof("vorbis")+ 4 bytes version)
				m_channels = (long long) channels;
				LOGV("channels from codec private -> %d", (int)channels);
			}
		}
		LOGV("Audio Track : sample rate -> %f, channels -> %lld, bits -> %lld",m_rate,m_channels,m_bitDepth);
#endif
