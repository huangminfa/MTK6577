...
public class PhoneNumberListAdapter extends ContactEntryListAdapter {
    ...
    protected void bindPhoneNumber(ContactListItemView view, Cursor cursor) {
        ///M: #AAS# extension to replace default behavior if needed @{
        if(aasBindPhoneNumber(view, cursor)) {
            return;
        }
        ///@}
    ...
    /**
     * M: If #AAS# available, the "bind phone number" should act in "aas way",
     * the PHONE_TYPE field means aas index rather than phone group index, the
     * label value would be get from USIM
     * @param view item view to bind
     * @param cursor contains information
     * @return true if aas operations actually took place, bind has finished, the
     * caller should return directly.
     * false means it's not an AAS case, and will remain act Google default way
     */
    private boolean aasBindPhoneNumber(ContactListItemView view, Cursor cursor) {
        int simId = cursor.getInt(PhoneQuery.PHONE_INDICATE_PHONE_SIM_COLUMN_INDEX);
        final int slot = SIMInfo.getSlotById(mContext, simId);
        ...
        if (OperatorUtils.isAasEnabled(accountType)) {
            final int aasIndex = cursor.getInt(PhoneQuery.PHONE_TYPE);
            final String label = USIMAas.getUSIMAASById(mSlot, aasIndex);
            Log.v(TAG, "#AAS# bindPhoneNumber, Slot: " + mSlot +
                    ", index: " + aasIndex + ", label: " + label);
            view.setLabel(label);
            view.showData(cursor, PhoneQuery.PHONE_NUMBER);
            return true;
        }
