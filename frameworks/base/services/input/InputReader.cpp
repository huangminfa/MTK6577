...
static void mtProf(int start)
{
    ...
    if (fd_profsts == -1){
        LOG(LOG_INFO,"AP_PROF","unable to open /proc/mtprof/status entry");
        return;
    }
    ...
    fd_memprof = open("/proc/mtprof/memprof",O_RDWR);

    if (fd_cputime == -1){
        LOG(LOG_INFO,"AP_PROF","unable to open /proc/mtprof/cputime entry");
        return;
    }
    if (fd_memprof == -1){
        LOG(LOG_INFO,"AP_PROF","unable to open /proc/mtprof/memprof entry");
        return;
    }
    if(1 == start){
        LOG(LOG_INFO,"AP_PROF","Start MT profiling");
        write(fd_cputime,"1", 1);
        write(fd_memprof,"1", 1);
    }
    else
    {
        write(fd_cputime,"0", 1);
        write(fd_memprof,"0", 1);
    ...
    if (currentIdBits == lastIdBits) {
        ...
        while (!upIdBits.isEmpty()) {
            uint32_t upId = upIdBits.clearFirstMarkedBit();
                ...
                LOGD("AP_PROF:AppLaunch_dispatchPtr:Up:%lld", when/1000000);
            dispatchMotion(when, policyFlags, mSource,
        ...
        while (!downIdBits.isEmpty()) {
            ...
            if (dispatchedIdBits.count() == 1) {
                ...
                mDownTime = when;
		LOGD("AP_PROF:AppLaunch_dispatchPtr:Down:%lld", mDownTime/1000000);
		mtProf(1);
            }
