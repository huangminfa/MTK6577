     ...
     // Yu for ICS
        ...
        btnConfirm.setOnClickListener(new Button.OnClickListener()
        {
        	...
        	{        		
        	    ...
        	    if(((false == mbMCCMNCHLRReadSIM)&&(false == mbMCCMNCHLRReadSIM1)&&(false == mbMCCMNCHLRReadSIM2))&&((7 > etMCCMNCHLR.getText().length())||(8 < etMCCMNCHLR.getText().length())))
        	    {
               ...
               // else if (etPwd.getText().toString().equals("56781234") == false)
               // {
...
//                }
                ...
                else
                {
                    ...
                    if(false == FeatureOption.MTK_GEMINI_SUPPORT)
                    {
                    	...
                    	if (mbMCCMNCHLRReadSIM == false)//user input
                    	{
                    		phone.getIccCard().setIccNetworkLockEnabled(1,2,etPwd.getText().toString(),etMCCMNCHLR.getText().toString(),null,null,callback);
                    	}
                    	else
                    	{
                    		phone.getIccCard().setIccNetworkLockEnabled(1,2,etPwd.getText().toString(),msSIMMCCMNCHLR,null,null,callback);
                    	}                	                	
                    ...
                    else
                    {
                    	...
                    	if (intSIMNumber == 0)
                    	{
                        	if (mbMCCMNCHLRReadSIM1 == false)//user input
                        	{
                        		mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_1).setIccNetworkLockEnabled(1,2,etPwd.getText().toString(),etMCCMNCHLR.getText().toString(),null,null,callback);
                        	}
                        	else
                        	{
                        		mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_1).setIccNetworkLockEnabled(1,2,etPwd.getText().toString(),msSIM1MCCMNCHLR,null,null,callback);
                        	}                	
                    	}
                    	else
                    	{
                        	if (mbMCCMNCHLRReadSIM2 == false)//user input
                        	{
                        		mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_2).setIccNetworkLockEnabled(1,2,etPwd.getText().toString(),etMCCMNCHLR.getText().toString(),null,null,callback);
                        	}
                        	else
                        	{
                        		mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_2).setIccNetworkLockEnabled(1,2,etPwd.getText().toString(),msSIM2MCCMNCHLR,null,null,callback);
                        	} 
                        }
    		        }
                }
...
//            	Toast.makeText(this,"Fail to read SIM1 MCC+MNC",Toast.LENGTH_LONG).show();
            ...
            else
            {
            	...
            	msSIM1IMSI = telephonyManager.getSubscriberIdGemini(0);//get  card 1 IMSI code
                if(msSIM1IMSI == null)
                {
...
//            	Toast.makeText(this,"Fail to read SIM2 MCC+MNC",Toast.LENGTH_LONG).show();
            ...
            else
            {
                ...
                msSIM2IMSI = telephonyManager.getSubscriberIdGemini(1);//get  card 2 IMSI code
                if(msSIM2IMSI == null)
                {
	...
	protected Dialog onCreateDialog(int id) {
    	...
    	case DIALOG_MCCMNCHLRLENGTHINCORRECT://Length is incorrect
		{
	    	...
	    	.setTitle(R.string.strAttention)
	    	.setIcon(android.R.drawable.ic_dialog_alert)
	    	.setMessage(R.string.strMCCMNCHLRLengthIncorrect)
	    	.setOnKeyListener(this)
	    	.setPositiveButton(R.string.strYes, new DialogInterface.OnClickListener() 
	    	{			
				public void onClick(DialogInterface dialog, int id) 
              ...
              case ADDLOCK_ICC_SML_COMPLETE:
               {
            	   ...
            	   {
            		   //Toast.makeText(NSPAddLockSetting.this, "Nsp add lock fail", Toast.LENGTH_LONG).show();
            		   showDialog(DIALOG_ADDLOCKFAIL);
            	   }
            	   else
            	   {
            		   //Toast.makeText(NSPAddLockSetting.this, "Nsp add lock succeed!", Toast.LENGTH_LONG).show();
    ...
    boolean mbMCCMNCHLRReadSIM1 = false;//whether to read MCC+MNC+HLR from SIM1 card,default to set false
    boolean mbMCCMNCHLRReadSIM2 = false;//whether to read MCC+MNC+HLR from SIM2 card,default to set false
