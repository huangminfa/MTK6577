        ...
        implements View.OnCreateContextMenuListener, MusicUtils.Defs, ServiceConnection, DialogInterface.OnClickListener
{
    ...
    private static final int ADD_SONG = CHILD_MENU_BASE + 7;
    ...
    // store the bitmap image decode from file to show it when switch screen 
    ...
    // When in background, need not to show album art, and restore the status to make sure show the
    // album art at resume.
    private boolean mIsInBackgroud = false;
    private boolean mShowAlbumArtAfterResume = false;
    ...
    public void onCreate(Bundle icicle)
    {
        ...
        if (icicle != null) {
            ...
            MusicLogUtils.i(TAG, "mCardId retrieved is: " + mCardId);
        } else {
            //ALPS00122166
...
//        if (!FeatureOption.MTK_THEMEMANAGER_APP)
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        // Configuration change set background again
        MusicLogUtils.d(TAG, "Configuration Changed, update background: mBackgroundBitmap = "
                + mBackgroundBitmap);
        setAlbumArtBackground();
//        if (!FeatureOption.MTK_THEMEMANAGER_APP)
            mTrackList.setCacheColorHint(0);
    ...
    public void onResume() {
        super.onResume();
        if (mShowAlbumArtAfterResume) {
            MusicLogUtils.d(TAG, "onResume, so update background bitmap again: mBackgroundBitmap = " + mBackgroundBitmap);
            setAlbumArtBackground();
            mShowAlbumArtAfterResume = false;
        }
    ...
    private BroadcastReceiver mScanListener = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_MEDIA_SCANNER_STARTED.equals(action) ||
                ...
                String internalCardPath = Environment.getExternalStorageDirectory().getPath();
                String mountedCardPath = intent.getData().getPath();
                MusicLogUtils.d(TAG, "MEDIA_MOUNTED: new card path=" + mountedCardPath + ", new card id=" + cardid + ", old cardid=" + mCardId);
                mIsMounted = true;
                if (mountedCardPath != null && mountedCardPath.equals(internalCardPath)) {
                    // Internal card has just been mounted again, check its id
                    if (mCardId != -1 && mCardId != cardid) {
                        MusicLogUtils.d(TAG, "MEDIA_MOUNTED: SD card changed, quit activity!");
                        finish();
                    }
                }
    ...
    public void init(Cursor newCursor, boolean isLimited) {
        ...
        // Refresh action bar after cursor is ready
        invalidateOptionsMenu();
        ...
        if (mResetLayout) {
            ...
            if (mIsInBackgroud) {
                mShowAlbumArtAfterResume = true;
            } else {
                MusicLogUtils.d(TAG, "init finish, update background: mBackgroundBitmap = "
                        + mBackgroundBitmap);
                setAlbumArtBackground();
            }
            mResetLayout = false;
    ...
    private void setAlbumArtBackground() {
        ...
        mTrackList.setCacheColorHint(0);
        */
        // Cancel the old one and execute new task to set background
    ...
    private TouchInterceptor.DropListener mDropListener =
        ...
        public void drop(int from, int to) {
            if (mTrackCursor instanceof NowPlayingCursor) {
                ...
                int order_from = mTrackCursor.getInt(mTrackCursor.getColumnIndexOrThrow(MediaStore.Audio.Playlists.Members.PLAY_ORDER));                
                mTrackCursor.moveToPosition(to);                
                int order_to = mTrackCursor.getInt(mTrackCursor.getColumnIndexOrThrow(MediaStore.Audio.Playlists.Members.PLAY_ORDER));                
                // update a saved playlist
                MediaStore.Audio.Playlists.Members.moveItem(getContentResolver(),
                        Long.valueOf(mPlaylist), order_from, order_to);
            }
        }
    };
        ...
        switch (item.getItemId()) {
            case PLAY_SELECTION: {
                ...
                if (FeatureOption.MTK_DRM_APP) {
                    ...
                    MusicLogUtils.d(TAG, "onListItemClick: rightsStatus=" + rightsStatus);
    ...
    {
        MusicLogUtils.i("MusicPerformanceTest", "[Performance test][Music] play song start ["+ System.currentTimeMillis() +"]");
        MusicLogUtils.d(TAG, "onListItemClick");
        ...
        if (FeatureOption.MTK_DRM_APP) {
            ...
            isDRM = mTrackCursor.getInt(mTrackCursor.getColumnIndexOrThrow(MediaStore.Audio.Media.IS_DRM));
            MusicLogUtils.d(TAG, "onListItemClick: isDRM=" + isDRM);
            if (isDRM == 1) {
                mCurTrackPos = position;
                drm_method = mTrackCursor.getInt(mTrackCursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DRM_METHOD));
                MusicLogUtils.d(TAG, "onListItemClick: drm_method=" + drm_method);
                int colIdx = -1;
                try {
                    colIdx = mTrackCursor.getColumnIndexOrThrow(MediaStore.Audio.Playlists.Members.AUDIO_ID);
                } catch (IllegalArgumentException ex) {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (mPlaylist != null) {
            ...
            if (IS_CMCC) {
                // Add clear playlist and add file to playlist option menu to playlist except "Recently added" and "Podcasts" 
                if (!mPlaylist.equals("recentlyadded") && !mPlaylist.equals("podcasts")) {
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        // MTK_OP01_PROTECT_START--->
        if (IS_CMCC && mPlaylist != null && menu.findItem(CLEAR_PLAYLIST) != null) {
            if (mTrackList != null && mTrackList.getCount() <= 0) {
                // Disable clear playlist option menu when playlist is empty
                menu.findItem(CLEAR_PLAYLIST).setVisible(false);
            } else if (!mPlaylist.equals("recentlyadded") && !mPlaylist.equals("podcasts")) {
                // Enable clear playlist option menu when playlist is not empty
        ...
        if (mTrackCursor == null) {
            MusicLogUtils.v(TAG, "Track cursor is null, need not show option menu: mTrackCursor = " + mTrackCursor);
            return false;
        }
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case CLEAR_PLAYLIST:
                ...
                return true;
            case ADD_SONG:
                intent = new Intent(CHOICE_FILEMANAGER_ACTION);
                intent.addCategory(Intent.CATEGORY_DEFAULT);
                startActivityForResult(intent, ADD_SONG);
                MusicLogUtils.d(TAG, "Add attachment FileManager, send intent " + intent.toString());
                return true;
            case EFFECTS_PANEL:
                try {
                    Intent i = new Intent(AudioEffect.ACTION_DISPLAY_AUDIO_EFFECT_CONTROL_PANEL);
        ...
        switch (requestCode) {
            ...
            case ADD_SONG:
                if (intent != null && intent.getData() != null ) {
                    addSongToPlaylistFromUri(intent.getData());
                }
                break;
        }
    }

    private void addSongToPlaylistFromUri(Uri uri) {
        ...
        MusicLogUtils.d(TAG, "addSongToPlaylistFromUri: data=" + data);
        data = data.replaceAll("'", "''");            
        String where = "_data LIKE '%" +data.replaceFirst("file:///", "") + "'";                
        MusicLogUtils.d(TAG, "addSongToPlaylistFromUri: where=" + where);
        ...
        if (c != null) {
            ...
            if (numOfRecords > 0) {
                ...
                MusicLogUtils.d(TAG, "addSongToPlaylistFromUri: track id=" + trackId);
                long[] list = new long[] { trackId };
                MusicLogUtils.d(TAG, "addSongToPlaylistFromUri: mPlaylist=" + mPlaylist);                
                if (mPlaylist.equals("nowplaying")) {
                    MusicUtils.addToCurrentPlaylist(this, list);
                } else {
                ...
                toastShow = getResources().getString(R.string.add_zero_file);
            }
            c.close();
            c = null;
        } else {
        ...
        private void makeNowPlayingCursor() {
            ...
            try {
                ...
                for (int i = mNowPlaying.length - 1; i >= 0; i--) {
                    ...
                    if (crsridx < 0) {
                        //MusicLogUtils.i("@@@@@", "item no longer exists in db: " + trackid);
    ...
    static class TrackListAdapter extends SimpleCursorAdapter implements SectionIndexer {
        ...
        class TrackQueryHandler extends AsyncQueryHandler {
            ...
            protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
                if(null != cursor) {
                    MusicLogUtils.i(TAG, "query complete: " + cursor.getCount() + "   " + mActivity);
                }
                else {
        ...
        public View newView(Context context, Cursor cursor, ViewGroup parent) {
            ...
            if(mActivity.mEditMode) {
                vh.edit_icon = (ImageView) v.findViewById(R.id.icon);
            }
        ...
        public void bindView(View view, Context context, Cursor cursor) {
            ...
            // mark for show "0:00" when the duration is 0
            //if (secs == 0) {
            //    vh.duration.setText(" ");
            //} else */{
            ...
            if(mActivity.mEditMode) {
                ImageView iv_edit = vh.edit_icon;
                iv_edit.setImageResource(R.drawable.ic_playlist_move);
        ...
        protected Bitmap doInBackground(Long... albumId) {
            ...
            if (mBackgroundBitmap == null) {
                try {
                    ...
                    mBackgroundBitmap = MusicUtils.getArtwork(TrackBrowserActivity.this, -1, id, false);
                } catch (Exception ex) {
                    MusicLogUtils.e(TAG, "AlbumArtFetched called with wrong parameters");
                    return null;
                }
                MusicLogUtils.d(TAG, "AlbumArtFetcher: getArtwork returns " + mBackgroundBitmap);
                
            }
            // wait the new view finish measure and then set the bitmap to be background.
