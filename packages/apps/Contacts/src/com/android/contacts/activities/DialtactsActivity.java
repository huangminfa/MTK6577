...
public class DialtactsActivity extends TransactionSafeActivity implements ProviderStatusListener {
    ...
    public void onBackPressed() {
        ...
        Log.i(TAG, "onBackPressed() Mode:" + bAutoRejectedFilter + " Tab:" + mViewPager.getCurrentItem());

        if (mInSearchUi) {
            // We should let the user go back to usual screens with tabs.
            exitSearchUi();
        }
        else if (bAutoRejectedFilter && 
                TAB_INDEX_CALL_LOG == mViewPager.getCurrentItem()) {
    ...
    private class PostDrawListener implements android.view.ViewTreeObserver.OnPostDrawListener {
        public boolean onPostDraw() {
            ...
            Xlog.i("[AppLaunch]", "[AppLaunch] twelvekeydialer onPostDraw");
            if(mLaunched) {
                mLaunched = false;
