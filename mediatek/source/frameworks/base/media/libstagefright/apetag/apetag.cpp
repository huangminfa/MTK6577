...
bool APETAG::parsetag(const sp<DataSource> &source)
{
	...
	///LOGE("getSize = %x, %c, %c, %c, %x, %x, %x", fileoffset, u8TmpBuf[0], u8TmpBuf[1], u8TmpBuf[2], u8TmpBuf[0], u8TmpBuf[1], u8TmpBuf[2] );
    ...
    fileoffset  -= u32Size;
	///LOGE("APE_READ_HEADER_SIZE before %x, %x, %x, %x", u32Size, u32Item, *(u8TmpBuf+12), *(u8TmpBuf+13));
    if ((source->readAt(fileoffset, u8TmpBuf, APE_READ_HEADER_SIZE) != APE_READ_HEADER_SIZE))
    {
        return false;
    }
    ...
    if (memcmp(u8TmpBuf, APE_TAG_MAGIC_ID, APE_TAG_MAGIC_SIZE)) 
	{	
		LOGV("APE_TAG_MAGIC_ID header error");
        return false;
    }

    if ((source->readAt(fileoffset+8, &u32Ver, 4) != 4))
    {
    	LOGV("read version error");
        return false;
    }
	
    if( u32Ver != 2000)
	{
		LOGV(" version error: %x", u32Ver);
		return false;
	}
...
void APETAG::Iterator::getString(String8 *id) const 
{
    ...
    for(int i=0; i<n ;i++)
        LOGE("getString() rawdata %x!!!!!",  *(temp+i));
...
///			LOGE("meta_tag_find_terminator --%c, %c", id[t], mID[t]);
        ...
        if (!strcmp(id, mID)) {
			LOGV("meta_tag_find_terminator ok !!!!!");
            break;
        }
