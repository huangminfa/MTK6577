...
static int bt_wake_up_chip(int bt_fd)
{
    ...
    /* If the receive event is not wake up complete event,
    ...
    while(1) {
        ...
        if(ret > 0){
            if(fds[0].revents & POLLIN){
                LOG_DBG("EINT receives! Host notify wake up\n");
                sleep_mode = 0;
                ...
                LOG_DBG("EINT monitor need to exit\n");
                goto exit;
            }
        }
...
int mtk_bt_enable(int flag, void *func_cb)
{
    ...
    if (!mtk || !bt_restore || !write_comm_port || !read_comm_port ||
        !bt_send_data || !bt_receive_data){
...
int mtk_bt_write(int bt_fd, unsigned char *buffer, unsigned long len)
{
    ...
    if (!glib_handle || !write_comm_port){
        LOG_ERR("mtk bt library is unloaded!\n");
        return -1;
    }
