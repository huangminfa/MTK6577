...
public class SettingsProvider extends ContentProvider {
    private static final String TAG = "Settings/Provide";
    ...
    private void sendNotify(Uri uri) {
        ...
        if (property != null) {
            ...
            if (LOCAL_LOGV) Xlog.v(TAG, "property: " + property + "=" + version);
        ...
        if (notify == null || "true".equals(notify)) {
            ...
            if (LOCAL_LOGV) Xlog.v(TAG, "notifying: " + uri);
        } else {
            if (LOCAL_LOGV) Xlog.v(TAG, "notification suppressed: " + uri);
        }
    }
        ...
        try {
            synchronized (cache) {
                ...
                if (rows > MAX_CACHE_ENTRIES) {
                    ...
                    Xlog.d(TAG, "row count exceeds max cache entries for table " + table);
                }
                Xlog.d(TAG, "cache for settings table '" + table + "' rows=" + rows + "; fullycached=" +
    ...
    private boolean ensureAndroidIdIsSet() {
        ...
        try {
            ...
            if (value == null) {
                ...
                Xlog.d(TAG, "Generated and saved new ANDROID_ID [" + newAndroidIdValue + "]");
    ...
    private Bundle lookupValue(String table, SettingsCache cache, String key) {
        synchronized (cache) {
            ...
            Xlog.d(TAG,"lookupValue, table "+table+" cache.containsKey " + key);
            if (value != null) {
        ...
        try {
            ...
            Xlog.d(TAG, "settings lookup error", e);
            return null;
        } finally {
    ...
    public Cursor query(Uri url, String[] select, String where, String[] whereArgs, String sort) {
        ...
        if (TABLE_FAVORITES.equals(args.table)) {
            ...
            if (cursor != null) {
                ...
                cursor.close();
                    Xlog.d(TAG,args.table+" query cursor's content is exists? "+String.valueOf(exists));
                if (!exists) return null;
            } else {
                    Xlog.d(TAG, args.table+" query cursor is null");
                return null;
            }
        }
        ...
        Cursor ret = qb.query(db, select, args.where, args.args, null, null, sort);
            Xlog.d(TAG,args.table+" query Cursor ret = "+String.valueOf(ret));
    ...
    public int bulkInsert(Uri uri, ContentValues[] values) {
        ...
        try {
            ...
            for (int i = 0; i < numValues; i++) {
                ...
                if (LOCAL_LOGV) Xlog.v(TAG, args.table + " <- " + values[i]);
            }
    ...
    public Uri insert(Uri url, ContentValues initialValues) {
        ...
        if (TABLE_FAVORITES.equals(args.table)) {
            Xlog.v(TAG, "insert, TABLE_FAVORITES.equals"+args.table);
            return null;
        }
        checkWritePermissions(args);
        ...
        if (SettingsCache.isRedundantSetValue(cache, name, value)) {
            Xlog.d(TAG,"SettingsCache.isRedundantSetValue "+name+" "+value);
            return Uri.withAppendedPath(url, name);
        }

        sKnownMutationsInFlight.incrementAndGet();
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        Xlog.d(TAG,"table "+args.table+" size" + db.getPageSize());
        final long rowId = db.insert(args.table, null, initialValues);
        Xlog.d(TAG,"table "+args.table+" rowId "+rowId+" size "+db.getPageSize());
        sKnownMutationsInFlight.decrementAndGet();
        if (rowId <= 0) return null;

        SettingsCache.populate(cache, initialValues);  // before we notify

        if (LOCAL_LOGV) Xlog.v(TAG, args.table + " <- " + initialValues);
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        sKnownMutationsInFlight.incrementAndGet();
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        Xlog.d(TAG, args.table + " delete "+args.where+" "+args.args);
        ...
        startAsyncCachePopulation();
        if (LOCAL_LOGV) Xlog.v(TAG, args.table + ": " + count + " row(s) deleted");
        return count;
    }
    ...
    public int update(Uri url, ContentValues initialValues, String where, String[] whereArgs) {
        ...
        startAsyncCachePopulation();
        if (LOCAL_LOGV) Xlog.v(TAG, args.table + ": " + count + " row(s) <- " + initialValues);
        return count;
    }
   ...
   private Uri getDefaultUri(Context context, int type) {
        ...
        if(type == RingtoneManager.TYPE_NOTIFICATION) {
            ...
            Xlog.e(TAG, "getDefaultUri: unexpectedType = " + type);
            return null;
        }
    ...
    private static final class SettingsCache extends LruCache<String, Bundle> {
        ...
        public static void populate(SettingsCache cache, ContentValues contentValues) {
            ...
            if (name == null) {
                Xlog.w(TAG, "null name populating settings cache.");
                return;
            }
        ...
        public void populate(String name, String value) {
            synchronized (this) {
                if (value == null || value.length() <= MAX_CACHE_ENTRY_SIZE) {
                    Xlog.d(TAG,"Cache populate "+name+" "+String.valueOf(value));
                    put(name, Bundle.forPair(Settings.NameValueTable.VALUE, value));
                } else {
                    Xlog.d(TAG,"Cache populate TOO_LARGE_TO_CACHE_MARKER "+name+" "+String.valueOf(value));
        ...
        public static boolean isRedundantSetValue(SettingsCache cache, String name, String value) {
            ...
            synchronized (cache) {
                ...
                Xlog.d(TAG,"isRedundantSetValue, oldValue"+String.valueOf(oldValue)+"value"+String.valueOf(value));
