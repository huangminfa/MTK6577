...
public class CallLogFragment extends ListFragment implements ViewPagerVisibilityListener,
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedState) {
        ...
        //The following lines are provided and maintained by Mediatek Inc.
        typeFilter_all = (Button) view.findViewById(R.id.btn_type_filter_all);
        typeFilter_outgoing = (Button) view.findViewById(R.id.btn_type_filter_outgoing);
        typeFilter_incoming = (Button) view.findViewById(R.id.btn_type_filter_incoming);
        typeFilter_missed = (Button) view.findViewById(R.id.btn_type_filter_missed);

        layout_autorejected = (LinearLayout) view.findViewById(R.id.calllog_auto_rejected_cluster);
        layout_searchbutton = (LinearLayout) view.findViewById(R.id.calllog_search_button_cluster);
        ...
        if (mShowOptionsMenu) {
            ...
            if (itemDeleteAll != null) {
                ...
                menu.findItem(R.id.show_all_calls).setVisible(
                        mVoicemailSourcesAvailable && mShowingVoicemailOnly);
                boolean bShowAutoRejectedMenu = false;
                if ("OP01".equals(OperatorUtils.getOptrProperties())) {
                    bShowAutoRejectedMenu = true;
                }
                bShowAutoRejectedMenu &= (!isAutoRejectedFilterMode());
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.show_auto_rejected_calls:
                ...
                simFilter = prefs.getInt(Constants.SIM_FILTER_PREF, Constants.FILTER_SIM_DEFAULT);
                typeFilter = prefs.getInt(Constants.TYPE_FILTER_PREF, Constants.FILTER_TYPE_AUTO_REJECT);
                mCallLogQueryHandler.fetchCallsJionDataView(simFilter, Constants.FILTER_TYPE_AUTO_REJECT);
                changeButton(null);
                return true;
    ...
    private LinearLayout layout_autorejected;
    ...
    private void changeButton(View view) {
        ...
        if (null == view) {
            if (FLAG_FILTER_MODE_AUTO_REJECTED != miModeFlag) {
                miModeFlag = FLAG_FILTER_MODE_AUTO_REJECTED;
                SharedPreferences.Editor editor = PreferenceManager.getDefaultSharedPreferences(
                        this.getActivity()).edit();
        ...
        if (FLAG_FILTER_MODE_NORMAL != miModeFlag) {
            setAutoRejectedModeVisibility(View.GONE);
            miModeFlag = FLAG_FILTER_MODE_NORMAL;
        }
    ...
    private void setAutoRejectedModeVisibility(int visibility) {
        if (View.VISIBLE == visibility) {
            ...
            layout_autorejected.setVisibility(View.VISIBLE);
        } else {
            layout_autorejected.setVisibility(View.GONE);
            layout_searchbutton.setVisibility(View.VISIBLE);
        }
    }
    ...
    private int miModeFlag = FLAG_FILTER_MODE_NORMAL;
    private View mViewRestored = null;

    public boolean isAutoRejectedFilterMode() {
        return (FLAG_FILTER_MODE_AUTO_REJECTED == miModeFlag);
    }

    public void onBackHandled() {
        Log.i(TAG, "onBackHandled() Mode:" + miModeFlag + " View:" + mViewRestored);
        if (isAutoRejectedFilterMode()) {
            if (null == mViewRestored) {
                mViewRestored = typeFilter_all;
            }
