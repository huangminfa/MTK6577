...
public class SlideshowDataAdapter implements SlideshowPage.Model {
    ...
    private GalleryActivity mActivity = null;
    ...
    public SlideshowDataAdapter(GalleryContext context, SlideshowSource source, int index,
        ...
        if (mIsDrmSupported) {
            mActivity = (GalleryActivity)context;
        }
    }
    ...
    private Bitmap getDrmThumb(MediaItem item, JobContext jc) {
        if (!(item instanceof LocalMediaItem) || 
            !((LocalMediaItem)item).isDrm()) {
            return item.requestImage(MediaItem.TYPE_THUMBNAIL).run(jc);
        }
        //now we are sure to treating local drm media
        LocalMediaItem localMediaItem = (LocalMediaItem)item;
        boolean isLocalVideo = localMediaItem instanceof LocalVideo;
        boolean isFlDrm = localMediaItem.isDrmMethod(
                                         DrmStore.DrmMethod.METHOD_FL);
        if (isLocalVideo || isFlDrm) {
            return DrmHelper.createDrmThumb((Activity)mActivity, jc,
                        (LocalMediaItem)item, MediaItem.TYPE_THUMBNAIL);
        } else {
            return DrmHelper.createDrmThumb((Activity)mActivity, jc,
                        (LocalMediaItem)item, MediaItem.TYPE_MICROTHUMBNAIL);
        }
    }
