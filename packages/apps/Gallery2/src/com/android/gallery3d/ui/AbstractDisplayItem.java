...
public abstract class AbstractDisplayItem extends DisplayItem {
    ...
    protected void updateImage(Bitmap bitmap, boolean isCancelled) {
        ...
        // when local drm media has no valid right, display
        // a default gray icon
        if (mIsDrmSupported &&
            (mMediaItem instanceof LocalMediaItem) &&
            ((LocalMediaItem)mMediaItem).isDrm() &&
            !((LocalMediaItem)mMediaItem).hasDrmRights()) {
            bitmap = null;
        }
