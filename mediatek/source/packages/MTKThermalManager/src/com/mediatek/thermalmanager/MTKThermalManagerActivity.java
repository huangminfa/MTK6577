...
public class MTKThermalManagerActivity extends Activity implements OnItemClickListener, CompoundButton.OnCheckedChangeListener, OnClickListener{
  ...
  private Switch thermal_protect_switch;
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    thermal_protect_switch = (Switch) findViewById(R.id.switch_thermal_protection);
    if (thermal_protect_switch != null) {
      thermal_protect_switch.setOnCheckedChangeListener(this);
    }
  ...
  public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
    ...
    if (((CompoundButton) thermal_protect_switch).equals(buttonView))
    {
      // Control...
      // Turn on/off all thermal protection...
      if (isChecked)
      {
        // Load current settings and write to procfs
        // Load current settings from /system/etc/.tp/thermal.curr.conf
        // TODO: If not found, copy thermal.conf (default) to thermal.curr.conf
        if(new File("/etc/.tp/thermal.curr.conf").exists())
        {
          ...
          turnOnThermalProtection("/etc/.tp/thermal.curr.conf");
          Toast.makeText(this, 
                  "Thermal protection is on",Toast.LENGTH_SHORT).show();
          if (true == thermal_log_on)
          {
            turnOnThermalLogger();
          }
        }
        else if (new File("/etc/.tp/thermal.conf").exists())
        {
          boolean thermal_log_on = isThermalLoggerEnabled();
          if (true == thermal_log_on)
          {
          ...
          turnOnThermalProtection("/etc/.tp/thermal.conf");
          Toast.makeText(this, 
                  "Thermal protection is on",Toast.LENGTH_SHORT).show();
          if (true == thermal_log_on)
          {
        ...
        {
          Toast.makeText(this, "Error! No thermal settings available.", Toast.LENGTH_SHORT).show();
          thermal_protect_switch.toggle();
          thermal_protect_switch.invalidate();
        }
      }
      else
      {
        // Turn off all thermal protections...
        // This settings should be a customized setting...
        // Load off settings from /system/etc/.tp/thermal.off.conf
        if(new File("/etc/.tp/thermal.off.conf").exists())
        {
          boolean thermal_log_on = isThermalLoggerEnabled();
          if (true == thermal_log_on)
          {
          ...
          turnOffThermalProtection();
          Toast.makeText(this, 
                  "Thermal protection is off",Toast.LENGTH_SHORT).show();
        ...
        {
          Toast.makeText(this, "No way to turn off thermal protection!", Toast.LENGTH_SHORT).show();
          //thermal_protect_switch.setChecked(false);
          thermal_protect_switch.toggle();
          thermal_protect_switch.invalidate();
        }
      }
    }
    else if (((CompoundButton) thermal_logger_switch).equals(buttonView))
    {
      if (isChecked)
      {
      ...
      else
      {
        if (isThermalLoggerEnabled())
        {
          ...
          if(new File("/mnt/sdcard").exists())
          {
            ...
            Xlog.d("MTKThermalManagerActivity", "onCheckedChanged() sdcard does exist!");
            //executeShellCommand("cat /proc/driver/storage_logger_display > /mnt/sdcard/storage_logger_dump"+ctstr);
            //executeShellCommand("/system/bin/cat /proc/driver/storage_logger_display > /data/tlog_"+ctstr);
            //Xlog.d("MTKThermalManagerActivity", "onCheckedChanged() finish cat");
            
            //Xlog.d("MTKThermalManagerActivity", "onCheckedChanged() test write to sdcard");
            //writeFile("/mnt/sdcard/storage_logger_dump_fake"+ctstr, "fake\n");
            ...
            //Xlog.d("MTKThermalManagerActivity", "onCheckedChanged() test cat to sdcard");
            //executeShellCommand("cat 12345 > /mnt/sdcard/storage_logger_dump_fake"+ctstr);
            //Xlog.d("MTKThermalManagerActivity", "onCheckedChanged() test cat to /data");
            //executeShellCommand("/system/bin/cat 12345 > /data/tlogf_"+ctstr);
  ...
  public void onClick(View v)
  {
    ...
    if (new File(newPolicyFilePath).exists())
    {
      ...
      if (newPolicyFilePath.contentEquals("/etc/.tp/thermal.conf"))
      {
        thermal_protect_switch.setChecked(true);
      }
      else
      {
        thermal_protect_switch.setChecked(false);
