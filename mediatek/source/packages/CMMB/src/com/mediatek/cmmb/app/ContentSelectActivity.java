...
public class ContentSelectActivity extends Activity implements View.OnClickListener {
    ...
    private static final String[] PROJECTION = new String[]{
        SG.PurchaseItemContent.GLOBAL_PURCHASE_ITEM_ID,
        SG.PurchaseItemContentDetail.PURCHASE_DATA_ID,
        SG.PurchaseItemContent.NAME,
        SG.PurchaseItemContentDetail.MONEY_INFO,
        SG.PurchaseItemContentDetail.START_TIME,
        SG.PurchaseItemContentDetail.END_TIME,
        SG.PurchaseItemContentDetail.GLOBAL_CONTENT_ID,
        SG.PurchaseItemContent._ID,
        SG.PurchaseItemContentDetail.PERIOD,
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mListAdapter = new OrderAdapter(this, R.layout.purchase_content_item, null,
                new String[]{}, new int[]{});
        ...
        mUIHandler = new Handler(){
            ...
            public void handleMessage(Message msg) {
                int id = msg.what;
                int result = msg.arg1;
                if (LOG) Log.i(TAG, "handleMessage() id=" + id + ", result=" + result);
                switch(id) {
    ...
    private void updateDisplay() {
        if (LOG) Log.i(TAG, "updateDisplay() mStep=" + mStep);
    ...
    private void listStep() {
        if (LOG) Log.i(TAG, "listStep() mSelectCursor=" + mSelectCursor);
        ...
        if (LOG) Log.i(TAG, "listStep() mLastSelectPosition=" + mLastSelectPosition);
    }
    ...
    private void newRow(Cursor sourceCursor, MatrixCursor target) {
        ...
        if (checked != null && checked.booleanValue() == true) {
            ...
            if (LOG) Log.i(TAG, "newRow() globalPurchaseId=" + globalPurchaseId + ", mNondupPrice=" + mNondupPrice);
        }
    }
    
    private void selectStep() {
        if (LOG) Log.i(TAG, "selectStep() mSelectCursor=" + mSelectCursor);
        mStep = STEP_SELECT;
        mListAdapter.getQueryHandler().startQuery(0, null, 
                SG.PurchaseItemContentDetail.CONTENT_URI,
                PROJECTION,
                SG.PurchaseItemContentDetail.RESPONSE_CODE + "=? ",
                new String[]{"0"},
                SG.PurchaseItemContentDetail.START_TIME
                );
    }
    ...
    /*package*/ class OrderAdapter extends SimpleCursorAdapter {
        class QueryHandler extends AsyncQueryHandler {
            ...
            protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
                if (LOG) Log.i(TAG, "onQueryComplete() cursor=" + cursor + ", mLastSelectPosition=" + mLastSelectPosition);
                ...
                if (LOG) Log.i(TAG, "onQueryComplete(" + token + "," + cookie + "," + cursor + ")");
        ...
        public OrderAdapter(Context context, int layout, Cursor c,
                String[] from, int[] to) {
        ...
        public void bindView(View view, Context context, Cursor cursor) {
            ...
            holder.moneyInfoView.setText(getMoney(
                    MBBMSStore.SG.PurchaseData.SUBSCRIPTION_TYPE_ENUM_CONTENT_TIMES,
            ...
            if (LOG) Log.i(TAG, "bindView() dataHolder=" + dataHolder);
        }
        
        @Override
        public void changeCursor(Cursor cursor) {
            super.changeCursor(cursor);
            if (LOG) Log.i(TAG, "changeCursor(" + cursor + ")");
        }
    ...
    /*package*/ class DataHolder {
        String purchaseGlobalId;
        String contentGlobalId;
        ...
        public String toString() {
            return new StringBuilder()
                .append("DataHolder(purchaseGlobalId=")
                .append(purchaseGlobalId)
                .append(", contentGlobalId=")
                .append(contentGlobalId)
                .append(", purchaseDataId=")
                .append(purchaseDataId)
                .append(", contentRowId=")
                .append(contentRowId)
                .append(", contentName=")
    ...
    private class ScrollPosition {
        ...
        public String toString() {
            return new StringBuilder()
                .append("ScrollPosition(firstVisiblePosition=")
                .append(firstVisiblePosition)
                .append(", firstTop=")
                .append(firstTop)
                .append(")")
                .toString();
    ...
    private String getLanguageString(String allString) {
        ...
        if (LOG) Log.i(TAG, "getLanguageString() = " + detail);
        return detail;
    }
    ...
    /*package*/ boolean getChecked(Long contentRowId) {
        ...
        if (LOG) Log.i(TAG, "getChecked(" + contentRowId + ") return " + result);
        return result;
    }
    ...
    /*package*/ void setChecked(Long contentRowId, boolean value) {
        ...
        if (LOG) Log.i(TAG, "setChecked(" + contentRowId + ") return " + result);
    }
    
    public void onClick(View v) {
        if (LOG) Log.v(TAG, "onClick()");
        switch(v.getId()) {
        case R.id.ckbSelect:
    ...
    private void subscribeSelected() {
        if (LOG) Log.i(TAG, "subscribeSelected() mDisplayCursor=" + mDisplayCursor
                + ", mServiceManager=" + mServiceManager);
        if (mDisplayCursor != null && mDisplayCursor.getCount() > 0 && mServiceManager != null) {
            ...
            while(mDisplayCursor.moveToNext()) {
                ...
                if (LOG) Log.i(TAG, "subscribeSelected() contentRowId=" + contentRowId + ", contentName=" + contentName
                ...
                if (Utils.checkLocalTime() && now > endTime) {
                    ...
                    PurchaseItem item = new PurchaseItem(mDisplayCursor.getString(COLUMN_GLOBAL_ID),
                            mDisplayCursor.getString(COLUMN_PURCHASE_DATA_ID));
    ...
    /*package*/ class PurchaseItem {
        PurchaseItem(String purChaseGlobalId, String purchaseDataId) {
            this.purChaseGlobalId = purChaseGlobalId;
            this.purchaseDataId = purchaseDataId;
        }
        ...
        public boolean equals(Object o) {
            if (o instanceof PurchaseItem) {
                ...
                return equalString(purChaseGlobalId, right.purChaseGlobalId)
                        && equalString(purchaseDataId, right.purchaseDataId);
            }
            return false;
        }
        ...
        private boolean equalString(String l, String r) {
            ...
            if (l != null && r != null && l.equals(r)) return true;
            return false;
        }
        
        @Override
        public int hashCode() {
            String key = purChaseGlobalId + "-" + purchaseDataId;
            return key.hashCode();
        }
        ...
        public String toString() {
            return new StringBuilder()
            .append("PurchaseItem(purChaseGlobalId=")
            .append(purChaseGlobalId)
            .append(", purchaseDataId=")
            .append(purchaseDataId)
            .append(")")
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (subscribePurchase(allName, item.purChaseGlobalId, item.purchaseDataId)) {
                        successCount++;
                    }
                }
                if (LOG) Log.i(TAG, "subscribePurchase() successCount=" + successCount);
                if (successCount == 0) {
    ...
    private boolean subscribePurchase(String allName, final String globalId, String purchaseDataId) {
        if (LOG) Log.i(TAG, "subscribePurchase() start");
        ...
        if (LOG) Log.i(TAG, "subscribePurchase() msg.arg1=" + msg.arg1 + ", " + msg.obj);
        msg.sendToTarget();
        if (LOG) Log.i(TAG, "subscribePurchase() end return " + ret);
    ...
    private void showMessage(String contentName) {
        new AlertDialog.Builder(this)
            ...
            .setMessage(getString(R.string.fail_subscribe_for_end_program, contentName))
