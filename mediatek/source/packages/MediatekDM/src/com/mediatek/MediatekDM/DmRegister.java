...
public class DmRegister {
    ...
    public void registerCCNodeIoHandler(String treeFileName) {
        ...
        uriList = (ArrayList<String>) getCCUriString(treeParser,
                  getSettingNode(treeParser), DmConst.tagname.Name);
        String uriStr = null;
        String itemStr = null;
...
//			Log.w(TAG.Service, "uriStr: " + uriStr);
            ...
            if (itemStr != null) {
                ioHandler = mHandlerFactory.createNodeHandler(itemStr, Uri
                            .parse(uriStr), mMccMnc);
            }
            if (ioHandler != null) {
                try {
    ...
    private List<String> getCCUriString(DmXMLParser xmlParser, Node node,
                                        String nodeName) {
        ...
        for (int i = 0; i < nodeList.size(); i++) {
            ...
            if (uriStr != null) {
                if (uriStrList.size() > 0
                        && uriStr.contains(uriStrList.get(uriStrList.size() - 1))) {
    ...
    private String getDmTreeNodeName(DmXMLParser xmlParser, Node node) {
        ...
        // if (childNode.getNodeName().equals(DmConst.tagname.Name)) {
        // Log.w("haman", "nodeValue: "
    ...
    private Node getSettingNode(DmXMLParser xmlParser) {
        ...
        for (int level = 1; level < 0x000F; level++) {
            ...
            for (int i = 0; i < nodeList.size(); i++) {
                ...
                if (nodeName != null
                        && nodeName.equals(DmConst.tagname.Setting)) {
...
class NodeIoHandlerFactory {
    ...
    public NodeIoHandler createNodeHandler(String configItem, Uri uri,
        ...
        try {
            conWith3Args = ioHandlerClass.getConstructor(Context.class,
            ...
            try {
                conWith2Args = ioHandlerClass.getConstructor(Context.class,
                               Uri.class);
        ...
        try {
            if (conWith3Args != null) {
                Log.d(TAG.NodeIOHandler,"[NodeHandlerFactory]created: "
                      + configItem + "(" + uri + "," + parameterString + ")");
                return (NodeIoHandler) (conWith3Args.newInstance(mContext, uri,
                                        parameterString));
            } else {
