...
item_t rtype_touch_items[] = {
    item(ITEM_TOUCH_CALIBRATION,   "Calibration"),
    item(ITEM_TOUCH_LINEARITY,   "RTP Linearity"),
    item(ITEM_TOUCH_ACCURACY,    "RTP Accuracy"),
    item(ITEM_TOUCH_SENSITIVITY, "Sensitivity"),
    item(ITEM_TOUCH_DEADZONE,    "Deadzone"),
    item(ITEM_TOUCH_FREEMODE,    "FreeMode"),
    item(ITEM_TOUCH_PASS,        "Test Pass"),
    item(ITEM_TOUCH_FAIL,        "Test Fail"),
    item(-1            ,  NULL),
};

item_t ctype_touch_items[] = {
	item(ITEM_TOUCH_MAIN,   "CTP Test"),
...
static void paint_redraw(struct touch *tpd) {
    ...
    case ITEM_TOUCH_CALIBRATION:
		if(cross_num==0) { 
			ui_color(255,0,0,255);
			ui_text(tpd->width/10,tpd->height/2,"touch screen to start!");
		} else if(cross_num==1) {
			paint_cross(tpd,0); ui_text(tpd->width/10,tpd->height/2,"touch the red cross!");
		} else if(cross_num==2) {
			paint_cross(tpd,1); ui_text(tpd->width/10,tpd->height/2,"touch the red cross!");
		} else if(cross_num==3) {
			paint_cross(tpd,2); ui_text(tpd->width/10,tpd->height/2,"touch the red cross!");
		} else if(cross_num==4) {
			paint_cross(tpd,3); ui_text(tpd->width/10,tpd->height/2,"touch the red cross!");
		}
		break;
...
static void part(struct touch *tpd, int mode) {
    ui_color(255, 255, 255, 255);
	ui_text(TOUCH_L1 + 10,TOUCH_L1 + 20,"pass! tap to continue");
    ui_flip();
