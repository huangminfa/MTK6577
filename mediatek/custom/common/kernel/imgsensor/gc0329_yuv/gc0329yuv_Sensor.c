...
void GC0329_Sensor_Init(void)
{
	...
	{
		GC0329_write_cmos_sensor(0x17, 0x14); // 16   lanking 20120423  mirror setting   0x14
	}
	else
	{
		GC0329_write_cmos_sensor(0x17, 0x17); // 16   lanking 20120423  mirror setting   0x14
	}
...
*/
UINT32 GC0329_SetMirror(kal_uint8 uiMirror)
{
	kal_uint8 temp;

	GC0329_SET_PAGE0;
	temp = (kal_uint8)GC0329_read_cmos_sensor(0x17);	//  The default value of this reg. is 0x14
	temp &= 0xFC;
	if (DUAL_CAMERA_MAIN_SENSOR == g_currDualSensorIdx || DUAL_CAMERA_MAIN_SECOND_SENSOR == g_currDualSensorIdx)	// main sensor
	{
		//uiMirror = 0x03;
		switch (uiMirror)
		{
		case 0x01:		// horizontal mirror
			break;

		case 0x02:		// vertical mirror, that is called flip
			break;

		case 0x03:		// mirror + flip
			break;
		...
		temp |= uiMirror;
		GC0329_SET_PAGE0;
		GC0329_write_cmos_sensor(0x17, temp);
	}
	else
	{
		// In the project F88, sub sensor.
		uiMirror = 0x00; //0x03;
		switch (uiMirror)
		{
		case 0x01:		// horizontal mirror
...
BOOL GC0329_set_param_banding(UINT16 para)
{
	switch (para)
	{
		case AE_FLICKER_MODE_50HZ:
			...
			GC0329_write_cmos_sensor(0x2b, 0x03);   //exp level 0  14.28fps  //2a0
			...
			GC0329_write_cmos_sensor(0x2f, 0x04);   //exp level 2  10.00fps  //3c0
			GC0329_write_cmos_sensor(0x30, 0x80); 
			GC0329_write_cmos_sensor(0x31, 0x06);   //exp level 3  7.14fps   //540
