...
/*
	...
	public MessageListObject getMessageList(MessageListRequest req) {
		...
		while(messageCursor.moveToNext() && (listSize  == 0 || list.getCurrentSize() < listSize)) {
			...
			if (recipient != null && address != null && (mailbox != Sms.MESSAGE_TYPE_INBOX )) {				
				if (address.indexOf(recipient) == -1) {
					continue;
				}
			}
			
			//orignator filter
			if (orignator!= null && address != null && (mailbox == Sms.MESSAGE_TYPE_INBOX)) {
				if (address.indexOf(orignator) == -1) {
					continue;
				}
			}
	...
	private void handleSentResult(Intent intent, int resultCode){
			...
			if (mListener != null && id != INVALID_VALUE_ID){
				mListener.onMessageSent(id, mType, MAP.RESULT_OK);
			}
	}	
