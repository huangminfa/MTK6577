...
void (*ipod_chgcontrol)(int *, int (*pFunc1)(void), int (*pFunc2)(void), int (*pFunc3)(void), int (*pFunc4)(int, int, int)) = NULL;
...
void loadlib()
{
        ...
        ipod_init_fb = dlsym(handle, "ipod_init_fb");
        ...
        ipod_start = dlsym(handle, "ipod_start");
        if (ipod_start == NULL) {
	...
	ipod_chgcontrol = dlsym(handle, "ipod_chgcontrol");
		...
		ipod_trigger_chganim = dlsym(handle, "ipod_trigger_chganim");
...
int main(int argc, char *argv[]) 
{
	...
	enablePowerOff(IPO_POWEROFF_TIME);
#endif
