...
struct MSTDATA
{
   ...
   SpecialMessage *pSpecailMSGTableBegin;   //the beginning of special message table
   SpecialMessage *pSpecailMSGTableEnd;   //the ending of special message table
   int curEntry;
   int interVal;//1s 
...
struct MST
{
   ...
   bool (*MST_UpdateSpecialMsgTable)(struct MST * mst,void * pSpecialEvent,int length);
   bool (*MST_GetParsingEvent)(struct MST * mst,int seektoPostition,EventOffset * pEO);
   int  (*MST_GetSpecialMsgCount)(struct MST * mst,int seektoPostition);
   ...
   void (*MST_FilterMsg)(struct MST * mst,SpecialEvent * pSEvent);
};
