...
public class Utils {
    ...
    /**
     * Extra name for passing service name.
     * <br/>Type: String
     */
    public static final String EXTRA_SERVICE_NAME = "ServiceName";
    /**
     * Extra name for passing service frequency no.
     * <br/>Type: int
     */
    public static final String EXTRA_FREQUENCY_NO = "FrequencyNO";
    /**
     * Extra name for passing current service is favorited or not.
     * <br/>Type: int
     */
    public static final String EXTRA_IS_FAVORITE = "IsFavorite";
    /**
     * Extra name for passing current service is for free or not.
     * <br/>Type: int
     */
    public static final String EXTRA_FOR_FREE = "ForFree";    
    /**
     * Extra name for half screen, used by player.
     * <br/>Type: boolean
     */
    ...
    /*package*/ static class CachedPreview extends Handler {
        ...
        public CachedPreview(Context context, BitmapDrawable defaultDrawable, boolean isSGMode,
                int usage, DrawableStateListener listener) {
            mContext = context;
        ...
        private void initTask() {
            ...
            synchronized(CachedPreview.class) {
                if (sLooper == null) {
                    HandlerThread t = new HandlerThread("cached-preview-thread", android.os.Process.THREAD_PRIORITY_BACKGROUND);
                    t.start();
                    sLooper = t.getLooper();
                }
            }
        ...
        public Drawable getCachedPreview(int contentRowID) {
            if (LOG) Log.v(TAG, "getCachedPreview(" + contentRowID + ") mPreviewStatus=" + mAllPreviewStatus);
            initTask();
            registerObserver();
            ...
            if (LOG) Log.v(TAG, "getCachedPreview() mPreviewStatus=" + mAllPreviewStatus
                    + ", cachedDrawable=" + cachedDrawable + ", return " + result);
            return result;
        }
    ...
    /*package*/ static boolean isExternalStorageReady() {
        ...
        boolean isReady = (Environment.MEDIA_MOUNTED.equals(state) ||
    ...
    public static void readBroadcast(Context context, Uri uri){
        if (LOG) Log.v(TAG, "readBroadcast(" + uri + ")");
    ...
    public static Cursor getSearchCursor() {
        if (LOG) Log.i(TAG, "getSearchCursor() return " + sSearchCursor);
        return sSearchCursor;
    }
    
    public static void setSearchCursor(Cursor cursor) {
        if (LOG) Log.i(TAG, "setSearchCursor(" + cursor + ")");
    ...
    public static String getErrorDescription(Resources res, ServerStatus status, String defaultDescription) {
        if (LOG) Log.v(TAG, "parseStatus(" + status + ", " + defaultDescription + ")");
        if (status == null) {//always wrong
    ...
    public static SavedLocation parseSavedLocation(String saved) {
        if (LOG) Log.v(TAG, "parseSavedLocation(" + saved + ")");
    ...
    public static class SavedLocation {
        /**
         * Hold for unknown province. 
         * if user select suggestion, then province will be this value.
         */
        ...
        @Override
        public String toString() {
            return new StringBuilder().append("SavedLocation(province=")
            .append(province)
            .append(", city=")
            .append(city)
            .append(", county=")
