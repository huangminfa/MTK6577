...
public class NotificationMgr implements CallerInfoAsyncQuery.OnQueryCompleteListener{
            ...
            switch (token) {
                ...
                case CONTACT_TOKEN:
                    ...
                    if ((cursor != null) && (cookie != null)){
                        ...
                        notifyMissedCall(n.name, n.number, n.label, n.date, n.callVideo);
     ...
     */
    void notifyMissedCall(String name, String number, String label, long date, int callVideo) {
        ...
        int resId = android.R.drawable.stat_notify_missed_call;
        // MTK_OP01_PROTECT_START
        if ("OP01".equals(PhoneUtils.getOptrProperties())) {
            if (1 == callVideo) {
                resId = R.drawable.ic_stat_notify_missed_call_video;
            }
        }
    ...
    private void updateInCallNotification(boolean allowFullScreenIntent) {
        ...
        if (DualTalkUtils.isSupportDualTalk()) {
            ...
            ring_call = dt.getFirstActiveRingingCall();
        } else {
        ...
        if (currentConn != null) {
            ...
            if (true == FeatureOption.MTK_VT3G324M_SUPPORT) {
                if (currentConn.isVideo()
                           && VTCallUtils.VTTimingMode.VT_TIMING_NONE == VTCallUtils.checkVTTimingMode(currentConn.getAddress())) {
                    callDurationMsec = 0;
...
//            if (null != ringingCall && (ringingCall.getState() == Call.State.WAITING) && !mApp.isShowingCallScreen()) {
...
//                }
        ...
        /// M: Always update the notification.
        //if(isScreenOn){
    ...
    /* package */ void showDataDisconnectedRoaming(int simId) {
        ...
        else
        {
            intent = new Intent(mContext,
                                com.android.phone.Settings.class);
        }
