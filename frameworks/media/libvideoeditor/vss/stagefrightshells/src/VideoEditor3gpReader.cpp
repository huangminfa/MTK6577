...
M4OSA_ERR VideoEditor3gpReader_jump(M4OSA_Context context,
    ...
    if ((pC->mAudioStreamHandler != M4OSA_NULL) &&
        ...
        LOGV("VideoEditor3gpReader_jump AUDIO time us %lld ", time64);
    ...
    LOGV("VideoEditor3gpReader_jump time ms before seekset %lld ", time64);
...
M4OSA_ERR VideoEditor3gpReader_reset(M4OSA_Context context,
    ...
    if (pStreamHandler == (M4_StreamHandler*)pC->mAudioStreamHandler) {
		...
		if (M4NO_ERROR != err) {
			LOGD("VideoEditor3gpReader_reset @@no seek it to begin!");
		}
#endif
...
M4OSA_ERR VideoEditor3gpReader_getNextAu(M4OSA_Context context,
    ...
    if (mMediaBuffer != NULL) {
        ...
        #if 0 // Morris Yang 20120320 remove the weird behavior
...
M4OSA_ERR VideoEditor3gpReader_getNextStreamHandler(M4OSA_Context context,
    ...
    temp = pC->mCurrTrack;
#ifndef ANDROID_DEFAULT_CODE	
	LOGV("VideoEditor3gpReader_getNextStreamHandler trackCount %d",trackCount);
#endif
    if(temp >= trackCount) {
       ...
       #if 1  // Morris Yang 20120320 remove NAL prefix when using MTK AVC decoder [
            if (!strncasecmp(mime, "video/avc", 9)) {
                //LOGD ("@@ remove NAL prefix for MTK AVC decoder !!!");
        ...
        // <--- Morris Yang 20120402 for (128160 H264 mp3 32k 35 fps.mp4)
                ...
                LOGD("VE unknown audio sream:pC->mCurrTrack = %d", pC->mCurrTrack);
