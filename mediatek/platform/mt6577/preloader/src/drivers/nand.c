...
*  reset descriptor
**************************************************************************/
void mt6577_nand_reset_descriptor (void)
{
...
//---------------------------------------------------------------------------
static bool mt6577_nand_set_address (u32 u4ColAddr, u32 u4RowAddr, u16 u2ColNOB,
                         u16 u2RowNOB)
{
    /* fill cycle addr */
...
static bool mt6577_nand_read_status(void)
{
    int status, i;
    mt6577_nand_reset();
...
//---------------------------------------------------------------------------
static void mt6577_nand_command_bp (unsigned command)
{
    u32 timeout;
...
//-----------------------------------------------------------------------------
static u8 mt6577_nand_read_byte (void)
{
...
bool getflashid(u8 *nand_id, int longest_id_number)
{
	...
	mt6577_nand_command_bp (NAND_CMD_READID);

	maf_id = mt6577_nand_read_byte ();
	dev_id = mt6577_nand_read_byte ();
...
int mt6577_nand_init (void)
{
    ...
    nand_maf_id = mt6577_nand_read_byte ();
    nand_dev_id = mt6577_nand_read_byte ();
    id = nand_dev_id | (nand_maf_id << 8);

    ext_id1 = mt6577_nand_read_byte ();
    ext_id2 = mt6577_nand_read_byte ();
    ext_id3 = mt6577_nand_read_byte ();
...
*/
    ...
    //MSG(INIT, "+ mt6577_nand_init : Finish device query +\n");
...
//-----------------------------------------------------------------------------
static bool mt6577_nand_read_page_data (u32 * buf)
{
    u32 timeout = 0xFFFF;
...
//-----------------------------------------------------------------------------
static bool mt6577_nand_write_page_data (u32 * buf)
{
    u32 timeout = 0xFFFF;
    u32 u4Size = g_nand_chip.oobblock;
...
//-----------------------------------------------------------------------------
static void mt6577_nand_read_fdm_data (u32 u4SecNum, u8 * spare_buf)
{
    u32 i;
...
//-----------------------------------------------------------------------------
static void
mt6577_nand_write_fdm_data (u32 u4SecNum, u8 *oob)
{
    u32 i;
    u32 *pBuf32 = (u32 *)oob;
...
//---------------------------------------------------------------------------
static bool mt6577_nand_ready_for_read (u32 page_addr, u32 sec_num, u8 *buf)
{
...
//-----------------------------------------------------------------------------
static bool mt6577_nand_ready_for_write (u32 page_addr, u32 sec_num, u8 *buf)
{
    bool bRet = FALSE;
    u32 u4RowAddr = page_addr;
    u32 colnob = 2;
...
//#
//#############################################################################
int mt6577_nand_read_page_hwecc (unsigned int logical_addr, char *buf)
{
    ...
    if (!mt6577_nand_read_page_hw(page_in_block + mapped_block * NAND_BLOCK_BLKS, buf, g_nand_spare)) // g_nand_spare
        return FALSE;
...
int mt6577_nand_read_page_hw (u32 page, u8 *dat, u8 *oob)
{
    bool bRet = TRUE;
    u8 *pPageBuf;
    ...
    if (mt6577_nand_ready_for_read (page, u4SecNum, pPageBuf))
    {
        if (!mt6577_nand_read_page_data ((u32 *) pPageBuf))
        {
            bRet = FALSE;
        }
...
#if 0
    for (i = 0; mt6577_nand_oob.oobfree[i][1]; i++)
    {
        /* Set the reserved bytes to 0xff */
        start = mt6577_nand_oob.oobfree[i][0];
        len = mt6577_nand_oob.oobfree[i][1];
...
int mt6577_nand_write_page_hwecc (unsigned int logical_addr, char *buf)
{
    ...
    if (!mt6577_nand_write_page_hw(page_in_block + mapped_block * NAND_BLOCK_BLKS, 
            buf, g_nand_spare))
    {
...
int mt6577_nand_write_page_hw(u32 page, u8 *dat, u8 *oob)
{
    bool bRet = TRUE;
    ...
    if (mt6577_nand_ready_for_write (page, u4SecNum, dat))
    {
        mt6577_nand_write_fdm_data (u4SecNum, oob);
        if (!mt6577_nand_write_page_data ((u32 *)dat))
        {
            bRet = FALSE;
        }
        if (!mt6577_nand_check_RW_count (g_nand_chip.oobblock))
        {
            bRet = FALSE;
        }
        ...
        return mt6577_nand_read_status();
    }
    else
    {
        return FALSE;
    ...
    mt6577_nand_read_page_hwecc (logical_addr, g_nand_buf);
...
bool nand_block_bad_hw (u32 logical_addr)
{
    ...
    if (mt6577_nand_ready_for_read (page, u4SecNum, tmp))
    {
        if (!mt6577_nand_read_page_data ((u32 *) tmp))
        {
            bRet = FALSE;
        }
        ...
        mt6577_nand_read_fdm_data (u4SecNum, g_nand_spare);

        if (!mt6577_nand_check_bch_error(tmp, u4SecNum - 1, page))
        {
            MSG(ERASE, "check bch error !\n");
            bRet = FALSE;
        }
...
bool mark_block_bad_hw(u32 offset)
{
    ...
    //mt6577_nand_erase(logical_addr);
    //delay(1000);
    ...
    if (mt6577_nand_ready_for_write (page_addr, u4SecNum, buf))
    {
          mt6577_nand_write_fdm_data (u4SecNum, g_nand_spare);
          if (!mt6577_nand_write_page_data ((u32 *) & buf))
          {
                bRet = FALSE;
          }
          if (!mt6577_nand_check_RW_count (g_nand_chip.oobblock))
    ...
    mt6577_nand_read_page_hwecc (LOGICAL_ADDR (page_addr), tmp);
...
//#
//#############################################################################
int mt6577_nand_erase (u32 logical_addr)
{
    int block = logical_addr / g_nand_chip.erasesize;
    int mapped_block = get_mapping_block_index(block);

    if (!mt6577_nand_erase_hw(mapped_block * g_nand_chip.erasesize))
    {
...
bool mt6577_nand_erase_hw (u32 offset)
{
    bool bRet = TRUE;
    ...
    mt6577_nand_set_address (0, page_addr, 0, rownob);
...
bool mt6577_nand_wait_for_finish (void)
{
    while (DRV_Reg32 (NFI_STA_REG32) & STA_NAND_BUSY);
    return TRUE;
}
