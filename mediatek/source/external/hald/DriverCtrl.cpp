...
int DriverCtrl::load(int u4NetIf)
{
    ...
    LOGD("Load_driver %d", u4NetIf);

#ifdef LOAD_MODULE_ONCE

    if (!checkLoaded(tarDrv->acModuleTag, tarDrv->acPropName)){
        if (sModuleCtrl->insmod( tarDrv->acModulePath, tarDrv->acModuleArg) < 0)
            goto error;
    }
    ...
    if (sModuleCtrl->insmod(tarDrv->acModuleName, tarDrv->acModuleArg) < 0)
        goto error;
...
int DriverCtrl::unload(int u4NetIf)
{
    ...
    char legacyModuleName[32];
    char legacyModuleTag[32];
    char *end;

    LOGD("Unload_driver %d", u4NetIf);

    LOGD("rmmod %s", tarDrv->acModuleName);
    ...
    if(!end) {
        LOGD("unable to locate legacy module name");
        return -1;
    }
    *end = '\0';
    sprintf(legacyModuleTag, "%s ", legacyModuleName);

    LOGD("rmmod %s", legacyModuleName);
