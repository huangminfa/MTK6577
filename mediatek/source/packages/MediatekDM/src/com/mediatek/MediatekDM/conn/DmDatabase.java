...
public class DmDatabase {
    ...
    public boolean updateDmServer(int simId, String serverAddr) {
        ...
        Log.i(TAG.Database, "The mccmnc is = " + mccMnc
              + " and the register mccmnc is " + mccMncRegistered);
        if (mccMnc.equals(mccMncRegistered) || serverAddr == null) {
            Log.e(TAG.Database,
                  "It is not the right mccmnc or server address is null!");
            return false;
        }
    ...
    public boolean DmApnReady(int simId) {
        ...
        try {
            if (MTKOptions.MTK_GEMINI_SUPPORT == true) {
                if (simId == GEMINI_SIM_1) {
                    mCursor = mContentResolver.query(dmUri, null, null, null,
                                                     null);
                } else if (simId == GEMINI_SIM_2) {
                    mCursor = mContentResolver.query(dmUriGemini, null, null,
                                                     null, null);
                }
            } else {
                mCursor = mContentResolver.query(dmUri, null, null, null, null);
            }

            if (mCursor != null && mCursor.getCount() > 0) {
                Log.w(TAG.Database,
                      "There are apn data in dm apn table, the record is "
                      + mCursor.getCount());
                return true;
            }
    ...
    private void getApnValuesFromConfFile() {
        ...
        if (nodeList != null && nodeList.size() > 0) {
            ...
            operatorName = DmCommomFun.getOperatorName();
    ...
    private boolean InitDmApnTable(ArrayList<DmApnInfo> apnInfo) {
        ...
        ArrayList<ContentValues> apnInfo_insert = new ArrayList<ContentValues>(
            size);
        ...
        try {
            ...
            if (MTKOptions.MTK_GEMINI_SUPPORT == true) {
                ...
                if (simId == 1) {
                    ...
                    mCursor = mContentResolver.query(dmUriGemini, null, where,
                                                     null, null);
                } else if (simId == 0) {
                    mCursor = mContentResolver.query(dmUri, null, where, null,
                                                     null);
                } else {
                ...
                mCursor = mContentResolver
                          .query(dmUri, null, where, null, null);
            }
            if (mCursor == null || mCursor.getCount() <= 0) {
            ...
            Log.e(TAG.Database,
                  "There are some exception accured during finding proxy address");
        } finally {
            if (mCursor != null) {
                mCursor.close();
            }
    ...
    public int getApnProxyPortFromSettings() {
        ...
        try {
            ...
            Log.e(TAG.Database,
                  "There are some exception accured during finding serveraddr");
        } finally {
            if (mCursor != null) {
                mCursor.close();
            }
