...
sp<ICamera> CameraService::connect(
    ...
    if (cameraId == 0xFF) {
        ...
        LOGD("It's connected from atv \n");
    }
    else {
        if (cameraId < 0 || cameraId >= (mNumberOfCameras - 1)) {
            LOGE("CameraService::connect X (pid %d) rejected (invalid cameraId %d), (mNumberOfCameras-1=%d).",
...
#ifdef  MTK_CAMERA_BSP_SUPPORT
    if (mHardware == 0) {
        LOGW("[disableMsgType] mHardware == 0 (CallingPid %d) (tid %d)", getCallingPid(), ::gettid());
        return;
    }
#endif
