...
#include "include/mtk_gps.h"
/*---------------------------------------------------------------------------*/
#define LOG_TAG "MNLD"
#include <cutils/sockets.h>
#include <cutils/xlog.h>     /*logging in logcat*/

#define MND_ERR(f, arg ...) XLOGE("%s: " f, __FUNCTION__ ,##arg)

#if LOG_ENALBE
#define MND_MSG(f, arg ...) XLOGD("%s: " f, __FUNCTION__ ,##arg)
#define MND_TRC(f)          XLOGD("%s\n", __FUNCTION__)
#define MND_VER(f, arg ...) XLOGD("%s: " f, __FUNCTION__ ,##arg) 
#else
#define MND_MSG(f, arg ...) ((void)0) 
#define MND_TRC(f)          ((void)0) 
#define MND_VER(f, arg ...) ((void)0)
#endif 
...
#define MNL_TIMEOUT_SLEEP   1
#define MNL_TIMEOUT_PWROFF  1 //Modify by LCH
/*---------------------------------------------------------------------------*/
...
{   
    ...
    *eof = 0;
    ...
    if (cmd == MNL_CMD_INIT) {
        ...
        kill_mnl_process();
        if ((ret = mnl_set_pwrctl(GPS_PWRCTL_OFF)))
            return ret;    
        return mnl_set_state(GPS_STATE_PWROFF);        
    } else if (cmd == MNL_CMD_START) {
        return start_mnl_process();
