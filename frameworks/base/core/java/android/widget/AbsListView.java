...
public abstract class AbsListView extends AdapterView<ListAdapter> implements TextWatcher,
	...
	private static final boolean DBG_DATACHANGE = false;
	private static final boolean DBG_LAYOUT = false;
	private static final boolean DBG_RECYCLE = false;
	static final boolean DBG_DRAW = false;

	/**
	 * The magnification of list over fling velocity.
	 */
    ...
    public void setFilterText(String filterText) {
        ...
        if (mTextFilterEnabled && !TextUtils.isEmpty(filterText)) {
            ...
            /*
             * CR 125819, setSelection will made the text show abnormal when the
             * filterText is too long.
             */
    ...
    protected int computeVerticalScrollExtent() {
        ...
        if (count > 0) {
            if (mSmoothScrollbarEnabled) {
                ...
                /*
                 * If content fits, return mItemCount (the same as scroll
                 * range), so the list can not scroll vertically (used in
                 * View.overScrollBy()), else if there is only one item in
                 * ListView and it is big enough(exceeds the height of
                 * ListView), we enable over scroll function on it, set the
                 * scroll extent to 0(scroll range is 1) to make it works. In
                 * other cases we return 1 by default.
                 */
                if (contentFits()) {
                    if (DBG_MOTION) {
                        Xlog.d(TAG, "Smooth scroll disabled and content fits, mItemCount = "
                                + mItemCount + ",count = " + count + ",this = " + this);
                    }
                    return mItemCount;
                } else {
                    if (DBG_MOTION) {
                        Xlog.d(TAG, "Smooth scroll disabled and content exceed ListView, mItemCount = "
                                + mItemCount + ",count = " + count + ",this = " + this);
                    }
                    return (mItemCount == 1) ? 0 : 1;
                }
            }
        }
        return 0;
            ...
            if (y != mLastY) {
                ...
                if (motionView != null) {
                    ...
                    if (atEdge) {
                        ...
                        overscroll = calcRealOverScrollDist(overscroll);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        /*
         * Abnormal case, intent to operate on non-exist data.
         */
...
//                mDirection = 0;
        ...
        case MotionEvent.ACTION_UP: {
            ...
            case TOUCH_MODE_DONE_WAITING:
                ...
                if (child != null && !child.hasFocusable() && inList) {
                    ...
                    // If user release the finger in the padding area, we need to reset the 
            ...
            case TOUCH_MODE_OVERSCROLL:
                ...
                // Decrease the velocity because it is too sensitive.
                initialVelocity = initialVelocity / OVERFLING_VELOCITY_MAGN;

                Xlog.i(TAG, "Fling back from over scroll with initialVelocity = "
                        + initialVelocity + ",mScrollY = " + mScrollY
                        + ",mFirstPosition = " + mFirstPosition + ",mChildCount = "
            ...
            case TOUCH_MODE_REST: 
                /* When touch the list item and press the key, touch mode will change to TOUCH_MODE_REST*/
    ...
    private class FlingRunnable implements Runnable {
        ...
        private final Runnable mCheckFlywheel = new Runnable() {
            public void run() {
                ...
                if (Math.abs(yvel) >= mMinimumVelocity
                    ...
                    final int orgTouchMode = mTouchMode;
                    endFling();
                    /*
                     * If user has scroll the list to overscroll mode (mScrollY not equals 0) in
                     * FLYWHEEL_TIMEOUT duration, we need to keep it.
                     */
                    if (orgTouchMode == TOUCH_MODE_OVERSCROLL) {
                        mTouchMode = TOUCH_MODE_OVERSCROLL;                        
                    } else {
                        mTouchMode = TOUCH_MODE_SCROLL;
                    }
        ...
        public void run() {
            ...
            default:
                if (mScrollY == 0) {
                    // Only end fling when the overs croll distance is 0.
                    endFling();
                } else {
                    // Logging the abnormal case.
                    Xlog.i(TAG, "FlingRunnable run into abnormal wrong case: touch mode = "
                            + mTouchMode + ",mScrollY = " + mScrollY);
                }
                return;
            ...
            case TOUCH_MODE_FLING: {
                ...
                if (atEnd && getOverScrollMode() == View.OVER_SCROLL_NEVER) {
                    Log.i(TAG, "Fling to end and can not go on scroll because the overscroll mode is never.");
                    endFling();
                    return;
                }
                if (DBG_MOTION) {
            ...
            case TOUCH_MODE_OVERFLING: {
                ...
                if (scroller.computeScrollOffset()) {
                    ...
                    final int deltaY;
                    final boolean needChangeMode = ((mScrollY >= 0 && currY <= 0) || (mScrollY <= 0 && currY >= 0));
                    /** If user fling on the list in a opposite direction during a over fling quickly,
                    ...
                    if (DBG_MOTION) {
                        Xlog.d(TAG, "FlingRunnable TOUCH_MODE_OVERFLING: mScrollY = "
                                + mScrollY + ",deltaY = " + deltaY + ",scrollY = " + scrollY
                                + ",currY = " + currY + ",needChangeMode = " + needChangeMode);
                    }
                    ...
                    if (overScrollBy(0, deltaY, 0, scrollY, 0, 0,
                        ...
                        if (needChangeMode && mOverscrollDistance > 0) {
                          ...
                          // If there is only one item, do not fling back.
                          if (mItemCount == 1) {
                              endFling();
                              return;
                          }
                          ...
                          // start a normal fling using current velocity. 
                          start(velocity);
                        }
    ...
    private int calcRealOverScrollDist(int dist) {
        return (int)(dist * (1 - Math.abs(mScrollY) * 1.0f / mOverscrollDistance));
    }
    
    /**
     * Reset the scroll offset in y axis of listview.
     */
    void resetScrollY() {
        Xlog.d(TAG, "resetScrollY: mScrollY = " + mScrollY + ",this = " + this);
        mScrollY = 0;
        requestLayout();
    }
   
    /**
     * Set size of the fast scroller pop up overlay drawable, you have to call
    ...
    /**
     * Set the size of text drawing in the pop up.
     * 
    ...
    /**
     * Set the color of text drawing in the pop up.
     * 
