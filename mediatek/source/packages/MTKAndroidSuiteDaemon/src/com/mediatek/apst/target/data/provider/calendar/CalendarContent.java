...
public abstract class CalendarContent {
	...
	public static Calendar cursorToCalendar(Cursor c) {
		...
		try {
			...
			if (-1 != colId) {
				calendar.setId(c.getLong(colId));
			}
			// name
			colId = c.getColumnIndex(COLUMN_NAME);
			if (-1 != colId) {
				calendar.setName(c.getString(colId));
			}
			// displayName
			colId = c.getColumnIndex(COLUMN_DISPLAY_NAME);
			if (-1 != colId) {
				calendar.setDisplayName(c.getString(colId));
			}
			// ownerAccount
			colId = c.getColumnIndex(COLUMN_OWNER_ACCOUNT);
			if (-1 != colId) {
				calendar.setOwnerAccount(c.getString(colId));
			}
