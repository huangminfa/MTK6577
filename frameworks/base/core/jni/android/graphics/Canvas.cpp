...
public:
    ...
    static void drawTextWithGlyphs(SkCanvas* canvas, const jchar* textArray,
        ...
        /** M: Add x, y offset vector to store GPOS info. The number for xoffset, yoffset is the same as glyph  @{ */
        if (value->getXOffset() == NULL || value->getYOffset() == NULL) {
            doDrawGlyphs(canvas, value->getGlyphs(), 0, value->getGlyphsCount(), x, y, flags, paint);
        } else {
            doDrawGlyphs(canvas, value->getGlyphs(), value->getXOffset(), value->getYOffset(), 0, value->getGlyphsCount(), x, y, flags, paint);
        } 
        /** @} */
    }
    ...
    static void doDrawGlyphs(SkCanvas* canvas, const jchar* glyphArray, const jfloat* xOffset, const jfloat* yOffset, int index, int count,
            jfloat x, jfloat y, int flags, SkPaint* paint) {
        // TODO: need to suppress this code after the GL renderer is modified for not
        // copying the paint
        ...
        canvas->drawText(glyphArray + index * 2, count * 2, xOffset, yOffset, x, y, *paint);
