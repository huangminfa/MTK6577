...
public class ClockCityUtils {
	...
	// the city3 of style3
	// protected static final String STYLEC_WEATHER_ID = "StyleC_WeatherID";
	...
	private static HashMap<String, ClockCityInfo> mTimeZoneMap = null;// = new
																		// HashMap<String,
																		// ClockCityInfo>();
	 ...
	 * @param isShowWeather
	 */
	public static void savePreferences(AppWidgetManager appWidgetManager,
			int widgetId, final ClockCityInfo city) {
		String appWidgetId = String.valueOf(widgetId);
		SharedPreferences.Editor preEdt = pref.edit();
		// preEdt.putInt(APPWIDGET_STYLE + appWidgetId, style);
		// if (ClockCityUtils.STYLE_1 == style) {
		// if (null != cityA) {
		// preEdt.putString(STYLEA_CITYA_INDEX + appWidgetId, cityA.getIndex());
		// preEdt.putString(STYLEA_WEATHERA_ID + appWidgetId,
		// cityA.getWeatherID());
		// preEdt.putString(STYLEA_CITYA_NAME + appWidgetId,
		// cityA.getCityName());
		// preEdt.putString(STYLEA_CITYA_ZONE + appWidgetId,
		// cityA.getTimeZone());
		// }
		// if (null != cityB) {
		// preEdt.putString(STYLEA_CITYB_INDEX + appWidgetId, cityB.getIndex());
		// preEdt.putString(STYLEA_WEATHERB_ID + appWidgetId,
		// cityB.getWeatherID());
		// preEdt.putString(STYLEA_CITYB_NAME + appWidgetId,
		// cityB.getCityName());
		// preEdt.putString(STYLEA_CITYB_ZONE + appWidgetId,
		// cityB.getTimeZone());
		//
		// }
		// }
		if (null != city && !("-1".equals(city.getIndex()))) {
			...
			preEdt.putString(STYLEC_WEATHER_ID + appWidgetId, city.getWeatherID());
	...
	public static void deletePreferences(int[] widgetIds) {
		...
		try {
			...
			for (int widgetId : widgetIds) {
				...
				// preEdt.remove(APPWIDGET_STYLE + key);
				// preEdt.remove(SHOW_WEATHER + key);
				// preEdt.remove(WEATHER_ID + key);
				// remove city1 values
				// preEdt.remove(STYLEA_CITYA_INDEX + key);
				// preEdt.remove(STYLEA_CITYA_NAME + key);
				// preEdt.remove(STYLEA_CITYA_ZONE + key);
				// preEdt.remove(STYLEA_WEATHERA_ID + key);

				// remove city2 values
				// preEdt.remove(STYLEA_CITYB_INDEX + key);
				// preEdt.remove(STYLEA_CITYB_NAME + key);
				// preEdt.remove(STYLEA_CITYB_ZONE + key);
				// preEdt.remove(STYLEA_WEATHERB_ID + key);
				// remove city3 values
			...
			Xlog.w(TAG,"remove data from SharedPreferences:pref_clock_widget_info.xml fail!");
		}
	}
	...
	public static ArrayList<Integer> getIntDelete(String weatherNameDelete) {
		...
		if(pref!=null){
			...
			HashMap<String,String> ha = (HashMap<String, String>) pref.getAll();
