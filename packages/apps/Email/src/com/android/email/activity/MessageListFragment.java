...
import com.android.email.activity.MessagesAdapter.SearchCursorLoader;
...
public class MessageListFragment extends ListFragment
    ...
    public static void clearDeletedSet(Set<Long> messageIds) {
        synchronized (MessagesAdapter.mDeletedSet) {
            if (MessagesAdapter.mDeletedSet.size() > 0) {
                MessagesAdapter.mDeletedSet.removeAll(messageIds);
            }
        }
    }
    ...
    private void onLoadMoreMessages() {
        ...
        final Preferences pref = Preferences.getPreferences(mActivity);
    ...
    private void toggleRead(Set<Long> selectedSet) {
        toggleMultiple(selectedSet, new MultiToggleHelper() {
            ...
            @Override
            public void setFields(Set<Long> messageIds, boolean newValue) {
    ...
    private void toggleFavorite(Set<Long> selectedSet) {
        ...
        // It should has same behavior as toggleRead(), so remove it. 
        // selectedSet.clear();
    }
...
//        final long[] messageIds = Utility.toPrimitiveLongArray(selectedSet);
    ...
    private void autoRefreshStaleMailbox() {
        ...
        /**
         * M: Make sure inbox will not be auto-refreshed when setting the 
         * frequency of checking mail is never
         */
        if(!mIsFirstLoad && (mAccount.mSyncInterval == Account.CHECK_INTERVAL_NEVER)) {
            return;
        }
        // If it's not the first load, we need to judge the switch "sync Email" in account settings,
        // this switch is consistent with the option "sync Email" in phone "account & sync" of this account.
    ...
    private int determineFooterMode() {
        ...
        // Only DRAFTS of Orange IMAP server can be synced
        if (mMailbox.mType == Mailbox.TYPE_DRAFTS) {
            HostAuth authRecv = HostAuth.restoreHostAuthWithId(mActivity, mAccount.mHostAuthKeyRecv);
    ...
    private final LoaderManager.LoaderCallbacks<Cursor> LOADER_CALLBACKS =
        ...
        public Loader<Cursor> onCreateLoader(int id, Bundle args) {
            ...
            // M: Initialize the menu item "R.id.send".
        ...
        public void onLoadFinished(Loader<Cursor> loader, Cursor c) {
            ...
            if((getListAdapter() != null) && cursor.getCount() > 0) {
                if (Logging.LOG_PERFORMANCE) {
                    Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] MessageList:ShowMessages ["
                            + System.currentTimeMillis() + "]");
                }
            }
            ...
            if (mIsFirstLoad) {
                ...
                if (!isEmptyAndLoading(cursor)) {
                    ...
                    mWaitLoading = true;
                    if (Logging.LOG_PERFORMANCE) {
                        Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] MessageList:ShowLoading ["
                                + System.currentTimeMillis() + "]");
                    }
                }
                ...
                setListAdapter(mListAdapter);
                if (Logging.LOG_PERFORMANCE) {
                    Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] MessageList:FisrtShowMessages ["
                            + System.currentTimeMillis() + "]");
                }
            }
    ...
    private class RefreshListener implements RefreshManager.Listener {
        ...
        public void onRefreshStatusChanged(long accountId, long mailboxId) {
            updateListFooter();
            if (mailboxId == -1) {
                Logging.d("MessagelistFragment:onRefreshStatusChanged(), mailboxId = -1, so we just return");
                return;
            }
            ...
            if (mWaitLoading && !mRefreshManager.isMessageListRefreshing(mailboxId) 
                ...
                if (mMailbox != null && mMailbox.mId == mailboxId) {
                    ...
                    new CheckMessageCountTask(accountId, mailboxId).cancelPreviousAndExecuteParallel();
                }
            }
        }
    }
    private class CheckMessageCountTask extends EmailAsyncTask<Void, Void, Long> {
        ...
        protected void onSuccess(Long messageCount) {
            // If messageCount == 0, we display no message, or we waiting for onLoadFinished setListAdapter;
            if (messageCount == 0) {
                setListAdapter(mListAdapter);
            }
        }
