...
//import android.bluetooth.BluetoothTethering;
...
public class BluetoothPanService extends Service {
    ...
    private native static void serverAuthorizeRspNative(String BT_Addr, boolean result);
    private native void serverActivateReqNative();
    private native void serverDeactivateReqNative();
    private native void serverConnectReqNative(int service,String BT_Addr);
    private native static void serverDisconnectReqNative(String BT_Addr);
    ...
    private static int pan_authorize_notify	= PAN_ID_START + 1;
       
    private Intent update_GN_state_intent = new Intent(BluetoothProfileManager.ACTION_PROFILE_STATE_UPDATE);
    private Intent update_NAP_state_intent = new Intent(BluetoothProfileManager.ACTION_PROFILE_STATE_UPDATE);
    ...
    private void updateSettingsState(int prevState, int state, BluetoothDevice device, int role) {
        ...
        try {
            mBluetoothService.sendConnectionStateChange(device, BluetoothPan.PAN, state,
                                                prevState);
...
/*
		update_GN_state_intent.putExtra(BluetoothProfileManager.EXTRA_PROFILE, BluetoothProfileManager.Profile.Bluetooth_PAN_GN);
		update_GN_state_intent.putExtra(BluetoothProfileManager.EXTRA_NEW_STATE, state);
		sendBroadcast(update_GN_state_intent);
		update_NAP_state_intent.putExtra(BluetoothProfileManager.EXTRA_PROFILE, BluetoothProfileManager.Profile.Bluetooth_PAN_NAP);
		update_NAP_state_intent.putExtra(BluetoothProfileManager.EXTRA_NEW_STATE, state);
...
*/
    ...
    private BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(action.equals(BluetoothDevice.ACTION_NAME_CHANGED)) {
                ...
                if(device == null){
                    if(DEBUG)Log.e(TAG,"ERROR: device null");
                    return;
                }
                ...
                if(notify_s == null){
                    if(DEBUG)Log.e(TAG,"ERROR: notify_s null");
                    return;
                }
                ...
                noti=genPanNotification(notify_id,device,false);
    ...
    public void authorizeRsp(String BT_Addr,boolean result ){
        if(BT_Addr==null)
            return;
        serverAuthorizeRspNative(BT_Addr, result);
    }
	
    public void connectPanDevice(String BT_Addr, int service){
        BluetoothDevice device = mAdapter.getRemoteDevice(BT_Addr);
        if (mPanDevices.containsKey(device)) {
            return;
        }
        
        BluetoothPanDevice panDevice = new BluetoothPanDevice(BluetoothPan.STATE_CONNECTING, service);
        mPanDevices.put(device, panDevice);

        updateSettingsState(BluetoothPan.STATE_DISCONNECTED, BluetoothPan.STATE_CONNECTING, device, service);
        serverConnectReqNative(service,BT_Addr);
    }
    
    public void disconnectPanDevice(String BT_Addr){
        BluetoothDevice device = mAdapter.getRemoteDevice(BT_Addr);
        BluetoothPanDevice panDevice = mPanDevices.get(device);

        if (panDevice == null) {
            if(DEBUG)Log.e(TAG, "unknown device");
            return;
        }
    ...
    private void disconnectPanServerDevices() {
        if(DEBUG)Log.d(TAG, "disconnect all remote panu devices");
	...
	private final IBluetoothPanAction.Stub mPanAction = new IBluetoothPanAction.Stub() {
		public void disconnectPanDeviceAction(String BT_Addr) {
			if(DEBUG)Log.d(TAG, "disconnectPanDeviceAction: ");
			disconnectPanDevice(BT_Addr);
		}
	
		public void authorizeRspAction(String BT_Addr,boolean result){
			if(DEBUG)Log.d(TAG, "authorizeRspAction");
			authorizeRsp(BT_Addr,result);
			}
	};
    ...
    private final IBluetoothPan.Stub mPan = new IBluetoothPan.Stub() {		
        ...
        public void connect(BluetoothDevice device) {
            ...
            if(DEBUG)Log.d(TAG, "BluetoothPanServer Connect");

            // TODO Here only indicate local role, the remote role shall be indicated as well
            connectPanDevice(BT_Addr, BluetoothPan.LOCAL_PANU_ROLE);
        }

        public void disconnect(BluetoothDevice device) {
            // TODO Auto-generated method stub
            if(DEBUG)Log.d(TAG, "BluetoothPanServer Disconnect");
            String BT_Addr=device.getAddress();
    ...
    public IBinder onBind(Intent intent) {
        // TODO Auto-generated method stub
        String action = intent.getAction();
        if(DEBUG)Log.i(TAG, "Enter onBind(): " + action);
    ...
    if(DEBUG)Log.d(TAG, "Enter onCreate()");
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if(DEBUG)Log.d(TAG,"Enter onStartCommand()");
        ...
        if (!mHasInitiated) {
            ...
            if (mNM == null) {
                if(DEBUG)Log.e(TAG, "Get Notification-Manager failed. Stop PAN service.");
                stopSelf();
            }
            ...
            if(DEBUG)Log.d(TAG,"Already started, just return!");
            return START_STICKY;
        }
        return START_STICKY;
    }
	...
	void localClearService(){
		...
		if (mServerState) {
			...
			try {
				...
				if(DEBUG)Log.e(TAG, "Waiting for server deregister-cnf was interrupted.");
			}
			cnt += 100;
		    }
		}
		...
		if (timeout) {
		     ...
		     */
			if(DEBUG)Log.w(TAG, "Waiting DEREGISTER_SERVER_CNF time-out. Force clear server context.");
		    mServerState = false;
		...
		if (mSocketListener != null) {
		    try {
		    	...
		    	if(DEBUG)Log.e(TAG, "mSocketListener close error.");
		    }
		}		
		mPanDevices.clear();
	...
	void localCreateService(){
		if(!mServerState){
			...
			if (initServiceNative()) {
				...
				if(DEBUG)Log.d(TAG,"Pre-enable PAN Server");
			}else {
				if(DEBUG)Log.d(TAG,"Failed to init BluetoothPanService.");
			}
		}
	}
	...
	private void sendServiceMsg(int what, String message) {
		...
		if(DEBUG)Log.i(TAG,"sendServiceMsg status="+what+"message="+message);
		msg.what = what;
    ...
    /* [IN] service: It shall be referred to bluetooth_pan_struct.h "bt_pan_service_enum" 
    *                           service > BluetoothPan: LOCAL_NAP_ROLE, LOCAL_GN_ROLE, LOCAL_PANU_ROLE 
    */
    private void callback_pan_handle_connection_authorize_ind(int service, String deviceAddr){
        if ((service == BluetoothPan.LOCAL_NAP_ROLE) && !mTetheringOn) {
            String toastMsg = getString(R.string.bluetooth_pan_NAP_unavailable,mAdapter.getRemoteDevice(deviceAddr).getName());
        ...
        int device_state=BluetoothPan.STATE_CONNECTING;
        BluetoothDevice device = mAdapter.getRemoteDevice(deviceAddr);
        BluetoothPanDevice panDevice = mPanDevices.get(device);

        if (panDevice == null) {
            panDevice = new BluetoothPanDevice(device_state, service);
            mPanDevices.put(device, panDevice);
        }
        else {
        ...
        updateSettingsState(BluetoothPan.STATE_DISCONNECTED, device_state, device, service);

        if(!notifyMap.containsKey(deviceAddr)){
            notifyMap.put(deviceAddr, pan_authorize_notify);
            pan_authorize_notify++;
        }
 			   
        notify_id=Integer.parseInt(notifyMap.get(deviceAddr).toString());
        noti=genPanNotification(notify_id, device,true);
        mNM.notify(notify_id, noti);
    }
	
    private void callback_pan_handle_connect_ind(int service, String deviceAddr, int unit){
        if ((service != BluetoothPan.LOCAL_PANU_ROLE) && !mTetheringOn) {
            disconnectPanDevice(deviceAddr);
            return;
        }

        int prevState = BluetoothPan.STATE_DISCONNECTED;
        int device_state=BluetoothPan.STATE_CONNECTED;
        BluetoothDevice device = mAdapter.getRemoteDevice(deviceAddr);
        BluetoothPanDevice panDevice = mPanDevices.get(device);
       ...
       try {
        ...
        updateSettingsState(prevState, device_state, device, service);
    ...
    private void callback_pan_handle_connect_cnf(boolean result, String deviceAddr, int unit){	
        ...
        if (panDevice == null) {
            if(DEBUG)Log.e(TAG,"unknow device when callback_pan_handle_connect_cnf:"+deviceAddr);
            return;
        }
    ...
    private void callback_pan_handle_disconnect_ind(String deviceAddr){
        ...
        if (panDevice == null) {
            if(DEBUG)Log.e(TAG,"unknow device when callback_pan_handle_disconnect_ind:"+deviceAddr);
            return;
        }
        ...
        updateSettingsState(panDevice.mState, BluetoothPan.STATE_DISCONNECTED, device, panDevice.mLocalRole);		
    }
    ...
    private void callback_pan_handle_disconnect_cnf(boolean result, String deviceAddr){	
        ...
        if (panDevice == null) {
            if(DEBUG)Log.e(TAG,"unknow device when callback_pan_handle_disconnect_cnf:"+deviceAddr);
            return;
        }
    ...
    private Notification genPanNotification(int type, BluetoothDevice device, boolean needSound) {
        ...
        int icon_id = -1;
        String clazz = null, ticker = null, title = null, action = null;
        if(DEBUG)Log.i(TAG, "genPanNotification "+device.getAddress()+";device_state "+panDevice.mState);
        ...
        tmpIntent.setClassName(getPackageName(), clazz)
                                .putExtra(BluetoothPan.DEVICE_ADDR, device.getAddress());
