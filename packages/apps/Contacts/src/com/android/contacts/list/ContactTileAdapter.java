    ...
    private ContactEntry createContactEntryFromCursor(Cursor cursor, int position) {
        ...
        if (slotId < 0) {
			...
			if (FeatureOption.MTK_GEMINI_SUPPORT) {
				...
				Log.i(TAG, "[createContactEntryFromCursor] i = " + i);
				/*
				...
				// }
				// default:
				// contact.photoUri = (photoUri != null ? Uri.parse(photoUri) :
				// null);
				// break;
				// }
				// } else if (i == 0) {
				...
				if (isSdnContact > 0) {
					if (i == 0) {
						contact.photoUri = Uri.parse("content://sdn-5");
					} else if (i == 1) {
						contact.photoUri = Uri.parse("content://sdn-6");
					} else if (i == 2) {
						contact.photoUri = Uri.parse("content://sdn-7");
					} else if (i == 3) {
						contact.photoUri = Uri.parse("content://sdn-8");
					} else {
						contact.photoUri = Uri.parse("content://sdn");
					}
				} else {
					if (i == 0) {
						contact.photoUri = Uri.parse("content://sim-10");
					} else if (i == 1) {
						contact.photoUri = Uri.parse("content://sim-11");
					} else if (i == 2) {
						contact.photoUri = Uri.parse("content://sim-12");
					} else if (i == 3) {
						contact.photoUri = Uri.parse("content://sim-13");
					} else {
        ...
        if (mDisplayType == DisplayType.STREQUENT_PHONE_ONLY) {
            ...
            if(OperatorUtils.isAasEnabled(accountType)) {
                contact.phoneLabel = USIMAas.getUSIMAASById(slotId, phoneNumberType);
                Log.i(TAG,"AAS: CallTileAdapter,getUSIMAASById, label = " + contact.phoneLabel);//TODO:remove it.
            }
            /*
             * New Feature by Mediatek End.
             */
