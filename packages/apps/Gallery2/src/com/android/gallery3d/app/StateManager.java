...
public class StateManager {
    ...
    public static final String KEY_IS_POPPED_FROM_STACK = "is-popped-from-stack";
    ...
    public void startStateForResult(Class<? extends ActivityState> klass,
        ...
        Log.v(TAG, "startStateForResult " + klass + ", requestCode = "
                + requestCode + ", mIsResumed = " + mIsResumed);
        ActivityState state = null;
        try {
            state = klass.newInstance();
        } catch (Exception e) {
    ...
    void finishState(ActivityState state) {
        ...
        mStack.pop();
        if (mIsResumed) {
            // KEY_IS_POPPED_FROM_STACK of value true means that
            // the ActivityState has been popped out of the state stack
            // An use case of KEY_IS_POPPED_FROM_STACK can be found in
            // AlbumSetPage.isPoppedFromStack()
            Bundle bundle = state.getData();
            if (bundle != null) {
                bundle.putBoolean(KEY_IS_POPPED_FROM_STACK, true);
            }
            state.onPause();
        }
