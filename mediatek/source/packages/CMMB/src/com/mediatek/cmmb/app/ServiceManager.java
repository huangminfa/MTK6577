...
public class ServiceManager {
    ...
    private static ServiceManager mManager;//singleton
    ...
    /**
     * Get the instance of ServiceManager.
     * it just init an instance which manages the real service.
     * @param context
     * @return
     */
    ...
    /**
     * Should be called if app do not use service.
     * You may not have to call this function if your activity do not need service.
     */
    public void closeService() {
        if (LOG) Log.v(TAG, "close() mMBBMSService=" + mMBBMSService
                + ", mServiceConnection=" + mServiceConnection + getThreadInfo());
    ...
    public MBBMSService getService() {
        if (mMBBMSService == null) {
            ...
            if (LOG) Log.v(TAG, "getService() " + getThreadInfo() + " enter lock. mMBBMSService=" + mMBBMSService);
            if (mMBBMSService == null) {//not bind the service
                ...
                /*
                 * if you config User agent here, 
                 * the <userAgent> config in packages\CMMB\res\raw\configurations.xml would't work anymore! 
                 */
                ...
                try {
                    if (LOG) Log.v(TAG, "getService() " + getThreadInfo() + " await " + lock);
                    lock.await();
                    if (LOG) Log.v(TAG, "getService() " + getThreadInfo() + " waked " + lock);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }
            mBindingDog.unlock();//release it
            if (LOG) Log.v(TAG, "getService() " + getThreadInfo() + " outer lock. mMBBMSService=" + mMBBMSService);
        }
        if (LOG) Log.v(TAG, "getService() return " + mMBBMSService);
     ...
     * @return
     */
    public ServerStatus doAreaSetRequest() {
        MBBMSService service = getService();
        if (service != null) {
            ServerStatus status = service.doAreaSetRequest();
        	...
        	if (MBBMSService.STATUS_FAIL_UAM_AUTH.equalsIgnoreCase(status.code)) {        		
        		try {
					...
					if (MBBMSService.STATUS_SUCCEED.equalsIgnoreCase(status.code)) {
			        	...
			        	if (LOG) Log.v(TAG, "subscribe: 3(" + status.code + ")");
					}
				} catch (InterruptedException e) {
				    service.stopDataConnect();
					e.printStackTrace();
    ...
    public ServerStatus unsubscribe(String purchaseItemGlobalId, String purchaseDataGlobalId) {
    	if (LOG) Log.v(TAG, "unsubscribe: (" + purchaseItemGlobalId + "," + purchaseDataGlobalId + ")");
        MBBMSService service = getService();
        if (service != null) {
        	ServerStatus status = service.unsubscribe(purchaseItemGlobalId, purchaseDataGlobalId);
        	if (LOG) Log.v(TAG, "unsubscribe: 1(" + status.code + ")");
        	if (MBBMSService.STATUS_FAIL_UAM_AUTH.equalsIgnoreCase(status.code)) {        		
        		try {
					status = service.processGBAInitialization();
					if (LOG) Log.v(TAG, "unsubscribe: 2(" + status.code + ")");
					if (MBBMSService.STATUS_SUCCEED.equalsIgnoreCase(status.code)) {
						status = service.unsubscribe(purchaseItemGlobalId, purchaseDataGlobalId); 
						if (LOG) Log.v(TAG, "unsubscribe: 3(" + status.code + ")");
					}
				} catch (InterruptedException e) {
				    service.stopDataConnect();
					e.printStackTrace();
    ...
    public ServerStatus unsubscribeAll() {
    	if (LOG) Log.v(TAG, "unsubscribeAll");
        MBBMSService service = getService();
        if (service != null) {
        	ServerStatus status = service.unsubscribeAll();
        	if (LOG) Log.v(TAG, "unsubscribeAll: 1(" + status.code + ")");
        	if (MBBMSService.STATUS_FAIL_UAM_AUTH.equalsIgnoreCase(status.code)) {
        		try {
					status = service.processGBAInitialization();
					if (LOG) Log.v(TAG, "unsubscribeAll: 2(" + status.code + ")");
					if (MBBMSService.STATUS_SUCCEED.equalsIgnoreCase(status.code)) {
						status = service.unsubscribeAll(); 
						if (LOG) Log.v(TAG, "unsubscribeAll: 3(" + status.code + ")");
					}
				} catch (InterruptedException e) {
					e.printStackTrace();
                    service.stopDataConnect();
     ...
     * @return
     */
    public ServerStatus doUpgradeRequest() {
        MBBMSService service = getService();
        if (service != null) {
            ServerStatus status = service.doUpgradeRequest();
            service.stopDataConnect();
            return status;
        }
        return null;
     ...
     * @return
     */
    public ServerStatus doIMDRequest(String serviceId, String contentId, String addr, String imdPointer) {
        MBBMSService service = getService();
        if (service != null) {
            ServerStatus status = service.doIMDRequest(serviceId, contentId, addr, imdPointer);
            service.stopDataConnect();
            return status; 
        }
        return null;
    ...
    /*package*/ class MyServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            if (LOG) Log.v(TAG, "onServiceConnected() " + getThreadInfo() + " count down " + mLock);
            mMBBMSService = ((MBBMSService.LocalBinder) service).getService();
            mLock.countDown();//notify the wait thread
        }
    
        public void onServiceDisconnected(ComponentName name) {
            if (LOG) Log.v(TAG, "onServiceDisconnected() " + getThreadInfo());
            mMBBMSService = null;
        }
    }
