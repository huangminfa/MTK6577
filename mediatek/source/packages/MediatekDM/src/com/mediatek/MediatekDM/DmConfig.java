...
public class DmConfig {
        ...
        try {
            if (Options.UseDirectInternet) {
                ...
                if (Proxy.getDefaultHost() != null) {
                    ...
                    mConfig.setDmProxy("http://" + proxyAddr + ":"
                                       + proxyPort);
                    mConfig.setDlProxy("http://" + proxyAddr + ":"
                                       + proxyPort);
                } else {
            ...
            if (mParamTable != null) {
                ...
                if (mParamTable.containsKey(SERVER_202_UNSUPPORTED)) {
                    String server202NotSupported = mParamTable
                                                   .getProperty(SERVER_202_UNSUPPORTED);
                ...
                if (mParamTable.containsKey(INSTALL_NOTIFY_SUCCESS_ONLY)) {
                    String installNotify = mParamTable
                                           .getProperty(INSTALL_NOTIFY_SUCCESS_ONLY);
                    if (installNotify != null) {
                        if (installNotify.charAt(0) == 'T'
                                || installNotify.charAt(0) == 't') {
    ...
    private void configureDMAcc() throws MdmException {
        ...
        if (mParamTable.containsKey(DM_VERSION)) {
            ...
            if (dmVersion != null) {
                if (dmVersion.equals("112") || dmVersion.equals("1.1.2")) {
                    dmacc.activeAccountDMVersion = MdmConfig.DmVersion.DM_1_1_2;
                }
            }
        } else {
...
//					dmacc.isExclusive = true;
        ...
        if (mParamTable.containsKey(UPDATE_INACTIVE_ACC)) {
            String updateInactiveAcc = mParamTable
                                       .getProperty(UPDATE_INACTIVE_ACC);
            if (updateInactiveAcc != null) {
                if (updateInactiveAcc.charAt(0) == 'T'
                        || updateInactiveAcc.charAt(0) == 't') {
    ...
    private void configureHttpAuth(boolean isDM) throws MdmException {
        ...
        if (mParamTable.containsKey(auth[HTTP_AUTH_LEVEL])) {
            ...
            if (level.equalsIgnoreCase("none")) {
                if (isDM)
                    mConfig.setDmHttpAuthentication(HttpAuthLevel.NONE, null,
                                                    null);
                else
                    mConfig.setDlHttpAuthentication(HttpAuthLevel.NONE, null,
                                                    null);
            } else {
                String username = mParamTable
                                  .getProperty(auth[HTTP_AUTH_UNAME]);
                String password = mParamTable.getProperty(auth[HTTP_AUTH_PWD]);

                if (username == null || password == null) {
                    Log.w(TAG.Common, "Missing credentials for " + DMDL
                          + " HTTP Authentication");
                    throw new MdmException(MdmException.MdmError.BAD_INPUT);
                }
                if (level.equalsIgnoreCase("digest")) {
                    Log.w(TAG.Common, DMDL
                          + " HTTP Authentication 'Digest' is not supported");
                    throw new MdmException(
                        MdmException.MdmError.NOT_IMPLEMENTED);
                }
                if (level.equalsIgnoreCase("basic") == false) {
                    Log.w(TAG.Common, "Invalid " + DMDL + " HTTP Authentication "
                          + level);
                    throw new MdmException(MdmException.MdmError.BAD_INPUT);
                }

                if (isDM)
                    mConfig.setDmHttpAuthentication(HttpAuthLevel.BASIC,
                                                    username, password);
                else
                    mConfig.setDlHttpAuthentication(HttpAuthLevel.BASIC,
                                                    username, password);
            }
        }
    }
    ...
    private void configureProxyAuth(boolean isDM) throws MdmException {
        ...
        if (mParamTable.containsKey(auth[HTTP_AUTH_LEVEL])) {
            ...
            if (level.equalsIgnoreCase("none")) {
                ...
                if (isDM)
                    mConfig.setDmProxyAuthentication(HttpAuthLevel.NONE, null,
                                                     null);
                else
                    mConfig.setDlProxyAuthentication(HttpAuthLevel.NONE, null,
                                                     null);
            } else {
                ...
                if (level.equalsIgnoreCase("basic") == false) {
                    Log.w(TAG.Common, "Invalid " + DMDL + " Proxy Authentication "
                          + level);
                    throw new MdmException(MdmException.MdmError.BAD_INPUT);
                }

                if (isDM)
                    mConfig.setDmProxyAuthentication(HttpAuthLevel.BASIC,
                                                     username, password);
                else
                    mConfig.setDlProxyAuthentication(HttpAuthLevel.BASIC,
                                                     username, password);
            }
        } else {
    ...
    private static final String DM_PROXY_AUTH[] = { "dm_proxy_auth_level", "dm_proxy_auth_username", "dm_proxy_auth_password" };
    private static final String DL_PROXY_AUTH[] = { "dl_proxy_auth_level", "dl_proxy_auth_username", "dl_proxy_auth_password" };
}
