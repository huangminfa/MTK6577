...
public class DialerSearchAdapter extends CursorAdapter implements OnClickListener {
    ...
    protected Drawable[] mCallTypeDrawables = new Drawable[5];
    protected Drawable[] mVideoCallTypeDrawables = new Drawable[5];
    ...
    public boolean needClearDigits = false;
    ...
    public DialerSearchAdapter(Context context, Listener listener) {
        ...
        // 1. incoming 2. outgoing 3. missed 4. auto rejected(Calls.AUTOREJECTED_TYPE)
        ...
        mCallTypeDrawables[4] = mContext.getResources().getDrawable(R.drawable.ic_call_autorejected_holo_dark);
        ...
        mVideoCallTypeDrawables[4] = mContext.getResources().getDrawable(R.drawable.ic_video_call_autorejected_holo_dark);
    ...
    public DialerSearchAdapter(Context context, Listener listener, CallOptionHandler callOptionHandler) {
    ...
    public void onResume() {
        ...
        if(FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            mVoiceMailNumber = telephonyManager.getVoiceMailNumberGemini(Phone.GEMINI_SIM_1);
            mVoiceMailNumber2 = telephonyManager.getVoiceMailNumberGemini(Phone.GEMINI_SIM_2);
        } else {
    ...
    public void bindContactView(View view, Context context, Cursor cursor) {
        ...
        CharSequence label = CommonDataKinds.Phone.getTypeLabel(context.getResources(),
                labelType, null);
        /*
        ...
        String accountType = atm.getAccountTypeBySlot(slot);
        Log.v(TAG,"bindContactView, slot=" + slot + ";accountType = " + accountType);
        if(OperatorUtils.isAasEnabled(accountType)) {
            label = USIMAas.getUSIMAASById(slot, labelType);
            Log.i(TAG,"name = " + name + " number = " + number + " label = " + label);//TODO:remove it.
        }
        /*
    ...
    public void bindCallLogView(View view, Context context, Cursor cursor) {
        ...
        if (Calls.AUTOREJECTED_TYPE == type) {
            viewHolder.type.setImageDrawable(callTypeDrawables[type-1]);
        } else {
            viewHolder.type.setImageDrawable(callTypeDrawables[type]);
        }
        ...
        switch(v.getId()) {
            ...
            case R.id.dialer_search_item_view: {
                ...
                if("OP01".equals(OperatorUtils.getOptrProperties())){
                	needClearDigits = true;
                	Intent intent = newItemClickIntent(v);
                	if(intent != null)
                    	mContext.startActivity(intent);
                }else{       
                    ViewHolder viewHolder = (ViewHolder) v.getTag();
                    final CallInfo callInfo = (CallInfo) viewHolder.call.getTag();
                    if (null != callInfo) {
                        if (null != mListener) {
                            log("callinfo number = " + callInfo.number);
                            mListener.onListViewItemClicked(callInfo.number);
                        }
                    }
                }
            }
