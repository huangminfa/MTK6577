    ...
    void                        *pMem)
{
    ...
    if ((pVars->aacPlusEnabled == true) && (pExt->aacPlusEnabled == true))
    {
       ...
       // 20121016 pc : backup system config for normal frame restore 
#ifndef ANDROID_DEFAULT_CODE
       pVars->configbak.valid = 1;
       pVars->configbak.upsamplingFactor        = pVars->mc_info.upsamplingFactor;
       pVars->configbak.psPresentFlag           = pVars->mc_info.psPresentFlag;
       pVars->configbak.sbrPresentFlag          = pVars->mc_info.sbrPresentFlag;
       pVars->configbak.sampling_rate_idx       = pVars->prog_config.sampling_rate_idx;
       pVars->configbak.SbrChannel0syncState    = pVars->sbrDecoderData.SbrChannel[0].syncState;
       pVars->configbak.SbrChannel1syncState    = pVars->sbrDecoderData.SbrChannel[1].syncState;
       pVars->configbak.aacPlusUpsamplingFactor = pExt->aacPlusUpsamplingFactor;
#endif
...
#ifndef ANDROID_DEFAULT_CODE
OSCL_EXPORT_REF void PVMP4AudioDecoderRestoreAacPlus(
    tPVMP4AudioDecoderExternal  *pExt,
    void                        *pMem)
{
    tDec_Int_File *pVars;
    ...
    if (pVars->configbak.valid)
    {
        pVars->configbak.valid = 0;

        pVars->aacPlusEnabled = true;
        pExt->aacPlusEnabled = true;

        pVars->mc_info.upsamplingFactor = pVars->configbak.upsamplingFactor;
        pVars->mc_info.psPresentFlag    = pVars->configbak.psPresentFlag;
        pVars->mc_info.sbrPresentFlag   = pVars->configbak.sbrPresentFlag;

        pVars->prog_config.sampling_rate_idx = pVars->configbak.sampling_rate_idx;
        
        pVars->sbrDecoderData.SbrChannel[0].syncState = pVars->configbak.SbrChannel0syncState;
        pVars->sbrDecoderData.SbrChannel[1].syncState = pVars->configbak.SbrChannel1syncState;

        pExt->samplingRate = samp_rate_info[pVars->prog_config.sampling_rate_idx].samp_rate;
        pExt->aacPlusUpsamplingFactor = pVars->configbak.aacPlusUpsamplingFactor;
