...
public class MtvPlayer extends Activity implements View.OnClickListener,
   ...
   private static final int AUDIO_MASK_NICAM = (NICAM_MONO | NICAM_STEREO | NICAM_DUAL1 | NICAM_DUAL2); 
            ...
            switch (msg.what) {
                ...
                case MtvEngine.MTV_MSG_INITIALIZE_DONE:
                    if (msg.arg1 == 0) {                        
                        ...
                        mEngine.configVideo(VIDEO_BRIGHTNESS,mPreferences.getInt(KEY_BRIGHTNESS, (MAXIMUM-MINIMUM)/2));            
                        mEngine.configVideo(VIDEO_CONTRAST,mPreferences.getInt(KEY_CONTRAST, (MAXIMUM-MINIMUM)/2));            
                        mEngine.configVideo(VIDEO_SATURATION,mPreferences.getInt(KEY_SATURATION, (MAXIMUM-MINIMUM)/2));    
                    }
                    break;
    ...
    private void setControlBarEnable(boolean enable) {
        ...
        Xlog.d(TAG, "setControlBarEnable() enable = "+enable);
        /*
        if (enable) {
    ...
    Xlog.d(TAG, "storeImage()");
        ...
        if (mLastContentUri != null/*this means we got an error*/) {
            ...
            //Xlog.d(TAG, "ImageManager.setImageSize needs " + (tt2 - tt1));
        }
        } catch (Exception ex) {
        Xlog.e(TAG, "Exception while compressing image.", ex);
        }
    }    
    
    private void doSnap() {

        Xlog.d(TAG, "doSnap(), state:" + mState);
    ...
    public void onCreate(Bundle icicle) {
        ...
        mPreviousChannel = mCurrentChannel = mEngine.getCurrentChannel();
    ...
    private void setDisplaySizeAndMenuBotton() {
        ...
        Xlog.d(TAG, "screen size: " + LcdWidth + "x" + LcdHeight);
        ...
        View menu_button = null;
        if (!ViewConfiguration.get(this).hasPermanentMenuKey()) {            
            menu_button = findViewById(R.id.options_menu);
        ...
        if (LcdWidth == 320 && LcdHeight == 240) {//QVGA
            ...
            if (menu_button != null) {
                RelativeLayout.LayoutParams obl = (RelativeLayout.LayoutParams)menu_button.getLayoutParams();
            ...
            Xlog.d(TAG, "control panel width: " + clp.width);
            int menu_width = 0;
            if (menu_button != null) {
                RelativeLayout.LayoutParams obl = (RelativeLayout.LayoutParams)menu_button.getLayoutParams();
                menu_width = obl.width;                
            }            
            lp.width = lp.width - clp.width - menu_width;
        ...
        Xlog.d(TAG, "display size: " + lp.width + "x" + lp.height);
    ...
    private void initSingleListPanel() {
        ...
        if ((mInitializationMark & INITIALIZE_SINGLE_LIST_PANEL) != 0) return;
    ...
    private void initRenameEditor() {
        if ((mInitializationMark & INITIALIZE_RENAME_EDITOR) != 0) return;
    ...
    private void initRecordingUI() {
        ...
        if ((mInitializationMark & INITIALIZE_RECORD_UI) != 0) return;
    ...
    private void initVideoPanel() {
        ...
        if ((mInitializationMark & INITIALIZE_VIDEO_PANEL) != 0) return;
        //video panel
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        Xlog.d(TAG, "onWindowFocusChanged() hasFocus = "+hasFocus+",mScreenOn = "+mScreenOn);
        ...
        if (!mScreenOn && hasFocus) {
            mScreenOn = true;
            Xlog.d(TAG, "onWindowFocusChanged() mState = "+mState+",mOpenFailed = "+mOpenFailed);
        ...
        mScreenStatusReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (action.equals(Intent.ACTION_SCREEN_OFF)) {
                    Xlog.d(TAG, "ACTION_SCREEN_OFF");                
                    mScreenOn = false;
                    if (mState == STATE_PAUSED) {
                        mEngine.turnOff(true);
                    }
                } else if (action.equals(Intent.ACTION_USER_PRESENT)) {//we only turn on tv when screen is unlocked other than ACTION_SCREEN_ON is fired
                    Xlog.d(TAG, "ACTION_USER_PRESENT");            
                    if (!mScreenOn) {
                    mScreenOn = true;
...
private class OpenChannelThread extends Thread {
    ...
    private void switchChannel() {
        Xlog.d(TAG, "switchChannel() mState = "+mState+" mChannelSwitching = "+mChannelSwitching);
    ...
    public void onClick(View v) {
        switch(mState) {
            ...
            case STATE_RECORDING:
                if (v.getId()!=R.id.btn_mute && v.getId()!=R.id.btn_stop_record) {//only allow mute and stop record when in recording state.
    ...
    private void openTrue() {            
        Xlog.d(TAG, "openTrue() mState = "+mState);
        switch(mState) {
    ...
    private void openFalse() {                    
        Xlog.d(TAG, "openFalse() mState = "+mState);
        if (mState == STATE_CREATED) {
            mOpenFailed = true;
            mProgressView.setVisibility(View.GONE); 
            showDialog(DIALOG_INITIALIZE_HW_FAIL);        
    ...
    public void onBackPressed() {
        Xlog.d(TAG, "onBackPressed() mBackPressed = "+mBackPressed);    
    ...
    public boolean onKeyLongPress(int keyCode, KeyEvent event) {
        Xlog.d(TAG, "onKeyLongPress() mState = "+mState);
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
        ...
        if (holder.getSurface() == null) {
            Xlog.d(TAG, "holder.getSurface() == null");
            return;
        }

        Xlog.d(TAG, "surfaceChanged() mState = "+mState);
                ...
                findViewById(R.id.rename_button_ok).setOnClickListener(new View.OnClickListener() {
                    public void onClick(View v) {
                                ...
                                if (!"".equals(name)) {
                                    ...
                                    item.mName = name;                
                                    mChannelProvider.updateChannelName(Integer.parseInt(item.mCh),name);                        
                                }                                 
                                slideOut(mRenameEditor);                                
                            }
                });        
    ...
    private void stopVideoRecording(boolean toSave) {
        Xlog.d(TAG, "stopVideoRecording");
        if (mState == STATE_RECORDING) {
            try {
                mEngine.stopRecord();
            } catch (RuntimeException e) {
                Xlog.e(TAG, "stop fail: " + e.getMessage());
            }

            Xlog.v(TAG, "Setting current video filename: "
    ...
    private void updateRecordingTime() {
        ...
        mMainHandler.sendEmptyMessageDelayed(
                MSG_UPDATE_RECORD_TIME, next_update_delay);
    }
    ...
    private class CallStateListener extends PhoneStateListener{
        ...
        public void onCallStateChanged(int state, String incomingNumber) {
            if (state == TelephonyManager.CALL_STATE_RINGING || state == TelephonyManager.CALL_STATE_OFFHOOK) {
                if (mState == STATE_RECORDING) {
                    Xlog.v(TAG, "Recording stopped due to incoming call");
                    stopVideoRecording(true);
                }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_MEDIA_EJECT) || action.equals(Intent.ACTION_MEDIA_UNMOUNTED)) {                
                if (mState == STATE_RECORDING) {
                    ...
                    String path_in_use = getVideoDirectory();  
                    String removing_path = intent.getData().getPath();
                    Xlog.d(TAG, "path_in_use = "+path_in_use + " removing_path = "+removing_path);

                    //if removing sdcard is not the one we are using,do nothing.
                    if (!path_in_use.equals(removing_path)) {
                        return;
                    }
    ...
    private void createVideoPath() {
        ...
        Xlog.v(TAG, "Current video filename: " + mCreatedVideoFilename);
    ...
    private void registerVideo() {
            ...
            videoTable = mContentResolver.insert(videoTable,
                    mCurrentVideoValues);
            Xlog.v(TAG, "Current video URI: " + videoTable);
    ...
    private void startVideoRecording() {
        Xlog.d(TAG, "startVideoRecording mState = "+mState);
        ...
        try {
            ...
            Xlog.e(TAG, "Could not start media recorder. ", e);            
            mEngine.releaseRecorder();
            return;
        }
        ...
        if (mIsPanelHidable) {
            mMainHandler.sendMessageDelayed(mMainHandler.obtainMessage(MSG_CONTROL_PANEL_ONOFF, 1,0, null),ANIMATION_DURATION);
            mMainHandler.sendMessageDelayed(mMainHandler.obtainMessage(MSG_CONTROL_PANEL_ONOFF, 0,0, null),CONTROL_PANEL_ON_DURATION);                    
        }
    ...
    private void cleanupEmptyFile() {
        if (mCreatedVideoFilename != null) {
            ...
            if (f.length() == 0 && f.delete()) {
                Xlog.v(TAG, "Empty video file deleted: " + mCreatedVideoFilename);
                mCreatedVideoFilename = null;
            }
        }
    } 
    ...
    private void onAudioFormatChanged(int newFormat) {
        ...
        Xlog.d(TAG, "onAudioFormatChanged() newFormat = "+newFormat+"; mAudioFormat = "+mAudioFormat);
