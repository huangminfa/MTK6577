...
/// M: fix ALPS00344913, do not resotre wrap scale for wap.163.com.
...
public final class WebViewCore {
    ...
    /// M: for scrolling performance. add a touch message queue. let webviewcore handle touch message first @{
    private final static boolean mCustomizeTouchEvent = true;
    /// @}
    ...
    */
    ...
    public class EventHub {
        ...
        static final int CLEAR_LAST_DRAW_DATA = 197;
        ...
        /// M: for scrolling performance. @{
        private ConcurrentLinkedQueue<Message> mTouchEventQueue = new ConcurrentLinkedQueue<Message>();
        private long mLastTouchSequence = Long.MIN_VALUE;
        /// @}
                ...
                public void handleMessage(Message msg) {
                    ...
                    switch (msg.what) {
                        ...
                        case TOUCH_EVENT: {
                            ...
                            /** M: for scrolling performance, handle TOUCH_EVENT first @{ */
                            Message touchMsg = Message.obtain(
                                    ...
                                    ted);
                            final long curSeq = ted.mSequence;
                            if (mLastTouchSequence > curSeq)
                                Log.e(LOGTAG, "[WebViewCore::TOUCH_EVENT] =TOUCH= sequence error [case 1] last="+mLastTouchSequence+" cur="+curSeq);

                            mLastTouchSequence = curSeq;
                            if (mWebView.mPrivateHandler.hasMessages(WebView.PREVENT_TOUCH_ID))
                                mWebView.mPrivateHandler.sendMessage(touchMsg);
                            else
                                mWebView.mPrivateHandler.sendMessageAtFrontOfQueue(touchMsg);

                            synchronized (mTouchEventQueue) {
                                if (!mTouchEventQueue.isEmpty() && !hasMessages(TOUCH_EVENT)) {
                                    Message msg1 = (Message)mTouchEventQueue.poll();
                                    TouchEventData ed = (TouchEventData)msg1.obj;
                                    if (ed.mSequence < curSeq)
                                        Log.e(LOGTAG, "[WebViewCore::TOUCH_EVENT] =TOUCH= sequence error [case 2] cur="+curSeq+" got="+ed.mSequence);
                        ...
                        case UPDATE_CACHE_AND_TEXT_ENTRY:
                            ...
                            if (mWebView != null) {
                                ...
                                Message.obtain(mWebView.mPrivateHandler, WebView.FINISH_VIEW_SIZE_CHANGE).sendToTarget();
                            }
                            sendUpdateTextEntry();
                            break;
                        ...
                        case AUTOFILL_FORM:
                            nativeAutoFillForm(msg.arg1, msg.arg2);
                        ...
                        case EXECUTE_JS:
                            ...
                            break;
                        /// M: fix native crash for ALPS00351926 @ {
                        /// webview.loadViewState will setBaseLayer which cause previous baseLayer
                        /// destroied (baseLayer is a smartPointer). but WebViewCore.mLastDrawData.mBaseLayer
                        /// hold this baseLayer, once call UpdateLayers will crash.
                        case CLEAR_LAST_DRAW_DATA:
                            mLastDrawData = null;
                            break;
                        /// M: @ }
                    }
        ...
        /// M: for scrolling performance @{
        private boolean hasMessages(int what) {
            return mHandler.hasMessages(what);
        }

        private void appendTouchEventMessage(int what, Object obj) {
            synchronized (mTouchEventQueue) {
                Message msg = Message.obtain(null, what, obj);
                mTouchEventQueue.add(msg);
                if (!hasMessages(what) && !mTouchEventQueue.isEmpty()) {
                    Message msg1 = (Message)mTouchEventQueue.poll();
                    sendMessageAtFrontOfQueue(msg1);
                }
            }
        ...
        private void clearTouchEventMessage() {
            synchronized (mTouchEventQueue) {
    ...
    /// M: for socrlling performance, send touch event first @{
    void sendTouchMessage(int what, Object obj) {
        if (!mCustomizeTouchEvent){
            sendMessage(what, obj);
            return;
        }
        mEventHub.appendTouchEventMessage(what, obj);
    }

    void removeTouchMessages(){
        if (!mCustomizeTouchEvent) {
            removeMessages(EventHub.TOUCH_EVENT);
            return;
        }
        mEventHub.clearTouchEventMessage();
    }
    /// @}
    ...
    private void viewSizeChanged(WebView.ViewSizeData data) {
        ...
        if (mWebView.mOrientationChange == true) {
            mWebView.mOrientationChange = false;
    ...
    private int calculateWindowWidth(int viewWidth) {
        ...
        if (mSettings.getUseWideViewPort()) {
            if (mViewportWidth == -1) {
                ...
                // For mobile web site.
                /// M: fix ALPS00324490, re-calculate the window width if initial-scale has been set. @{
                if (mViewportInitialScale > 0 && mViewportInitialScale < 100 * mAdjust)
                    width = Math.round(mWebView.getViewWidth() / (mViewportInitialScale * 0.01f));
                else                
    ...
    private Object mLastDrawDataLock = new Object();

    void clearLastDrawData(int layerPtr) {
        synchronized (mLastDrawDataLock) {
            if (null != mLastDrawData && layerPtr == mLastDrawData.mBaseLayer)
                mLastDrawData = null;
        }
    }
    /// M: @}
    ...
    private void webkitDrawLayers() {
        /// M: synchronized mLastDrawDataLock for ALPS00355738
        synchronized (mLastDrawDataLock) {
    ...
    private void setupViewport(boolean updateViewState) {
        ...
        float adjust = 1.0f;
        /// M: If mViewportDensityDpi equals to 0, that means the device will use personal value. @{
         if (mViewportDensityDpi == -1 || (mViewportDensityDpi == 0 && mWebView != null && mWebView.isBrowserApp() == true)) {
        /// @}
        ...
        /// M: fix ALPS00344913, do not resotre wrap scale for mobile site. @{
        if (mIsRestored == true && ZoomManager.exceedsMinScaleIncrement(mRestoredScale, mRestoredTextWrapScale > 0 ? mRestoredTextWrapScale : mInitialViewState.mViewScale) == false) {
            if (mInitialViewState.mMobileSite == true)
                mIsRestored = false;
        }
        /// @}
    ...
    private void domFocusChanged() {
        mWebView.mPrivateHandler.obtainMessage(WebView.DOM_FOCUS_CHANGED).sendToTarget();
    }
   ...
   private native void nativeAutoFillForm(int queryId, int nodePointer);
