...
sp<MediaExtractor> MediaExtractor::Create(
    ...
    sp<AMessage> meta;
    LOGD("+MediaExtractor::Create");
    String8 tmp;
/*
#ifndef ANDROID_DEFAULT_CODE
	bool isNeedSniff = (mime == NULL) || (mime != NULL && !isKnownType(mime));
    if (isNeedSniff) 
#else
*/ //remove this check as some server send the wrong content-type
	if (mime == NULL) 
//#endif
	{
        float confidence;
        ...
        mime = tmp.string();
        LOGD("Autodetected media content as '%s' with confidence %.2f",
             mime, confidence);
    }
    ...
    if (!strncmp(mime, "drm+", 4)) {
        LOGD("create MediaExtractor: original mime: %s", mime);
        ...
        if (!strncmp(mime, "drm+es_based+", 13)) {
            ...
            mime = originalMime;
            isDrm = true;
            // OMA DRM v1 implementation: return NULL instead of reaturn specific Extractor type
            LOGD("mime is container_based, return NULL");
            return NULL;
        } else {
            return NULL;
        }
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_CONTAINER_MPEG4)
        ...
        LOGE(" MediaExtractor::is PS file, not support playing now");
        ret =NULL;
