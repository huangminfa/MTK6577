        ...
        + pC->iInOutTimeOffset) >= pC->pC1->iEndTime )
    {
        ...
        * Video is done for this clip */
        ...
        if (err == M4VSS3GPP_WAR_SWITCH_CLIP) {
            LOGI("do once more mode check for switching clip");
...
#ifndef ANDROID_DEFAULT_CODE
#if 0
    LOGD ("@@ M4VSS3GPP_intEditStepVideo state(%d)", pC->Vstate);
    if (pC->pC1 && pC->pC1->pSettings) {
        LOGD ("@@ pC1(%s)", pC->pC1->pSettings->pFile);
    }
    if (pC->pC2 && pC->pC2->pSettings) {
        LOGD ("@@ pC2(%s)", pC->pC2->pSettings->pFile);
    }
#endif
#endif
...
#ifndef ANDROID_DEFAULT_CODE
    LOGD("M4VSS3GPP_intCheckVideoMode >> state = %d", pC->Vstate);
#endif
...
#ifndef ANDROID_DEFAULT_CODE
    LOGD("M4VSS3GPP_intCheckVideoMode << state = %d, bEncodeTillEoF = %d", pC->Vstate, pC->bEncodeTillEoF);
#endif
    /**
    * Return with no error */
                           ...
                           M4VIFI_ImagePlane *pPlaneOut )
{
    ...
    * VPP context is actually the VSS3GPP context */
    ...
    //LOGD ("@@ M4VSS3GPP_intVPP pC->bTransitionEffect(%d)", pC->bTransitionEffect);
        ...
        if (pC->pC1->isRenderDup == M4OSA_FALSE) {
            ...
            *   Check if resizing is needed */
            if (M4OSA_NULL != pC->pC1->m_pPreResizeFrame) {
                ...
                #ifndef ANDROID_DEFAULT_CODE
                    LOGD ("@@ Resize m_pFctRender return ERROR!! (0x%08X)", err);
                    return err;
                #else
                    return M4NO_ERROR;
                #endif
                ...
                // Morris Yang 20120417 [
                #ifndef ANDROID_DEFAULT_CODE
                    LOGD ("@@ Resize M4VSS3GPP_intApplyVideoEffect return ERROR!! (0x%08X)", err);
                #endif
                // ]
                        return M4NO_ERROR;
                    }
            ...
            else
            {
                ...
                if (M4NO_ERROR != err) {
                 ...
                 // Morris Yang 20120417 [
               #ifndef ANDROID_DEFAULT_CODE
                    LOGD ("@@ No resize m_pFctRender return ERROR!! (0x%08X)", err);
                    return err;
               #else
                    return M4NO_ERROR;
                #endif
                             ...
                             M4VIFI_ImagePlane *pPlaneOut )
{
    ...
    //LOGD ("@@ M4VSS3GPP_intVideoTransition iDur=%d, TransitionType(%d), TransitionBehaviour(%d)", iDur, pC->pTransitionList[pC->uiCurrentClip].VideoTransitionType, pC->pTransitionList[pC->uiCurrentClip].TransitionBehaviour);
