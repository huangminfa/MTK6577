...
public class LatinKeyboard extends Keyboard {
    ...
    @Override
	public CharSequence adjustLabelCase(Key key) {
		final CharSequence label = key.mLabel;
		final boolean isAlphaKeyboard = isAlphaKeyboard();
		final boolean isShiftedOrShiftLocked = isShiftedOrShiftLocked();
		final boolean isNotEmpty = !TextUtils.isEmpty(label);
		final boolean isLengthOk = label.length() < 3;
		final boolean isLowerCase = Character.isLowerCase(label.charAt(0));
		if (isAlphaKeyboard && isShiftedOrShiftLocked && isNotEmpty
				& isLengthOk && isLowerCase) {
			return label.toString().toUpperCase(mId.mLocale);
		} else if (isAlphaKeyboard && isShiftedOrShiftLocked && isNotEmpty
				& isLengthOk) {
			if (key.mUpperCaseLabel != null) {
				return key.mUpperCaseLabel;
			}
		}
		return label;
	}
