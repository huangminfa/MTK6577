...
static unsigned short batteryTempBuffer[BATTERY_AVERAGE_SIZE];  //add here
...
#if defined(MTK_JEITA_STANDARD_SUPPORT)
		static bool temp_error_recovery_chr_flag =KAL_TRUE;
...
void bq24158_set_ac_current(void)
{
    ...
    bq24158_config_interface_reg(0x06,0x10); //set ISAFE
	 ...
	 {
		  // bq24158_set_low_chg(1);  //limit output current control to 325mA
     ...
     {
          //bq24158_set_low_chg(0);  //release limitation of 325mA  FAN_CON5
...
int BAT_CheckBatteryStatus_bq24158(void)
{
   ...
   //-----------------------------------------------------------------------------
    ...
    if (Enable_BATDRV_LOG >= 1) {
    	printk("[BATTERY:ADC:bq24158] VCHR:%ld BAT_SENSE:%ld I_SENSE:%ld TBAT:%d \n", 
	...
	if (Enable_BATDRV_LOG >= 1) {
    	printk("[BATTERY:AVG:bq24158] BatTemp:%d Vbat:%ld VBatSen:%ld SOC:%ld ChrDet:%d Vchrin:%ld Icharging:%ld ChrType:%d USBstate:%d\r\n", 
    ...
    if (Enable_BATDRV_LOG >= 1) {		
		printk("[BATTERY:FG:bq24158] %d,%ld,%ld,%ld,%d,%ld,%ld,%d,%d,%d,%d\r\n", 
       	BMT_status.temperature ,BMT_status.bat_vol, BMT_status.ADC_BAT_SENSE, BMT_status.SOC, 
    ...
    if( upmu_is_chr_det() == KAL_TRUE)
    {   
        pchr_turn_on_charging_bq24158();	//mtk71259   20120707
