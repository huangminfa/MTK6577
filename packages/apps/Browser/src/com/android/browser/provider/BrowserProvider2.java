...
public class BrowserProvider2 extends SQLiteContentProvider {
    ...
    public static interface SearchesSuggestions {
        public static final Uri CONTENT_URI = Uri.withAppendedPath(
                BrowserContract.AUTHORITY_URI, "new_searches");
        public static final String _ID = "_id";
    ...
    static final String VIEW_NEW_SEARCHES = "new_searches";
    ...
    public static interface BookmarkFoldersColumns {
        public static final String _ID = "_id";
    ...
    public static final String[] BOOKMARK_FOLDERS_PROJECTION = new String[] {
        BookmarkFoldersColumns._ID, BookmarkFoldersColumns.PARENT_ID, 
    ...
    static final int NEW_SEARCHS = 40;
    ...
    static {
        ...
        matcher.addURI(authority, "new_searches", NEW_SEARCHS);
        // For DM
    ...
    final class DatabaseHelper extends SQLiteOpenHelper {
        ...
        void createNewSearches(SQLiteDatabase db) {
            db.execSQL(SQL_CREATE_NEW_SEARCHES);
        }
        ...
        boolean importFromBrowserProvider(SQLiteDatabase db) {
            ...
            if (oldDbFile.exists()) {
                ...
                try {
                    ...
                    String table_folder = BrowserProvider.TABLE_NAMES[BrowserProvider.URI_MATCH_BOOKMARK_FOLDERS];
        ...
        private int addDefaultBookmarks(SQLiteDatabase db, long parentId, int position) {
            Resources res = getContext().getResources();
            final CharSequence[] bookmarks = res.getTextArray(
                    R.array.bookmarks);
            int size = bookmarks.length;
    ...
    public static CharSequence replaceSystemPropertyInString(Context context, CharSequence srcString) {
        ...
        final String client_id = getClientId(context);
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            ...
            case NEW_SEARCHS: {
                qb.setTables(VIEW_NEW_SEARCHES);
                break;
            }
    ...
    public Uri insertInTransaction(Uri uri, ContentValues values, boolean callerIsSyncAdapter) {
        ...
        switch (match) {
            case BOOKMARKS: {
                ...
                if (values.containsKey(Bookmarks.IS_FOLDER)) {
                    if (values.getAsBoolean(Bookmarks.IS_FOLDER) && values.containsKey(Bookmarks.PARENT)
                        ...
                        boolean isValid = isValidAccountName(values.getAsLong(Bookmarks.PARENT), values.getAsString(Bookmarks.TITLE));
                        if (!isValid) {
                            return null;
                        }
                    }
            ...
            case SEARCHES: {
            	  // If no created time is specified set it to now
                if (!values.containsKey(Searches.DATE)) {
    ...
    // Does folder name exist in folder who's id is parentId.    
    ...
    /**
     * Modified homepage in preferences.
     */
...
*/ 
    private static final String SQL_CREATE_VIEW_OMNIBOX_SUGGESTIONS =
            "CREATE VIEW IF NOT EXISTS v_omnibox_suggestions "
            + " AS "
            + "  SELECT _id, url, title, 1 AS bookmark, 0 AS visits, created"
            + "  FROM bookmarks "
            + "  WHERE deleted = 0 AND folder = 0 "
            + "  UNION ALL "
            + "  SELECT _id, url, title, 0 AS bookmark, visits, created "
            + "  FROM history "
            + "  WHERE url NOT IN (SELECT url FROM bookmarks"
            + "    WHERE deleted = 0 AND folder = 0) "
            + "  ORDER BY bookmark DESC, created DESC, visits DESC ";
            
    private static final String SQL_CREATE_NEW_SEARCHES =
            "CREATE VIEW IF NOT EXISTS new_searches "
            + " AS "
