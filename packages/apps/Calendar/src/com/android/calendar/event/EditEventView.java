...
public class EditEventView implements View.OnClickListener, DialogInterface.OnCancelListener,
    ...
    private static final Time END_TIME = new Time();
    static {
        END_TIME.set(0, 59, 23, 31, 11, END_YEAR);
    }
	///@}
    ...
    private class TimeListener implements OnTimeSetListener {
        ...
        public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
           ...
           ///M:Check the endTime.if date out of max range,set it max range value@{
            if (endTime.after(END_TIME)) {
            ...
            ///M:Set the event type wether activate by the start and end Millis@{
            endMillis = enableSpinnerByTime(startMillis, endMillis);
    ...
    ///M:#Lunar#
    private class LunarDateListener implements  LunarDatePickerDialog.OnDateSetListener{
        View mView;
        ...
        @Override
        public void onDateSet(LunarDatePicker view, int year, int month, int monthDay) {
            Log.d(TAG, "onDateSet: " + year + " " + month + " " + monthDay);
            // Cache the member variables locally to avoid inner class overhead.
            Time startTime = mStartTime;
            Time endTime = mEndTime;
    ...
    private class DateClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if(mIsUseLunarDatePicker){
                ...
                // The maxDate and minDate in DatePicker, is in millis rather than in day. 
                // So, we need to set the max border in 2036.12.31.59.59.999, which is 1 millis 
                ...
                ((DatePickerDialog)dpd).getDatePicker().setMaxDate(maxDate.toMillis(false) - 1);
                ((DatePickerDialog)dpd).getDatePicker().setMinDate(minDate.toMillis(false));
                ///:@}
            }
            ///:@}
    ...
    public EditEventView(Activity activity, View view, EditDoneRunnable done) {
        ...
        ///M:add input filter @{
        setLengthInputFilter(mTitleTextView, mActivity, TITLE_MAX_LENGTH);
        setLengthInputFilter(mLocationTextView, mActivity, LOCATION_MAX_LENGTH);
        setLengthInputFilter(mDescriptionTextView, mActivity, DESCRIPTION_MAX_LENGTH);;
        ///@}
    ...
    public void setModel(CalendarEventModel model) {
        ...
        if(Features.canLunarCalendarBeShown()) {
            ...
            if(model.mIsLunar) {
                RadioButton radioBtn = (RadioButton) mActivity.findViewById(R.id.switch_lunar);
                if(radioBtn != null){
                    radioBtn.setChecked(true);
                }
            } else {
                RadioButton radioBtn = (RadioButton) mActivity.findViewById(R.id.switch_gregorain);
                if(radioBtn != null){
                    radioBtn.setChecked(true);
                }
            }
    ...
    private static InputFilter[] sRecipientFilters = 
    	   { new Rfc822InputFilter() ,new InputFilter.LengthFilter(2000) };
    ...
    private void setDate(TextView view, long millis) {
        ...
        synchronized (TimeZone.class) {
            ...
            if(mIsUseLunarDatePicker) {
                Time time = new Time();
                time.set(millis);
                dateString = LunarUtil.getLunarDateString(time.year, time.month+1, time.monthDay);
            }
            ///@}
        }
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        ...
        ///M:#Guest# filter the local account for "Guest".@{
        int accountTypeColumn = c.getColumnIndexOrThrow(Calendars.ACCOUNT_TYPE);
    ...
    private void setLengthInputFilter(TextView inputText, final Context context, final int maxLength) {
    ...
    private class DialogManager implements DialogInterface.OnDismissListener,
        ...
        public void dialogShown() {
            if (isAnyDialogShown()) {
                LogUtil.w(TAG, "There is already a dialog shown, but another dialog is " +
                		"going to show.");
            }
    ...
    public void updateDatePickerSelection() {
        ...
        if(radioGroup != null ) {
            if(Features.canLunarCalendarBeShown()){
                ...
                /*RadioButton gregorain = (RadioButton) radioGroup.findViewById(R.id.switch_gregorain);
                if(gregorain!=null){
                    gregorain.setChecked(true);
                }*/
    ...
    private boolean isLunarDataPickerClicked(){
        ...
        ///M:check @{
        if(radioGroup == null){
            LogUtil.w(TAG, "RadioGroup is null,return false to show Gregorian Datepicker.");
            return false;
        }
        //if radioGroup is not visible,It says now cannot show lunar so can't be clicked.
        if(radioGroup.getVisibility() != View.VISIBLE){
            LogUtil.d(TAG, "RadioGroup is not visible,return false to show Gregorian Datepicker");
            return false;
        }
        ///@}
    ...
    public void dismissAllSpinners(){
        ...
        if (parent != null) {
            ...
            for (int i = 0; i < count; i++) { 
                childSpinner = (Spinner)parent.getChildAt(i).findViewById(R.id.reminder_minutes_value);
     ...
     */
    public long enableSpinnerByTime(long startMillis,long endMillis){
        boolean isInRange = endMillis <= startMillis + REPEAT_EVENT_MAX_DURATION ? true : false;
        if (!isInRange) {
            ...
            if (EVENT_ONLY_ONCE_INDEX == selectionIndex) {
                setSpinnerSelectionEnable(false);
            } else {
                endMillis = startMillis + REPEAT_EVENT_MAX_DURATION;
            }
        } else {
            setSpinnerSelectionEnable(true);
        }
        return endMillis;
    }
    
    ///Set the view only can be selected once event
    
    /**
     * set the spinner enable
     * @param isEnable true the enable,or false.
     */
    private void setSpinnerSelectionEnable(boolean isEnable) {
        mRepeatsSpinner.setEnabled(isEnable);
        mRepeatsSpinner.setClickable(isEnable);
        mRepeatsSpinner.setFocusable(isEnable);
    }
    ///@}
