...
#define WAPI_KEY_MGMT_CERT BIT1
#define WAPI_KEY_MGMT_PSK BIT2

#define IW_ENCODE_ALG_SM4   0x20
extern bool rda5890_assoc_in_progress;
			      ...
			      char *extra)
{
	...
	if ((alg == IW_ENCODE_ALG_NONE) || (dwrq->flags & IW_ENCODE_DISABLED)) {
		...
		if (ext->ext_flags & IW_ENCODE_EXT_GROUP_KEY) {
			 ...
			 */
			ret = rda5890_set_gtk(priv, (dwrq->flags & IW_ENCODE_INDEX) - 1, ext->tx_seq, IW_ENCODE_SEQ_MAX_SIZE,
				pkey->key, pkey->len);
			if (ret)
				goto out;
		} else {
		...
		if (ext->ext_flags & IW_ENCODE_EXT_GROUP_KEY) {
			 ...
			 * need to toggle among 1, 2, 3
			 */
			ret = rda5890_set_gtk(priv, (dwrq->flags & IW_ENCODE_INDEX) - 1, tmp, IW_ENCODE_SEQ_MAX_SIZE,
				ext->key, ext->key_len);
			if (ret)
				goto out;
		} else {
			 ...
			 char *extra)
{
    ...
    case IW_AUTH_WAPI_ENABLED:
        if(dwrq->value & WAPI_KEY_MGMT_PSK)
            {
                disable_wpa(priv);
                disable_wep(priv);
                priv->secinfo.auth_mode = IW_AUTH_ALG_WAPI;
            }
        else if(dwrq->value & WAPI_KEY_MGMT_CERT)
            {
                disable_wpa(priv);
                disable_wep(priv); 
                priv->secinfo.auth_mode = IW_AUTH_ALG_WAPI;
...
void rda5890_indicate_connected(struct rda5890_private *priv)
{
    ...
    RDA5890_DBGLAP(RDA5890_DA_WEXT, RDA5890_DL_NORM,"%s >>> \n connect mac: %2x:%2x:%2x:%2x:%2x:%2x \n", __func__,
         priv->curbssparams.bssid[0],priv->curbssparams.bssid[1],priv->curbssparams.bssid[2],
         priv->curbssparams.bssid[3],priv->curbssparams.bssid[4],priv->curbssparams.bssid[5]);
}
...
static struct bss_descriptor *get_bss_desc_from_scanlist(
		struct rda5890_private *priv, unsigned char *bssid)
{
		...
		struct sockaddr *awrq, char *extra)
{
    ...
    if(((!test_bit(ASSOC_FLAG_ASSOC_RETRY, &priv->assoc_flags) || memcmp(ap_addr, priv->assoc_bssid ,6))
        || memcmp(ap_addr, priv->assoc_bssid, 6))
					    ...
					    struct bss_descriptor *bss_desc)
{
	...
	if (bss_desc->wapi_ie_len) {
		...
		while(pos < bss_desc->wapi_ie_len)
        {
            ...
            num_2_str(bss_desc->wapi_ie[pos], (unsigned char*)(buf + 8 + 2*pos));
            pos ++;
        }      
		iwe.cmd = IWEVCUSTOM;
...
void rda5890_scan_worker(struct work_struct *work)
{
	...
	rda5890_shedule_timeout(1000); //old value: 1500
