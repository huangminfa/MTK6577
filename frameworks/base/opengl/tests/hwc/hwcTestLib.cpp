    ...
    EGLint *width, EGLint *height)
{
    ...
    surfaceCtl = client->createSurface(String8("hwc"), 0, 480, 800, PIXEL_FORMAT_RGB_565, 0);
    ...
    *dpy = eglGetDisplay(EGL_DEFAULT_DISPLAY);
    ...
    returnValue = EGLUtils::selectConfigForNativeWindow(*dpy,
        sConfigAttribs, native_surface.get(), &myConfig);
    if (returnValue) {
...
void hwcTestDisplayList(hwc_layer_list_t *list)
{
    ...
    for (unsigned int layer = 0; layer < list->numHwLayers; layer++) {
        ...
        testPrintI("      handle: %p",
                   list->hwLayers[layer].graphicBuffer->handle);
...
void hwcTestDisplayListHandles(hwc_layer_list_t *list)
{
    ...
    // [mtk04189] replaced buffer_handle_t with GraphicBuffer, and buffer_handle_t is deprecated now
    for (unsigned int layer = 0; layer < list->numHwLayers; layer++) {
        str << ' ' << list->hwLayers[layer].graphicBuffer->handle;
