...
void ap_handle_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	if ((sta->flags & WLAN_STA_ASSOC) &&
		...
		if (inactive_sec == -1) {
			wpa_msg(hapd->msg_ctx, MSG_DEBUG, "Check inactivity: Could not "
				"get station info rom kernel driver for "
				MACSTR, MAC2STR(sta->addr));
		} else if (inactive_sec < hapd->conf->ap_max_inactivity &&
			   sta->flags & WLAN_STA_ASSOC) {
			/* station activity detected; reset timeout state */
			wpa_msg(hapd->msg_ctx, MSG_DEBUG, "Station " MACSTR " has been "
				"active %is ago",
	...
	if ((sta->flags & WLAN_STA_ASSOC) &&
		...
		wpa_msg(hapd->msg_ctx, MSG_DEBUG, "Station " MACSTR " has ACKed data "
