...
//	private ControllerListener mListener;
	...
	public boolean queryAccount(long id) {
		Cursor accountCursor =mResolver.query(Email.ACCOUNT_URI, 
	...
	public long queryDefaultAccount() {
		...
		if (accountCursor != null && accountCursor.moveToFirst()) {
			return accountCursor.getLong(0);
		} else {
			log("can not find default email account");
			return -1;
		}
	}
	...
	public boolean restoreMessage (long messageID) {
		...
		Cursor cs =  mResolver.query(uri, projection, null, null, null);
	...
	public boolean deleteMessage (long messageID) {
		...
		Cursor messageCursor = mResolver.query(uri, message_PROJECTION,
	...
	private long getMaiboxId(int mailboxType){		
		...
		if (mailboxCursor != null && mailboxCursor.moveToNext()){
			return mailboxCursor.getLong(0);
		}
