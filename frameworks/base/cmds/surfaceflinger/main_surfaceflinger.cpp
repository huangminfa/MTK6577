...
int main(int argc, char** argv) {
    ...
    // ---------------------------------------------------------------------------------------------
    if (AID_ROOT == getuid()) {
        LOGI("[%s] set surfaceflinger is in root user, adjust caps for its thread", __func__);
        if (-1 == prctl(PR_SET_KEEPCAPS, 1, 0, 0)) {
            ...
            if (-1 == capset(&hdr, &data)) {
                LOGW("    cap setting failed, %s", strerror(errno));
            }
            ...
            data.inheritable = 0xffffffff;
            if (-1 == capset(&hdr, &data)) {
                LOGW("    cap re-setting failed, %s", strerror(errno));
            }
        }
    } else {
        LOGI("[%s] surfaceflinger is not in root user", __func__);
    }
    // ---------------------------------------------------------------------------------------------
