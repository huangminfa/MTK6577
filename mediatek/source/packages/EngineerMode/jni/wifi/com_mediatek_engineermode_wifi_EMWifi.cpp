...
static int com_mediatek_engineermode_wifi_EMWifi_setTestMode(JNIEnv *env,
		jobject thiz) {
	LOGE("JNI, Enter setTestMode succeed");
	INT_32 index = -1;
...
static int com_mediatek_engineermode_wifi_EMWifi_setNormalMode(JNIEnv *env,
		jobject thiz) {
	LOGE("JNI, Enter setNormalMode succeed");
	INT_32 index = -1;
...
static int com_mediatek_engineermode_wifi_EMWifi_setStandBy(JNIEnv *env,
		jobject thiz) {
	LOGE("JNI, Enter setStandBy succeed");
	INT_32 index = -1;
...
static int com_mediatek_engineermode_wifi_EMWifi_setEEPRomSize(JNIEnv *env,
	...
	INT_32 index = -1;
	LOGE("JNI Set the eep_rom size is size = (%d)", ((int) i4EepromSz));
...
static int com_mediatek_engineermode_wifi_EMWifi_setEEPRomFromFile(JNIEnv *env,
	...
	if (name) {
		...
		LOGV("setEEPRomFromFile, file name = (%s)", name);

		index = CWrapper::setEEPRomFromFile(name);

		delete[] name;
	} else {
		LOGV("Error when to alloc memeory at setEEPRomFromFile");
	}
...
static int com_mediatek_engineermode_wifi_EMWifi_readTxPowerFromEEPromEx(
	...
	if (PowerStatus) {
		...
		LOGE("NULL java array of TxPower from EE Prom Ex");
		return -2;
	}

	if (arrayLength != 3) {
		env->ReleaseLongArrayElements(PowerStatus, jniParameter, JNI_ABORT);
		LOGE("Error length pass to the array");
		return STATUS_ERROR;
	} else {
		...
		if (index == 0) {
			...
			LOGE("Error to call readTxPowerFromEEPromEx in native");
		}
	}
	LOGE(
...
static int com_mediatek_engineermode_wifi_EMWifi_getPacketTxStatusEx(
	...
	if (PktStatus) {
		...
		LOGE("NULL java array of Packet Tx Status Ex");
		return -2;
	}
...
static int com_mediatek_engineermode_wifi_EMWifi_setEEPromCKSUpdated(
	...
	LOGE("JNI, Enter setEEPromCKSUpdated succeed");
	INT_32 index = -1;
...
static int com_mediatek_engineermode_wifi_EMWifi_getPacketRxStatus(JNIEnv *env,
	...
	LOGE("JNI, Enter getPacketRxStatus succeed");
	INT_32 index = -1;
	jlong *jniParameter = NULL;
	INT_32 pi4RxOk, pi4RxCrcErr;

	if (arraylen != 2) {
		LOGE("Wrong array length for getPacketRxStatus");
		return STATUS_ERROR;
	}
	...
	if (i4Init) {
		...
		LOGE("NULL java array of getPacketRxStatus");
		return -2;
	}
	...
	if (index == 0) {
		...
		LOGE("JNI, getPacketRxStatus value pi4RxOk = (%d), pi4RxCrcErr = (%d)",
				(int) pi4RxOk, (int) pi4RxCrcErr);
	} else {
		LOGE("Native, get getPacketRxStatus failed");
	}
...
static int com_mediatek_engineermode_wifi_EMWifi_setLocalFrequecy(JNIEnv *env,
	...
	LOGE("JNI, Enter setLocalFrequecy succeed, i4TxPwrGain = %d, i4TxAnt = %d",
...
static int com_mediatek_engineermode_wifi_EMWifi_setOperatingCountry(
	...
	LOGE("JNI, Enter setOperatingCountry succeed, country name = %s", name);
...
static int com_mediatek_engineermode_wifi_EMWifi_setChannel(JNIEnv *env,
	...
	LOGE("JNI, Enter setChannel succeed, country i4ChFreqKHz = %d",
			(int) i4ChFreqKHz);
	INT_32 index = -1;
...
static int com_mediatek_engineermode_wifi_EMWifi_getSupportedRates(JNIEnv *env,
	...
	if (pu2RateBuf) {
		...
		LOGE("NULL java array of getSupportedRates");
		return -2;
	}
	...
	if (index == 0) {
		...
		LOGE("Native, methods call failed");
	}
	LOGE(
...
static int com_mediatek_engineermode_wifi_EMWifi_readEEPRom16(JNIEnv *env,
	...
	LOGE("JNI, Enter readEEPRom16 succeed, u4Offset = %d, pu4Value = %d",
...
static int com_mediatek_engineermode_wifi_EMWifi_readSpecEEPRom16(JNIEnv *env,
	...
	if (pu4Value) {
		...
		LOGE("NULL java array of readSpecEEPRom16");
		return -2;
	}
	...
	LOGE("JNI, Enter readSpecEEPRom16 succeed, u4Offset = %d, pu4Value = %d",
			(int) u4Offset, (int) value);
	env->ReleaseLongArrayElements(pu4Value, jniParameter, JNI_ABORT);
...
static int com_mediatek_engineermode_wifi_EMWifi_writeEEPRom16(JNIEnv *env,
	...
	LOGE("JNI, Enter writeEEPRom16 succeed, u4Offset = %d, u4Value = %d",
...
static int com_mediatek_engineermode_wifi_EMWifi_eepromReadByteStr(JNIEnv *env,
	...
	if (paucStr) {
		...
		LOGE("NULL java array of eepromReadByteStr");
		return -2;
	}
	...
	if (index == 0) {
		...
		LOGE("JNI, Enter eepromReadByteStr succeed, paucStr = %s", jniParameter);
	} else {
		LOGE("Native, eepromReadByteStr call failed");
	}
...
static int com_mediatek_engineermode_wifi_EMWifi_eepromWriteByteStr(JNIEnv *env,
	...
	if (name) {
		...
		LOGE("JNI, Enter eepromWriteByteStr succeed, name = %s", name);
...
static int com_mediatek_engineermode_wifi_EMWifi_setATParam(JNIEnv *env,
	...
	INT_32 index = -1;
	LOGE("JNI, Enter setATParam succeed, u4FuncIndex = %d, pu4FuncData = %d",
...
static int com_mediatek_engineermode_wifi_EMWifi_getATParam(JNIEnv *env,
	...
	if (pu4FuncData) {
		...
		LOGE("NULL java array of getATParam");
		return -2;
	}
	...
	LOGE("JNI, Enter getATParam succeed, u4FuncIndex = %d, pu4FuncData = %d",
...
static int com_mediatek_engineermode_wifi_EMWifi_setXtalTrimToCr(JNIEnv *env,
	...
	//	LOGD("com_mediatek_engineermode_wifi_EMWifi_setXtalTrimToCr_u4Value = %l", u4Value);
	int index;
	LOGE("JNI, Enter getATParam succeed, u4Value = %d", (int) u4Value);
...
static int com_mediatek_engineermode_wifi_EMWifi_queryThermoInfo(JNIEnv *env,
	...
	if (len != 2) {
		LOGE("Wrong length of queryThermoInfo array");
		return -2;
	}
	...
	if (pi4Enable) {
		...
		LOGE("NULL java array of queryThermoInfo");
		return -2;
	}
	...
	if (index == 0) {
		...
		LOGE("Native, Error to call queryThermoInfo");
	}
...
static int com_mediatek_engineermode_wifi_EMWifi_setThermoEn(JNIEnv *env,
	...
	INT_32 index = -1;
	LOGE("JNI, Enter setThermoEn succeed, i4Enable = %d", (int) i4Enable);
...
static int com_mediatek_engineermode_wifi_EMWifi_getSpecEEPRomSize(JNIEnv *env,
	...
	if (pi4EepromSz) {
		...
		LOGE("NULL java array of getSpecEEPRomSize");
		return -2;
	}
	index = CWrapper::getSpecEEPRomSize(&value);
	LOGE("JNI, Enter getSpecEEPRomSize succeed, value = %d", (int) value);
	jniParameter[0] = value;
...
static int com_mediatek_engineermode_wifi_EMWifi_getEEPRomSize(JNIEnv *env,
	...
	if (pi4EepromSz) {
		...
		LOGE("NULL java array of getEEPRomSize");
		return -2;
	}
	index = CWrapper::getEEPRomSize(&value);
	LOGE("JNI, Enter getEEPRomSize succeed, value = %d", (int) value);
	jniParameter[0] = value;
	env->ReleaseLongArrayElements(pi4EepromSz, jniParameter, JNI_ABORT);
	return index;
}
...
static int com_mediatek_engineermode_wifi_EMWifi_setPnpPower(JNIEnv *env,
	...
	INT_32 index = -1;
	LOGE("JNI, Enter setPnpPower succeed, i4PowerMode = %d", (int) i4PowerMode);
...
static int com_mediatek_engineermode_wifi_EMWifi_setAnritsu8860bTestSupportEn(
	...
	INT_32 index = -1;
	LOGE("JNI, Enter setAnritsu8860bTestSupportEn succeed, i4Enable = %d",
			(int) i4Enable);
...
static int com_mediatek_engineermode_wifi_EMWifi_writeMCR32(JNIEnv *env,
	...
	INT_32 index = -1;
	LOGE("JNI, Enter writeMCR32 succeed, offset = %d, value = %d",
...
static int com_mediatek_engineermode_wifi_EMWifi_readMCR32(JNIEnv *env,
	...
	if (value) {
		...
		LOGE("NULL java array of readMCR32");
		return -2;
	}
...
static int com_mediatek_engineermode_wifi_EMWifi_getXtalTrimToCr(JNIEnv *env,
	...
	if (value) {
		...
		LOGE("NULL java array of getXtalTrimToCr, can't get value");
		return -2;
	}
	...
	LOGE("step into getXtalTrimToCr and got the value result = (%d)",
			(int) result);
...
static int com_mediatek_engineermode_wifi_EMWifi_getDPDLength(JNIEnv *env,
	...
	if (pi4DPDLength) {
		...
		LOGE("NULL java array of getDPDLength");
		return -2;
	}
	index = CWrapper::getDPDLength(&value);
	LOGE("JNI, Enter getDPDLength succeed");
	jniParameter[0] = value;
...
static int com_mediatek_engineermode_wifi_EMWifi_writeDPD32(JNIEnv *env,
		jobject thiz, jlong offset, jlong value) {
	INT_32 index = -1;
	LOGE("JNI, Enter writeDPD32 succeed, offset = %d, value = %d",
			(int) offset, (int) value);
...
static int com_mediatek_engineermode_wifi_EMWifi_readDPD32(JNIEnv *env,
	...
	if (value) {
		...
		LOGE("NULL java array of readDPD32");
		return -2;
	}
	index = CWrapper::readDPD32(offset, &val);
	jniParameter[0] = val;
	LOGE("JNI, Enter readDPD32 succeed, offset = %d, value = %d",
			(int) offset, (int) val);
	env->ReleaseLongArrayElements(value, jniParameter, JNI_ABORT);
...
static int com_mediatek_engineermode_wifi_EMWifi_setDPDFromFile(JNIEnv *env,
	...
	if (name) {
		...
		LOGV("setDPDFromFile, file name = (%s)", name);

		index = CWrapper::setDPDFromFile(name);

		delete[] name;
	} else {
		LOGV("Error when to alloc memeory at setDPDFromFile");
	}
	env->ReleaseStringUTFChars(atcFileName, file_name);
...
static int com_mediatek_engineermode_wifi_EMWifi_getSupportChannelList(JNIEnv *env,
	...
	if (pau4Channel) {
		...
		LOGE("NULL java array of getSupportChannelList");
		return -2;
	}
	index = CWrapper::getSupportChannelList(value);
	if (!index) {
		LOGE("JNI, Enter getSupportChannelList succeed, channel list length is %d. value[1] is %d, value[2] is %d", (int)value[0], (int)value[1], (int)value[2]);
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
	...
	if (register_com_mediatek_engineermode_wifi_EMWifi(env) < 0) {
		LOGE("ERROR: Wi-Fi native for engineermode registration failed\n");
		goto bail;
	}
