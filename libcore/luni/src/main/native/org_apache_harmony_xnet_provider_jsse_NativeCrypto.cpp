...
//close the debug info for ALPS00274004
...
static void info_callback_LOG(const SSL* s __attribute__ ((unused)), int where, int ret)
{
    ...
    if (where & SSL_CB_LOOP) {
        LOGD("ssl=%p %s:%s %s", s, str, SSL_state_string(s), SSL_state_string_long(s));
    } else if (where & SSL_CB_ALERT) {
        str = (where & SSL_CB_READ) ? "read" : "write";
        LOGD("ssl=%p SSL3 alert %s:%s:%s %s %s",
                  s,
                  str,
        ...
        if (ret == 0) {
            LOGD("ssl=%p %s:failed exit in %s %s",
                      s, str, SSL_state_string(s), SSL_state_string_long(s));
        } else if (ret < 0) {
            LOGD("ssl=%p %s:error exit in %s %s",
                      s, str, SSL_state_string(s), SSL_state_string_long(s));
        } else if (ret == 1) {
            LOGD("ssl=%p %s:ok exit in %s %s",
                      s, str, SSL_state_string(s), SSL_state_string_long(s));
        } else {
            LOGD("ssl=%p %s:unknown exit %d in %s %s",
                      s, str, ret, SSL_state_string(s), SSL_state_string_long(s));
        }
    } else if (where & SSL_CB_HANDSHAKE_START) {
        LOGD("ssl=%p handshake start in %s %s",
                  s, SSL_state_string(s), SSL_state_string_long(s));
    } else if (where & SSL_CB_HANDSHAKE_DONE) {
        LOGD("ssl=%p handshake done in %s %s",
                  s, SSL_state_string(s), SSL_state_string_long(s));
    } else {
        LOGD("ssl=%p %s:unknown where %d in %s %s",
...
static int sslSelect(JNIEnv* env, int type, jobject fdObject, AppData* appData, int timeout) {
    ...
    do {
        ...
        if (fd.isClosed()) {
			...
			LOGD("sslSelect: fd is closed");
            break;
        }
        int intFd = fd.get();
        LOGD("sslSelect type=%s fd=%d appData=%p timeout=%d",
        ...
        result = select(maxFd + 1, &rfds, &wfds, NULL, ptv);
            LOGD("sslSelect %s fd=%d appData=%p timeout=%d => %d",
...
static int cert_verify_callback(X509_STORE_CTX* x509_store_ctx, void* arg __attribute__ ((unused)))
{
    ...
    LOGD("ssl=%p cert_verify_callback => %d", ssl, result);
    return result;
}
...
static void info_callback(const SSL* ssl, int where, int ret __attribute__ ((unused))) {
    LOGD("ssl=%p info_callback where=0x%x ret=%d", ssl, where, ret);
//#ifdef WITH_JNI_TRACE
    info_callback_LOG(ssl, where, ret);
//#endif
    if (!(where & SSL_CB_HANDSHAKE_DONE)) {
          LOGD("ssl=%p info_callback ignored", ssl);
        return;
    }
    ...
    if (env == NULL) {
        ...
        LOGD("ssl=%p info_callback env error", ssl);
        return;
    }
    if (env->ExceptionCheck()) {
        LOGD("ssl=%p info_callback already pending exception", ssl);
        return;
    }
    ...
    LOGD("ssl=%p info_callback calling handshakeCompleted", ssl);
    env->CallVoidMethod(sslHandshakeCallbacks, methodID);

    if (env->ExceptionCheck()) {
        LOGD("ssl=%p info_callback exception", ssl);
    }
    LOGD("ssl=%p info_callback completed", ssl);
}
...
{
    SSL* ssl = to_SSL(env, ssl_address, true);
    LOGD("ssl=%p NativeCrypto_SSL_do_handshake fd=%p shc=%p timeout=%d client_mode=%d",
    ...
    while (appData->aliveAndKicking) {
		...
		LOGD("doing handshake ++");
        ret = SSL_do_handshake(ssl);
		LOGD("doing handshake -- ret=%d",ret);
        appData->clearCallbackState();
        // cert_verify_callback threw exception
        if (env->ExceptionCheck()) {
            SSL_clear(ssl);
            LOGD("ssl=%p NativeCrypto_SSL_do_handshake exception => 0", ssl);
            return 0;
        }
        // success case
        if (ret == 1) {
        ...
        // retry case
        if (errno == EINTR) {
			LOGD("doing handshake Retry");
            continue;
        }
        // error case
        int sslError = SSL_get_error(ssl, ret);
        LOGD("ssl=%p NativeCrypto_SSL_do_handshake ret=%d errno=%d sslError=%d timeout=%d",
        ...
        if (sslError == SSL_ERROR_WANT_READ || sslError == SSL_ERROR_WANT_WRITE) {
            ...
            if (selectResult == THROWN_EXCEPTION) {
                // SocketException thrown by NetFd.isClosed
                SSL_clear(ssl);
                LOGD("ssl=%p NativeCrypto_SSL_do_handshake THROWN_EXCEPTION => 0", ssl);
                return 0;
            }
            if (selectResult == -1) {
                throwSSLExceptionWithSslErrors(env, ssl, SSL_ERROR_SYSCALL, "handshake error");
                SSL_clear(ssl);
                LOGD("ssl=%p NativeCrypto_SSL_do_handshake selectResult == -1 => 0", ssl);
                return 0;
            }
            if (selectResult == 0) {
             ...
             LOGE("Unknown error during handshake");
            break;
        }
    }
    ...
    if (ret == 0) {
        ...
        SSL_clear(ssl);
        LOGD("ssl=%p NativeCrypto_SSL_do_handshake clean error => 0", ssl);
        return 0;
    }
    ...
    if (ret < 0) {
        ...
        SSL_clear(ssl);
        LOGD("ssl=%p NativeCrypto_SSL_do_handshake unclean error => 0", ssl);
        return 0;
    }
...
{
    SSL* ssl = to_SSL(env, ssl_address, true);
    LOGD("ssl=%p NativeCrypto_SSL_renegotiate", ssl);
    if (ssl == NULL) {
        return;
    }
...
static jobjectArray NativeCrypto_SSL_get_certificate(JNIEnv* env, jclass, jint ssl_address)
{
    ...
    X509* certificate = SSL_get_certificate(ssl);
    if (certificate == NULL) {
        LOGD("ssl=%p NativeCrypto_SSL_get_certificate => NULL", ssl);
        return NULL;
    }
...
static jobjectArray NativeCrypto_SSL_get_peer_cert_chain(JNIEnv* env, jclass, jint ssl_address)
{
    ...
    if (ssl == NULL) {
		LOGD("NativeCrypto_SSL_get_peer_cert_chain, Break:ssl==NULL ");
        return NULL;
    }
    ...
    if (ssl->server) {
        ...
        if (x509 == NULL) {
            LOGD("ssl=%p NativeCrypto_SSL_get_peer_cert_chain => NULL", ssl);
            return NULL;
        }
...
static int sslRead(JNIEnv* env, SSL* ssl, jobject fdObject, jobject shc, char* buf, jint len,
     ...
     LOGD("Entering sslRead, caller requests to read %d bytes timeout=%d...", len,timeout);
    ...
    while (appData->aliveAndKicking) {
        ...
        unsigned int bytesMoved = BIO_number_read(bio) + BIO_number_written(bio);
         ...
         LOGD("Doing SSL_Read() ssl=%p, appData=%p",ssl,appData);
        if (!appData->setCallbackState(env, shc, fdObject)) {
            MUTEX_UNLOCK(appData->mutex);
        ...
        if (result <= 0) {
         ...
         LOGD("Returned from SSL_Read() with result %d, error code %d ssl=%p, appData=%p", result, sslError,ssl,appData);
...
static int sslWrite(JNIEnv* env, SSL* ssl, jobject fdObject, jobject shc, const char* buf, jint len,
    ...
    while (appData->aliveAndKicking && len > 0) {
        ...
        unsigned int bytesMoved = BIO_number_read(bio) + BIO_number_written(bio);
         ...
         LOGD("Doing SSL_write() with %d bytes to go ssl=%p, appData=%p", len,ssl,appData);
        if (!appData->setCallbackState(env, shc, fdObject)) {
            MUTEX_UNLOCK(appData->mutex);
            return THROWN_EXCEPTION;
        }
        ...
        if (result <= 0) {
         ...
         LOGD("Returned from SSL_write() with result %d, error code %d ssl=%p, appData=%p", result, sslError,ssl,appData);
#ifdef WITH_JNI_TRACE_DATA
        for (int i = 0; i < result; i+= WITH_JNI_TRACE_DATA_CHUNK_SIZE) {
            int n = std::min(result - i, WITH_JNI_TRACE_DATA_CHUNK_SIZE);
