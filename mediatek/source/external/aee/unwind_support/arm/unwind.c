                    ...
                    pid_t pid)
{
  ...
  switch (regclass)
    ...
    case _UVRSC_VFP:
      {
        _uw *ptr;
        _uw i, fpreg_start, fpreg_count;

        if (representation != _UVRSD_DOUBLE)
          return _UVRSR_FAILED;

        fpreg_start = discriminator >> 16;
        fpreg_count = discriminator & 0xffff;
        ptr = (_uw *) vrs->core.r[R_SP];
        for (i = 0; i < fpreg_count; i++)
          {
            /* FIXME: retrive double value from remote process */
            ptr += 2;
          }
        vrs->core.r[R_SP] = (_uw) ptr;
      }
                                        ...
                                        unsigned int sp_list[])
{
    ...
    // For deeper framers, rollback pc by one instruction
        ...
        if (pc & 1) {
            ...
            // Long offset 
            if ((prev_word & 0x0000f000) == 0x0000f000 && 
                (prev_word & 0xe0000000) == 0xe0000000) {
                pc -= 4;
            }
            else {
                pc -= 2;
...
int unwind_backtrace_with_ptrace(int tfd, pid_t pid, mapinfo *map, 
                                 unsigned int pc_list[], unsigned int sp_list[], 
                                 int *frame0_pc_sane)
{
...
#if DEBUG_UNWIND
	_LOG(tfd, 1, "Unwind frame %02d, PC=%#x SP=%#x\n", stack_level, saved_vrs.core.r[R_PC], 
	     saved_vrs.core.r[R_SP]);
#endif
	
        _Unwind_Reason_Code urc = get_eit_entry(ucbp, saved_vrs.core.r[R_PC], pid, map, &this_map);
        if (urc != _URC_OK) {
            ...
            case _URC_END_OF_STACK:
                _LOG(tfd, 1, "Relative PC=%#x from %s mark as CANTUNWIND\n", 
            ...
            default: {
                ...
                _LOG(tfd, 1, "Unwind failed, PC=%#x SP=%#x\n", 
        ...
        if (log_function ((_Unwind_Context *) &saved_vrs, pid, tfd, stack_level,
                          map, pc_list, sp_list) != _URC_NO_REASON) {
            code = _URC_FAILURE;
            break;
        }
        stack_level++;
