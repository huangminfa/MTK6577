...
public class WifiGprsSelector extends WifiSettings implements Preference.OnPreferenceChangeListener {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(TelephonyIntents.ACTION_ANY_DATA_CONNECTION_STATE_CHANGED)) {
                ...
                if (reason.equals(Phone.REASON_DATA_ENABLED) &&
                        (state == Phone.DataState.CONNECTED) && (mIsGprsSwitching)) {
                    ...
                    updateDataEnabler();
                        ...
                        (state == Phone.DataState.DISCONNECTED) && (mIsGprsSwitching)) {
    ...
    private boolean init() {         
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            try {
                mIsSIMExist = mITelephony.isSimInsert(Phone.GEMINI_SIM_1) ||
                                mITelephony.isSimInsert(Phone.GEMINI_SIM_2);
    ...
    private void initPhoneState() {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            mSimSlot = it.getIntExtra(Phone.GEMINI_SIM_ID_KEY, SIM_CARD_UNDEFINED);
        ...
        switch(mSimSlot) {
            case SIM_CARD_1:
                boolean sim1Ready = TelephonyManager.SIM_STATE_READY == 
                    TelephonyManager.getDefault().getSimStateGemini(0);
            ...
            case SIM_CARD_2:
                boolean sim2Ready = TelephonyManager.SIM_STATE_READY == 
                    TelephonyManager.getDefault().getSimStateGemini(1);
                mApnList.setEnabled(mScreenEnable && mIsCallStateIdle && 
            ...
            .setSingleChoiceItems(mAdapter, mInitValue, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        Xlog.d(TAG,"mIsSim=" + simItem.mIsSim + ",mState=" + simItem.mState
                            + ",SIM_INDICATOR_LOCKED=" + Phone.SIM_INDICATOR_LOCKED);

                        if (simItem.mIsSim) {
                            if (mCellConnMgr != null && simItem.mState == Phone.SIM_INDICATOR_LOCKED) {
