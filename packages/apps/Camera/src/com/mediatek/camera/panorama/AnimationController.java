...
public class AnimationController {
	public static final int ANIM_DURATION = 190;
	...
	private Runnable mApplyCenterArrowAnim = new Runnable(){
		...
		public void run() {
			...
			if (mCenterDotIndex >= mDotCount) return;
	...
	private Runnable mApplyDirectionAnim = new Runnable(){
		...
		public void run() {
			...
			if (mDirectionDotIndex >= mDotCount) return;
			Log.i("camera", "start Direction animation: " + mDirectionDotIndex); 
			AlphaAnimation alpha = new AlphaAnimation(1.0f, 0.0f);
			alpha.setDuration(ANIM_DURATION * mDotCount * 2);
			alpha.setRepeatCount(Animation.INFINITE);
			...
			mDirectionDotIndex++;
	    	mHanler.postDelayed(this, ANIM_DURATION * 2 / 3);
		}
	};
