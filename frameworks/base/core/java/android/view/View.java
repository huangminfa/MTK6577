...
public class View implements Drawable.Callback, Drawable.Callback2, KeyEvent.Callback,
    ...
    private static final boolean DBG_SAVE_RESTORE = false;
    ...
    */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (DBG_MOTION) {
            Xlog.d(VIEW_LOG_TAG, "(View)onTouchEvent 1: event = " + event + ",mTouchDelegate = " +
                    mTouchDelegate + ",enable = " + isEnabled() + ",clickable = " +
                    isClickable() + ",isLongClickable = " + isLongClickable() + ",this = " + this);
        }
        if ((viewFlags & ENABLED_MASK) == DISABLED) {
    ...
    protected void dispatchSaveInstanceState(SparseArray<Parcelable> container) {
        if (mID != NO_ID && (mViewFlags & SAVE_DISABLED_MASK) == 0) {
            ...
            if (DBG_SAVE_RESTORE) {
                Xlog.d(TAG, "dispatchSaveInstanceState: state = " + state + ",mID = "
                        + Integer.toHexString(mID) + ",this = " + this);
            }
            if ((mPrivateFlags & SAVE_STATE_CALLED) == 0) {
                throw new IllegalStateException(
                        "Derived class did not call super.onSaveInstanceState(), this = " + this);
            }
            if (state != null) {
    ...
    protected void dispatchRestoreInstanceState(SparseArray<Parcelable> container) {
        if (mID != NO_ID) {
            ...
            if (state != null) {
                if (DBG_SAVE_RESTORE) {
                    Xlog.i(TAG, "Restoring #" + Integer.toHexString(mID) + ": " + state
                            + ",this = " + this);
                }            
                mPrivateFlags &= ~SAVE_STATE_CALLED;
                ...
                if ((mPrivateFlags & SAVE_STATE_CALLED) == 0) {
                    throw new IllegalStateException(
                            "Derived class did not call super.onRestoreInstanceState(), this = " + this);
                }
            }
        }
    }
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        if (state != BaseSavedState.EMPTY_STATE && state != null) {
            this.mAttachInfo.mRootView.debug();
            throw new IllegalArgumentException("Wrong state class, expecting View State but "
                    + "received " + state.getClass().toString() + " instead. This usually happens "
                    + "when two views of different type have the same id in the same hierarchy. "
                    + "This view's id is " + ViewDebug.resolveId(mContext, getId()) + ". Make sure "
                    + "other views do not use the same id." + ",mID = " + mID + ",this = " + this);
        }
    }
    ...
    public void buildLayer() {
        ...
        if (mAttachInfo == null) {
            throw new IllegalStateException("This view must be attached to a window first, this = " + this);
        }
    ...
    */
    ...
    protected void debug(int depth) {
        ...
        if (mDisplayList != null) {
            output = debugIndent(depth);
            output += "displayList={" + "java displayList=" + mDisplayList +
                      ",  native DisplayList=0x" + Integer.toHexString(mDisplayList.getNativeDisplayList()) +
                      "}";
            Log.d(VIEW_LOG_TAG, output);
        }
    }
    ...
    public void requestLayout() {
        ...
        if (mParent != null) {
            if (DBG_LAYOUT) {
                Log.d(VIEW_LOG_TAG, "requestLayout: mParent = " + mParent + ",mLayoutParams = "
                        + mLayoutParams + ",mParent.isLayoutRequested() = "
                        + mParent.isLayoutRequested() + ",this = " + this);
            }            
            if (mLayoutParams != null) {
    ...
    static class AttachInfo {
        ...
        /**
         * Indicates whether the ActionBar need measure
         */
        boolean mActionBarMeasureRequired;
        
        /**
         * In case two continuous dispatchAppVisiblity message with different parameters come,
         * This flag will make sure the two message be force processed in ViewRoot, or SurfaceView's 
         * create new surface or ViewRoot's relayout may be skipped.
         */
        boolean mForceVisibilityChange;
    ...
    /**
     * This is used by viewgroups to clear child view's measured height previoulsy gained. For example,
    ...
    /**
     * This is used by viewgroups to clear child view's measured width previoulsy gained. For example,
    ...
    /**
     * This is used by ActionBarView to set its measured state as need force measurement. When ActionBarView init it's
     * title layout in onConfigurationChanged, it will call this method to make sure the title layout get measured. 
     * (either {@link #GONE}, {@link #INVISIBLE}, or {@link #VISIBLE}).
     * 
     * {@hide}
     */
    public final void setActionBarMeasureRequired() {
    	final AttachInfo attachInfo = mAttachInfo;
    	if (attachInfo != null) {
    		attachInfo.mActionBarMeasureRequired = true;
    	}
    }
}
