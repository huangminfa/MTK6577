...
static unsigned char* decryptDcfSource(JNIEnv* env,
    ...
    while (readSize < fileLength) {
        ...
        XLOGD("decryptDcfSource: pread read decrypted data %d Bytes", readLength);
...
static jobject decodeDcfSource(JNIEnv* env, jobject options,
    ...
    if (bitmap != NULL) {
        XLOGD("decodeDcfSource: createdBitmap is not NULL, wrap it into a Java Bitmap");
        return GraphicsJNI::createBitmap(env, bitmap, false, NULL);
    }
    else {
        XLOGD("decodeDcfSource: createdBitamp is NULL");
        return NULL;
    }
}
...
static jobject nativeDecodeFile(JNIEnv* env, jobject clazz,
    ...
    sp<DecryptHandle> decryptHandle = drmManagerClient->openDecryptSession(fd, 0, 0);
...
static jobject nativeForceDecodeFile(JNIEnv* env, jobject clazz,
    ...
    sp<DecryptHandle> decryptHandle =
            drmManagerClient->openDecryptSession(fd, 0, 0);
    if (NULL == decryptHandle.get()) {
