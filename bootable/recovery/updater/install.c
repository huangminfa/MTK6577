...
Value* FormatFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    //wschen 2012-05-24 
    if (!strcmp(location, "/system") || !strcmp(location, "/data") || !strcmp(location, "/cache")) {
        
        if (!strcmp(location, "/system")) {
            free(location);
            location = strdup(dev[SYSTEM_INDEX]);
            fprintf(stderr, "format /system %s\n", location);
        } else if (!strcmp(location, "/data")) {
            free(location);
            location = strdup(dev[DATA_INDEX]);
            fprintf(stderr, "format /data %s\n", location);
        } else if (!strcmp(location, "/cache")) {
            free(location);
            location = strdup(dev[CACHE_INDEX]);
...
Value* RetouchBinariesFn(const char* name, State* state,
    ...
    FILE *f_random = fopen("/dev/random", "wb");
    uint16_t random_bits = 0;
    if (f_random != NULL) {
#if 1 //wschen 2011-12-20 fix when pool is empty then read /dev/random will block update
        fwrite(&random_base, 4, 1, f_random);
#endif
