...
public class LauncherModel extends BroadcastReceiver {
    ...
    final static String SWITCH_SCENE_ACTION = "com.mediatek.intent.switch_scene";
    
    static final String SYSTEM_SCENE_WORK = "work";

    static final String SYSTEM_SCENE_DEFAULT = "default";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
        	...
        	String scene = intent.getStringExtra("selectedScene");
        	if (mCallbacks != null) {
        		Callbacks callbacks = mCallbacks.get();
        		if (callbacks != null) {
        			callbacks.clearAndSwitchScene(scene);
        		}
        	}
        }
    }
    ...
    /**
     * Stop loader and set loaded flags, we do not need to return the result of
     * stopLoaderLocked because we don't need it when called in launcher onCreate.
     */
    void stopLoaderAndSetLoadedFlags() {
        if (LauncherLog.DEBUG_LOADER) {
            LauncherLog.d(TAG, "stopLoaderAndSetLoadedFlags: mLoaderTask =" + mLoaderTask
                    + ",this = " + this);
        }
        synchronized (mLock) {
    ...
    public static void resetScene(Context context, String sceneName) {
    	final ContentResolver contentResolver = context.getContentResolver();
    	
    	contentResolver.delete(LauncherSettings.Favorites.CONTENT_URI, LauncherSettings.Favorites.SCENE + " = '" + sceneName + "'", null);
    }
    
    public static boolean exists(Context context, String sceneName) {
        final ContentResolver contentResolver = context.getContentResolver();
