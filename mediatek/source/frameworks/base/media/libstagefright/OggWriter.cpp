...
status_t OggWriter::stop() {
   ...
   //status_t status = mSource->stop();
    ...
    LOGV("OggWriter::stop:1");
    void *dummy;
    pthread_join(mThread, &dummy);
    LOGV("OggWriter::stop:2");
...
//If stop set mDone = true, threadFunc execute the following code, there is an NE error because buffer1 = 0
/*
	if (mDone)
	{
	    LOGE("WQG:OggWriter::threadFunc:buffer1->release,buffer1=%p",buffer1);
	    buffer1->release();
	    buffer1 = NULL;	
	}
*/
...
//After stop, there are some buffers ogg encoder have to deal, so send a new Mediabuffer(size=0) to it, end of stream
/*
    for (int i = 0; i < 2; i++)
    {
    MediaBuffer *buf_end = new MediaBuffer(0);
    MediaBuffer *buf1 = buf_end;
    err = mSource->read(&buf_end);
    if (err != OK)
    {
        LOGE("WQG:ThreadFunc End of stream deal error");
	if (buf_end != buf1)
	{
	    buf_end->release();
	    buf_end = NULL;
    ...
    {
        LOGE("WQG:OggWriter::threadFunc: mFile=%p,buf_end->data()=%p,buf_end->range_offset()=%d,buf_end->range_length()=%d,buf_end->range->size=%d",
		  mFile, buf_end->data(),buf_end->range_offset(),buf_end->range_length(),buf_end->size());
        if (buf_end->range_length() > 0)
        {
		ssize_t n = fwrite((const uint8_t *)buf_end->data() + buf_end->range_offset(), 1, buf_end->range_length(), mFile);
        }	
