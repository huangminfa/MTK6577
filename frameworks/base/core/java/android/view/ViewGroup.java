...
public abstract class ViewGroup extends View implements ViewParent, ViewManager {
    ...
    private boolean dispatchTransformedTouchEvent(MotionEvent event, boolean cancel,
        ...
        if (newPointerIdBits == 0) {
            Xlog.i(TAG, "Dispatch transformed touch event with no pointers in it.");
            return false;
        }
    ...
    protected void dispatchSaveInstanceState(SparseArray<Parcelable> container) {
        ...
        final View[] children = mChildren;
        if (DBG_SAVE_RESTORE) {
            Log.i(TAG, "dispatchSaveInstanceState: id = " + Integer.toHexString(mID)
                    + ",mChildrenCount = " + mChildrenCount + ",this = " + this);
        }    
    ...
    protected void dispatchRestoreInstanceState(SparseArray<Parcelable> container) {
        ...
        final View[] children = mChildren;
        if (DBG_SAVE_RESTORE) {
            Log.i(TAG, "dispatchRestoreInstanceState: id = " + Integer.toHexString(mID)
                    + ",mChildrenCount = " + mChildrenCount + ",this = " + this);
        }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if ((flags & FLAG_USE_CHILD_DRAWING_ORDER) == 0) {
            for (int i = 0; i < count; i++) {
                final View child = children[i];
                if (child != null && (child.mViewFlags & VISIBILITY_MASK) == VISIBLE || child.getAnimation() != null) {
