...
//              		Log.i("SIMMELOCK","[LockSetting]Equal ? "+(et.getText().toString().equals(re_et.getText().toString())));
               ...
               Message callback = Message.obtain(mHandler, LOCK_ICC_SML_COMPLETE);
                    ...
                    if(false == FeatureOption.MTK_GEMINI_SUPPORT)
                    {
                    	...
                    	phone.getIccCard().setIccNetworkLockEnabled(lockCategory,1,et.getText().toString(),null,null,null,callback);
                    }
                    else
                    {
                    	...
                    	if (intSIMNumber == 0)
                    	{
                    		mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_1).setIccNetworkLockEnabled(lockCategory,1,et.getText().toString(),null,null,null,callback);
                    		//editPwdProcess(et, "12345678");       	//compare with the true password "lockPassword"	
                    	}
                    	else
                    	{
                    		mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_2).setIccNetworkLockEnabled(lockCategory,1,et.getText().toString(),null,null,null,callback);
                    		//editPwdProcess(et, "12345678");       	//compare with the true password "lockPassword"	
	...
	protected void showAlertDialog(int id) {
		if(id == DIALOG_LOCKFAIL || id == DIALOG_PASSWORDLENGTHINCORRECT || id == DIALOG_LOCKSUCCEED || id == DIALOG_PASSWORDWRONG){
    	...
    	case DIALOG_LOCKFAIL: //password is wrong
    		{
    	    	...
    	    	.setMessage(R.string.strLockFail)
    	...
    	case DIALOG_LOCKSUCCEED://Length is incorrect
	    	...
	    	.setTitle(R.string.strAttention)
	    	.setIcon(android.R.drawable.ic_dialog_alert)
	    	.setMessage(R.string.strLockSucceed)
	    	.setOnKeyListener(this)
              ...
              case LOCK_ICC_SML_COMPLETE:
               {
            	   ...
            	   {
            		   //Toast.makeText(LockSetting.this, "lock fail", Toast.LENGTH_LONG).show();
               		   showAlertDialog(DIALOG_LOCKFAIL);
            	   }
            	   else
            	   {
            		   //Toast.makeText(LockSetting.this, "lock Succeed!", Toast.LENGTH_LONG).show(); 
