...
public class DmClient extends Activity {
    ...
    public void onNewVersionDetected(DownloadDescriptor dd) {
        ...
        String description = mDlDescriptions.getText() + " " + mDd.getField(Field.VERSION) + " (" +mDd.size + " Bytes)";
        ...
        mDownloadButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                boolean hasEnoughSpace = PersistentContext.getInstance(DmClient.this)
                                         .getMaxSize() > mDd.size;
                if (!hasEnoughSpace) {
                        ...
                        if (net != null) {
                            ...
                            if (netType == ConnectivityManager.TYPE_MOBILE) {
                                if (net.isRoaming()) {
                                    Log
                                    .d(TAG.Client,
                                       "[WARNING] downloading in roaming network.");
                                    showDialog(DIALOG_ROAMING_DOWNLOAD);
                                } else {
                                    Log
                                    .d(TAG.Client,
                                       "[WARNING] downloading in mobile network.");
                                ...
                                Log.e(TAG.Client, "[ERROR] invalid network type: "
                                      + netType + "." + net.getTypeName());
                            }
                        } else {
    ...
    private void onDownloadingPkg(int status) {
        ...
        description = mDlDescriptions.getText() + " "
                      + mDd.getField(Field.VERSION) + " (" + mDd.size + " Bytes)";
        ...
        Log.i(TAG.Client, "mDownloadedSize is " + downloadedSize
              + " ,mDd size is " + mDd.size);
    ...
    public void onDlPkgUpgrade(long dlSize, long totalSize) {
        Log.i(TAG.Client, "DmClient==>onDlPkgUpgrade(" + dlSize + ","
              + totalSize + ")");
        int ratial = (int) (((float) dlSize / (float) totalSize) * 100);
        if (mProgressBarDlPkg != null)
            mProgressBarDlPkg.setProgress(ratial);
        CharSequence text = Integer.toString(ratial) + "%    "
                            + Integer.toString((int) dlSize) + " Bytes / "
     ...
     *
     * @param String
     *            [] updateTypes - string array which contains the text content
     *            of update types.
     */
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        case DIALOG_NETWORKERROR:
            return DialogFactory.newAlert(this)
                   .setTitle(R.string.networkerror).setNeutralButton(
        ...
        case DIALOG_NONEWVERSION:
            return DialogFactory.newAlert(this)
                   .setTitle(R.string.nonewversion).setNeutralButton(
            R.string.ok, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog,
                int whichButton) {
        ...
        case DIALOG_NOENOUGHSPACE:
            ...
            new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog,
                int whichButton) {
                    // On user go to application manager to release
                    // space
                    Intent intent = new Intent();
                    intent
                    .setAction("com.android.settings.ManageApplications");
        ...
        case DIALOG_CANCELDOWNLOAD:
            return DialogFactory.newAlert(this)
                   ...
                   .setMessage(R.string.canceldownload).setPositiveButton(
            R.string.ok, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog,
                int whichButton) {
                    ...
                    if (mHandler!=null) {
                        ...
                        Log.w(TAG.Client,"[DIALOG_CANCELDOWNLOAD]positive button, mHandler is null");
                    }
                    finish();
                }
            }).setNegativeButton(R.string.cancel,
            ...
            new DialogInterface.OnClickListener() {
                ...
                int whichButton) {
                    if (mHandler!=null) {
                        ...
                        Log.w(TAG.Client,"[DIALOG_GPRSDOWNLOAD] negative button, mHandler is null");
                    }
                    finish();
                }
            }).create();
            ...
            new DialogInterface.OnClickListener() {
                ...
                int whichButton) {
                    if (mHandler!=null) {
                        ...
                        Log.w(TAG.Client,"[DIALOG_ROAMING_DOWNLOAD] negative button, mHandler is null");
                    }
                    finish();
                }
            }).create();
    ...
    public class SessionStateControlThread extends Thread {
        ...
        public void run() {
            Log.i(TAG.Client,
                  "In DmClient's new thread to invoke service's function: "
                  + mStateType);
            Looper.prepare();
            mHandler = new Handler() {
                ...
                public void handleMessage(Message msg) {
                    ...
                    case IDmPersistentValues.STATE_PAUSEDOWNLOAD:
                        Log.i(TAG.Client,
                              "SessionStateControlThread state is pausedownload");
                        mService.pauseDlPkg();
                        break;
                    ...
                    case IDmPersistentValues.STATE_NOTDOWNLOAD:
                        mService
                        .reportresult(MdmFumoUpdateResult.ResultCode.USER_CANCELED);
                        break;
                    default:
                        break;
                    }
    ...
    private ServiceConnection mConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            Log.i(TAG.Client,
                  "DmClient->onServiceConnected(), got service reference");
            mService = ((DmService.DmBinder) service).getService();
            if (mService == null) {
                Log.w(TAG.Client, "onServiceConnected mService is null");
                return;
            }
            if (DmService.getServiceInstance() != null && fumoListener != null) {
                Log.w(TAG.Client,
                      "onServiceConnected DmClient register listener");
                DmService.getServiceInstance().registListener(fumoListener);
            }
    ...
    private void executeUpdate() {
        ...
        try {
            if (DmService.getServiceInstance().IsInitDmController() == false) {
...
// modify for CR ALPS002821226(need to remove)
//        if (mStatus == IDmPersistentValues.STATE_WAPCONNECT_SUCCESS) {
//            Log.d(TAG.Client, "whether delta package exist?");
//            File updateFile = new File(DmConst.pathname.pathDelta);
//            File resumeFile = new File(DmConst.pathname.pathResume);
//            if (resumeFile.exists() && updateFile.exists()) {
//                Log.d(TAG.Client, "delta package exists");
...
//                    mStatus = IDmPersistentValues.STATE_PAUSEDOWNLOAD;
//                }else {
//                    mStatus = IDmPersistentValues.STATE_DLPKGCOMPLETE;
//                }
//                Log.i(TAG.Client, "After sync with xml file mStatus = " + mStatus);
//                Log.i(TAG.Client, "current size = " + currentSize+" ,totalSize "+totalSize);
//            }
//
//        }
//end modify
    ...
    private class FumoListener implements FumoUpdateListener {
        ...
        public void onupdate(Message msg) {
            ...
            case IDmPersistentValues.MSG_NEWVERSIONDETECTED:
                ...
                int sessionInitor = DmService.getServiceInstance()
                                    .getSessionInitor();
            ...
            case IDmPersistentValues.MSG_DLPKGCOMPLETE:
                ...
                if (DmService.getServiceInstance() != null) {
                    if (DmService.getServiceInstance().getUpdateTypes() != null)
                        onDlPkgComplete(DmService.getServiceInstance()
                                        .getUpdateTypes());
                }
                // }
                break;
            case IDmPersistentValues.MSG_DMSESSIONCOMPLETED:
                Log.d(TAG.Client, "msg>>dm session complete. status=" + mStatus
                      + ",initor="
            ...
            case IDmPersistentValues.MSG_DMSESSIONABORTED: {
                ...
                if ((lasterror == MdmException.MdmError.COMMS_FATAL.val)
                    ...
                    String errorMsg = "Error happens. Last error is "
                                      + lasterror;
    ...
    public Handler apnConnHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            if (mStatus != IDmPersistentValues.STATE_DETECT_WAP) {
                Log.w(TAG.Client, "apnConnHandler state is not STATE_DETECT_WAP, the status = "+mStatus);
                return;
            }
    ...
    private void checkNetwork() {
        ...
        try {
            ...
            if (result == MTKPhone.APN_ALREADY_ACTIVE) {
                ...
                Log.i(TAG.Client,
                      "checkNetwork network is not ok, request network establish");
//                setDmStatus(IDmPersistentValues.STATE_DETECT_WAP);
    ...
    private void setNetworkTimeoutAlarm() {
        ...
        if (alarmMgr != null) {
            networkTimeoutOp = PendingIntent.getBroadcast(this, 0, intent,
    ...
    private static PendingIntent networkTimeoutOp = null;
    ...
    private static DmClient mdmClientInstance = null;
    ...
    private FumoListener fumoListener = new FumoListener();
}
