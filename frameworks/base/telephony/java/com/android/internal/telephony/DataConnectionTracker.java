...
public abstract class DataConnectionTracker extends Handler {
    ...
    protected static final int EVENT_SCRI_RESULT = BASE + 33;      
    protected static final int EVENT_SCRI_RETRY_TIMER = BASE + 34;
    protected static final int EVENT_SCRI_CMD_RESULT = BASE + 35;
    protected static final int EVENT_NW_RAT_CHANGED = BASE + 36;
    protected static final int EVENT_PS_RAT_CHANGED = BASE + 37;
    ...
    protected static final int APN_MAX_ID = APN_CMMAIL_ID;
    //MTK-END
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            if (action.equals(Intent.ACTION_SCREEN_ON)) {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT) {
                    if (mPhone.getMySimId() == intent.getIntExtra(Phone.GEMINI_SIM_ID_KEY, -1))
                ...
                logd("Recv WIFIMgr NW State Changed Intent:mIsWifiConnected=" + mIsWifiConnected);
                    ...
                    if (FeatureOption.MTK_GEMINI_SUPPORT) {
                        ...
                        if ( mPhone.getMySimId()== currentDataConnectionSimId) {
                            if (mIsWifiConnected && isApnTypeEnabled(Phone.APN_TYPE_DEFAULT)) {
                                logd("mIsWifiConnected is true and DEFAULT is enabled.");
                                mPhone.disableApnType(Phone.APN_TYPE_DEFAULT);
                            } else if (!mIsWifiConnected && !isApnTypeEnabled(Phone.APN_TYPE_DEFAULT)){
                                logd("mIsWifiConnected is false and DEFAULT is NOT enabled.");                            
                                mPhone.enableApnType(Phone.APN_TYPE_DEFAULT);
                            }
                        }
                    }        
        ...
        public void register(Context context) {
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT && mPhone.getMySimId() == Phone.GEMINI_SIM_2) {
                resolver.registerContentObserver(
                        Settings.Secure.getUriFor(Settings.Secure.DATA_ROAMING_2), false, this);
    ...
    protected void onActionIntentReconnectAlarm(Intent intent) {
        ...
        Message msg = obtainMessage(EVENT_TRY_SETUP_DATA);
        msg.obj = reason;
    ...
    protected DataConnectionTracker(PhoneBase phone) {
        ...
        dataEnabled[APN_DEFAULT_ID] = (dataOnBoot && mUserDataEnabled);
    ...
    public void setDataOnRoamingEnabled(boolean enabled) {
        ...
        if (getDataOnRoamingEnabled() != enabled) {
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT && simId == Phone.GEMINI_SIM_2) {
                Settings.Secure.putInt(resolver, Settings.Secure.DATA_ROAMING_2, enabled ? 1 : 0);
            } else {
        ...
        try {
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                if (FeatureOption.MTK_GEMINI_ENHANCEMENT) {
                    SIMInfo simInfo = SIMInfo.getSIMInfoBySlot(mPhone.getContext(), simId);
                    if (simInfo != null) {
                        int mDataRoaming = simInfo.mDataRoaming;
                        logd("mDataRoaming = " + mDataRoaming);
                        return (mDataRoaming == SimInfo.DATA_ROAMING_ENABLE);
                    }
                    return false;
                } else {
                    if (simId == Phone.GEMINI_SIM_1) {
                        ...
                        return Settings.Secure.getInt(resolver, Settings.Secure.DATA_ROAMING_2) > 0;
                    }
                }
            } else {
    ...
    public boolean getAnyDataEnabled() {
        ...
        synchronized (mDataEnabledLock) {
            ...
            result = (mInternalDataEnabled && sPolicyDataEnabled
                    && (mUserDataEnabled
                        || (mRequestedApnType.equals(Phone.APN_TYPE_MMS) && dataEnabled[APN_MMS_ID]))
                    && (enabledCount != 0));
        }
    ...
    protected int apnTypeToId(String type) {
        if (TextUtils.equals(type, Phone.APN_TYPE_DEFAULT)) {
            ...
            return APN_DM_ID;
        } else if (TextUtils.equals(type, Phone.APN_TYPE_NET)) {
            return APN_NET_ID;
        } else if (TextUtils.equals(type, Phone.APN_TYPE_WAP)) {
            return APN_WAP_ID;
        } else if (TextUtils.equals(type, Phone.APN_TYPE_CMMAIL)) {
            return APN_CMMAIL_ID;
        } else {
                ...
                if (prevEnabled != getAnyDataEnabled()) {
                    if (!prevEnabled) {
                        ...
                        onCleanUpAllConnections(Phone.REASON_DATA_DISABLED);
                            ...
                            if (optr.equals("OP01")) {
                                 /* added by MTK: clean all interfaces except mms's interface for cmcc requirement */
                                 for (ApnContext apnContext : mApnContexts.values()) {
                                ...
                                /* added by MTK: only clean default interface. The mobile setting on/off only control default interface */
