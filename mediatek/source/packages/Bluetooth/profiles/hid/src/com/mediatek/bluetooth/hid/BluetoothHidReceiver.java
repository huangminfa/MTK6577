...
public class BluetoothHidReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
    	...
    	if(DEBUG)Xlog.v(TAG, "HID broadcast receiver receives intent");
        try{
	        String action = intent.getAction();
	         ...
	         if (action.equals(BluetoothDevicePicker.ACTION_DEVICE_SELECTED)) {
	            ...
	            if(remoteDevice == null){
	            	if(DEBUG)Xlog.e(TAG,"ERROR: remoteDevice null");
	            	return;
	            }
	            	
	            if(DEBUG)Xlog.v(TAG, "Received BT device selected intent, bt device: " + remoteDevice.getAddress());
        	...
        	if(DEBUG)Xlog.i(TAG,"hid broadcastReceiver error");
			e.printStackTrace();
		}
    }
	...
	private void restorePreferenceList(Context context,BluetoothDevice remoteDevice){
    	SharedPreferences settings=context.getSharedPreferences(BluetoothHidActivity.BT_HID_SETTING_INFO, 0);

			int preferenceCount=settings.getInt("preferenceCount", 0);	
			if(DEBUG)Xlog.i(TAG,"restorePreferenceList,preferenceCount="+preferenceCount);
			int preferenceIndex=0;
			for(preferenceIndex=0;preferenceIndex<preferenceCount;preferenceIndex++){
				String tmpAddr=settings.getString("deviceAddr"+Integer.toString(preferenceIndex), BluetoothHidActivity.BT_HID_NOT_FOUNT);
				if(tmpAddr.equals(remoteDevice.getAddress())){
					if(DEBUG)Xlog.i(TAG,"restorePreferenceList,already exist:"+remoteDevice.getName());
					settings.edit().putString("newAdd"+Integer.toString(preferenceIndex), "TRUE")
									.commit();
					return;
				}
			}
			...
			settings.edit().putString("deviceAddr"+Integer.toString(preferenceIndex), remoteDevice.getAddress())
							.putString("newAdd"+Integer.toString(preferenceIndex), "TRUE")
							.commit();
