...
static  int  mmc_operation(char *opr_code) {
    ...
    if (access(MMC_DEV, F_OK) < 0) {
        LOGE("AT_MMC_CMD " "The SD/MMC device isn't exist.");
        return MMC_OPR_FAIL;
    }

    if (NULL == opr_code) {
        LOGE("AT_MMC_CMD " "Operation has not defined.");
        return MMC_OPR_FAIL;
    }

    child = fork();
    if (child < 0) {
        LOGE("AT_MMC_CMD " "Fork fail.");
        return MMC_OPR_FAIL;
    }

    if (child == 0) {
        if (execl("/system/bin/vdc", "vdc", "volume", opr_code, "/mnt/sdcard", NULL) < 0) {
            LOGE("AT_MMC_CMD " "Child process execl fail.");
            exit(1);
        }
    } else {
        waitpid(child, &state, 0);
        if (WIFEXITED(state) && !WEXITSTATUS(state)) {
            LOGE("AT_MMC_CMD " "Child process %s SD/MMC successfully.", opr_code);
            return MMC_OPR_SUCCESS;
        } else {
            LOGE("AT_MMC_CMD " "Child process %s SD/MMC fail.", opr_code);
            LOGE("AT_MMC_CMD " "WIFEXITED    = %d\n", WIFEXITED(state));
            LOGE("AT_MMC_CMD " "WEXITSTATUS  = %d\n", WEXITSTATUS(state));
            LOGE("AT_MMC_CMD " "WIFSIGNALED  = %d\n", WIFSIGNALED(state));
            LOGE("AT_MMC_CMD " "WTERMSIG     = %d\n", WTERMSIG(state));
            LOGE("AT_MMC_CMD " "WCOREDUMP    = %d\n", WCOREDUMP(state));
            LOGE("AT_MMC_CMD " "WIFSTOPPED   = %d\n", WIFSTOPPED(state));
            LOGE("AT_MMC_CMD " "WSTOPSIG     = %d\n", WSTOPSIG(state));
            return MMC_OPR_FAIL;
        }
    }
...
int mmc_format_handler(char* cmdline, ATOP_t at_op, char* response){
    ...
    LOGE("AT_MMC_CMD " "handle cmdline:%s", cmdline);
...
int mmc_totalsize_handler(char* cmdline, ATOP_t at_op, char* response){
    ...
    if (fs_info.f_blocks == 0) { // May the card has not been mounted
        if (mmc_operation("mount") == MMC_OPR_SUCCESS) {
            ...
            LOGE("AT_MMC_CMD " "May the card has not been inserted.");
    }
