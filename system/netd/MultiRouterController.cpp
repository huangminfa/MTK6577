...
int MultiRouterController::setDefaults() {
   ...
   snprintf(cmd, sizeof(cmd), "route flush table %d",TETHER_TAB_NO);
	if (runIpCmd(cmd))
        {
          LOGE("setDefaults: RUN: %s fail ",cmd);
		  return -1;
	    }
...
int MultiRouterController::doMultiRouterCommands(const char *intIface,const char *extIface, bool add) {
  ...
  /*
	 property_get("ro.operator.optr", prop_value, NULL);
   	 ...
   	 {
       LOGE("ERR:Call MultiRouterCommands In No-Orange Building" );
	   return -1;
	 }*/

	LOGD("doMultiRouterCommands:downstreaminterface: %s, upstreaminterface: %s, add = %d, MultiRouterCount =%d ",intIface,extIface,add,MultiRouterCount );
    if (add == false) {
    ...
    snprintf(cmd, sizeof(cmd), "rule %s iif %s table tethering_table",
    ...
    LOGD("run cmd %s successfully ", cmd);
	
    /*
    snprintf(cmd, sizeof(cmd), "ip route %s default dev %s table tethering_table", (add ? "add" : "del"),
            intIface);
    if (runIpCmd(cmd)) {
        LOGE("run %s fail",cmd );
        return -1;
  ...
  if( ((0 == MultiRouterCount)&& add) ||((1 == MultiRouterCount)&& !add) )
   {
	if (add) {
         snprintf(cmd, sizeof(cmd), "route %s default dev %s table tethering_table",  "add" ,
            extIface);
   
    } else {
        snprintf(cmd, sizeof(cmd), "route flush table tethering_table" );
           }
	 if (runIpCmd(cmd)) {
        LOGE("run %s fail",cmd );
        setDefaults();
   ...
   {
     LOGD(" skip add default route to tethering_table "); 
   }
