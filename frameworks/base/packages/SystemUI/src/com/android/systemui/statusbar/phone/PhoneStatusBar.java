...
import com.android.systemui.statusbar.SIMIndicator;
import com.android.systemui.statusbar.SIMSwitchTip;
...
import com.android.systemui.statusbar.policy.SmallSIMIndicatorIcons;
...
public class PhoneStatusBar extends StatusBar {
    ...
    protected View makeStatusBarView() {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            if (IS_CU) {
                sb = (PhoneStatusBarView)View.inflate(context, R.layout.zzz_status_bar_gemini_cu, null);
            } else {
                sb = (PhoneStatusBarView)View.inflate(context, R.layout.zzz_status_bar_gemini, null);
            }
        } else {
        ...
        mToolBarView = (ToolBarView)expanded.findViewById(R.id.tool_bar_view);
        ToolBarIndicator indicator = (ToolBarIndicator)expanded.findViewById(R.id.indicator);
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            mSIMIndicator = (SIMIndicator) sb.findViewById(R.id.sim_indicator);
            mSIMIndicator.setVisibility(View.INVISIBLE);
            mSimIcon = (ImageView)sb.findViewById(R.id.simicon);
        }
        // [SystemUI] Support "SIM indicator". }
        ...
        // [SystemUI] Support "Dual SIM". {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            mTrackingView = (TrackingView)View.inflate(context, R.layout.zzz_status_bar_tracking_gemini, null);
            mCarrier1 = (LinearLayout)mTrackingView.findViewById(R.id.carrier1);
            mCarrier1Icon = (CarrierIconGemini)mTrackingView.findViewById(R.id.carrier1_icon);
            mCarrier1Name = (CarrierLabelGemini)mTrackingView.findViewById(R.id.carrier1_name);
            mCarrierDivider = mTrackingView.findViewById(R.id.carrier_divider);
            mCarrier2 = (LinearLayout)mTrackingView.findViewById(R.id.carrier2);
            mCarrier2Icon = (CarrierIconGemini)mTrackingView.findViewById(R.id.carrier2_icon);
            mCarrier2Name = (CarrierLabelGemini)mTrackingView.findViewById(R.id.carrier2_name);
            mCarrier1Icon.setSlotId(Phone.GEMINI_SIM_1);
            mCarrier2Icon.setSlotId(Phone.GEMINI_SIM_2);
            mCarrier1Name.setSlotId(Phone.GEMINI_SIM_1);
            mCarrier2Name.setSlotId(Phone.GEMINI_SIM_2);
        ...
        mBatteryController.addLabelView((TextView)sb.findViewById(R.id.percentage));
        // [SystemUI] Support "Dual SIM". {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            mNetworkControllerGemini = new NetworkControllerGemini(mContext);
            final SignalClusterViewGemini signalCluster = (SignalClusterViewGemini)sb.findViewById(R.id.signal_cluster);
...
//        }
    ...
    public void addNotification(IBinder key, StatusBarNotification notification) {
        ...
        Slog.d(TAG, "addNotification(" + key + " -> " + notification + ") notification ticker is: "+notification.notification.tickerText);
    ...
    public void updateNotification(IBinder key, StatusBarNotification notification) {
        Slog.d(TAG, "updateNotification(" + key + " -> " + notification + ") notification ticker is: "+notification.notification.tickerText);
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (simInfo != null) {
                NotificationData.Entry entry = mNotificationData.findByKey(key);
                updateNotificationSimInfo(simInfo, notification.notification, null, entry.expanded);
            }
        }
        // Recalculate the position of the sliding windows and the titles.
        setAreThereNotifications();
...
//        content.setOnFocusChangeListener(mFocusChangeListener);
    ...
    private void setAreThereNotifications() {
        ...
        if (mClearButton.isShown()) {
            if (clearable != (mClearButton.getAlpha() == 1.0f)) {//here the getAlpha() not 1.0f,but when the getAlpha() is not 0.0f it is shown,it is not we wanted
    ...
    private void makeExpandedVisible() {
        ...
        //Xlog.d("DateView", "makeExpandedVisible updateClock() ="+mExpandedVisible);
        if(mDateView != null)
        {
    ...
    void performExpand() {
        ...
        // [SystemUI] Support "Notification toolbar". {
        mToolBarView.enlargeTouchRegion();
        // [SystemUI] Support "Notification toolbar". }
    ...
    private class MyTicker extends Ticker {
        ...
        public void tickerStarting() {
            ...
            // [SystemUI] Support "SIM indicator". {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                if (mSIMIndicator.isShowing()) {
            	setSIMIndicatorVisibility(false, com.android.internal.R.anim.push_up_out);
            }
            }
            // [SystemUI] Support "SIM indicator". }
        ...
        public void tickerDone() {
        	if(!mShouldTickerDone) return;
            mIcons.setVisibility(View.VISIBLE);
            mTickerView.setVisibility(View.GONE);
            // [SystemUI] Support "SIM indicator". {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                if (mSIMIndicator.isShowing() ) {
                    setSIMIndicatorVisibility(true, com.android.internal.R.anim.push_down_in);
                    //setNotificationIconVisibility(false, com.android.internal.R.anim.fade_out);
                }
            }
            // [SystemUI] Support "SIM indicator". }
            ...
            if(mNeedShowSmallSIMIndicator){
            	showSmallSIMIndicator();
        		Log.i(TAG,"mNeedShowSmallSIMIndicator  is "+mNeedShowSmallSIMIndicator);
            	mNeedShowSmallSIMIndicator = false;
            }
            mShouldTickerDone = false;
        }
        ...
        public void tickerHalting() {
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                if (mSIMIndicator.isShowing()) {
                    setSIMIndicatorVisibility(true, com.android.internal.R.anim.fade_in);
                }
            }
            // [SystemUI] Support "SIM indicator". }
...
//        lp.token = mStatusBarView.getWindowToken();
    ...
    void setNotificationIconVisibility(boolean visible, int anim) {
        ...
        Log.i(TAG, " setNotificationIconVisibility  mShouldShowNotificationIcons is "+mShouldShowNotificationIcons, new Throwable());
        if (old != v && mShouldShowNotificationIcons) {
            ...
            synchronized (mNotificationData) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        int width = 0; //when the size of snapshot is 0,do not call get(int) function
                        if(snapshot.size() > 0) {
                            View sampleView = snapshot.get(0);
                            width = sampleView.getWidth();
                        }
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_CLOSE_SYSTEM_DIALOGS.equals(action)
                ...
                ImageView closeView = (ImageView)mTrackingView.findViewById(R.id.closeImg);
                closeView.setImageDrawable(context.getResources().getDrawable(R.drawable.status_bar_close_on));
                if (FeatureOption.MTK_GEMINI_SUPPORT) {/// M: Support "Theme manager". @{
                    ImageView carrierDivider = (ImageView)mTrackingView.findViewById(R.id.carrier_divider);
                    carrierDivider.setImageDrawable(context.getResources().getDrawable(R.drawable.zzz_carrier_divider));
                } /// M: Support "Theme manager". }@               
		repositionNavigationBar();
		updateResources();            
		mToolBarView.moveToCurrentScreen();
            }
            else if (Intent.ACTION_CONFIGURATION_CHANGED.equals(action)) {
            	Configuration currentConfig = context.getResources().getConfiguration();
                if (Math.abs(currentConfig.fontScale-mPreviousConfigFontScale)>0.00001f) {                	
                	refreshExpandedView(context);
                	mPreviousConfigFontScale = currentConfig.fontScale;
                }
                repositionNavigationBar();
                updateResources();
                // [SystemUI] Support "Notification toolbar". {
                ...
                if (Telephony.Intents.SPN_STRINGS_UPDATED_ACTION.equals(action)) {
                	int tempSimId = intent.getIntExtra(Phone.GEMINI_SIM_ID_KEY, Phone.GEMINI_SIM_1);
                	if (tempSimId == mCarrier1Name.getSlotId()) {
                		mCarrier1Name.updateNetworkName(intent.getBooleanExtra(Telephony.Intents.EXTRA_SHOW_SPN, false),
                        intent.getStringExtra(Telephony.Intents.EXTRA_SPN),
                        intent.getBooleanExtra(Telephony.Intents.EXTRA_SHOW_PLMN, false),
                        intent.getStringExtra(Telephony.Intents.EXTRA_PLMN));
                     } else {
                    	 mCarrier2Name.updateNetworkName(intent.getBooleanExtra(Telephony.Intents.EXTRA_SHOW_SPN, false),
                        intent.getStringExtra(Telephony.Intents.EXTRA_SPN),
                        intent.getBooleanExtra(Telephony.Intents.EXTRA_SHOW_PLMN, false),
                        intent.getStringExtra(Telephony.Intents.EXTRA_PLMN));
                     }
    ...
    void updateResources() {
        ...
        if (mSIMSwitchTip != null) {
            mSIMSwitchTip.updateResources();
        }
        // [SystemUI] Support "SIM Switch Tip". }
...
//            Slog.d(TAG, "size=" + newIconSize + " padding=" + newIconHPadding);
    ...
    NetworkControllerGemini mNetworkControllerGemini;

    LinearLayout mCarrier1 = null;
    CarrierIconGemini mCarrier1Icon = null;
    CarrierLabelGemini mCarrier1Name = null;
    View mCarrierDivider = null;
    LinearLayout mCarrier2 = null;
    CarrierIconGemini mCarrier2Icon = null;
    CarrierLabelGemini mCarrier2Name = null;
    ...
    private BroadcastReceiver mSIMInfoReceiver = new BroadcastReceiver() {
        public void onReceive(final Context context, final Intent intent) {
            ...
            } else if (action.equals(TelephonyIntents.ACTION_SIM_INSERTED_STATUS) || action.equals(TelephonyIntents.ACTION_SIM_INFO_UPDATE)) {
                mHandler.post(new Runnable() {
                    public void run() {
                        SIMHelper.updateSIMInfos(context);
                        if (FeatureOption.MTK_GEMINI_SUPPORT) {
                            mCarrier1Icon.setSIMInfoBySlot(Phone.GEMINI_SIM_1);
                            mCarrier2Icon.setSIMInfoBySlot(Phone.GEMINI_SIM_2);
                        }
                    }
                });
            }
    ...
    private void updateNotificationSimInfo(SIMInfo simInfo, Notification n, StatusBarIconView iconView, View itemView) {
        ...
        // item part.
        if ((simInfoType == 1 || simInfoType == 3) && simInfo != null) {
            Xlog.d(TAG, "updateNotificationSimInfo, add sim info to notification item. simInfo.mSimBackgroundRes = "+simInfo.mSimBackgroundRes+"  simInfo.mDisplayName = "+simInfo.mDisplayName);
            ImageView bgView = (ImageView) itemView.findViewById(com.android.internal.R.id.notification_sim_indicator_bg);
            bgView.setBackgroundResource(simInfo.mSimBackgroundRes);
            bgView.setVisibility(View.VISIBLE);
            TextView nameView = (TextView) itemView.findViewById(com.android.internal.R.id.notification_sim_indicator_text);
            nameView.setText(simInfo.mDisplayName);
            nameView.setVisibility(View.VISIBLE);
        } else {
            View bgView = itemView.findViewById(com.android.internal.R.id.notification_sim_indicator_bg);
            bgView.setVisibility(View.GONE);
            View nameView = itemView.findViewById(com.android.internal.R.id.notification_sim_indicator_text);
    ...
    SIMIndicator mSIMIndicator;

    long mSimId = -100;
    public void showSIMIndicator(String businessType) {
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {
            Xlog.d(TAG, "showSIMIndicator, only gemini version support SIM indicator.");
            return;
        }// [SystemUI] Support "SIMIndicator Animation". { 

        mShouldShowNotificationIcons = false;
        mCanceled = false;
        long simId = SIMHelper.getDefaultSIM(mContext, businessType);
        mSimId = simId;
        Xlog.d(TAG, "showSIMIndicator, begin --- show SIM indicator which business is " + businessType + " mShouldShowNotificationIcons = "+mShouldShowNotificationIcons+"  simId = "+simId+".");
        ...
        boolean ok = false;
        if (businessType.equals(android.provider.Settings.System.VOICE_CALL_SIM_SETTING) && simId == android.provider.Settings.System.VOICE_CALL_SIM_SETTING_INTERNET) {
            ok = mSIMIndicator.setSIPInfo();
        } else {
            ok = mSIMIndicator.setSIMInfo(simId);
        }
        if (ok) {
                hideSmallSIMIndicator();
                mNotificationIcons.setVisibility(View.INVISIBLE);
            mSIMIndicator.setShowing(true);
            if(!mTicking){
                setSIMIndicatorVisibility(true, -1);
            }
            mToolBarView.showSimSwithPanel(businessType);
            // [SystemUI] Support "SIM Switch Tip". {
            if (mSIMSwitchTip == null) {
                mSIMSwitchTip = new SIMSwitchTip(mContext);
            }
            mSIMSwitchTip.show(businessType);
            // [SystemUI] Support "SIM Switch Tip". }
                // [SystemUI] Support "SIMIndicator Animation". { 
                playRotate();
                // [SystemUI] Support "SIMIndicator Animation". } 
    ...
    public void hideSIMIndicator() {
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {
            Xlog.d(TAG, "hideSIMIndicator, only gemini version support SIM indicator.");
            return;
        }
        if(!mCanceled)
        	cancelAnimation();
        Xlog.d(TAG, "hideSIMIndicator, hide SIM indicator.");
        if(mSIMIndicator.isShowing()){
        mSIMIndicator.setShowing(false);
        mSIMIndicator.clearSIMInfo();
        setSIMIndicatorVisibility(false, -1);
        }
        mToolBarView.hideSimSwithPanel();
        // [SystemUI] Support "SIM Switch Tip". {
        if (mSIMSwitchTip != null) {
    ...
    void setSIMIndicatorVisibility(boolean visible, int anim) {
        int old = mSIMIndicator.getVisibility();
        int v = mSIMIndicator.isShowing() ? (visible ? View.VISIBLE : View.INVISIBLE) : View.INVISIBLE;
        if (old != v) {
            mSIMIndicator.setVisibility(v);
            if (anim != -1) {
                mSIMIndicator.startAnimation(loadAnim(anim, null));
            }
            Xlog.d(TAG, "setSIMIndicatorVisibility is " + String.valueOf(v == View.VISIBLE));
        }
    }
    ...
    private void refreshExpandedView(Context context) {    	
          ...
          expanded.mService = this;       
          mExpandedView = expanded;
          mToolBarView = (ToolBarView)mExpandedView.findViewById(R.id.tool_bar_view);
          ToolBarIndicator indicator = (ToolBarIndicator)mExpandedView.findViewById(R.id.indicator);
          mToolBarView.setStatusBarService(this);
          mToolBarView.setScrollToScreenCallback(indicator);
          mToolBarView.setToolBarIndicator(indicator);
          mToolBarView.hideSimSwithPanel();
          ...
          mNoNotificationsTitle.setVisibility(View.GONE); // disabling for now
          mClearButton = mExpandedView.findViewById(R.id.clear_all_button);
          mClearButton.setOnClickListener(mClearButtonListener);
          mClearButton.setAlpha(0f);
          mClearButton.setEnabled(false);
          mDateView = (DateView)mExpandedView.findViewById(R.id.date);
          mSettingsButton = mExpandedView.findViewById(R.id.settings_button);
          mSettingsButton.setOnClickListener(mSettingsButtonListener);
          mPile = (NotificationRowLayout)mExpandedView.findViewById(R.id.latestItems);
 ...
 // [SystemUI] Support "SIMIndicator Animation". {
   ...
   private final Handler handlerTime = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			Log.i(TAG," handlerTime --- handleMessage msg.what = "+msg.what);
			if (msg.what != 0) {
				//do nothing
			} else {
				Log.i(TAG,"handlerTime  mTicking  is "+mTicking);
				if(!mTicking){
					applyRotation(0, 0, 180);
				} else {
					mNeedShowSmallSIMIndicator = true;
				}
			}
		}
	};
	...
	private void cancelAnimation(){
		Log.i(TAG,"line 2784  cancelAnimation start  mAnimationRuning "+mAnimationRuning);
		mCanceled = true;
		if(mAnimationRuning){
			if(mRotation != null && mAnimationRuning){
				mRotation.cancel();
				mRotation.reset();
			}
		}
		...
		mNotificationIcons.setVisibility(View.VISIBLE);
		Log.i(TAG,"2796  cancelAnimation end mCanceled "+mCanceled);
		
	}
	private boolean mNeedShowSmallSIMIndicator = false;
	private Timer timer;
	...
	private void applyRotation(int position, float start, float end) {
		mRotation = AnimationUtils.loadAnimation(mContext, com.android.internal.R.anim.push_up_out);
		mRotation.setAnimationListener(new SIMIndicatorAnimationListener());
		
		Log.i(TAG,"applyRotation  mTicking  is "+mTicking+"  mCanceled is "+mCanceled);
		if(!mTicking && !mCanceled){
			mSIMIndicator.setShowing(true);
			mSIMIndicator.startAnimation(mRotation);
		} else {
			Log.i(TAG,"2800  mTicking  is "+mTicking);
			if(mTicking){
				mNeedShowSmallSIMIndicator = true;
			}
	...
	private void showSmallSIMIndicator(){
		if(mSimId == android.provider.Settings.System.VOICE_CALL_SIM_SETTING_INTERNET) {
			showAlwaysAskOrInternetCall(mSimId);
			return;
		}
		SIMInfo simInfo = mSIMIndicator.getCurrentSIMInfo();
		Log.i(TAG,"showSmallSIMIndicator    simInfo = "+simInfo+"   mSimIcon = "+mSimIcon);
		if(simInfo != null && mSimIcon != null){
			Log.i(TAG,"showSmallSIMIndicator    simInfo = "+simInfo+"  simInfo.mColor"+simInfo.mColor+"   mSimIcon = "+mSimIcon);
			if(simInfo.mColor >-1 && simInfo.mColor < 4)
				mSimIcon.setBackgroundResource(SmallSIMIndicatorIcons.SMALL_SIMINDICATOR[simInfo.mColor]);
			else
				mSimIcon.setBackgroundResource(SmallSIMIndicatorIcons.SMALL_SIMINDICATOR[4]);
	...
	private void showAlwaysAskOrInternetCall(long simId){
		if(simId == android.provider.Settings.System.VOICE_CALL_SIM_SETTING_INTERNET) {
			mSimIcon.setBackgroundResource(SmallSIMIndicatorIcons.SMALL_SIMINDICATOR[5]);
		} else {
			cancelAnimation();
			mSimIcon.setBackgroundResource(SmallSIMIndicatorIcons.SMALL_SIMINDICATOR[4]);
		}
	...
	private void hideSmallSIMIndicator(){
		if(mSimIcon != null) {
			mSimIcon.setVisibility(View.GONE);
			Xlog.d(TAG, "hideSmallSIMIndicator, hide SmallSIM indicator.");
		}
	}

	private void hideSimIndiatorForShortTime() {
		if (!FeatureOption.MTK_GEMINI_SUPPORT) {
            Xlog.d(TAG, "hideSIMIndicator, only gemini version support SIM indicator.");
            return;
        }
        Xlog.d(TAG, "hideSimIndiatorForShortTime, hide SIM indicator ForShortTime.");
        mSIMIndicator.setShowing(false);
    ...
    private boolean mShouldShowNotificationIcons = true;
	private final class SIMIndicatorAnimationListener implements
			Animation.AnimationListener {

		private SIMIndicatorAnimationListener() {
		}
		...
		public void onAnimationEnd(Animation animation) {
			...
			hideSimIndiatorForShortTime();
			mNotificationIcons.setVisibility(View.VISIBLE);
	        mShouldShowNotificationIcons = true;
		}
