...
public:
   ...
   {
      Parcel data, reply;
      LOGV("AutoScan_CB precent=%d, ch=%d, chnum=%d, ptr=%p, len=%d",precent,ch,chnum,ptr,len);
   ...
   {
      Parcel data, reply;
      LOGV("FullScan_CB precent=%d, freq=%d, freq_start=%d, freq_end=%d",precent,freq,freq_start,freq_end);      
      data.writeInterfaceToken(IATVCtrlClient::getInterfaceDescriptor());
      data.writeInt32(precent);
   ...
   {
      Parcel data, reply;
      LOGV("ScanFinish_CB chnum=%d",chnum);      
      data.writeInterfaceToken(IATVCtrlClient::getInterfaceDescriptor());
      data.writeInt32(chnum);        
   ...
   {
      Parcel data, reply;
      LOGV("AudioFmt_CB format=%d",format);
      data.writeInterfaceToken(IATVCtrlClient::getInterfaceDescriptor());
   ...
   {
      Parcel data, reply;
      LOGV("Shutdown_CB format=%d",source);
      data.writeInterfaceToken(IATVCtrlClient::getInterfaceDescriptor());
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
        case AUTO_SCAN_CB: {
            ...
            void *ptr = malloc(len);
            data.read(ptr,len);            
            LOGV("onTransact:AUTO_SCAN_CB precent=%d, ch=%d, chnum=%d, ptr=%p, len=%d",precent,ch,chnum,ptr,len);
        ...
        case FULL_SCAN_CB: {
            ...
            LOGV("onTransact:FULL_SCAN_CB precent=%d, freq=%d, freq_start=%d, freq_end=%d",precent,freq,freq_start,freq_end);
        ...
        case SCAN_FINISH_CB: {
            CHECK_INTERFACE(IATVCtrlClient, data, reply);
            int chnum = data.readInt32();
            LOGV("onTransact:SCAN_FINISH_CB chnum=%d",chnum);            
            ScanFinish_CB(chnum);
            return NO_ERROR;
        } break;
        case AUDIO_FORMAT_CB: {
            ...
            LOGV("onTransact:AUDIO_FORMAT_CB format=%d",format);            
            AudioFmt_CB(format);
            return NO_ERROR;
        } break;
        case SHUTDOWN_CB: {
            ...
            LOGV("onTransact:SHUTDOWN_CB source=%d",source);            
            Shutdown_CB(source);
            return NO_ERROR;
        } break;
        case NOTIFY: {
