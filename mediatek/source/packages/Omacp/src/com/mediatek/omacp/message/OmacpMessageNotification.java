...
public class OmacpMessageNotification {
	...
	private static final String XLOG = "Omacp/OmacpMessageNotification";
    ...
    private static final String[] OMACP_STATUS_PROJECTION = new String[] {
    	OmacpProviderDatabase._ID, OmacpProviderDatabase.DATE, OmacpProviderDatabase.TITLE, OmacpProviderDatabase.SUMMARY};
    ...
    private static final String NEW_INCOMING_SM_CONSTRAINT =
            "(" + OmacpProviderDatabase.SEEN + " = 0)";
    
    public static final String NOTIFICATION_ENABLED     = "pref_key_enable_notifications";
    public static final String NOTIFICATION_VIBRATE     = "pref_key_vibrate";
    public static final String NOTIFICATION_RINGTONE    = "pref_key_ringtone";

    private static final OmacpNotificationInfoComparator INFO_COMPARATOR =
            new OmacpNotificationInfoComparator();
    ...
    public static void nonBlockingUpdateNewMessageIndicator(final Context context, final boolean isNew){    	
    	if(DEBUG){
    		Xlog.d(XLOG, " OmacpMessageNotification nonBlockingUpdateNewMessageIndicator");
    	}
    ...
    private static final int accumulateNotificationInfo(
            SortedSet set, OmacpNotificationInfo info) {
        if (info != null) {
            set.add(info);
            return info.mCount;
        }
    ...
    private static final class OmacpNotificationInfoComparator
            implements Comparator<OmacpNotificationInfo> {
        public int compare(
                OmacpNotificationInfo info1, OmacpNotificationInfo info2) {
    ...
    public static final OmacpNotificationInfo getOmacpNewMessageNotificationInfo(Context context) {
        ...
        Cursor cursor = SqliteWrapper.query(context, resolver, URL_MESSAGES,
                            OMACP_STATUS_PROJECTION, NEW_INCOMING_SM_CONSTRAINT,
                            null, OmacpProviderDatabase.DATE + " desc");
        ...
        try {
            ...
            String summary = OmacpMessageUtils.getSummary(context, cursor.getString(COLUMN_SUMMARY));
            String messageType = context.getString(R.string.configuration_message);            
            
            OmacpNotificationInfo info = getNewMessageNotificationInfo(
            		context, messageId, title, summary, messageType, R.drawable.status_notify,
    ...
    private static final OmacpNotificationInfo getNewMessageNotificationInfo(
    		Context context,
    		long messageId,
    		String title,
            String summary,
...
//        Intent clickIntent = OmacpMessageList.createIntent(context, messageId);
        ...
        String senderInfo = buildTickerMessage(
                context, messageType, null).toString();
        String senderInfoName = senderInfo.substring(
                0, senderInfo.length() - 2);
        CharSequence ticker = buildTickerMessage(
                context, messageType, summary);

        return new OmacpNotificationInfo(
                clickIntent, summary, iconResourceId, ticker, timeMillis,
    ...
    private static void updateNotification(
		...
		try{
			...
			Xlog.e(XLOG, "OmacpMessageNotification NameNotFoundException.");
			return;
		}
		if(otherAppContext == null){
    		Xlog.e(XLOG, "OmacpMessageNotification otherAppContext is null.");
    		return;
    	}
		
    	SharedPreferences sp = otherAppContext.getSharedPreferences("com.android.mms_preferences", context.MODE_WORLD_READABLE);
    	if(sp == null){
    		Xlog.e(XLOG, "OmacpMessageNotification sp is null......");
    		return;
    	}
        ...
        if (isNew) {
            ...
            boolean vibrate = audioManager.getVibrateSetting(AudioManager.VIBRATE_TYPE_NOTIFICATION) == AudioManager.VIBRATE_SETTING_ON;
            if (vibrate) {
    ...
    protected static CharSequence buildTickerMessage(
            Context context, String messageType, String summary) {
