...
public class ApnEditor extends PreferenceActivity
    ...
    private final static String SS_TAG = "ssr";
    ...
    private final static String KEY_APN_TYPE_LIST = "apn_type_list";
    ...
    private final static int SOURCE_TYPE_DEFAULT = 0;
    private final static int SOURCE_TYPE_USER_EDIT = 1;
    private final static int SOURCE_TYPE_OMACP = 2;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(action.equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)) {
		        ...
		        if(Utils.isOrangeLoad()
		        		&&(ApnSettings.TETHER_TYPE.equals(mApnTypeList.getTypeString()))){
		    		mApnTypeList.setEnabled(false);
		        } 
    ...
    private void setSpn(){
        TelephonyManager telephonyManager = (TelephonyManager) getSystemService(Context.TELEPHONY_SERVICE);
        if (GEMINI_PHONE) {
            mSpn = telephonyManager.getSpNameInEfSpnGemini(mSimId);
        } else {
            mSpn = telephonyManager.getSpNameInEfSpn();
        }
        Xlog.d(TAG, "spn = " + mSpn);
    }
    ...
    private void setIMSI(){
        TelephonyManager telephonyManager = (TelephonyManager) getSystemService(Context.TELEPHONY_SERVICE);
        if (GEMINI_PHONE) {
            mIMSI = telephonyManager.isOperatorMvnoForImsiGemini(mSimId);
        } else {
            mIMSI = telephonyManager.isOperatorMvnoForImsi();
        }
    ...
    private void setScreenEnabledStatus(){
        ...
        getPreferenceScreen().setEnabled(!mReadOnlyMode && !mAirplaneModeEnabled && mNotInMmsTransaction && mDualSimMode != 0);
    }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        mSimId = intent.getIntExtra(Phone.GEMINI_SIM_ID_KEY, SIM_CARD_SINGLE);
        if(Utils.opType == Utils.OpIndex.OP_CMCC){

            if (Phone.GEMINI_SIM_2 == mSimId) {
                mProviderUri = Telephony.Carriers.SIM2Carriers.CONTENT_URI;
            } else if (Phone.GEMINI_SIM_1 == mSimId) {
                mProviderUri = Telephony.Carriers.SIM1Carriers.CONTENT_URI;
            } else {
                mProviderUri = Telephony.Carriers.CONTENT_URI;
            }
        } else {
            if (Phone.GEMINI_SIM_2 == mSimId) {
                mProviderUri = Telephony.Carriers.GeminiCarriers.CONTENT_URI;
            } else {
                mProviderUri = Telephony.Carriers.CONTENT_URI;
            }
        }
        Log.d(TAG, "mSimId: " + mSimId);
        Log.d(TAG, "mProviderUri: " + mProviderUri);
        ...
        if (action.equals(Intent.ACTION_EDIT)) {
            ...
            if (mFirstTime || icicle.getInt(SAVED_POS) == 0) {
            	...
            	if (Utils.isOrangeLoad()) {
                	...
                	String strType = intent.getStringExtra(ApnSettings.APN_TYPE);                	
                	if((strType != null) && (strType.equals(ApnSettings.TETHER_TYPE))){
                        value.put(Telephony.Carriers.TYPE, ApnSettings.TETHER_TYPE);
                	}
                    mUri = getContentResolver().insert(intent.getData(), value);
            	} else {
            	//MTK_OP03_PROTECT_END
                    mUri = getContentResolver().insert(intent.getData(), new ContentValues());
                //MTK_OP03_PROTECT_START
    ...
    public void onResume() {
        ...
        mNotInMmsTransaction = Settings.System.getInt(getContentResolver(),MMS_TRANSACTION, 0) == 0;        
        setScreenEnabledStatus();
        //MTK_OP03_PROTECT_START
        if(Utils.isOrangeLoad()
        		&&(ApnSettings.TETHER_TYPE.equals(mApnTypeList.getTypeString()))){
    ...
    private void fillUi() {
        ...
        if (mFirstTime) {
            ...
            if (mNewApn) {
                ...
                if (GEMINI_PHONE && (mSimId == Phone.GEMINI_SIM_2)) {
                    simSysProperty = TelephonyProperties.PROPERTY_ICC_OPERATOR_NUMERIC_2;
                } else {
                ...
                if(Utils.opType == Utils.OpIndex.OP_ORANGE
				    && ApnSettings.TETHER_TYPE.equals(apnType)){
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (KEY_AUTH_TYPE.equals(key)) {
            ...
            mApnTypeList.setSummary(mApnTypeList.getTypeString());        	
        }
    ...
    private boolean validateAndSave(boolean force) {
        ...
        if(FeatureOption.MTK_MVNO_SUPPORT){
            if(GEMINI_PHONE){
                isMVNO = telephonyManager.isIccCardProviderAsMvnoGemini(mSimId);
            }else{
                isMVNO = telephonyManager.isIccCardProviderAsMvno();
            }
        }
	...
	values.put(Telephony.Carriers.CARRIER_ENABLED, mCarrierEnabled.isChecked()?1:0); 
        ...
        if(mUri != null) {
            Xlog.i(TAG, "Update Apn Type: " + mApnTypeList.getTypeString());
