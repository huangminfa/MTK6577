...
#if 0
typedef kal_uint16 em_agpslog_fixed_point_t;

/* The user can define a new macro to abbreviate.
 * For example:
 * #define _F(i,f) EM_AGPSLOG_FIXED_POINT(i,f)        */
#define EM_AGPSLOG_FIXED_POINT(integer_part, fraction_part) \
    ( ((kal_uint16)(integer_part) << 8) | ((kal_uint16)(fraction_part)) )
#define EM_AGPSLOG_FIXED_POINT_INTEGER_PART(val)   ( ( (val) >> 8 ) & 0xff )
#define EM_AGPSLOG_FIXED_POINT_FRACTION_PART(val)  ( (val) & 0xff )
#endif
...
extern kal_bool em_agpslog_empty_func_returns_KAL_FALSE(void);

#define em_agpslog_init()
#define mmi_em_agpslog_init()


#define em_agpslog_start_session(session_id) em_agpslog_empty_func_returns_KAL_FALSE()
#define em_agpslog_set_active_session(session_id) em_agpslog_empty_func_returns_KAL_FALSE()
