...
public class EmailWidget implements RemoteViewsService.RemoteViewsFactory,
    ...
    public void start() {
        ...
        Logging.d(EmailWidget.TAG,"EmailWidget start() mAccountId: "+ mAccountId + ", mWidgetId: " + mWidgetId);
        synchronized(this) {
            mLoader.init();
    ...
    private static void openMessage(final Context context, final long mailboxId,
        ...
        EmailAsyncTask.runAsyncParallel(new Runnable() {
            @Override
            public void run() {
                EmailAsyncTask.printStartLog("EmailWidget#openMessage");
                Mailbox mailbox = Mailbox.restoreMailboxWithId(context, mailboxId);
                if (mailbox == null) {
                    EmailAsyncTask.printStopLog("EmailWidget#openMessage");
                    return;
                }
    ...
    private CharSequence getStyledSubjectSnippet(String subject, String snippet, boolean read) {
        ...
        if (!TextUtils.isEmpty(subject)) {
            ...
            ss.setSpan(new StyleSpan(read ? Typeface.NORMAL : Typeface.BOLD), 0, ss.length(),
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            ss.setSpan(new ForegroundColorSpan(mResource.getColor(R.color.widget_default_text_color)), 0, ss.length(),
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            ssb.append(ss);
            hasSubject = true;
        }
        if (!TextUtils.isEmpty(snippet)) {
            ...
            ss.setSpan(new ForegroundColorSpan(mResource.getColor(R.color.widget_light_text_color)), 0, snippet.length(),
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            ssb.append(ss);
        }
