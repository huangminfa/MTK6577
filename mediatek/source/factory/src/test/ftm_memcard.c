...
#if defined(MTK_EMMC_SUPPORT) && !defined(MTK_SHARED_SDCARD) 
		item(ITEM_FORMAT_EMMC_FAT,	"Format eMMC FAT (Recommend in product line)"),
#endif
...
static void mcard_update_info_multi_storage(struct mcard_array *ma, char *info,int first)
{
    ...
    ptr  = info;
		...
		if(mc_fix->id == fix_host)
			ptr += sprintf(ptr, "eMMC FAT: \n");
			ptr += sprintf(ptr, "Format Stat: %s\n", mc_fix->format_stat);
	#else
		if(mc_fix->id == fix_host)
			ptr += sprintf(ptr, "SD Card 1: \n");
	#endif
    ptr += sprintf(ptr, "Card Avail: %s\n", mc_fix->avail ? "Yes" : "No");
    ptr += sprintf(ptr, "Total Size: %d MB\n", 
        (unsigned int)(stat.f_blocks * stat.f_bsize >> 20));
	#ifndef MTK_EMMC_SUPPORT
    ptr += sprintf(ptr, "Free Size : %d MB\n", 
        (unsigned int)(stat.f_bfree * stat.f_bsize >> 20));
  #endif
    ptr += sprintf(ptr, "Checksum  : 0x%.x\n\n", mc_fix->checksum);
    ...
    /* prepare info */
	ptr += sprintf(ptr, "SD Card 2: \n");
	
    ptr += sprintf(ptr, "Card Avail: %s\n", mc_removable->avail ? "Yes" : "No");
    ptr += sprintf(ptr, "Total Size: %d MB\n", 
        (unsigned int)(stat.f_blocks * stat.f_bsize >> 20));
    ptr += sprintf(ptr, "Free Size : %d MB\n", 
        (unsigned int)(stat.f_bfree * stat.f_bsize >> 20));
    ptr += sprintf(ptr, "Checksum  : 0x%.x\n", mc_removable->checksum);
done:
    return;
}
#else
static void mcard_update_info(struct mcard *mc, char *info)
{
    ...
    ptr  = info;
	#ifdef MTK_EMMC_SUPPORT
		ptr += sprintf(ptr, "eMMC: \n");
	#else
		ptr += sprintf(ptr, "SD Card : \n");
	#endif
    ptr += sprintf(ptr, "Card Avail: %s\n", mc->avail ? "Yes" : "No");
    ptr += sprintf(ptr, "Total Size: %d MB\n", 
        (unsigned int)(stat.f_blocks * stat.f_bsize >> 20));
        ...
        switch (chosen) {
				...
				case ITEM_FORMAT_EMMC_FAT:
					...
					else if (pid == 0)  /*child process*/
					{
						...
						ma->mcard[0]->format_stat = "Formatting eMMC FAT...";
            ...
            sprintf(fat_partition_path, "/emmc@fat");
					...
					else  /*parent process*/
					{
						...
						if (WIFEXITED(child_stat) && (WEXITSTATUS(child_stat) == 0)) {
							LOGD(TAG"eMMC: pid = %d\n", pid);
							ma->mcard[0]->format_stat = "Format eMMC FAT Successfully!";
						}
						else {
							LOGD(TAG"eMMC: execl error: %s\n", strerror(errno));
							ma->mcard[0]->format_stat = "Format eMMC FAT FAILED";
						}
...
int mcard_init(void)
{
    int ret = 0;
    struct ftm_module *mod;
    struct mcard *mc = (struct mcard*)malloc(sizeof(struct mcard));
	struct mcard_array *ma = (struct mcard_array*)malloc(sizeof(struct mcard_array));
