...
public class NfcRawData extends Activity implements OnClickListener {
	...
	private void disableForTemp() {
		mRadioItems.get(0).setEnabled(false);
		mRadioItems.get(1).setEnabled(false);
	}
		...
		new Thread() {
			...
			public void run() {
				try {
					...
					NfcNativeCallClass.nfc_test_response resp =
						NfcNativeCallClass.testEntry(req);
					...
					enableAllRadioBox(true);
					threadSendMsg("Exception: " + e.getMessage());
				}
				mInStartTest = false;
			}
	...
	private NfcNativeCallClass.nfc_test_request makeRequest(boolean isStart) {
		...
		case TEST_ID_UID_RW:
			...
			nfcScriptUidReq.data[2] = 0x03;
			nfcScriptUidReq.data[3] = 0x04;
