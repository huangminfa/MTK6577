...
public class ScrollerView extends FrameLayout {
    ...
    private void initScrollView() {
        ...
        initOverScrollDistance();

        mDownCoords = new PointF();
    }

    // Use original view config to scroll tab view.
    // see OVERSCROLL_DISTANCE and OVERFLING_DISTANCE in ViewConfiguration.java 
    private void initOverScrollDistance() {
        final int OVERSCROLL_DISTANCE_ORIGINAL = 0;
        final int OVERFLING_DISTANCE_ORIGINAL = 6;
        ...
        float sizeAndDensity = 0.0f;
        if (config.isLayoutSizeAtLeast(android.content.res.Configuration.SCREENLAYOUT_SIZE_XLARGE)) {
            sizeAndDensity = density * 1.5f;
        } else {
            sizeAndDensity = density;
        }
        mOverscrollDistance = (int) (sizeAndDensity * OVERSCROLL_DISTANCE_ORIGINAL + 0.5f); 
        mOverflingDistance = (int) (sizeAndDensity * OVERFLING_DISTANCE_ORIGINAL + 0.5f);
    }
