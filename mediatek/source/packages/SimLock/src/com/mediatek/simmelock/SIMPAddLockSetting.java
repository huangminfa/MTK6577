      ...
      //  btnConfirm.setHighFocusPriority(true);
        btnConfirm.setOnClickListener(new Button.OnClickListener()
        {
        	public void onClick(View view)
        	{
        	    ...
        	    if(((false == mbIMSIReadSIM)&&(false == mbIMSIReadSIM1)&&(false == mbIMSIReadSIM1))&&((15 > etIMSI.getText().length())))
        	    {
...
//                }
                ...
                else  
                {
                    ...
                    if(false == FeatureOption.MTK_GEMINI_SUPPORT)
                    {
                    	...
                    	if (mbIMSIReadSIM == false)
                    	{
                    		phone.getIccCard().setIccNetworkLockEnabled(4,2,etPwd.getText().toString(),etIMSI.getText().toString(),null,null,callback);
                    	}
                    	else
                    	{
                    		phone.getIccCard().setIccNetworkLockEnabled(4,2,etPwd.getText().toString(),msSIMIMSI,null,null,callback);
                    	}
                    ...
                    else
                    {
                    	...
                    	if (intSIMNumber == 0)
                    	{
                    		if (mbIMSIReadSIM1 == false)
                    		{
                    			mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_1).setIccNetworkLockEnabled(4,2,etPwd.getText().toString(),etIMSI.getText().toString(),null,null,callback);
                        		//editPwdProcess(et, "12345678");       	//compare with the true password "lockPassword"	
                    		}
                    		else
                    		{
                    			mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_1).setIccNetworkLockEnabled(4,2,etPwd.getText().toString(),msSIM1IMSI,null,null,callback);
                        		//editPwdProcess(et, "12345678");       	//compare with the true password "lockPassword"	
                    		}
                    	...
                    	{
                    		if (mbIMSIReadSIM2 == false)
                    		{
                    			mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_2).setIccNetworkLockEnabled(4,2,etPwd.getText().toString(),etIMSI.getText().toString(),null,null,callback);
                        		//editPwdProcess(et, "12345678");       	//compare with the true password "lockPassword"	
                    		}
                    		else
                    		{
                    			mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_2).setIccNetworkLockEnabled(4,2,etPwd.getText().toString(),msSIM2IMSI,null,null,callback);
                        		//editPwdProcess(et, "12345678");       	//compare with the true password "lockPassword"	
                    		}               		
                    	}
    		        }                	
...
//            	Toast.makeText(this,"Fail to read SIM2 IMSI",Toast.LENGTH_LONG).show();
	...
	protected Dialog onCreateDialog(int id) {
    	...
    	case DIALOG_IMSILENGTHINCORRECT://Length is incorrect
		{
	    	...
	    	.setTitle(R.string.strAttention)
	    	.setIcon(android.R.drawable.ic_dialog_alert)
	    	.setMessage(R.string.strIMSILengthIncorrect)
	    	.setOnKeyListener(this)
	    	.setPositiveButton(R.string.strYes, new DialogInterface.OnClickListener() 
	    	{			
				public void onClick(DialogInterface dialog, int id) 
    ...
    int miSIMState = 0;
    boolean mbIMSIReadSIM1 = false;//whether to read IMSI from SIM1 card,default to set false
    boolean mbIMSIReadSIM2 = false;//whether to read IMSI from SIM2 card,default to set false
