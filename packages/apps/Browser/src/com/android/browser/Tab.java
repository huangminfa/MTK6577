...
class Tab implements PictureListener {
    ...
    private void showError(ErrorDialog errDialog) {
        if (mInForeground) {
            ...
            if (! mWebViewController.getActivity().isFinishing()) {
                d.show();
            }
        }
    }
    ...
    private final WebViewClient mWebViewClient = new WebViewClient() {
        ...
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            if (LOGD_ENABLED) {
                ...
                Log.i(LOGTAG, "["+tc.getTabPosition(Tab.this)+"/"+tc.getTabCount()+"] onPageStarted url="+url);
            }
            mInPageLoad = true;
...
//            }
            if (LOGD_ENABLED) {
                TabControl tc = mWebViewController.getTabControl();
                Log.i(LOGTAG, "["+tc.getTabPosition(Tab.this)+"/"+tc.getTabCount()+"] onPageFinished url="+url);
            }
            mInPageLoad = false;
    ...
    public void loadUrl(String url, Map<String, String> headers) {
        if (mMainView != null) {
            ...
            //M: Add for op01 test case, length limitation
            String optr = SystemProperties.get("ro.operator.optr");
            if (null != optr && optr.equals("OP01")) {
                final int nLimit = mContext.getResources().getInteger(com.mediatek.internal.R.integer.max_input_browser_search_limit);
                if(url != null && url.length() >= nLimit){
                    url = url.substring(0, nLimit);
                }
            }
