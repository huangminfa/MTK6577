    ...
    void                        *pMem)
{
    ...
    // 20121016 pc : for CMMB error bitstream, once we disable aac plus decoding by PVMP4AudioDecoderDisableAacPlus()
    //               there is no chance to enable aacplus decodeing when correct frame coming
    //               so we check if aacplus decode have ever been disabled, if yes, restore it. 
#ifndef ANDROID_DEFAULT_CODE // ALPS00364839
    if (((tDec_Int_File *)pMem)->configbak.valid)
    {
        PVMP4AudioDecoderRestoreAacPlus(pExt, pMem);
    }
#endif
    ...
    if (empty_frame == TRUE)
    {
        ...
        pVars->bno++;
#ifndef ANDROID_DEFAULT_CODE
        //resolved for ALPS00110924
		if(MP4AUDEC_INCOMPLETE_FRAME != status){
			//resolved for ALPS00068306 and ALPS00083928
	    	status = MP4AUDEC_INVALID_FRAME;
		}
#endif
        return(status);
    ...
    else
    {
        ...
        {
#ifndef ANDROID_DEFAULT_CODE	
            //for error bitstream,only config info mismatch.Resolved for ALPS00093821 and ALPS00092088
            if(status == SUCCESS){
            	PVMP4AudioDecoderDisableAacPlus(pExt, pMem);
            }else{
            status = MP4AUDEC_INVALID_FRAME;
    ...
    if (status == SUCCESS)
    {
        ...
        if (sbrBitStream->NrElements || pMC_Info->upsamplingFactor == 2)
        {
            ...
            /* reuse right aac spectrum channel  */
            {
...
#ifndef ANDROID_DEFAULT_CODE
                //resolved for ALPS00067154
                SBR_ERROR sbr_err = SBRDEC_OK;

                sbr_err = sbr_applied(sbrDecoderData,
                                    sbrBitStream,
                                    pt_left,
                                    pt_right,
                                    pExt->pOutputBuffer,
                ...
                if (sbr_err == SBRDEC_TOO_MANY_SBR_ENVELOPES)
                {
                    LOGW("AAC SBR Envelope energy check error! Disable SBR decode");
                    PVMP4AudioDecoderDisableAacPlus(pExt, pMem);
                }
                else if (sbr_err != SBRDEC_OK)
                {
                    status = MP4AUDEC_INVALID_FRAME;
                }
...
#ifndef ANDROID_DEFAULT_CODE
       if ((sbrBitStream->NrElements == 0 && pMC_Info->upsamplingFactor == 1) &&
           (!((tDec_Int_File *)pMem)->configbak.valid))
#else
        if (sbrBitStream->NrElements == 0 && pMC_Info->upsamplingFactor == 1)
#endif        
        {
...
#ifndef ANDROID_DEFAULT_CODE
// sam sun for aac seek table --->
OSCL_EXPORT_REF Int PVMP4AudioDecodeFrameTwo(
    tPVMP4AudioDecoderExternal  *pExt,
    void                        *pMem)
