      ...
      mCacheMode(cacheMode), mCacheValid(false)
{
	...
	if (mThemePath == NULL) {
	    LOGE("mThemePath allocate failed.");
	}
#ifdef THEME_NO_BUILD
    LOGV("THEME_NO_BUILD is set for host.");
#else
    LOGV("THEME_NO_BUILD is not set in device and parse here .mThemeManager =  %d.", mThemeManager);
...
{
	LOGV("AssetManager destructor called here.");
	if (mThemePath != NULL) {
		delete mThemePath;
		mThemePath = NULL;
	}
...
Asset* AssetManager::openNonAsset(void* cookie, const char* fileName, AccessMode mode)
{
	...
	LOGV("openNonAsset MTK_THEMEMANAGER_APP feature option is opened.");
	if (which < mAssetPaths.size()) {
		LOGV("Looking for non-asset '%s' in '%s',which = %d.\n", fileName,
				mThemePath->path.string(), which);
		Asset* pAsset = getAssetFromTheme(which, fileName, *mThemePath, mode);
		if (pAsset == NULL) {
			LOGV("Asset not found from theme package.");
...
#else
	LOGV("openNonAsset MTK_THEMEMANAGER_APP feature option is not opened.");
    if (which < mAssetPaths.size()) {
        LOGV("Looking for non-asset '%s' in '%s'\n", fileName,
...
char* AssetManager::getLowercaseApkName(char* str) {
	...
	LOGV("getLowercaseApkName: The strRes = %s, len = %d.\n", strRes, strlen(strRes));
	return strRes;
}
...
void AssetManager::appendApkPath(char* apkName, const char* fileName,
	...
	free(ptr1);
    LOGV("appendApkPath apkFileName Info '%s'\n", outApkFileName);
}
...
Asset* AssetManager::getAssetFromTheme(int which, const char* fileName,
	...
	if (mThemeManager != NULL && mThemeManager->isFileInMap(mAssetPaths.itemAt(which).path.string(),
			fileName)) {
		LOGV("filename mapped success which = %d, fileName = %s.\n", which,
				fileName);
...
void AssetManager::testThemeMap()
{
	...
	LOGD("openNonAsset test:ret1 = %d, ret2 = %d, ret3 = %d, ret4 = %d, ret5 = %d, ret6 = %d,"
