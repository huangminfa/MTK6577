...
M4OSA_ERR VideoEditorPreviewController::applyVideoEffect(
    ...
    LOGD ("@@ applyVideoEffect getBufferYV12 modified outBufferStride(%d)", postProcessParams.outBufferStride);
    #endif
#endif
...
M4OSA_ERR VideoEditorPreviewController::doImageRenderingMode(
    ...
    // In plane
    prepareYUV420ImagePlane(planeIn, videoWidth, videoHeight, (M4VIFI_UInt8 *)inBuffer, videoWidth, videoHeight);
    ...
    LOGD("doMediaRendering CALL getBuffer(), outputBufferWidth(%d), outputBufferHeight(%d), mRenderingMode(%d)", outputBufferWidth, outputBufferHeight, mRenderingMode);
    mTarget->getBufferYV12(&outBuffer, &outBufferStride);

    LOGD ("@@ getBufferYV12 outBufferStride(%d)", outBufferStride);
    //LOGD ("@@ mOutputVideoWidth(%d), videoWidth(%d)", mOutputVideoWidth, videoWidth);

#ifndef ANDROID_DEFAULT_CODE
    #ifdef MTK_YV12_16_STRIDE
    // Morris Yang 20120313 (force 16 alignment, due to 3D buffers for MTK video decoders are 32 alignment but NATIVE_WINDOW_API_CPU expect 16 alignment)
    outBufferStride = ((mOutputVideoWidth + 0xF) & ~0xF);
    LOGD ("@@ doImageRenderingMode getBufferYV12 modified outBufferStride(%d)", outBufferStride);
    #endif
#endif
