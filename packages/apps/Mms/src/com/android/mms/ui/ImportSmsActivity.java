...
public class ImportSmsActivity extends PreferenceActivity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        smsCategory = (PreferenceCategory) findPreference(PREF_IMPORT);
        newMainHandler();
    }
    ...
    private void getSMSFileRecursively(String path) throws IOException {
        ...
        if (files == null) {
            Xlog.w(TAG, "listFiles() returned null (directory: " + directory + ")");
    ...
    private OnPreferenceClickListener listener = new OnPreferenceClickListener(){
        ...
        public boolean onPreferenceClick(Preference arg0) {
            ...
            Xlog.d(TAG, "Click listener you choosed file "+importFileName);
            //importMessages(key);
    ...
    protected void onPrepareDialog(int id, Dialog d) { 
        if (id == IMPORT_SMS && d != null) {
            ((AlertDialog)d).setMessage(getString(R.string.whether_import_item) + " " + importFileName + "?");
        }
    }
    @Override
    ...
    private boolean importMessages(final String key){
        Xlog.d(TAG,"importMessages begin");
        if(!isSDcardReady() || key == null){
            return false;
        }
        progressdialog = ProgressDialog.show(this, "",getString(R.string.import_message_ongoing), true);
        new Thread() {
            public void run() {
                SQLiteDatabase db;
                Cursor cursor = null; 
                try {
                    ...
                    if (cursor == null) {
                        Xlog.w(TAG, "importDict sms cursor is null ");
                        mMainHandler.sendEmptyMessage(IMPORT_FAILED);
                        return;
                    }
                    int count = cursor.getCount();
                    Xlog.d(TAG, "importDict sms count = " + count);
                    if (count > 0) {
                        ...
                        Xlog.w(TAG, "importDict sms is empty");
                        return;
                    }
                    Xlog.d(TAG, "import message success");
                    if (progressdialog != null && progressdialog.isShowing()){
                    	mMainHandler.sendEmptyMessage(IMPORT_SUCCES);
                    }
                    db.close();
                } catch (SQLiteException e) {
                    Xlog.e(TAG, "can't open the database file");
                    mMainHandler.sendEmptyMessage(IMPORT_FAILED);
                } finally {
                    if (cursor != null) {
                        cursor.close();
