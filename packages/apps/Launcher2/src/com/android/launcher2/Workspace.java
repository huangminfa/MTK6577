...
public class Workspace extends SmoothPagedView
    ...
    public Workspace(Context context, AttributeSet attrs, int defStyle) {
        ...
        mUnreadMarginRight = (int)getResources().getDimension(R.dimen.hotseat_unread_margin_right);
    }
    ...
    void addInScreen(View child, long container, int screen, int x, int y, int spanX, int spanY,
        ...
        if (container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
            ...
            if (child instanceof FolderIcon) {
                ...
                ((FolderIcon) child).setFolderUnreadMarginRight(mUnreadMarginRight);
            } else if (child instanceof MTKShortcut) {
                ((MTKShortcut) child).setShortcutUnreadMarginRight(mUnreadMarginRight);
            }
    ...
    private void screenScrolledLargeUI(int screenCenter) {
        ...
        if (LauncherLog.DEBUG_DRAW) {
            LauncherLog.d(TAG, "screenScrolledLargeUI: screenCenter = " + screenCenter
    ...
    private void screenScrolledStandardUI(int screenCenter) {
        if (LauncherLog.DEBUG_DRAW) {
            LauncherLog.d(TAG, "screenScrolledStandardUI: screenCenter = " + screenCenter
                    + ",mOverScrollX = " + mOverScrollX + ",mMaxScrollX = " + mMaxScrollX
                    + ",mScrollX = " + mScrollX + ",this = " + this);
        }
    ...
    */
    ...
    public Bitmap createDragBitmap(View v, Canvas canvas, int padding) {
        ...
        if (FeatureOption.MTK_THEMEMANAGER_APP) {
            outlineColor = getResources().getThemeMainColor();
        }

        if (outlineColor == 0) {
            outlineColor = getResources().getColor(android.R.color.holo_blue_light);
        }   
        ...
        if (v instanceof TextView) {
        	...
        	try {
        		...
        		LauncherLog.d(TAG, "createDragBitmap out of memory " + e.getMessage());
        		return null;
        	}
        }
    ...
    private Bitmap createExternalDragOutline(Canvas canvas, int padding) {
        ...
        int outlineColor = 0;
        if (FeatureOption.MTK_THEMEMANAGER_APP) {
            outlineColor = r.getThemeMainColor();
        }

        if (outlineColor == 0) {
            outlineColor = r.getColor(android.R.color.holo_blue_light);
        }   
    ...
    public void beginDragShared(View child, DragSource source) {    	
        ...
        if (b == null) {
        	LauncherLog.d(TAG, "beginDragShared b == null maybe out of memory");
            // out of memory?
            return;
        }
    ...
    public void onDrop(DragObject d) {
        ...
        if (LauncherLog.DEBUG_DRAG) {
            LauncherLog.d(TAG, "onDrop 1: drag view = " + d.dragView + ",drag info = " + d.dragInfo
                    + ",drag source  = " + d.dragSource + ",dropTargetLayout = " + dropTargetLayout
    ...
    public void onDragExit(DragObject d) {
    	if (LauncherLog.DEBUG_DRAG) {
            LauncherLog.d(TAG, "onDragExit: drag source = " + d.dragSource + ",drag info = "
                    + d.dragInfo);
    	}
        doDragExit(d);
    }
   ...
   static private float squaredDistance(float[] point1, float[] point2) {
    ...
    private void onDropExternal(final int[] touchXY, final Object dragInfo,
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "onDropExternal: touchXY[0] = " + touchXY[0] + ", touchXY[1] = "
                    + touchXY[1] + ", dragInfo = " + dragInfo + ",info = " + info
                    + ", cellLayout = " + cellLayout + ", insertAtFirst = " + insertAtFirst
                    + ",drag info = " + d.dragInfo + ",screen = " + screen + ",container = "
                    + container);
        } 
                    ...
                    if (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION &&
                        ...
                        for (int k = 0; k < appCount; k++) {
                            ...
                            if (app.componentName.equals(name)) {
                                ...
                                ((MTKShortcut) view).setIcon(new FastBitmapDrawable(info
                                        .getIcon(mIconCache)));
                                }
                        }
                    }
    ...
    public void setWallpaper(String wallpaper) {
    	...
    	if (wallpaper.equals("default_wallpaper")) {
    		try {
    		Context context = mLauncher.createPackageContext("android", 0);
    		Drawable d = context.getResources().getDrawable(com.android.internal.R.drawable.default_wallpaper);
    		try {
    			mWallpaperManager.setBitmap(((BitmapDrawable)d).getBitmap());
    		} catch (IOException e) {
    			e.printStackTrace();
    		}
    		...
    		int res = resources.getIdentifier(wallpaper, "drawable", pkgName);
    		Log.d(TAG, "res = " + res);
