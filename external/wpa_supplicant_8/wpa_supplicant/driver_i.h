...
static inline int wpa_drv_set_value(struct wpa_supplicant *wpa_s, 
	                  struct wpa_driver_test_params *params)
{
	if (!wpa_s->driver->set_value)
		return -1;
	return wpa_s->driver->set_value(wpa_s->drv_priv, params);
}
