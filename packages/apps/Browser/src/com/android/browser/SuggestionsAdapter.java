...
import com.android.browser.provider.BrowserProvider2.SearchesSuggestions;
...
public class SuggestionsAdapter extends BaseAdapter implements Filterable,
    ...
    public static final int TYPE_SUGGEST_URL = 3;
    ...
    private static final String[] SEARCHES_PROJECTION = {
            SearchesSuggestions._ID,
            SearchesSuggestions.TEXT,
    };            

    private static final String COMBINED_SELECTION =
            "(url LIKE ? OR url LIKE ? OR url LIKE ? OR url LIKE ? OR title LIKE ?)";
            
    private static final String SEARCH_SELECTION = "search LIKE ?";            

    final Context mContext;
    final Filter mFilter;
    SuggestionResults mMixedResults;
    List<SuggestItem> mSuggestResults, mFilterResults, mSearchResults;
    ...
    SuggestionResults buildSuggestionResults() {
        ...
        List<SuggestItem> filter, suggest, search;
        synchronized (mResultsLock) {
            filter = mFilterResults;
            suggest = mSuggestResults;
            search = mSearchResults;
        }

        if (search != null) {
            for (SuggestItem item : search) {
    ...
    class SearchsCursor extends CursorSource {

        @Override
        public SuggestItem getItem() {
            if ((mCursor != null) && (!mCursor.isAfterLast())) {
                String title = mCursor.getString(1);
                return new SuggestItem(title, "",
                        TYPE_SEARCH);
            }
            return null;
        }
        ...
        public void runQuery(CharSequence constraint) {
            ...
            Uri.Builder ub = SearchesSuggestions.CONTENT_URI.buildUpon();
            ub.appendQueryParameter(BrowserContract.PARAM_LIMIT,
                    Integer.toString(Math.max(mLinesLandscape, mLinesPortrait)));
            mCursor = mContext.getContentResolver().query(ub.build(), SEARCHES_PROJECTION,
                            SEARCH_SELECTION, (constraint != null) ? args : null, null);
            if (mCursor != null) {
                mCursor.moveToFirst();
            }
        }
