...
public class MtkNfcEnabler implements Preference.OnPreferenceChangeListener,
    ...
    public MtkNfcEnabler(Context context, NfcPreference switchpref,
            PreferenceScreen androidBeam, Switch switchButton, NfcAdapter adapter) {
        mContext = context;
    ...
    private void handleNfcStateChanged(int newState) {
        ...
        updateAndroidBeamPref(newState);
    }
    ...
    /**
     * update the android beam pref according to the NFC state
     * @param state the current nfc adapter state
     */
    private void updateAndroidBeamPref(int state) {
        if (mAndroidBeam == null) {
            return;
        }
        switch (state) {
