...
public class NfcSoftwareStackSecure extends Activity implements OnClickListener {
				...
				if (NfcCommonDef.ERROR_OK == setSeResult) {
					...
					Toast
						.makeText(getApplicationContext(),
							"Response status is: " + setSeResult,
	...
	private int setSEOption(int seIdx) throws Exception {
		...
		try {
			...
			Elog.e(TAG, "NfcNativeCallClass.setSEOption(req) EXP!"
				+ e.getMessage());
			throw e;
		}
	...
	private void initUI() {
		...
		for (ModeMap m : displayChkBoxArray) {
	...
	private ArrayList<ModeMap> displayChkBoxArray = new ArrayList<ModeMap>();
