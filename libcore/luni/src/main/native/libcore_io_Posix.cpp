...
static void Posix_connect(JNIEnv* env, jobject, jobject javaFd, jobject javaAddress, jint port) {
    ...
    //add by mtk80538 for debug 
	{
      ...
      char cmdline[50]; 
        ...
        if(fd1 == 0) {
            r = 0;
			LOGE("[Posix_connect Debug]open file fail:  %s",cmdline); 
        } else {
			...
			{
              r = 0;
			  LOGE("[Posix_connect Debug]read file fail: %s",cmdline);
			}
        }
		...
		LOGD("[Posix_connect Debug]Process %s :%d ",cmdline, (int)port);
	}
	//add by mtk80538 for debug  END
}
