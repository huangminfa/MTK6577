...
public class CdsPdpActivity extends Activity implements View.OnClickListener {
    private final String TAG = "CdsPdpActivity";
    private Context mContext;    
    private final String PREFIX_APN = "enable";
    String[] apnList = new String[]{"MMS", "SUPL", "WAP", "HIPRI"};
    int[] apnTypeList = new int[]{ConnectivityManager.TYPE_MOBILE_MMS, ConnectivityManager.TYPE_MOBILE_SUPL, ConnectivityManager.TYPE_MOBILE_WAP, ConnectivityManager.TYPE_MOBILE_HIPRI};
    int    selectApnType = ConnectivityManager.TYPE_MOBILE_MMS;
    
    private Toast mToast;
    
    Spinner mApnSpinner = null;
    private ConnectivityManager mConnMgr = null;
    private static final int MSG_KEEP_CONN = 0x3001;
    private String selectApnFeature = Phone.FEATURE_ENABLE_MMS;
    private int selectApnPos = 0;
    ...
    private IntentFilter connFilter = null;
    ...
    public void onCreate(Bundle icicle) {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,android.R.layout.simple_spinner_item, apnList);
        ...
        mApnSpinner.setOnItemSelectedListener(new Spinner.OnItemSelectedListener() {
            ...
            int position, long arg3) {
                ...
                try{
                    selectApnFeature = PREFIX_APN + adapterView.getSelectedItem().toString();
                    selectApnPos     = position;
                    
                    mApnSpinner.requestFocus();
                    updateConnectButton();
                    
                }catch(Exception e){
                    selectApnFeature = Phone.FEATURE_ENABLE_MMS;
                    e.printStackTrace();
                }
            }
            @Override
    ...
    private void handleStopAPN(){
        ...
        if(FeatureOption.MTK_GEMINI_SUPPORT){
            mConnMgr.stopUsingNetworkFeatureGemini(ConnectivityManager.TYPE_MOBILE, selectApnFeature, getDataConnectionFromSetting());
        }else{
            mConnMgr.stopUsingNetworkFeature(ConnectivityManager.TYPE_MOBILE, selectApnFeature);
        }
        
        Xlog.i(TAG, "[Feature][Stop]" + selectApnFeature);
    }    
    ...
    private void handleStartAPN(){
        ...
        if(FeatureOption.MTK_GEMINI_SUPPORT){
            mConnMgr.startUsingNetworkFeatureGemini(ConnectivityManager.TYPE_MOBILE, selectApnFeature, getDataConnectionFromSetting());
        }else{
            mConnMgr.startUsingNetworkFeature(ConnectivityManager.TYPE_MOBILE, selectApnFeature);
        }
                                                
        Xlog.i(TAG, "[Feature][Start]" + selectApnFeature);
    ...
    private void updateConnectButton(){
        ...
        nwInfo = mConnMgr.getNetworkInfo(apnTypeList[selectApnPos]);
    ...
    private void handleAddIPAddressToRoute(){
        ...
        if(nwInfo != null && nwInfo.getState() == NetworkInfo.State.CONNECTED){
            ...
            try{
                ...
                mToast.setText("Add host:" + ipAddress + " to " + apnList[selectApnPos]);
                mToast.show();
                
                mConnMgr.requestRouteToHost(apnTypeList[selectApnPos], addr);
            ...
            mToast.setText("The connection(" + apnList[selectApnPos] + ") is not connected");
            mToast.show();
        }
    ...
    private BroadcastReceiver mNetworkConnReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            if(intent.getAction().equals(ConnectivityManager.CONNECTIVITY_ACTION)) {
                ...
                if(type == apnTypeList[selectApnPos]){
    ...
    private void updateApnStatus(){
        ...
        for(int i =0; i < apnTypeList.length; i++){
            nwInfo = mConnMgr.getNetworkInfo(apnTypeList[i]);            
            sb = nwInfo.toString() + "\r\n\r\n";
            tb.append(sb.replace(',', '\n'));
            
            if(nwInfo.getState() == NetworkInfo.State.CONNECTED){                
                nwLink = mConnMgr.getLinkProperties(apnTypeList[i]);
                sb = nwLink.toString() + "\r\n\r\n";
                tb.append(sb.replace(',', '\n'));
            }
        }
