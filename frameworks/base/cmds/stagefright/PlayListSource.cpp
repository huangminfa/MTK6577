...
//    LOGD("@debug: (eos = %d) to readbytes = %d", (int)eos, readbytes);

    ssize_t n = ::read(mFd, mem->pointer(), readbytes);
    mLeftInFile -= n;
    LOGD("@debug: read %d, curPos = %lld,  left = %lld", n, lseek64(mFd, 0, SEEK_CUR), mLeftInFile);

    if (eos) {
        LOGD("read EOS, mark discontinuity");
