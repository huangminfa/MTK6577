...
status_t SurfaceMediaSource::setBufferCount(int bufferCount) {
    LOGD("SurfaceMediaSource::setBufferCount (%d)", bufferCount);
    if (bufferCount > NUM_BUFFER_SLOTS) {
...
status_t SurfaceMediaSource::requestBuffer(int slot, sp<GraphicBuffer>* buf) {
    LOGD("SurfaceMediaSource::requestBuffer slot(%d), mBufferCount(%d)", slot, mBufferCount);
    Mutex::Autolock lock(mMutex);
...
void SurfaceMediaSource::passMetadataBufferLocked(MediaBuffer **buffer) {
    ...
    *buffer = tempBuffer;
    ...
    LOGD("[SMS] handle = %p, data = 0x%08X, offset = %d, length = %d",
            mCurrentBuf->handle, tempBuffer->data(), (*buffer)->range_offset(), (*buffer)->range_length());
}
