...
static item_t gyro_cali_items[] = {
    ...
    item(ITEM_DO20,   "Do Calibration(20)"),
    item(ITEM_DO40,   "Do Calibration(40)"),
    item(ITEM_EXIT,   "Exit"),
    item(-1, NULL),
};
/*---------------------------------------------------------------------------*/
...
static void *gyro_cali_update_iv_thread(void *priv)
{
	...
	while(1)
	{
		...
		if(op == GYRO_OP_CLEAR)
		{
			...
			if(err)
			{
				snprintf(dat->gyroc.status, sizeof(dat->gyroc.status), "Clear fail(%d)\n", err);
			}
			else
			{
				snprintf(dat->gyroc.status, sizeof(dat->gyroc.status), "Clear okay\n");
			}
		...
		else if(op == GYRO_OP_CALI_PRE)
		{
			err = 0;
			/*by-pass*/
			snprintf(dat->gyroc.status, sizeof(dat->gyroc.status), "Calibrating: don't touch\n");            
			pthread_mutex_lock(&dat->gyroc.evtmutex);
		...
		else if(op == GYRO_OP_CALI)
		{
			...
			if(err)
			{
				len = snprintf(dat->gyroc.status, sizeof(dat->gyroc.status), "Calibrating: fail\n");  
				dat->mod->test_result = FTM_TEST_FAIL;
			}
			else
			{
				len = snprintf(dat->gyroc.status, sizeof(dat->gyroc.status), "Calibrating: okay\n");
