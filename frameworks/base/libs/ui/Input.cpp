...
VelocityTracker::VelocityTracker() { 
    ...
    if (fd < 0) 
	{
            LOGD("Couldn't open '%s' (%s)", TPD_DEV, strerror(errno));
    }
    ...
    if (err) 
	{
       LOGD("tpd read x fail: %s", strerror(errno));
	   m_velocity_magnify_x =1;
        
    }
	else
	{
      LOGD("VelocityTracker: int datax = %d\n",datax);   
      m_velocity_magnify_x =(float)datax/10;
	  LOGD("VelocityTracker: int m_velocity_magnify_x = %f\n",m_velocity_magnify_x);   
	}
	
	err = ioctl(fd, TPD_GET_VELOCITY_CUSTOM_Y, &datay);       
    if (err) 
	{
       LOGD("tpd read y fail: %s", strerror(errno));
	   m_velocity_magnify_y =1;
        
    }
	else
	{
      LOGD("VelocityTracker: int datay = %d\n",datay);   
      m_velocity_magnify_y =(float)datay/10;
	  LOGD("VelocityTracker: int m_velocity_magnify_y = %f\n",m_velocity_magnify_y);  
	}
...
bool VelocityTracker::getVelocity(uint32_t id, float* outVx, float* outVy) const {
    ...
    if (getEstimator(id, DEFAULT_DEGREE, DEFAULT_HORIZON, &estimator)) {
		...
		{
			if(estimator.xCoeff[1]>=2000.0f&&estimator.xCoeff[1]<=2001.0f 
				|| estimator.xCoeff[1]>=20000.0f&&estimator.xCoeff[1]<=20001.0f
				|| estimator.xCoeff[1]>=824.0f&&estimator.xCoeff[1]<=825.0f
				|| estimator.xCoeff[1]>=850.0f&&estimator.xCoeff[1]<=851.0f )
			{
			  *outVx = estimator.xCoeff[1] ;
               *outVy = estimator.yCoeff[1] ;
			   LOGD(" android  outVx=%f", *outVx);
			   LOGD(" android  outVy=%f", *outVy);
			}
			else
			{
			   *outVx = estimator.xCoeff[1] * m_velocity_magnify_x;
               *outVy = estimator.yCoeff[1] * m_velocity_magnify_y;
			   LOGD(" fwq outVx=%f", *outVx);
			   LOGD(" fwq outVy=%f", *outVy); 
...
#endif
//fix bug
            LOGD("xvelocity before =%f",outEstimator->xCoeff[1]);
			LOGD("yvelocity before =%f",outEstimator->yCoeff[1]);
			
            if((x[m-1]-x[0])>0)
            {
              //Xvolocity  < 0 from right to left
              LOGD("1 x[m-1]=%f",x[m-1]);
			  LOGD("1 x[0]=%f",x[0]);
              LOGD("1 x[m-1]-x[0]=%f",(x[m-1]-x[0]));
			...
			if((x[m-1]-x[0])<0) 
			{
			  ...
			  LOGD("2 x[m-1]=%f",x[m-1]);
			  LOGD("2 x[0]=%f",x[0]);
			  LOGD("2 x[m-1]-x[0]=%f",(x[m-1]-x[0]));
			...
			if((y[m-1]-y[0])>0)
            {
              ...
              LOGD("1 y[m-1]=%f",y[m-1]);
			  LOGD("1 y[0]=%f",y[0]);
              LOGD("1 y[m-1]-y[0]=%f",(y[m-1]-y[0]));
			...
			if((y[m-1]-y[0])<0)
			{
			  ...
			  LOGD("2 y[m-1]=%f",y[m-1]);
			  LOGD("2 y[0]=%f",y[0]);
			  LOGD("2 y[m-1]-y[0]=%f",(y[m-1]-y[0]));
			...
			LOGD("xvelocity=%f",outEstimator->xCoeff[1]);
			LOGD("yvelocity=%f",outEstimator->yCoeff[1]);
//end
            return true;
        }
    }
