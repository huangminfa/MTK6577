...
void Layer::setGeometry(hwc_layer_t* hwcl)
{
    ...
    if ((s.alpha < 0xFF) || (true == isProtected())) {    // [mtk03712] skip from overlay when protected
        hwcl->flags = HWC_SKIP_LAYER;
    }
...
#ifdef ADVANCED_HWC
    // let HWC to decide if it can do arbitrary transformation
    ...
    // [mtk03712] pass info for mtk hwcomposer
    //----------------------------------------------------------------------------------------------
...
void Layer::setPerFrameData(hwc_layer_t* hwcl) {
    ...
    if (buffer == NULL) {
        ...
        hwcl->s3dBufferFlags = mDrawingState.s3d_buffer_flags;
		hwcl->s3dManualOffset = mSurfaceTexture->getS3DOffset();
...
void Layer::onDraw(const Region& clip) const
{
    if (CC_UNLIKELY(mActiveBuffer == 0)) {
        ...
        if (!holes.isEmpty()) {
            XLOGI("!!! [%s] clearScreen holes ..(%d %d - %d %d) !!!",
                getName().string(), holes.getBounds().left, holes.getBounds().top, holes.getBounds().width(), holes.getBounds().height());
    ...
    if (!isProtected()) {
        ...
        if (true == mSurfaceTexture->isAuxSlotDirty()) {    // bind to aux buffer if converted successfully
            mSurfaceTexture->bindToAuxSlotLocked();
        } else {    // bind to original buffer
            //LOGW_IF(mSurfaceTexture->isFormatNeedConversion(), "%s texture format needs conversion", getName().string());
        ...
        if (getFiltering() || needsFiltering() || isFixedSize() || isCropped() ||
            (SurfaceFlinger::State::eComposing2D != mFlinger->mDrawingState.composingPhase)) {     // [mtk03712] for S3D rendering
...
void Layer::lockPageFlip(bool& recomputeVisibleRegions)
{
    if (mQueuedFrames > 0) {
        ...
        // [mtk03712] for more layer status
        //--------------------------------------------------------------------------------------
...
void Layer::dump(String8& result, char* buffer, size_t SIZE) const
{
    ...
    if (-1 == layerdump || identity == (uint32_t)layerdump) {   // dump given identity, or -1 to dump all
        char             fname[128];
        void*            ptr;
        SkBitmap         b;
        SkBitmap::Config c;
        ...
        switch (buf0->format) {
            ...
            case 0x1ff:                     // tricky format for BGRX_8888 in fact
                if (0 == raw) {
            ...
            case HAL_PIXEL_FORMAT_YV12:
                ...
                break;
            default:
                XLOGE("[%s] cannot dump format:%d for identity:%d", __func__, buf0->format, identity);
                return;
        }

        buf0->lock(GraphicBuffer::USAGE_SW_READ_OFTEN, &ptr);
        {
            XLOGI("*** [Layer] %s", getName().string());
