...
public class SlotView extends GLView {
    ...
    public boolean setSlotCount(int slotCount) {
        ...
        // as current scroll position may be invalid after slot count
        // changed, we need to verify the scroll position first.
        int scrollLimit = mLayout.getScrollLimit();
        if (WIDE) {
            mScrollX = Utils.clamp(mScrollX, 0, scrollLimit);
        } else {
            mScrollY = Utils.clamp(mScrollY, 0, scrollLimit);
        }
        // update scroller's position also.
        int position = mScroller.getPosition();
        position = Utils.clamp(position, 0, scrollLimit);
        mScroller.setPosition(position);
