...
public class HorizontalScrollView extends FrameLayout {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mIsBeingDragged) {
                    ...
                    int realDeltaX = calcRealOverScrollDist(deltaX);
                    mLastMotionX = x;
                    ...
                    if (overScrollBy(realDeltaX, 0, mScrollX, 0, range, 0,
                    ...
                    if (canOverscroll) {
                        final int pulledToX = oldX + realDeltaX;
                        if (pulledToX < 0) {
                            mEdgeGlowLeft.onPull((float) realDeltaX / getWidth());
                            if (!mEdgeGlowRight.isFinished()) {
                                mEdgeGlowRight.onRelease();
                            }
                        } else if (pulledToX > range) {
                            mEdgeGlowRight.onPull((float) realDeltaX / getWidth());
    ...
    private int calcRealOverScrollDist(int dist) {
        if (mScrollX <= 0) {
            return (int)(dist * (1 - Math.abs(mScrollX) * 1.0f / mOverscrollDistance));
        } else if (mScrollX >= getScrollRange()){
            return (int)(dist * (1 - Math.abs(mScrollX - getScrollRange()) * 1.0f / mOverscrollDistance));
        } else {
            return dist;
        }
