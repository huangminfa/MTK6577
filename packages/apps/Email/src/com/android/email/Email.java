...
public class Email extends Application {
    ...
    //Common max input text length, e.g. 'Your Name','Account Name', 'Subject','User name','Password'
    ...
    // Orange IMAP server address
    private static ArrayList<String> sDraftSyncImapServer = new ArrayList<String>();
    ...
    public void onCreate() {
        ...
        // Read need draft sync server address from raw resouce file
        initDraftSyncImapServer();
    }
    ...
    private void initDraftSyncImapServer() {
        String optr = android.os.SystemProperties.get("ro.operator.optr");
        if (null != optr && optr.equals("OP03")) {
            InputStream is = null;
            String serverName = null;
            try {
                is = getResources().openRawResource(R.raw.orangeimapserver);
                BufferedReader br = new BufferedReader(new InputStreamReader(is));
                if (br != null) {
                    while((serverName = br.readLine()) != null) {
                    sDraftSyncImapServer.add(serverName);
                    }
                }
            } catch (IOException e) {
            } finally {
    ...
    /**
     * Check if the address were need draft sync server
     * @param context
     * @param address
     * @return true if this address were Orange IMAP server
     */
    public static boolean isDraftSyncAccount(String address) {
        if (address != null) {
            for (String server : sDraftSyncImapServer) {
                if (server.equalsIgnoreCase(address)) {
                    return true;
                }
            }
        }
