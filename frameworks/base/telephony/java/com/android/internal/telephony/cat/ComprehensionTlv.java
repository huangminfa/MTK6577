...
class ComprehensionTlv {
	...
	throws ResultException {
		...
		while (startIndex < endIndex) {
			...
			//ICS Migration start
			if(ctlv == null) {
				CatLog.d("CAT", "CT-decodeMany: ingore invalid byte");
				startIndex += 1;
			} else {
	...
	public static ComprehensionTlv decode(byte[] data, int startIndex) throws ResultException 
	{
		try {
			...
			case 0x80:
				//Add by Huibin Mao Mtk80229
				//ICS Migration start
				//if(isSetUpMenu && hasCommandDetails && hasDeviceIdentifier && hasMenuItem) {
				CatLog.d("CAT", "CT-decode: ignore invalid data first, we will check it later");
				return null;
				//}
