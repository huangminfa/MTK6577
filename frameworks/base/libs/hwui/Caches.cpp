...
bool Caches::dumpActiveDisplayLists(DisplayList *displayList) {
    ...
    LOGW("Active DisplayLists: %d (%lld)", activeDisplayLists.size(), systemTime(SYSTEM_TIME_MONOTONIC));
    bool isFound = false;
    DisplayList *item;
    for (size_t i = 0; i < activeDisplayLists.size(); i++)
    {
        item = activeDisplayLists.itemAt(i);
        LOGW("\t[%d] %p, tid=%d, time=%lld", i, item, item->mTid, item->mTimestamp);
        if (item == displayList)
            isFound = true;            
    }
    return isFound;
...
bool Caches::dumpActiveLayers(Layer *layer) {
    ...
    LOGW("Active Layers: %d (%lld)", activeLayers.size(), systemTime(SYSTEM_TIME_MONOTONIC));
    bool isFound = false;
    Layer *item;
