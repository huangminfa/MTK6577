    ...
    Res_value* outValue)
{
    ...
    if (e != NULL) {
        ...
        while (pos < end && !failed) {
            ...
            if(( start - name ) > nameLen ) {
                 printf("AAPT-- exceed original length, skip, len %d; nameLen is %d \n", (start - name ), nameLen);
                 failed = true;
                 break;
            }
            end++;
            ...
            if (i >= N) {
                ...
                printf("i>N! getAttributeFlags #%08x %s\n", attrID, String8(name, nameLen).string());
				printf("i>N! getAttributeFlags %s \n", String8(name+nameLen+1, nameLen+20).string());
				printf("i>N! getAttributeFlags not find flag identifier=%s,namelen=%d;pos %c,end=%c\n", String8(nameStr).string(),nameLen, *pos,*end);
                printf("i>N! bag as follows\n");
                for(i=0;i<N;i++){
					printf("i>N! Flags= %s\n",String8(e->getBag().keyAt(i)).string());
				}
