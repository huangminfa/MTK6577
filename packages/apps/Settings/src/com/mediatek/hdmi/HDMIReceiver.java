...
public class HDMIReceiver extends BroadcastReceiver{
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(Intent.ACTION_BOOT_COMPLETED.equals(action) || "android.intent.action.ACTION_BOOT_IPO".equals(action)){
            Log.i(TAG, "HDMIReceiver >> get boot/IPO completed broadcast, start HDMI local service");
            Log.d(TAG, "  Bootup action="+action);
            serviceIntent.putExtra("bootup_action", action);
            context.startService(serviceIntent);
        }else if(ACTION_IPO_SHUTDOWN.equals(action)){
            Log.i(TAG, "HDMIReceiver >> IPO shut down, stop HDMI local service, have no effect now");
            //keep this service live even when IPO shut down, since HDMI driver will not die
