...
static void itemview_start_menu(struct itemview *iv, int item_sel)
{
    ...
    /* title text */
    if (title) {
        strncpy(iv->menu[i], title->string, text_cols-1);
        iv->menu[i][text_cols-1] = '\0';
        i++;
    }
    top = i;
    ...
    for (; i < MAX_ROWS; ++i) {
        ...
        strncpy(iv->menu[i], items[i - top].name, text_cols-1);
        iv->menu[i][text_cols-1] = '\0';
    }
...
int paintview_redraw(struct paintview *pv)
{
    ...
    if (pv->m_left_btn || pv->m_center_btn || pv->m_right_btn) {
        ...
        if (pv->m_left_btn) {
            ...
            x = 1 * s - (strlen(pv->m_left_btn->string) >> 1) * CHAR_WIDTH;
            gr_text(x, y, pv->m_left_btn->string);
        }
        if (pv->m_center_btn) {
            set_gr_color(pv->m_center_btn->color);
            x = 3 * s - (strlen(pv->m_center_btn->string) >> 1) * CHAR_WIDTH;
            gr_text(x, y, pv->m_center_btn->string);
        }
        if (pv->m_right_btn) {
            set_gr_color(pv->m_right_btn->color);
            x = 5 * s - (strlen(pv->m_right_btn->string) >> 1) * CHAR_WIDTH;
