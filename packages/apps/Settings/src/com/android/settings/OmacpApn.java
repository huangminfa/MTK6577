...
public class OmacpApn {
    ...
    public long insert(final Context context, ContentValues values) {
        ...
        if (FeatureOption.MTK_MVNO_SUPPORT && isMVNO) {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                spn = mTelephonyManager.getSpNameInEfSpnGemini(mSimId);
                imsi = mTelephonyManager.isOperatorMvnoForImsiGemini(mSimId);
            } else {
                spn = mTelephonyManager.getSpNameInEfSpn();
                imsi = mTelephonyManager.isOperatorMvnoForImsi();
            }
            Log.d(TAG, "spn = " + spn);
            Log.d(TAG, "imsi = " + imsi);
