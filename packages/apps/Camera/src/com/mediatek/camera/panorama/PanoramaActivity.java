 ...
 */
public class PanoramaActivity extends ActivityBase implements FocusManager.Listener,
        ModePicker.OnModeChangeListener,ShutterButton.OnShutterButtonListener,
        SurfaceHolder.Callback,PanoramaController.CaptureEventListener,
    ...
    private static final int MSG_3D_SWITCH_TEST = 100;
    ...
    private RotateImageView mPanorama3DSwitch;
    protected RotateImageView mDoneButton;
    ...
    private RotateLayout mDoneGroup;
                ...
                switch (msg.what) {
                    ...
                    case MSG_FINAL_IMAGE_READY:
                        ...
                        if (mCameraState != SNAPSHOT_IN_PROGRESS) {
                        	hidePost3DAutoRamaControlAlert();
                        }
                    ...
                    case MSG_GET_THUMBNAIL_DONE:
						...
						updateThumbnailButton();
                        break;
                    case MSG_3D_SWITCH_TEST:
                    	showPost3DAutoRamControlAlert();
                    	break;
                }
            }
    };	
    ...
    private void setOrientationIndicator(int degree) {
        ...
        if (Util.getS3DMode() && degree != 270) return;
        mPanoramaController.setOrientation(degree);
        if (mZoomControl != null)
            mZoomControl.setOrientation(degree);
        if (mOnScreenDisplayLayout != null)
            mOnScreenDisplayLayout.setOrientation(degree);
        if (mRotateTextToast != null) mRotateTextToast.setOrientation(degree);
        mDoneGroup.setOrientation(degree);
        mCancelGroup.setOrientation(degree);
        if (mOnScreenProgress != null)
            mOnScreenProgress.setOrientation(degree);
    }
	...
	public void onMergeStarted() {
		if (!mPausing) {
			...
			mPanoramaController.hidePanoramaUI();
		...
		if (isMerge) {
			new Thread() {
				...
				public void run() {
                    ...
                    int pictureType = Util.getS3DMode() ? Storage.PICTURE_TYPE_MPO_3D : Storage.PICTURE_TYPE_JPG;
                    final String fpath = Storage.generateFilepath(name, pictureType);
					...
					Log.d(TAG, "onCaptureDone.run orientation " + orientation + 
							" w * h: " + width + " * " + height);
					Uri uri = Storage.addImage(getContentResolver(), name, mTimeTaken, null,
					        orientation, width, height, pictureType);
					Thumbnail tb = null;									
					if (uri != null) {
    ...
    public void onCreate(Bundle icicle) {
		...
		mPanoramaController = new PanoramaController(this,this);
		mAlertControlBar = (ViewGroup) findViewById(R.id.review_control_bar);
		mDoneGroup = (RotateLayout)findViewById(R.id.review_done_group);
	    mCancelGroup = (RotateLayout)findViewById(R.id.review_cancel_group);
    ...
    private void openCamera() throws CameraHardwareException, CameraDisabledException {
		...
		mPanoramaController.setCamera(mCameraDevice);
    }
	...
	private void initOnScreenIndicator() {
        ...
        if (!CameraHolder.instance().getStereo3DSupport() && !supportPanorama3dSwitch()) {
            mRemainPictureView.setVisibility(View.VISIBLE);
        }
        if (supportPanorama3dSwitch() && mPanorama3DSwitch != null) {
            setCaptureMode();
            updatePanorama3DSwitch();
            mPanorama3DSwitch.setVisibility(View.VISIBLE);
        }
	}
	...
	public void setCapturePath() {
		...
		final String name = createName(mNameFormat,mTimeTaken);
        final String fpath = Storage.generateFilepath(name, 
                Util.getS3DMode() ? Storage.PICTURE_TYPE_MPO_3D : Storage.PICTURE_TYPE_JPG);
    ...
    public boolean capture() {
		...
		if (!mPanoramaController.start()) {//it is still busy.
            return false;
		}

		showPost3DAutoRamControlAlert();
		mRotateTextToast.changeTextContent(getGuideString(GUIDE_MOVE));
    ...
    private void stopCapture(boolean isMerge) {		
		...
		if (!mPanoramaController.hasCaptured()) {
			isMerge = false;
		}
		resetCapture(!isMerge);
		mPanoramaController.stop(isMerge);
    }	
    ...
    private void enableCameraControls(boolean enable) {
        ...
        if (mPanorama3DSwitch != null) mPanorama3DSwitch.setEnabled(enable);
    }

	private long pictureSize() {
        return Storage.getSize("autorama");
	}
    ...
    private void createContentView() {
        ...
        mPanorama3DSwitch = (RotateImageView) findViewById(R.id.panorama3D_switch);
        mDoneButton = (RotateImageView) findViewById(R.id.btn_done_pano);
        mCancelButton = (RotateImageView) findViewById(R.id.btn_cancel_pano);
	...
	private boolean initializeZoomMax(Parameters parameters) {
		...
		Log.d(TAG, "initializeZoomMax zoom max = " + mZoomMax);
	...
	private void safeStop(){
		...
		mPanoramaController.checkStopProcess();
		synchronized(holder) {
			stopPreview();
		}
    ...
    protected void doOnResume() {
        ...
        //mHandler.sendEmptyMessageDelayed(MSG_3D_SWITCH_TEST, 2000);
    }
    ...
    private void setDisplayOrientation() {
		...
		mPanoramaController.setDisplayOrientation(orientation);
		Log.d(TAG, "setDisplayOrientation orientation = "+orientation);		
    }
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_MEDIA_MOUNTED)
                ...
                Storage.updateDefaultDirectory(PanoramaActivity.this,true);
                checkStorage();
				updateThumbnail();
    ...
    private void initializeFirstTime() {
        ...
        if (mFirstTimeInitialized || mPausing) return;
		...
		mRotateTextToast = new RotateTextToast(this,getGuideString(GUIDE_SHUTTER)
				,0,(ViewGroup) findViewById(R.id.guide_text));
	...
	private void showCaptureError(){
	    mRotateDialog.dismissDialog();
	    if (!mPausing) {
	        final String dialogTitle = getResources().getString(R.string.pano_dialog_title);
	...
	private class ZoomChangeListener implements 
		...
		public void onZoomValueChanged(int index) {
			PanoramaActivity.this.onZoomValueChanged(index);
		}
		...
		public void onZoomStateChanged(int state) {
			...
			if (state == ZoomControl.ZOOM_IN) {
				PanoramaActivity.this.onZoomValueChanged(mZoomMax);
			} else if (state == ZoomControl.ZOOM_OUT) {
				PanoramaActivity.this.onZoomValueChanged(0);
			} else {
				mTargetZoomValue = -1;
				if (mZoomState == ZOOM_START) {
					mZoomState = ZOOM_STOPPING;
	...
	public void showPost3DAutoRamControlAlert() {
		Util.fadeOut(mShutterButton, mThumbnailView,
		        mPanorama3DSwitch, mZoomControl, mRemainPictureView);
        ...
        mDoneButton.setEnabled(!Util.getS3DMode());
        mCancelButton.setEnabled(true);
	}

	public void hidePost3DAutoRamaControlAlert() {
		Util.fadeOut(mAlertControlBar);
		mAlertControlBar.setEnabled(false);

		Util.fadeIn(mShutterButton, mThumbnailView, mModePicker, mZoomControl);
		updateIndicatorVisibility();
        if (mRotateTextToast != null) mRotateTextToast.changeTextContent(getGuideString(GUIDE_SHUTTER));
    ...
    public boolean supportPanorama3dSwitch() {
        if (mParameters == null) return FeatureOption.MTK_S3D_SUPPORT;
        ...
        if (FeatureOption.MTK_S3D_SUPPORT && supportCaptureModes != null && 
                supportCaptureModes.indexOf(Parameters.CAPTURE_MODE_PANORAMA3D) != -1) {
            return true;
        }
        return false;
    }
    ...
    @OnClickAttr
    public void onPanorama3DClicked(View v) {
        if (isCameraIdle() && mPanorama3DSwitch != null) {
            Util.switch3DMode();
            setCaptureMode();
            updatePanorama3DSwitch();
            mRotateTextToast.changeTextContent(getGuideString(GUIDE_SHUTTER));
        }
    }

    private void setCaptureMode() {
        mParameters.setCaptureMode(Util.getS3DMode()
                ? Parameters.CAPTURE_MODE_PANORAMA3D
                : Parameters.CAPTURE_MODE_NORMAL);
        mCameraDevice.setParameters(mParameters);
        mParameters = mCameraDevice.getParameters();
    }

    private void updatePanorama3DSwitch() {
        Util.setModeSupport(mParameters);
        mModePicker.setModeSupport();
        if (Util.getS3DMode()) {
            mPanorama3DSwitch.setImageResource(R.drawable.btn_ic_3d_switch_off);
            if (mZoomControl != null) mZoomControl.setOrientation(270);
            if (mOnScreenDisplayLayout != null)
                mOnScreenDisplayLayout.setOrientation(270);
            if (mRotateTextToast != null) mRotateTextToast.setOrientation(270);
            if (mDoneGroup != null) mDoneGroup.setOrientation(270);
            if (mCancelGroup != null) mCancelGroup.setOrientation(270);
            mPanoramaController.setOrientation(270);
        } else {
            mPanorama3DSwitch.setImageResource(R.drawable.btn_ic_3d_switch_on);
            setOrientationIndicator(mOrientationCompensation);
        }
        mPanorama3DSwitch.requestLayout();
    }

	@OnClickAttr
	public void onReviewDoneClicked(View v) {
		Log.i(TAG, "onReviewDoneClicked");
		mPanoramaController.mOkOnClickListener.onClick(null);
	}

	@OnClickAttr
	public void onReviewCancelClicked(View v) {
		Log.i(TAG, "onReviewCancelClicked");
		mPanoramaController.mCancelOnClickListener.onClick(null);
		hidePost3DAutoRamaControlAlert();
	}

	public void movingTips() {
		mRotateTextToast.changeTextContent(getGuideString(GUIDE_CAPTURE));
	}

	public void resetControlBar() {
		mAlertControlBar.setVisibility(View.GONE);
	...
	private int getGuideString(int step) {
		int guideId = 0;
		switch (step) {
		case GUIDE_SHUTTER:
			guideId = Util.getS3DMode() ? R.string.panorama3d_guide_shutter :
                R.string.panorama_guide_shutter;
			break;
		case GUIDE_MOVE:
			guideId = Util.getS3DMode() ? R.string.panorama3d_guide_move :
                R.string.panorama_guide_choose_direction;
			break;
		case GUIDE_CAPTURE:
			guideId = Util.getS3DMode() ? R.string.panorama3d_guide_capture :
                R.string.panorama_guide_capture;
			break;
		default:
			break;
		}
    ...
    public void onSizeChanged(int width, int height) {
        // Base on the width of sensor buff is larger than height.
        if (width > height) {
        	mPanoramaController.setPreviewSize(width, height);
        } else {
        	mPanoramaController.setPreviewSize(height, width);
        }
    }
}
