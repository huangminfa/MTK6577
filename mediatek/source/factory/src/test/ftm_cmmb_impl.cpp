...
#include "CmmbHelper.cpp"

#ifdef CMMB_CHIP_INNO
#include "InnoAppDriver.h"
#include "CmmbAdapterInno.cpp"
#include "InnoAppDriver.cpp"
#else   // using siano chip
#include "CmmbAdapterSiano.cpp"
#include "SmsAdrLiteLinux.c"
#include "hostlib/SmsHostlibLiteCmmb.cpp"
#include "hostlib/SmsHostUtils.c"
#include "hostlib/SmsHostLibLiteCommon.cpp"
#include "osw/linux/Osw_Event.c"
#include "osw/linux/Osw_FileSystem.c"
#include "osw/linux/Osw_Init.c"
#include "osw/linux/Osw_Memory.c"
#include "osw/linux/Osw_Mutex.c"
#include "osw/linux/Osw_Task.c"
#include "osw/linux/Osw_Time.c" 
...
CmmbResult CmmbFtInit()
{
	...
	if (errCode == CMMB_S_OK)
	{
		...
		SP_LOGE("CmmbDriverInit() OK");
	}
	else
	{
		SP_LOGE("CmmbDriverInit() error");
	}
...
CmmbResult CmmbFtAutoScan(CMMB_AUTOSCAN_CALLBACK scanProc)
{
	...
	for (int ch = CMMB_CHANNEL_MIN; ch <= CMMB_CHANNEL_MAX; ch++)
	{
		...
		if (CmmbFtChannelTest(ch) == CMMB_S_OK)
		{
...
int GetMfsSize()
{
   return (mfsWriter.fileSize()/1024);
}
