...
void dumpFileHeader(const DexFile* pDexFile)
{
    ...
    if (pOptHeader != NULL) {
        ...
        printf("dex_length          : %d (0x%06x)\n", 
            pOptHeader->dexLength, pOptHeader->dexLength);
        printf("deps_offset         : %d (0x%06x)\n",
            pOptHeader->depsOffset, pOptHeader->depsOffset);
        printf("deps_length         : %d (0x%06x)\n", 
            pOptHeader->depsLength, pOptHeader->depsLength);
    ...
    printf("map_off             : %d (0x%06x)\n",
        pHeader->mapOff, pHeader->mapOff);
    printf("string_ids_size     : %d (0x%06x)\n", pHeader->stringIdsSize, pHeader->stringIdsSize);
    printf("string_ids_off      : %d (0x%06x)\n",
        pHeader->stringIdsOff, pHeader->stringIdsOff);
    printf("type_ids_size       : %d (0x%06x)\n", pHeader->typeIdsSize, pHeader->typeIdsSize);
    printf("type_ids_off        : %d (0x%06x)\n",
        pHeader->typeIdsOff, pHeader->typeIdsOff);
    printf("proto_ids_size      : %d (0x%06x)\n", pHeader->protoIdsSize, pHeader->protoIdsSize);
    printf("proto_ids_off       : %d (0x%06x)\n",
        pHeader->protoIdsOff, pHeader->protoIdsOff);
    printf("field_ids_size      : %d  (0x%06x)\n", pHeader->fieldIdsSize, pHeader->fieldIdsSize);
    printf("field_ids_off       : %d (0x%06x)\n",
        pHeader->fieldIdsOff, pHeader->fieldIdsOff);
    printf("method_ids_size     : %d (0x%06x)\n", pHeader->methodIdsSize, pHeader->methodIdsSize);
    printf("method_ids_off      : %d (0x%06x)\n",
        pHeader->methodIdsOff, pHeader->methodIdsOff);
    printf("class_defs_size     : %d (0x%06x)\n", pHeader->classDefsSize, pHeader->classDefsSize);
    printf("class_defs_off      : %d (0x%06x)\n",
        pHeader->classDefsOff, pHeader->classDefsOff);
    printf("data_size           : %d (0x%06x)\n", pHeader->dataSize, pHeader->dataSize);
...
void dumpClassDef(DexFile* pDexFile, int idx)
{
    ...
    printf("class_idx           : %d (0x%04x)\n", pClassDef->classIdx, pClassDef->classIdx);
    printf("access_flags        : %d (0x%04x)\n",
        pClassDef->accessFlags, pClassDef->accessFlags);
    printf("superclass_idx      : %d (0x%04x)\n", pClassDef->superclassIdx, pClassDef->superclassIdx);
    printf("interfaces_off      : %d (0x%06x)\n",
        pClassDef->interfacesOff, pClassDef->interfacesOff);
    printf("source_file_idx     : %d (0x%04x)\n", pClassDef->sourceFileIdx, pClassDef->sourceFileIdx);
    ...
    printf("static_value_off    : %d (0x%04x)\n",
        pClassDef->staticValuesOff, pClassDef->staticValuesOff);
    printf("static_fields_size  : %d (0x%04x)\n",
        pClassData->header.staticFieldsSize, pClassData->header.staticFieldsSize);
    printf("instance_fields_size: %d (0x%04x)\n",
            pClassData->header.instanceFieldsSize, pClassData->header.instanceFieldsSize);
    printf("direct_methods_size : %d (0x%04x)\n",
            pClassData->header.directMethodsSize, pClassData->header.directMethodsSize);
    printf("virtual_methods_size: %d (0x%04x)\n",
            pClassData->header.virtualMethodsSize, pClassData->header.virtualMethodsSize);
    printf("\n");
