...
int displayRoutes( )
{
   ...
   fp = fopen("/proc/net/route", "r");
        if(fp == NULL) {           
			LOGE("DumpRoute -- : open file fail");
			return -2 ;
        }	

	LOGI( "Destination     Gateway         Genmask         Flags %s Iface\n",
		   "Metric Ref    Use");
	...
	while (1) {
		...
		if (r != 11) {
			if ((r < 0) && feof(fp)) { /* EOF with no (nonspace) chars read. */
				break;
            LOGE("[displayRoutes] fscanf incorrect bytes");
			}
 ERROR:
            fclose(fp);
			LOGE("DumpRoute -- :fscanf I/O Error");
			return -1 ;
		}
	...
	char *gw_ptr =  strdup(inet_ntoa(gw));
        ...
        LOGI("%-15.15s %-15.15s %-16s%-6s%-6d %-2d %7d %s", 
