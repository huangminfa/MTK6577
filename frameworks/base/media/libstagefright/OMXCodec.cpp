...
#endif
//#include "include/VorbisEncoder.h"
#else
#include "include/AACEncoder.h"
#include "include/AMRWBEncoder.h"
#endif
...
#ifndef ANDROID_DEFAULT_CODE
// FIXME .h should be included instead of copied here if more things are needed
// from pv_omxdefs.h
static const int PV_OMX_COMPONENT_CAPABILITY_TYPE_INDEX = 0xFF7A347;
// from pv_omxcore.h
struct PV_OMXComponentCapabilityFlagsType
{
    ////////////////// OMX COMPONENT CAPABILITY RELATED MEMBERS
    int32_t iIsOMXComponentMultiThreaded;
    int32_t iOMXComponentSupportsExternalOutputBufferAlloc;
    int32_t iOMXComponentSupportsExternalInputBufferAlloc;
    int32_t iOMXComponentSupportsMovableInputBuffers;
    int32_t iOMXComponentSupportsPartialFrames;
    int32_t iOMXComponentUsesNALStartCodes;
    int32_t iOMXComponentCanHandleIncompleteFrames;
    int32_t iOMXComponentUsesFullAVCFrames;
...
#endif
//FACTORY_CREATE_ENCODER(VorbisEncoder)
#else
FACTORY_CREATE_ENCODER(AMRWBEncoder)
FACTORY_CREATE_ENCODER(AACEncoder)
#endif
...
#ifndef ANDROID_DEFAULT_CODE 
	{ MEDIA_MIMETYPE_AUDIO_MPEG, "OMX.MTK.AUDIO.DECODER.MP3" }, 
	{ MEDIA_MIMETYPE_AUDIO_WMA, "OMX.MTK.AUDIO.DECODER.WMA" }, 	
#ifdef MTK_AUDIO_APE_SUPPORT
///#ifdef USES_MTK_OMX_CORE
    { MEDIA_MIMETYPE_AUDIO_APE, "OMX.MTK.AUDIO.DECODER.APE" },
///#endif

	///{ MEDIA_MIMETYPE_AUDIO_APE, "APEDecoder" },
#endif
    { MEDIA_MIMETYPE_AUDIO_FLAC, "OMX.MTK.AUDIO.DECODER.FLAC" },
	{ MEDIA_MIMETYPE_AUDIO_AMR_NB, "OMX.MTK.AUDIO.DECODER.AMR.NB" },
	{ MEDIA_MIMETYPE_AUDIO_AMR_WB, "OMX.MTK.AUDIO.DECODER.AMR.WB" },
	{ MEDIA_MIMETYPE_AUDIO_AAC, "OMX.MTK.AUDIO.DECODER.AAC" },
	{ MEDIA_MIMETYPE_AUDIO_G711_ALAW, "OMX.MTK.AUDIO.DECODER.G711" },
	{ MEDIA_MIMETYPE_AUDIO_G711_MLAW, "OMX.MTK.AUDIO.DECODER.G711" },
#endif			
...
#ifndef ANDROID_DEFAULT_CODE 
    { MEDIA_MIMETYPE_VIDEO_MPEG4, "OMX.MTK.VIDEO.DECODER.MPEG4" },
    { MEDIA_MIMETYPE_VIDEO_H263, "OMX.MTK.VIDEO.DECODER.H263" },
    { MEDIA_MIMETYPE_VIDEO_AVC, "OMX.MTK.VIDEO.DECODER.AVC" },
    { MEDIA_MIMETYPE_VIDEO_VPX, "OMX.MTK.VIDEO.DECODER.VPX" },
    { MEDIA_MIMETYPE_VIDEO_WMV, "OMX.MTK.VIDEO.DECODER.VC1" },
#else
...
#endif
#ifndef ANDROID_DEFAULT_CODE
    { MEDIA_MIMETYPE_AUDIO_VORBIS, "OMX.MTK.AUDIO.DECODER.VORBIS" },
#else
...
#ifndef ANDROID_DEFAULT_CODE
    { MEDIA_MIMETYPE_AUDIO_AMR_NB, "OMX.MTK.AUDIO.ENCODER.AMR" },
#ifdef HAVE_AWBENCODE
    { MEDIA_MIMETYPE_AUDIO_AMR_WB, "OMX.MTK.AUDIO.ENCODER.AWB" },
#endif
#ifdef HAVE_AACENCODE
    { MEDIA_MIMETYPE_AUDIO_AAC, "OMX.MTK.AUDIO.ENCODER.AAC" },
#endif
    { MEDIA_MIMETYPE_AUDIO_VORBIS, "OMX.MTK.AUDIO.ENCODER.VORBIS" },
...
#ifndef ANDROID_DEFAULT_CODE
    { MEDIA_MIMETYPE_VIDEO_MPEG4, "OMX.MTK.VIDEO.ENCODER.MPEG4" },
    { MEDIA_MIMETYPE_VIDEO_H263, "OMX.MTK.VIDEO.ENCODER.H263" },
    { MEDIA_MIMETYPE_VIDEO_AVC, "OMX.MTK.VIDEO.ENCODER.AVC" },
#else
...
#define CODEC_LOGD(x, ...) LOGD("[%s] "x, mComponentName, ##__VA_ARGS__)
...
///#ifdef USES_MTK_OMX_CORE
#ifdef MTK_AUDIO_APE_SUPPORT
    if (!strcmp(componentName, "OMX.MTK.AUDIO.DECODER.APE")) {
        quirks |= kDecoderLiesAboutNumberOfChannels;
...
///#endif
    ...
    if (!strcmp(componentName, "OMX.MTK.VIDEO.DECODER.AVC")) {
...
#ifndef ANDROID_DEFAULT_CODE
// Morris Yang OMXVEnc With Camera [
    if (!strncmp(componentName, "OMX.MTK.VIDEO.ENCODER.", 22)) {
        quirks |= kAvoidMemcopyInputRecordingFrames;
        LOGD ("@@ OMX.MTK.VIDEO.ENCODER Disable memcpy on input buffer!!!");
    }
// ]
#endif
...
#ifndef ANDROID_DEFAULT_CODE
	if (!strcmp(componentName, "OMX.MTK.AUDIO.DECODER.G711")) {
		quirks |= kDecoderLiesAboutNumberOfChannels;
	}
	if (!strcmp(componentName, "OMX.MTK.AUDIO.DECODER.MP3")) {
		quirks |= kDecoderLiesAboutNumberOfChannels;
		quirks |= kSupportsMultipleFramesPerInputBuffer;
    }
  if (!strcmp(componentName, "OMX.MTK.AUDIO.DECODER.VORBIS")) {
		quirks |= kDecoderLiesAboutNumberOfChannels;
		}
#endif
...
// Morris Yang OMXVEnc With Camera [
            ...
            if (meta->findInt32(kKeyCamMemInfo, &prCamMemInfo)) {
                LOGD ("@@ this is camera recording!!!");

                for(uint32_t u4I = 0; u4I < ((CamMemInfo_t*)prCamMemInfo)->u4VdoBufCount; u4I++)
                {
                    LOGD ("VdoBufVA = 0x%08x, VdoBufCount = %d, VdoBufSize = %d,\n",
...
status_t OMXCodec::configureCodec(const sp<MetaData> &meta) {
    ...
    if (!(mFlags & kIgnoreCodecSpecificData)) {
...
#endif
		{
...
#ifndef ANDROID_DEFAULT_CODE
			{
				...
				if (err != OK)
				{
					LOGE("esds.InitCheck() ERROR %d!!!", err);
					return err;
				}
			}
#else
            ...
            LOGI("OMXCodec::configureCodec--config Codec Info for AAC");
            addCodecSpecificData(data, size);
#endif
    ...
    if (mIsEncoder) {
        ...
        OMX_INDEXTYPE index;
        status_t err2;
        int32_t Stereo3DMode = VIDEO_STEREO_DEFAULT;
    ...
    if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_FLAC, mMIME))
    {
        ...
        profile.sample_rate = ((FLAC__StreamMetadata_Info_ *)data)->sample_rate;           
        ...
        profile.channels = ((FLAC__StreamMetadata_Info_ *)data)->channels;    
        ...
        SXLOGV("kKeyFlacMetaInfo = %lld, %d, %d, %d, %d, %d, %d, %d",profile.total_samples, profile.min_framesize, profile.max_framesize, 
                    profile.sample_rate, profile.min_blocksize, profile.max_blocksize, profile.channels, profile.bits_per_sample);
		...
		if(profile.bits_per_sample/8 < 2)		//default output 16 bit pcm.
	        outputdef.nBufferSize = profile.max_blocksize * profile.channels * 2;
		else
			outputdef.nBufferSize = profile.max_blocksize * profile.channels * profile.bits_per_sample/8;
        err = mOMX->setParameter(
                mNode, OMX_IndexParamPortDefinition, &outputdef, sizeof(outputdef));
    ...
    else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_APE, mMIME)) 
    {
		...
		if (!strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.AAC")) {
			...
			if(bIsAACADIF){
				...
				errAAC = setAACFormat(numChannels, sampleRate, bitRate);
			}
		    if (errAAC != OK) {
    ...
    //MTK_OP01_PROTECT_START
            ...
            LOGD("Omxcodec configureCodec IsCMMBFlag = %d", IsCMMBFlag);
#endif
    ...
    if ((!strcasecmp(MEDIA_MIMETYPE_AUDIO_WMA, mMIME))) {
        ...
        if (meta->findData(kKeyWMAC, &type, &data, &size)) {
            LOGE ("addCodecSpecificData for WMA");
            const uint8_t *ptr = (const uint8_t *)data;
    ...
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_WMV, mMIME)) {
        ...
        if (meta->findData(kKeyWMVC, &type, &data, &size)) {
            LOGE ("addCodecSpecificData for WMV");
            const uint8_t *ptr = (const uint8_t *)data;
            addCodecSpecificData(ptr, size);
        }
    }
...
#ifndef ANDROID_DEFAULT_CODE   // Morris Yang add for RM
...
#ifndef ANDROID_DEFAULT_CODE
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_REAL_VIDEO, mime)) { // Morris Yang add for RM
        compressionFormat = OMX_VIDEO_CodingRV;
...
OMXCodec::OMXCodec(
    ...
    if (false == mIsEncoder) {
        if ((!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVC, mime)) ||
            ...
            (!strcasecmp(MEDIA_MIMETYPE_VIDEO_REAL_VIDEO, mime)) || // Morris Yang add for RM
            ...
            CODEC_LOGD ("mVideoInputErrorRate(%f)", mVideoInputErrorRate);
        }
    }
    else {
        if ((!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVC, mime)) ||
...
// ]
    ...
    LOGD ("!@@!>> create tid (%d) OMXCodec mOMXLivesLocally=%d, mIsVideoDecoder(%d), mIsVideoEncoder(%d), mime(%s)", gettid(), mOMXLivesLocally, mIsVideoDecoder, mIsVideoEncoder, mime);
#endif
}
...
OMXCodec::~OMXCodec() {
    ...
    if (mInputBufferPoolMemBase)
    {
        sf_pmem_free(mInputBufferPoolMemBase);
        mInputBufferPoolMemBase = NULL;
    }

    if (mOutputBufferPoolMemBase)
    {
        sf_pmem_free(mOutputBufferPoolMemBase);
        mOutputBufferPoolMemBase = NULL;
    }
#endif

    LOGD ("!@@!>> destroy tid (%d) ", gettid());
#endif
}
...
status_t OMXCodec::init() {
    ...
    CHECK(meta->findCString(kKeyMIMEType, &mime));
     ...
     if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_VPX, mime)) { // Morris Yang add for VP8
        ...
        LOGD ("Set VP8 bitstream size. width=%d, height=%d", width, height);
        InitOMXParams(&def);
        def.nPortIndex = kPortIndexInput;
...
status_t OMXCodec::allocateBuffersOnPort(OMX_U32 portIndex) {
    ...
    switch (portIndex)
    {
        ...
        default:
            LOGE ("OMXCodec::allocateBuffersOnPort  invalid port index");
            break;
    }
...
// Morris Yang 20120214 add for live effect recording
        LOGD ("@@ allocateBuffersOnInputPort (kStoreMetaDataInVideoBuffers)");
        OMX_PARAM_PORTDEFINITIONTYPE def;
        InitOMXParams(&def);
        def.nPortIndex = kPortIndexInput;
        status_t err;
        ...
        LOGD ("@@ def.nBufferSize(%d)", def.nBufferSize);
        err = mOMX->setParameter(
                mNode, OMX_IndexParamPortDefinition, &def, sizeof(def));
...
// Morris Yang OMXVEnc With Camera [
    ...
    if ((mCameraMeta.get() != NULL) && mCameraMeta->findInt32(kKeyCamMemInfo, &prCamMemInfo)) {
        LOGD ("@@ allocateBuffersOnInputPort (CAMERA RECORDING!!!!)");
    ...
    if (bIsCameraRecording) {
        ...
        // TODO: CHECK IF _cam_pmem_count==def.nBufferCountActual
...
// ]
    ...
    CODEC_LOGD ("allocating %lu buffers of size %lu on input port", def.nBufferCountActual, def.nBufferSize);
    ...
    if (mIsVideoEncoder) {
        ...
        if (mOMXLivesLocally) {
            ...
            if (mMemHeapBase == NULL) {
                CODEC_LOGE ("[ERROR][M4U] M4UMemPool error: could not create master heap!");
                return UNKNOWN_ERROR;
            }
            mInputBufferPoolMemBase = (unsigned char*) mMemHeapBase->getBase();
            CODEC_LOGD ("mMemHeapBase, = 0x%x, mInputBufferPoolPmemBase = 0x%08X\n", mMemHeapBase.get(), (unsigned int)mInputBufferPoolMemBase);
            ...
            CODEC_LOGD ("before: base = 0x%x, mMemHeapBase = 0x%08X, size = %d", (unsigned int)memheapinfo.base, (unsigned int)memheapinfo.mMemHeapBase, (unsigned int)memheapinfo.size);
            sf_memheap_set_info(&memheapinfo);

            MemHeapInfo memheapinfo_2;
            sf_memheap_get_info(mInputBufferPoolMemBase, &memheapinfo_2);
            CODEC_LOGD("after: base = 0x%x, mMemHeapBase = 0x%08X, size = %d", (unsigned int)memheapinfo_2.base, (unsigned int)memheapinfo_2.mMemHeapBase, (unsigned int)memheapinfo_2.size);
        }
        else {
            mMemHeapBase = new MemoryHeapBase(totalSize, 0, "M4UMemPool");
            if (mMemHeapBase == NULL) {
            ...
            mInputBufferPoolMemBase = (unsigned char*) mMemHeapBase->getBase();
            CODEC_LOGD ("mMemHeapBase = 0x%08X, mInputBufferPoolPmemBase = 0x%08X, size=%d", mMemHeapBase.get(), (unsigned int)mInputBufferPoolMemBase, mMemHeapBase->getSize());

            if (mMemHeapBase->getHeapID() < 0) {
                CODEC_LOGE("Error creating frame buffer heap");
                return UNKNOWN_ERROR;
            }
        ...
        OMX_U32 const TRYALLOCMEMCOUNT = 200;
        OMX_U32 u4TryAllocMemCount = TRYALLOCMEMCOUNT;
		
        while (u4TryAllocMemCount) {
            mInputBufferPoolMemBase = (unsigned char*)sf_pmem_alloc(totalSize);

            if (mInputBufferPoolMemBase == 0) {
                LOGW("Alloc mOutputBufferPoolPmemBase fail %u times!!, Try alloc again!!", (unsigned int)(TRYALLOCMEMCOUNT-u4TryAllocMemCount));
                u4TryAllocMemCount--;
                usleep(10*1000);
            }
            else {				
    	...
    	if (mInputBufferPoolMemBase == NULL) {
    		CODEC_LOGE ("Alloc mInputBufferPoolMemBase fail %u times!!, Return error!!\n", (unsigned int)TRYALLOCMEMCOUNT);
    		return UNKNOWN_ERROR;
    	}
    	CODEC_LOGD ("mInputBufferPoolPmemBase(%p) for input port PA=0x%08X", mInputBufferPoolMemBase, (unsigned int)sf_pmem_vir2phy((void *)mInputBufferPoolMemBase, 777));

    	sf_pmem_map_into_noncached(mInputBufferPoolMemBase);  // map to non-cache before access

    	if (!mOMXLivesLocally) {
    	    const char *pmemDev = "/dev/pmem_multimedia";
    	    PmemInfo pmemInfo;

    	    sf_pmem_get_info((void*)mInputBufferPoolMemBase, &pmemInfo);
    	    //sp<MemoryHeapBaseMTK> mHeap = new MemoryHeapBaseMTK(pmemInfo.shared_fd, pmemInfo.base, pmemInfo.size, MemoryHeapBase::DONT_MAP_LOCALLY, pmemDev);
    	    sp<MemoryHeapBase> mHeap = new MemoryHeapBase(pmemInfo.shared_fd, pmemInfo.size, MemoryHeapBase::DONT_MAP_LOCALLY, pmemInfo.offset);

    	    if (mHeap->getHeapID() < 0) {
    	        CODEC_LOGE ("Error creating frame buffer heap");
    	        return UNKNOWN_ERROR;
    	    }
    	    err = mOMX->registerBuffer(mNode, kPortIndexInput, mHeap);
    	}
#endif
        ...
        {
            CODEC_LOGE ("Error creating input buffer heap fail.");
            return UNKNOWN_ERROR;
        }
    }  
    ...
    for (OMX_U32 i = 0; i < def.nBufferCountActual; ++i) {
        ...
        if ((mQuirks & kRequiresAllocateBufferOnInputPorts) || (mFlags & kUseSecureInputBuffers)) {
            ...
            if (mIsVideoEncoder && mOMXLivesLocally) {
             ...
             #else
                PmemInfo pmemInfo;
                sf_pmem_get_info((void*)info.mData, &pmemInfo);
                err = mOMX->useBuffer(mNode, kPortIndexInput, NULL, def.nBufferSize, pmemInfo.offset, &buffer);
             #endif
            }
            else {
...
// Morris Yang OMXVEnc With Camera [
    ...
    int32_t prCamMemInfo;   //CamMemInfo_t pointer
    if ((mCameraMeta.get() != NULL) && mCameraMeta->findInt32(kKeyCamMemInfo, &prCamMemInfo)) {
        LOGD ("@@ allocateBuffersOnOutputPort (CAMERA RECORDING!!!!)");
        bIsCameraRecording = true;
    }
    ...
    if (bIsCameraRecording) {
        ...
        LOGD ("@@ def.nBufferSize(aligned)=%d", def.nBufferSize);
        ...
        mMemHeapBase = new MemoryHeapBase(totalSize, 0, "M4UMemPool");
        if (mMemHeapBase == NULL) {
            LOGE("[ERROR][M4U] M4UMemPool error: could not create master heap!");
            return UNKNOWN_ERROR;
        }
        mOutputBufferPoolMemBase = (unsigned char*) mMemHeapBase->getBase();
        LOGD("mMemHeapBase = 0x%08X, mOutputBufferPoolMemBase = 0x%08X, size = %d", mMemHeapBase.get(), (unsigned int)mOutputBufferPoolMemBase, mMemHeapBase->getSize());

        if (mMemHeapBase->getHeapID() < 0) {
            LOGE("Error creating frame buffer heap (M4U)");
            return UNKNOWN_ERROR;
        }
#endif
        for (OMX_U32 i = 0 ; i < def.nBufferCountActual ; ++i) {
...
// ]
    ...
    if (mIsVideoDecoder) {
        ...
        mOutputBufferPoolMemBase = (unsigned char*) mMemHeapBase->getBase();
        CODEC_LOGD("mMemHeapBase = 0x%08X, mOutputBufferPoolMemBase = 0x%08X, size = %d", mMemHeapBase.get(), (unsigned int)mOutputBufferPoolMemBase, mMemHeapBase->getSize());
        ...
        while (u4TryAllocMemCount) {
            mOutputBufferPoolMemBase = (unsigned char*)sf_pmem_alloc(totalSize);

            if (mOutputBufferPoolMemBase == 0) {
                LOGW("Alloc mOutputBufferPoolMemBase fail %u times!!, Try alloc again!!", (unsigned int)(TRYALLOCMEMCOUNT-u4TryAllocMemCount));
                u4TryAllocMemCount--;
                usleep(10*1000);
            }
            else {
        ...
        if (mOutputBufferPoolMemBase == NULL) {
            CODEC_LOGE("Alloc mOutputBufferPoolMemBase fail %u times!!, Return error!!", (unsigned int)TRYALLOCMEMCOUNT);
            return UNKNOWN_ERROR;
        }

#ifndef USES_CACHE_PMEM
        sf_pmem_map_into_noncached(mOutputBufferPoolMemBase);  // map to non-cache before access
        memset (mOutputBufferPoolMemBase, 0x00, totalSize);
#endif
        ...
        if (!mOMXLivesLocally) {
            ...
            sf_pmem_get_info((void*)mOutputBufferPoolMemBase, &pmemInfo);
            //sp<MemoryHeapBaseMTK> mHeap = new MemoryHeapBaseMTK(pmemInfo.shared_fd, pmemInfo.base, pmemInfo.size, MemoryHeapBase::DONT_MAP_LOCALLY, pmemDev);
            sp<MemoryHeapBase> mHeap = new MemoryHeapBase(pmemInfo.shared_fd, pmemInfo.size, MemoryHeapBase::DONT_MAP_LOCALLY,  pmemInfo.offset);

            if (mHeap->getHeapID() < 0) {
                CODEC_LOGE ("Error creating frame buffer heap (PMEM)");
                return UNKNOWN_ERROR;
            }
            err = mOMX->registerBuffer(mNode, kPortIndexOutput, mHeap);        
        }
#endif
    }
    else {
        ...
        if (pheap->getHeapID() < 0)
        {
            CODEC_LOGE("Error creating output buffer heap fail.");
            return UNKNOWN_ERROR;
        }
        CODEC_LOGD ("OMXCodec::allocateBuffersOnOutputPort:totalSize=%d",totalSize);
    }
    ...
    for (OMX_U32 i = 0; i < def.nBufferCountActual; ++i) {    
        ...
        if (mQuirks & kRequiresAllocateBufferOnOutputPorts) {
            ...
            if (mIsVideoDecoder && mOMXLivesLocally) {
                ...
                PmemInfo pmemInfo;
                sf_pmem_get_info((void*)info.mData, &pmemInfo);
                err = mOMX->useBuffer(mNode, kPortIndexOutput, NULL, def.nBufferSize, pmemInfo.offset, &buffer);
              #endif
            }
            else{
        ...
        CODEC_LOGV("allocated buffer %p on output port", buffer);
    }
...
status_t OMXCodec::allocateOutputBuffersFromNativeWindow() {
    ...
    LOGD ("native_window_set_buffers_geometry W(%d), H(%d), Stride(%d), SliceH(%d)", def.format.video.nFrameWidth, def.format.video.nFrameHeight, def.format.video.nStride, def.format.video.nSliceHeight);
...
#endif
    LOGD("native_window_set_usage usage=0x%lx", usage);
    ...
    for (OMX_U32 i = 0; i < def.nBufferCountActual; i++) {
        ...
        CODEC_LOGD ("registered GraphicBuffer ptr(0x%08X), width(%d), height(%d), stride(%d), format(0x%08X), ID(0x%08X)",
            graphicBuffer.get(), graphicBuffer->width, graphicBuffer->height, graphicBuffer->stride, graphicBuffer->format, bufferId);           
    }
        ...
        case omx_message::FILL_BUFFER_DONE:
        {
            ...
            if (mPortStatus[kPortIndexOutput] == DISABLING) {
                ...
                if (msg.u.extended_buffer_data.flags & OMX_BUFFERFLAG_3D_SIDEBYSIDE) {
                    buffer->meta_data()->setInt32(kKeyVideoStereoMode, VIDEO_STEREO_SIDE_BY_SIDE);
                }
                if (msg.u.extended_buffer_data.flags & OMX_BUFFERFLAG_3D_TOPANDBOTTOM) {
                    buffer->meta_data()->setInt32(kKeyVideoStereoMode, VIDEO_STEREO_TOP_BOTTOM);
                }
                if (msg.u.extended_buffer_data.flags & OMX_BUFFERFLAG_2D) {
                    buffer->meta_data()->setInt32(kKeyVideoStereoMode, VIDEO_STEREO_2D);
                }
#endif
#endif
                ...
                if (mTargetTimeUs >= 0) {
#ifndef ANDROID_DEFAULT_CODE
					LOGD("Key time=%lld, Target time=%lld", msg.u.extended_buffer_data.timestamp, mTargetTimeUs);
#else
...
void OMXCodec::onEvent(OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
        ...
        case OMX_EventError:
        {
            ...
            {                        
                LOGW("OMXCodec::onEvent--OMX Error Stream Corrupt!!");
#ifdef MTK_AUDIO_APE_SUPPORT                
...
#endif                
                if(mIsVideoEncoder){
                    LOGW("OMXCodec::onEvent--Video encoder error");
            ...
            else
            {
                if (mIsVideoDecoder && (OMX_S32)data1 == OMX_ErrorBadParameter) {
                    LOGW("OMXCodec::onEvent--OMX Bad Parameter!!");
                    mFinalStatus = ERROR_UNSUPPORTED_VIDEO;
                }
				if(!mIsEncoder && !mIsVideoDecoder && (OMX_S32)data1 == OMX_ErrorBadParameter){
					LOGW("OMXCodec::onEvent--Audio OMX Bad Parameter!!");
        ...
        case OMX_EventPortSettingsChanged:
        {
            ...
            if (data2 == 0 || data2 == OMX_IndexParamPortDefinition) {
                ...
                if (data2 == OMX_IndexVendorMtkOmxVdecGetAspectRatio) {
                    LOGE ("@@ GOT OMX_IndexVendorMtkOmxVdecGetAspectRatio");
                    OMX_S32 aspectRatio = 0;
                    if (OK == mOMX->getConfig(mNode, OMX_IndexVendorMtkOmxVdecGetAspectRatio, &aspectRatio, sizeof(aspectRatio))) {
                        LOGE ("@@ AspectRatioWidth (%d), AspectRatioHeight(%d)", (aspectRatio & 0xFFFF0000) >> 16, (aspectRatio & 0x0000FFFF));
		...
		case OMX_EventComponentInfoNotified:
		{
			if (!strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.AAC")) 
			{
				...
				CODEC_LOGD("OMX Notify AAC Profile %d",getAACProfile());
			}
		}			
#endif
...
void OMXCodec::drainInputBuffers() {
    ...
    if (true == IsCMMBFlag)	
    {
        ...
        Vector<BufferInfo> *buffers = &mPortBuffers[kPortIndexInput];
        LOGE("OMXCodec::drainInputBuffers buffer pool size = %d", buffers->size());
        if ((buffers->size() > 3))
        {
            buffersize = 3;
        }
        ...
        if (mSeekTimeUs >= 0) {
            ...
            if (mMaxQueueBufferNum > 0 && !init) {
                ...
                while ((mFilledBuffers.size() > mMaxQueueBufferNum) &&
                    ...
                    if (err == -ETIMEDOUT) {
                        LOGI("drainInputBuffer wait timeout with state %d, seek %lld",
                                mState, mSeekTimeUs);
                    }
                }
                mQueueWaiting = false;
                if (mSeekTimeUs >= 0 || 
                    ...
                    LOGD("drainInputBuffer break with state %d, seek %lld", 
                            mState, mSeekTimeUs);
                    return false;
                }
                mLock.unlock();
...
//deal time out MTK80721 2012-03-29
            ...
            {
                LOGE("OMXCodec::drainInputBuffer: time out,tid=%d",gettid());       
                srcBuffer->release();
                srcBuffer = NULL;
                setState(ERROR);
                return false;    
...
//
        ...
        if (srcBuffer->range_length() > remainingBytes) {
            ...
            LOGD("OMXCodec: split big input buffer %d to %d", 
...
#ifndef ANDROID_DEFAULT_CODE
// Morris Yang OMXVEnc With Camera [
            LOGD ("@@ drainInputBuffer NO memcpy [PA=0x%08X, range_offset=%d, range_length=%d]", srcBuffer->data(), srcBuffer->range_offset(), srcBuffer->range_length());
// ]
#endif
...
int32_t OMXCodec::getAACProfile()
{
	...
	CHECK_EQ((status_t)OK, errAAC);
	LOGD("profileAAC.eAACProfile %d",profileAAC.eAACProfile);
...
status_t OMXCodec::start(MetaData *meta) {
    ...
    if (meta) {
        ...
        if (meta->findInt32(kKeyRTSPSeekMode, &mode) && mode != 0) {
            ...
            LOGI("set StreamingMode, index = %x, err = %x, err2 = %x", index, err, err2);
        }
        int32_t number = -1;
		...
		if((mFlags & kMtkAudDecForVE)){
			...
			if (!strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.MP3")) {
				...
				err = mOMX->setParameter(
					mNode, OMX_IndexVendorMtkMP3Decode, &defmp3, sizeof(defmp3));
				CHECK_EQ((int)err, (int)OK);
				CODEC_LOGD("set port num %d",maxframenum);	
			}
			CODEC_LOGD("set buffer num %d and size %d",def.nBufferCountActual,def.nBufferSize);			
		}	
#endif
#endif // #ifndef ANDROID_DEFAULT_CODE
    }
...
#ifndef ANDROID_DEFAULT_CODE
    // get PV capabilities
    if (!strncmp("OMX.PV.", mComponentName, 7)) {
        // check if codec supports partial frames input
        struct PV_OMXComponentCapabilityFlagsType caps;
        status_t err = mOMX->getParameter(mNode, 
                (OMX_INDEXTYPE)PV_OMX_COMPONENT_CAPABILITY_TYPE_INDEX, 
                &caps, sizeof(caps));
        CHECK_EQ((int)err, (int)OK);
        LOGI("mSupportsPartialFrames %d", mSupportsPartialFrames);
        mSupportsPartialFrames = caps.iOMXComponentSupportsPartialFrames;
    }
    if (!strncmp("OMX.MTK.", mComponentName, 8)) {
        ...
        LOGI("mSupportsPartialFrames %d err %d ", mSupportsPartialFrames, err);
    }
#endif
...
//MTK80721 Vorbis Enc
    ...
    {
        mSignalledEOS = true;
	    LOGD("OMXCodec::read:*buffer=%p",*buffer);
	    (*buffer)->release();
    }
#endif
...
// Morris Yang OMXVEnc With Camera [
        if (mCameraMeta.get() != NULL) {
            ...
            if (mIsVENCTimelapseMode) {
            	...
            	LOGD ("Submit one frame for timelapse mode!!");
            }
            Vector<BufferInfo> *buffers = &mPortBuffers[kPortIndexInput];
            for (size_t i = 0; i < buffers->size(); ++i) {
                BufferInfo *info = &buffers->editItemAt(i);
