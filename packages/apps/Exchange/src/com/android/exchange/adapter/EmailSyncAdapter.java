...
public class EmailSyncAdapter extends AbstractSyncAdapter {
    ...
    // Whether we deduced the specific lookback value from GIE
//    public boolean mLookbackDeduced = false;
    ...
    @Override
    public void wipe() {
        Logging.d(TAG, "Wipe all messages");
        ...
        if (mFetchRequestList.isEmpty()) {
            ...
            if (protocolVersion < Eas.SUPPORTED_PROTOCOL_EX2007_DOUBLE) {
                ...
                s.data(Tags.SYNC_WINDOW_SIZE, (syncStatus <= Eas.EAS_SYNC_INIT)
                        ? EMAIL_FIRST_WINDOW_SIZE : EMAIL_WINDOW_SIZE);
            }
...
//            }
            ...
            if (protocolVersion >= Eas.SUPPORTED_PROTOCOL_EX2007_DOUBLE) {
                ...
                s.data(Tags.SYNC_MIME_TRUNCATION, Eas.EAS2_5_FULL_SIZE);
            }
            s.end();
        } else {
            s.start(Tags.SYNC_OPTIONS);
        ...
        public void addData (Message msg, int endingTag) throws IOException {
            ...
            if (atts.size() > 0) {
                if (msg.mAttachments == null) {
                    ...
                    fixAttachmentLocation(msg.mAttachments, atts);
                }
            }
            msg.mSize = mSize;
        }
            ...
            while (nextTag(Tags.SYNC_FETCH) != END) {
                switch (tag) {
                    case Tags.SYNC_SERVER_ID:
                        ...
                        Logging.d(Logging.LOG_TAG, "+++ Email responsesParser SYNC_SERVER_ID: " + msg.mServerId);
                        break;
                    case Tags.SYNC_APPLICATION_DATA:
                        Logging.d(Logging.LOG_TAG, "+++ Email responsesParser SYNC_APPLICATION_DATA");
                        addData(msg, tag);
                        break;
                    default:
                        skipTag();
            ...
            while (nextTag(Tags.EMAIL_BODY) != END) {
                switch (tag) {
                    ...
                    case Tags.BASE_TRUNCATED:
                        ...
                        Logging.d(Logging.LOG_TAG, "+++ airsyncbase body truncated: " + bodyTruncated);
                        break;
                    default:
                        skipTag();
                }
...
//                            .build());
            ...
            synchronized (mService.getSynchronizer()) {
                ...
                try {
                    ...
                    if (Logging.LOG_PERFORMANCE) {
                        Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] EmailProvider:applyBatch:EmailSyncAdapter:commit ["
                                + (end - start) + "]");
                    }
                    if (newEmails.size() > 0) {
                        if (mIsFirstLoaded) {
                            ...
                            if (Logging.LOG_PERFORMANCE) {
                                Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] EmailProvider:applyBatch:EmailSyncAdapter:firstShownMessages:newEmail:" + newEmails.size()
                                        + " ["
                                        + end + "]");
                            }
                        } else {
                            if (Logging.LOG_PERFORMANCE) {
                                Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] EmailProvider:applyBatch:EmailSyncAdapter:newEmail:" + newEmails.size()
                                        + " ["
                                        + end + "]");
                            }
                        }
                    ...
                    for (Message msg : fetchedEmails) {
                     // Find the original message's id (by serverId and mailbox)
                        ...
                        try {
                            ExchangeService.callback().fetchMessageCallBack(Integer.valueOf(id), EmailServiceStatus.SUCCESS, 100);
     ...
     * @param updatedAtts attachments parsed from mime message which need to be updated
     * @param olderAtts attachments received in attachment fields
     */
    public static void fixAttachmentLocation(ArrayList<Attachment> updatedAtts,
