...
#undef LOG_TAG
#endif
#define LOG_TAG  "HeadphoneCompensationFilter"
...
//#define ENABLE_LOG_HeadphoneCompensationFilter
#ifdef ENABLE_LOG_HeadphoneCompensationFilter
#undef LOGV
#define LOGV(...) LOGD(__VA_ARGS__)
#endif

//#define HEADPHONE_Test_OnlyMemcpy
...
HeadphoneCompensationFilter::HeadphoneCompensationFilter()
{
    mpBloudHandle = NULL;
    mpWorkingBuf = NULL;
    mpHCFInputBuf = NULL;
...
HeadphoneCompensationFilter::~HeadphoneCompensationFilter()
{
    ...
    if(mpHCFInputBuf != NULL)
    {
        delete [] mpHCFInputBuf;
        mpHCFInputBuf = NULL;
        LOGV("mpHCFInputBuf delete");
    }
...
bool HeadphoneCompensationFilter::Start(void)
{
    LOGD("HeadphoneCompensationFilter::Start() ");
...
//mpBloudHandle->Open can be only called during init
//no need to called before every process called or after mInitParam modified
bool HeadphoneCompensationFilter::Stop(void)
{
    LOGD("HeadphoneCompensationFilter::Stop()");

    mpHCFInput_Wleft= mpHCFInputBuf;
    mpHCFInput_W = mpHCFInputBuf;
    mHCFInput_Count = 0;
    memset(mpHCFInputBuf, 0, 12288);//reset 12K byte buffer
...
bool HeadphoneCompensationFilter::Init(void)
{
    LOGD("Init HeadphoneCompensationFilter");
    ...
    if(mpHCFInputBuf == NULL)
    {
        mpHCFInputBuf = new short[6144]; // allocate 12K byte buffer
        LOGV("allocate mpHCFInputBuf size = %d mpHCFInputBuf = %p", 12288, mpHCFInputBuf);
        mpHCFInput_Wleft= mpHCFInputBuf;
        mpHCFInput_W = mpHCFInputBuf;
        mHCFInput_Count = 0;
    ...
    LOGV("HeadphoneCompensationFilter::Init(), mpBloudHandle->bEnableOLA=%d", mpBloudHandle->bEnableOLA);
...
void HeadphoneCompensationFilter::Deinit(void)
{
    if(mpWorkingBuf != NULL)
    {
        delete [] mpWorkingBuf;
...
void HeadphoneCompensationFilter::LoadHCFParameter()
{
    int i, j, k;
    ...
    GetHeadphoneCompFltCustParamFromNV(&audioParam);
    LOGD("HeadphoneCompensationFilter::LoadHCFParameter!! ");
...
void HeadphoneCompensationFilter::SetHCFPreviewParameter(AUDIO_ACF_CUSTOM_PARAM_STRUCT* PreviewParam)
{
    int i,j,k;
    LOGD("HeadphoneCompensationFilter::SetHCFPreviewParameter!! ");
...
*/
void HeadphoneCompensationFilter::SetWorkMode(unsigned int chNum, unsigned int smpRate, unsigned int workMode)
{
    ...
    LOGD("HeadphoneCompensationFilter mWorkMode=%d, Channel_Num=%d, Sample_Rate=%d", mWorkMode, mInitParam.Channel_Num, mInitParam.Sample_Rate);
}
//reset working buffer and left input buffer
void HeadphoneCompensationFilter::ResetBuffer(void)
{
    LOGV("HeadphoneCompensationFilter::ResetBuffer() mintrSize=%d, mHCFInput_Count=%d", mintrSize, mHCFInput_Count);

    memset(mpWorkingBuf, 0, mintrSize);    
    //clear left input data to 0 
    if(mpHCFInputBuf != NULL)
    {
        memset((void*)(mpHCFInputBuf), 0 , 12288);
    }
}

void HeadphoneCompensationFilter::Process(const short *pInputBuffer,int *InputSampleCount,short *pOutputBuffer,int *OutputSampleCount)
{
    ...
    LOGV("HeadphoneCompensationFilter Channel_Num=%d, Sample_Rate=%d", mInitParam.Channel_Num, mInitParam.Sample_Rate);
...
#if 1
    LOGV("b copy whole data mACFInput_Count = %d, *InputSampleCount  = %d ", mHCFInput_Count, *InputSampleCount );

    memcpy((void*)(mpHCFInput_W), pInputBuffer , ( *InputSampleCount)<<1);
    mHCFInput_Count +=  *InputSampleCount;
    mpHCFInput_W += *InputSampleCount;//update write ptr
    LOGV("copy whole data mHCFInput_Count = %d, *InputSampleCount  = %d ", mHCFInput_Count, *InputSampleCount );

    a = mHCFInput_Count >>10;
    b = a<<10;
    InputCountLeft =mHCFInput_Count - (b); // 512 sample x 2ch
    mHCFInput_Count -= InputCountLeft;
    mpHCFInput_Wleft = mpHCFInputBuf+mHCFInput_Count;

    LOGV("Before Process InputCountLeft = %d, mHCFInput_Count = %d InputSampleCount = %d, mpHCFInputBuf = %p, mpHCFInput_W = %p, mpHCFInput_Wleft= %p, a=%d, b=%d"
         , InputCountLeft, mHCFInput_Count, *InputSampleCount, mpHCFInputBuf , mpHCFInput_W , mpHCFInput_Wleft,a, b);


    while(mHCFInput_Count >= 1024){
        int sum_input =0;
        ConsumedSampleCount = mHCFInput_Count;
        LOGV("Process mHeadphoneCompFlt InputSampleCount=%d, ConsumedBytes=%d, OutputBytes=%d", mHCFInput_Count, TotalConsumedSample, TotalOuputSample);
#ifdef HCF_Test_OnlyMemcpy
        LOGD("HCF_Test_OnlyMemcpy!!");
        //test only memcpy
        memcpy(pOutputBuffer+TotalOuputSample, mpHCFInputBuf+TotalConsumedSample, 2048);
        ConsumedSampleCount = 1024;
        *OutputSampleCount = 1024;
#else
        mpBloudHandle->Process(mpBloudHandle, NULL, mpHCFInputBuf+TotalConsumedSample, &ConsumedSampleCount, pOutputBuffer+TotalOuputSample, OutputSampleCount);
        ...
        {        
                ...
                {
                    sum_input = sum_input + *(mpHCFInputBuf+TotalConsumedSample+i);
                }
                if(sum_input == 0)
                {
                    LOGD("skip DRC result of input 0: OutputSampleCount:%d, mWorkMode=%d", *OutputSampleCount, mWorkMode);
        ...
        mHCFInput_Count -= ConsumedSampleCount;
    }
    ...
    *OutputSampleCount = TotalOuputSample;
    ...
    LOGV("after Process InputCountLeft = %d, mHCFInput_Count = %d InputSampleCount = %d, mpHCFInputBuf = %p, mpHCFInput_W = %p, mpHCFInput_Wleft= %p"
         , InputCountLeft, mHCFInput_Count, *InputSampleCount, mpHCFInputBuf , mpHCFInput_W , mpHCFInput_Wleft);
    if(InputCountLeft != 0)
    {
        //copy left data
        memcpy((void*)(mpHCFInput_W), mpHCFInput_Wleft , InputCountLeft<<1);
        mpHCFInput_W = mpHCFInputBuf+InputCountLeft;//update write ptr
    }
    mpHCFInput_Wleft = mpHCFInputBuf;

    LOGV("after copy left data InputCountLeft = %d, mHCFInput_Count = %d InputSampleCount = %d, mpHCFInputBuf = %p, mpHCFInput_W = %p, mpHCFInput_Wleft= %p"
         , InputCountLeft, mHCFInput_Count, *InputSampleCount, mpHCFInputBuf , mpHCFInput_W , mpHCFInput_Wleft);
