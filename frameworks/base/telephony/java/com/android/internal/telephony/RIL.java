...
class RILRequest {
    ...
    //***** Class Variables
    static int[] sNextSerial = new int[2];
    ...
    class RILReceiver implements Runnable {
        ...
        run() {
            ...
            try {
                for (;;) {
//MTK-START [mtk04258][20120806][International card support]
                    if (FeatureOption.EVDO_DT_SUPPORT && mStoped) return;
//MTK-END [mtk04258][20120806][International card support]   
...
/* 3G switch end */
                ...
                try {
                    Log.i (LOG_TAG, "RIL socket: " + socketRil);
                    s = new LocalSocket();
    ...
    public RIL(Context context, int preferredNetworkType, int cdmaSubscription) {
       this (context, preferredNetworkType, cdmaSubscription, Phone.GEMINI_SIM_1);
    }
...
//MTK-START [mtk04258][20120806][International card support]
	private void appointSocket() {
	    SOCKET_NAME_RIL_1 = "rild";
        SOCKET_NAME_RIL_2 = "rild2";

		/*riljLog("appointSocketFeatureOption.MTK_DT_SUPPORT = " + FeatureOption.MTK_DT_SUPPORT +
		    ", FeatureOption.EVDO_DT_SUPPORT = " + FeatureOption.EVDO_DT_SUPPORT +
		    ", FeatureOption.EVDO_DT_VIA_SUPPORT = " + FeatureOption.EVDO_DT_VIA_SUPPORT +
		    ", PhoneFactory.getExternalModemSlot() = " + PhoneFactory.getExternalModemSlot() +
		    ", GeminiPhone.isDualTalkMode() = " + GeminiPhone.isDualTalkMode());*/
		
	    if (FeatureOption.MTK_DT_SUPPORT) {
            if (PhoneFactory.getExternalModemSlot() == Phone.GEMINI_SIM_2) {
                SOCKET_NAME_RIL_1 = "rild";
                SOCKET_NAME_RIL_2 = "rild3";
            } else {
...
//MTK-END [mtk04258][20120806][International card support]
    ...
    startDtmf(char c, Message result) {
        RILRequest rr
                = RILRequest.obtain(RIL_REQUEST_DTMF_START, result, mySimId);
    ...
    stopDtmf(Message result) {
        RILRequest rr
                = RILRequest.obtain(RIL_REQUEST_DTMF_STOP, result, mySimId);
...
//MTK-END [mtk04258][20120806][International card support]
    ...
    private void switchToRadioState(RadioState newState) {
        ...
        if (mInitialRadioStateChange) {
            ...
            if ((mIs3GSwitch && mySimId == Phone.GEMINI_SIM_2) || (!mIs3GSwitch && mySimId == Phone.GEMINI_SIM_1)) {
                if (newState.isOn()) {
...
//MTK-START [mtk04258][20120806][International card support]
            ...
            case RIL_REQUEST_2SATGES_POWER_ON_CONTINUE: ret =  responseVoid(p); break;
			case RIL_REQUEST_OPERATOR_LIMIT_SERVICE: ret = responseOperator(p); break;
//MTK-END [mtk04258][20120806][International card support]
...
*/
        ...
        //MTK-END [mtk80950][120810][ALPS00XXXXXX] add UTK
            ...
            case RIL_UNSOL_NEED_QUERY_DATA_LIST: ret = responseVoid(p); break;
            //MTK-END [mtk04070][111121][ALPS00093395]MTK added
        ...
        //MTK-END [mtk80950][120810][ALPS00XXXXXX] add UTK
            ...
            case RIL_UNSOL_INCOMING_CALL_INDICATION:
                ...
                break;
            case RIL_UNSOL_NEED_QUERY_DATA_LIST:
                if (RILJ_LOGD) riljLog("RIL_UNSOL_NEED_QUERY_DATA_LIST, execute request data call list");
                getDataCallList(mDataListChangedHandler.obtainMessage(MSG_GET_DATA_CALL_LIST_DONE));
                break;
    ...
    responseOperatorInfos(Parcel p) {
        ...
        for (int i = 0 ; i < strings.length ; i += 5) {
            ...
            if (strings[i+0] != null && (strings[i+0].equals("") || strings[i+0].equals(strings[i+2]))) {
                riljLog("lookup RIL responseOperatorInfos()"); // ALPS00296353 MVNO
...
//MTK-START [mtk04258][20120806][International card support]
            ...
            case RIL_REQUEST_2SATGES_POWER_ON_CONTINUE: return "RIL_REQUEST_2SATGES_POWER_ON_CONTINUE";
            case RIL_REQUEST_OPERATOR_LIMIT_SERVICE: return "RIL_REQUEST_OPERATOR_LIMIT_SERVICE";
//MTK-END [mtk04258][20120806][International card support]
            // ADD BY via start
...
*/
        switch(request) {
            ...
            case RIL_UNSOL_NEED_QUERY_DATA_LIST: return "RIL_UNSOL_NEED_QUERY_DATA_LIST";
            //MTK-END [mtk04070][111121][ALPS00093395]MTK added
...
//MTK-START [mtk04258][20120806][International card support]
    ...
    public void continueMdInit(Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_2SATGES_POWER_ON_CONTINUE,
                response, mySimId);
    ...
    public void getLimitServiceOperator(Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_OPERATOR_LIMIT_SERVICE,
                response, mySimId);
...
//MTK-END [mtk04258][20120806][International card support]
    ...
    public void
    setUnsolOemHookRowPrefix (String[] urcPrefixes, Message result) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_UNSOL_OEM_HOOW_RAW_PREFIX,
                                        result, mySimId);

        if (RILJ_LOGD) riljLog(rr.serialString() + "> " + requestToString(rr.mRequest));
            int nLen = urcPrefixes.length;
            rr.mp.writeInt(nLen);
            for(int i = 0; i < nLen; i++) {
                rr.mp.writeString(urcPrefixes[i]);
            }
        send(rr);
    }
    ...
    */
    ...
    responseOperator(Parcel p) {
        ...
        if (response[0] != null && (response[0].equals("") || response[0].equals(response[2]))) {
            riljLog("lookup RIL responseOperator()"); // ALPS00296353 MVNO
    ...
    public String lookupOperatorName(String numeric, boolean desireLongName) {
        ...
        if (mvnoOperName == null && desireLongName) { // ALPS00296353 MVNO 
        ...
        else if (mvnoOperName == null && desireLongName == false) // ALPS00296353 MVNO
        {
            if ((numeric.equals("46000")) || (numeric.equals("46002")) || (numeric.equals("46007"))){
    ...
    private void disableVTCapability() {
        ...
        {
            if (PhoneFactory.isDualTalkMode() &&
                mySimId == PhoneFactory.getExternalModemSlot()) {
                //CDMA did not support RIL_REQUEST_DISABLE_VT_CAPABILITY.
                return;
            }
        }
    ...
    // add for Hisense start - ALPS00353644
    public void setNewMessageIndication(int mode, int mt, int bm, int ds, int bfr, Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_NEW_MESSAGE_INDICATION, response, mySimId);
        rr.mp.writeInt(5);
        rr.mp.writeInt(mode);
        rr.mp.writeInt(mt);
        rr.mp.writeInt(bm);
    ...
    public void setSmsPreStore(int preStore, Message response) {
        setNewMessageIndication(preStore, -1, -1, -1, -1, response);
    }
    
    public void setSimSmsRead(int index, Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_SIM_SMS_READ, response, mySimId);
        rr.mp.writeInt(1);      
        rr.mp.writeInt(index);
    ...
    // add for Hisense end - ALPS00353644
    ...
    public void requestAGPSSetMpcIpPort(String ip, String port, Message result) 
    {
        ...
        rr.mp.writeInt(2);
        rr.mp.writeString(ip);
        rr.mp.writeString(port);
        if (RILJ_LOGD)riljLog(rr.serialString() + "> " + requestToString(rr.mRequest)
                                + " : " + ip + ", " + port);
        send(rr);
    }
