...
public:
    APESource(
        ...
        int32_t TotalFrame, uint32_t  *table_of_contents,int sample_per_frame, int32_t frm_size
	);
...
status_t APESource::start(MetaData *) {
    ...
    LOGD("APESource::start In done 1stpos %x",mFirstFramePos);

    return OK;
}

status_t APESource::stop() {
    LOGD("APESource::stop() In");
    CHECK(mStarted);
    delete mGroup;
    mGroup = NULL;
