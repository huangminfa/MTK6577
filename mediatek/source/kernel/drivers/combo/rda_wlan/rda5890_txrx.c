		...
		struct sk_buff *skb, struct net_device *dev)
{
	...
	RDA5890_DBGLAP(RDA5890_DA_TXRX, RDA5890_DL_TRACE,
		"netif tx len %d\n", pkt_len);
	RDA5890_DBGLAP(RDA5890_DA_TXRX, RDA5890_DL_TRACE,
		"%02x %02x %02x %02x ...prot: %2x %2x ... %02x %02x %02x %02x\n",
		skb->data[0], skb->data[1], skb->data[2], skb->data[3], skb->data[14], skb->data[15],
		skb->data[skb->len - 4], skb->data[skb->len - 3],
		skb->data[skb->len - 2], skb->data[skb->len - 1]);
	...
	ret = rda5890_host_to_card(priv, pkt_data, pkt_len, DATA_REQUEST_PACKET);
	if (ret) {
