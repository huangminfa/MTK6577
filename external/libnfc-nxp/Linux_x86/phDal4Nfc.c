...
NFCSTATUS phDal4Nfc_Config(pphDal4Nfc_sConfig_t config,void **phwref)
{
   ...
   {
      char szBuf[256];
      sprintf(szBuf, "phDal4Nfc_Config (1): config[%p], phwref[%p], nClientId[%u]", config, phwref, config->nClientId);
      DAL_PRINT(szBuf);
      return NFCSTATUS_INVALID_PARAMETER;
   }
   ...
   {
      char szBuf[256];
      sprintf(szBuf, "phDal4Nfc_Config (3)");
      DAL_PRINT(szBuf);
      return retstatus;
   }
...
void phDal4Nfc_DeferredCb (void  *params)
{
    ...
    switch(*pParam)
    {
        ...
        case PHDAL4NFC_WRITE_MESSAGE:
            ...
            usleep(6000);  /* NXP advise 500us sleep required between I2C writes */
