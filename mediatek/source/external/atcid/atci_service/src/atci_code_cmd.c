...
int code_crc_handler(char* cmdline, ATOP_t at_op, char* response){
    ...
    if (access(SYSDEV, R_OK) < 0) {
        LOGE("AT_CODE_CMD " "Permission denied: %s can't be open, errno = %d", SYSDEV,errno);
        sprintf(response,"\r\nERROR\r\n\r\n");
        return 0;
    }
    ...
    if (fd<0){
        LOGE("AT_CODE_CMD " "Open %s failed, errno = %d", SYSDEV,errno);
        sprintf(response,"\r\nERROR\r\n\r\n");
        return 0;
    }

    if (stat(SYSDEV, &finfo) < 0) {
        LOGE("AT_CODE_CMD " "Stat %s failed, errno = %d", SYSDEV,errno);
        sprintf(response,"\r\nERROR\r\n\r\n");
        return 0;
    }
    ...
    do {
        if (read(fd, &buff, 1) != 1) {
            LOGE("AT_CODE_CMD " "Read %s failed, errno = %d", SYSDEV, errno);
            sprintf(response,"\r\nERROR\r\n\r\n");
            return 0;
        }
