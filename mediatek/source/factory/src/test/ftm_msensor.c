...
static void *msensor_update_iv_thread(void *priv)
{
    ...
    while(1)
	{
        ...
        len = 0;    
		len += snprintf(dat->info+len, sizeof(dat->info)+len, "shipmemt test: %s \n",status);
		len += snprintf(dat->info+len, sizeof(dat->info)+len, "status: %d\n",mag->evt.status);
		len += snprintf(dat->info+len, sizeof(dat->info)+len, "Yaw: %d\nPietch: %d\nRoll: %d\n",
			mag->evt.yaw,mag->evt.pitch,mag->evt.roll);
		len += snprintf(dat->info+len, sizeof(dat->info)+len, "Msensor Raw data: \n");
