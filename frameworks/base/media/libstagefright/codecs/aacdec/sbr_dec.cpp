            ...
            (hParametricStereoDec != NULL))
    {
        ...
        for (i = 0; i < 32; i++)
        {
            ...
            if (xoverBand > sbrDec->highSubband)
            {
				...
				//Added by HP Cheng
				//force highSubband to 32 to prevent (sbrDec->highSubband - xoverBand) to become a minus value, this causes exception
				sbrDec->highSubband = 32;

				//Added by HP Cheng, Solution2: limit xoverBand to current highSubband
				//xoverBand = (sbrDec->highSubband > 32) ? 32 : sbrDec->highSubband;
				//LOGD("xoverBand force to highSubband or 32, lowSubband:%d, sbrDec->highSubband",xoverBand,sbrDec->highSubband);				 
