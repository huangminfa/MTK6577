...
public final class GsmCallTracker extends CallTracker {
    ...
    //MTK-START [mtk04070][111118][ALPS00093395]Enable debug poll
    private static final boolean DBG_POLL = true;
    //MTK-END [mtk04070][111118][ALPS00093395]Enable debug poll
    ...
    GsmCallTracker (GSMPhone phone) {
        ...
        try {
            if (dmAgent != null) {
                ...
                isFullLock = dmAgent.getLockType();
    ...
    dial (String dialString, int clirMode, UUSInfo uusInfo) throws CallStateException {
        ...
        if (pendingMO.address == null || pendingMO.address.length() == 0
            ...
            if (!isSwitchBeforeDial) {
                if (shouldBeDialedAsECC(dialString)) {
    ...
    operationComplete() {
        ...
        if (pendingOperations == 0 && needsPoll) {
            //MTK-START [mtk04070][111118][ALPS00093395]MTK marked
            //lastRelevantPoll = obtainMessage(EVENT_POLL_CALLS_RESULT);
            //cm.getCurrentCalls(lastRelevantPoll);
            //MTK-END [mtk04070][111118][ALPS00093395]MTK marked
    ...
    hangup (GsmConnection conn) throws CallStateException {
        ...
        if (conn == pendingMO) {
            ...
            try {
                //MTK-START [mtk04070][111118][ALPS00093395]Obtain EVENT_HANG_UP_RESULT message
                cm.hangupConnection (conn.getGSMIndex(), obtainCompleteMessage(EVENT_HANG_UP_RESULT));
                //MTK-END [mtk04070][111118][ALPS00093395]Obtain EVENT_HANG_UP_RESULT message
            } catch (CallStateException ex) {
                // Ignore "connection not found"
                // Call may have hung up already
                //MTK-START [mtk04070][120103][ALPS00109412]Solve "Can't disconnect the VT call"
                //Merge from ALPS00093274
                hasPendingHangupRequest = false;
                pendingHangupRequest = 0;
                //MTK-END [mtk04070][120103][ALPS00109412]Solve "Can't disconnect the VT call"
    ...
    handleMessage (Message msg) {
        ...
        switch (msg.what) {
            ...
            //MTK-START [mtk04070][111118][ALPS00093395]MTK added and modified
            ...
            case EVENT_SWITCH_RESULT:
                ...
                if (ar.exception != null) {
                    ...
                    if (isSwitchBeforeDial) {
                        if (shouldBeDialedAsECC(delayedDialString)) {
            ...
            default:
                break;
            //MTK-END [mtk04070][111118][ALPS00093395]MTK added and modified
        }
    }
    ...
    */
    ...
    /** 
     * Check if the phone number should be dialed as ECC call.
     * @return Return true if the number is ECC number and there is no SIM inserted to SIM slot which to be dialed out or the number 
     *         is ECC number and is not special number. 
     *         Else return false.
     *
    */
    boolean shouldBeDialedAsECC(String dialString) {
        boolean bIsSimInserted = true;
        boolean bIsEccNumber = PhoneNumberUtils.isEmergencyNumber(dialString);
        boolean bIsSpecialEccNumber = PhoneNumberUtils.isSpecialEmergencyNumber(dialString);
        try {
              ITelephony iTel = ITelephony.Stub.asInterface(ServiceManager.getService(Context.TELEPHONY_SERVICE));
              bIsSimInserted = iTel.isSimInsert(phone.getMySimId());
        } catch(Exception e) {
              //Do nothing.
        }
        
        return ((bIsEccNumber && !bIsSimInserted) || (bIsEccNumber && !bIsSpecialEccNumber)); 
    }
