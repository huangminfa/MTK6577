...
protected:
    ...
    /** M: add xOffset and yOffset to support complex language @{ */
public:
    void render(SkPaint* paint, const char *text, const float* xOffset, const float* yOffset, uint32_t start, uint32_t len,
            int numGlyphs, int x, int y, uint8_t *bitmap = NULL,
            uint32_t bitmapW = 0, uint32_t bitmapH = 0);
protected:
    void render(SkPaint* paint, const char *text, const float* xOffset, const float* yOffset, uint32_t start, uint32_t len,
...
public:
    bool renderText(SkPaint* paint, const Rect* clip, const char *text, const float* xOffset, const float* yOffset, uint32_t startIndex,
            uint32_t len, int numGlyphs, int x, int y, Rect* bounds);
    /** @} */
