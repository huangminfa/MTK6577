...
public class DataAction implements Action {
    ...
    public DataAction(Context context, String mimeType, DataKind kind, long dataId, Cursor cursor) {
        ...
        String exMimeType = mContactExtention.getExtentionMimeType();
        ...
        if (mKind.actionBody != null) {
             ...
             * M:AAS
             */
            if (TextUtils.isEmpty(mBody) && OperatorUtils.isAasEnabled(kind.mAccountType)
                    && Phone.CONTENT_ITEM_TYPE.equals(kind.mimeType)) {
                mSubtitle = "";
            }
            /*
        ...
        else if (exMimeType != null && exMimeType.equals(mimeType)) {
            String number = getAsString(cursor,Data.DATA1);
            mSubtitle = mPhoneAndSubtitle.get(number);
//            int suttitile = getAsInt(cursor,Data.DATA2);
//            int i = Phone.getTypeLabelResource(suttitile);
//            Log.i(TAG,"suttitile : "+suttitile+" | i : "+i);
//            mSubtitle = mContext.getText(i).toString();
            int im = getAsInt(cursor,Data.DATA5);
            int ft = getAsInt(cursor,Data.DATA6);
            ...
            if (intent != null) {
                String name = getAsString(cursor, Contacts.DISPLAY_NAME);
