  ...
  private void updateViews() {
    ...
    playPauseReplayView.setVisibility(
        (state != State.LOADING && state != State.ERROR &&
                //!(state == State.ENDED && !canReplay) && //show end when user stopped it.
                state != State.BUFFERING && state != State.RETRY_CONNECTING &&
                !(state != State.ENDED && state != State.RETRY_CONNECTING_ERROR && !canPause))//for live streaming
        ? View.VISIBLE : View.GONE);
    if (showTimeBar) {
        screenView.setVisibility(View.VISIBLE);//always show it // comment deprecated
  ...
  public void onScrubbingEnd(int time) {
    ...
    public void setBottomPanel(boolean alwaysShow) {
        ...
        if (!alwaysShow) {//clear background
            ...
            setAudioBackground(!(getVisibility() == VISIBLE));
        }
    ...
    public State getState() {
        if (LOG) MtkLog.v(TAG, "getState() state=" + state);
        return state;
    }
