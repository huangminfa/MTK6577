...
import com.android.server.location.LocationAppLookUtility;
import com.android.server.location.LocationBindServiceSelection;
...
public class LocationManagerService extends ILocationManager.Stub implements Runnable
{
	...
	private String mNetworkLocationProviderPackageName = null;
	private String mGeocodeProviderPackageName = null;
	private GeocoderProxy mGeocodeProvider = null;
	...
	// removeTestProvider()
	LocationProviderProxy mNetworkLocationProvider = null;
	...
	private LocationAppLookUtility mLocationAppLookUtility = null;
	private LocationBindServiceSelection mLocationServiceSelection = null;
	...
	private final class Receiver implements IBinder.DeathRecipient, PendingIntent.OnFinished
	{
		...
		String mAplicationName;
		Receiver(ILocationListener listener)
		{
			mListener = listener;
			mPendingIntent = null;
			mKey = listener.asBinder();
			mAplicationName = "default";
		}
		...
		public String toString()
		{
			...
			if (mListener != null)
			{
				result = "Receiver{" + Integer.toHexString(System.identityHashCode(this)) + " Listener " + mKey + "mAplicationName: "+mAplicationName+"}";
			}
			else
			{
				result = "Receiver{" + Integer.toHexString(System.identityHashCode(this)) + " Intent " + mKey + "mAplicationName: "+mAplicationName+ "}";
			}
		...
		{
			Xlog.d(TAG,"callStatusChangedLocked() is called for ap name: "+mAplicationName+"provider: "+provider+ "status: "+status+"time stamp: "+System.currentTimeMillis());
			if (mListener != null)
			{
				try
				{
				...
				{
					Xlog.d(TAG,"callStatusChangedLocked failed listener ap name: "+mAplicationName + "Exception stack"+e);
					e.printStackTrace();
					return false;
				}
			}
			else
			{
				...
				{
					Xlog.d(TAG,"callStatusChangedLocked failed PendingIntent ap name: "+mAplicationName + "Exception stack"+e);
					e.printStackTrace();
					return false;
				}
			}
		...
		{
			Xlog.d(TAG,"callLocationChangedLocked() is called for ap name: "+mAplicationName+"location: "+location+"time stamp: "+System.currentTimeMillis());
			if (mListener != null)
			{
				// Binder b = (Binder)mKey;
				try
				{
					synchronized (this)
					{
				...
				{
					Xlog.d(TAG,"callLocationChangedLocked failed listener ap name: "+mAplicationName + "Exception stack trace "+ e);
					e.printStackTrace();
					return false;
				}
			}
			else
			{
				...
				{
					Xlog.d(TAG,"callLocationChangedLocked failed pending intent ap name: "+mAplicationName + "Exception stack trace "+ e);
					e.printStackTrace();
					return false;
				}
			}
		...
		{
			Xlog.d(TAG,"callProviderEnabledLocked() is called for ap name: "+mAplicationName+"provider: "+provider+"enabled: "+enabled+"time stamp: "+System.currentTimeMillis());
			if (mListener != null)
			{
				try
				{
	...
	{
		//Log.d(TAG,"addProvider() is called provideName: "+(provider!=null?provider.getName():"null"));
	...
	{
		Log.d(TAG,"removeProvider() is called provideName: "+(provider!=null?provider.getName():"null"));
	...
	{
		Log.d(TAG,"_loadProvidersLocked() is called");
		synchronized (mLock)
		{
			if (sProvidersLoaded)
			{
	...
	private void _loadProvidersLocked()
	{
		...
		{
			Log.e(TAG,"GPS is not supported");
		}
	...
	private void _loadNetworkLocationAndGeocoderProvider()
	{
		long startTime;
		long endTime;
		startTime  = System.currentTimeMillis();
		mNetworkLocationProviderPackageName = mLocationServiceSelection.getNetworkLocationServiceName();
		mGeocodeProviderPackageName = mLocationServiceSelection.getGeocoderServiceName();
		endTime  = System.currentTimeMillis();
		Xlog.d(TAG,"The time to get PackageName is: "+(endTime - startTime));
		PackageManager pm = mContext.getPackageManager();
		//Change for solving the CR of 273370		
		...
		{
			startTime  = System.currentTimeMillis();
			mNetworkLocationProvider = new LocationProviderProxy(mContext, LocationManager.NETWORK_PROVIDER, mNetworkLocationProviderPackageName,
			        mLocationHandler);
			addProvider(mNetworkLocationProvider);
			endTime  = System.currentTimeMillis();
			Log.d(TAG,"NetworkLocationProvider has been added to the LMS Time: "+(endTime - startTime));
		}
		else
		{
			startTime  = System.currentTimeMillis();
			Settings.Secure.putString(mContext.getContentResolver(), "ActualNetworkServiceName", mNetworkLocationProviderPackageName);
			Settings.Secure.putInt(mContext.getContentResolver(), "ActualNetworkServiceSucess", 0);
			endTime  = System.currentTimeMillis();
			Xlog.e(TAG, "Can not find the Network Location Provider Package mNetworkLocationProviderPackageName:"+(mNetworkLocationProviderPackageName!=null?mNetworkLocationProviderPackageName:"null") + "Time: "+(endTime - startTime));
		}
		if (mGeocodeProviderPackageName != null && pm.resolveService(new Intent(mGeocodeProviderPackageName), 0) != null)
		{
			startTime  = System.currentTimeMillis();
			mGeocodeProvider = new GeocoderProxy(mContext, mGeocodeProviderPackageName);
			endTime  = System.currentTimeMillis();
			Log.d(TAG,"GeocodeProvider has been added to the LMS Time: "+(endTime - startTime));
		}
		else
		{
			mGeocodeProvider = null;
			startTime  = System.currentTimeMillis();
			Settings.Secure.putString(mContext.getContentResolver(), "ActualGeocoderServiceName", mGeocodeProviderPackageName);
			Settings.Secure.putInt(mContext.getContentResolver(), "ActualGeocoderServiceSucess",0);
			endTime  = System.currentTimeMillis();
			Xlog.e(TAG, "Can not find the Geocoder Provider Package mGeocodeProviderPackageName:"+(mGeocodeProviderPackageName!=null?mGeocodeProviderPackageName:"null") + "Time: "+(endTime - startTime));
		}
	}
	...
	{
		super();
		Xlog.d(TAG, "Constructed LocationManager Service");
		mContext = context;
		mLocationAppLookUtility = new LocationAppLookUtility(context);
		mLocationServiceSelection = LocationBindServiceSelection.getSingleInstance(context);
	...
	private void initialize()
	{
		...
		// Load providers
		_loadNetworkLocationAndGeocoderProvider();
		loadProviders();
		...
		Cursor settingsCursor = resolver.query(Settings.Secure.CONTENT_URI, null, "(" + Settings.System.NAME + "=?)", new String[]
	...
	private boolean isAllowedBySettingsLocked(String provider)
	{
		...
		{
			//Xlog.d(TAG, "isAllowedBySettingsLocked provider: " + provider + "is true");
			return true;
		}
		if (mDisabledProviders.contains(provider))
		{
			//Xlog.d(TAG, "isAllowedBySettingsLocked provider: " + provider + "is false");
			return false;
		}
		// Use system settings
		ContentResolver resolver = mContext.getContentResolver();
		boolean bRet = Settings.Secure.isLocationProviderEnabled(resolver, provider);
		//Xlog.d(TAG, "isAllowedBySettingsLocked provider: " + provider + "is " + bRet);
		return bRet;
	}

	private void checkPermissionsSafe(String provider)
	{
		//Xlog.d(TAG, "checkPermissionsSafe is called provider:" + provider);
	...
	private boolean isAllowedProviderSafe(String provider)
	{
		if ((LocationManager.GPS_PROVIDER.equals(provider) || LocationManager.PASSIVE_PROVIDER.equals(provider))
		        && (mContext.checkCallingOrSelfPermission(ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED))
		{
			//Xlog.d(TAG, "isAllowedProviderSafe is called provider:" + provider + "returnning false");
			return false;
		}
		if (LocationManager.NETWORK_PROVIDER.equals(provider)
		        && (mContext.checkCallingOrSelfPermission(ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED)
		...
		//Xlog.d(TAG, "isAllowedProviderSafe is called provider:" + provider + "returnning true");
		return true;
	}
	...
	{
		//Xlog.d(TAG, "_getAllProvidersLocked is called");
	...
	{
		//Xlog.d(TAG, "_getProvidersLocked is called criteria:" + criteria + "enabledOnly:" + enabledOnly);
		ArrayList<String> out = new ArrayList<String>(mProviders.size());
		for (int i = mProviders.size() - 1; i >= 0; i--)
		{
			LocationProviderInterface p = mProviders.get(i);
	...
	private class LpCapabilityComparator implements Comparator<LocationProviderInterface>
	{
		...
		{
			return (p.supportsAltitude() ? ALTITUDE_SCORE : 0) + (p.supportsBearing() ? BEARING_SCORE : 0) + (p.supportsSpeed() ? SPEED_SCORE : 0);
		}
	...
	{
		//Xlog.d(TAG, "getBestProvider is called criteria:" + criteria + "enabledOnly:" + enabledOnly);
	...
	{
		//Xlog.d(TAG, "updateProvidersLocked() is called");
		...
		for (int i = mProviders.size() - 1; i >= 0; i--)
		{
			...
			{
				//Xlog.d(TAG, "provider " + name + "is disabling");
				updateProviderListenersLocked(name, false);
				changesMade = true;
			}
			else if (!isEnabled && shouldBeEnabled)
			{
				//Xlog.d(TAG, "provider " + name + "is enabling");
		...
		if (changesMade)
		{
			//Xlog.d(TAG, "PROVIDERS_CHANGED_ACTION intent is fired");
	...
	{
		int listeners = 0;
		//Xlog.d(TAG, "updateProviderListenersLocked() is called provider: " + provider + "enabled: " + enabled);
		LocationProviderInterface p = mProvidersByName.get(provider);
		if (p == null)
		{
			return;
	...
	private class UpdateRecord
	{
		...
		public String toString()
		{
			return "UpdateRecord{" + Integer.toHexString(System.identityHashCode(this)) + " mProvider: " + mProvider + " mUid: " + mUid +"mMinDistance: " +mMinDistance + "mMinTime: "+mMinTime + "receiver: "+mReceiver.mAplicationName+"}";
		}
	...
	private Receiver getReceiver(ILocationListener listener)
	{
		...
		if (receiver == null)
		{
			...
			//Xlog.d(TAG, "put new Receiver: " + receiver + "binder: " + binder);
			try
			{
		...
		Xlog.d(TAG, "The same Receiver is got binder: " + binder);
		return receiver;
	}
	...
	public void requestLocationUpdates(String provider, Criteria criteria, long minTime, float minDistance, boolean singleShot, ILocationListener listener)
	{
		if (criteria != null)
		{
			...
			if (provider == null)
			{
				throw new IllegalArgumentException("no provider_s found for criteria");
			}
		}
		try
		{
			synchronized (mLock)
			{
				requestLocationUpdatesLocked(provider, minTime, minDistance, singleShot, getReceiver(listener));
			}
		}
		catch (SecurityException se)
		{
	...
	public void requestLocationUpdatesPI(String provider, Criteria criteria, long minTime, float minDistance, boolean singleShot, PendingIntent intent)
	{
		...
		try
		{
			synchronized (mLock)
			{
				requestLocationUpdatesLocked(provider, minTime, minDistance, singleShot, getReceiver(intent));
			}
		}
		catch (SecurityException se)
		{
	...
	{
		LocationProviderInterface p = mProvidersByName.get(provider);
		receiver.mAplicationName = mLocationAppLookUtility.getAppName(Binder.getCallingPid());
		Xlog.d(TAG, "requestLocationUpdatesLocked is called provider:" + provider +"minTime: "+minTime+"minDistance: "+minDistance+ "singleShot: " + singleShot + "receiver: " + receiver + "callingPid: "
		       + Binder.getCallingPid() + "ap name:" + receiver.mAplicationName);
		if (p == null)
		{
			Xlog.d(TAG, "requestLocationUpdatesLocked is called provider: " +provider + "is null");
			//throw new IllegalArgumentException("provider=" + provider);
			return;
		}
		...
		try
		{
			...
			if (isProviderEnabled)
			{
				...
				{
					Xlog.d(TAG, "is calling enableLocationTracking(true) provider:" + provider);
					p.enableLocationTracking(true);
				}
			}
			else
			{
				...
				Xlog.d(TAG, "Notify the listener that updates are currently disabled");
	...
	{
		Xlog.d(TAG, "removeUpdatesLocked is called provider:" + "receiver: " + receiver + "callingPid: " + Binder.getCallingPid() + "ap name:"
		        + mLocationAppLookUtility.getAppName(Binder.getCallingPid()));
		// so wakelock calls will succeed
		final int callingUid = Binder.getCallingUid();
		long identity = Binder.clearCallingIdentity();
		try
	...
	public boolean addGpsStatusListener(IGpsStatusListener listener)
	{
		...
		try
		{
			if(listener != null)
			{
				Log.d(TAG,"addGpsStatusListener() is called ProcessName: "+ mLocationAppLookUtility.getAppName(Binder.getCallingPid()));
			}
	...
	public void removeGpsStatusListener(IGpsStatusListener listener)
	{
		synchronized (mLock)
		{
			try
			{
				Log.d(TAG,"removeGpsStatusListener() is called ProcessName: "+ mLocationAppLookUtility.getAppName(Binder.getCallingPid()));
	...
	public boolean sendExtraCommand(String provider, String command, Bundle extras)
	{
		...
		if ((mContext.checkCallingOrSelfPermission(ACCESS_LOCATION_EXTRA_COMMANDS) != PackageManager.PERMISSION_GRANTED))
		{
	...
	class ProximityAlert
	{
		...
		public String toString()
		{
			return "ProximityAlert{" + Integer.toHexString(System.identityHashCode(this)) + " uid " + mUid + mIntent + "}";
		}
	...
	{
		Log.d(TAG,"addProximityAlert is called latitude: "+latitude+"longitude: "+longitude+"radius: "+radius+"expiration: "+expiration+"ap name"+mLocationAppLookUtility.getAppName(Binder.getCallingPid())+"intent: "+intent);
	...
	{
		Xlog.d(TAG, "_getLastKnownLocationLocked is called provider: " + provider + Binder.getCallingPid() + "ap name:"
		        + mLocationAppLookUtility.getAppName(Binder.getCallingPid()));
		checkPermissionsSafe(provider);
		...
		if (loc != null)
		{
			Xlog.d(TAG, "getLastKnownLocation is called provider: " + provider + "location:" + loc);
		}
		else
		{
			Xlog.d(TAG, "getLastKnownLocation is called provider: " + provider + "location: null");
		}
		return loc;
	}
	...
	private static boolean shouldBroadcastSafe(Location loc, Location lastLoc, UpdateRecord record)
	{
		...
		Xlog.d(TAG, "shouldBroadcastSafe() is called loc: "+loc+"lastLoc: "+lastLoc+"record: "+record);
		if (lastLoc == null)
		{
			Xlog.d(TAG, "shouldBroadcastSafe() is returnning true due to lastLoc is null");
			return true;
		}
		...
		{
			Xlog.d(TAG, "shouldBroadcastSafe() is returnning false due to the same time");
			return false;
		}
		...
		if (minDistance > 0.0)
		{
			double realDistance = loc.distanceTo(lastLoc);
			if (realDistance <= minDistance)
			{
				Xlog.d(TAG, "shouldBroadcastSafe() is returnning false due to the distance realDistance:"+realDistance+"minDistance:"+minDistance);
				return false;
			}
		}
		return true;
	...
	private void handleLocationChangedLocked(Location location, boolean passive)
	{
		...
		Xlog.d(TAG, "handleLocationChangedLocked is called provider:" + provider + "location: " + location);
		LocationProviderInterface p = mProvidersByName.get(provider);
		if (p == null)
		{
			return;
		...
		{		
			...
			{
				Log.d(TAG, "location has been blocked "+location.toString());
				return;
			}
		}
		...
		if (lastLocation == null)
		{
			...
			Xlog.d(TAG, "handleLocationChangedLocked is called frist fix is sucess provider:" + provider + "location:" + location);
		}
		else
		{
		...
		for (int i = 0; i < N; i++)
		{
			...
			if ((lastLoc == null) || shouldBroadcastSafe(location, lastLoc, r))
			{
				...
				{
					Xlog.d(TAG, "RemoteException calling onLocationChanged on receiver :" + receiver.mAplicationName );
					receiverDead = true;
				}
			}

			long prevStatusUpdateTime = r.mLastStatusBroadcast;
			Xlog.d(TAG,"receiver:"+receiver.mAplicationName+"provider:" +provider+ "newStatusUpdateTime:"+newStatusUpdateTime+"prevStatusUpdateTime:"+prevStatusUpdateTime+"status: "+status);
			...
			try
			{
				if (msg.what == MESSAGE_LOCATION_CHANGED)
				{
					synchronized (mLock)
					{
						...
						Xlog.d(TAG, "MESSAGE_LOCATION_CHANGED is send provider:" + provider + "location:" + location + "passive:" + passive + "mProviders:"
						        + mProviders);
						if (!passive)
						{
				...
				else if (msg.what == MESSAGE_PACKAGE_UPDATED)
				{
					...
					Xlog.d(TAG, "MESSAGE_PACKAGE_UPDATED is send packageName:" + packageName);
					// reconnect to external providers after their packages have
					// been updated
					if (mNetworkLocationProvider != null && mNetworkLocationProviderPackageName != null
					        && mNetworkLocationProviderPackageName.startsWith(packageDot))
					{
						Xlog.d(TAG, "mNetworkLocationProvider is reconnecting");
						mNetworkLocationProvider.reconnect();
					}
					if (mGeocodeProvider != null && mGeocodeProviderPackageName != null && mGeocodeProviderPackageName.startsWith(packageDot))
					{
						Xlog.d(TAG, "mGeocodeProvider is reconnecting");
	...
	private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver()
	{
		...
		{
			Xlog.d(TAG, "Location Manager Service mBroadcastReceiver onReceive: " + intent);
			String action = intent.getAction();
			boolean queryRestart = action.equals(Intent.ACTION_QUERY_PACKAGE_RESTART);
			if (queryRestart || action.equals(Intent.ACTION_PACKAGE_REMOVED) || action.equals(Intent.ACTION_PACKAGE_RESTARTED)
			...
			else if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION))
			{
				...
				Xlog.d(TAG, "ConnectivityManager.CONNECTIVITY_ACTION is triggered NetworkInfo: " + info + "Thread_Name: " + "Thread_Name:"
			...
			else if (FeatureOption.MTK_IPO_SUPPORT == true)
			{
				if (action.equals("android.intent.action.ACTION_SHUTDOWN_IPO"))
				{
					Xlog.d(TAG, "Location Manager Service is trying to close after SHUTDOWN_IPO");
				}
				else if (action.equals("android.intent.action.ACTION_BOOT_IPO"))
				{
					Xlog.d(TAG, "Location Manager Service is trying to reconnect after BOOT_IPO");
					_loadNetworkLocationAndGeocoderProvider();
				}
			}
		}
	...
	private final PackageMonitor mPackageMonitor = new PackageMonitor()
	{
		...
		{
			Xlog.d(TAG, "PackageMonitor onPackageUpdateFinished is called");
	...
	public String getFromLocation(double latitude, double longitude, int maxResults, GeocoderParams params, List<Address> addrs)
	{
		...
		Xlog.d(TAG, "getFromLocation is called " + "geocoderIsPresent:" + (geocoderIsPresent() == true ? "true" : "false") + "latitude:" + latitude
		        + "longitude:" + longitude + "callingPid: " + Binder.getCallingPid() + "ap name:" + mLocationAppLookUtility.getAppName(Binder.getCallingPid()));
		if (mGeocodeProvider != null)
		{
			retString = mGeocodeProvider.getFromLocation(latitude, longitude, maxResults, params, addrs);
			Xlog.d(TAG, "getFromLocation is called retString:" + (retString != null ? retString : "null") + "addrs:" + addrs);
		}
		return retString;
	}
	...
	{
		Xlog.d(TAG, "getFromLocationName is called " + "geocoderIsPresent:" + (geocoderIsPresent() == true ? "true" : "false") + "locationName:" + locationName
		        + "callingPid: " + Binder.getCallingPid() + "ap name:" + mLocationAppLookUtility.getAppName(Binder.getCallingPid()));
		if (mGeocodeProvider != null)
		{
			return mGeocodeProvider.getFromLocationName(locationName, lowerLeftLatitude, lowerLeftLongitude, upperRightLatitude, upperRightLongitude,
	...
	public void addTestProvider(String name, boolean requiresNetwork, boolean requiresSatellite, boolean requiresCell, boolean hasMonetaryCost,
	        boolean supportsAltitude, boolean supportsSpeed, boolean supportsBearing, int powerRequirement, int accuracy)
	{
		...
		synchronized (mLock)
		{
			MockProvider provider = new MockProvider(name, this, requiresNetwork, requiresSatellite, requiresCell, hasMonetaryCost, supportsAltitude,
