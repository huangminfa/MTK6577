...
#undef LOGD
#define LOGD(...) if (inputLogEnabled) __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, __VA_ARGS__)
...
void InputDispatcher::releaseInboundEventLocked(EventEntry* entry) {
    ...
    if (injectionState && injectionState->injectionResult == INPUT_EVENT_INJECTION_PENDING) {
        ...
        LOGI("Injected inbound event was dropped.");
#endif
...
int32_t InputDispatcher::handleTargetsNotReadyLocked(nsecs_t currentTime,
    ...
    if (applicationHandle == NULL && windowHandle == NULL) {
        ...
        if (mInputTargetWaitCause != INPUT_TARGET_WAIT_CAUSE_APPLICATION_NOT_READY) {
            ...
            if(LightWeight_KeyDispatchAnr::IS_ENG_BUILD)
            {
                static int tmpWinPid = -1;
                static int tmpAppPid = -1;
                if(windowHandle != NULL)
                {
                    const InputWindowInfo* windowInfo = windowHandle->getInfo();
                    ...
                    if(windowInfo!=NULL) 
                    {
                        if(tmpWinPid != windowInfo->ownerPid && windowInfo->ownerPid > 0)
                        {
                            LOGI("Focus window's pid=%d", windowInfo->ownerPid);
                            tmpWinPid = windowInfo->ownerPid;
                        }
                        LightWeight_KeyDispatchAnr::curPid = windowInfo->ownerPid;  
                        pthread_mutex_lock(&LightWeight_KeyDispatchAnr::mut_keydispatch_anr_start);
                        pthread_cond_signal(&LightWeight_KeyDispatchAnr::cond_keydispatch_anr_start);
                        pthread_mutex_unlock(&LightWeight_KeyDispatchAnr::mut_keydispatch_anr_start);
                        /*
                        if(windowInfo->ownerPid != getpid())
                        {
                            LightWeight_KeyDispatchAnr::curPid = getpid();
                            pthread_mutex_lock(&LightWeight_KeyDispatchAnr::mut_keydispatch_anr_start);
                            pthread_cond_signal(&LightWeight_KeyDispatchAnr::cond_keydispatch_anr_start);
                            pthread_mutex_unlock(&LightWeight_KeyDispatchAnr::mut_keydispatch_anr_start);
                        }
                ...
                {
                    //LOGD("LightWeight: windowHandle is NULL");
                    //LightWeight_KeyDispatchAnr::curPid = -1;
                    if(mFocusedApplicationHandle!=NULL)
                    {
                        if(mFocusedApplicationHandle->getInfo()!= NULL && mFocusedApplicationHandle->getInfo()->pid != -1)
                        {
                            if(tmpAppPid != mFocusedApplicationHandle->getInfo()->pid)
                            {
                                LOGI("FocusedApplication pid=%d", mFocusedApplicationHandle->getInfo()->pid);
                                tmpAppPid = mFocusedApplicationHandle->getInfo()->pid;
                            }
                            LightWeight_KeyDispatchAnr::curPid = mFocusedApplicationHandle->getInfo()->pid;
                            pthread_mutex_lock(&LightWeight_KeyDispatchAnr::mut_keydispatch_anr_start);
                            pthread_cond_signal(&LightWeight_KeyDispatchAnr::cond_keydispatch_anr_start);
                            pthread_mutex_unlock(&LightWeight_KeyDispatchAnr::mut_keydispatch_anr_start);
                            /*
                            if(mFocusedApplicationHandle->getInfo()->pid != getpid())
                            {
                                LightWeight_KeyDispatchAnr::curPid = getpid();
                                pthread_mutex_lock(&LightWeight_KeyDispatchAnr::mut_keydispatch_anr_start);
                                pthread_cond_signal(&LightWeight_KeyDispatchAnr::cond_keydispatch_anr_start);
                        ...
                        {
                            LOGE("InputDispatcher Can't get focus application's pid ");
                        }
...
int32_t InputDispatcher::findFocusedWindowTargetsLocked(nsecs_t currentTime,
    ...
    if (mFocusedWindowHandle == NULL) {
        if (mFocusedApplicationHandle != NULL) {
#if DEBUG_FOCUS
            LOGI("Waiting because there is no focused window but there is a "
...
#if DEBUG_FOCUS
        LOGI("Waiting because focused window is paused.");
#endif
        injectionResult = handleTargetsNotReadyLocked(currentTime, entry,
...
#if DEBUG_FOCUS
        LOGI("Waiting because focused window still processing previous input.");
#endif
        injectionResult = handleTargetsNotReadyLocked(currentTime, entry,
                mFocusedApplicationHandle, mFocusedWindowHandle, nextWakeupTime);
        goto Unresponsive;
...
int32_t InputDispatcher::findTouchedWindowTargetsLocked(nsecs_t currentTime,
    ...
    if (newGesture || (isSplit && maskedAction == AMOTION_EVENT_ACTION_POINTER_DOWN)) {
...
#if DEBUG_FOCUS
            LOGI("Waiting because system error window is pending.");
#endif
            injectionResult = handleTargetsNotReadyLocked(currentTime, entry,
        ...
        if (newTouchedWindowHandle == NULL) {
            if (mFocusedApplicationHandle != NULL) {
#if DEBUG_FOCUS
                LOGI("Waiting because there is no touched window but there is a "
...
#if DEBUG_FOCUS
                LOGI("Waiting because touched window is paused.");
#endif
                injectionResult = handleTargetsNotReadyLocked(currentTime, entry,
...
void InputDispatcher::prepareDispatchCycleLocked(nsecs_t currentTime,
    ...
    if (connection->status != Connection::STATUS_NORMAL) {
#if DEBUG_DISPATCH_CYCLE
        LOGI("channel '%s' ~ Dropping event because the channel status is %s",
...
void InputDispatcher::enqueueDispatchEntryLocked(
...
#if DEBUG_DISPATCH_CYCLE
            LOGI("channel '%s' ~ enqueueDispatchEntryLocked: skipping inconsistent key event",
                    connection->getInputChannelName());
#endif
    ...
    case EventEntry::TYPE_MOTION: {
...
#if DEBUG_DISPATCH_CYCLE
        LOGI("channel '%s' ~ enqueueDispatchEntryLocked: filling in missing hover enter event",
                connection->getInputChannelName());
#endif
            dispatchEntry->resolvedAction = AMOTION_EVENT_ACTION_HOVER_ENTER;
        }
        ...
        if (!connection->inputState.trackMotion(motionEntry,
                dispatchEntry->resolvedAction, dispatchEntry->resolvedFlags)) {
#if DEBUG_DISPATCH_CYCLE
            LOGI("channel '%s' ~ enqueueDispatchEntryLocked: skipping inconsistent motion event",
                    connection->getInputChannelName());
#endif
            delete dispatchEntry;
            return; // skip the inconsistent event
    ...
    case EventEntry::TYPE_MOTION: {
        ...
        if (dispatchEntry->resolvedAction == AMOTION_EVENT_ACTION_MOVE
            ...
            for (; nextMotionSample != NULL; nextMotionSample = nextMotionSample->next) {
                ...
                if (status == NO_MEMORY) {
#if DEBUG_DISPATCH_CYCLE
                    LOGI("channel '%s' ~ Shared memory buffer full.  Some motion samples will "
...
#if DEBUG_DISPATCH_CYCLE
    LOGI("channel '%s' ~ abortBrokenDispatchCycle - notify=%s",
...
void InputDispatcher::dump(String8& dump) {
    ...
    property_get("ro.monkey", buf, "false");
    mIsMonkeyEnabled = false;
    if (!strcmp(buf,"true")) {
        mIsMonkeyEnabled = true;
        LOGI("ro.monkey : %s", toString(mIsMonkeyEnabled));
    }
}
