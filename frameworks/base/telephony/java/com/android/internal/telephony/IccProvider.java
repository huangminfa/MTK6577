...
//MTK-END [mtk80601][111215][ALPS00093395]
    ...
    public Cursor query(Uri url, String[] projection, String selection,
            ...
            switch (match) {
                ...
                case SDN: /* default SIM*/
                case SDN1:
                case SDN2:
                    efType = IccConstants.EF_SDN;
                    if (match == SDN) {
                        simId = getDefaultSim();
                    } else if (match == SDN2) {
                        simId = Phone.GEMINI_SIM_2;
    ...
    public Uri insert(Uri url, ContentValues initialValues) {
        ...
        if (UPB == match || UPB1 == match || UPB2 == match) {
            ...
            if (ADDRESS_BOOK_COLUMN_NAMES.length >= ADDRESS_SUPPORT_AAS) {
                ...
                result = updateUsimPBRecordsBySearchWithError(efType, new AdnRecord("", "", ""), record, simId);
            } else {
...
/*            String[] pair = param.split("=");
    ...
    public int update(Uri url, ContentValues values, String where, String[] whereArgs) {
        ...
        if (UPB == match || UPB1 == match || UPB2 == match) {
            ...
            if(index > 0){
                ...
                if (ADDRESS_BOOK_COLUMN_NAMES.length >= ADDRESS_SUPPORT_AAS) {
                    result = updateUsimPBRecordsBySearchWithError(efType, new AdnRecord(tag, number, ""), record, simId);
                } else {
                    result = updateUsimRecordInEf(efType, tag, number, newTag, newNumber, strAnr, strEmail, simId);
                }
    ...
    private int
    addIccRecordToEf(int efType, String name, 
...
//MTK-START [mtk80601][111215][ALPS00093395]
    ...
    private int
    addUsimRecordToEf(int efType, String name, String number, String strAnr,  String strEmail, String strGas, int simId) {
        
        if (DBG) log("addUSIMRecordToEf: efType=" + efType + ", name=" + name +
                ", number=" + number +  ", anr =" + strAnr + ", emails=" + strEmail + ", simId=" + simId);
    ...
    private int
    updateIccRecordInEfByIndex(int efType, int nIndex, String newName, 
                                     String newNumber, String pin2, int simId) {
        if (DBG) log("updateIccRecordInEfByIndex: efType=" + efType + ", index=" + nIndex 
			                    + ", newname=" + newName + ", newnumber=" + newNumber);
        int result = ERROR_ICC_PROVIDER_UNKNOWN;
    ...
    private int
    updateUsimRecordInEf(int efType, String oldName, String oldNumber,
            String newName, String newNumber, String strAnr,  String strEmail, int simId) {
            
        if (DBG) log("updateUsimRecordInEf: efType=" + efType +
                ", oldname=" + oldName + ", oldnumber=" + oldNumber +
                ", newname=" + newName + ", newnumber=" + newNumber  + ", anr =" + strAnr + ", emails=" + strEmail );
        int result = ERROR_ICC_PROVIDER_UNKNOWN;
	 ...
	 if (strEmail != null) {
        ...
        try {
            ...
            if (iccIpb != null) {
   	         result = iccIpb.updateUsimPBRecordsInEfBySearchWithError (efType,
            oldName, oldNumber, "", null, null, newName, newNumber, strAnr, null, emails);	  
            }
        } catch (RemoteException ex) {
            // ignore it
        } catch (SecurityException ex) {
    ...
    private int
    updateUsimRecordInEfByIndex(int efType, int nIndex, String newName, String newNumber, 
                                String strAnr,  String strEmail, int simId) {
            
        if (DBG) log("updateUsimRecordInEfByIndex: efType=" + efType + ", Index=" + nIndex + ", newname=" + newName + 
...
//MTK-START [mtk80601][111215][ALPS00093395]
    private int deleteIccRecordFromEfByIndex(int efType, int nIndex, String pin2, int simId) {
        if (DBG) log("deleteIccRecordFromEfByIndex: efType=" + efType +
    ...
    private int deleteUsimRecordFromEf(int efType, String name, 
        ...
        if (DBG) log("deleteUsimRecordFromEf: efType=" + efType +
    ...
    private int deleteUsimRecordFromEfByIndex(int efType, int nIndex, int simId) {
        if (DBG) log("deleteUsimRecordFromEfByIndex: efType=" + efType + ", index=" + nIndex );
...
//MTK-END [mtk80601][111215][ALPS00093395]
    ...
    private void loadRecord(AdnRecord record, MatrixCursor cursor, int id) {
        ...
        if (!record.isEmpty()) {
            ...
            int aas = record.getAasIndex();
            int aasIndex = -1;
            
            if (len >= ADDRESS_SUPPORT_AAS) {
                aasIndex = record.getAasIndex();
                contact[7] = aasIndex;
            }
            //if (DBG) log("loadRecord: " + alphaTag + ", " + number + ",");
            if (DBG) log("loadRecord: alphaTag:" + alphaTag + ", number:" + number + ",anr:" + anr + ",aasIndex:" + aasIndex + ",grpId:" + grpIds + ",aas:" + aas);
...
//MTK-START [mtk80601][111215][ALPS00093395]
    ...
    private int updateUsimPBRecordsBySearchWithError(int efType, AdnRecord oldAdn, AdnRecord newAdn, int simId) {
        if (DBG) log("updateUsimRecordBySearch simId:" + simId + ",oldAdn:" + oldAdn + ",newAdn:" + newAdn);
        int result = ERROR_ICC_PROVIDER_UNKNOWN;
    ...
    private int updateUsimPBRecordsByIndexWithError(int efType, AdnRecord newAdn, int index, int simId) {
        if (DBG) log("updateUsimRecordByIndex simId:" + simId + ",index:" + index + ",newAdn:" + newAdn);
        int result = ERROR_ICC_PROVIDER_UNKNOWN;
