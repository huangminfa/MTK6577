...
void Font::drawCachedGlyph(CachedGlyphInfo* glyph, int x, int y,
    ...
    int nPenX = x + glyph->mBitmapLeft;
    int nPenY = y + glyph->mBitmapTop;
    if (nPenX >= (int32_t) bitmapW || nPenY >= (int32_t) bitmapH) {
        LOGE("Exceed bitmap size %dx%d: (%d, %d)", bitmapW, bitmapH, nPenX, nPenY);
        return;
    }
...
void FontRenderer::checkTextureUpdate() {
    ...
    dumpAlphaTexture(mCacheWidth, mCacheHeight, mTextTexture, "FontTexture");
#endif
    mUploadTexture = false;
}
...
FontRenderer::DropShadow FontRenderer::renderDropShadow(SkPaint* paint, const char *text,
    ...
    if (paddedWidth > (uint32_t) mMaxTextureSize) {
        LOGW("Shadow width too large to be fully uploaded into a texture");
        paddedWidth = mMaxTextureSize;
    }
     if (paddedHeight > (uint32_t) mMaxTextureSize) {
        LOGW("Shadow height too large to be fully uploaded into a texture");
...
/** M: add xOffset and yOffset for complex language text layout @{ */


void Font::render(SkPaint* paint, const char* text, const float* xOffset, const float* yOffset, uint32_t start, uint32_t len,
        int numGlyphs, int x, int y, RenderMode mode, uint8_t *bitmap,
        uint32_t bitmapW, uint32_t bitmapH,Rect *bounds) {
    if (numGlyphs == 0 || text == NULL || len == 0) {
        return;
    ...
    while (glyphsLeft > 0) {
        ...
        if (cachedGlyph->mIsValid) {
            switch(mode) {
            case FRAMEBUFFER:
                drawCachedGlyph(cachedGlyph, (int) floorf(newX), newY);
                break;
            case BITMAP:
                drawCachedGlyph(cachedGlyph, (int) floorf(newX), newY, bitmap, bitmapW, bitmapH);
                break;
            case MEASURE:
                measureCachedGlyph(cachedGlyph, (int) floorf(newX), newY, bounds);
                break;
...
void Font::render(SkPaint* paint, const char* text, const float* xOffset, const float* yOffset, uint32_t start, uint32_t len,
    ...
    if (bitmap != NULL && bitmapW > 0 && bitmapH > 0) {
        render(paint, text, xOffset, yOffset, start, len, numGlyphs, x, y, BITMAP, bitmap,
                bitmapW, bitmapH, NULL);
    } else {
        render(paint, text, xOffset, yOffset, start, len, numGlyphs, x, y, FRAMEBUFFER, NULL,
                0, 0, NULL);
    }
...
bool FontRenderer::renderText(SkPaint* paint, const Rect* clip, const char *text, const float* xOffset, const float* yOffset, uint32_t startIndex,
    ...
    mBounds = bounds;
    mClip = clip;
    mCurrentFont->render(paint, text, xOffset, yOffset, startIndex, len, numGlyphs, x, y);
    mBounds = NULL;
