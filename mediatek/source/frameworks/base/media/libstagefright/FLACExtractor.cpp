...
class FLACSource : public MediaSource, public TableOfContentThread 
{
      ...
      mMetaInfo(MetaInfo)
{
    ...
    mCurrentFrm = 0;
    mCurrentfrmTime = 0;
    mSeekTableValid = Parser->mSeekTableValid;    
        ...
        MediaBuffer **outBuffer, const ReadOptions *options)
{
    *outBuffer = NULL;
    ...
    if (options != NULL && options->getSeekTo(&seekTimeUs, &mode)) 
    {
        if(mSeekTableValid)
        {
            ...
            if(mParser->parser_getseektable((FLAC__uint64 *)&sample, (FLAC__uint64 *)&ptmpPos))
            {
                ...
                LOGD("buildin seektable seektime=%lld,actual=%lld, filePos=%ld",seekTimeUs, seektmpTimeUs, mCurrentPos);
    ...
    else
    {
        ...
        LOGD("FLACSource::find(%lld) syncpos %d read fail %d",filesize, mCurrentPos, frame_size);
    ...
    *outBuffer = buffer;
    mCurrentFrm++;
    LOGV("FLACSource::kKeyTime Cur 0x%08x, frm %d, frm_size %d", mCurrentPos, mCurrentFrm, frame_size);
    return OK;
}
