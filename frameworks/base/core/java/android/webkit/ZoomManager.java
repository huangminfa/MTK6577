...
class ZoomManager {
    ...
    public void animateZoom(Canvas canvas) {
        ...
        /// M: keep the translate offset
        mZoomOffset.set(tx, ty);
        if (mHardwareAccelerated) {
    ...
    public void handleDoubleTap(float lastTouchX, float lastTouchY) {
        ...
        if (!isDoubleTapEnabled()) {
            if(!mWebView.isEmailUsing()) {
                // default behavior of WebView
                return;
            }
            // the Email application will use double-tap without wide-view-port
        }
    ...
    public void onSizeChanged(int w, int h, int ow, int oh) {
        ...
        /// M: fix CRALPS00312730, ALPS00324490, the NEW_PICTURE_MSG_ID cannot be done first. @{
        ...
        if (bBaiduURL == true && mWebView.mPrivateHandler.hasMessages(WebView.NEW_PICTURE_MSG_ID) == true)
            mWebView.mPrivateHandler.removeMessages(WebView.NEW_PICTURE_MSG_ID);
        /// @}
    ...
    private class PostScale implements Runnable {
        ...
        public void run() {
            if (mWebView.getWebViewCore() != null) {
                ...
                if (mWebView.getBlockWebkitViewMessages() == true) {
                    mWebView.invalidate();
                }
                /// @}
            }
    ...
    public void onNewPicture(WebViewCore.DrawData drawData) {
        ...
        if (!mMinZoomScaleFixed || settings.getUseWideViewPort()) {
            /// M: fix ALPS00323727, ALPS00324490, do not modify min and max zoom scale if the two value are the same. @{
            if (mMinZoomScale == mMaxZoomScale && mMinZoomScaleFixed == true) {
                float newZoomScale = newZoomOverviewScale;
                if (mWebView != null && mWebView.getWebViewCore() != null && mWebView.getWebViewCore().getViewportInitialScale() > 0) {
                    newZoomScale = mWebView.getWebViewCore().getViewportInitialScale() * 0.01f;
                    mTextWrapScale = newZoomScale;
                }
                mMaxZoomScale = mMinZoomScale = newZoomScale;//newZoomOverviewScale;
        ...
        if (!mWebView.drawHistory() &&
            ...
            mInitialZoomOverview = false;
            /// M: fix ALPS00324490, initial-scale issue. @{
            float newZoomScale = newZoomOverviewScale;
            if (mWebView != null && mWebView.getWebViewCore() != null && mWebView.getWebViewCore().getViewportInitialScale() > 0) {
                newZoomScale = mWebView.getWebViewCore().getViewportInitialScale() * 0.01f;
                mTextWrapScale = newZoomScale;
    ...
    /**
     * M: There is a translate offset while animateZoom
     * And it caused TitleBar drawing in wrong position
     * So we keep the offset and give it to WebView when drawing TitleBar
     */
    private Point mZoomOffset = new Point(0, 0);
    /* package*/ Point getZoomOffset() {
    	return mZoomOffset;
    }
}
