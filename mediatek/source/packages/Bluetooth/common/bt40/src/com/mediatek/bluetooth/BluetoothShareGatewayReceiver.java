...
//import com.mediatek.bluetooth.bip.BipInitEntry;
...
/*
			Log.v(TAG, "SEND broadcast to gateway");
			Intent inBSG = new Intent(BluetoothShareGatewayActivity.ACTION_DEVICE_SELECTED);
			inBSG.putExtra(BluetoothShareGatewayActivity.EXTRA_DEVICE_ADDRESS, remoteDevice);
			inBSG.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS);
			//context.sendBroadcast(inBSG);
			context.startActivity(inBSG);
			*/
	...
	private void profileDispatcher(Context context, BluetoothDevice remoteDevice){
		...
		else
		{
			...
			{
				Toast.makeText( context, R.string.bt_base_profile_feature_disabled, Toast.LENGTH_SHORT ).show();
				return;
			}
