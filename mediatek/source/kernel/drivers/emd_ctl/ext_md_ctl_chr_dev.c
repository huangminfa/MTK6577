...
#define EXT_MD_IOCTL_DOWNLOAD_MODE	_IO(EXT_MD_IOC_MAGIC, 4)
#define EXT_MD_IOCTL_DOWNLOAD_DONE	_IO(EXT_MD_IOC_MAGIC, 5)
...
extern void cm_release_rst_signal(void);
extern void cm_hold_dl_pin(void);
extern void cm_release_dl_pin(void);
...
static long emd_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case EXT_MD_IOCTL_REQUEST_RESET:
		if(2 == client->sub_dev_id){ // Only /dev/ext_md_ctl2 has the ablility of request modem reset
	...
	case EXT_MD_IOCTL_DOWNLOAD_MODE:
		if(0 == client->sub_dev_id){ // Only /dev/ext_md_ctl0 has the ablility of download modem
			...
			msleep(EMD_RST_LOW_TIME);
			check_drv_rdy_to_rst();
			cm_release_rst_signal();
		}
		break;

	case EXT_MD_IOCTL_DOWNLOAD_DONE:
		if(0 == client->sub_dev_id){ // Only /dev/ext_md_ctl0 has the ablility of download modem
			cm_release_dl_pin();
			cm_hold_rst_signal();
			// after this, emd_daemon should be started, and does the modem boot flow again
		}
		break;
