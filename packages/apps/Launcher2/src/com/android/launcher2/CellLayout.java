...
public class CellLayout extends ViewGroup {
    ...
    static int widthInPortrait(Resources r, int numCells) {
        ...
        int cellWidth = (int) (r.getDimensionPixelSize(R.dimen.workspace_cell_width) / r.getDisplayMetrics().density);
        int minGap = Math.min((int) (r.getDimensionPixelSize(R.dimen.workspace_width_gap) / r.getDisplayMetrics().density),
                (int) (r.getDimensionPixelSize(R.dimen.workspace_height_gap) / r.getDisplayMetrics().density));
    ...
    static int heightInLandscape(Resources r, int numCells) {
        ...
        // have. We ignore the left/right padding on CellLayout because it turns out in our design
        // the padding extends outside the visible screen size, but it looked fine anyway.
        int cellHeight = (int) (r.getDimensionPixelSize(R.dimen.workspace_cell_height) / r.getDisplayMetrics().density);
        int minGap = Math.min((int) (r.getDimensionPixelSize(R.dimen.workspace_width_gap) / r.getDisplayMetrics().density),
                (int) (r.getDimensionPixelSize(R.dimen.workspace_height_gap) / r.getDisplayMetrics().density));
    ...
    void onDragEnter() {
        if (LauncherLog.DEBUG_DRAG) {
            LauncherLog.d(TAG, "onDragEnter: mDragging = " + mDragging + ",mCrosshairsAnimator = " + mCrosshairsAnimator);
        }
        if (!mDragging) {
