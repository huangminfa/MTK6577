...
public abstract class GroupContent {
    //==============================================================
    // Constants                                                    
    //==============================================================
...
//            }
            ...
            for (int i = 0; i < groups.size(); i++) {
                ...
                String account_name =groups.get(i).getAccount_name();                
                if(null!=account_name )
                values[i].put(Groups.ACCOUNT_NAME,account_name );
                
                String account_type =groups.get(i).getAccount_type();                
                if(null!=account_type )
                values[i].put(Groups.ACCOUNT_TYPE,account_type );
                ...
                String group_visible=groups.get(i).getGroup_visible();                
                if(null!=group_visible )
                values[i].put(Groups.GROUP_VISIBLE,group_visible );
                
                String should_sync=groups.get(i).getShould_sync();                
                if(null!=should_sync )
                values[i].put(Groups.SHOULD_SYNC,should_sync );              
            }
        }
        return values;
    }
    ...
    public static ContentValues groupToValues(Group group) {
        ...
        if (null != group) {
            ...
            String account_name = group.getAccount_name();
            if (null != account_name)
                values.put(Groups.ACCOUNT_NAME, account_name);

            String account_type = group.getAccount_type();
            if (null != account_type)
                values.put(Groups.ACCOUNT_TYPE, account_type);
            ...
            String group_visible = group.getGroup_visible();
            if (null != group_visible)
                values.put(Groups.GROUP_VISIBLE, group_visible);

            String should_sync = group.getShould_sync();
            if (null != should_sync)
                values.put(Groups.SHOULD_SYNC, should_sync);
