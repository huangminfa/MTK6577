...
public class DmEntry extends PreferenceActivity {
    private final static String TAG = "DM/Entry";
    ...
    protected void onDestroy() {
        ...
        if (DmService.getServiceInstance()!=null)
        {
            ...
            DmService.getServiceInstance().unregisterListener(entryListener);
        }
        super.onDestroy();
    }
    ...
    protected void onStart() {
        super.onStart();
        mStatus=getSWUpdateStatus();
        Log.i(TAG, "onStart the status is "+mStatus);
        updateUI();
    }
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        if (Options.UseSmsRegister
                && DmCommomFun.getSimRegisterID(mContext) == -1) {
            showDialog(DIALOG_SIMNOTREGISTER);
        } else if (key.equals(UPDATE_PREFERENCE)) {
            Log.i(TAG, "System update key clicked! status is "+mStatus);
            if (mStatus == IDmPersistentValues.STATE_PAUSEDOWNLOAD ) {
                showDialog(DIALOG_COVEROLDPKG);
            } else if (mStatus == IDmPersistentValues.STATE_DLPKGCOMPLETE) {
    ...
    {
        Log.i(TAG, "System update stauts is : " + mStatus+" dm status is "+dmStatus);
        ...
        if (mStatus == IDmPersistentValues.STATE_PAUSEDOWNLOAD ) {
            mUpdatePreference.setSummary(
                getResources().getString(R.string.download_status) + " " + dlVersion);
        } else if (mStatus ==  IDmPersistentValues.STATE_DOWNLOADING) {
        ...
        if (dmStatus==IDmPersistentValues.STATE_NIASESSION_START)
        {
            mUpdatePreference.setSummary("The nia DM session in going");
            mUpdatePreference.setEnabled(false);
        }
        else if (dmStatus==IDmPersistentValues.STATE_NIASESSION_CANCLE
        ...
        //Set install status
        if (mStatus == IDmPersistentValues.STATE_DLPKGCOMPLETE) {
            mInstallPreference.setEnabled(true);
            mInstallPreference.setSummary(
                getResources().getString(R.string.install_version) + " " + dlVersion);
        } else {
    ...
    private ServiceConnection mConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            if (DmService.getServiceInstance()!=null&&entryListener!=null)
                DmService.getServiceInstance().registListener(entryListener);
