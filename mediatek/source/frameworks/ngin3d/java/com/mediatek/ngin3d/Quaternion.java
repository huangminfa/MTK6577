...
public class Quaternion {
    ...
    public Vector3D getAxis() {
        ...
        if (squaredSine == 0) {
            ...
            return new Vector3D(mQ1 * inverse, mQ2 * inverse, mQ3 * inverse);
        }
    }
    ...
    public float getAxisAngle() {
        if ((mQ0 < -0.1) || (mQ0 > 0.1)) {
            return 2 * (float) Math.toDegrees(Math.asin(Math.sqrt(mQ1 * mQ1 + mQ2 * mQ2 + mQ3 * mQ3)));
        } else {
    ...
    public Vector3D applyTo(Vector3D u) {
        ...
        return new Vector3D(2 * (mQ0 * (x * mQ0 - (mQ2 * z - mQ3 * y)) + s * mQ1) - x,
    ...
    public Vector3D applyInverseTo(Vector3D u) {
        ...
        return new Vector3D(2 * (m0 * (x * m0 - (mQ2 * z - mQ3 * y)) + s * mQ1) - x,
