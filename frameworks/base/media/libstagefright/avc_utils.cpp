...
status_t FindAVCSPSInfo(
    ...
    if (profile_idc == 100 || profile_idc == 110
        ...
        if (br.getBits(1) != 0)
        {
        	LOGE("seq_scaling_matrix_present_flag != 0");
			return -EINVAL;
        }
    }
...
status_t ParseSpecificSEI(const uint8_t* sei, uint32_t size, struct SEIInfo *s)
{
	...
	LOGD("Target payload type %d, input size=%d", s->payload_type, size);
	...
	if (u32Data & 0x1f != 6) {
		LOGE("It is not SEI Nal");
		return UNKNOWN_ERROR;
	}

    while (br.numBitsLeft() >= 16) {
		LOGD("br.numBitsLeft()=%d", br.numBitsLeft());
        payload_type = 0;
        u32Data = br.getBits(8);
        while (u32Data == 0xFF) {
            payload_type += 255;
		...
		LOGD("Found payload type %d, size %d", payload_type, payload_size);
        ...
        if (payload_type != s->payload_type) {
			...
			switch (payload_type) {
				...
				default:
				{
					LOGE("Unsupport payload type %d", s->payload_type);
					return ERROR_UNSUPPORTED;
				}
			}
        }
    }
	LOGD("Can not find payload type %d!", s->payload_type);
