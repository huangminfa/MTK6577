...
M4OSA_ERR VideoEditorAudioDecoder_create(M4AD_Type decoderType,
    ...
    if(pDecoderContext->mDecoderType == M4AD_kTypeAMRNB ||
		...
		LOGV("VideoEditorAudioDecoder_create:outbufsize=%d",outbufsize);
    }
...
M4OSA_ERR VideoEditorAudioDecoder_processOutputBuffer(M4AD_Context pContext,
	...
	LOGV("VideoEditorAudioDecoder_processOutputBuffer begin buffer timestamp %lld",i64Tmp);
#else
...
M4OSA_ERR VideoEditorAudioDecoder_step(M4AD_Context pContext,
    ...
    if (INFO_FORMAT_CHANGED == result) {
		...
		if (pDecoderContext->mDecoderType == M4AD_kTypeAAC){
			...
			LOGD("VideoEditorAudioDecoder_step M4WAR_INFO_FORMAT_CHANGE profile %d",profile);
		}
#endif
