...
int ifc_reset_connection_by_uid(int uid)
{
    ...
    appuid = uid;       
    if(appuid < 0){
        LOGE("ifc_reset_connection_by_uid, invalide uid: %d", appuid);
        return -1;
    }
    
    LOGD("ifc_reset_connection_by_uid, appuid = %d ",appuid);
    result = ioctl(tcp_ctl_sock, SIOCKILLSOCK, &appuid);
    if(result < 0)
        LOGE("ifc_reset_connection_by_uid, result= %d, error =%s ", result, strerror(errno));

    LOGD("ifc_reset_connection_by_uid, result= %d ",result);
    return result;
#else
    return 0;
#endif
