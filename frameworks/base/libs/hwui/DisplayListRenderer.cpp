...
const unsigned int pattern[] = {
    0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee,
    0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee,
    0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee,
    0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee,
    0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee, 0xdeadeeee,
...
const char* DisplayList::OP_NAMES[] = {
    ...
    "SetupShadow",
    "DrawGLFunction",
    /** M: add to support draw text with offset information from GPOS @{ */
    "DrawTextOffset"
    /** @} */
};
...
DisplayList::~DisplayList() {
    clearResources();
    memcpy(&mReader, pattern, sizeof(mReader)); 
...
void DisplayList::destroyDisplayListDeferred(DisplayList* displayList) { 
    if (displayList) { 
        DISPLAY_LIST_LOGD("Deferring display list destruction %p", displayList); 
...
void DisplayList::clearResources() {
    ...
    Mutex::Autolock _l(caches.mFlushLock);
...
void DisplayList::output(OpenGLRenderer& renderer, uint32_t level) {
    ...
    {
        LOGE("%sThe displaylist has been destroyed!!", (char*)indent);
        return;
    }
#endif
        ...
        switch (op) {
            ...
            case DrawLayer: {
                ...
                if (!Caches::getInstance().isLayerActive(layer))
                    LOGE("%sThe layer has been destroyed!!", (char*) indent);
#endif
            }
            break;
            case DrawBitmap: {
            ...
            break;
            /** M: draw text with offset from GPOS @{ */
            case DrawTextOffset: {
                getText(&text);
                int count = getInt();
                float x = getFloat();
                float y = getFloat();
                SkPaint* paint = getPaint();
                const float* xOffset = getFloats(count);
                const float* yOffset = getFloats(count);
                LOGD("%s%s %s, %d, %d, %x, %x,%.2f, %.2f, %p", (char*) indent, OP_NAMES[op],
                    text.text(), xOffset, yOffset, text.length(), count, x, y, paint);
            }
            break;
            /** @} */
            default:
 ...
 */
bool DisplayList::replay(OpenGLRenderer& renderer, Rect& dirty, uint32_t &err, uint32_t level) {
    bool needsInvalidate = false;
    TextContainer text;
    mReader.rewind();

    if ((unsigned int)mReader.peek() == 0xdeadeeee)
    {
        // Should not reach here in normal case
        LOGW("Invalid pointer!! %p, %d", mReader.peek(), mReader.readInt());
    }
    ...
    while (!mReader.eof()) {
        ...
        switch (op) {
            ...
            case DrawDisplayList: {
                ...
                if (err)
                {
                    LOGE("ERROR %s: %p, %dx%d, %d", OP_NAMES[op], displayList, width, height, level + 1);
                    break;
                }
            }
            break;
            case DrawLayer: {
                ...
                if (!Caches::getInstance().isLayerActive(layer))
                {
                    LOGE("ERROR %s: %p, %.2f, %.2f, %p, %d", OP_NAMES[op], layer, x, y, paint, level);
                    if(!Caches::getInstance().dumpActiveLayers(layer))
                    {
                        err = (unsigned int)this;
                        break;
                    }
                    LOGW("WARNING: Found the layer %p in lists. Keep renderering.", layer);                    
                }
#endif
            ...
            case DrawTextOffset: {
                ...
                const float* xOffset = getFloats(count);
                const float* yOffset = getFloats(count);
                DISPLAY_LIST_LOGD("%s%s %s, %x, %x, %d, %d, %.2f, %.2f, %p", (char*) indent, OP_NAMES[op],
                    text.text(), xOffset, yOffset, text.length(), count, x, y, paint);
                renderer.drawText(text.text(), xOffset, yOffset, text.length(), count, x, y, paint);
            }
            break;
            /** @} */
            default:
	...
	{
	    int w = renderer.getDrawSurfaceWidth();
	    int h = renderer.getDrawSurfaceHeight();
	    dumpDraw(w, h, level);
	}
#endif
    }
...
bool DisplayListRenderer::drawDisplayList(DisplayList* displayList,
        uint32_t width, uint32_t height, Rect& dirty, uint32_t &err, uint32_t level) {
...
/** M: add x offset and y offset for complex text drawing @{*/
void DisplayListRenderer::drawText(const char* text, const float* xOffset, const float* yOffset, int bytesCount, int count, float x, float y,
            SkPaint* paint) {
    if (count <= 0) return;

    /* if one of xoffset or yoffset is null, use drawText without offset */
    if (xOffset == NULL || yOffset == NULL) {
        ...
        addOp(DisplayList::DrawTextOffset);
        addText(text, bytesCount);
        addInt(count);
        addPoint(x, y);
        addPaint(paint);
        addFloats(xOffset, count);
        addFloats(yOffset, count);
    }
}
/** @} */
