...
public class NetworkPolicyManagerServiceTest extends AndroidTestCase {
    ...
    public void testOverWarningLimitNotification() throws Exception {
...

        {
            ...
            replay();
            // MTK: change policy config
            //setNetworkPolicies(new NetworkPolicy(
            //        sTemplateWifi, CYCLE_DAY, 1 * MB_IN_BYTES, 2 * MB_IN_BYTES, SNOOZE_NEVER));
            NetworkPolicy policy = new NetworkPolicy(sTemplateWifi, CYCLE_DAY, 1 * MB_IN_BYTES, 2 * MB_IN_BYTES, SNOOZE_NEVER);
            policy.active = true;
