...
public class CdmaServiceStateTracker extends ServiceStateTracker {
   ...
   private boolean mAutoTimeChanged = false;
    ...
    public CdmaServiceStateTracker(CDMAPhone phone) {
        ...
        cm = phone.mCM;
        ss = new ServiceState(phone.getMySimId());
        newSS = new ServiceState(phone.getMySimId());
        cellLoc = new CdmaCellLocation();
        newCellLoc = new CdmaCellLocation();
        mSignalStrength = new SignalStrength(phone.getMySimId());
    ...
    public void handleMessage (Message msg) {
        ...
        case EVENT_RADIO_AVAILABLE:
            ...
            // via suport to add this API invocation
            queryCurrentNitzTime();
            break;
        ...
        case EVENT_RUIM_READY:
            ...
            Log.d(LOG_TAG, "after pin unlock get NitzTime!!!!");
       ...
       case EVENT_GET_NITZ_TIME:
            ...
            if (!(cm.getRadioState().isOn()) || (cm.getRadioState().isGsm()) || networkType==0) {
                Log.d(LOG_TAG, "EVENT_GET_NITZ_TIME ignore: isOn()=" +cm.getRadioState().isOn() +
                            ", isGsm()="+cm.getRadioState().isGsm() +
   ...
   //             return;
            ...
            if (ar.exception != null) {
                Log.e(LOG_TAG, "EVENT_GET_NITZ_TIME: "+ar.exception.toString());
                // The request failed.
                return;
            }
            ...
            Log.d(LOG_TAG, "EVENT_GET_NITZ_TIME  nitzStringSolicite = " + nitzStringSolicite + "nitzReceiveTimeSolicite = " + nitzReceiveTimeSolicite);
       ...
       case EVENT_QUERY_NITZ_TIME:
            ...
            cm.getNitzTime(obtainMessage(EVENT_GET_NITZ_TIME));
            break;
       // via support end
        ...
        case EVENT_RUIM_RECORDS_LOADED:
            if (DBG) log("EVENT_RUIM_RECORDS_LOADED getCDMASubscription");
            cm.getCDMASubscription(obtainMessage(EVENT_POLL_STATE_CDMA_SUBSCRIPTION));
            if (DBG) log("Receive EVENT_RUIM_RECORDS_LOADED and Send Request getCDMASubscription.");

            Log.d(LOG_TAG, "EVENT_RUIM_RECORDS_LOADED get NitzTime!!!!");
            // via support to add this API invocation
            queryCurrentNitzTime();
    ...
    private boolean isSimInserted(int slotId) {
        ...
        Log.d(LOG_TAG, "isSimInserted(" + slotId + "), SimInserted=" + simInserted);
        return simInserted;
    }	
    ...
    */
    protected void setCdmaTechnology(int radioTechnology){
        mNewDataConnectionState = 
                (cdmaConnType == 0)? ServiceState.STATE_OUT_OF_SERVICE : radioTechnologyToDataServiceState(radioTechnology);
    ...
    */
    protected void handlePollStateResultMessage(int what, AsyncResult ar){
        ...
        case EVENT_POLL_STATE_REGISTRATION_CDMA: // Handle RIL_REQUEST_REGISTRATION_STATE.
            ...
            newSS.setRegState(registrationState);
            //via support end
            ...
            SystemProperties.set("cdma.operator.sid", Integer.toString(systemId));
            //via support end
        ...
        case EVENT_POLL_STATE_OPERATOR_CDMA: // Handle RIL_REQUEST_OPERATOR
            ...
            if (opNames != null && opNames.length >= 3) {
                ...
                log("RIL_REQUEST_OPERATOR.opNames[0] = " + opNames[0]  + ", = opNames[1] = " + opNames[1] 
                    + opNames[2] +"opNames[2]" );
                ...
                if((opNames[2] != null) && opNames[2].length() >= 5) {
                    ...
                    SystemProperties.set("cdma.operator.mcc", mcc_ncc.substring(0,3));
                }
                //via support end
            } else {
	...
	log("pollStateDone: hasRegStateChanged = " + hasRegStateChanged);
	    ...
	    if (Phone.GEMINI_SIM_1 == phone.getMySimId()) {	
            	phone.setSystemProperty(TelephonyProperties.PROPERTY_DATA_NETWORK_TYPE,
                    	ServiceState.radioTechnologyToString(networkType));
	    } else {
		phone.setSystemProperty(TelephonyProperties.PROPERTY_DATA_NETWORK_TYPE_2,
                    	ServiceState.radioTechnologyToString(networkType));
	    }
        }
		...
		operatorNumeric = ss.getOperatorNumeric();
            	phone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC_2, operatorNumeric);
		...
		if(Phone.GEMINI_SIM_1 == phone.getMySimId()) {
		    ...
		    phone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY_2, isoCountryCode);
		}
   ...
   /*Poll NitzTime by via*/
    ...
    void setTimeFromNITZString (String nitz, long nitzReceiveTime)
    {
		...
		+ ", hour = " + hour + ", minute = " + minute  + ", second = "+ second);	
            ...
            SystemProperties.set("cdma.operator.ltmoffset", Integer.toString(ltmoffset));
            //via support end
		...
		log("NITZ:millisSinceNitzReceived = " + millisSinceNitzReceived);	
                ...
                if (getAutoTime()) {
                    ...
                    log("[NITZ] setTimeFromNITZString, gained = " + gained 
    ...
    public void setRadioPowerOn() {
        ...
        Log.e(LOG_TAG, "setRadioPowerOn() mDesiredPowerState " + mDesiredPowerState);
    ...
    setRadioPower(boolean power) {
        Log.d(LOG_TAG, "setRadioPower " + power +
    ...
    public void setRadioPower(boolean power, boolean shutdown) {
        Log.d(LOG_TAG, "setRadioPower " + power + " shutdown = " + shutdown +
            " mPendingRadioPowerOffAfterDataOff = " + mPendingRadioPowerOffAfterDataOff);
        mDesiredPowerState = power;
        setPendingRadioPowerOff();
        if (shutdown) {
