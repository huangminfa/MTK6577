...
public class VideoPage extends Page implements PageTransitionDetector.ActionListener, MediaSourceListener {
    private static final String TAG = "VideoPage";
    private static final int VIDEO_PER_PAGE = 4;

    private final static int STYLE_ENTER_RIGHT = 0;
    private final static int STYLE_EXIT_RIGHT = 1;
    private final static int STYLE_ENTER_LEFT = 2;
    private final static int STYLE_EXIT_LEFT = 3;
    private final static int STYLE_NEXT_ENTER = 4;
    private final static int STYLE_NEXT_EXIT = 5;
    private final static int STYLE_LAST_ENTER = 6;
    private final static int STYLE_LAST_EXIT = 7;
    private final static int STYLE_DRIFTING = 8;

    // an alias to group to common animation together.
    private final static int STYLE_GO_NEXT = 9;  // STYLE_NEXT_ENTER + STYLE_NEXT_EXIT;
    private final static int STYLE_GO_LAST = 10;  // STYLE_LAST_ENTER + STYLE_LAST_EXIT;
    private final static int STYLE_FADE_IN = 11;
    private final static int STYLE_FADE_OUT = 12;
    ...
    private static int ACTOR_TAG_BASE = 100;
    ...
    private final static float ANIM_SPEED_NORMAL = 1.0f;
    private final static float ANIM_SPEED_FASTER = 1.2f;
    ...
    private MediaItemSet getVideoItemSet() {
        if (mVideoItemSet == null) {
            if (Media3D.isDemoMode()) {
                ...
                ((MediaDbItemSet)mVideoItemSet).registerObserver(mVideoContentObserver);
            }
        }
    ...
    public static void loadConfiguration(Setting setting) {
        sThumbnailWidth = setting.getInt(Setting.VIDEO_THUMBNAIL_WIDTH);
        sThumbnailHeight = setting.getInt(Setting.VIDEO_THUMBNAIL_HEIGHT);
        sPortalThumbnailWidth = setting.getInt(Setting.VIDEO_PORTAL_THUMBNAIL_WIDTH);
        sPortalThumbnailHeight = setting.getInt(Setting.VIDEO_PORTAL_THUMBNAIL_HEIGHT);
        sMaxDragRange = setting.getFloat(Setting.VIDEO_DRAGGING_MAX);
        sDragThreshold = setting.getFloat(Setting.VIDEO_DRAGGING_THRESHOLD);
    }
    ...
    private static class ThumbLoader implements Runnable {
        ...
        public void run() {
            ...
            p.setTextSize(GLRootView.dpToPixel(mContext, VideoThumbnailActor.getFontSize()));
            p.setAntiAlias(true);
            ...
            for (int i = 0; i < count; i++) {
                ...
                BitmapGenerator generator = new BitmapGenerator() {
                    public Bitmap generate() {
                        Bitmap bmp = getThumbnail(mi, sThumbnailWidth, sThumbnailHeight);
    ...
    private final static int OUTSIDE_WINDOW_RANGE = VIDEO_PER_PAGE + THUMB_PRELOAD_RANGE;
    private static class IndexWindow {
        public final static int END_INDEX_SYMBOL = -1;
    ...
    private ThumbLoader getFreeLoader(int ownerPage) {
        ...
        ThumbLoader newLoader = new ThumbLoader(getActivity());
    ...
    private void prepareAnimations() {
        ...
        Log.v(TAG, "Test " + R.string.animation_cloudy_weather_effect_ani);
        ...
        mAnimBackgroundExit.enableOptions(Animation.HIDE_TARGET_ON_COMPLETED);
        ...
        Color invis_dark = new Color(0, 0, 0, 0);
        Color full_show = new Color(255, 255, 255, 255);

        mFadeIn = new PropertyAnimation(mPageRoot, "color", invis_dark, full_show);
        ...
        mFadeOut = new PropertyAnimation(mPageRoot, "color", full_show, invis_dark);
        ...
        public void onCompleted(Animation animation) {
            ...
            if (tag == STYLE_FADE_IN) {
                ...
                getActivity().runOnUiThread(new Runnable() {
                    public void run() {
                        if (!isEmpty()) {
                            getAnimationAndBindActor(mActorNow, mLeavingStyle).start();
                        }
                    }
                });
                return;
    ...
    private Actor getNoVideoText() {
        if (mNoVideoAlert == null) {
            ...
            mNoVideoAlert.setPosition(new Point(0.5f, 0.5f, true));
    ...
    private void showNoVideoText() {
        ((Text) getNoVideoText()).setVisible(true);
    }
    ...
    private void enterPage(int type) {
        ...
        mFadeIn.start();
        getAnimationAndBindActor(mActorNow, getTransition(type, "enter")).start();
    ...
    private void loadSettings() {
        ...
        if (mBucketId == null) {
            mBucketId = MediaUtils.CAMERA_IMAGE_BUCKET_ID;
            saveSettings();
        }
    }
    ...
    private Actor getBackground() {
        if (mBackground == null) {
            ...
            mBackground.setAlphaSource(Plane.FROM_TEXEL_VERTEX);
    ...
    private Actor getIconLast() {
        if (mIconLast == null) {
            ...
            mIconLast.setPosition(new Point(ARROW_X, 0.05f, true));
    ...
    private Actor getIconNext() {
        if (mIconNext == null) {
            ...
            mIconNext.setPosition(new Point(ARROW_X, 0.95f, true));
    ...
    private boolean peekChangeAndRequery(boolean isNull) {
        VideoDbItemSet dbItemSet = (isNull) ? createNullVideoDbItemSet() : createVideoDbItemSet();
        if (dbItemSet.getItemCount() != getTotalVideoCount()) {
            destroyVideoItemSet();
            dbItemSet.registerObserver(mVideoContentObserver);
            mVideoItemSet = dbItemSet;
            return true;
        }
        dbItemSet.close();
        return false;
    ...
    private final VideoContentObserver mVideoContentObserver =
            new VideoContentObserver(new Handler());
    ...
    private final class VideoContentObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            if (!mIsMediaItemSetModified && mVideoItemSet != null &&
                    ((MediaDbItemSet)mVideoItemSet).peekChange()) {
