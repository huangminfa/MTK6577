    ...
    public Job<Bitmap> requestImage(int type) {
        ...
        return new Job<Bitmap>() {
            public Bitmap run(JobContext jc) {
                ...
                Bitmap ret = DecodeUtils.requestDecode(jc, thumbnail, null);
                if (imageType == MediaItem.TYPE_MICROTHUMBNAIL) {
                    // for micro thumbnail used in AlbumSetPage/AlbumPage,
                    // resize and scale to proper size as the LocalImage
                    ret = BitmapUtils.resizeDownAndCropCenter(ret, LocalImage.getTargetSize(imageType),true);
                } else if (imageType == MediaItem.TYPE_THUMBNAIL) {
                    // for mini thumbnail, since PTP gives us 160x120(4:3) thumbnails,
                    // we might need to crop it to fit the full image's aspect ratio
                    // to avoid screennail been squeezed by region decoder/tile image view.
                    ret = BitmapUtils.cropToFitAspectRatio(ret, mImageWidth, mImageHeight, true);
                }
