...
dispatchCallback(void*const param)
{
    ...
    case MHAL_CAM_CB_POSTVIEW:
        ...
        postBuffer(mQvMemPool, 0, CAMERA_MSG_POSTVIEW_FRAME);
        break;
    //
    case MHAL_CAM_CB_JPEG:
        ...
        if  ( msgTypeEnabled(CAMERA_MSG_COMPRESSED_IMAGE) ) {
            ...
            LOG(LOG_DEBUG, "MCamHw-pn", "***Callback time, %8lld, %8lld, %8lld, %8lld, %8lld, %8lld",
...
postPreviewFrame()
{
    ...
    if  ( ! previewEnabled() )
    {
        MY_LOGW("Preview not enabled - skip mDispBufNo(%d)", mDispBufNo);
        return  false;
    }
    //
    //  [2] Load a strong pointer to preview memory pool.
    int32_t const       i4DispBufNo = mDispBufNo;
    ...
    //
    postBuffer(pImagePool, i4DispBufNo, CAMERA_MSG_PREVIEW_FRAME);
    //
    if  ( profile.print_overtime(10, "postBuffer for CAMERA_MSG_PREVIEW_FRAME") )
    {
        MY_LOGW(
            "mDispBufNo(%d), ImagePool(%p/%d), image WxH=%dx%d", 
            mDispBufNo, pImagePool->getVirAddr(i4DispBufNo), pImagePool->getBufSize(), 
...
//    }
    ...
    while ( ! mIsCancelPicture && burstCnt > 0 ) {
        ...
        if ( 0 != mmHalCamParam.camZsdParam.u4ZsdAddr ){
            mmHalCamParam.camZsdParam.u4ZsdAddr = NULL;
            mmHalCamParam.u4ZoomVal = getZoomValue(mZoomValue);
...
status_t MTKCameraHardware::cancelPicture()
{
    ...
    {
        LOGW("[cancelPicture] The caller is mainThread");
    ...
    while   ( eCS_TakingPicture == meCamState )
    {
        ...
        ::property_set("camera.hal.event.cancelPicture", "1");
        CAM_LOGD("[cancelPicture] camera.hal.event.cancelPicture = 1");
        mStateCond.wait(mStateLock);
        ::property_set("camera.hal.event.cancelPicture", "0");
        CAM_LOGD("[cancelPicture] camera.hal.event.cancelPicture = 0");
    }
    ...
    case CAMERA_CMD_GET_MEM_INFO:
        {
            ...
            {
                LOGE("<CAMERA_CMD_GET_MEM_INFO> argument size(%d) != struct size(%d)", arg2, sizeof(CameraMemInfo));
                status = BAD_VALUE;
                break;
            }
            if  ( CameraMemInfo::eTYPE_PMEM != rInfo.u4Type )
            {
                LOGE("<CAMERA_CMD_GET_MEM_INFO> type(%d) != PMEM", rInfo.u4Type);
                status = BAD_TYPE;
                break;
            }
            if  ( ! pMemPool.get() )
            {
                LOGE("<CAMERA_CMD_GET_MEM_INFO> NULL mVideoMemPool");
                status = NO_MEMORY;
                break;
            }
            ...
            LOGI("<CAMERA_CMD_GET_MEM_INFO> type:%d (VA, PA)=(0x%08X, 0x%08X) (u4MemSize, u4MemCount)=(%d, %d)", rInfo.u4Type, rInfo.u4VABase, rInfo.u4PABase, rInfo.u4MemSize, rInfo.u4MemCount);
        }
        break;
