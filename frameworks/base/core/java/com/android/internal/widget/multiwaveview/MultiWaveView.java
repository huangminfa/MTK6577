...
public class MultiWaveView extends View {
    ...
    // block HideTarget actions between user start hit and release the hit.
    private boolean mBlockHideTargetsIfNeeded = false;
    ...
    /**
     * Set new image(s) as handleDrawable. 
     * Usage example: multiWaveView.setHandleDrawableImage(R.drawable.ic_lockscreen_handle1);
     * @hide
     */
    public void setHandleDrawableImage(int id){
    	Resources res = getResources();
        mHandleDrawable = new TargetDrawable(res,id);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.i(TAG, "onMeasure, viewWidth="+viewWidth+", viewHeight="+viewHeight);
    ...
    private void doFinish() {
        ...
        if (targetHit) {
            ...
            Log.v(TAG,"onTrigger mActiveTarget="+mActiveTarget);
            if(!mActionCancel) dispatchTriggerEvent(mActiveTarget);
            
            // ALPS00276280. Avoid that TargetHit event was continuous reacted when receive quick ACTION_DOWN - ACTIONUP pair.
            mActiveTarget = -1;
        ...
        if(!mActionCancel) setGrabbedState(OnTriggerListener.NO_HANDLE);
    }
    ...
    private void hideTargets(boolean animate) {
    	Xlog.i(TAG,"hideTargets animate="+animate+ "mBlockHideTargetsIfNeeded = "+mBlockHideTargetsIfNeeded);
    	if (mBlockHideTargetsIfNeeded)
    		return;
    ...
    private void showTargets(boolean animate) {
    	Xlog.i(TAG,"showTargets animate = "+animate);
        if (mTargetAnimations.size() > 0) {
            stopTargetAnimation();
        }
        mAnimatingTargets = animate;
    ...
    private void internalSetTargetResources(int resourceId) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            Log.i(TAG, "internalSetTargetResources, i="+i+", drawable="+drawable);
    ...
    private void handleUp(MotionEvent event) {
        ...
        mBlockHideTargetsIfNeeded = false;
    ...
    private boolean trySwitchToFirstTouchState(MotionEvent event) {
        ...
        if (dist2(dx,dy) <= getScaledTapRadiusSquared()) {
            ...
            mBlockHideTargetsIfNeeded = true;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        Log.i(TAG, "onLayout, changed="+changed+",left="+left+", top="+top+", right="+right+", bottom="+bottom
            +", mHorizontalOffset="+mHorizontalOffset+", mVerticalOffset="+mVerticalOffset);
        ...
        Log.i(TAG, "onLayout, newWaveCenterX="+newWaveCenterX+", newWaveCenterY="+newWaveCenterY+", mWaveCenterX="+mWaveCenterX
            +", mWaveCenterY="+mWaveCenterY);
    ...
    private void updateTargetPositions() {
        ...
        for (int i = 0; i < mTargetDrawables.size(); i++) {
            ...
            if (targetIcon != null){
                Log.i(TAG, "updateTargetPositions, i="+i+", isValid="+targetIcon.isValid());
                if (targetIcon.isValid()) {
                	mCount ++;
                }
            }else {
                Log.i(TAG, "updateTargetPositions, i="+i+", targetIcon != null");
            }
            ...
            Log.i(TAG, "updateTargetPositions, i="+i+", xPosition="+xPosition+", yPosition="+yPosition+", angle="+angle
                +", mOuterRadius="+mOuterRadius+", mWaveCenterY="+mWaveCenterY+", mWaveCenterX="+mWaveCenterX);
        }
        
        if (!FeatureOption.MTK_TB_APP_LANDSCAPE_SUPPORT && mCount == 2) {
        	final TargetDrawable targetIcon0 = mTargetDrawables.get(0);
        	final TargetDrawable targetIcon1 = mTargetDrawables.get(1);
        	Xlog.v(TAG,"targetIcon0.isValid() = "+targetIcon0.isValid());
        	Xlog.v(TAG,"targetIcon1.isValid() = "+targetIcon1.isValid());
        	Xlog.v(TAG,"this.getWidth() = "+this.getWidth());
        	
        	if (!targetIcon0.isValid() || targetIcon1.isValid() || (this.getWidth() != 480 && this.getWidth() != 0)) {
        		this.getRootView().debug();
        	}
        }
    }
