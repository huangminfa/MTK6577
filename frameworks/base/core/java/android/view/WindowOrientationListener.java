...
public abstract class WindowOrientationListener {
    ...
    private static boolean localLOGV = DEBUG || false;
    ...
    public void disable() {
	    ...
	    mSensorEventListener.clearProposal();
    ...
    static final class SensorEventListenerImpl implements SensorEventListener {
        ...
        private int mHistoryIndex = 0; // index of most recent sample yucong modified for avoid issue
        private int mHistoryLength = 0; // length of historical trace yucong modified for avoid issue
        ...
        private void updateProposal(int rotation, long timestampMS,
			...
			final boolean log = mOrientationListener.mLogEnabled;//yucong add
            ...
            for (int i = 1; i < mHistoryLength; i++) {
                int olderIndex = (index + HISTORY_SIZE - i) % HISTORY_SIZE;
				if(olderIndex < 0 || olderIndex > 20)
					{
						if (log) {Slog.v(TAG, "olderIndex =" + olderIndex);}
						olderIndex = 0;
					}
