 ...
 */
#undef LOG_TAG
#define LOG_TAG "AudioPlayer_to_android"
...
SLresult aplayer_setPlayState(const android::sp<android::GenericPlayer> &ap, SLuint32 playState,
    ...
    switch (playState) {
     case SL_PLAYSTATE_STOPPED:
		 ...
		 LOGD("setting GenericPlayer to SL_PLAYSTATE_STOPPED");
         ap->stop();
         break;
     case SL_PLAYSTATE_PAUSED:
         SL_LOGV("setting GenericPlayer to SL_PLAYSTATE_PAUSED");
		 LOGD("setting GenericPlayer to SL_PLAYSTATE_PAUSED");
         switch(objState) {
     ...
     case SL_PLAYSTATE_PLAYING: {
		 ...
		 LOGD("setting GenericPlayer to SL_PLAYSTATE_PLAYING");
         switch(objState) {
         case ANDROID_UNINITIALIZED:
             *pObjState = ANDROID_PREPARING;
             ap->prepare();
...
void audioPlayer_dispatch_headAtEnd_lockPlay(CAudioPlayer *ap, bool setPlayStateToPaused,
    ...
    if (NULL != playCallback) {
		...
		LOGD("USE_ASYNCHRONOUS_PLAY_CALLBACK (*playCallback)");
...
SLresult android_audioPlayer_realize(CAudioPlayer *pAudioPlayer, SLboolean async) {
    ...
    case AUDIOPLAYER_FROM_PCM_BUFFERQUEUE:
	    ...
	    LOGD("android_audioPlayer_realize AUDIOPLAYER_FROM_PCM_BUFFERQUEUE");
    ...
    case AUDIOPLAYER_FROM_URIFD: {
		LOGD("android_audioPlayer_realize AUDIOPLAYER_FROM_URIFD");
        ...
        switch (pAudioPlayer->mDataSource.mLocator.mLocatorType) {
            case SL_DATALOCATOR_URI: {
				LOGD("android_audioPlayer_realize SL_DATALOCATOR_URI");
            ...
            case SL_DATALOCATOR_ANDROIDFD: {
				LOGD("android_audioPlayer_realize SL_DATALOCATOR_ANDROIDFD");
    ...
    case AUDIOPLAYER_FROM_TS_ANDROIDBUFFERQUEUE: {
		LOGD("android_audioPlayer_realize AUDIOPLAYER_FROM_TS_ANDROIDBUFFERQUEUE");
    ...
    case AUDIOPLAYER_FROM_URIFD_TO_PCM_BUFFERQUEUE: {
		LOGD("android_audioPlayer_realize AUDIOPLAYER_FROM_URIFD_TO_PCM_BUFFERQUEUE");
        AudioPlayback_Parameters app;
        app.sessionId = pAudioPlayer->mSessionId;
        app.streamType = pAudioPlayer->mStreamType;
    ...
    case AUDIOPLAYER_FROM_ADTS_ABQ_TO_PCM_BUFFERQUEUE: {
		LOGD("android_audioPlayer_realize AUDIOPLAYER_FROM_ADTS_ABQ_TO_PCM_BUFFERQUEUE");
        AudioPlayback_Parameters app;
        app.sessionId = pAudioPlayer->mSessionId;
        app.streamType = pAudioPlayer->mStreamType;
