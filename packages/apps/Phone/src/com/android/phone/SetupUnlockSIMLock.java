...
import com.android.internal.telephony.CommandException; // ALPS00312695
...
//    public int ifinished = -1;//whether finished action
    ...
    // ALPS00312695 START
    private int queryWaitForSIM1 = 0;
    private int queryWaitForSIM2 = 0;
    private boolean isRadioNotAvailable = false;
    // ALPS00312695 END
		...
		Log.d(LOGTAG, "[onResume]+");
	        if(false == FeatureOption.MTK_GEMINI_SUPPORT)
	        {
	        	Log.d(LOGTAG, "[onResume][Single Card]");
                        queryWaitForSIM1 = 5; // ALPS00312695
	      		...
	      		phone.getIccCard().QueryIccNetworkLock(4,4,null,null,null,null,msgQuerySIM1LockStatusSIMP);
	    	    	...
	    	    	if (iSIMMEUnlockNo == 0){//deal with SIM1
		    	    	...
		    	    	mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_1).QueryIccNetworkLock(4,0,null,null,null,null,msgQuerySIM1LockStatusSIMP);
                                ...
                                queryWaitForSIM2 = 5; // ALPS00312695
   ...
   Handler mHandlerSIMLock = new Handler(){
    	...
    	switch(msg.what){
	    	case SIM1QUERY:
                    // ALPS00312695 START
                    queryWaitForSIM1--;
                    if(queryWaitForSIM1 == 0 && isRadioNotAvailable == true) {
                        ProcessQueryWhenRadioNotAvailable(SIM1QUERY);
                        return;
                    }
                    // ALPS00312695 END
	    	    ...
	    	    if (ar.exception != null)//Query exception occurs
         	    {
                        // ALPS00312695 START
                        Log.d(LOGTAG, "[mHandlerSIMLock][SIM1QUERY] exception: " + ar.exception);
                        if (ar.exception instanceof CommandException &&
                            ((CommandException)(ar.exception)).getCommandError() == CommandException.Error.RADIO_NOT_AVAILABLE ) {
                            Log.d(LOGTAG, "[mHandlerSIMLock][SIM1QUERY] CommandException.Error.RADIO_NOT_AVAILABLE");
                            if(queryWaitForSIM1 == 0) {
                                ProcessQueryWhenRadioNotAvailable(SIM1QUERY);
                                return;
                            }
                            isRadioNotAvailable = true;
                        }
                        // ALPS00312695 END
         	    }else{
	    	...
	    	case SIM2QUERY: 
                    // ALPS00312695 START
                    queryWaitForSIM2--;
                    if(queryWaitForSIM2 == 0 && isRadioNotAvailable == true) {
                        ProcessQueryWhenRadioNotAvailable(SIM2QUERY);
                        return;
                    }
                    // ALPS00312695 END
	    	    ...
	    	    if (ar.exception != null)//Query exception occurs
         	    {
                        // ALPS00312695 START
                        Log.d(LOGTAG, "[mHandlerSIMLock][SIM2QUERY] exception: " + ar.exception);
                        if (ar.exception instanceof CommandException &&
                            ((CommandException)(ar.exception)).getCommandError() == CommandException.Error.RADIO_NOT_AVAILABLE ) {
                            Log.d(LOGTAG, "[mHandlerSIMLock][SIM2QUERY] CommandException.Error.RADIO_NOT_AVAILABLE");
                            if(queryWaitForSIM2 == 0) {
                                ProcessQueryWhenRadioNotAvailable(SIM2QUERY);
                                return;
                            }
                            isRadioNotAvailable = true;
    ...
    // ALPS00312695
    private void ProcessQueryWhenRadioNotAvailable(int slotId) {
        Log.d (LOGTAG, "ProcessQueryWhenRadioNotAvailable: " + slotId);
