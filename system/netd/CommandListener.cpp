...
int CommandListener::InterfaceCmd::runCommand(SocketClient *cli,
    ...
    if (!strcmp(argv[1], "list")) {
        ...
        if (readInterfaceCounters(argv[2], &rx, &tx)) {
            cli->sendMsg(ResponseCode::OperationFailed, "Failed to read rx counters", true);
            return 0;
        }
        ...
        unsigned long rx = 0, tx = 0;
        if (readInterfaceCounters(argv[2], &rx, &tx)) {
            cli->sendMsg(ResponseCode::OperationFailed, "Failed to read tx counters", true);
            return 0;
        }
        ...
        if (!strcmp(argv[1], "route")) {
            ...
            LOGD("[run Route CMD]: %s %s %s %s %s %s %s" ,argv[1],argv[2],argv[3],argv[4],argv[5],argv[6],argv[7]);
        ...
        if (!strcmp(argv[1], "getcfg")) {
            ...
            LOGD("interface ipv6 %s %s", argv[2],argv[3]);
            char *tmp;
...
int CommandListener::SoftapCmd::runCommand(SocketClient *cli,
    ...
    if (!strcmp(argv[1], "start")) {
        ...
        rc = sSoftapCtrl->setFlag();

    } else if (!strcmp(argv[1], "getchannellist")) {
        char *tmp = NULL;
        char *channelList = (char *) malloc(MAX_BUF_SIZE);
        memset(channelList, 0, MAX_BUF_SIZE);
        rc = sSoftapCtrl->getChannelList(MAX_BUF_SIZE, channelList);
        if (!rc) {
...
int CommandListener::MultiRouterCmd::runCommand(SocketClient *cli, int argc, char **argv) {
    ...
    if (argc < 3) {
		LOGE("MultiRouterCmd argc error");
        cli->sendMsg(ResponseCode::CommandSyntaxError, "Missing argument", false);
        return 0;
    }
