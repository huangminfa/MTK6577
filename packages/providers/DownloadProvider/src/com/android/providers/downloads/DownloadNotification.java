        ...
        for (NotificationItem item : mNotifications.values()) {
            ...
            if (hasPausedText) {
                ...
                if (item.mTotalTotal != -1 && (item.mTotalTotal < item.mTotalCurrent)) {
                    ...
                    Xlog.d(LOGTAG, "updateActiveNotification: mTotalCurrent is: " + 
                            item.mTotalCurrent + " more then mTotalTotal: " + 
                            item.mTotalTotal + " and set to same");
                }
                builder.setProgress(
    ...
    private void updateCompletedNotification(Collection<DownloadInfo> downloads) {
        for (DownloadInfo download : downloads) {
            ...
            notificationForCompletedDownload(download.mId, download.mTitle,
                    ...
                    download.mPackage, download.mMimeType, download.mFileName);
        }
    }
    void notificationForCompletedDownload(long id, String title, int status,
        ...
        if (Downloads.Impl.isStatusError(status)) {
            ...
            if (optr != null && optr.equals("OP01") && 
                    packageName != null && mimeType != null) {
                if ((packageName.equals(Constants.BROWSER_NOTIFICATION_PACKAGE)
                        || packageName.equals(Constants.OMADL_NOTIFICATION_PACKAGE))
                    ...
                    caption = mContext.getResources()
                            .getString(R.string.notification_download_complete_op01);
