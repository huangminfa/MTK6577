...
//
//		int pid = NotificationFactory.getProfileNotificationId( BluetoothProfile.ID_OPP, id );
...
//	}
	 ...
	 * @return
	 */
	protected static Notification getSimpleNotification(
			int id,
			int iconDrawableId,
			long when,
			String tickerText,
...
//		return n;
	 ...
	 * @return
	 */
	protected static Notification getProgressNotification(
			int id,
			int iconDrawableId,
			long when,
			String tickerText,
...
//
//		// view
//		RemoteViews expandedView = new RemoteViews( Options.APPLICATION_PACKAGE_NAME, R.layout.bt_opp_notification_progress );
//		expandedView.setImageViewResource( R.id.notification_icon, iconDrawableId );
//		expandedView.setTextViewText( R.id.progress_text, progressText );
//		expandedView.setTextViewText( R.id.description, message );
//		expandedView.setProgressBar( R.id.progress_bar, total, progress, false );
...
//		return n;
	...
	public static Notification getOppIncomingNotification( Context context, BluetoothShareTask task ){
		...
		intent.setFlags( Intent.FLAG_ACTIVITY_NEW_TASK );	// required for startActivity() outside the Activity
		...
		Notification n = getSimpleNotification(
					task.getId(),
					R.drawable.bt_opp_in,
					task.getCreationDate(),
					context.getString( R.string.bt_opp_notification_in_coming_ticker ),
					context.getString( R.string.bt_opp_notification_in_coming_title, task.getPeerName() ),
					context.getString( R.string.bt_opp_notification_in_coming_message ),
					PendingIntent.getActivity( context, task.getId(), intent, PendingIntent.FLAG_ONE_SHOT ),
					context );
	...
	public static Notification getOppOngoingNotification( Context context, BluetoothShareTask task ){
		...
		return getProgressNotification( task.getId(),
				icon,
				task.getCreationDate(),
				context.getString( ticker ),
				( done + "%" ),
				context.getString( message, task.getObjectName(), Formatter.formatFileSize( context, total) ),
				100,
				done,
				createCancelIntent( context, task.getId() ),
				context );
	}
	...
	private static PendingIntent createCancelIntent( Context context, int id ){
		...
		intent.setData( Uri.withAppendedPath( BluetoothShareTaskMetaData.CONTENT_URI, Integer.toString(id) ) );
	...
	private static PendingIntent createCancelingIntent( Context context, int taskId ){
		...
		intent.putExtra( BluetoothReceiver.EXTRA_TEXT, context.getString( R.string.bt_opp_canceling_task_message ) );
