    ...
    : mFd(-1), mMicMute(false)
{
   ...
   if ( ret != 0 ) LOGE("Failed to initialize pthread HardwareOnOffMutex!");
   //~ALPS00264744

#if defined(MTK_DT_SUPPORT)
    ret |= pthread_mutex_init(&mSpeechCtrlMutex, NULL);
    if ( ret != 0 ) LOGE("Failed to initialize mSpeechCtrlMutex!");
#endif
...
status_t AudioYusuHardware::setMode(int mode)
{
    ...
    */
    ...
    if(mMode == android_audio_legacy::AudioSystem::MODE_IN_CALL &&  mode == android_audio_legacy::AudioSystem::MODE_NORMAL)
    {
        ...
        {
            LOGD("setMode=%d,-(InCall) +(Normal) skip(%d) usleep(%d ms) ", mode,mFlag_SkipEndCallDelay, m_sleep);
            mFlag_SkipEndCallDelay = 0;
        }
#else
        LOGD("setMode=%d, -(InCall) +(Normal) usleep(%d ms)", mode,m_sleep);
    ...
    else if(mMode == android_audio_legacy::AudioSystem::MODE_IN_COMMUNICATION &&  mode == android_audio_legacy::AudioSystem::MODE_NORMAL)
    {
        ...
        {
            LOGD("setMode=%d,-(MODE_IN_COMMUNICATION) +(Normal) skip(%d) usleep(%d ms) ", mode,mFlag_SkipEndCallDelay, m_sleep);
            mFlag_SkipEndCallDelay = 0;
        }
#else
        LOGD("setMode=%d, -(MODE_IN_COMMUNICATION) +(Normal) usleep(%d ms)", mode,m_sleep);
    ...
    LOGD("setMode, Get HardwareOnOffMutex");
    //~ALPS00264744
   ...
   mAudioUCM->SetPhoneMode (mode);
    ...
    LOGD("setMode, Release HardwareOnOffMutex");
    //~ALPS00264744
    return NO_ERROR;
}
    ...
    case android_audio_legacy::AudioSystem::MODE_IN_CALL:  // incall mode
    {
        ...
        if(mFlag_SecondModemPhoneCall == 0)
        {
            while(SpeechHwCtrlLock() != true)
            {
                ...
                {
                    LOGE("1st MD Speech_Off Wait SpeechHwCtrlLock Time out!!! Counter_SpeechHwCtrlLock(%d)", Counter_SpeechHwCtrlLock);
                    break;
                }
            }
    ...
    case android_audio_legacy::AudioSystem::MODE_IN_CALL:
    {
        ...
        if(mFlag_SecondModemPhoneCall == 1)
        {
            while(SpeechHwCtrlLock() != true)
            {
                ...
                if(Counter_SpeechHwCtrlLock == MAXTIMES_SpeechHwCtrlLock)
                {
                    LOGE("2nd MD Speech_On Wait SpeechHwCtrlLock Time out!!! Counter_SpeechHwCtrlLock(%d)", Counter_SpeechHwCtrlLock);
                    break;
                }
            }
            LOG_HARDWARE("Counter_SpeechHwCtrlLock(%d)", Counter_SpeechHwCtrlLock);
        ...
        case android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADSET :
        {
            ...
            if(mFlag_SecondModemPhoneCall == 1)
            {
                ...
                mAnaReg->AnalogAFE_Set_LineIn_Gain(0x4);//4->0dB
                mAnaReg->AnalogAFE_Open(FM_PATH_STEREO);
            }
#endif
            SetSpeakerStatus(false);
        ...
        {
            SpeechHwCtrlUnLock();
            LOGD("2nd MD Speech_Off SpeechHwCtrlUnLock");
        }
#endif
...
#else
                if(mFlag_SecondModemPhoneCall == 1)//external md speech path
                {
                    mAnaReg->AnalogAFE_Set_LineIn_Gain(0x4);//4->0dB
                    mAnaReg->AnalogAFE_Open(FM_PATH_STEREO);
                    SetSpeakerStatus(true); // set spaker status true
...
//	   if(mMode == android_audio_legacy::AudioSystem::MODE_NORMAL && mFakeMode && value != android_audio_legacy::AudioSystem::DEVICE_OUT_EARPIECE)
    ...
    if (param.getInt(keySecondModemPhoneCall, value) == NO_ERROR)
    {
        ...
        {
            LOGE("AudioYusuHardware::setParameters SecondModemPhoneCall return false mode = %d value = %d", mMode, value);
        }
    ...
    if (param.getInt(keySetWarningTone, value) == NO_ERROR)
    {
        ...
        {
            LOGE("AudioYusuHardware::setParameters SetWarningTone return false mode = %d value = %d", mMode, value);
        }
    ...
    if (param.getInt(keyStopWarningTone, value) == NO_ERROR)
    {
        ...
        {
            LOGE("AudioYusuHardware::setParameters StopWarningTone return false mode = %d value = %d", mMode, value);
        }
    ...
    if (param.getInt(keySkipEndCallDelay, value) == NO_ERROR)
    {
        ...
        {
            LOGE("AudioYusuHardware::setParameters keySkipEndCallDelay return false value = %d\n", value);
        }
            ...
            if(GetFmRxStatus() == true)
            {
                if(!IsHeadsetConnect())
                {
                    ...
                    if((mHeadsetDetect->isPlaying()!= 0) && GetHeadPhoneStatus())
                        LOGD("Not ForceEnableSpeaker due to headset pluged-in");    //ALPS00298330:the FM sound will come out the speaker temporary while headset pluged in and kill mediaserver
                    else
                        ForceEnableSpeaker ();
                }
                else
...
status_t AudioYusuHardware::SetAudioCommand(int par1, int par2)
{
    ...
    {
        LOGD(" SET_SPEECH_VM_ENABLE(%d)", par2);
        GetCustParamFromNV(&sndParam);
        if(par2 == 0){
            //disable EPL
            sndParam.speech_common_para[0] = 0;
        ...
        LOGD("SET_SPEECH_VM_ENABLE +LAD_SetSpeechParameters ret=%d",i4ret);
        break;
    }
    case SET_DUMP_SPEECH_DEBUG_INFO:
    {
        LOGD(" SET_DUMP_SPEECH_DEBUG_INFO(%d)", par2);
    ...
    {
        LOGD(" SET_DUMP_AUDIO_DEBUG_INFO(%d)", par2);
        ::ioctl(mFd, AUDDRV_LOG_PRINT, 0);
        break;
    }
    case SET_DUMP_AUDIO_STREAM_OUT:
    {
        LOGD(" SET_DUMP_AUDIO_STREAM_OUT(%d)", par2);
        if(par2 == 0){
    ...
    {
        LOGD(" SET_DUMP_AUDIO_MIXER_BUF(%d)", par2);
        if(par2 == 0){
    ...
    {
        LOGD(" SET_DUMP_AUDIO_TRACK_BUF(%d)", par2);
        if(par2 == 0){
    ...
    {
        LOGD(" SET_DUMP_A2DP_STREAM_OUT(%d)", par2);
        if(par2 == 0){
    ...
    {
        LOGD(" SET_DUMP_AUDIO_STREAM_IN(%d)", par2);
        if(par2 == 0){
    ...
    {
        LOGD(" SET_DUMP_IDLE_VM_RECORD(%d)", par2);
#if defined(MTK_AUDIO_HD_REC_SUPPORT)
        if(par2 == 0){
...
status_t AudioYusuHardware::GetAudioCommand(int parameters1)
{
    ...
    case GET_DUMP_AUDIO_STREAM_OUT:
    {
        ...
        result = atoi(value);
        LOGD(" GET_DUMP_AUDIO_STREAM_OUT=%d", result);
        break;
    }
    case GET_DUMP_AUDIO_MIXER_BUF:
    {
        ...
        result = atoi(value);
        LOGD(" GET_DUMP_AUDIO_MIXER_BUF=%d", result);
        break;
    }
    case GET_DUMP_AUDIO_TRACK_BUF:
    {
        ...
        result = atoi(value);
        LOGD(" GET_DUMP_AUDIO_TRACK_BUF=%d", result);
        break;
    }
    case GET_DUMP_A2DP_STREAM_OUT:
    {
        ...
        result = atoi(value);
        LOGD(" GET_DUMP_A2DP_STREAM_OUT=%d", result);
        break;
    }
    case GET_DUMP_AUDIO_STREAM_IN:
    {
        ...
        result = atoi(value);
        LOGD(" GET_DUMP_AUDIO_STREAM_IN=%d", result);
        break;
    }
    case GET_DUMP_IDLE_VM_RECORD:
    {
        ...
        result = 0;
#endif
        LOGD(" GET_DUMP_IDLE_VM_RECORD=%d", result);
        break;
    }
    default:
    {
        LOGD(" GetAudioCommand: Unknown command\n");
        break;
    }
    }
...
static void *Special_Format_Record_Routine(void *arg)
{
    ...
    */
...
#endif
    {
        ...
        strftime (path,80,"/sdcard/VMLog_%Y_%m%d_%H%M%S.vm", timeinfo);
        LOGD( "path is : %s", path);
        pFile = fopen(path,"w");
    }
...
{
    LOGD("SetFlag_SecondModemPhoneCall mMode=%d, value=%d", mMode, value);
    ...
    {
        LOGE("SetFlag_SecondModemPhoneCall Fail!!! invalid mMode=%d, value=%d", mMode, value);
...
{
    LOGD("SetWarningTone mMode=%d, toneIdx=%d, mFlag_SecondModemPhoneCall=%d", mMode, toneIdx, mFlag_SecondModemPhoneCall);
    if((toneIdx>=0)&&(toneIdx<=15))
    {
        ...
        {
            LOGE("invalid SetWarningTone mMode=%d, mFlag_SecondModemPhoneCall=%d", mMode, mFlag_SecondModemPhoneCall);
            return INVALID_OPERATION;
        }
    }
    else
    {
        LOGE("invalid SetWarningTone toneIdx=%d, mFlag_SecondModemPhoneCall=%d", toneIdx, mFlag_SecondModemPhoneCall);
        return BAD_VALUE;
    }
}
...
{
    LOGD("StopWarningTone mMode=%d, toneIdx=%d, mFlag_SecondModemPhoneCall=%d", mMode, toneIdx, mFlag_SecondModemPhoneCall);
    ...
    {
        LOGE("invalid StopWarningTone mMode=%d, mFlag_SecondModemPhoneCall=%d", mMode, mFlag_SecondModemPhoneCall);
        return INVALID_OPERATION;
    }
}
...
{
    LOGD("SetFlag_SkipEndCallDelay value=%d\n", value);
