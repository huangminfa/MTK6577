...
#ifndef ANDROID_DEDAULT_CODE
#ifdef MTK_AUDIO
#include "AudioResamplermtk.h"
#endif
#endif //ANDROID_DEDAULT_CODE
namespace android {
...
#ifndef ANDROID_DEDAULT_CODE
 void AudioResampler::setOutputSampleRate(int32_t outSampleRate)
 {
     LOGW("setOutputSampleRate outSampleRate = %d mInSampleRate = %d",outSampleRate,mInSampleRate);
     if(outSampleRate <= 0){
	 LOGE("AudioResampler::setOutputSampleRate fail");
	 return;
     }
     mSampleRate = outSampleRate;
     mPhaseIncrement = (uint32_t)((kPhaseMultiplier * mInSampleRate) / mSampleRate);
 }
#endif
