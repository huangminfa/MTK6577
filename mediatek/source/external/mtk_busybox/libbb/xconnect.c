...
#else
int  setsockopt_bindtodevice(int fd UNUSED_PARAM,
...
//int xsocket_type(len_and_sockaddr **lsap, IF_FEATURE_IPV6(int family,) int sock_type)
static int xxsocket_type(len_and_sockaddr **lsap, int family, int sock_type)
{
	family = AF_INET; //IF_NOT_FEATURE_IPV6(enum { family = AF_INET };)
	len_and_sockaddr *lsa;
	int fd;
	int len;
...
{
	return xxsocket_type(lsap, AF_UNSPEC, SOCK_STREAM);
}
...
static int create_and_bind_or_die(const char *bindaddr, int port, int sock_type)
{
	...
	if (bindaddr && bindaddr[0]) {
		...
		fd = xxsocket_type(&lsa, AF_UNSPEC, sock_type);
