...
setWindow(preview_stream_ops*const window)
{
    ...
    CPTLogStr(Event_DispAdpt_setWindow, CPTFlagPulse, "onDestroy");
    onDestroy();
    //
    //  [4.1]
    if  ( mpTgtBufQue == 0 )
    ...
    //
    //  [4.2]
    CPTLogStr(Event_DispAdpt_setWindow, CPTFlagPulse, "TgtBufQue->setWindow");
...
postFrame(DispFrame& rFrame)
{
    ...
    else
    {
        ...
        mpTgtBufQue->enqueueBuffer(currTgtBuf);
        //
        mpTgtBufQue->prepareQueueAsync();
    }
    //
