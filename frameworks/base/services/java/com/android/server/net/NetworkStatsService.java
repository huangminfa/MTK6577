...
public class NetworkStatsService extends INetworkStatsService.Stub {
    ...
    private int mSimIdCount;
    private long mSimId1;
    private long mSimId2;
    private long mDataActivedSimId;
    private static final String IFACE_GEMINI = "duiface";
    ...
    /** [MTK-START] we prefer not use it. due to some mobile Internet 
     *  can't access the ntp server and always timeout.
     *  The timeout impact device's performance.
     */
    public static final boolean USE_TRUESTED_TIME = false; 
    ...
    private static File getUserDir() {
        return new File(Environment.getDataDirectory(), "user");
    }
    ...
    public void systemReady() {
        ...
        // watch for data usage helper to be claimed
        IntentFilter appusgFilter = new IntentFilter();
        appusgFilter.addAction("mediatek.net.datausg.DATA_INFO_WRITE");
        appusgFilter.addAction("mediatek.net.datausg.DATA_INFO_DEL");
        mContext.registerReceiver(mDUHelper, appusgFilter);
    ...
    private void shutdownLocked() {
        Xlog.d(TAG, "shutdownLocked");
       
        // MTK: Don't unregister due to ACTION_SHUTDOWN_IPO
        ...
        */
        ...
        // MTK: Don't clear data due to ACTION_SHUTDOWN_IPO
        // systemReady will not be call when IPO power up.
        /*
    ...
    private NetworkStatsHistory getHistoryForNetwork(NetworkTemplate template, int fields,
        ...
        synchronized (mStatsLock) {
            if (LOGV) Slog.v(TAG, "getHistoryForNetwork() susbId="+ template.getSubscriberId());
            ...
            for (NetworkIdentitySet ident : source.keySet()) {
                if (templateMatches(template, ident)) {
                    ...
                    if (history != null) {
                        if (LOGV) Slog.v(TAG, "getHistoryForNetwork matched! ident:"+ident);
    ...
    private void writeDataUsageHelper() {
        forceUpdate();
        ...
        long maxBytes = 0;
        long total = 0;        
        UidStatsKey maxKey = null;
        String uidname = null;
        
        ArrayList<UidStatsKey> keys = Lists.newArrayList();
        keys.addAll(mUidStats.keySet());
        Collections.sort(keys);

        String datetime = "_"+android.text.format.DateFormat.format("yyyyMMdd_hhmmss", new java.util.Date());
        AtomicFile dataUsageHelperFile = new AtomicFile(new File(getUserDir(), "duhelper"+datetime+".log"));
        FileOutputStream fos = null;
        
        try {
            fos = dataUsageHelperFile.startWrite();
            XmlSerializer out = new FastXmlSerializer();
            out.setOutput(fos, "utf-8");
            out.startDocument(null, true);

            out.startTag(null, "datausage-helper");
            writeIntAttribute(out, "version", 1);
            out.attribute(null, "DateTime", datetime);
            
            while (!keys.isEmpty()) {
                for (UidStatsKey key : keys) {
                    final NetworkStatsHistory history = mUidStats.get(key);
                    historyEntry = history.getValues(Long.MIN_VALUE, Long.MAX_VALUE, now, historyEntry);
                    total = historyEntry.rxBytes + historyEntry.txBytes;
                    ...
                    if (key.set == 1) {
                        ...
                        maxBytes = total;
                        maxKey = key;
                        uidname = mContext.getPackageManager().getNameForUid(key.uid);
                        if (uidname == null)
                            uidname = "unknown";
                    }
                } 
                if (maxKey != null) {
                    ...
                    writeIntAttribute(out, "SysUid", maxKey.uid);
                    out.attribute(null, "Name", uidname);
                    writeLongAttribute(out, "BgData", maxBytes);
                    out.endTag(null, "App");
                    keys.remove(keys.indexOf(maxKey));
                    maxBytes =0;
                    maxKey = null;
                } else {
                    break;
            ...
            out.endTag(null, "datausage-helper");
            out.endDocument();
            dataUsageHelperFile.finishWrite(fos);
            readFileSendBroadcast("duhelper"+datetime+".log");
        } catch (Exception e) {
            Slog.w(TAG, "writeDataUsageHelper err:"+e);
            if (fos != null) {
                dataUsageHelperFile.failWrite(fos);
            }
        } finally {
    ...
    private void readFileSendBroadcast(String filename) {
        String result ="";
        String buffer;

        BufferedReader reader = null;
        try {
            reader = new BufferedReader(new FileReader(new File(getUserDir(), filename)));
            ...
            Slog.w(TAG, "readFileSendBroadcast err:"+e);
        }
        Slog.w(TAG, "readFileSendBroadcast:"+result);
        Intent intent = new Intent("mediatek.net.datausg.DATA_INFO_DONE");
    ...
    private void clearStatsFiles() {
        Slog.w(TAG, "clearStatsFiles");

        try {
            Runtime.getRuntime().exec("rm -r /data/system/netstats.bin");
            Runtime.getRuntime().exec("rm -r /data/system/netstats_xt.bin");
            Runtime.getRuntime().exec("rm -r /data/system/netstats_uid.bin");
        } catch (IOException e) {
            Slog.w(TAG, "clearStatsFiles e:"+e);
        }
        PowerManagerService pms = (PowerManagerService) ServiceManager.getService("power");
        pms.reboot("clearStatsFiles");
    }
    
     /**
     * MediaTek AppUsage for dump app deatiled info
     */
    private BroadcastReceiver mDUHelper = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals("mediatek.net.datausg.DATA_INFO_WRITE")) {
                writeDataUsageHelper();
            } else if (intent.getAction().equals("mediatek.net.datausg.DATA_INFO_DEL")) {
                clearStatsFiles();
            }
        }
    };
    ...
    private void updateSimListInfo() {
        List<SIMInfo> simList;
        ...
        mDataActivedSimId = Settings.System.getLong(mContext.getContentResolver(), Settings.System.GPRS_CONNECTION_SIM_SETTING, Settings.System.DEFAULT_SIM_NOT_SET);
    }
    ...
    private void updateIfacesLocked() {
        ...
        ///M: update updateSimListInfo after save statistic to previous interface
        updateSimListInfo();
        ...
        for (NetworkState state : states) {
            final int netType = state.networkInfo.getType();

            if (state.networkInfo.isConnected()) {
                // collect networks under their parent interfaces
                final String iface = state.linkProperties.getInterfaceName();
                if (LOGV) Slog.v(TAG, iface + " is connected");
                if (FeatureOption.EVDO_DT_SUPPORT &&                  
                    netType == ConnectivityManager.TYPE_MOBILE) {

                        Slog.v(TAG," is EVDO_DT_SUPPORT");
                        ifaces[0] = IFACE_GEMINI + mSimId1;
                        ifaces[1] = IFACE_GEMINI + mSimId2;
                        // no need to create networkid for history SIMs
                      // Just need to create two for current actived SIMs.
                      NetworkIdentitySet ident = mActiveIfaces.get(ifaces[0]);
                      if (ident == null ) {
                          if (LOGV) Slog.v(TAG, "new netIdent for " + ifaces[0] + "simId:" + mSimId1);
                          ident = new NetworkIdentitySet();
                          mActiveIfaces.put(ifaces[0], ident);
                      }
                      ident.add(NetworkIdentity.buildNetworkIdentityGemini(mContext, state, mSimId1));
    
                      ident = mActiveIfaces.get(ifaces[1]);
                      if (ident == null ) {
                          if (LOGV) Slog.v(TAG, "new netIdent for " + ifaces[1] + "simId:" + mSimId2);
                          ident = new NetworkIdentitySet();
                          mActiveIfaces.put(ifaces[1], ident);
                      }
                      ident.add(NetworkIdentity.buildNetworkIdentityGemini(mContext, state, mSimId2));


                }
                else if ((FeatureOption.MTK_DT_SUPPORT || FeatureOption.MTK_GEMINI_SUPPORT) && netType == ConnectivityManager.TYPE_MOBILE) {
    ...
    private void dumpiface() {
        Slog.v(TAG,"Active interfaces +");
        for (String iface : mActiveIfaces.keySet()) {
            final NetworkIdentitySet ident = mActiveIfaces.get(iface);
            Slog.v(TAG," iface="+iface);
            Slog.v(TAG," ident="+ident.toString());
        }
        Slog.v(TAG,"Active interfaces -");
    }
    ...
    private void performPollLocked(int flags) {
        ...
        if (persistForce || (persistNetwork && networkOverThreshold)) {
            ...
            Xlog.d(TAG, "no write Dev/Xt delat:" + persistNetworkDevDelta + " Force:" + persistForce + " Net:" + persistNetwork);
        }
        ...
        if (persistForce || (persistUid && persistUidDelta > threshold)) {
            ...
            Xlog.d(TAG, "no write Uid delat:" + persistUidDelta);
        }
    ...
    private void performNetworkDevPollLocked(NetworkStats networkDevSnapshot, long currentTime) {
        ...
        if (LOGV) Slog.v(TAG, "performNetworkDevPollLocked"); 
        ...
        for (int i = 0; i < delta.size(); i++) {
            ...
            if (FeatureOption.EVDO_DT_SUPPORT && (ifaceName.contains("ccmni")||ifaceName.contains("ppp")) ){
                
                ifaceName = (IFACE_GEMINI + mDataActivedSimId);   
                Xlog.d(TAG, "performNetworkDevPollLocked EVDO_DT_SUPPORT ifaceName=" + ifaceName);                    
            }
            else if (FeatureOption.MTK_DT_SUPPORT) {

            
            } else if (FeatureOption.MTK_GEMINI_SUPPORT && ifaceName.contains("ccmni")) {
                ifaceName = (IFACE_GEMINI + mDataActivedSimId);
            }
            
            final NetworkIdentitySet ident = mActiveIfaces.get(ifaceName); 
                
            if (ident == null) {
                unknownIface.add(entry.iface);
                continue;
            }
            Xlog.d(TAG, "performNetworkDevPollLocked  entry.iface=" + entry.iface);                    
    ...
    private void performNetworkXtPollLocked(NetworkStats networkXtSnapshot, long currentTime) {
        ...
        for (int i = 0; i < delta.size(); i++) {
            ...
            if (FeatureOption.EVDO_DT_SUPPORT && (ifaceName.contains("ccmni")||ifaceName.contains("ppp")) ){
                ...
                ifaceName = (IFACE_GEMINI + mDataActivedSimId);   
                Xlog.d(TAG, "performNetworkXtPollLocked EVDO_DT_SUPPORT ifaceName=" + ifaceName);                    
            }
            else if (FeatureOption.MTK_DT_SUPPORT) {
            ...
            Xlog.d(TAG, "performNetworkXtPollLocked  entry.iface=" + entry.iface);       
    ...
    private void performUidPollLocked(NetworkStats uidSnapshot, long currentTime) {
        ...
        for (int i = 0; i < delta.size(); i++) {
            ...
            if (FeatureOption.EVDO_DT_SUPPORT && (ifaceName.contains("ccmni")||ifaceName.contains("ppp"))) {
                ...
                ifaceName = (IFACE_GEMINI + mDataActivedSimId);   
                Xlog.d(TAG, "performUidPollLocked EVDO_DT_SUPPORT ifaceName=" + ifaceName);                    
            }
            else if (FeatureOption.MTK_DT_SUPPORT) {
            ...
            Xlog.d(TAG, "performUidPollLocked  entry=" + entry);       
    ...
    private void performSample() {
        ...
        // Need to un-comment after EventLogTag migrated ICS4.0.3
    ...
    private NetworkStatsHistory findOrCreateNetworkStatsLocked(
        ...
        Slog.v(TAG, "findOrCreateNetworkStatsLocked");
        final NetworkStatsHistory existing = source.get(ident);
        if (existing == null)
            Slog.v(TAG, "Create new history");
    ...
    private void readUidStatsLocked() {
        ...
        try {
            ...
            Log.e(TAG, "problem reading uid stats:"+ e);
        } finally {
            IoUtils.closeQuietly(in);
        }
    }
    ...
    private void writeNetworkStats(
        ...
        try {
            ...
            Log.e(TAG, "problem writing stats:"+ e);
            if (fos != null) {
        ...
        try {
            ...
            for (NetworkIdentitySet ident : keysByIdent.keySet()) {
                ...
                for (UidStatsKey key : keys) {
                    ...
                    history.writeToStream(out);
                    //MTK: add for debug only
                    if (!IS_USER_BUILD) dumpUidSummary(key.uid, key.set, key.tag, history);
                }
            }
    ...
    private void dumpUidSummary(int uid, int set, int tag, NetworkStatsHistory history) {
        if(tag == TAG_NONE) {
            final long now = System.currentTimeMillis();
            NetworkStatsHistory.Entry historyEntry = null;
            long total = 0;        
            ...
            historyEntry = history.getValues(Long.MIN_VALUE, Long.MAX_VALUE, now, historyEntry);
            total = historyEntry.rxBytes + historyEntry.txBytes;
            uidName = mContext.getPackageManager().getNameForUid(uid);
            Xlog.d(TAG, "-------------------");
            Xlog.d(TAG, "uid:" + uid + " set:" + set + " tag:" + tag + " name:" + uidName);
            Xlog.d(TAG, "total:" + total + "(rx:" + historyEntry.rxBytes + " tx:" + historyEntry.txBytes + ")");
        }
    }
        ...
        if (before != null) {
            try {
                ...
                try {
                    ...
                    Log.e(TAG, "found non-monotonic values; returning empty delta:"+ e1);
    ...
    private NetworkStats getNetworkStatsTethering() throws RemoteException {
        try {
            ...
            /* [MTK40247] don't throw "What a Terrible Failure"
    ...
    private static String getActiveSubscriberId(Context context) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            final ConnectivityManager conn = (ConnectivityManager) context.getSystemService(
                ...
                return conn.getDataActivedSubscriberId();
        } else {
