...
class IconLoader extends Handler {
	...
	static IconLoader getInstance(Handler caller, IccFileHandler fh) {
		...
		if (fh != null) {
			//HandlerThread thread = new HandlerThread("Cat Icon Loader");
			//thread.start();
			return new IconLoader(Looper.myLooper(), fh);
		}
		return null;
	}
		...
		try {
			...
			case EVENT_READ_ICON_DONE:
				...
				if (mId.codingScheme == ImageDescriptor.CODING_SCHEME_BASIC) {
					...
					throw new Exception("Invalid ImageDescriptor codingScheme");
				}
				break;
	...
	private void readIconData() {
		...
		mSimFH.loadEFImgTransparent(mId.imageId, mId.highOffset, mId.lowOffset, mId.length ,msg);
	}
            //MTK-END [mtk80950][120810][ALPS00XXXXXX] add UTK
	}
