    ...
    public boolean parse() throws IOException, CommandStatusException {
                    ...
                    if (status == 3 || CommandStatus.isBadSyncKey(status)) {
                        ...
                        if (Build.TYPE.equals("eng")) {
                            ...
                            handler.post(new Runnable() {
                                public void run() {
                                    Toast.makeText(mContext, R.string.wipe_all_messages, Toast.LENGTH_LONG).show();
                                }
                            });
                        }
                        mAdapter.wipe();
        ...
        if (Logging.LOG_PERFORMANCE) {
            if ("Email".equalsIgnoreCase(mAdapter.getCollectionName())) {
                Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] EasSyncService:Email.parse spend [" + (System.currentTimeMillis() - startParse) + "] ms");
            }
        }
        // Commit any changes
        commit();
