...
SkiaComposeShader::SkiaComposeShader(SkiaShader* first, SkiaShader* second,
	...
	LOGW("ComposeShader contain shaders of the same type: %d", first->type());
    if (first->type() == kCompose || second->type() == kCompose)
	LOGW("ComposeShader cannot contain a ComposeShader"); 
}
