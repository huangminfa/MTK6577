...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    // [ALPS00294607]Restore keygurad related state when top window is status bar expand
    ...
    // The global variable to save the state of stk.enable.user_activity
    boolean mIsStkUserActivityEnabled = false;

    // Protect mIsStkUserActivityEnabled be accessed at the multiple places
    private Object mStkLock = new Object();
    
    // Save the screen off reason from the power manager service.
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        keyDispatchFilter.addAction(POWERKEY_DISABLE);        
        context.registerReceiver(mKeyDispatchReceiver, keyDispatchFilter);

        // register for stk  events
        ...
        if (attrs.type == TYPE_INPUT_METHOD) {
            ...
            if ((fl & (FLAG_LAYOUT_IN_SCREEN | FLAG_FULLSCREEN | FLAG_LAYOUT_INSET_DECOR))
                ...
                if (attached != null) {
                    ...
                    // Becuase it's a non full screen window, the top posistion should not be
                    // higher than Status Bar. Otherwise, the ViewRoot can receiver 2 resize info 
                    // with 2 top position with the visible and content insect
                    if (mStatusBarCanHide) {                    
                        if (cf.top < mStatusBarHeight) {
                            cf.top = mStatusBarHeight;
                }

                        if (vf.top < mStatusBarHeight) {
                            vf.top = mStatusBarHeight;
                        }
                    }      
                }
    ...
    public void animatingWindowLw(WindowState win,
        ...
        if (mTopFullscreenOpaqueWindowState == null &&
            ...
            if (attrs.type >= FIRST_APPLICATION_WINDOW
                ...
                if ((attrs.flags & FLAG_SHOW_WHEN_LOCKED) != 0) {
                    ...
                    if(mKeyguardMediator.DM_Check_Locked()){
                        if(attrs.packageName.matches("com.android.phone"))
            ...
            } else if (attrs.type == TYPE_STATUS_BAR_SUB_PANEL
                && attrs.x >=0 && attrs.y >= 0) {
                // Status Bar Expand wants to show in the screen
                // It can occupy the full screen.
                if (DEBUG_LAYOUT) Log.v(TAG, "Fullscreen window: " + win);            
                mTopFullscreenOpaqueWindowState = win;
    ...
    public int finishAnimationLw() {
        ...
        if (mStatusBar != null) {
            ...
            // When DM locks, we ensure the status bar hide
            if(mKeyguardMediator.DM_Check_Locked()) {
                if (DEBUG_LAYOUT) Log.v(TAG, "Hiding status bar by DM_Check_Locked");
                if (mStatusBar.hideLw(true)) changes |= FINISH_LAYOUT_REDO_LAYOUT;
            } else if (mForceStatusBar) {
        ...
        if (mKeyguard != null) {
            ...
            if (mDismissKeyguard && !mKeyguardMediator.isSecure()) {
                ...
                // In order to enhanced the showing performance of keyguard
                // We disable the enter animation.
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags, boolean isScreenOn) {
        ...
        synchronized (mKeyDispatchLock) {
            ...
            if (KeyEvent.KEYCODE_POWER == keyCode && 
                KEY_DISPATCH_MODE_POWER_DISABLE == mKeyDispatcMode){
                return 0;
            }                
        }
                    ...
                    if (telephonyService != null) {
                        try {
                            if (telephonyService.isRinging()) {
                                ...
                                // for a better user experimence, we only end call when the screen on
                                hungUp = telephonyService.endCall();
                            }
                        } catch (RemoteException ex) {
                            Log.w(TAG, "ITelephony threw RemoteException", ex);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            synchronized (mKeyDispatchLock) {
                if (action.equals(IPO_ENABLE)) {
                    ...
                    // For the tablet, the status bar can't be hided.
                    // When IPO boots up, the status bar won't be covered by IPO window
                    // Therefore, we need to hide it at IPO Boot.
                    ...
                    // When finishing IPO boot up, recover the mStatusBarCanHide
                    mStatusBarCanHide = mStatusBarCanHideBeforeIPO;
                } else if (action.equals(POWERKEY_DISABLE)) {
                    if (intent.getBooleanExtra("state", false)) {
                        if (DEBUG_INPUT) {
	                        Log.v(TAG, "Receive POWERKEY_DISABLE as true");
	                }   						
                        mKeyDispatcMode = KEY_DISPATCH_MODE_POWER_DISABLE;
                    } else {
                        if (DEBUG_INPUT) {
                            Log.v(TAG, "Receive POWERKEY_DISABLE as false");
                        }   						
                        mKeyDispatcMode = KEY_DISPATCH_MODE_ALL_ENABLE;
                    }                    
                } else {
    ...
    public void userActivity() {
        ...
        // When the user activiy flag is enabled, it notifies the intent "STK_USERACTIVITY"
        synchronized(mStkLock) {
