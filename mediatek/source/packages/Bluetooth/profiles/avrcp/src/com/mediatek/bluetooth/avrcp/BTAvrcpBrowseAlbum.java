...
public class BTAvrcpBrowseAlbum {
	public final static String TAG = "AVRCP_ALBUM";
	...
	public final static byte MIX_TYPE = 0x00;
	public final static byte TITLE_TYPE = 0x01;
	public final static byte ALBUM_TYPE = 0x02;
	public final static byte ARTIST_TYPE = 0x03;
	public final static byte GENRES_TYPE = 0x04;
	public final static byte PLAYLIST_TYPE = 0x05;
	public final static byte YEAR_TYPE = 0x06;
	public final static byte ALL_TYPE = 0x0E;
	public final static byte EMPTY_TYPE = 0x0F;
	...
	*/
	 ...
	 * 
	 */
	public String getAttributeByIndex(int index, int attr_id){
		if( 0 == mSelectId ){
			// return current list's by index
			if( index < mIdList.length ){
				return mNameList[index];
	...
	private long[] updateSongs(Context context) {
		...
		switch(mtype){
			case EMPTY_TYPE:
				...
				//break;
			case ALL_TYPE:
				ccols = new String[] { MediaStore.Audio.Artists._ID,
						               MediaStore.Audio.Artists.ARTIST };			
	 			c = query(context, MediaStore.Audio.Artists.EXTERNAL_CONTENT_URI,
						ccols, null, null, null);
				break;
			case ARTIST_TYPE:
				ccols = new String[] { MediaStore.Audio.Artists._ID,
						               MediaStore.Audio.Artists.ARTIST };			
			...
			case ALBUM_TYPE:
			default:
				ccols = new String[] { MediaStore.Audio.Albums._ID,
						               MediaStore.Audio.Albums.ALBUM };
	 			c = query(context, MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI,
						ccols, null, null, null);
	...
	public String getItemAttribute(byte scope, long identifier, short uid_counter, int attr_id){
		int i = 0;
		int index;
		if( attr_id >= 0x01 && attr_id <= 0x03 ){
			if( 0 != mSelectId){
	...
	/*
	 * @brief Update the subfolder's list by current selected id
	 * if the type is ALL_TYPE, the current selected id is always 1.
	 */
	private long[] updateSubSongs(Context context, long selected_id) {
		boolean printDetail = true;
		...
		switch( mtype ){
        	...
        	case ALL_TYPE:
				...
				break;
			case ARTIST_TYPE:
         		where = MediaStore.Audio.Media.ARTIST_ID + "=" + selected_id + " AND " + 
        			MediaStore.Audio.Media.IS_MUSIC + "=1";
        		break;
        	case ALBUM_TYPE:
        	default:
         		where = MediaStore.Audio.Media.ALBUM_ID + "=" + selected_id + " AND " + 
        			MediaStore.Audio.Media.IS_MUSIC + "=1";
        		break;
		...
		try {
			...
			if (len > 0) {
				...
				for (int i = 0; i < len; i++) {
					...
					if (printDetail == true) {
						cursor = c;
						Log.v(TAG, String.format("[AVRCP][UTIL] id:%d title:%s artist:%s album:%s",
