...
readyToRun()
{
    ...
    int const priority = RTPM_PRIO_CAMERA_DISPLAY;
    //
...
SCOPE_TGTBUFQUE::
TgtBufQue::
queueWindow(BufInfo const& rBuf)
{
...
//------------------------------------------------------------------------------
    ...
    ret = true;
lbExit:
    MY_LOGD_IF(ENABLE_LOG, "- ret(%d), (address, phandle)=(%p, %p), stride(%d)", ret, address, phandle, stride);
