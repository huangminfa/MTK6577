...
#include "kal_adm.h"
#else
//[2010-05-31] Chiwei: Adding for removing undefined warnings
#include "vt_swip_sys_service.h"
#endif

/* Forward declarations so it can be used by memory driver headers */
typedef KAL_ADM_ID RvMemory;
...
RvStatus RVCALLCONV RvMemoryAllocEx(
    IN  RvSize_t        size,
    OUT void        **resultptr);
...
#define RvMemoryAllocEx(_s,_p) RvMemoryAllocExDbg((_s), __LINE__, __FILE__, (_p))
...
RvStatus RVCALLCONV RvMemoryAllocExDbg(
    IN  RvSize_t          size,
    IN  RvInt               line,
    IN  RvChar          *filename,
    OUT void        **resultptr);
...
RVCOREAPI
RvStatus RVCALLCONV RvMemoryFreeEx(
    IN void     *ptr);
 ...
 */
#define RvMemoryPrintDbg(reqregion, logMgr) ((void)(regregion), (void)(logMgr), RV_ERROR_NOTSUPPORTED)
