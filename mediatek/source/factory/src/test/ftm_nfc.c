    ...
    else
    {
        ...
        while (1) {
             ...
             if(i4TestItem == ITEM_19)
            {
            	  ...
            	  ilm_struct *nfc_ilm_ptr_loc;
            	  nfc_msg_struct *nfc_msg_struct_loc;
            	  nfc_setting_request param_loc;
            	  
                memset(nfc->info, '\n', INFO_SIZE);                
                ptr = nfc->info;
                ptr += sprintf(ptr, "SEND Message ITEM 18 OK...k...\n");
                iv->redraw(iv);
                
                //Set Test message
                param_loc.nfc_enable = 1;
                param_loc.debug_enable = 1;
                param_loc.sw_protocol = 1;
                param_loc.get_capabilities = 1;
                
                //Set NEC MEssage
                nfc_msg_struct_loc = malloc(sizeof(nfc_msg_struct) + sizeof(nfc_setting_request));
                
                nfc_msg_struct_loc->msg_type = MSG_ID_NFC_SETTING_REQ;
                nfc_msg_struct_loc->msg_length = sizeof(nfc_setting_request);
                P1 = nfc_msg_struct_loc;
                
                P1 += sizeof(nfc_msg_struct);
                
                memcpy(P1, &param_loc, sizeof(nfc_setting_request) );
                
                //memcpy( (nfc_msg_struct_loc->data), &nfc_script_request_loc, sizeof(nfc_script_request_loc) );
                    
                //TRY to SEND MESSAGE to NFC driver!! 
                nfc_ilm_ptr_loc = malloc(sizeof(ilm_struct) + sizeof(nfc_msg_struct) + sizeof(nfc_setting_request));
                nfc_ilm_ptr_loc->msg_id = MSG_ID_NFC_TEST_REQ; 
                nfc_ilm_ptr_loc->src_mod_id = 0;
                memcpy((nfc_ilm_ptr_loc->local_para_ptr), (char *)nfc_msg_struct_loc, (sizeof(nfc_msg_struct) + sizeof(nfc_setting_request)));           

                ret = write(nfc_sockfd, (const char*)nfc_ilm_ptr_loc, (sizeof(ilm_struct) + sizeof(nfc_msg_struct) + sizeof(nfc_setting_request))); 
                ptr += sprintf(ptr, " Write,%d, sizeof(%d)...\n",ret, (sizeof(ilm_struct) + sizeof(nfc_msg_struct) + sizeof(nfc_setting_request)) );                 
                iv->redraw(iv);
                ...
                {
                    rec_bytes = read(nfc_sockfd, &tmpbuf[0], sizeof(tmpbuf));  
                    //rec_bytes = read(nfc_sockfd, (char*)&nfc_ilm_ptr, sizeof(ilm_struct));            
                    if (rec_bytes > 0)
                    {
                    	  char *p;
            	          ilm_struct *nfc_ilm_ptr;
                    	  nfc_setting_response *result_ptr;
            	          //nfc_msg_struct *nfc_msg_struct_ptr;
                    	  
                    	  nfc_ilm_ptr = (ilm_struct*) tmpbuf;
                    	  
                    	  //nfc_msg_struct_ptr = (nfc_msg_struct*) nfc_ilm_ptr->local_para_ptr;
                    	  p = nfc_ilm_ptr->local_para_ptr;
                    	  p += sizeof(nfc_msg_struct);
                    	  
                    	  result_ptr = (nfc_setting_response*) p;
                    	                      	  
                    	  //nfc_script_response_loc = 
                        ptr += sprintf(ptr, " %d,ilm,Msg,%d,test_result,%d,0x%x\n",
                                      rec_bytes,
                                      nfc_ilm_ptr->msg_id,
                                      //nfc_msg_struct_ptr->msg_type,
                                      result_ptr->status,
                                      result_ptr->sw_ver
                                      ); 
                        iv->redraw(iv);
             ...
             if(i4TestItem == ITEM_20)
             {
                ...
                nfc_msg_struct_loc->msg_type = MSG_ID_NFC_DISCOVERY_REQ;
                nfc_msg_struct_loc->msg_length = sizeof(nfc_setting_request);
                P1 = nfc_msg_struct_loc;
                ...
                nfc_ilm_ptr_loc = malloc(sizeof(ilm_struct) + sizeof(nfc_msg_struct) + sizeof(nfc_setting_request));
                nfc_ilm_ptr_loc->msg_id = MSG_ID_NFC_TEST_REQ; 
                //nfc_ilm_ptr_loc->local_para_ptr =  nfc_msg_struct_loc;  
                memcpy((nfc_ilm_ptr_loc->local_para_ptr), (char *)nfc_msg_struct_loc, (sizeof(nfc_msg_struct) + sizeof(nfc_setting_request)));           
