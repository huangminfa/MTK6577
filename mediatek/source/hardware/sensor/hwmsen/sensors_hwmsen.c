...
extern struct sensor_t sSensorList[MAX_NUM_SENSORS];
...
static int open_input(int mode)
{
	...
	*filename++ = '/';
	...
	if (fd < 0)
	{
		LOGE("%s: Couldn't find or open '%s' driver (%s)",__func__, HWM_SENSOR_DEV_NAME, strerror(errno));
	}
	return fd;
}
        ...
        int handle, int enabled)
{
	...
	{
		LOGE("%s: handle %d para error!",__func__, handle);
		return -1;
	}
	...
	{
		LOGE("%s: file handle is null!",__func__);
		return -1;
	}
	
	LOGD("%s: handle %d, enable or disable %d!", __func__, handle, enabled);
	...
	if(enabled == 1)
	{
		...
		{
			LOGE("%s: Enable sensor %d error!",__func__, handle);
			return -1;
		}
			...
			&& (sensor & SENSOR_ORIENTATION))					// new orientation sensor start
		{
			...
			{
				LOGE("%s: Enable ACCELEROMETR sensor error!",__func__);
				return -1;
			}
			...
			if(ioctl(dev->device_io_fd, HWM_IO_ENABLE_SENSOR_NODATA, &io_value))
			{
				LOGE("%s: Enable MAGNETIC sensor error!",__func__);
				return -1;
			}
		}
	...
	else
	{
			...
			&& (sensor & SENSOR_ORIENTATION))
		{
			...
			{
				LOGE("%s: Disable ACCELEROMETR sensor error!",__func__);
				return -1;
			}
			...
			if(ioctl(dev->device_io_fd, HWM_IO_DISABLE_SENSOR_NODATA, &io_value))
			{
				LOGE("%s: Disable MAGNETIC sensor error!",__func__);
				return -1;
			}
		...
		{
			LOGE("%s: Disable sensor %d error!",__func__, handle);
			return -1;
		}
	}
...

{
	...
	LOGD("%s: Set delay %lld ns", __func__,ns);
    ...
    {
        delayPara.delay = 10;
        LOGD("Control set delay %lld ns is too small \n", ns);
    }	

	// TODO: Set delay time to device
	if(ioctl(dev->device_io_fd, HWM_IO_SET_DELAY, &delayPara))
	{
		LOGE("%s: Set delay %d ms error ", __func__, delayPara.delay);
		return -errno;
	}
...
static int hwm__poll(struct sensors_data_context_t *dev, sensors_event_t* values, int count)
{
	...
	if(fd_event < 0)
	{
		LOGE("%s: invalid file descriptor, fd=%d",__func__, fd_event);
		return -1;
	}
	...
	if(nread == sizeof(event))
	{
		...
		if(event.type == EV_REL)
		{
			if((event.code == EVENT_TYPE_SENSOR) && (event.value != 0))
			{
				...
				if(res != 0)
				{
					LOGE("%s: Get sensors data error",__func__);
				}
				else
				{
...
static int data__close(struct hw_device_t *dev) 
{
	...
	if (ctx)
	{
		if (ctx->events_fd > 0)
		{
			LOGD("%s: about to close fd=%d",__func__, ctx->events_fd);
			close(ctx->events_fd);
		}
		if(ctx->device_io_fd > 0)
		{
			LOGD("%s: about to close fd=%d",__func__, ctx->device_io_fd);
