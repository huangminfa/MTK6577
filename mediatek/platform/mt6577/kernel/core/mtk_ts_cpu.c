...
mutex_lock(&TS_lock);
		...
		t_ret = thermal_cal_exec_abb(ret);
		t_ret = t_ret * 100;
		//last_abb_t = t_ret;
		abb[MA_counter] = t_ret;


		//turn off buffer to save power
...
mutex_unlock(&TS_lock);
    ...
    t_ret = t_ret * 1000;
		...
		if(MA_first_time==0)
		{
				last_cpu_t = cpu[MA_counter];
				last_abb_t = abb[MA_counter];
				last_pa_t = pa[MA_counter];
		}	
		else
		{
				t_ret = t_ret1 = t_ret2 = 0;
				for(i=0; i<MA_len; i++)
				{
						t_ret += cpu[i];
						t_ret1 += abb[i];
				...
				last_cpu_t = t_ret / MA_len;
				last_abb_t = t_ret1 / MA_len;
				last_pa_t = t_ret2 / MA_len;
...
static int CPU_full_loading(void)
{
		...
		if (fp!=NULL) 
		{
				...
				//mtktscpu_dprintk("[mtktscpu_full_loading]read write end\n");
				closeFile(fp); 
				return 0;
		}
		else{
...
/*
void disable_old_cpu_management(void)
{
	//disable previous_step
	if(previous_step==0)
		cpu_opp_limit(EVENT_THERMAL,DVFS_OPP1 ,2, false);
	else if(previous_step==1)
		cpu_opp_limit(EVENT_THERMAL,DVFS_OPP2 ,2, false);
	else if(previous_step==2)
		cpu_opp_limit(EVENT_THERMAL,DVFS_OPP3 ,2, false);
	else if(previous_step==3)
		cpu_opp_limit(EVENT_THERMAL,DVFS_OPP4 ,2, false);
	else if(previous_step==4)
		cpu_opp_limit(EVENT_THERMAL,DVFS_OPP1 ,1, false);
	else if(previous_step==5)
		cpu_opp_limit(EVENT_THERMAL,DVFS_OPP2 ,1, false);
	else if(previous_step==6)
		cpu_opp_limit(EVENT_THERMAL,DVFS_OPP3 ,1, false);
	else if(previous_step==7)
		cpu_opp_limit(EVENT_THERMAL,DVFS_OPP4 ,1, false);
	else if(previous_step==8)
		cpu_opp_limit(EVENT_THERMAL,DVFS_OPP5 ,1, false);
	else if(previous_step==9)
		cpu_opp_limit(EVENT_THERMAL,DVFS_OPP6 ,1, false);		
}
*/
...
static ssize_t mtktscpu_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
							...
							&time_msec, &MA_len_temp) == 33)
	{
		...
		if(0 != previous_step)
		{
			//cpu_opp_limit(EVENT_THERMAL,DVFS_OPP1 ,2, true);
			//disable_old_cpu_management();
			cpu_opp_mask("thermal", step0_mask, true);
			previous_step=0;
		}
#endif
