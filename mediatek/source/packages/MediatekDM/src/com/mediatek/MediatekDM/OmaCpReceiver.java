...
//	private static final String TAG = "OmaCpReceiver";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (intentAction.equals(CP_DM_CAPABILITY_ACTION)) {
            ...
            handleCpConfigMessage(simId, dmServerAddr);
        } else {
    ...
    private boolean handleCpConfigMessage(int simId, String serverAddr) {
        ...
        Log.i(TAG.CP, "Enter config DmServer addr : server addr = " + serverAddr + "sim id is "+simId);
        if (serverAddr == null) {
        ...
        if (Options.UseSmsRegister) {
            int sim = DmCommomFun.getSimRegisterID(mContext);
            if (sim == -1 || simId != sim) {
                Log.e(TAG.CP, "sim card is not register OR cp sim card is not the register sim card.");
                return false;
            }
        }
