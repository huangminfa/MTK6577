...
private:
    ...
    /** M: Add 
         *    1. mXOffset, mYOffset to store GPOS table information
         *    2. getRun function to devide text into single run with same font and same script
       @{ */
private:
    /**
         * X, Y offset to store GPOS informaiton
         */
    Vector<jfloat> mXOffset;
    Vector<jfloat> mYOffset;
    /**
        * Devide text into single run 
        */
    static void getRun(SkPaint* paint, const UChar* chars, size_t start, size_t length, size_t* runLength, int32_t* fontID, size_t* glyphBase);
    static void computeValuesWithHarfbuzz(SkPaint* paint, const UChar* chars,
            size_t start, size_t count, size_t contextCount, int dirFlags,
            Vector<jfloat>* const outAdvances, jfloat* outTotalAdvance,
            Vector<jchar>* const outGlyphs
            /** M: Add x, y offset vector to store GPOS info  @{ */
            , 
    ...
    static void computeRunValuesWithHarfbuzz(HB_ShaperItem& shaperItem, SkPaint* paint,
            ...
            Vector<jchar>* const outGlyphs
            /** M: 
                       *    1. Add new parameter to convert font glyph index to Skia global glyph id  
                       *    2. Add x, y offset vector to store GPOS info
                       *
                       @{ */
            , 
            size_t glyphBase,
...
public:
    inline const jfloat* getXOffset() const { return mXOffset.isEmpty()? NULL: mXOffset.array(); }
    inline const jfloat* getYOffset() const { return mYOffset.isEmpty()? NULL : mYOffset.array(); }
    /** @} */
