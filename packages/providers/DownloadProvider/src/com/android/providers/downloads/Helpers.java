...
public class Helpers {
    ...
    public static boolean mContinueDownload;
    public static String mNotificationPackage;
    ...
    static String generateSaveFile(
        ...
        String path;
        File base = null;
        mNotificationPackage = notificationPackage;
        mContinueDownload = continueDownload;
        if (destination == Downloads.Impl.DESTINATION_FILE_URI) {
            if (hint.contains("%")) {
                // Ur.parse().getPath will decode the path, if there are any '%' in hint, it will be
                // decoded as an encoded url, which is not what we want. Folder name can have '%', which is
                // completely legal, but it will be decoded and becomes another different folder, as a result
                // files are saved to a different folder instead of ones user has specified.
                // So, only extract path without any decoding.
    ...
    private static String chooseUniqueFilename(int destination, String filename,
        ...
        if ( optr != null && optr.equals("OP02")
                && (mNotificationPackage.equals(Constants.BROWSER_NOTIFICATION_PACKAGE)
                || mNotificationPackage.equals(Constants.OMADL_NOTIFICATION_PACKAGE))) {
    ...
    static boolean isMtkDRMFile(String mimeType) {
        ...
        return mimeType.equalsIgnoreCase(DrmStore.DrmObjectMime.MIME_DRM_MESSAGE) ||
                mimeType.equalsIgnoreCase(DrmStore.DrmObjectMime.MIME_DRM_CONTENT) ||
                mimeType.equalsIgnoreCase(DrmStore.DrmObjectMime.MIME_RIGHTS_WBXML)||
                mimeType.equalsIgnoreCase(DrmStore.DrmObjectMime.MIME_RIGHTS_XML);
    }
    ...
    static boolean isMtkDRMRightFile(String mimeType) {
        ...
        return mimeType.equalsIgnoreCase(DrmStore.DrmObjectMime.MIME_RIGHTS_WBXML)||
                mimeType.equalsIgnoreCase(DrmStore.DrmObjectMime.MIME_RIGHTS_XML);
    }
}
