...
class VisualizerView extends View {
    ...
    private final int MARGIN = 3;//margin between bars in horizontal direction.
    private byte[] mBytes;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mBarCoordines == null) {
            mBarCoordines = new Rect[BANDS];
            int bar_width = (getWidth()-MARGIN - 1*BANDS)/BANDS;
            int height = getHeight();
            ...
            for (int i = 0;i < BANDS;i++) {
                ...
                mBarCoordines[i].left = MARGIN + (1 + bar_width)*i;
                mBarCoordines[i].right = mBarCoordines[i].left + bar_width;
                mBarCoordines[i].top = mBarCoordines[i].bottom = mCeilings[i] = height;
            }   
        }
        calculateTops();
