...
static bool bt_sendind(native_data_t *nat, void* ptr, int size)
{
    ...
    */
    ...
    LOGI("[JNI] bt_sendind(ptr=0x%X, len=%d)", ptr, size);

    ret = -1;
    ilm = (ilm_struct*)ptr;
    ilm->src_mod_id = MOD_MMI;
    ilm->dest_mod_id = MOD_BT;
    LOGI("[JNI] send ind=%d", ilm->msg_id);
    if(nat->servsock >= 0)
    {
        ret = sendto(nat->servsock, ptr, size, 0, (const sockaddr*)&name, namelen);
        if(ret < 0)
        {
            LOGE("[JNI] send ind fail : %s, %d", strerror(errno), errno);
        }
        else
        {
            LOGI("[JNI] send ind success : %d", ret);
        }
    }
    else
    {
...
static void btmtk_gap_send_name_change_event(native_data_t *nat, char *name, int namelen)
{
    ...
    LOGI("[GAP] btmtk_gap_send_name_change_event");
    ...
    {
        LOGE("[GAP] Send ANDROID_EV_GAP_LOCAL_NAME_CHANGE failed");
    }
}
...
static void btmtk_gap_send_scan_mode_timeout_change_event(native_data_t *nat, unsigned int timeout)
{
    ...
    LOGI("[GAP] btmtk_gap_send_scan_mode_timeout_change_event");
    memset(&event, 0x0, sizeof(ev_struct));
    ...
    *(unsigned int *)event.ilm_data = timeout;
    ...
    {
        LOGE("[GAP] Send ANDROID_EV_GAP_SCAN_MODE_TIMOUET_CHANGE failed");
    }
}
...
static void btmtk_gap_send_discovery_start_event(native_data_t *nat)
{
    ...
    LOGI("[GAP] btmtk_gap_send_discovery_start_event");
    memset(&event, 0x0, sizeof(ev_struct));
    event.msg_id = ANDROID_EV_GAP_DISCOVERY_START;
    if(bt_sendind(nat, (void*)&event, 0) == false)
    {
        LOGE("[GAP] Send ANDROID_EV_GAP_DISCOVERY_START failed");
    }
}
...
static void btmtk_gap_send_discovery_stop_event(native_data_t *nat)
{
    ...
    LOGI("[GAP] btmtk_gap_send_discovery_stop_event");
    memset(&event, 0x0, sizeof(ev_struct));
    event.msg_id = ANDROID_EV_GAP_DISCOVERY_STOP;
    if (bt_sendind(nat, (void*)&event, 0) == false)
    {
        LOGE("[GAP] Send ANDROID_EV_GAP_DISCOVERY_STOP failed");
    }
}
...
static void btmtk_gap_send_sdp_device_create_event(native_data_t *nat, bt_addr_struct *addr)
{
    ...
    LOGI("[GAP] btmtk_gap_send_device_create_event");
    memset(&event, 0x0, sizeof(ev_struct));
    ...
    {
        LOGE("[GAP] Send ANDROID_EV_GAP_DEVICE_CREATE failed");
    }
}
...
static void btmtk_gap_send_sdp_paired_device_create_event(native_data_t *nat, bt_addr_struct *addr)
{
    ...
    LOGI("[GAP] btmtk_gap_send_sdp_paired_device_create_event");
    memset(&event, 0x0, sizeof(ev_struct));
    event.msg_id = ANDROID_EV_SDP_PAIRED_DEVICE_CREATE;
    memcpy(event.ilm_data, addr, sizeof(bt_addr_struct));
    if (bt_sendind(nat, (void*)&event, sizeof(bt_addr_struct)) == false)
    {
        LOGE("[GAP] Send ANDROID_EV_SDP_PAIRED_DEVICE_CREATE failed");
    }
}
...
static void btmtk_gap_send_device_trusted_event(native_data_t *nat, bt_addr_struct *addr, bool trusted)
{
    ...
    LOGI("[GAP] btmtk_gap_send_device_trusted_event: trusted=%d", trusted);
    memset(&event, 0x0, sizeof(ev_struct));
    ...
    {
        LOGE("[GAP] Send ANDROID_EV_DEVICE_TRUSTED failed");
    }
}
...
static void btmtk_gap_send_paired_device_rename_event(native_data_t *nat, bt_addr_struct *addr, char *name)
{
    ...
    LOGI("[GAP] btmtk_gap_send_paired_device_rename_event");
    memset(&event, 0x0, sizeof(ev_struct));
	...
	LOGI("name is %s,len is %d",ptr->name,ptr->name_len);

	
    if (bt_sendind(nat, (void*)&event, sizeof(btmtk_android_gap_remote_name_event_struct)) == false)
    {
        LOGE("[GAP] Send btmtk_gap_send_paired_device_rename_event failed");
    }
}
...
static void btmtk_gap_send_sdp_discover_event(native_data_t *nat, bt_addr_struct *addr, const char *pattern, int pattern_len)
{
    ...
    LOGI("[GAP] btmtk_gap_send_sdp_discover_event");
    memset(&event, 0x0, sizeof(ev_struct));
    ...
    {
        LOGE("[GAP] Send ANDROID_EV_SDP_DISCOVER failed");
    }
}
...
static void btmtk_gap_init(JNIEnv *env, jobject object)
{
  ...
  //  event_nat->host_cache.scan_mode = BTBM_ADP_P_ON_I_OFF;
    ...
    btmtk_gap_set_scanable_mode(nat, (btbm_scan_enable_type)event_nat->host_cache.scan_mode);
...
bool btmtk_gap_power_on(native_data_t *nat)
{
        ...
        wait_response(nat->servsock, MSG_ID_BT_POWERON_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_POWERON_CNF: result=%d", cnf_p->result);
        ret = cnf_p->result;
    }

    if(!ret)
    {
        LOGE("[GAP] btmtk_gap_power_on failed");
    }
    return ret;
}
...
bool btmtk_gap_power_off(native_data_t *nat)
{
       ...
       wait_response(nat->servsock, MSG_ID_BT_POWEROFF_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_POWEROFF_CNF: result=%d", cnf_p->result);
        ret = cnf_p->result;
    }

    if(!ret)
    {
        LOGE("[GAP] btmtk_gap_power_off failed");
    }
    return ret;
}
...
bool btmtk_gap_get_scanable_mode(native_data_t *nat, unsigned char *scanablemode)
{
    ...
    LOGI("[GAP] btmtk_gap_get_scanable_mode");
    memset(&ilm, 0x0, sizeof(ilm_struct));
       ...
       wait_response(nat->servsock, MSG_ID_BT_BM_READ_SCANENABLE_MODE_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_BM_READ_SCANENABLE_MODE_CNF: result=%d, mode=%d", 
    ...
    if(!ret)
    {
        LOGE("[GAP] btmtk_gap_get_scanable_mode failed");
    }
    return ret;
}
...
bool btmtk_gap_set_scanable_mode(native_data_t *nat, btbm_scan_enable_type scanablemode)
{
    ...
    LOGI("[GAP] btmtk_gap_set_scanable_mode: mode=0x%X", scanablemode);
    memset(&ilm, 0x0, sizeof(ilm_struct));
       ...
       wait_response(nat->servsock, MSG_ID_BT_BM_WRITE_SCANENABLE_MODE_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_BM_WRITE_SCANENABLE_MODE_CNF: result=%d", cnf_p->result);
        if (cnf_p->result == BTBM_ADP_SUCCESS)
        {
    ...
    if(!ret)
    {
        LOGE("[GAP] btmtk_gap_set_scanable_mode failed");
    }
    return ret;
}
...
bool btmtk_gap_get_local_cod(native_data_t *nat, unsigned long *cod)
{
    ...
    LOGI("[GAP] btmtk_gap_get_local_cod");
    memset(&ilm, 0x0, sizeof(ilm_struct));
       ...
       wait_response(nat->servsock, MSG_ID_BT_BM_READ_LOCAL_COD_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_BM_READ_LOCAL_COD_CNF is received : result=%d, CoD=0x%X", cnf_p->result, cnf_p->cod);
        if(cnf_p->result == BTBM_ADP_SUCCESS)
        {
            *cod = cnf_p->cod;
            ret = true;
    ...
    if(!ret)
    {
        LOGE("[GAP] btmtk_gap_get_local_cod failed");
    }
    return ret;
}
...
bool btmtk_gap_set_local_cod(native_data_t *nat, unsigned long cod)
{
    ...
    LOGI("[GAP] btmtk_gap_set_local_cod: cod = 0x%X", cod);
    memset(&ilm, 0x0, sizeof(ilm_struct));
       ...
       wait_response(nat->servsock, MSG_ID_BT_BM_WRITE_LOCAL_COD_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT)  > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_BM_WRITE_LOCAL_COD_CNF: result=%d", cnf_p->result);
        ret = (cnf_p->result == BTBM_ADP_SUCCESS);
    }
    
    if(!ret)
    {
        LOGE("[GAP] btmtk_gap_set_local_cod failed");
    }
    return ret;
}
...
bool btmtk_gap_get_local_name(native_data_t *nat, char *name, int *namelen)
{
    ...
    LOGI("[GAP] btmtk_gap_get_local_name");
    memset(&ilm, 0x0, sizeof(ilm_struct));
       ...
       wait_response(nat->servsock, MSG_ID_BT_BM_READ_LOCAL_NAME_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_BM_READ_LOCAL_NAME_CNF: name=%s, namelen=%d", cnf_p->name, cnf_p->name_len);
        if (cnf_p->name_len)
        {
    ...
    if(!ret)
    {
        LOGE("[GAP] btmtk_gap_get_local_name failed");
    }
    return ret;
}
...
bool btmtk_gap_set_local_name(native_data_t *nat, char *name, int namelen)
{
    ...
    LOGI("[GAP] btmtk_gap_set_local_name : name=%s, len=%d", name, namelen);
    memset(&ilm, 0x0, sizeof(ilm_struct));
       ...
       wait_response(nat->servsock, MSG_ID_BT_BM_WRITE_LOCAL_NAME_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_BM_WRITE_LOCAL_NAME_CNF: result=%d", cnf_p->result);
        if(cnf_p->result == BTBM_ADP_SUCCESS)
        {
            if (nat->state != BTMTK_POWER_STATE_TURNING_ON)
            {
    ...
    if(!ret)
    {
        LOGE("[GAP] btmtk_gap_set_local_name failed");
    }
...
bool btmtk_gap_get_local_addr(native_data_t *nat, char *buf)
{
    ...
    LOGI("[GAP] btmtk_gap_get_local_addr");
    memset(&ilm, 0x0, sizeof(ilm_struct));
       ...
       wait_response(nat->servsock, MSG_ID_BT_BM_READ_LOCAL_ADDR_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_BM_READ_LOCAL_ADDR_CNF: result=%d, address=%s", cnf_p->result, buf);
        ret = (cnf_p->result == BTBM_ADP_SUCCESS);
    }
...
bool btmtk_gap_local_uuid(native_data_t *nat, unsigned short buf[], int * len)
{
       ...
       wait_response(nat->servsock, MSG_ID_BT_BM_READ_LOCAL_UUID_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
		...
		*len = btmtk_util_convert_uuidlist_2_uuid16(cnf_p->service_list1,
											 ...
											 cnf_p->service_list6,
											 buf);
        LOGI("[GAP] MSG_ID_BT_BM_READ_LOCAL_UUID_CNF: result=%d", cnf_p->result);
        ret = (cnf_p->result == BTBM_ADP_SUCCESS);
    }

    if(!ret)
    {
        LOGE("[GAP] btmtk_gap_get_local_uuid failed");
    }
    return ret;
...
bool btmtk_gap_get_remote_name_cod(native_data_t *nat, btbm_bd_addr_t *bd_addr, char *name, unsigned long *namelen,  unsigned long *cod, unsigned long timeout)
{
    ...
    LOGI("[GAP] btmtk_gap_get_remote_name_cod: addr=%X:%X:%X", bd_addr->lap, bd_addr->uap, bd_addr->nap);
    memset(&ilm, 0x0, sizeof(ilm_struct));
       ...
       wait_response(nat->servsock, MSG_ID_BT_BM_READ_REMOTE_NAME_CNF, &ilm, timeout) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_BM_READ_REMOTE_NAME_CNF: result=%d, name=%s, cod=0x%X", cnf_p->result, cnf_p->name, cnf_p->cod);
        if(cnf_p->result == BTBM_ADP_SUCCESS)
        {
            strncpy(name, (char *)cnf_p->name, *namelen);
    ...
    if(!ret) 
    {
        LOGE("[GAP] btmtk_gap_get_remote_name_cod failed");
    }
    return ret;
}
...
void btmtk_gap_discovery_cancel(native_data_t *nat)
{
       ...
       wait_response(nat->servsock, MSG_ID_BT_BM_DISCOVERY_CANCEL_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_BM_DISCOVERY_CANCEL_CNF: result=%d, total=%d", msg->result, msg->total_number);
        btmtk_gap_send_discovery_stop_event(nat);
    }
}
...
bool btmtk_gap_bond(native_data_t *nat, bt_addr_struct *addr, bool do_sdp)
{
    ...
    LOGI("[GAP] btmtk_gap_bond: addr=%X:%X:%X, do_sdp=%d", addr->lap, addr->uap, addr->nap, do_sdp);
    memset(&ilm, 0x0, sizeof(ilm_struct));
...
bool btmtk_gap_bond_cancel(native_data_t *nat, bt_addr_struct *addr)
{
    ...
    LOGI("[GAP] btmtk_gap_bond_cancel: addr=%X:%X:%X", addr->lap, addr->uap, addr->nap);
    memset(&ilm, 0x0, sizeof(ilm_struct));
    ...
    if (!ret)
    {
        LOGI("[GAP] btmtk_gap_bond_cancel failed");
    }
...
bool btmtk_gap_unpair(native_data_t *nat, bt_addr_struct *addr)
{
    ...
    LOGI("[GAP] btmtk_gap_unpair: addr=%X:%X:%X", addr->lap, addr->uap, addr->nap);
    memset(&ilm, 0x0, sizeof(ilm_struct));
...
bool btmtk_gap_pin_res(native_data_t *nat, bt_addr_struct *addr, const char *pin)
{
    ...
    LOGI("[GAP] btmtk_gap_pin_res: addr=%X:%X:%X, pin=%s", addr->lap, addr->uap, addr->nap, pin);
    memset(&ilm, 0x0, sizeof(ilm_struct));
...
bool btmtk_gap_security_user_confirm(native_data_t *nat, bt_addr_struct *addr, bool accept)
{
    ...
    LOGI("[GAP] btmtk_gap_security_user_confirm: addr=%X:%X:%X", addr->lap, addr->uap, addr->nap);
    memset(&ilm, 0x0, sizeof(ilm_struct));
...
bool btmtk_gap_security_passkey_entry(native_data_t *nat, bt_addr_struct *addr, bool accept, int passkey)
{
    ...
    LOGI("[GAP] btmtk_gap_security_passkey_entry: addr=%X:%X:%X, accept=%d, passkey=%d", addr->lap, addr->uap, addr->nap, accept, passkey);
    memset(&ilm, 0x0, sizeof(ilm_struct));
...
bool btmtk_gap_security_passkey_notify(native_data_t *nat, bt_addr_struct *addr, bool accept)
{
    ...
    LOGI("[GAP] btmtk_gap_security_passkey_notify: addr=%X:%X:%X, accept=%d", addr->lap, addr->uap, addr->nap, accept);
    memset(&ilm, 0x0, sizeof(ilm_struct));
...
bool btmtk_gap_security_oob_data(native_data_t *nat, bt_addr_struct *addr, bool accept, char *hash, char *rand)
{
    ...
    LOGI("[GAP] btmtk_gap_security_oob_data: addr=%X:%X:%X, accept=%d", addr->lap, addr->uap, addr->nap, accept);
    memset(&ilm, 0x0, sizeof(ilm_struct));
...
bool btmtk_gap_service_search_request(native_data_t *nat, bt_addr_struct *addr)
{
    ...
    LOGI("[GAP] btmtk_gap_service_search_request addr=%X:%X:%X", addr->lap, addr->uap, addr->nap);
    memset(&ilm, 0x0, sizeof(ilm_struct));
	...
	if (entry == NULL) 
	{
		...
		if (entry == NULL) 
		{
			LOGE("The device is not in cache list");
			return FALSE;
		}
	}
...
bool btmtk_gap_service_attrib_search_request(native_data_t *nat, bt_addr_struct *addr, btbm_uuid_type uuid_type, unsigned short uuid16, char *uuid128, unsigned short attrib, void *value)
{
    ...
    LOGI("[GAP] btmtk_gap_service_attrib_search_request addr=%X:%X:%X, uuid_type=%d, uuid=0x%X, attrib=0x%X", addr->lap, addr->uap, addr->nap, uuid_type, uuid16, attrib);
    memset(&ilm, 0x0, sizeof(ilm_struct));
       ...
       wait_response(nat->twinservsock, MSG_ID_BT_BM_SEARCH_ATTRIBUTE_CNF, &ilm, 30000) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_BM_SEARCH_ATTRIBUTE_CNF: result=%d", cnf_p->result);
        if (cnf_p->result == BTBM_ADP_SUCCESS)
        {
            LOGI("[GAP] MSG_ID_BT_BM_SEARCH_ATTRIBUTE_RESULT_IND: len=%d", cnf_p->len);
            LOGI("0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x", cnf_p->data[0], cnf_p->data[1], cnf_p->data[2]
    ...
    if(!ret)
    {
        LOGE("[GAP] btmtk_gap_service_attrib_search_request failed");
    }
    return ret;
}
...
bool btmtk_gap_service_search_raw_request(native_data_t *nat, bt_addr_struct *addr, const char *pattern, int pattern_size)
{
    ...
    LOGI("[GAP] btmtk_gap_service_search_raw_request addr=%X:%X:%X, size=%d", addr->lap, addr->uap, addr->nap, pattern_size);
    memset(&ilm, 0x0, sizeof(ilm_struct));
...
int btmtk_jsr82_int_compose_record(char **buf, const char *name, char *uuid, short channel)
{
    ...
    LOGI("btmtk_jsr82_int_compose_record: calculated size=%d ", size);
    ...
    if (idx >= size + 50)
    {
        LOGE("error!!!! btmtk_jsr82_int_compose_record: size=%d >= malloc size=%d", idx, size + 50);
    }
    LOGI("btmtk_jsr82_int_compose_record: size=%d ", idx);
    return idx;
}
...
int btmtk_jsr82_create_record(native_data_t *nat, char **rec, int rec_len)
{
    ...
    LOGI("[GAP] btmtk_jsr82_create_record rec_len=%d", rec_len);
    memset(&ilm, 0x0, sizeof(ilm_struct));
        ...
        wait_response(nat->servsock, MSG_ID_BT_APP_SDPDB_GET_HANDLE_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_APP_SDPDB_GET_HANDLE_CNF: result=%d, handle=%d", cnf_p->result, cnf_p->handle);
        if (cnf_p->result == BTSDPDBAPP_SUCCESS)
        {
            ...
            {
                LOGE("[GAP] rec_len=%d > BT_SDPDB_MAX_SERVICE_RECORD_SIZE", rec_len);
                free(*rec);
                *rec = NULL;
                return -1;
            }
                ...
                wait_response(nat->servsock, MSG_ID_BT_APP_SDPDB_REGISTER_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
            {
                ...
                LOGI("[GAP] MSG_ID_BT_APP_SDPDB_REGISTER_CNF: result=%d, handle=%d", cnf2_p->result, cnf2_p->handle);
...
bool btmtk_jsr82_remove_record(native_data_t *nat, int handle)
{
    ...
    LOGI("[GAP] btmtk_jsr82_remove_record, handle=%d", handle);
    memset(&ilm, 0x0, sizeof(ilm_struct));
        ...
        wait_response(nat->servsock, MSG_ID_BT_APP_SDPDB_DEREGISTER_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_APP_SDPDB_DEREGISTER_CNF: result=%d, handle=%d", cnf_p->result, cnf_p->handle);
        ret = (cnf_p->result == BTSDPDBAPP_SUCCESS) ? true : false;
    }

    if(!ret)
    {
        LOGE("[GAP] btmtk_jsr82_remove_record failed");
    }
    return ret;
}
...
bool btmtk_gap_set_ssp_debug_mode(native_data_t *nat, bool on)
{
    ...
    LOGI("[GAP] btmtk_gap_set_ssp_debug_mode: on = 0x%X", on);
    memset(&ilm, 0x0, sizeof(ilm_struct));
       ...
       wait_response(nat->servsock, MSG_ID_BT_SSP_DEBUG_MODE_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT)  > 0)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_SSP_DEBUG_MODE_CNF: result=%d", cnf_p->result);
        ret = cnf_p->result;
    }
    
    if(!ret)
    {
        LOGE("[GAP] btmtk_gap_set_ssp_debug_mode failed");
    }
    return ret;
}
...
int btmtk_hdp_create_socket(U8 l2capchannelId)
{
	...
	LOGI("btmtk_hdp_create_socket : len=%d, addr=%s", hdpnamelen, hdpname.sun_path);

	if (socketFd < 0)
    {
        LOGE("[HDP]create HDP socket failed : %s, errno=%d", strerror(errno), errno);
        goto exit;
    }
    ...
    {
    	LOGE("[HDP] getsockname failed : %s, errno=%d", strerror(errno), errno);
		goto exit;
    }
    else
	{
    	LOGI("[HDP] getsockname success : len=%d, addr=%s", hdpnamelen, &hdpname.sun_path[1]);
    }
    if ( connect(socketFd, (const struct sockaddr*)&btname, btnamelen) < 0)
    {
        LOGE("[HDP]] connect HDP socket failed : %s, errno=%d", strerror(errno), errno);
        goto exit;
    }
	else
	{
...
bool btmtk_hdp_destroy_socket(int socketFd)
{
	...
	socklen_t   hdpnamelen;
	bool ret = FALSE;
	LOGE("[HDP] btmtk_hdp_destroy_socket: %d", socketFd);
	...
	if (socketFd >= 0)
	{
        ...
        {
            LOGI("[HDP]getsockname : len=%d, addr=%s", hdpnamelen, &hdpname.sun_path[1]);
        }
		if (shutdown(socketFd, 2) != 0)
		{
			LOGI("[HDP]shutdown socket : %s, errno=%d", strerror(errno), errno);
		}
		close(socketFd);
		ret = TRUE;
	}
...
btmtk_hdp_fd_struct *btmtk_hdp_util_find_hdp_fd(native_data_t *nat, bt_addr_struct *addr, unsigned short mdlid)
{
    ...
    LOGI("[HDP] btmtk_util_find_hdp_fd search 0x%lX:0x%X:0x%X: mdl id=0x%X", addr->lap, addr->uap, addr->nap, mdlid);
    while (ptr)
    {
        LOGI("[HDP] btmtk_util_find_hdp_fd found 0x%lX:0x%X:0x%X", ptr->addr.lap, ptr->addr.uap, ptr->addr.nap);
...
void btmtk_hdp_add_fd(native_data_t *nat, bt_addr_struct *addr, unsigned short mdlid, int fd)
{
	...
	LOGI("btmtk_hdp_add_fd:header addr=%d", &nat->hdp_fds);
	LOGI("btmtk_hdp_add_fd:addr=%X:%X:%X, mdlid=%d, fd=%d",addr->lap, addr->uap, addr->nap, mdlid, fd);
	if (NULL != hdp_fd)
	{
		hdp_fd->fd = fd;
		hdp_fd->mdl_id = mdlid;
...
void btmtk_hdp_remove_fd(native_data_t *nat, bt_addr_struct *addr, unsigned short mdlid)
{
	...
	LOGI("btmtk_hdp_remove_fd:header addr=%d", &nat->hdp_fds);
	LOGI("btmtk_hdp_remove_fd:addr=%X:%X:%X, mdlid=%d",addr->lap, addr->uap, addr->nap, mdlid);
	if (NULL != hdp_fd)
	{
...
{
	LOGI("[HDP]btmtk_hdp_parse_channel channel path is :%s", channelPath);
	...
	*mdlId = atoi(p);
	...
	LOGI("[HDP]first string :%s", p);
	
//	p = strtok(NULL, HDP_CHANNEL_DELIMS); 
	LOGI("[HDP]sec string :%s", p);
...
bool btmtk_hdp_compose_channel(bt_addr_struct *addr, U16 mdlId, char *channelPath)
{
	...
	LOGI("[HDP]btmtk_hdp_compose_channel channel path is :%s", channelPath);
	return TRUE;	
										...
										U8 *mdepId)
{
	...
	LOGI("[HDP] btmtk_hdp_register_instance: on = 0x%X");
       ...
       wait_response(nat->servsock, MSG_ID_BT_HDP_REGISTER_INSTANCE_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
	{
        ...
        if(cnf->result == BT_HDP_SUCCESS)
        {
        	*mdepId = cnf->mdepId;
			...
			LOGI("[HDP] btmtk_hdp_register_instance success: mdep id = 0x%X", cnf->result);
        }
		else
		{
			LOGI("[HDP] btmtk_hdp_register_instance failed: result = 0x%X", cnf->result);
		}
	} 
	else
	{
		LOGE("[HDP] btmtk_hdp_register_instance timeout");
...
bool btmtk_hdp_deregister_instance(native_data_t *nat, U8 id)
{
	...
	LOGI("[HDP] btmtk_hdp_deregister_instance: 0x%X", id);
       ...
       wait_response(nat->servsock, MSG_ID_BT_HDP_DEREGISTER_INSTANCE_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
	{
		...
		if (cnf->mdepId != id)
		{
			LOGE("[GAP] btmtk_hdp_deregister_instance failed: mdep id mismatch 0x%X", cnf->mdepId);
		}
		else
		{
			LOGI("[GAP] btmtk_hdp_deregister_instance result:  0x%X", cnf->result);
...
bool btmtk_hdp_connect(native_data_t *nat, bt_addr_struct *addr, U8 mdepId, U8 config, U32 index)
{
	...
	LOGI("[HDP] btmtk_hdp_connect: %d", mdepId);
...
bool btmtk_hdp_get_main_channel(native_data_t *nat, bt_addr_struct *addr, U8 *mldId)
{
	...
	LOGI("[HDP] btmtk_hdp_get_main_channel");
       ...
       wait_response(nat->servsock, MSG_ID_BT_HDP_GET_MAIN_CHANNEL_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
	{
        ...
        {
        	*mldId = cnf->mdlId;
			LOGI("[HDP] btmtk_hdp_get_main_channel success: channel id = 0x%X", cnf->mdlId);
        }
		else
		{
			LOGI("[HDP] btmtk_hdp_get_main_channel failed: result = 0x%X", cnf->result);
		}
		ret = cnf->result == BT_HDP_SUCCESS;
	} 
	else
	{
		LOGE("[HDP] btmtk_hdp_get_main_channel timeout");
		ret = false;
	}
	return ret;
...
bool btmtk_hdp_get_instance(native_data_t *nat,  bt_addr_struct *addr, U8 mdlId, U8 *mdepId)
{
	...
	LOGI("[HDP] btmtk_hdp_get_instance: 0x%X", mdlId);
       ...
       wait_response(nat->servsock, MSG_ID_BT_HDP_GET_INSTANCE_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
	{
        ...
        {
        	*mdepId = cnf->mdepId;
			LOGI("[HDP] btmtk_hdp_get_instance success: mdep id = 0x%X", cnf->mdepId);
        }
		else
		{
			LOGI("[HDP] btmtk_hdp_get_instance failed: result = 0x%X", cnf->result);
		}
		ret = cnf->result == BT_HDP_SUCCESS;
	} 
	else
	{
		LOGE("[HDP] btmtk_hdp_get_instance timeout");
		ret = false;
	}
	return ret;
}
...
bool btmtk_hdp_get_l2cap_channel(native_data_t *nat, bt_addr_struct *addr, U8 mdlId, U16 *l2capChnId)
{
	...
	LOGI("[HDP] btmtk_hdp_get_l2cap_channel");
       ...
       wait_response(nat->servsock, MSG_ID_BT_HDP_GET_L2CAP_CHANNEL_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
	{
			...
			cnf->mdlId == mdlId)
        {
        	*l2capChnId = cnf->l2capId;
			LOGI("[HDP] btmtk_hdp_get_main_channel success: mdl id = 0x%X, l2cap id is %d", cnf->mdlId, cnf->l2capId);
        }
		else
		{
			LOGI("[HDP] btmtk_hdp_get_main_channel failed: result = 0x%X", cnf->result);
...
static bool initializeNativeDataNative(JNIEnv* env, jobject object) {
...
#ifdef __BTMTK__
    native_data_t *nat = (native_data_t *)calloc(1, sizeof(native_data_t));
    LOGI("android_server_BluetoothService.cpp: initializeNativeDataNative");
    if (NULL == nat) {
        LOGE("%s: out of memory!", __FUNCTION__);
        return false;
    }
    ...
    if (nat->servsock < 0)
    {
        LOGE("[GAP][ERR] create server socket failed");
        return false;
    }
...
static const char *get_adapter_path(JNIEnv* env, jobject object) {
    ...
    event_loop_native_data_t *event_nat =
        get_EventLoop_native_data(env, env->GetObjectField(object,
                                                           field_mEventLoop));
    LOGI("[GAP][API] get_adapter_path : set object path to \"%s\"", event_nat ? event_nat->adapter : "");
    if (event_nat == NULL)
    return NULL;
    return event_nat->adapter;
#endif
...
#ifdef __BTMTK__
    LOGI("[GAP][API] getAdapterPathNative : set object path to \"MTKBT\"");
    native_data_t *nat = get_native_data(env, object);
    if (nat) {
        return (env->NewStringUTF(get_adapter_path(env, object)));
    }    
...
static jboolean startDiscoveryNative(JNIEnv *env, jobject object, int mode) {
    ...
    LOGI("[GAP][API] startDiscoveryNative");
...
static jboolean stopDiscoveryNative(JNIEnv *env, jobject object) {
  ...
  native_data_t *nat = get_native_data(env, object);
    ...
    LOGI("[GAP][API] stopDiscoveryNative");
...
static jboolean createPairedDeviceNative(JNIEnv *env, jobject object,
    ...
    if (nat && nat->event_nat && nat->state == BTMTK_POWER_STATE_ON)
    {
        ...
        LOGI("[GAP][API] createPairedDeviceNative: address=%s, eventloop native data=0x%X", c_address, nat->event_nat);
        btmtk_util_convert_string2bdaddr((char*)c_address, &addr);
        if((ret = btmtk_gap_bond(nat, &addr, true)) == true)
        {
            btmtk_gap_send_sdp_paired_device_create_event(nat, &addr);
        }
        env->ReleaseStringUTFChars(address, c_address);
        LOGI("[GAP] createPairedDeviceNative sdp return %d", ret);
        return ret ? JNI_TRUE : JNI_FALSE;
    }
#endif
...
static jint getDeviceServiceChannelNative(JNIEnv *env, jobject object,
    ...
    if (nat && nat->event_nat && nat->state == BTMTK_POWER_STATE_ON)
    {
        ...
        LOGI("[GAP][API] getDeviceServiceChannelNative addr=%s, pattern:%s, attr=0x%X ", c_object_path, c_pattern, attr_id);
        ...
        for (i = 0; i < entry->sdp.uuid_no; i++)
        {
            ...
            {
                LOGI("[GAP][API] getDeviceServiceChannelNative cached channel=%d", entry->sdp.channel[i]);
...
static jboolean cancelDeviceCreationNative(JNIEnv *env, jobject object,
    ...
    if (nat && nat->state == BTMTK_POWER_STATE_ON)
    {
        ...
        LOGI("[GAP][API] cancelDeviceCreationNative: addr=%s", c_address);
        btmtk_util_convert_string2bdaddr((char*)c_address, &addr);
        ret = btmtk_gap_bond_cancel(nat, &addr);
        env->ReleaseStringUTFChars(address, c_address);
        LOGI("[GAP] btmtk_gap_bond_cancel return %d", ret);
        return ret ? JNI_TRUE : JNI_FALSE;
    }
#endif
...
static jboolean removeDeviceNative(JNIEnv *env, jobject object, jstring object_path) {
    ...
    if (nat && nat->state == BTMTK_POWER_STATE_ON) 
    {
        ...
        LOGI("[GAP][API] removeDeviceNative: addr=%s", c_object_path);
        btmtk_util_convert_objpath2bdaddr((char*)c_object_path, &addr);
        ret = btmtk_gap_unpair(nat, &addr);
        env->ReleaseStringUTFChars(object_path, c_object_path);
        LOGI("[GAP] btmtk_gap_unpair return %d", ret);
        return ret ? JNI_TRUE : JNI_FALSE;
    }
#endif
...
static jboolean setPairingConfirmationNative(JNIEnv *env, jobject object,
    ...
    if (nat && nat->state == BTMTK_POWER_STATE_ON) {
        ...
        LOGI("[GAP][API] setPairingConfirmationNative : addr=%s, confirm=%d, nativeData=%d", c_address, confirm, nativeData);
        btmtk_util_convert_string2bdaddr((char*)c_address, &addr);
        env->ReleaseStringUTFChars(address, c_address);
        ...
        default:
            LOGE("Unexpected type of pairing type=%d", nativeData);
            break;
        }
...
static jboolean setPasskeyNative(JNIEnv *env, jobject object, jstring address,
    ...
    if (nat && nat->state == BTMTK_POWER_STATE_ON) {
        ...
        LOGI("[GAP][API] setPasskeyNative : addr=%s, passkey=%u", c_address, passkey);
        btmtk_util_convert_string2bdaddr((char*)c_address, &addr);
        env->ReleaseStringUTFChars(address, c_address);
...
static jboolean setRemoteOutOfBandDataNative(JNIEnv *env, jobject object, jstring address,
    ...
    if (nat) {
        ...
        if (nat && nat->state == BTMTK_POWER_STATE_ON) {
            ...
            LOGI("[GAP][API] setRemoteOutOfBandDataNative : addr=%s", c_address);
            btmtk_util_convert_string2bdaddr((char*)c_address, &addr);
            env->ReleaseStringUTFChars(address, c_address);
...
static jboolean setPinNative(JNIEnv *env, jobject object, jstring address,
    ...
    if (nat && nat->state == BTMTK_POWER_STATE_ON) {
        const char *c_pin = env->GetStringUTFChars(pin, NULL);
        const char *c_address = env->GetStringUTFChars(address, NULL);
        LOGI("[GAP][API] setPinNative : addr=%s, pin=%s", c_address, c_pin);
        if (c_address)
        {
            btmtk_util_convert_string2bdaddr((char*)c_address, &addr);
            if (c_pin)
...
static jboolean cancelPairingUserInputNative(JNIEnv *env, jobject object,
    ...
    LOGI("[GAP][API] cancelPairingUserInputNative: addr=%s, nativeData=%d", c_address, nativeData);
    if(nat && c_address)
    {
        ilm_struct ilm;
        bt_addr_struct addr;
                                                    ...
                                                    jstring path)
{
    ...
    LOGI("[GAP][API] getDevicePropertiesNative: addr=%s", c_path);
	...
	entry->sdp.app_uuid_no = (entry->sdp.app_uuid_no < 0) ? 0 : entry->sdp.app_uuid_no;
		...
		LOGI("[GAP] create_prop_array: Nickname is %s ",entry->nickname);
        ...
        if (entry->sdp.uuid_no + entry->sdp.app_uuid_no)
        {
            ...
            for (i = 0; i < entry->sdp.uuid_no; i++)
            {
                ...
                if(value.array_val[i])
                {
                    ...
                    LOGI("[GAP] UUID: %s", value.array_val[i]);  
                }
            }
...
static jobjectArray getAdapterPropertiesNative(JNIEnv *env, jobject object) {
    ...
    LOGI("[GAP][API] getAdapterPropertiesNative");

    if (0 == env->PushLocalFrame(PROPERTIES_NREFS) )
        LOGI("[GAP]PushLocalFrame(%d) success", PROPERTIES_NREFS);
    else
        LOGI("[GAP]PushLocalFrame(%d) failed", PROPERTIES_NREFS);

    stringClass = env->FindClass("java/lang/String");
  
    if(btmtk_gap_local_uuid(nat, uuid, &uuid_len))
    {
        strArray = env->NewObjectArray(BTMTK_ADAPTER_PROP_TOTAL * 2 + nat->event_nat->paired_cache_no+uuid_len, 
							stringClass, NULL);
        LOGI("[GAP] uuid_len:%d",uuid_len);
    }	
    else 
    {
        strArray = env->NewObjectArray(BTMTK_ADAPTER_PROP_TOTAL * 2 + nat->event_nat->paired_cache_no, stringClass, NULL);
    }

    /* Get address */
    LOGI("[GAP] Get local address");	
    ...
    LOGI("[GAP] Get Discoverable & pairable");	
    ...
    if (nat->event_nat->paired_cache_no)
    {
        ...
        for (i = 0; i < nat->event_nat->paired_cache_no; i++)
        {
            ...
            if(value.array_val[i])
            {
                ...
                LOGI("[GAP] Get Devices: %s", value.array_val[i]);  
            }
        }
    }
	...
	for (i = 0; i < uuid_len; i++)
	{
		...
		LOGI("[GAP] Get UUID: %s", value.array_val[i]); 
		}
	}
    }
...
static jboolean setAdapterPropertyNative(JNIEnv *env, jobject object, jstring key,
    ...
    LOGI("[GAP][API] setAdapterPropertyNative");
    if (nat) 
    {
        bool ret = false;
        const char *c_key = env->GetStringUTFChars(key, NULL);
        ...
        {
            LOGW("[WARN] Unsupported property %s", c_key);
        }
        env->ReleaseStringUTFChars(key, c_key);
...
#ifdef __BTMTK__
    const char *c_value = env->GetStringUTFChars(value, NULL);
    LOGI("[GAP][API] setAdapterPropertyStringNative : %s", c_value);
    jboolean ret =  setAdapterPropertyNative(env, object, key, (void *)&c_value, DBUS_TYPE_STRING);
    env->ReleaseStringUTFChars(value, (char *)c_value);
    return ret;
#endif
...
#elif defined __BTMTK__
    LOGI("[GAP][API] setAdapterPropertyIntegerNative");
    return setAdapterPropertyNative(env, object, key, (void *)&value, DBUS_TYPE_UINT32);
#else
    return JNI_FALSE;
#endif
...
#elif defined __BTMTK__
    LOGI("[GAP][API] setAdapterPropertyBooleanNative");
    return setAdapterPropertyNative(env, object, key, (void *)&value, DBUS_TYPE_BOOLEAN);
#else
    return JNI_FALSE;
#endif
...
#endif
#ifdef __BTMTK__
    LOGI("[GAP][API] setDevicePropertyNative");
    native_data_t *nat = get_native_data(env, object);
   if (nat && nat->event_nat) 
    {
        ...
        LOGI("key=%s, path=%s", c_key, c_path);
        btmtk_util_convert_objpath2bdaddr(c_path, &addr);
        ...
        if(!strcmp(c_key, "Trusted"))
        {
			...
			LOGI("the string is %s",*(char**)value);
			btmtk_gap_send_paired_device_rename_event(nat, &addr, *(char**)value);
        }
        else
        {
            LOGW("key %s is not supported", c_key);
            ret = false;
        }
        env->ReleaseStringUTFChars(key, c_key);
        env->ReleaseStringUTFChars(path, c_path);
...
#elif defined __BTMTK__
    LOGI("[GAP][API] setDevicePropertyBooleanNative");
    return setDevicePropertyNative(env, object, path, key,
                                        (void *)&value, DBUS_TYPE_BOOLEAN);
#else
    return JNI_FALSE;
...
#elif defined __BTMTK__
	const char *c_value = env->GetStringUTFChars(value, NULL);
	LOGI("[GAP][API] setDevicePropertyStringNative : %s", c_value);
	jboolean ret = setDevicePropertyNative(env, object, path, key,
                                        (void *)&c_value, DBUS_TYPE_STRING);
	env->ReleaseStringUTFChars(value, (char *)c_value);
	return ret;
...
static jboolean createDeviceNative(JNIEnv *env, jobject object,
    ...
    if (nat)
    {
        ...
        LOGI("[GAP][API] createDeviceNative : addr=%s", c_address);
        btmtk_util_convert_string2bdaddr((char*)c_address, &addr);
...
static jboolean discoverServicesNative(JNIEnv *env, jobject object,
    ...
    LOGI("[GAP][API] discoverServicesNative : addr=%s, pattern=%s", c_path, c_pattern);
    if (nat && c_path)
    {
        ...
        if (c_pattern && strlen(c_pattern))
        {
            ...
            {
                LOGE("[GAP][API] discoverServicesNative : idx=%d exceed max %d", idx, BT_MAX_SDAP_RAW_SIZE);
            }
        }
        else
        {
...
static jint addRfcommServiceRecordNative(JNIEnv *env, jobject object,
    ...
    if (nat) 
    {
        ...
        LOGI("[GAP][API] addRfcommServiceRecordNative : name=%s, uuidMsb=%llX, uuidLsb=%llX, channel=%d", c_name, uuidMsb, uuidLsb, channel);
...
static jboolean removeServiceRecordNative(JNIEnv *env, jobject object, jint handle) {
    ...
    if (nat) 
    {
        LOGI("[GAP][API] removeServiceRecordNative : handle=%d", handle);
...
static jboolean setSSPDebugModeNative(JNIEnv *env, jobject object, jboolean on) {
    ...
    LOGI("[GAP][API] setSSPDebugModeNative");
    if (nat)
    {
...
static jstring registerHealthApplicationNative(JNIEnv *env, jobject object,
	...
	LOGI("[HDP]registerHealthApplicationNative: role-> %s, channel type->%s, name->%s",
    ...
    if (nat) {
		...
		if (BT_HDP_ROLE_INVALID != i_role)
		{
			...
			{
				LOGE("[HDP]fail to register APP: result %d, id %d", ret, id);
			}
		} 
                else
		{
			LOGE("[HDP] invalid parms: role %d, type %d", i_role, i_type);
		}
    }
	env->ReleaseStringUTFChars(role, c_role);
    env->ReleaseStringUTFChars(name, c_name);
...
#ifdef __BTMTK__
	native_data_t *nat = get_native_data(env, object);
	LOGI("[HDP]registerSinkHealthApplicationNative");
	if (nat) {
		const char *c_role = env->GetStringUTFChars(role, NULL);
		const char *c_name = env->GetStringUTFChars(name, NULL);
		U8 id = BT_HDP_INSTANCE_INVALID_ID;
...
static jboolean destroyChannelNative(JNIEnv *env, jobject object, jstring devicePath,
	...
	native_data_t *nat = get_native_data(env, object);
	bool ret;
	LOGI("[HDP]destroyChannelNative: invalide parms");
	if (nat) {
		const char *c_devicePath = env->GetStringUTFChars(devicePath, NULL);
		...
		{
			 LOGI("[HDP][API] destroyChannelNative: invalide parms");
		}
...
static jstring getMainChannelNative(JNIEnv *env, jobject object, jstring devicePath) {
    ...
    LOGI("[HDP][API] getMainChannelNative: addr=%s", c_address);
    if(nat && c_address)
    {
        bt_addr_struct addr;
...
static jboolean releaseChannelFdNative(JNIEnv *env, jobject object, jstring channelPath) {
    ...
    if (nat) {
		...
		const char *c_channelPath = env->GetStringUTFChars(channelPath, NULL);
		bool ret = FALSE;
		LOGE("[HDP] releaseChannelFdNative: %s", c_channelPath);
		...
		//		pthread_mutex_unlock(&(nat->event_nat->thread_mutex));
			...
			{
				LOGE("[HDP] releaseChannelFdNative: invalid parm");
			}
		}
		env->ReleaseStringUTFChars(channelPath, c_channelPath); 
		if (ret)
...
static jobject getChannelFdNative(JNIEnv *env, jobject object, jstring channelPath) {
    ...
    if (nat) {
		...
		LOGE("[HDP] getChannelFdNative invalid parm:%s", c_channelPath);
		if(nat && c_channelPath)
		{
			bt_addr_struct addr;			
			U8 mdlId;
		...
		//			pthread_mutex_unlock(&(nat->event_nat->thread_mutex));
				...
				{
					LOGE("[HDP] getChannelFdNative: fail to get l2cap channel");
				}
			}
			else 
			{
				LOGE("[HDP] getChannelFdNative invalid parm");
			}
