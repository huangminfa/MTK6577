...
public class OmacpReceiverService extends Service {
	...
	private static final String XLOG = "Omacp/OmacpReceiverService";
    ...
    public void onCreate() {
		if(DEBUG){
			Xlog.i(XLOG, "OmacpReceiverService onCreate");
		}		
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
		if(DEBUG){
			Xlog.i(XLOG, "OmacpReceiverService onStartCommand");
		}		
    ...
    public void onDestroy() {
    	if(DEBUG){
    		Xlog.i(XLOG, "OmacpReceiverService onDestroy");
    	}
        ...
        public void handleMessage(Message msg) {
            ...
            if (intent != null) {
                ...
                if (WAP_PUSH_RECEIVED_ACTION.equals(action)) {
                	...
                	OmacpMessageNotification.blockingUpdateNewMessageIndicator(OmacpReceiverService.this, false);                    
                } 
            }
	...
	private void handleOmacpReceived(Intent intent, int error) { 
		if(DEBUG){
			Xlog.i(XLOG, "OmacpReceiverService handleOmacpReceived: " + intent.getAction());
		}        
        ...
        String service_center = bundle.getString(Telephony.WapPush.SERVICE_ADDR);
        //get sim id
        ...
        if(DEBUG){
        	Xlog.i(XLOG, "OmacpReceiverService handleOmacpReceived from : " + sender + " " + "service center is : " + service_center + " " + "simId is : " + simId);
        }
        
        if(sender == null || service_center == null){
        	Xlog.e(XLOG, "OmacpReceiverService handleOmacpReceived: sender or service center is null!");
        	return;
        }
        
        //get dec and pin code
        String mac = null;
        String sec = null;
        HashMap<String,String> contentTypeParamaters = (HashMap<String,String>)intent.getSerializableExtra("contentTypeParameters");
        if(contentTypeParamaters == null){
        	if(DEBUG){
        		Xlog.i(XLOG, "OmacpReceiverService contentTypeParamaters is null.");
        	}        	
        }else{
        	mac = contentTypeParamaters.get("MAC");
            sec = contentTypeParamaters.get("SEC");
            
            if(DEBUG){
            	Xlog.i(XLOG, "OmacpReceiverService handleOmacpReceived: MAC is : " + mac + " " + "SEC is : " + sec);
            }            
        }
        
        //sec == "" indicates Network PIN, in this case, check SIM's IMSI, if verify fails, then ignore this message
        if(sec != null && sec.equalsIgnoreCase("") && mac != null && !mac.equalsIgnoreCase("")){
        	...
        	if(correct == false){
        		Xlog.e(XLOG, "OmacpReceiverService Network PIN IMSI verify failed. Will ignore this message.");
        		return;
        	}
        }
        ...
        /*
         * Parse and store the omacp Message
         */
        ...
        if((apList == null || apList.size() == 0) && (napList == null || napList.size() == 0) 
        		&& (pxList == null || pxList.size() == 0)){
        	Xlog.e(XLOG, "OmacpReceiverService handleOmacpReceived parser error.");
        	return;
        }
        ...
        value.put("service_center", service_center);
        value.put("seen", 0);
        value.put("read", 0);
        ...
        if(uri != null){
        	...
        	//Modify the shared preference value to indicate the mms that configuration message exist
	...
	private String getSavedSummary(ArrayList<ApplicationClass> apList, ArrayList<NapdefClass> napList){
		...
		if(apList != null){
			...
			for(int i = 0; i < size; i ++){
				...
				if(application.APPID.equalsIgnoreCase("25") || application.APPID.equalsIgnoreCase("110")
						|| application.APPID.equalsIgnoreCase("143")){
					validAppId = "25";
				}else if(application.APPID.equalsIgnoreCase("w4")){//remove invalid mms setting if it only has mmsc, because it moved to apn
					if(application.CM != null){
						validAppId = "w4";
					}
				}else if(application.APPID.equalsIgnoreCase("ap0005")){
