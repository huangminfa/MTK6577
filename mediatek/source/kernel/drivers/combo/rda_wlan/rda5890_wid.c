...
send_again:
	...
	ret = rda5890_host_to_card(priv, wid_req, wid_req_len, WID_REQUEST_PACKET);
	if (ret) {
...
out:
	...
	RDA5890_DBGLAP(RDA5890_DA_WID, RDA5890_DL_DEBUG,
		"%s >>> wid: %x %x retry: %d \n", __func__, wid_req[4],wid_req[5], retry);
#ifdef WIFI_UNLOCK_SYSTEM
    rda5990_wakeUnlock();
#endif
	return ret;
...
re_send:
    ...
    count += 1;
	ret = rda5890_host_to_card(priv, wid_req, wid_req_len, WID_REQUEST_POLLING_PACKET);
	if (ret) {
		RDA5890_ERRP("host_to_card send failed, ret = %d\n", ret);
		goto out;
	}
		...
		char *wid_status, unsigned short wid_status_len)
{
	...
	if (mac_status == MAC_CONNECTED) {
		...
		rda5890_assoc_in_progress = false;
...
int rda5890_start_scan_enable_network_info(struct rda5890_private *priv)
{
...

	{
	    ...
	    {                   
			...
			if(i > 33)  //ssid len 32 + 1 end char
			    break;		
	    }
	}
...
int rda5890_start_join(struct rda5890_private *priv)
{
	...
	wid_req[6] = 1;
    wid_req[7] = (priv->imode == 0x05)?0x07:priv->imode;//for wep104 need set imode 0x07 firmware problem
    ...
    if(priv->imode == 3 || priv->imode == 5) //write wep key
    {
        for(i = 0 ; i < 4; i ++)
        {
...
//  ssid
	...
	rda5890_assoc_in_progress = true;
	ret = rda5890_wid_request(priv, wid_req, wid_req_len, wid_rsp, &wid_rsp_len);
	if (ret) {
...
int rda5890_set_core_init_polling(struct rda5890_private *priv, const unsigned int (*data)[2], unsigned char num)
{
        ...
        unsigned short wid;
        char wid_msg_id = priv->wid_msg_id++;
	    unsigned char count = 0, *p_wid_req = NULL;
        ...
        for(count = 0; count < num; count ++)
        {
            ...
            p_wid_req[2] = (char)4;
            memcpy((unsigned char *)(p_wid_req + 3), (unsigned char*)(&data[count][0]), 4);
            ...
            p_wid_req[9] = (char)4;
            memcpy((unsigned char *)(p_wid_req + 10), (unsigned char*)(&data[count][1]), 4);
            p_wid_req += 14;
        }
...
int rda5890_set_core_init(struct rda5890_private *priv, const unsigned int (*data)[2], unsigned char num)
{
        ...
        for(count = 0; count < num; count ++)
        {
            ...
            p_wid_req[2] = (char)4;
            memcpy(p_wid_req + 3, (unsigned char*)&data[count][0], 4);
            ...
            p_wid_req[9] = (char)4;
            memcpy(p_wid_req + 10, (unsigned char*)&data[count][1], 4);
            p_wid_req += 14;
        }
        ret = rda5890_wid_request(priv, wid_req, wid_req_len, wid_rsp, &wid_rsp_len);
     	if (ret) {
...
int rda5890_set_core_patch_polling(struct rda5890_private *priv, const unsigned char (*patch)[2], unsigned char num)
{
    	...
    	for(count = 0; count < num; count ++)
        {
            ...
            p_wid_req[3] = (char )patch[(unsigned char)count][0];
            ...
            p_wid_req[7] = (char)patch[(unsigned char)count][1];
            p_wid_req += 8;
        }
        ret = rda5890_wid_request_polling(priv, wid_req, wid_req_len, wid_rsp, &wid_rsp_len);
        if (ret) {
...
{
	int ret;
    static unsigned char a_rssi[3] = {0, 0, 0}; 
    static unsigned char index = 0;
    unsigned char *p_rssi;

    p_rssi = &a_rssi[index];
	ret = rda5890_generic_get_uchar(priv, WID_RSSI, p_rssi);
	if (ret) {
		goto out;
	}
    ...
    if(( ++ index) >= sizeof(a_rssi))
        index = 0;
    
    {
        unsigned char i = 0, b_rssi = 0;
        b_rssi = a_rssi[0];
        for(i = 1; i < sizeof(a_rssi); i ++)
        {
            if(a_rssi[i] > b_rssi)
                b_rssi = a_rssi[i];
        }
	...
	RDA5890_DBGLAP(RDA5890_DA_WID, RDA5890_DL_TRACE,
		"Get RSSI p_rssi %d rssi %d index %d <<< \n", *p_rssi,*(char*)rssi, index);
out:
	return ret;
}
		...
		unsigned char *ssid, unsigned char *ssid_len)
{
    ...
    RDA5890_DBGLAP(RDA5890_DA_WID, RDA5890_DL_TRACE,
	"Get SSID Done len:%d %s\n", *ssid_len, (*ssid_len > 1)? ssid:(unsigned char*)"NULL");
