...
#define RDA5990_USE_DCDC  // if use LDO mode, should close this
//#define RDA5990_FOR_CTA		// if need pass CTA authenticate, should open this define
...
static int i2c_read_4_addr_4_data(struct i2c_client* client, const u32 addr, u32* data)
{
	...
	if(!isBigEnded)
	{
		*data = (DATA[0] << 24) | (DATA[1] << 16) | (DATA[2] << 8) | (DATA[3] << 0);
	}else{
		*data = DATA[3] << 24 | DATA[2] << 16 | DATA[1] << 8 | DATA[0] << 0;
	}
...
EXPORT_SYMBOL(rda_bt_power_off);
