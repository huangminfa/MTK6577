...
public class MtvEngine implements AtvService.EventCallback{
    ...
    private static final int RECORDER_SPEC = MediaRecorder.VideoEncoder.MPEG_4_SP;//H.263 don't support 320*240,so only support MP4 for ATV now.
    ...
    private static final int mMtvMode = MTV_ATV;
    private AtvService mService;//in MtvEngine,mService is only responsible for controlling ATV chip only,not used for player and recorder.So its lifetime is from turnOn to (internal)turnOff.    
    ...
    public static MtvEngine getEngine(int mode) {
        ...
        Xlog.d(TAG, "getEngine() sEngine = "+sEngine +" mode = " + mode);
        return sEngine;        
    }
    ...
    public int getChannelNum() {
      ...
      private void onScanProgressEvent(int chnum, int ch, long chInfo, Object obj) {      
        ...
        if (mFirstChannelFound) {
            ...
            Cursor mCursor = mChannelProvider.getCursor(new String[]{ChannelProvider.CHANNEL_NUM,ChannelProvider.CHANNEL_NAME},true);
            
            Xlog.d(TAG, "onScanProgressEvent() getCount() = "+mCursor.getCount());
            ...
            if (mCacheList == null) {                        
                ...
                for(;i < size;i++) {
                    ...
                    if (ch == Integer.parseInt(item.mCh)) {                                
                        //old channel found
                        Xlog.d(TAG, "onScanProgressEvent() ch ="+ch+" is in old list "+i);
                        name = item.mName;
    ...
    public void watchOn(){
        ...
        Xlog.d(TAG, "watchOn() mState = "+mState);
        ...
        switch (mMtvMode){
            ...
            switch(mState) {
                ...
                Xlog.v(TAG, "watchOn() remove MTV_MSG_RELEASE_ENGINE ");
    ...
    public void turnOn(){
        ...
        Xlog.d(TAG, "turnOn() mState = "+mState);
        if (mMtvMode == MTV_ATV){            
                switch(mState) {
                    ...
                    case MTVENGINE_STATE_INITIALIZING_OPENED:                        
                        break;
                    case MTVENGINE_STATE_SCANNING://activity may be recreated due to configuration change during scanning and call turnOn again.
        ...
        Xlog.v(TAG, "turnOn() remove MTV_MSG_RELEASE_ENGINE ");
        mMainHandler.removeMessages(MTV_MSG_RELEASE_ENGINE);
    }
    ...
    public final void setEventCallback(EventCallback cb) {
        Xlog.d(TAG, "setEventCallback() cb = " + cb);
    ...
    public void callOnEvent (int what, int arg1, int arg2, Object obj) {     
        ...
        Xlog.d(TAG, "callOnEvent()");
        mMainHandler.sendMessage(mMainHandler.obtainMessage(what, arg1,arg2, obj));        
    }
    
    public void setNeedSetTable(boolean set) {
        
        Xlog.d(TAG, "setNeedSetTable() set = "+set);
        mNeedSetTable = set;
    }  

    public void setFirstChannel(int ch,ChannelManager.ChannelHolder holder)  throws ChannelManager.ChannelTableEmpty{        
        Xlog.d(TAG, "setFirstChannel,ch = "+ch);
        if (mNeedSetTable) {                
    ...
    public void changeChannel(int ch){        
        Xlog.d(TAG, "changeChannel,ch = "+ch);
        if (mState == MTVENGINE_STATE_OPENED) {
            mChannelManager.changeChannel(ch);
        }
    }
    ...
    public void startRecord(String path,long maxFileSize,Surface s) throws Exception{
        ...
        Xlog.d(TAG, "startRecord() path = "+path+";maxFileSize = "+maxFileSize);
        if (mMtvMode == MTV_ATV){
            if (mRecorder == null) {
    ...
    //we need to do synchronzation in MtvEngine since AtvPlayer do not have synchronization on its own.
    ...
    public void watchOff(){    
        ...
        Xlog.d(TAG, "watchOff() mState = "+mState);
        switch(mState) {
            case MTVENGINE_STATE_OPENED:
                releasePlayer();
                mState = MTVENGINE_STATE_INITIALIZED;
    ...
    private void releaseVideo(){
        Xlog.d(TAG, "releaseVideo()");
        //release recorder first and release player later,not quite sure whether the reverse order will cause lock/unlock failure.
        releaseRecorder();        
        releasePlayer();        
    }      
    ...
    public void configVideo(byte item,int val){
        Xlog.d(TAG, "configVideo() item = "+item+";val = "+val);
    ...
    public boolean channelScan(final int area){
        ...
        Xlog.d(TAG, "channelScan() area = "+area);
        if (mState == MTVENGINE_STATE_INITIALIZED) {
                ...
                Xlog.d(TAG, "channelScan() error mState = "+mState);            
                return false;
        }
    } 

    public void stopChannelScan(){
        Xlog.d(TAG, "stopChannelScan()");        
        if (mMtvMode == MTV_ATV) {
            if (mState != MTVENGINE_STATE_SCANNING) {
                Xlog.w(TAG, "stopChannelScan() error mState = "+mState);    
                return;
            }
            
            if (mScanThread != null) {//means that it is force closed last time but has not closed yet,we need to wait to avoid corrupt the state of driver.
                try {
                    mScanThread.join();
    ...
    public void setAudioFormat(int val){
        Xlog.d(TAG, "setAudioFormat() val = "+val);
        if (mState == MTVENGINE_STATE_OPENED) {
    ...
    private void onDriverShutdown(){
        Xlog.d(TAG, "onDriverShutdown() mState = "+mState);
        ...
        switch(mState) {    
            ...
            case MTVENGINE_STATE_SCANNING:    
                //fall through
            case MTVENGINE_STATE_INITIALIZED:
                //video may be stopped but not released yet and we need to release it immediately to let user to enter camera related functions normally.
                break;                
            case MTVENGINE_STATE_INITIALIZING:    
    ...
    public void turnOff(boolean force){
        Xlog.d(TAG, "turnOff() mState = "+mState);
        ...
        switch(mState) {            
            ...
            case MTVENGINE_STATE_IDLE:
            case MTVENGINE_STATE_SCANNING:
                //at present we assume turnOff(true) is caused by pressing power key,and we don't want to abort scanning even in such case.    
            ...
            case MTVENGINE_STATE_INITIALIZED:
                if (force) {
                    ...
                    //video may be stopped but not released yet and we need to release it immediately to let user to enter camera related functions normally.
                    releasePlayer();                    
                    Xlog.d(TAG, "Start timer to delay shutdown()");        
    ...
    private void internalTurnOff(){
        Xlog.d(TAG, "internalTurnOff() mState = "+mState);
