...
public class ImageWallpaper extends WallpaperService {
    ...
    static boolean FIXED_SIZED_SURFACE = true;
    ...
    public void onCreate() {
        ...
        FIXED_SIZED_SURFACE = resources.getBoolean(R.bool.config_fix_sized_surface);
    ...
    class DrawableEngine extends Engine {
        ...
        public void onSurfaceChanged(SurfaceHolder holder, int format, int width, int height) {
            ...
            synchronized (mLock) {
                // Reset this to fetch the latest wallpaper dimenssions
                mBackgroundWidth = mBackgroundHeight = -1;
                mRedrawNeeded = true;
                drawFrameLocked();
            }
		...
		public void onSurfaceRedrawNeeded(SurfaceHolder holder) {
			...
			if (DEBUG) {
                Xlog.d(TAG, "onSurfaceRedrawNeeded(" + holder + ")");
            }
			synchronized (mLock) {
                mRedrawNeeded = true;
                drawFrameLocked();
		...
		void drawFrameLocked() {
            ...
            if (DEBUG) {
                Log.d(TAG, "surface frame=" + frame + ", availw=" + availw + ", availh=" 
                    + availh + ", xPixels=" + xPixels + ", yPixels=" + yPixels);
            }
            ...
            Xlog.v(TAG,"mIsHwAccelerated = "+mIsHwAccelerated);
            if (mIsHwAccelerated) {
		...
		//will be called in updateWallpaperLocked() when background's width and height is smaller than SurfaceFrame's.
		private void scaleBackgroundIfNeeded(){
			if (mBackground == null) return; 
			int backgroundWidthNow = mBackground.getWidth();
			int backgroundHeightNow = mBackground.getHeight();
			SurfaceHolder sh = getSurfaceHolder();
            final Rect frame = sh.getSurfaceFrame();
			final int backgroundWidthDesired = frame.width();
			final int backgroundHeightDesired = frame.height();
			
			if(backgroundWidthNow < backgroundWidthDesired || backgroundHeightNow <  backgroundHeightDesired){
				mBackground = generateBitmap(mBackground,backgroundWidthDesired,backgroundHeightDesired);
			}
		}
		
		// the method is copied from WallpaperManager.java
	    private Bitmap generateBitmap(Bitmap bm, int width, int height) {
	    	Xlog.v(TAG,"generateBitmap, bm = "+bm+", width = "+width+", height = "+height);
	        if (bm == null) {
	            return null;
	        }
        ...
        void updateWallpaperLocked() {
            ...
            Xlog.v(TAG,"updateWallpaperLocked, mBackgroundWidth = "+mBackgroundWidth+", mBackgroundHeight"+mBackgroundHeight);
        }
            ...
            if (c != null) {
                try {
                    if (DEBUG) {
                        ...
                        Xlog.d(TAG, "Redrawing:w ="+w+",h="+h+",sh="+sh);
                    }
