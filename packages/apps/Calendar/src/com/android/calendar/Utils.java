...
public class Utils {
    ...
    ///M: the key in unread_support_shortcuts.xml defined in Launcher.
    public static final String CALENDAR_MTK_UNREAD_KEY = "com_android_calendar_mtk_unread";
    
    ///M:SECONDE_IN_MILLIS
    private static final long SECONDE_IN_MILLIS = 1000;
    
    ///M:Indicate wether is other app request calendar @{
    ...
    public static final long timeFromIntentInMillis(Intent intent) {
        ...
        } else if(millis < 1*SECONDE_IN_MILLIS){
        /**M: to fix one issue:the view can not go to the 1970-01-01 when the intent millis equals 0
         * or millis < 1second. So, here, if 0<=millis<1second, set millis to 1s.@{*/
            millis = 1*SECONDE_IN_MILLIS;//+1second.
        }
        /**@}*/
   ...
   ///M:if time out of range,return max or min julian day@{
    public static int getValidJuLianDay(Time t){
        ...
        juLianDay = juLianDay < FIRST_CALENDAR_DAY ? FIRST_CALENDAR_DAY : juLianDay;
        juLianDay = juLianDay > LAST_CALENDAR_DAY ? LAST_CALENDAR_DAY : juLianDay;
        return juLianDay;
    }
    
    /**
     * get the julian day the in the right range.
     * @param julianDay 
     * @return return FIRST_CALENDAR_DAY if the julianDay < FIRST_CALENDAR_DAY
     * return LAST_CALENDAR_DAY if the julianDay > LAST_CALENDAR_DAY,else return it self
     */
    public static int getValidJuLianDay(int julianDay){
        julianDay = julianDay < FIRST_CALENDAR_DAY ? FIRST_CALENDAR_DAY : julianDay;
        julianDay = julianDay > LAST_CALENDAR_DAY ? LAST_CALENDAR_DAY : julianDay;
        return julianDay;
    }
    ...
    public static void updateEventReminder(Context context) {
        ...
        Intent intent = new Intent(AlertService.UPDATE_EVENT_REMINDER_ACTION);
        context.sendBroadcast(intent);
    }

    ///M:Add for ThemeManager
    /**
     * get the color managered by ThemeManager
     * @param context
     * @param defaultColor if failed,return this color
     * @return
     */
    public static int getThemeMainColor(Context context, int defaultColor) {
        if (Features.isThemeManagerEnabled()) {
            Resources res = context.getResources();
