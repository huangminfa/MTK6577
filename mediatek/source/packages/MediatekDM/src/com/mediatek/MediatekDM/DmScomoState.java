...
public class DmScomoState implements Serializable {
    ...
    // used by scomo listener to decide
    // whether to interact with
    // user. mVerbose==false will hide any
    // UI. mVerbose will be set in
    // MmiConfirmation (ALERT 1101), and
    // be reset after DM session
    ...
    private void readObject(ObjectInputStream in) throws IOException,
        ...
        if (!dpName.equals("")) {
            try {
                this.currentDp = MdmScomo.getInstance(
        ...
        } else if (this.mState == INSTALLING || this.mState == UPDATING
                   || this.mState == CONFIRM_INSTALL
                   || this.mState == CONFIRM_UPDATE) {
            this.mState = CONFIRM_INSTALL;
            // reload pkgInfo from archivePath;
            this.setArchivePath(this.archiveFilePath);
        } else if (mState==DOWNLOADING
                   ||mState==DOWNLOADING_STARTED
                   ||mState==RESUMED) {
            this.mState=PAUSED;
        } else {
            // reset state
    ...
    public void setArchivePath(String path) {
        ...
        this.pkgInfo = DmScomoPackageManager.getInstance()
                       .getMinimalPackageInfo(path);
    }

    static public DmScomoState load(Context context) {
        Object obj = null;
        try {
        ...
        if (obj != null) {
            ...
            Log.i(TAG.Scomo, "DmScomoState: state loaded: state= " + ret.mState);
            return ret;
        }
        return new DmScomoState();
    }

    static public void store(Context context, DmScomoState state) {
    ...
    public String getName() {
        ...
        if (pkgInfo != null && pkgInfo.label != null) {
            ret = pkgInfo.label;
                   ...
                   && currentDd.getField(DownloadDescriptor.Field.NAME) != null) {
