...
bool OpenGLRenderer::drawDisplayList(DisplayList* displayList, uint32_t width, uint32_t height,
        Rect& dirty, uint32_t &err, uint32_t level) {
    ...
    if (displayList && displayList->isRenderable()) {
        ...
        if (!mCaches.isDisplayListActive(displayList))
        {
            LOGE("ERROR: The displaylist %p (level=%d) has been destroyed!!", displayList, level);
            if(!mCaches.dumpActiveDisplayLists(displayList))
            {
                err = (unsigned int)displayList;
                return true;
            }
            LOGW("WARNING: Found the displaylist %p in lists. Keep renderering.", displayList);
        }
#endif

        bool bRet = displayList->replay(*this, dirty, err, level);
#if defined(MTK_DEBUG_RENDERER)
        dumpDisplayList(mWidth, mHeight, level);
#endif
	return bRet;
    }
...
void OpenGLRenderer::drawBitmapMesh(SkBitmap* bitmap, int meshWidth, int meshHeight,
    ...
    if (colors)
	LOGW("drawBitmapMesh: colors array is ignored");
...
void OpenGLRenderer::drawText(const char* text, const float* xOffset, const float* yOffset, int bytesCount, int count, float x, float y, SkPaint* paint) {
    if (text == NULL || count == 0) {
        return;
    }
    if (mSnapshot->isIgnored()) return;
    ...
    int positionSlot = mCaches.currentProgram->position;
    fontRenderer.setAttributeBindingSlots(positionSlot, mTexCoordsSlot);
    if (fontRenderer.renderText(paint, clip, text, xOffset, yOffset, 0, bytesCount, count, x, y,
            hasActiveLayer ? &bounds : NULL)) {
#if RENDER_LAYERS_AS_REGIONS
        if (hasActiveLayer) {
            if (!pureTranslate) {
