...
/* [mtk03712]
 * to determine if the boot animation shows or not
 */
...
void SurfaceFlinger::init()
{
    ...
    // [mtk04189] case 4: Draw and swap if debug region is turned on
    ...
    // [mtk04189] mBusySwap is a switch that can be turned on/off at run-time
    //------------------------------------------------------------------------------------------
    property_get("debug.sf.busyswap", value, "0");
    mBusySwap = atoi(value);
    //------------------------------------------------------------------------------------------

    // [mtk03712] debug log properties switches
    // mCompose for internal screen composition update
    //------------------------------------------------------------------------------------------
    property_get("debug.sf.log_repaint", value, "0");
    mLogRepaint = atoi(value);
    //------------------------------------------------------------------------------------------

    // [mtk03712] switch to debug S3D composing
    //------------------------------------------------------------------------------------------
    property_get("debug.sf.s3d", value, "0");
    mDebugS3D = atoi(value);
    //------------------------------------------------------------------------------------------
    ...
    LOGI_IF(mBusySwap,          "    - busyswap enabled");
    LOGI_IF(mLogRepaint,        "    - repaint debug enabled");
    LOGI_IF(mDebugS3D,          "    - S3D composing debug enabled");

    // [mtk04189] project configuration
    //------------------------------------------------------------------------------------------
#if MTK_TVOUT_SUPPORT
    sMtkConfig.mtk_tvout_support = true;
    LOGI("MTK_TVOUT_SUPPORT enabled");
#endif

#if MTK_HDMI_SUPPORT
    sMtkConfig.mtk_hdmi_support = true;
    LOGI("MTK_HDMI_SUPPORT enabled");
#endif

#if MTK_S3D_SUPPORT
    sMtkConfig.mtk_s3d_support = true;
    LOGI("MTK_S3D_SUPPORT enabled");
#endif
    //------------------------------------------------------------------------------------------
}
...
void SurfaceFlinger::bootFinished()
{
    ...
    LOG(LOG_INFO,"boot","BOOTPROF:BootAnimation:End:%ld", long(ns2ms(systemTime())));
    bootProf(0);
}
...
status_t SurfaceFlinger::readyToRun()
{
    ...
    // [mtk03712] try to change SurfaceFlinger priority to RR 84
    //----------------------------------------------------------------------------------------------
    sched_param sched_p;
    ...
    // [mtk03712]
    // start boot animation if needed
    ...
    if (true == mBootAnimationEnabled) {
        ...
        property_set("ctl.start", "bootanim");
        LOG(LOG_INFO,"boot","BOOTPROF:BootAnimation:Start:%ld", long(ns2ms(systemTime())));
        bootProf(1);
    } else {
...
//        if (mDelayTime) usleep(1000*mDelayTime);
        // for watch dog timing
#if ENABLE_SF_WATCHDOG        
        ...
        if (mDelayTime)
        {
            XLOGI("Surface Flinger slow motion, timer: %d ms", mDelayTime);
...
void SurfaceFlinger::handleTransactionLocked(uint32_t transactionFlags)
{
    ...
    if (transactionFlags & eTransactionNeeded) {
        ...
        if (mNeedUpdateFreezeDisplayState) {
            ...
            //XLOGI("[%s] freezeDisplay changed %d -> %d", __func__, mDrawingState.freezeDisplay, mCurrentState.freezeDisplay);
        ...
        if (mLayersRemoved) {
            mLayersRemoved = false;
            // [mtk04189] case 1: Draw and swap if layer removed
...
void SurfaceFlinger::handleRepaint()
{
    ...
    // [mtk03712] make SF context current, default dpy is 0 only
	//------------------------------------------------------------
    ...
    // [mtk04189] do not render transparent, wormhole, and dirty regions if unnecessary
    //------------------------------------------------------------------------------------------
    setupHardwareComposer(mDirtyRegion);
    if (true == mLayersSwapRequired) {
...
void SurfaceFlinger::setupHardwareComposer(Region& dirtyInOut)
{
    ...
    // [mtk03712] debug log
    //------------------------------------------------------------------------------------------
    if (UNLIKELY(NO_ERROR != err)) {
        LOGE("@ HWComposer::prepare failed (%s)", strerror(-err));
    } else {
        if (true == mLogRepaint) {
            XLOGD("@ HWComposer::prepare success (HWC_FRAMEBUFFER:%d, HWC_OVERLAY:%d, HWC_OVERLAY_EX:%d)",
                hwc.getLayerCount(HWC_FRAMEBUFFER), hwc.getLayerCount(HWC_OVERLAY), hwc.getLayerCount(HWC_OVERLAY_EX));
        }
    }
...
void SurfaceFlinger::composeSurfaces(const Region& dirty)
{
    ...
    if (UNLIKELY(false == mWormholeRegion.isEmpty())) {
...
void SurfaceFlinger::debugShowFPS() const
{
    ...
    nsecs_t duration = now - lastFpsTime;
    if (duration > ms2ns(1000)) {
        FPS = frames * ms2ns(1000) / (float)duration;

        XLOGI("[SurfaceFlinger] frames:%d, duration:%f, fps:%f",
            frames, 0.001 * ns2ms(duration), FPS);
...
void SurfaceFlinger::screenAcquired(int dpy)
{
    ...
    // [mtk04189] case 6: Draw and swap if screen is about to return
    mLayersSwapRequired = true;
    signalEvent();

    // [mtk03712]
    // pass IPO a signal for the black screen sync issue
    usleep(16667);
...
void SurfaceFlinger::repaintEverything() {
    ...
    // [mtk04189] case 7: Draw and swap if region is invalidated
    mLayersSwapRequired = true;
    signalEvent();
}
    ...
    if (status == GL_FRAMEBUFFER_COMPLETE_OES) {
        ...
        if (glGetError() != GL_NO_ERROR) {
            ...
            if (ptr) {
                ...
                LOGE("[%s] dump %s done", __func__, fname);
                cnt += 1;
                //----------------------------------------------------------------------------------//*/
