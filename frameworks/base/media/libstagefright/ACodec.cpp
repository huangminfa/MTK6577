...
#ifndef ANDROID_DEFAULT_CODE
    bool mIdleCompleted;
    int64_t mWaitingIdleCompleteTime;
    enum {
        kWhatCheckIdleCompleted  = 'idle'
    };
    void checkIdleCompleted(int64_t IntervalUs);
    void notifyIdleCompleted(bool bSuccess);
#endif
...
#ifndef ANDROID_DEFAULT_CODE
void ACodec::initiateSetup(const sp<AMessage> &msg, bool bAutoRun) {
    mAutoRun = bAutoRun;
    initiateSetup(msg);
}
#endif
...
status_t ACodec::allocateOutputBuffersFromNativeWindow() {
    ...
    for (OMX_U32 i = 0; i < def.nBufferCountActual; i++) {
        ...
        LOGV("registered GraphicBuffer ptr(0x%08X), width(%d), height(%d), stride(%d), format(0x%08X), ID(0x%08X)",
...
void ACodec::sendFormatChange() {
    ...
    LOGD("[%s] send format change", mComponentName.c_str());
...
void ACodec::BaseState::postFillThisBuffer(BufferInfo *info) {
    ...
    if (mCodec->mLeftOverBuffer != NULL) {
        ...
        reply->setObject("buffer", mCodec->mLeftOverBuffer);
...
void ACodec::BaseState::onInputBufferFilled(const sp<AMessage> &msg) {
    ...
    if (!msg->findObject("buffer", &obj)) {
        ...
        LOGD("[%s] saw error %d instead of an input buffer %p",
             mCodec->mComponentName.c_str(), err, bufferID);
        ...
        case RESUBMIT_BUFFERS:
        {
            if (buffer != NULL) {
                ...
                if (buffer != info->mData) {
                    ...
                    if (buffer->size() > capacity) {
                        if (mCodec->mSupportsPartialFrames) {
                            sp<ABuffer> leftBuffer = new ABuffer(
                            ...
                            LOGI("split big input buffer %d to %d + %d",
                                    buffer->size(), capacity, leftBuffer->size());
                            ...
                            LOGE("Codec's input buffers are too small to accomodate "
                ...
                if (err != ERROR_END_OF_STREAM) {
                    LOGD("[%s] Signalling EOS on the input port "
                         "due to error %d",
...
bool ACodec::BaseState::onOMXFillBufferDone(
    ...
    //TODO: audio or video ?
    if (rangeLength != 0)
        dumpProfile("decoded", mCodec->mComponentName.c_str(), timeUs);
#endif
...
void ACodec::BaseState::onOutputBufferDrained(const sp<AMessage> &msg) {
    ...
    if (mCodec->mNativeWindow != NULL
       ...
       if ((err = mCodec->mNativeWindow->queueBuffer(
            ...
            mCodec->signalError();
            LOGE("error when queueBuffer (error 0x%08x)", err);
            info->mStatus = BufferInfo::OWNED_BY_US;
        }
    } else {
        info->mStatus = BufferInfo::OWNED_BY_US;
...
#if DUMP_PROFILE
    if (mCodec->mNativeWindow != NULL && msg->findInt32("render", &render)) {
        int64_t mediaTimeUs;
        CHECK(info->mData->meta()->findInt64("timeUs", &mediaTimeUs));
        dumpProfile("render", mCodec->mComponentName.c_str(), mediaTimeUs);
    }
#endif
...
#ifndef ANDROID_DEFAULT_CODE
bool ACodec::BaseState::isAudio(const char* strName) {
    const char strAudio[] = "audio";
    //find strName has substring "audio" (case insensitive)
    for (; *strName; strName++) {
        if (tolower(*strName) == *strAudio) {
            const char *s, *d;
            for (s = strName, d = strAudio; (*s!='\0') && (*d!='\0'); s++, d++) {
                if (tolower(*s) != *d) {
                    break;
                }
            }
...
void ACodec::BaseState::dumpProfile(const char* tag, const char* strName, int64_t timeUs) {
    LOGD("[dump] %s %s %.2f", isAudio(strName) ? "audio" : "video", tag, timeUs / 1E6);
...
void ACodec::UninitializedState::stateEntered() {
    LOGD("[%s] Now Uninitialize", mCodec->mComponentName.c_str());
}
...
ACodec::LoadedToIdleState::LoadedToIdleState(ACodec *codec)
    : BaseState(codec) {
#ifndef ANDROID_DEFAULT_CODE
        mIdleCompleted = false;
        int64_t mWaitingIdleCompleteTime = 0;
#endif
}

void ACodec::LoadedToIdleState::stateEntered() {
    LOGD("[%s] Now Loaded->Idle", mCodec->mComponentName.c_str());
    ...
    if ((err = allocateBuffers()) != OK) {
        ...
        if (!mCodec->mAutoRun)
            checkIdleCompleted(100*1000ll); 
    }
#endif
}

#ifndef ANDROID_DEFAULT_CODE
void ACodec::LoadedToIdleState::checkIdleCompleted(int64_t IntervalUs) {
        (new AMessage(kWhatCheckIdleCompleted, mCodec->id()))->post(IntervalUs);
}

void ACodec::LoadedToIdleState::notifyIdleCompleted(bool bSuccess) {
    sp<AMessage> notify = mCodec->mNotify->dup();
    notify->setInt32("what", ACodec::kWhatPrepareCompleted);
    notify->setInt32("success", (int32_t)bSuccess);
    notify->post();
}
#endif
...
bool ACodec::LoadedToIdleState::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
...
#ifndef ANDROID_DEFAULT_CODE
           if (!mIdleCompleted) {
               sp<AMessage> msg2 = msg->dup();
               msg2->post(100000ll);
               LOGD("waiting to Idle to shutdown");
               return true;
           }
           if (!mCodec->mAutoRun) {
                LOGD("[%s] Shutting down in state of Loaded->Idle",
                    mCodec->mComponentName.c_str());
                ...
                LOGD("[%s]postpone kWhatShutdown", mCodec->mComponentName.c_str());
            } 
#endif
        ...
        case kWhatResume:
        {
            ...
            LOGD("@debug: send command to OMX: Executing");
            mCodec->changeState(mCodec->mIdleToExecutingState);
            return true;
        }
        case kWhatCheckIdleCompleted:
        {
            if (!mIdleCompleted) {
                if (mWaitingIdleCompleteTime > 1*1000*1000ll) {
                    LOGE("send OMX Idle command timeout");
                    notifyIdleCompleted(false);
                } else {
                    mWaitingIdleCompleteTime += 100*1000ll;
                    checkIdleCompleted(100*1000ll);
                }
            } else {
                LOGD("idling complete");
                notifyIdleCompleted(true);
            }
            return true;
        }
#endif
...
void ACodec::IdleToExecutingState::stateEntered() {
    LOGD("[%s] Now Idle->Executing", mCodec->mComponentName.c_str());
}
...
void ACodec::ExecutingState::stateEntered() {
    LOGD("[%s] Now Executing", mCodec->mComponentName.c_str());
...
bool ACodec::ExecutingState::onMessageReceived(const sp<AMessage> &msg) {
    ...
    switch (msg->what()) {
        ...
        case kWhatFlush:
        {
            LOGD("[%s] ExecutingState flushing now "
...
#ifndef ANDROID_DEFAULT_CODE
            if (mCodec->mLeftOverBuffer != NULL) {
                LOGI("clear mLeftOverBuffer %x", mCodec->mLeftOverBuffer.get());
                mCodec->mLeftOverBuffer = NULL;
            }
#endif
            CHECK_EQ(mCodec->mOMX->sendCommand(
...
void ACodec::OutputPortSettingsChangedState::stateEntered() {
    LOGD("[%s] Now handling output port settings change",
         mCodec->mComponentName.c_str());
}
...
void ACodec::ExecutingToIdleState::stateEntered() {
    LOGD("[%s] Now Executing->Idle", mCodec->mComponentName.c_str());
...
void ACodec::IdleToLoadedState::stateEntered() {
    LOGD("[%s] Now Idle->Loaded", mCodec->mComponentName.c_str());
}
...
bool ACodec::IdleToLoadedState::onOMXEvent(
    ...
    switch (event) {
        case OMX_EventCmdComplete:
        {
            ...
            LOGD("[%s] Now Loaded", mCodec->mComponentName.c_str());

            CHECK_EQ(mCodec->mOMX->freeNode(mCodec->mNode), (status_t)OK);

            LOGD("[%s] --> Unitialize", mCodec->mComponentName.c_str());
...
void ACodec::FlushingState::stateEntered() {
    LOGD("[%s] Now Flushing", mCodec->mComponentName.c_str());
