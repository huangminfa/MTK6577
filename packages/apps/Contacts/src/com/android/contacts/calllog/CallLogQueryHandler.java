...
/*package*/ public class CallLogQueryHandler extends AsyncQueryHandler {
    ...
    protected synchronized void onQueryComplete(int token, Object cookie, Cursor cursor) {
        log("onQueryComplete(), token = " + token + " cookie " + cookie + " cursor " + cursor);
        if (token == QUERY_NEW_CALLS_TOKEN) {
            ...
            return;
        } else if (token == QUERY_SEARCH_TOKEN) {
			updateAdapterData(cursor);
          //The following lines are provided and maintained by Mediatek Inc.
        } else if (token == QUERY_ALL_CALLS_TOKEN) {
            updateAdapterData(cursor);
        } else if (QUERY_ALL_CALLS_JOIN_DATA_VIEW_TOKEN == token) {
            updateAdapterData(cursor);
    ...
    public void fetchAllCalls(int simFilter, int typeFilter) {
        ...
        cancelFetch();
        invalidate();
        fetchCalls(QUERY_ALL_CALLS_TOKEN, simFilter, typeFilter);
    }
    ...
    public void fetchCallsJionDataView(int simFilter, int typeFilter) {
        ...
        cancelFetch();
        invalidate();
        fetchCalls(QUERY_ALL_CALLS_JOIN_DATA_VIEW_TOKEN, simFilter, typeFilter);
    }

    private void fetchCalls(int token, int simFilter, int typeFilter) {
        log("fetchCalls(), token = " + token + " simFilter = " + simFilter + " typeFilter "
                + typeFilter);
        String selection = getSelection(simFilter, typeFilter);
        log("startQuery  selection===" + selection);
        ...
        CallLogDateFormatHelper.refreshData();
        startQuery(token, null, queryUri, queryProjection, selection, null,
                Calls.DEFAULT_SORT_ORDER);
    }
    ...
    private String getSelection(int simFilter, int typeFilter) {
        ...
        if (typeFilter != Constants.FILTER_TYPE_ALL) {
            ...
            if ("OP01".equals(OperatorUtils.getOptrProperties())) {
                ...
                builder.append(Calls.TYPE + "!=" + Calls.AUTOREJECTED_TYPE);
            }
        }
