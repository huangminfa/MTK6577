        ...
        implements View.OnCreateContextMenuListener, MusicUtils.Defs, ServiceConnection
{
    ...
    SearchView.OnQueryTextListener mQueryTextListener = new SearchView.OnQueryTextListener() {
        public boolean onQueryTextSubmit(String query) {
            ...
            intent.setClass(ArtistAlbumBrowserActivity.this, QueryBrowserActivity.class);
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (mArtistCursor == null) {
            MusicLogUtils.v(TAG, "Artist cursor is null, need not show option menu: mArtistCursor = " + mArtistCursor);
            return false;
        }
        return true;
    }
    ...
    static class ArtistAlbumListAdapter extends SimpleCursorTreeAdapter implements SectionIndexer {
        ...
        private int getGroupPositon(int position){
            ...
            //MusicLogUtils.v(TAG, "position is " + position);
            //MusicLogUtils.v(TAG, "groupCount is " + groupCount);
            ExpandableListView listview = mActivity.getExpandableListView();
            if (listview == null) {
                MusicLogUtils.v(TAG, "ExpandableListView is null");
                return 0;
            }
            ...
            for(int i = 0; i < groupCount; i++){
                if (listview.isGroupExpanded(i)) {
                    ...
                    //MusicLogUtils.v(TAG, "Group " + i + " childern is " + temp);
                    expandingChildCount += temp;
                }
                if (position - expandingChildCount <= i) {
                    //MusicLogUtils.v(TAG, "expandingChildCount is " + expandingChildCount + "; i is " + i);
                    return i;
                }
            }
            return groupCount - 1;
