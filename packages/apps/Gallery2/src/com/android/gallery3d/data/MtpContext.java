...
public class MtpContext implements MtpClient.Listener {
    ...
    public void pause() {
        // move below operation out of UI thread to avoid ANR.
        // better not to remove the synchronize lock in addListener,
        // since it might cause the mListeners to change during using it
        // in MtpClient.mUsbReceiver.onReceive
        new Thread() {
            public void run() {
                mClient.removeListener(MtpContext.this);
            }
        }.run();
    }
    ...
    public void resume() {
        ...
        new Thread() {
            public void run() {
                mClient.addListener(MtpContext.this);
                notifyDirty();
            }
        }.run();
    }
