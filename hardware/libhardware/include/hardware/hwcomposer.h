...
typedef struct hwc_layer {
    ...
    // [mtk04189] replaced buffer_handle_t with GraphicBuffer, and buffer_handle_t is deprecated now
    //--------------------------------------------------------------------------
    ...
    // [mtk03712] for mtk hwcomposer use
    //----------------------------------------------------------------------------------------------
    int32_t connectedApi;   // layer connected native window api type
    int32_t identity;       // SF layer identity
    Mutex *mutex;           // for sync with other threads

    uint32_t flagsEx;               // SF layer flagsEx

    uint32_t s3dBufferFlags;        // buffer flags for S3D
    int32_t  s3dManualOffset;       // for S3D manual convergence

    uint32_t bufferTransform;       // for cases that need get correct buffer look
    //----------------------------------------------------------------------------------------------
