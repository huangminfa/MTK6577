...
public class DmController {
    ...
    public DmController(Context context) {
        ...
        if (Options.UseSmsRegister) {
            int simSlot = DmCommomFun.getSimRegisterID(context);
            if (simSlot != -1) {
                ...
                dr.registerCCNodeIoHandler(DmConst.pathname.DmTreeFile);
            } else {
    ...
    public void createEngine(Context context) {
        try {
            ...
            mEngine = new MdmEngine(mContext, new DmMmiFactory(),
            ...
            mLogger.logMsg(Log.ERROR, "MdmEngine ctor exception "
                           + e.getError().name());
            e.printStackTrace();
        }
    }
    ...
    public void registerSwVNodeIOHandler() {
        try {
            mDmTree.registerNodeIoHandler(DmConst.nodeUri.DevDetailSwVUri,
                                          mSwVIOHandler);
    ...
    public void registerDevIdNodeIOHandler() {
        try {
            mDmTree.registerNodeIoHandler(DmConst.nodeUri.DevInfoDevId,
                                          mDevIdIOHandler);
    ...
    public void startEngine() {
        try {
            ...
            mLogger.logMsg(Log.ERROR, "MdmEngine start exception "
                           + e);
        } catch (Exception e) {
    ...
    public void executeFwUpdate() {
        ...
        //Set a flag file to indicate
        Log.d(TAG.Controller, "touching flag file: "+DmConst.pathname.FOTAExecFlagFile);
        File ff = new File(DmConst.pathname.FOTAExecFlagFile);
        try {
            ff.createNewFile();
        } catch (IOException e1) {
            e1.printStackTrace();
            Log.e(TAG.Controller, "touching flag file failed: "+e1);
        }
        ...
        try {
            ...
            Log.e(TAG.Controller, "Exception accur when reboot for upgrade.");
            e.printStackTrace();
        }
    }
    ...
    public void triggerNiaDmSession(byte[] message) {
        ...
        try {
            mEngine.triggerNIADmSession(message, mDmNiaMsgHandler,
                                        mDmNiaMsgHandler);
        } catch (MdmException e) {
     ...
     * @param byte[] message - message body of wap push
     */
    public void triggerBootstrapSession(BootProfile profile,
                                        CpSecurity security, String mac, byte[] message) {
        try {
            mEngine.triggerBootstrapSession(null, profile, security, mac,
                                            message, mDmBootstrapHandler, mDmBootstrapHandler);
        } catch (MdmException e) {
            ...
            if (agent != null) {
                ...
                if (agent.isLockFlagSet() == true) {
                    ...
                    if (lockStatus == DmConst.LawmoStatus.FULLY_LOCK
                        ...
                        Log.i(TAG.Controller, "Lawmo status in tree is "
                              + treeLawmoStatus);
    ...
    public boolean triggerLawmoReportSession(LawmoOperationType type,
            LawmoResultCode code) {
        Log.w(TAG.Controller, "triggerLawmoReportSession begin");
        boolean ret = true;
        if (mLawmo != null) {
            try {
                Log.w(TAG.Controller,
                      "triggerLawmoReportSession trigger report session type is "
                      + type);
                ...
                Log.e(TAG.Controller,
                      "triggerLawmoReportSession" + e.getMessage());
            }
        } else {
            ret = false;
    ...
    public static DmAction getDmAction() {
        ...
        Log.w(TAG.Controller, "getDmAction left, fumo action is "
              + action.fumoAction + ",lawmo action is " + action.lawmoAction
              + ",scomo action is " + action.scomoAction);
        return action;
    }
    ...
    private boolean syncDmServerAddr() {
        ...
        try {
            ...
            if (opName.equals("cmcc")) {
                nodeUri = "./SyncML/DMAcc/OMSAcc/Addr";
            } else if (opName.equals("cu")) {
                nodeUri = "./SyncML/DMAcc/CUDMAcc/Addr";
            } else {
