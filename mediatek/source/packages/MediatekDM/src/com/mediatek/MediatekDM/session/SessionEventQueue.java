...
public class SessionEventQueue {
    ...
    public DLAbortState analyzeAbortState(long nowMillis) {
        ...
        synchronized (mLock) {
            ...
            if (mLastSessionEvent.type == EVENT_DL_SESSION_ABORTED) {
                ...
                if (errCode == MdmError.COMMS_FATAL.val
                        || errCode == MdmError.COMMS_NON_FATAL.val
        ...
        Log.d(TAG.Debug, "[DLAbortState]analyze result=>" + state.value + ","
              + state.leftTime);
        return state;
    }
    ...
    private boolean isConnEvent(int eventType) {
        return (eventType == EVENT_CONN_UNKNOWN
                || eventType == EVENT_CONN_CONNECTED
    ...
    public static class Event {
        ...
        public String toString() {
            return String.format("[session event]type=%d, extra=%s, time=%d",
                                 type, extra, timestamp);
        }
    }
