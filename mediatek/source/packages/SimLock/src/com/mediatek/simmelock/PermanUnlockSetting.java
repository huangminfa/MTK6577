	...
	protected Dialog onCreateDialog(int id) {
    	...
    	{			
			public void onClick(DialogInterface arg0, int arg1) 
			{
				...
				//PermanUnlock a lock
		        if(false == FeatureOption.MTK_GEMINI_SUPPORT)//Single SIM: directly permanently remove a lock
		        { 
		            //Single Card:
		        	Phone phone = PhoneFactory.getDefaultPhone();
		        	phone.getIccCard().setIccNetworkLockEnabled(lockCategory,4,null,null,null,null,callback);
		        }
		        else//Gemini SIM:permanently remove the given SIM 's lock
		        {
		        	...
		        	if (intSIMNumber == 0)
		        	{
		        		mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_1).setIccNetworkLockEnabled(lockCategory,4,null,null,null,null,callback);
		        		//editPwdProcess(et, "12345678");       	//compare with the true password "lockPassword"	
		       	
		        	}
		        	else
		        	{
		        		mGeminiPhone.getIccCardGemini(Phone.GEMINI_SIM_2).setIccNetworkLockEnabled(lockCategory,4,null,null,null,null,callback);
		        		//editPwdProcess(et, "12345678");       	//compare with the true password "lockPassword"	
              ...
              case PERREMOVELOCK_ICC_SML_COMPLETE:
               {
            	   ...
            	   {
            		   //Toast.makeText(PermanUnlockSetting.this, "PermanUnlock lock fail", Toast.LENGTH_LONG).show();
            	   }
            	   else
            	   {
            		   //Toast.makeText(PermanUnlockSetting.this, "PermanUnlock lock succeed!", Toast.LENGTH_LONG).show();
               		   PermanUnlockSetting.this.finish();
            	   }
               }
               break;
