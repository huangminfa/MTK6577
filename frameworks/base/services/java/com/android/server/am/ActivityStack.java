...
final class ActivityStack {
    ...
    final boolean realStartActivityLocked(ActivityRecord r,
        ...
        try {
            ...
            mService.logAppLaunchTime(TAG, "scheduleLaunchActivity -> ActivityThread"); // [Note] It's for debugging App Launch time
        ...
        if (andResume) {
            ...
            r.stopped = false;
            r.waitingVisible = false; // [Note] It's to fix ALPS00141627
            mResumedActivity = r;
    ...
    private final void startSpecificActivityLocked(ActivityRecord r,
        ...
        //+MediaTek 2012-03-20 ALPS00105328 Fix black screen when starting an activty with DeadObjectException after called realStartActivityLocked()
    ...
    void checkReadyForSleepLocked() {
        ...
        if (!mSleepTimeout) {
            ...
            if (mStoppingActivities.size() > 0) {
                ...
                // workaround: fix "can't enter recovery mode" issue
    ...
    private final void completePauseLocked() {
        ...
        if (prev != null) {
            ...
            // [Note] it's to avoid deadlock in AMS/WMS
            // Ref: ALPS00231742
    ...
    final boolean resumeTopActivityLocked(ActivityRecord prev) {
        ...
        if (mResumedActivity == next && next.state == ActivityState.RESUMED) {
            ...
            // [Note] Fix a Google deadlock issue
            // we use a thread to perform executeAppTransition (ALPS00065219)
            new Thread("executeAppTransition") {
                @Override
                public void run() {
                    if (DEBUG_TRANSITION) Log.v(TAG, "perform executeAppTransition for " + mResumedActivity);
                    mService.mWindowManager.executeAppTransition();
                    mNoAnimActivities.clear();
                }
            }.start();
    ...
    final int startActivityUncheckedLocked(ActivityRecord r,
        ...
        if (DEBUG_TASKS) Slog.i(TAG, "launchFlags(original): 0x" + Integer.toHexString(launchFlags) + ", launchMode:" + r.launchMode
                                        + ", onlyIfNeeded: " + onlyIfNeeded + ", doResume:" + doResume);
        //+MediaTek 2012-04-04 Log for debugging
        if(!ActivityManagerService.IS_USER_BUILD)
           Slog.d(TAG, "ACT-launchFlags(original): 0x" + Integer.toHexString(launchFlags) + ", launchMode:" + r.launchMode
                                        + ", onlyIfNeeded: " + onlyIfNeeded + ", doResume:" + doResume);   
        //-MediaTek 2012-04-04 Log for debugging
        ...
        if (DEBUG_TASKS) Slog.i(TAG, "launchFlags(update): 0x" + Integer.toHexString(launchFlags));
            ...
            if (r.resultTo == null) {
                ...
                if (taskTop != null) {
                    ...
                    if (onlyIfNeeded) {
                        ...
                        if (DEBUG_TASKS) Slog.i(TAG, "START_RETURN_INTENT_TO_CALLER, doResume = " + doResume);
                    ...
                    if ((launchFlags &
                        ...
                        if (top != null) {
                            ...
                            if (DEBUG_TASKS) Slog.i(TAG, "special case ...");
                            addingToTask = true;
                        ...
                        if ((launchFlags&Intent.FLAG_ACTIVITY_SINGLE_TOP) != 0
                            ...
                            if (DEBUG_TASKS) Slog.i(TAG, "since different intents, start new activity...");
                            addingToTask = true;
                            sourceRecord = taskTop;
                        }
                        ...
                        if (DEBUG_TASKS) Slog.i(TAG, "place the new activity on top oc the current task...");
                        addingToTask = true;
                        sourceRecord = taskTop;
                    ...
                    if (!addingToTask && reuseTask == null) {
                        ...
                        if (DEBUG_TASKS) Slog.i(TAG, "START_TASK_TO_FRONT, doResume = " + doResume);
    ...
    final ActivityRecord activityIdleInternal(IBinder token, boolean fromTimeout,
        ...
        if (mService.mAlarmBoot && token != null) {
            int index = indexOfTokenLocked(token);
            ActivityRecord r = (ActivityRecord)mHistory.get(index);
            Slog.v(TAG, "name = " + r.realActivity.getClassName());
            if (r.realActivity.getClassName().equals("com.android.deskclock.AlarmAlertFullScreen")) {
                Slog.v(TAG, "reset mAlarmBoot & remove IPOWin");
                mService.mAlarmBoot = false;
    ...
    private final ActivityRecord finishCurrentActivityLocked(ActivityRecord r,
        ...
        if (DEBUG_SWITCH) Slog.v(TAG, "prevState = " + prevState);
        if (mode == FINISH_IMMEDIATELY
            ...
            // [Note] finish activity directly (ALPS00070338)
    ...
    final boolean destroyActivityLocked(ActivityRecord r,
        ...
        //+MediaTek 2012-04-04 Log for debugging
        if(!ActivityManagerService.IS_USER_BUILD)
           Slog.d(TAG, "ACT-Removing activity: token=" + r
              + ", app=" + (r.app != null ? r.app.processName : "(null)"));
        //-MediaTek 2012-04-04 Log for debugging    
