 ...
 * @author mtk01635
 * 
 * 1. process Intent with actions: ACTION_PUSH_REQUEST
 * 
 */
public class OppServerActivity extends CancelableActivity {
	...
	protected void onActivityCancel( int id ){
		...
		if( id == CancelableActivity.NULL_CANCEL_ID ){
			...
			try {
				...
				// will be canceled
				OppLog.e( "OppServerActivity.onActivityCancel() error: intent[" + this.getIntent() + "], exception:" + ex.getMessage() );
			}
		}
	...
	protected void onCreate( Bundle savedInstanceState ){
		...
		if( savedInstanceState != null ){
			...
			this.isActive = savedInstanceState.getBoolean( IS_ACTIVE, false );
		}

		// check if it's: launched from history && not active => skip it
		if( ( intent.getFlags() & Intent.FLAG_ACTIVITY_LAUNCHED_FROM_HISTORY ) != 0 && !this.isActive ){
	...
	protected void onStop(){
		...
		// re-send incoming notification when incoming request is active (undetermined)
		if( this.isActive ){
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		if( this.isActive ){
			...
			outState.putBoolean( IS_ACTIVE, this.isActive );
		}
	}
		...
		switch( id ){
			case DIALOG_PUSH_CONFIRMATION:
				...
				return new AlertDialog.Builder(this)
					.setTitle( R.string.bt_opps_confirm_push_title )
					.setMessage( this.getString(R.string.bt_opps_confirm_push_message, 
							objectName,
							Formatter.formatFileSize( this, totalBytes ),
							peerName ) )
					.setPositiveButton( R.string.bt_opps_confirm_push_yes, new DialogInterface.OnClickListener() {
