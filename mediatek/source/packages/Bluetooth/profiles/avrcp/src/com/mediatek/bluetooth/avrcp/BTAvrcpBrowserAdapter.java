...
public class BTAvrcpBrowserAdapter extends Thread {
	...
	private BTAvrcpBrowserSearch searchAdapter;
	...
	private final static byte OK = BTAvrcpProfile.OK;
	private final static byte FAIL = BTAvrcpProfile.FAIL;
    ...
    public final static byte STATUS_FAIL = BTAvrcpProfile.AVRCP_ERRCODE_INVALID_PARAM; /* AVRCP status definition */
	...
	BTAvrcpBrowserAdapter(BluetoothAvrcpService service) {
		mAvrcpServ = service;
		isSearch = false;
		searchAdapter = new BTAvrcpBrowserSearch(service);
		setName( "BTAvrcpBrowserAdapterThread" );

		mAlbumCategory = new BTAvrcpBrowseAlbum(0x1001, mAvrcpServ,
				BTAvrcpBrowseAlbum.ALBUM_TYPE);
		mArtistCategory = new BTAvrcpBrowseAlbum(0x1002, mAvrcpServ,
				BTAvrcpBrowseAlbum.ARTIST_TYPE);
		mEmptyCategory = new BTAvrcpBrowseAlbum(0x1003, mAvrcpServ,
				BTAvrcpBrowseAlbum.EMPTY_TYPE);
		mAllCategory = new BTAvrcpBrowseAlbum(0x1004, mAvrcpServ,
				BTAvrcpBrowseAlbum.ALL_TYPE);
		if (true == checkPTSMode()) {
		...
		if( true == BluetoothAvrcpService.isSupportBrowse() ){
	...
	public void onConnect() {
		...
		if( true != BluetoothAvrcpService.bSupportBrowse ){
	...
	public boolean changePath(short uid_counter, byte dir, long uid) {
		boolean bRet = false;
		if (uid_counter != UNAWARE_UIDCOUNTER) {
			Log.i(TAG, "[BT][AVRCP] changePath fail to  uid_counter:"
					+ uid_counter);
			return false;
		}
		if (uid_counter == UNAWARE_UIDCOUNTER && uid == 0 && dir != 0) {
			Log.i(TAG, "[BT][AVRCP] changePath fail  uid_counter:"
					+ uid_counter + " dir:" + dir + " uid:" + uid);
			return false;
		}
	...
	public boolean getFileSystemitemsList(int start, int end, byte count,
			int attr_ids[]) {
		//
		mGetFileAttrCount = count;
		mGetFileAttr = attr_ids;
	...
	public int getCurPathItems() {
		...
		Log.v(TAG, "[BT][AVRCP] getCurPathItems mCurPathItemNum:"
				+ mCurPathItemNum);
	...
	public boolean isItemExist(byte scope, long identifier, short uid_counter) {
		if (uid_counter != UNAWARE_UIDCOUNTER) {
			return false;
		}
	...
	String getItemAttribute(byte scope, long identifier, short uid_counter,
			int attr_id) {
		String sRet = null;

		if (uid_counter != UNAWARE_UIDCOUNTER) {
			Log.e(TAG, "[BT][AVRCP] getItemAttribute wrong uid_counter"
					+ uid_counter);
			return null;
		}
		...
		{
			if (null != mCurCategory) {
				return mCurCategory.getItemAttribute(scope, identifier,
						uid_counter, attr_id);
			}
		...
		case BTAvrcpProfile.AVRCP_SCOPE_SEARCH: // 0x02
		{
			...
			if (identifier == mSearchId) {
				...
				Log.e(TAG, "[BT][AVRCP] fail to updateSearchSongId ! id:"
						+ identifier);
			}
		}
		break;
		...
		case BTAvrcpProfile.AVRCP_SCOPE_NOW_PLAYING: // 0x03
		{
			...
			if (identifier == mNowId) {
				...
				Log.e(TAG, "[BT][AVRCP] fail to updateNowSongId ! id:"
						+ identifier);
			}
		}
		break;
	...
	String getSearchedItemAttribute(int index, int attr_id) {
		switch (attr_id) {
		case 0x01:
			return searchAdapter.getSearchedTitleString(index);
		case 0x02:
			return searchAdapter.getSearchedArtistString(index);
		case 0x03:
			return searchAdapter.getSearchedAlbumString(index);
		}
		return null;
	}
		...
		try {
			if (cursor != null) {
				if (cursor.getCount() > 0) {
					...
					mSearchId = newId;
					Log.v(TAG, "[BT][AVRCP] updateSearchSongId '"
							+ mSearchTtitle + "' '" + mSearchArtist + "' '"
							+ mSearchAlbum + "'");
					return true;
				} else {
		...
		try {
			if (cursor != null) {
				if (cursor.getCount() > 0) {
					...
					Log.v(TAG, "[BT][AVRCP] updateNowSongId '" + mNowTtitle
							+ "' '" + mNowArtist + "' '" + mNowAlbum + "'");
					return true;
				} else {
					Log.e(TAG, "[BT][AVRCP] query and get empty result !");
				}
	...
	public long[] getSearchedList() {
		mSearchResultList = searchAdapter.getSearchedList();
	...
	public boolean registerNotification(byte eventId, int interval) {
		...
		if( eventId == BTAvrcpProfile.EVENT_UIDS_CHANGED){
			mAvrcpServ.notificationUIDSChangedNative(OK, (byte)1, (short)0); // player_id			
			return true;
		}
		 */
		return false;
	...
	public void passToHandleMessage(Message msg) {
		...
		sMsg = String.format("[BT][AVRCP] AVRCPBrowser Receive a msg.what:%d msg.arg1:%d msg.arg2:%d ", msg.what, msg.arg1, msg.arg2);
		...
		case ACTION_GET_FILELIST:
			handleGetFileList(msg.arg1, msg.arg2, mGetFileAttrCount,
					mGetFileAttr);
			break;

		default:
			Toast.makeText(mAvrcpServ, "[BT][AVRCP][BWS]no Handle msg !",
					Toast.LENGTH_LONG).show();
			break;
		}
	}

	private void handleGetFileList(int start, int end, int attr_count,
			int[] attr_ids) {
		int total;
		long[] idlist;
		int length = 0;
		int i = 0, j = 0;
		byte attrIndex;
		short u2AttrLength, u2ItemLength;
		String sAttrValue, sItemValue;
		...
		if (null != mCurCategory) {
			...
			if (null == idlist || start >= length) {
				// out of bound. PTS TC_TG_MCN_CB_BI_03_C test start with the length.
				Log.v(TAG, String.format("[BT][AVRCP] handleGetFileList  out-of-bound start:%d list.length:%d", start, length) );
				mAvrcpServ.getFileSystemItemEndRspNative((byte)FAIL, BTAvrcpProfile.AVRCP_ERRCODE_RANGE_OUT_OF_BOUNDS, mUidCounter);
				return;
			}
			if (idlist.length == 0) {
				mAvrcpServ.getFileSystemItemEndRspNative((byte)FAIL, BTAvrcpProfile.AVRCP_ERRCODE_RANGE_OUT_OF_BOUNDS, mUidCounter);
			...
			Log.v(TAG, String.format("[BT][AVRCP] handleGetFileList start:%d end:%d list.length:%d", start, end, length) );
			if (end >= length) {
				end = length - 1;
			}
			...
			switch (mCurCategory.getType()) {
				case BTAvrcpProfile.ITEM_TYPE_FOLDER:
				{
					...
					for (i = start; i <= end; i++) {
						// configure the attribute
							//getFileSystemItemFileAttrRspNative(byte error, byte item, byte attrIndex, int attrId, short charset, short len, String AttrValue);
						attrIndex = 0;
						...
						if (null != sItemValue) {
							...
							//getFileSystemItemFolderRspNative(byte error, byte item, byte total, long uid_counter, byte foldertype, byte playable, short charset, short len, String foldername );
							mAvrcpServ.getFileSystemItemFolderRspNative(OK, (byte)(i-start), (byte)total, idlist[i], mCurCategory.getFolderType(), (byte)0,  UTF8_CHARSET, u2ItemLength, sItemValue);
					}
				...
				case BTAvrcpProfile.ITEM_TYPE_ELEMENT:
				{
					...
					for (j = 0; j < attr_count; j++) {
							sAttrValue = mCurCategory.getAttributeByIndex(i, attr_ids[j]);

						if (sAttrValue != null) {
							u2AttrLength = (short) sAttrValue.length();
							//getFileSystemItemFileAttrRspNative(byte error, byte item, byte attrIndex, int attrId, short charset, short len, String AttrValue);
							mAvrcpServ.getFileSystemItemFileAttrRspNative(OK, (byte)(i-start), attrIndex, attr_ids[j], UTF8_CHARSET, u2AttrLength, sAttrValue);
							attrIndex++;
						}
					}
					...
					//getFileSystemItemFileRspNative(byte error, byte item, byte total, long uid, byte mediatype, short charset, short len, String filename);
					mAvrcpServ.getFileSystemItemFileRspNative(OK, (byte)(i-start), (byte)total, idlist[i],  (byte)0x00, UTF8_CHARSET, u2ItemLength, sItemValue);
			...
			if (start > mCategory.length) {
				// out-of-bound
				Log.e(TAG, String.format("[BT][AVRCP] handleGetFileList out-of-bound start:%d mCategory.length:%d", start, mCategory.length) );
				mAvrcpServ.getFileSystemItemEndRspNative((byte)FAIL, BTAvrcpProfile.AVRCP_ERRCODE_RANGE_OUT_OF_BOUNDS, mUidCounter);
				return;
			}
			...
			for (i = start; i <= end; i++) {
				if( i >= mCategory.length){
					Log.e(TAG, String.format("[BT][AVRCP][ERR] Out-of-Array mCategory.length:%d", i, mCategory.length) );
					mAvrcpServ.getFileSystemItemEndRspNative((byte)FAIL, BTAvrcpProfile.AVRCP_ERRCODE_RANGE_OUT_OF_BOUNDS, mUidCounter);
					return;
				}else{
					...
					u2ItemLength = (short) sItemValue.length();
					//getFileSystemItemFolderRspNative(byte error, byte item, byte total, long uid_counter, byte foldertype, byte playable, short charset, short len, String foldername );
					mAvrcpServ.getFileSystemItemFolderRspNative(OK, (byte)(i-start), (byte)total, mCategory[i].getPresentId() , BTAvrcpProfile.ITEM_TYPE_FOLDER, (byte)0,  UTF8_CHARSET, u2ItemLength, sItemValue);
				}
			}
	...
	private void handleSearch(String sText) {
		...
		if (0 == sText.length()) {
			...
			num_of_items = 0;
			mAvrcpServ.searchRspNative(BTAvrcpProfile.OK, status, UNAWARE_UIDCOUNTER, num_of_items);
			isSearch = false;
			return;
		}

		if (null != searchAdapter) {
			num_of_items = searchAdapter.search(sText);
		}
	...
	private void handleChangePath(short uid_counter, byte dir, long uid) {
		boolean bRet = false;
		byte status = 0;
		int i = 0;

		// success or fail - both invoke server's
		Log.i(TAG, "[BT][AVRCP] handleChangePath uid_counter:" + uid_counter
				+ " dir:" + dir + " uid:" + uid);

		if (dir == BTAvrcpProfile.DIR_UP) {
			if (null != mCurCategory) {
				if (true == mCurCategory.isCategoryRoot()) {
					mCurCategory = null; // up to category level
					mCurPathItemNum = mCategory.length;
					Log
					.v(TAG,
							"[BT][AVRCP][BWS] handleChangePath goUp to catagory");
					bRet = true;
				} else {
					mCurCategory.goUp();
					...
					Log.v(TAG,
							"[BT][AVRCP][BWS] handleChangePath goUp ok num-of-items:"
							+ mCurPathItemNum);
					bRet = true;
				}
			} else {
			...
			if (null != mCurCategory) {
				if (true == mCurCategory.goDown(uid)) {
					// success to change path
					mCurPathItemNum = mCurCategory.getCurPathItems();
					Log.v(TAG, "[BT][AVRCP][BWS] handleChangePath goDown ok:"
							+ uid + " num-of-items:" + mCurPathItemNum);
					bRet = true;
				} else {
					// fail to chang path
					status = BTAvrcpProfile.AVRCP_ERRCODE_NOT_A_DIRECTORY;
					Log.w(TAG,
							"[BT][AVRCP][BWS] handleChangePath mCurCategory.goDown fail uid"
							+ uid);
					bRet = false;
				}
			} else {
				...
				for (i = 0; i < mCategory.length; i++) {
					if (uid == mCategory[i].getPresentId()) {
						...
						bRet = true;
						Log.v(TAG,
								"[BT][AVRCP][BWS] handleChangePath down to category ok:"
								+ uid + " num:" + mCurPathItemNum);
						break;
					}
				}

				if (true != bRet) {
					Log.e(TAG, "[BT][AVRCP][BWS] handleChangePath down to "
							+ uid + " fail! cannot found ");
					bRet = false;
		...
		Log.i(TAG, "[BT][AVRCP] changePath final bRet:"
		...
		if (true == bRet) {
			...
			switch(status){
				...
				mAvrcpServ.changePathRspNative(
						(byte) status, status,
						mCurPathItemNum);
				break;
				default:
				mAvrcpServ.changePathRspNative(
						(byte) BTAvrcpProfile.AVRCP_ERRCODE_NOT_EXIST, status,
						mCurPathItemNum);
				break;
			}
		}
