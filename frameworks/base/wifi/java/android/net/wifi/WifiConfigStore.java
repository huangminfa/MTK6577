...
class WifiConfigStore {
    ...
    static void enableAllNetworks() {
        ...
        int connectPolicy = Settings.System.getInt(sContext.getContentResolver(), Settings.System.WIFI_CONNECT_AP_TYPE, Settings.System.WIFI_CONNECT_AP_TYPE_AUTO);
        int cellToWiFiPolicy = Settings.System.getInt(sContext.getContentResolver(), Settings.System.WIFI_CONNECT_TYPE, Settings.System.WIFI_CONNECT_TYPE_AUTO);
        Xlog.e(TAG, "enableAllNetworks, connectPolicy=" + connectPolicy + ", cellToWiFiPolicy=" + cellToWiFiPolicy);
        if (OPTR.equals("OP01") 
            && !(connectPolicy == Settings.System.WIFI_CONNECT_AP_TYPE_AUTO && cellToWiFiPolicy == Settings.System.WIFI_CONNECT_TYPE_AUTO)) {
            return;
        }
        ...
        setVariables: {
            ...
            if (FeatureOption.MTK_EAP_SIM_AKA) {
                ...
                if (config.SIMSLOT != null &&
                        !WifiNative.setNetworkVariableCommand(
                            netId,
                            WifiConfiguration.simSlotVarName,
                            config.SIMSLOT)) {
    ...
    private static void readNetworkVariables(WifiConfiguration config) {
        ...
        // Add for EAP-SIM begin
        if (FeatureOption.MTK_EAP_SIM_AKA) {
            value = WifiNative.getNetworkVariableCommand(netId, WifiConfiguration.imsiVarName);
            if (!TextUtils.isEmpty(value)) {
                config.IMSI = value;
            } else {
                config.IMSI = null;
            ...
            value = WifiNative.getNetworkVariableCommand(netId, WifiConfiguration.simSlotVarName);
            if (!TextUtils.isEmpty(value)) {
            ...
            value = WifiNative.getNetworkVariableCommand(netId, WifiConfiguration.pcscVarName);
            if (!TextUtils.isEmpty(value)) {
                config.PCSC = value;
            } else {
                config.PCSC = null;
    ...
    //MTK_OP01_PROTECT_START
    static boolean saveAPPriority() {
        return WifiNative.saveAPPriorityCommand();
    }
    //MTK_OP01_PROTECT_END
}
