...
/* MTK */
/******************************************************************************
 * Rv3G324mCallSendSelectedMultiplexEntry
 * ----------------------------------------------------------------------------
 * General: Generate and send selected the entries from the Multiplex Entry Table
 *          used by H.223 and send an H.245 MultiplexEntrySend (MES) message
 *          to the remote side of the call.
 *          The application has to call this function after all outgoing
 *          channels were opened and every time it wants to update the table
 *          due to changes in its outgoing channels.
 *
 ...
 * Input:  h3G324mCall  - The call handle.
 *****************************************************************************/
RVAPI RvStatus RVCALLCONV Rv3G324mCallSendSelectedMultiplexEntry(
    IN  HCALL                   h3G324mCall,
    IN  RvUint16*               entries);

/******************************************************************************
 * getValidDemuxEntries
 * ----------------------------------------------------------------------------
 * General: 
 *
 * Return Value: entries  - if successful.
 *               0 on failure
 * ----------------------------------------------------------------------------
 * Arguments:
 * Input:  h3G324mCall  - The call handle.
 * ----------------------------------------------------------------------------
 * Notes:
 * This function may need modification for each RV release!
 *****************************************************************************/
RVAPI RvUint16 RVCALLCONV getValidDemuxEntries(
      IN   HCALL      h3G324mCall);

/******************************************************************************
 * Rv3G324mCallSendFunctionNotSupported
 * ----------------------------------------------------------------------------
 * General: Send FunctionNotSupported command
 *
 * Return Value: 
 *               
 * ----------------------------------------------------------------------------
 ...
 * Output: 
 *****************************************************************************/
RvInt32 Rv3G324mCallSendFunctionNotSupported(
    IN HAPP hApp, 
    IN HPVT hVal, 
    IN HCALL hCall);
/* MTK */
...
/* MTK */
RVAPI RvStatus RVCALLCONV RvCallSetControlbIsPeerSupportNSRP(
   IN  HCALL                   h3G324mCall,
   IN  RvBool                  bIsPeerSupportNSRP);

RvBool Rv3G324mCallIsMuxEntryValid(
    IN HCALL hCall,
