...
class HTML5VideoViewProxy extends Handler
    ...
    private static final int RELEASE             = 110;
    private static final int SET_PAUSE_DURING_PREPARING = 111;
    /// @}
    ...
    /// M: improve youtube html5 video rotate performance. @{
    private static final int TOGGLE_PLAY_STATE = 220;
    private static final int TOGGLE_PAUSE_STATE = 221;
    /// @}
    ...
    private static final class VideoPlayer {
        ...
        public static int getCurrentState() {
            ...
            return mHTML5VideoView.getCurrentState();
        }

        public static void setPauseDuringPreparing(boolean pause) {
            if (mHTML5VideoView == null)
                return;

            mHTML5VideoView.setPauseDuringPreparing(pause);
        }
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case SET_PAUSE_DURING_PREPARING: {
                VideoPlayer.setPauseDuringPreparing(msg.arg1 == 1);
            }
            /// @}
        }
    }
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    ...
                    case TOGGLE_PLAY_STATE:
                        nativeToggleHTML5VideoPlayState(mNativePointer);
                        break;
                    case TOGGLE_PAUSE_STATE:
                        nativeToggleHTML5VideoPauseState(mNativePointer);
                        break;
                    /// @}
                }
            }
    ...
    private native static boolean nativeSendSurfaceTexture(SurfaceTexture texture,
            ...
            int playerState);
    /// M: improve youtube html5 video rotate performance. @{
    private native void nativeToggleHTML5VideoPlayState(int nativePointer);
    private native void nativeToggleHTML5VideoPauseState(int nativePointer);
    /// @}
    ...
    public void release() {
        Message message = obtainMessage(RELEASE);
        sendMessage(message);
    }

    public void toggleHTML5VideoPlayState() {
        if (mWebCoreHandler.hasMessages(TOGGLE_PAUSE_STATE) == true)
            mWebCoreHandler.removeMessages(TOGGLE_PAUSE_STATE);
        if (mWebCoreHandler.hasMessages(TOGGLE_PLAY_STATE) == false) {
            Message msg = Message.obtain(mWebCoreHandler, TOGGLE_PLAY_STATE);
            mWebCoreHandler.sendMessage(msg);
        }
    }

    public void toggleHTML5VideoPauseState() {
        if (mWebCoreHandler.hasMessages(TOGGLE_PLAY_STATE) == true)
            mWebCoreHandler.removeMessages(TOGGLE_PLAY_STATE);
        if (mWebCoreHandler.hasMessages(TOGGLE_PAUSE_STATE) == false) {
            Message msg = Message.obtain(mWebCoreHandler, TOGGLE_PAUSE_STATE);
            mWebCoreHandler.sendMessageAtFrontOfQueue(msg);
        }
    }

    public void setPauseDuringPreparing(boolean pause) {
        Message message = obtainMessage(SET_PAUSE_DURING_PREPARING, (pause==true)?1:0, 0);
        sendMessage(message);
    }

    public boolean isFullScreenMode() {
        return VideoPlayer.isFullScreenMode();
    }
    /// @}
}
