...
import com.mediatek.apst.target.util.FeatureOptionControl;
 ...
 * <p>Package: com.mediatek.apst.target.service
 * <p>Created on: 2010-6-30
 * <p>
 * <p>Description: 
 * <p>Main service of Android PC Sync Tool target daemon. Stays alive and 
 * handles connection and communication with PC. 
 * <p>
 * @author mtk80734 Siyang.Miao
 * @version V1.0
 */
public class MainService extends Service 
implements IBatteryListener, IPackageListener, ISdStateListener, 
ISimStateListener, ISmsListener, IContactsListener, ICalendarEventListener,IMmsListener {
    //==============================================================
    // Constants                                                    
    //==============================================================
    ...
    // Fields                                                       
    //==============================================================
    private static Boolean mHasInstance = false;
    ...
    private CommandHandler mCmdHandler = new CommandHandler(500); // add size to 500
    ...
    public void onCreate() {
        ...
        mCalendarEventOb = new CalendarEventObserver(mMsgHandler, mCalendarProxy);
    ...
    private void notifyStart() {
        ...
        Notification notification = new Notification(R.drawable.stat_pcsync_tool, text, 
    ...
    private void installCommunicationCallbacks(){
        ...
        mDispatcher.installCallback(BaseCommand.FEATURE_CONTACTS, 
                mMainCallback);
        ...
        mDispatcher.installCallback(BaseCommand.FEATURE_CALENDAR, mMainCallback);
		mDispatcher.installCallback(BaseCommand.FEATURE_BOOKMARK, mMainCallback);
        mDispatcher.installCallback(BaseCommand.FEATURE_CALENDAR_SYNC, mMainCallback);
    ...
    private void uninstallCommunicationCallbacks(){
        if (mDispatcher != null) {
            mDispatcher.uninstallCallback(ConnManageEntity.ConnManageFeatureID);
    ...
    private void registerEventListeners(){
        ...
        mContactsOb.start();*/
        getContentResolver().registerContentObserver(
                SmsContent.CONTENT_URI, true, mMessageOb);
        getContentResolver().registerContentObserver(
                MmsContent.CONTENT_URI_OB, true, mMulMessageOb);
        mMessageOb.start();
        getContentResolver().registerContentObserver(
    ...
    private synchronized boolean enqueueHandleCommand(BaseCommand cmd){
        if (null != mCmdHandler) {
            ...
            Debugger.logW(new Object[]{cmd}, "Command handler thread is null.");
            return false;
        }
    }
    ...
    private synchronized boolean enqueueSendCommand(BaseCommand cmd){
        if (null != mCmdSender) {
            ...
            Debugger.logW(new Object[]{cmd}, "Command sender thread is null.");
            return false;
        }
    }
     ...
     * @param rspCmd Response command to handle.
     */
    ...
    public void onSmsSent(Event event){
        ...
        new Thread(){
            ...
            public void run(){
                ...
                req.setSent(sent);
                req.setMessageType(
                        com.mediatek.apst.util.entity.message.Message.TYPE_SMS);
    ...
    public void onSmsReceived(Event event){
        mIncomingSmsFinder.appendTask(new NewSmsFinder.Clue(
                event.getLong(ISmsListener.AFTER_TIME_OF), 
                event.getString(ISmsListener.ADDRESS), 
                event.getString(ISmsListener.BODY), 
                Sms.BOX_INBOX));
    }
    ...
    public void onContactsContentChanged(Event event){
        if (!event.getBoolean(IContactsListener.BY_SELF)){
            NotifyContactsContentChangeReq req = 
                new NotifyContactsContentChangeReq();
            req.setToken(mDispatcher.getToken());
        ...
        new Thread(){
            ...
            public void run(){
                ArrayList<ApplicationInfo> apps = 
                    mApplicationProxy.getApplicationsForUid(uid);
                if (apps.size() > 0){
    ...
    public void onPackageDataCleared(Event event) {
        ...
        try {
            ...
            if (contactsAppUid == uid){
                ...
                NotifyContactsDataClearedReq req = 
                    new NotifyContactsDataClearedReq();
                req.setToken(mDispatcher.getToken());
            ...
            if (messageAppUid == uid){
                ...
                NotifyMessageDataClearedReq req = 
                    new NotifyMessageDataClearedReq();
                req.setToken(mDispatcher.getToken());
...
//    @Override
	public void onCalendarEventContentChanged(Event event) {
    	...
    	if (!event.getBoolean(ICalendarEventListener.BY_SELF)){
            NotifyCalendarEventChangeReq req = 
                new NotifyCalendarEventChangeReq();
            req.setToken(mDispatcher.getToken());
            req.setEvent((CalendarEvent) event.get(ICalendarEventListener.CALENDAREVENT));
    ...
    class MainCallback implements ICallback {
        ...
        public boolean execute(TransportEntity entity) {
            Debugger.logI(new Object[]{entity}, 
                    "Entity received.");
            if (null == entity) {
            ...
            } else if (entity.getFeatureID() == 
                ConnManageEntity.ConnManageFeatureID) {
    ...
    class CommandSender extends BlockingCommandHandlingThread{
        ...
        private void send(BaseCommand cmd){
            if (null != mComm){
                ...
                if(-1 != token){
                    Debugger.logI(this.getClassName(), "send",new Object[]{cmd}, 
                            "Successfully, FeatureID=" + cmd.getFeatureID());
                } else {
                    Debugger.logW(this.getClassName(), "send",new Object[]{cmd}, 
                            "Failed, FeatureID=" + cmd.getFeatureID());
                }
            } else {
                Debugger.logW(this.getClassName(), "send", new Object[]{cmd}, 
    ...
    class CommandHandler extends BlockingCommandHandlingThread{
        ...
        public void handle(BaseCommand cmd){
            ...
            if (BaseCommand.FEATURE_MAIN == cmd.getFeatureID()) {
                ...
                Debugger.logE(this.getClassName(), "handle", new Object[]{cmd}, 
                        "Unsupported feature.");
            }
            ...
            if (!handled){
                Debugger.logE(this.getClassName(), "handle", new Object[]{cmd}, 
                        "): Unsupported command.");
                onRespond(new UnsupportedRequestResponse(
                        cmd.getFeatureID(), cmd.getToken()));
            }
        }
        ...
        public boolean handleMainFrameFeatures(BaseCommand cmd){
            ...
            if (cmd instanceof GreetingReq){
                ...
                if (SystemInfoProxy.isSdMounted()){
                    ...
                    rsp.setSdCardAvailableSpace(
                            SystemInfoProxy.getSdAvailableSpace());
                } else {
                ...
                rsp.setInternalTotalSpace(
                        SystemInfoProxy.getInternalTotalSpace());
                rsp.setInternalAvailableSpace(
                        SystemInfoProxy.getInternalAvailableSpace());
                // Applications & Data
                rsp.setContactsCount(mContactsProxy.getAvailableContactsCount());
//                rsp.setSimContactsCount(mContactsProxy.getSimContactsCount());
                rsp.setMessagesCount(mMessageProxy.getMessagesCount());
                rsp.setApplicationsCount(
                        mApplicationProxy.getApplicationsCount());
                // SIM
                if (Config.MTK_GEMINI_SUPPORT){
                    ...
                    rsp.setSim1Info(Global.getSimInfoBySlot(SimDetailInfo.SLOT_ID_ONE));
                    rsp.setSim2Info(Global.getSimInfoBySlot(SimDetailInfo.SLOT_ID_TWO));
                    ...
                    rsp.setSimInfo(Global.getSimInfoBySlot(SimDetailInfo.SLOT_ID_SINGLE));
                    Debugger.logD(this.getClassName(), "handle", 
        ...
        public boolean handleContactsFeatures(BaseCommand cmd){
            ...
            if (cmd instanceof ContactsBatchReq){
                ...
                GetDetailedContactRsp rsp = new GetDetailedContactRsp(
                        reqToken);
                // Get data and fill them in response command
                GetDetailedContactReq req = (GetDetailedContactReq)cmd;
                RawContact detailedContact = mContactsProxy.getContact(req.getContactId(), true);
                if (null != detailedContact) {
                    rsp.setDetailedContact(detailedContact, Global.getByteBuffer(), Config.VERSION_CODE);
                } else {
                ...
                long result = mContactsProxy.insertContact(
                        contact, true);                
                if(result < 0 ) {
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                    ...
                    rsp.setInsertedId(result);
                    rsp.setResult(mContactsProxy.getContact(
                            result, true));
                }
                // Send response command or append it to batch
                onRespond(rsp);
                ...
                if (simIndex == null) {
                	rsp.setInsertedIds(mContactsProxy.insertGroupMembership(
                            req.getContactIds(), req.getGroupId()));
                } else {
                	rsp.setInsertedIds(mContactsProxy.insertGroupMembership(
                            req.getContactIds(), req.getGroupId(), group, simIndex));
                }
                ...
                if (oldName == null) { 
                	updateGroupCount = mContactsProxy.updateGroup(req.getUpdateId(), group);
                } else { 
                	updateGroupCount = mContactsProxy.updateGroup(req.getUpdateId(), group, oldName);
                }
                ...
                int updateRawContactCount = mContactsProxy.updateContact(
                        req.getUpdateId(), RawContact.SOURCE_NONE, 
                        null, null, contact, 
                        false/*Do not update PIM*/);
                ...
                RawContact contact = req.getNewOne();    
//                int updateDetailedContactCount = mContactsProxy.updateContact(
//                        req.getUpdateId(), req.getSourceLocation(), 
//                        req.getSimName(), req.getSimNumber(), contact, 
//                        true/*Update PIM data*/);
                int updateDetailedContactCount = mContactsProxy.updateContact(
                        req.getUpdateId(), req.getSourceLocation(), 
                        req.getSimName(), req.getSimNumber(), req.getSimEmail(), contact, 
                        true/*Update PIM data*/);
                if (updateDetailedContactCount < 1){
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                } else {
                    rsp.setResult(mContactsProxy.getContact(
                            req.getUpdateId(), true));
                }
                // Send response command or append it to batch
                onRespond(rsp);
                ...
                int updateSimContactCount = mContactsProxy.updateSimContact(
                        req.getOldName(), req.getOldNumber(), 
                        req.getNewOne().getDisplayName(), 
                        req.getNewOne().getPrimaryNumber(), 
                        req.getSimId());
                ...
                if (groups == null) {
                	rsp.setDeleteResults(mContactsProxy.deleteGroup(
                            req.getDeleteIds()));
                } else {
                	rsp.setDeleteResults(mContactsProxy.deleteGroup(
                            req.getDeleteIds(), groups));
                }
...
//                        req.getSimNames(), req.getSimNumbers()));
                rsp.setDeleteResults(mContactsProxy.deleteContacts (
                        req.getDeleteIds(), false, req.getSourceLocation(), 
                        req.getSimNames(), req.getSimNumbers(), req.getSimEmails()));
                // Send response command or append it to batch
                onRespond(rsp);
                ...
                rsp.setDeleteResults(mContactsProxy.deleteSimContacts(
                        req.getNames(), req.getNumbers(), req.getSimId()));
                // Send response command or append it to batch
                onRespond(rsp);
                ...
                if (simIndex == null) {
                	...
                	rsp.setDeleteResults(mContactsProxy.deleteContactData(
                            req.getDeleteIds(), req.getGroupId(), group, simIndex));
                }               
                ...
                mContactsProxy.asyncGetAllGroups(
                        new IRawBlockConsumer(){
                    
                    //@Override
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                        AsyncGetAllGroupsRsp rsp = 
                            new AsyncGetAllGroupsRsp(reqToken);
                        rsp.setRaw(block);
                ...
                if (simAccesible){
                    mContactsProxy.asyncGetAllSimContacts(
                        ...
                        public void consume(byte[] block, int blockNo, 
                                int totalNo) {
                            AsyncGetAllSimContactsRsp rsp = 
                                new AsyncGetAllSimContactsRsp(reqToken);
                            rsp.setRaw(block);
                            rsp.setProgress(blockNo);
                            rsp.setTotal(totalNo);
                            // Send response command or append it to batch
                ...
                mContactsProxy.asyncGetAllRawContacts(
                        new IRawBlockConsumer(){
                    
                    //@Override
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                        AsyncGetAllRawContactsRsp rsp = 
                            new AsyncGetAllRawContactsRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                mContactsProxy.asyncGetAllContactData(
                    ...
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                        AsyncGetAllContactDataRsp rsp = 
                            new AsyncGetAllContactDataRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                        ImportDetailedContactsRsp rsp = 
                            new ImportDetailedContactsRsp(reqToken);
                        rsp.setPhase(ImportDetailedContactsRsp.PHASE_RAW_CONTACT);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, 
                        ...
                        ImportDetailedContactsRsp rsp = 
                            new ImportDetailedContactsRsp(reqToken);
                        rsp.setPhase(ImportDetailedContactsRsp.PHASE_CONTACT_DATA);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
            ...
            if (cmd instanceof MessageBatchReq){
                ...
                mMessageProxy.asyncGetPhoneList(new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, int totalNo){
                        AsyncGetPhoneListRsp rsp = 
                            new AsyncGetPhoneListRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                mMessageProxy.asyncGetAllSms(new IRawBlockConsumer(){
                    ...
                    //@Override
                    public void consume(byte[] block, int blockNo, int totalNo){
                        AsyncGetAllSmsRsp rsp = 
                            new AsyncGetAllSmsRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                mMulMessageOb.stop();
                long[] insertedIds = 
                    mMessageProxy.importSms(req.getRaw(), temp);
                if (null == insertedIds){
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                } else {
                    if (null != temp){
                ...
                boolean isBackup = ((ImportMmsReq) cmd).getIsBackup();
                ...
                long[] insertedIds = 
                    mMessageProxy.importMms(req.getRaw(), temp , isBackup);
                if(isBackup) {
                ...
                // Send response command or append it to batch
                if (req.isIsLastImport() && !isBackup) {
                    onRespond(rsp);
                }
                ...
                rsp.setDeletedCount(mMessageProxy.deleteAllMessages(
                        req.isKeepLockedMessage()));
                // Send response command or append it to batch
                onRespond(rsp);
                ...
                rsp.setDeleteSmsCount(mMessageProxy.deleteSms(
                        req.getDeleteSmsIds(), true, req.getDeleteSmsDates()));
                rsp.setDeleteMmsCount(mMessageProxy.deleteMms(
                        req.getDeleteMmsIds(), true, req.getDeleteMmsDates()));
                // Send response command or append it to batch
                onRespond(rsp);
                ...
                rsp.setUpdateSmsCount(mMessageProxy.markSmsAsRead(
                        req.getUpdateSmsIds(), req.isRead()));
                rsp.setUpdateMmsCount(mMessageProxy.markMmsAsRead(
                        req.getUpdateMmsIds(), req.isRead()));
                // Send response command or append it to batch
                onRespond(rsp);
                ...
                rsp.setUpdateSmsCount(mMessageProxy.lockSms(
                        req.getUpdateSmsIds(), req.isLocked()));
                rsp.setUpdateMmsCount(mMessageProxy.lockMms(
                        req.getUpdateMmsIds(), req.isLocked()));
                // Send response command or append it to batch
                onRespond(rsp);
                ...
                Sms result = mMessageProxy.saveSmsDraft(req.getBody(), 
                        req.getRecipients());
                if (null != result) {
                ...
                mSmsSender.pause();
                Sms[] results = mMessageProxy.sendSms(req.getBody(), 
                        req.getRecipients(), 
                ...
                rsp.setSimId(req.getSimId());
                mSmsSender.pause();
                Sms result = mMessageProxy.resendSms(
                        req.getId(), 
                        req.getDate(), 
                        req.getBody(), 
                        req.getRecipient(), 
            ...
            if (cmd instanceof AsyncGetAllAppInfoReq){
                mApplicationProxy.fastGetAllApplications(
                    new IRawBlockConsumer(){
                        ...
                        public void consume(byte[] block, int blockNo,
                                int totalNo) {
                            AsyncGetAllAppInfoRsp rsp = 
                                new AsyncGetAllAppInfoRsp(reqToken);
                            rsp.setRaw(block);
                            rsp.setProgress(blockNo);
                            rsp.setTotal(totalNo);
                            // Send response command or append it to batch
        ...
        public boolean handleSyncFeatures(BaseCommand cmd){
            ...
            if (cmd instanceof ContactsSyncStartReq){
                ...
                ContactsSlowSyncInitRsp rsp = 
                    new ContactsSlowSyncInitRsp(reqToken);
                // Get data and fill them in response command
                rsp.setCurrentMaxId(mContactsProxy.getMaxRawContactsId());
                // Send response command or append it to batch
                onRespond(rsp);
            } else if (cmd instanceof ContactsSlowSyncAddDetailedContactsReq){
                ContactsSlowSyncAddDetailedContactsRsp rsp = 
                    new ContactsSlowSyncAddDetailedContactsRsp(reqToken);
                // Get data and fill them in response command
                // Insert new raw contacts and get their sync flags after
                byte[] detailedContactsInRaw = 
                    ((ContactsSlowSyncAddDetailedContactsReq)cmd).getRaw();
                byte[] results = mContactsProxy.slowSyncAddDetailedContacts(
                        detailedContactsInRaw);
                if (null == results){
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                } else {
                    rsp.setRaw(results);
                ...
                long contactIdLimit = 
                    ((ContactsSlowSyncGetAllRawContactsReq)cmd)
                    .getContactIdLimit();
                mContactsProxy.slowSyncGetAllRawContacts(
                        contactIdLimit,
                        new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                        ContactsSlowSyncGetAllRawContactsRsp rsp = 
                            new ContactsSlowSyncGetAllRawContactsRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                long contactIdLimit = 
                    ((ContactsSlowSyncGetAllContactDataReq)cmd)
                    .getContactIdLimit();
                mContactsProxy.slowSyncGetAllContactData(
                        contactIdLimit,
                        new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                        ContactsSlowSyncGetAllContactDataRsp rsp = 
                            new ContactsSlowSyncGetAllContactDataRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                mContactsProxy.fastSyncGetAllSyncFlags(new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                        ContactsFastSyncInitRsp rsp = 
                            new ContactsFastSyncInitRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                long[] requestedContactsIds = 
                    ((ContactsFastSyncGetRawContactsReq)cmd)
                    .getRequestedContactIds();
                mContactsProxy.fastSyncGetRawContacts(
                        requestedContactsIds,
                        new IRawBlockConsumer(){
                    
                    //@Override
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                        ContactsFastSyncGetRawContactsRsp rsp = 
                            new ContactsFastSyncGetRawContactsRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                long[] requestedContactsIds = 
                    ((ContactsFastSyncGetContactDataReq)cmd)
                    .getRequestedContactIds();
                mContactsProxy.fastSyncGetContactData(
                        requestedContactsIds,
                        new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                        ContactsFastSyncGetContactDataRsp rsp = 
                            new ContactsFastSyncGetContactDataRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                ContactsFastSyncAddDetailedContactsRsp rsp = 
                    new ContactsFastSyncAddDetailedContactsRsp(reqToken);
                // Get data and fill them in response command
                // Insert new raw contacts and get their sync flags after
                byte[] detailedContactsInRaw = 
                    ((ContactsFastSyncAddDetailedContactsReq)cmd).getRaw();
                byte[] results = mContactsProxy.fastSyncAddDetailedContacts(
                        detailedContactsInRaw);
                if (null == results){
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                } else {
                ...
                ContactsFastSyncUpdateDetailedContactsRsp rsp = 
                    new ContactsFastSyncUpdateDetailedContactsRsp(reqToken);
                // Get data and fill them in response command
                // Insert new raw contacts and get their sync flags after
                byte[] detailedContactsInRaw = 
                    ((ContactsFastSyncUpdateDetailedContactsReq)cmd).getRaw();
                byte[] results = mContactsProxy.fastSyncUpdateDetailedContacts(
                        detailedContactsInRaw);
                if (null == results){
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                } else {
                ...
                ContactsFastSyncDeleteContactsRsp rsp = 
                    new ContactsFastSyncDeleteContactsRsp(reqToken);
                // Get data and fill them in response command
                ContactsFastSyncDeleteContactsReq req = 
                    (ContactsFastSyncDeleteContactsReq)cmd;
                rsp.setDeleteCount(
                        mContactsProxy.fastDeleteContactsSourcedOnPhone(
                        req.getDeleteIds(), false));
                // Send response command or append it to batch
                onRespond(rsp);
            } else {
                return false;
        ...
        public boolean handleCalendarFeatures(BaseCommand cmd){
        	...
        	if (cmd instanceof CalendarBatchReq){
                ...
                if (insertedEventId == DatabaseRecordEntity.ID_NULL){
                    ...
                    rsp.setResult(mCalendarProxy.getEvent(insertedEventId, true, true));
                }
                // Send response command or append it to batch
                onRespond(rsp);
                ...
                rsp.setDeleteResults(mCalendarProxy.deleteEvents(req.getDeleteIds()));
                // Send response command or append it to batch
                onRespond(rsp);
        	} else if (cmd instanceof GetCalendarsReq) {
        		mCalendarProxy.getCalendars(
                        new IRawBlockConsumer(){
                ...
                int updateGroupCount = mCalendarProxy.updateEvent(req.getUpdateId(), event);
                if (updateGroupCount < 1){
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                }
                // Send response command or append it to batch
        ...
        public boolean handleCalendarSyncFeatures(BaseCommand cmd) {
            ...
            if (cmd instanceof CalendarSyncStartReq){
            	...
            	CalendarSlowSyncInitRsp rsp = 
                    new CalendarSlowSyncInitRsp(reqToken);
                // Get data and fill them in response command
                ...
                long eventIdLimit = ((CalendarSlowSyncGetAllEventsReq)cmd)
                		.getEventIdLimit();
                mCalendarProxy.slowSyncGetAllEvents(
                        eventIdLimit,
                        new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                    	CalendarSlowSyncGetAllEventsRsp rsp = 
                            new CalendarSlowSyncGetAllEventsRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
            	...
            	long eventIdLimit = ((CalendarSlowSyncGetAllRemindersReq)cmd)
        				.getEventIdLimit();
            	mCalendarProxy.slowSyncGetAllReminders(
            			eventIdLimit,
            			new IRawBlockConsumer(){
		            ...
		            public void consume(byte[] block, int blockNo, 
		                    int totalNo) {
		            	CalendarSlowSyncGetAllRemindersRsp rsp = 
		                    new CalendarSlowSyncGetAllRemindersRsp(reqToken);
		                rsp.setRaw(block);
		                rsp.setProgress(blockNo);
		                rsp.setTotal(totalNo);
		                // Send response command or append it to batch
            	...
            	long eventIdLimit = ((CalendarSlowSyncGetAllAttendeesReq)cmd)
						.getEventIdLimit();
		    	mCalendarProxy.slowSyncGetAllAttendees(
		    			eventIdLimit,
		    			new IRawBlockConsumer(){
		            ...
		            public void consume(byte[] block, int blockNo, 
		                    int totalNo) {
		            	CalendarSlowSyncGetAllAttendeesRsp rsp = 
		                    new CalendarSlowSyncGetAllAttendeesRsp(reqToken);
		                rsp.setRaw(block);
		                rsp.setProgress(blockNo);
		                rsp.setTotal(totalNo);
		                // Send response command or append it to batch
            	...
            	CalendarSlowSyncAddEventsRsp rsp = 
                    new CalendarSlowSyncAddEventsRsp(reqToken);
                // Get data and fill them in response command
                // Insert new raw contacts and get their sync flags after
                byte[] detailedEventsInRaw = 
                    ((CalendarSlowSyncAddEventsReq)cmd).getRaw();
                byte[] results = mCalendarProxy.slowSyncAddEvents(
                        detailedEventsInRaw);
                if (null == results){
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                } else {
            	...
            	mCalendarProxy.fastSyncGetAllSyncFlags(new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                        CalendarFastSyncInitRsp rsp = 
                            new CalendarFastSyncInitRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                long[] requestedEventsIds = 
                    ((CalendarFastSyncGetEventsReq)cmd)
                    .getRequestedEventIds();
                mCalendarProxy.fastSyncGetEvents(
                        requestedEventsIds,
                        new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                    	CalendarFastSyncGetEventsRsp rsp = 
                            new CalendarFastSyncGetEventsRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                long[] requestedEventsIds = 
                    ((CalendarFastSyncGetRemindersReq)cmd)
                    .getRequestedEventIds();
                mCalendarProxy.fastSyncGetReminders(
                        requestedEventsIds,
                        new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                    	CalendarFastSyncGetRemindersRsp rsp = 
                            new CalendarFastSyncGetRemindersRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
                ...
                long[] requestedEventsIds = 
                    ((CalendarFastSyncGetAttendeesReq)cmd)
                    .getRequestedEventIds();
                mCalendarProxy.fastSyncGetAttendees(
                        requestedEventsIds,
                        new IRawBlockConsumer(){
                    ...
                    public void consume(byte[] block, int blockNo, 
                            int totalNo) {
                    	CalendarFastSyncGetAttendeesRsp rsp = 
                            new CalendarFastSyncGetAttendeesRsp(reqToken);
                        rsp.setRaw(block);
                        rsp.setProgress(blockNo);
                        rsp.setTotal(totalNo);
                        // Send response command or append it to batch
            	...
            	CalendarFastSyncAddEventsRsp rsp = 
                    new CalendarFastSyncAddEventsRsp(reqToken);
                // Get data and fill them in response command
                // Insert new events and get their sync flags after
                byte[] eventsInRaw = 
                    ((CalendarFastSyncAddEventsReq)cmd).getRaw();
                byte[] results = mCalendarProxy.fastSyncAddEvents(
                        eventsInRaw);
                if (null == results){
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                } else {
            	...
            	CalendarFastSyncDeleteEventsRsp rsp = 
                    new CalendarFastSyncDeleteEventsRsp(reqToken);
                // Get data and fill them in response command
            	CalendarFastSyncDeleteEventsReq req = 
                    (CalendarFastSyncDeleteEventsReq)cmd;
                rsp.setDeleteCount(mCalendarProxy.fastDeleteEvents(req.getDeleteIds()));
                // Send response command or append it to batch
                onRespond(rsp);
            } else if (cmd instanceof CalendarFastSyncUpdateEventsReq){
            	CalendarFastSyncUpdateEventsRsp rsp = 
                    new CalendarFastSyncUpdateEventsRsp(reqToken);
                // Get data and fill them in response command
                // Insert new events and get their sync flags after
                byte[] eventsInRaw = 
                    ((CalendarFastSyncUpdateEventsReq)cmd).getRaw();
                byte[] results = mCalendarProxy.fastSyncUpdateEvents(
                        eventsInRaw);
                if (null == results){
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                } else {
        ...
        public boolean handleMediaFeatures(BaseCommand cmd){
            ...
            if (cmd instanceof GetContentDirectoriesReq){
                GetContentDirectoriesRsp rsp = 
                    new GetContentDirectoriesRsp(reqToken);
                // Get data and fill them in response command
                ...
                MediaInfo[] result = mMediaProxy.getFiles(
                        req.getRequestedContentTypes());
                if (null == result){
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                } else {
                    rsp.setFiles(result);
		...
		public boolean handleBookmarkFeatures(BaseCommand cmd) {
			...
			if (cmd instanceof AsyncGetAllBookmarkInfoReq) {
				...
				mBookmarkProxy.asynGetAllBookmarks(mBookmarkDataList, mBookmarkFolderList);
...
//				MainService.this.sendBroadcast(new Intent("com.mediatek.apst.target.data.proxy.bookmark.sync"));
//				Debugger.logI(new Object[]{cmd},"InsertBookmark_____SendBroadcastToBrowser");
				onRespond(rspCmd);
...
//				mBookmarkProxy.sendBroadcastToBrowser(MainService.this);
//				MainService.this.sendBroadcast(new Intent("com.mediatek.apst.target.data.proxy.bookmark.sync"));
//				Debugger.logI(new Object[]{cmd},"DeleteBookmark_____SendBroadcastToBrowser");
				onRespond(rspCmd);
			} else {
				return false;
			}
        ...
        public boolean handleBackupFeatures(BaseCommand cmd) {
            ...
            if (cmd instanceof GetAllGroupsForBackupReq) {
                ...
                // raw bytes of importing detailed contacts
                        ...
                        new IRawBlockConsumer() {
                            ...
                            public void consume(byte[] block, int blockNo,
                                ...
                                RestoreContactsRsp rsp = new RestoreContactsRsp(
                                        reqToken);
                                rsp.setPhase(RestoreContactsRsp.PHASE_RAW_CONTACT);
                                rsp.setRaw(block);
                                rsp.setProgress(blockNo);
                                rsp.setTotal(totalNo);
                                // Send response command or append it to batch
                        ...
                        new IRawBlockConsumer() {
                            ...
                            public void consume(byte[] block, int blockNo,
                                ...
                                RestoreContactsRsp rsp = new RestoreContactsRsp(
                                        reqToken);
                                rsp.setPhase(RestoreContactsRsp.PHASE_CONTACT_DATA);
                                rsp.setRaw(block);
                                rsp.setProgress(blockNo);
                                rsp.setTotal(totalNo);
                                // Send response command or append it to batch
                ...
                ArrayList<Long> temp = new ArrayList<Long>();
                mMulMessageOb.onSelfChangeStart();
                long[] insertedIds = mMessageProxy.importMms(req.getRaw(),
                        temp, true);
                mMulMessageOb.onSelfChangeDone();
                if (null == insertedIds) {
                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
		...
		if (req.isIsLastImport()) {
		    onRespond(rsp);
                ...
                MediaGetStorageStateRsp rsp = new MediaGetStorageStateRsp(reqToken);
...
//                ArrayList<String> oldPathsArrayTest = new ArrayList<String>();
//                ArrayList<String> newPathsArrayTest = new ArrayList<String>();
//                mMediaProxy.getFilesEndWith("/mnt/sdcard", ".jpg", oldPathsArrayTest, newPathsArrayTest);
        ...
        public Sms findSms(long date, String address, String body, int box){
            if (null != mMessageProxy){
                Sms receivedSms = mMessageProxy.findSms(date, address, body, 
                        box);
                if (null != receivedSms){
                    Debugger.logD(this.getClassName(), "findSms", 
                            new Object[]{date, address, body, box}, 
                            "id=" + receivedSms.getId() + 
                            ", date=" + receivedSms.getDate());
                ...
                Debugger.logE(this.getClassName(), "findSms", 
                        new Object[]{date, address, body, box}, 
