        ...
        Utility.runAsync(new Runnable() {
            ...
            public void run() {
                if (Logging.LOG_PERFORMANCE) {
                    Logging.d(Logging.LOG_TAG, "[Exchange Download Speed] Controller:updateMailboxList ["
...
//            }
     ...
     * Note: this method only send the filed callback and this successful calback will 
     * be sent by SmtpSender.
     * @param accountId
     * @param mailboxId
     * @param callback
     */
    public void sendMessageForBT(long accountId, Uri uri, EmailExternalCalls callback)
        ...
        try {
            ...
            Logging.w(TAG, "Error while pasring inputstream:" + e.getMessage());
            result = EmailExternalConstants.RESULT_FAIL;
        } finally {
            try {
                if (null != inputStream) {
        ...
        try {
            ...
            if (sender instanceof SmtpSender) {
                ...
                to = mimeMessage.getRecipients(com.android.emailcommon.mail.Message.RecipientType.TO);
                cc = mimeMessage.getRecipients(com.android.emailcommon.mail.Message.RecipientType.CC);
                bcc = mimeMessage.getRecipients(com.android.emailcommon.mail.Message.RecipientType.BCC);
                ...
                success = smtp.sendMailStream(fromAdr, toAdr, ccAdr, bccAdr, inputStream,
                        callback, accountId);
            ...
            Logging.d(TAG, "Send SMTP message result for result = " + result + " accountID = " + accountId + " resultType = " + resultType );
     ...
     * @param callback
     */
    public void sendMailForBT(long accountId, Uri uri, EmailExternalCalls callback, boolean saveToSent) {
        IEmailService service = getServiceForAccount(accountId);
        if (service != null) {
            // Service implementation
            try {
        ...
        if (service != null) {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    if (account == null || mailbox == null) {
                        try {
                            callback.updateCallback( EmailExternalConstants.RESULT_FAIL, accountId, mailboxId);
                        } catch (RemoteException e) {
                            // skip it
                        }
                        return;
                    }
                    mLegacyController.synchronizeMailbox(account, mailbox,
                            mLegacyListener, callback);
                }
            }.start();
        }
    }
    ...
    /* package */ void deleteMessageSyncQuickly(long accountId, long mailboxId, long messageId, 
        ...
        AttachmentUtilities.deleteAllAttachmentFiles(mProviderContext, accountId,
                messageId);
     ...
     */
    public void setMessageReadAsync(final Set<Long> messageIds, boolean isRead) {
        copySetValuesIfSearchBox(messageIds, MessageColumns.FLAG_READ, isRead);
        setMessagesBooleanValues(messageIds, EmailContent.MessageColumns.FLAG_READ, isRead);
    }
     ...
     * @param isFavorite the new value for the isFavorite flag
     */
    public void setMessageFavoriteAsync(final Set<Long> messageIds, boolean isFavorite) {
        copySetValuesIfSearchBox(messageIds, MessageColumns.FLAG_FAVORITE, isFavorite);
        setMessagesBooleanValues(messageIds, EmailContent.MessageColumns.FLAG_FAVORITE, isFavorite);
    }
    ...
    public EmailAsyncTask<Void, Void, Void> setMessagesBooleanValues(final Set<Long> messageIds,
        ...
        return EmailAsyncTask.runAsyncParallel(new Runnable() {
            ...
            public void run() {
                ...
                EmailAsyncTask.printStartLog("MessageListFragment Controller#setMessagesBoolean start at "
                        + System.currentTimeMillis());

                setMessagesBooleanSync(messageIds, columnName, columnValue);

                EmailAsyncTask.printStopLog("MessageListFragment Controller#setMessagesBoolean end and spend "
     ...
     */
    private void setMessagesBooleanSync(final Set<Long> messageIds, String columnName, boolean columnValue) {
        ...
        for (long messageId : messageIds) {
            ...
            Logging.d(Logging.LOG_TAG, "MessageListFragment setMessagesBooleanSync for messageId=" + messageId);
            ops.add(ContentProviderOperation.newUpdate(ContentUris.withAppendedId(EmailContent.Message.SYNCED_CONTENT_URI, messageId))
                    .withValues(cv)
                    .build());
            accounts.add(accountId);
        }
        try {
            ...
            // There is nothing to be done here; fail by returning null
            Logging.d(Logging.LOG_TAG, "MessageListFragment OperationApplicationException=" + e.getMessage());
        }
        
        for (long accountId : accounts) {
            if (accountId == Account.NO_ACCOUNT) {
                Logging.w(String.format("===setMessagesBooleanSync: account %d is null, so jump it ===",
                        accountId));
                continue;
            }
            if (isMessagingController(accountId)) {
...
private void copySetValueIfSearchBox(long messageId, String columnName, boolean columnValue) {
    Message message = Message.restoreMessageWithId(mProviderContext, messageId);
    if (message == null) {
        // This message in OutBox has been sent successfully and can't be found again.
    ...
    if (null != mailbox && Mailbox.TYPE_SEARCH == mailbox.mType) {
        ...
        Cursor cursor = mProviderContext.getContentResolver().query(
                Message.CONTENT_URI,
        ...
        if (cursor != null) {
            try {
                ...
                setMessageBooleanSync(messageNotInSearchBox, columnName,
                        columnValue);
                } catch (Exception e) {
                    e.printStackTrace();
                } finally {
...
private void copySetValuesIfSearchBox(final Set<Long> messageIds, final String columnName, final boolean columnValue) {
    EmailAsyncTask.runAsyncParallel(new Runnable() {
        @Override
        public void run() {
            EmailAsyncTask.printStartLog("MessageListFragment Controller#copySetValuesIfSearchBox");
            ...
            EmailAsyncTask.printStopLog("MessageListFragment Controller#copySetValuesIfSearchBox");
        }});
        ...
        if (service != null) {
            ...
            try {
                ...
                if (service instanceof EmailServiceProxy) {
                    ((EmailServiceProxy)service).setTimeout(180);
                }
                searchResultCount = service.searchMessages(accountId, searchParams, searchMailboxId);
            } catch (RemoteException e) {
                // TODO Change exception handling to be consistent with however this method
                // is implemented for other protocols
            ...
            searchResultCount = mLegacyController.searchMailbox(accountId, searchParams, searchMailboxId);
        }
        if (Email.DEBUG) {
     ...
     * Delete an account.
     */
    public void deleteAccount(final long accountId, final AccountSettings.RemoveAccountCallback callback) {
        EmailAsyncTask.runAsyncParallel(new Runnable() {
            @Override
            public void run() {
    ...
    public class MessageRetrievalListenerBridge implements MessageRetrievalListener {
        ...
        public void loadAttachmentProgress(int progress) {
            // Do not feed back the progress to UI if the attachment downloading was canceled.
            ...
            synchronized (mListeners) {
                for (Result listener : mListeners) {
                    listener.loadAttachmentCallback(null, mAccountId, mMessageId, 
                            mAttachmentId, progress);
            }
          }
        }
    ...
    public class LegacyListener extends MessagingListener {
        ...
        public void listFoldersFailed(long accountId, String message) {
            Logging.d("LegacyListener::listFoldersFailed accountId = " + accountId+" message = " + message);
            synchronized (mListeners) {
                for (Result l : mListeners) {
        ...
        public void synchronizeMailboxStarted(long accountId, long mailboxId) {
            Logging.d("LegacyListener::synchronizeMailboxStarted accountId = "+ accountId + " mailboxId = " + mailboxId);
            synchronized (mListeners) {
                for (Result l : mListeners) {
        ...
        @Override
        public void synchronizeEnvelopeFinished(long accountId, long mailboxId) {
            Logging.d("LegacyListener::synchronizeEnvelopeFinished accountId = "+ accountId + " mailboxId = " + mailboxId);
            synchronized(mListeners) {
                for (Result l : mListeners) {
        ...
        public void synchronizeMailboxFinished(long accountId, long mailboxId,
            ...
            Logging.d("LegacyListener::synchronizeMailboxFinished accountId = "+ accountId + " mailboxId = " + mailboxId);
            synchronized (mListeners) {
                for (Result l : mListeners) {
        ...
        public void synchronizeMailboxFailed(long accountId, long mailboxId, Exception e) {
            Logging.w("LegacyListener::synchronizeMailboxFailed accountId = "
                    + accountId + " mailboxId = " + mailboxId + " Exception = " + (e == null ? "null" : e.toString()));
        ...
        public void loadAttachmentFailed(long accountId, long messageId, long attachmentId,
            ...
            Logging.d("LegacyListener::loadAttachmentFailed accountId = " + accountId 
                    + ", messageId = " + messageId + ", attachmentId = " + attachmentId
                    + ", background = " + background + ", MessagingException = " + (me == null ? null : me.getMessage()));
            try {
        ...
        synchronized public void sendPendingMessagesFailed(long accountId, long messageId,
            ...
            Logging.d("LegacyListener::sendPendingMessagesFailed accountId = " + accountId 
                    + ", messageId = " + messageId 
                    + ", Exception = " + (reason == null ? null : reason.getMessage()));
            MessagingException me = null;
    ...
    private class ServiceCallback extends IEmailServiceCallback.Stub {
        ...
        public void sendMessageStatus(long messageId, String subject, int statusCode,
            ...
            Logging.d("ServiceCallback::sendMessageStatus messageId = " + messageId 
                    + ", subject = " + subject + ", statusCode = " + statusCode 
                    + ", progress = " + progress);
        ...
        public void fetchMessageCallBack(long messageId, int statusCode, int progress) {
            MessagingException result = mapStatusToException(statusCode);
            if (Email.DEBUG) {
                Logging.d(Logging.LOG_TAG, "+++ fetchMailCallBack messageId =" + messageId + " statusCode="
    ...
    public static class ControllerService extends Service {
        ...
        private final IEmailService.Stub mBinder = new IEmailService.Stub() {
            ...
            public void cancelLoadAttachment(long attachmentId) {
                Attachment att = Attachment.restoreAttachmentWithId(ControllerService.this, attachmentId);
                if (att != null) {
            ...
            @Override
            public void sendMessageForBT(long accountId, long mailboxId,
                    Uri uri, EmailExternalCalls callback, boolean saveToSent)
                    throws RemoteException {
            }

            @Override
            public void startSyncForBT(long mailboxId,
                    EmailExternalCalls callback) throws RemoteException {
