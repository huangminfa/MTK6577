...
public class ViaGpsProcess extends Handler {
    ...
    /*indicator for Datacall status from ACTION_ANY_DATA_CONNECTION_STATE_CHANGED*/
    private String mDataConnectionState;
    
    public ViaGpsProcess(Context context, CommandsInterface ci,int simId) {
        mContext = context;
        mCM = ci;
        mSimId = simId;
        mConnectivityManager = (ConnectivityManager) context.getSystemService(Context.CONNECTIVITY_SERVICE);
    }
    public ViaGpsProcess(Context context, CommandsInterface ci) {
        this(context,ci,Phone.GEMINI_SIM_2);
    }
    public void start(){
    ...
    public void stop(){
        ...
        mCM.requestAGPSTcpConnected(0, null);
    ...
    private void startGpsWapService() {
        ...
        if("CONNECTING".equals(mDataConnectionState)) {
        ...
        if(mDataCallState != DATACALL_CONNECTED){
            Log.d(LOG_TAG,"[VIA] startGpsWapService() requestAGPSTcpConnected("+mDataCallState+")");
            mCM.requestAGPSTcpConnected(mDataCallState, null);
        }else{
            int result;
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (result == Phone.APN_ALREADY_ACTIVE) {
                mCM.requestAGPSTcpConnected(1, null);
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION)) {
               NetworkInfo info =(NetworkInfo)intent.getExtra(ConnectivityManager.EXTRA_NETWORK_INFO);                NetworkInfo.
              ...
              if(detailedState == NetworkInfo.DetailedState.CONNECTED){
                ...
                if(mGpsWapSrvStatus == GPS_WAP_SERVICE_STATUS_STOPPING &&
                   mGpsWapConnConnected){
                    Log.d(LOG_TAG,"[VIA] requestAGPSTcpConnected(0)");
                    mCM.requestAGPSTcpConnected(0, null);
                    mGpsWapSrvStatus = GPS_WAP_SERVICE_STATUS_NO_OPT;
                    mGpsWapConnConnected = false;
                }
                ...
                mDataConnectionState = intent.getStringExtra(Phone.STATE_KEY);
                Log.d(LOG_TAG, "[VIA] New DataCall Connection Status = " + mDataConnectionState);
