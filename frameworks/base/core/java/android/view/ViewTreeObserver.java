...
public final class ViewTreeObserver {
    ...
    private CopyOnWriteArrayList<OnPostDrawListener> mOnPostDrawListeners;
    private CopyOnWriteArrayList<OnViewRootScrollChangedListener> mOnViewRootScrollChangedListeners;
    ...
    /**
     * Interface definition for a callback to be invoked after the view tree has finished drawing.
     * @hide
     */
    ...
    /**
     * Interface definition for a callback to be invoked when
     * ViewRoot has been scrolled.
     * @hide
     */
    public interface OnViewRootScrollChangedListener {
    	/**
         * Callback method to be invoked when ViewRoot has been scrolled.
         */
        public void onScrollChanged();
    }
    ...
    /**
     * Register a callback to be invoked when the view tree has finished drawing
     *
     * @param listener The callback to add
     *
     * @throws IllegalStateException If {@link #isAlive()} returns false
    ...
    public void addOnPostDrawListener(OnPostDrawListener listener) {
        ...
        if (mOnPostDrawListeners == null) {
            mOnPostDrawListeners = new CopyOnWriteArrayList<OnPostDrawListener>();
        }
    ...
    /**
     * Remove a previously installed post-draw callback
     *
     * @param victim The callback to remove
     *
     * @throws IllegalStateException If {@link #isAlive()} returns false
    ...
    /**
     * Register a callback to be invoked when ViewRoot has been scrolled.
     *
     * @param listener The callback to add
     *
     * @throws IllegalStateException If {@link #isAlive()} returns false
     * @hide
     */
    public void addOnViewRootScrollChangedListener(OnViewRootScrollChangedListener listener) {
        checkIsAlive();

        if (mOnViewRootScrollChangedListeners == null) {
            mOnViewRootScrollChangedListeners = new CopyOnWriteArrayList<OnViewRootScrollChangedListener>();
        }

        mOnViewRootScrollChangedListeners.add(listener);
    }

    /**
     * Remove a previously installed ViewRoot scroll-changed callback
     *
     * @param victim The callback to remove
     *
     * @throws IllegalStateException If {@link #isAlive()} returns false
     *
     * @see #addOnViewRootScrollChangedListener(OnViewRootScrollChangedListener)
     * @hide
     */
    public void removeOnViewRootScrollChangedListener(OnViewRootScrollChangedListener victim) {
        checkIsAlive();
        if (mOnViewRootScrollChangedListeners == null) {
            return;
        }
        mOnViewRootScrollChangedListeners.remove(victim);
    }
    ...
    /**
     * Notifies registered listeners that the drawing pass has finished. If a
     * listener returns true, then indicate the post draw action finished normally. This can
     * be called manually if you want do some thing after ViewRoot finished drawing, such as
     * using it for sync between 2d and 3d drawing thread.
     *
    ...
    public final boolean dispatchOnPostDraw() {
        ...
        boolean cancelDraw = false;
        final CopyOnWriteArrayList<OnPostDrawListener> listeners = mOnPostDrawListeners;
        if (listeners != null) {
    ...
    /**
     * Notifies registered listeners that ViewRoot has been scrolled.
     */
    final void dispatchOnViewRootScrollChanged() {
        // NOTE: because of the use of CopyOnWriteArrayList, we *must* use an iterator to
        // perform the dispatching. The iterator is a safe guard against listeners that
        // could mutate the list by calling the various add/remove methods. This prevents
        // the array from being modified while we iterate it.
        final CopyOnWriteArrayList<OnViewRootScrollChangedListener> listeners = mOnViewRootScrollChangedListeners;
        if (listeners != null) {
            for (OnViewRootScrollChangedListener listener : listeners) {
                listener.onScrollChanged();
            }
        }
    }
