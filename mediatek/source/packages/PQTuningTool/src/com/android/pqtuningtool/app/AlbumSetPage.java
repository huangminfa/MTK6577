...
import  com.android.pqtuningtool.data.LocalAlbum;
...
import  com.android.pqtuningtool.settings.GallerySettings;
import  com.android.pqtuningtool.ui.ActionModeHandler;
import  com.android.pqtuningtool.ui.ActionModeHandler.ActionModeListener;
...
import  com.android.pqtuningtool.ui.GridDrawer;
import  com.android.pqtuningtool.ui.HighlightDrawer;
import  com.android.pqtuningtool.ui.PositionProvider;
...
public class AlbumSetPage extends ActivityState implements
    ...
    public void onSingleTapUp(int slotIndex) {
        ...
        MtkLog.i(TAG, "slotIndex is " + slotIndex);

        if (mShowDetails) {
            Path path = targetSet.getPath();
            mHighlightDrawer.setHighlightItem(path);
            MtkLog.i(TAG, " mDetailsHelper  reloadDetails ");
            mDetailsHelper.reloadDetails(slotIndex);
        } else if (!mSelectionManager.inSelectionMode()) {
            MtkLog.i(TAG, " mSelectionManager isnot inSelectionMode ");
            ...
            if (mGetAlbum && targetSet.isLeafAlbum()) {
                ...
                if (MediatekFeature.hasCustomizedForVLW() && targetSet instanceof LocalAlbum) {
            ...
            MtkLog.i(TAG, " showdetails is  " + mShowDetails);
            mSelectionManager.toggle(targetSet.getPath());
            mDetailsSource.findIndex(slotIndex);  //junwen 
    ...
    private void onDown(int index) {
        ...
        //mDetailsSource.findIndex(index);  //junwen 
        mAlbumSetView.invalidate();
    }
    ...
    private void initializeData(Bundle data) {
        ...
        if (mIsDrmSupported) {
            ...
            mMediaSet = mActivity.getDataManager().getMediaSet(mediaPath,mDrmInclusion);
        } else {
    ...
    private void showDetails() {
        mShowDetails = true;
        MtkLog.i(TAG, "showDetails");
        if (mDetailsHelper == null) {
            mHighlightDrawer = new HighlightDrawer(mActivity.getAndroidContext(),
                    mSelectionManager);
            mDetailsHelper = new DetailsHelper(mActivity, mRootPane, mDetailsSource);
            MtkLog.i(TAG, "mDetailSource  size is "  + mDetailsSource.getIndex());
    ...
    private class MyDetailsSource implements DetailsHelper.DetailsSource {
        private int mIndex;
        private Set<Integer> mClickedIndex = new HashSet<Integer>();
        public int size() {
        ...
        public int findIndex(int indexHint) {
            if (mAlbumSetDataAdapter.isActive(indexHint)) {
                //mIndex = indexHint;
                if (mClickedIndex.contains(indexHint)) {
                    mClickedIndex.remove(indexHint);
                } else {
                    mClickedIndex.add(indexHint);
                }
                if (mClickedIndex.size() > 0) {
                    mIndex = mClickedIndex.iterator().next();
                    MtkLog.i(TAG, "MyDetailsSource  mIndex is "  + mIndex);
                }
            } else {
