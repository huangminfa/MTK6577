...
private:
    ...
    status_t readNextChunk(uint8_t* data, int size, int offset = 0);
...
status_t MtkAVIExtractor::parseDataChunk(int32_t pos, int ID, int size, int sync) {
    ...
    switch(type) {
        ...
        case BFOURCC(0,0,'p','c'):
            ...
            break;
        default:
            LOGW("skip chunk at pos %x " FORMATFOURCC, pos, PRINTFOURCC(ID));
    }
    return OK;
}
...
status_t MtkAVISource::readNextChunk(uint8_t* data, int size, int offset) {
...
status_t MtkAVISource::read(
    ...
    *out = NULL;
    ...
    do{
        ...
        int bitrate;
        if (start >= 0 && get_mp3_frame_size(header, (size_t*)&frameSize, NULL, NULL, &bitrate)) {
            LOGV("mp3 start %d header %x frameSize %d length %d bitrate %d", 
                    start, header, frameSize, length, bitrate);
            if (frameSize + start > length) {
                // try to get data from next chunk, ugly code ..
                MediaBuffer *tmp;
                CHECK(mGroup->acquire_buffer(&tmp) == OK);
                int needSize = frameSize + start - length;
                int existSize = length - start;

                if (readNextChunk((uint8_t*)tmp->data() + existSize, needSize) == OK) {
                    memcpy(tmp->data(), 
                            (uint8_t*)mBuffer->data() + mBuffer->range_offset() + start, existSize);
                    mBuffer->release();
                    mBuffer = NULL;
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_AVC)
        ...
        if (mSampleSizes.size() > 0) {
            ...
            if (!found) {
                ...
                int s = strfData->size() > 256 ?  256 : strfData->size();
