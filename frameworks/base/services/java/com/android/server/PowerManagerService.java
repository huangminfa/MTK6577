...
public class PowerManagerService extends IPowerManager.Stub
    ...
    private static final int PROXIMITY_SENSOR_DELAY = 200;  //500;   //1000;
    ...
    */
    ...
    private static boolean FEATURE_AUTO_BRIGHTNESS_DECREASE = false;  //backlight can increase or decrease 
    private boolean mIsLowBattery = false;
    private static boolean mShutdownConfig = false;
	private boolean mShutdownFlag = false;  //wanghow

	private int mOldBacklightValue = 0;  //wanghow
	private TvOut mTvOut = new TvOut(); // add for TV-out power control
	private HDMINative mHDMI = new HDMINative();	// add for HDMI power control

    private static final boolean WAKELOCK_DUMP_ENABLED = true;
    ...
    */
            ...
            synchronized (mLocks) {
                ...
                if (mUseSoftwareAutoBrightness && FEATURE_DISABLE_LS_ENABLED && mIsLowBattery != wasLowBattery && mDoneBooting) {
                    ...
                    if ((mPowerState & SCREEN_ON_BIT) != 0 && mAutoBrightessEnabled) {
                        enableLightSensorLocked(!mIsLowBattery);	
                        if (mIsLowBattery) {
    ...
    void initInThread() {
       ...
       ContentResolver resolver = mContext.getContentResolver();
        ...
        // MTK_OP01_PROTECT_START		   
            if(cust.equals("OP01")){
                FEATURE_AUTO_BRIGHTNESS_DECREASE = true;
            }
    ...
    public void acquireWakeLockLocked(int flags, IBinder lock, int uid, int pid, String tag,
        ...
        if (index < 0) {
            ...
            diffsource = false;
            oldsource = null;
            wl.active_since = SystemClock.uptimeMillis();
        } else {
            wl = mLocks.get(index);
            newlock = false;
            oldsource = wl.ws;
    ...
    private void releaseWakeLockLocked(IBinder lock, int flags, boolean death) {
        ...
        if (wl.activated) {
            wl.total_time += SystemClock.uptimeMillis() - wl.active_since;
        }

        if (mSpew) {
            Slog.d(TAG, "releaseWakeLock flags=0x"
                    + Integer.toHexString(wl.flags) + " tag=" + wl.tag + " total_time=" + wl.total_time + "ms");
        }
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        synchronized (mLocks) {
            ...
            for (int i=0; i<N; i++) {
                ...
                String activated = "";
                long total_time = wl.total_time;
                if (wl.activated) {
                    activated = " activated";
                    total_time += SystemClock.uptimeMillis() - wl.active_since;
                }
                pw.println("  " + type + " '" + wl.tag + "'" + acquireCausesWakeup
                        + activated + " (minState=" + wl.minState + ", uid=" + wl.uid
                        + ", pid=" + wl.pid + " total=" + total_time +  "ms)");
            }
    ...
    private void dumpWakeLockLocked() {
        if (WAKELOCK_DUMP_ENABLED) {
            int N = mLocks.size();
            Slog.d(TAG, "wakelock list dump: mLocks.size=" + N + ":");
            for (int i=0; i<N; i++) {
                WakeLock wl = mLocks.get(i);
                String type = lockType(wl.flags & LOCK_MASK);
                String activated = "";
                ...
                Slog.d(TAG, "No." + i + ": " + type + " '" + wl.tag + "'"
                    + activated + "(flags=" + wl.flags + ", minState=" + wl.minState 
                    + ", uid=" + wl.uid + ", pid=" + wl.pid + " total=" + total_time +  "ms)");
            }
        }
    } 
    ...
    private void sendNotificationLocked(boolean on, int why) {
        ...
        if ( mSkippedScreenOn && !inPowerOff ) {
            ...
            if (oldScreenOn != newScreenOn) {
                if (newScreenOn) {
                    ...
                    if (!mScreenBrightness.animating) {
                        if(!KeyguardViewMediator.isKeyguardEnabled() || mLockPatternUtils.isLockScreenDisabled())//turn screen off in default flow, only when keyguard is disabled 
                        {
                             clearLightSensorValue();
    ...
    private void updateLightsLocked(int newState, int forceState) {
        ...
        if ((difference & (SCREEN_ON_BIT | SCREEN_BRIGHT_BIT)) != 0) {
            ...
            if (!mSkippedScreenOn) {
                if ( (mMaxBrightness != -1) && (mMaxBrightness < brightness) )
    ...
    private void setLightBrightness(int mask, int value) {
        ...
        if ((mask & SCREEN_BRIGHT_BIT) != 0) {
           ...
           //if(value == Power.BRIGHTNESS_OFF){
            ...
            mOldBacklightValue = value;  //add by how.wang
        }
        ...
        private void finishAnimationLocked(boolean more, int curIntValue) {
            ...
            if (!more) {
                if (mask == SCREEN_BRIGHT_BIT && curIntValue == Power.BRIGHTNESS_OFF) {
                    clearLightSensorValue();
                    if((!KeyguardViewMediator.isKeyguardEnabled()) || (mLockPatternUtils.isLockScreenDisabled()) || (mScreenOffReason == WindowManagerPolicy.OFF_BECAUSE_OF_PROX_SENSOR))
                    {
        ...
        if (mLightSensorValue != value) {
            ...
            if ((mPowerState & BATTERY_LOW_BIT) == 0) {
                ...
                int lcdValue = getAutoBrightnessValue(
                        ( (mIsDocked || FEATURE_AUTO_BRIGHTNESS_DECREASE) ? value : mHighestLightSensorValue),
                        mLcdBacklightValues);
                ...
                if (mAutoBrightessEnabled && mScreenBrightnessOverride < 0) {
                    if (mAnimateScreenLights && !mSkippedScreenOn) 
                    {
    ...
    private void setScreenBrightnessMode(int mode) {
        ...
        synchronized (mLocks) {
            ...
            if (mUseSoftwareAutoBrightness && mAutoBrightessEnabled != enabled) {
        	    ...
        	    if (FEATURE_DISABLE_LS_ENABLED && !mIsLowBattery && mBootCompleted) { 
        	    	if (mDebugLightSensor) {
    ...
    void systemReady() {
        ...
        synchronized (mLocks) {
            ...
            if (!FEATURE_DISABLE_LS_ENABLED || (mAutoBrightessEnabled && !mIsLowBattery)) {
                enableLightSensorLocked(mUseSoftwareAutoBrightness && (mAutoBrightessEnabled && !mIsLowBattery));
            }
    ...
    public void setBacklightBrightnessOff(boolean enable) {
        ...
        if(mShutdownFlag) {
            ...
            synchronized (mLocks) {
                ...
                mLcdLight.setBrightness(Power.BRIGHTNESS_OFF);
                mButtonLight.setBrightness(Power.BRIGHTNESS_OFF);
                mKeyboardLight.setBrightness(Power.BRIGHTNESS_OFF);
                mPowerState &= ~(SCREEN_BRIGHT_BIT | BUTTON_BRIGHT_BIT | KEYBOARD_BRIGHT_BIT);
                long identity = Binder.clearCallingIdentity();
                try {
                    mBatteryStats.noteScreenBrightness(Power.BRIGHTNESS_OFF);
                } catch (RemoteException e) {
    ...
    public void setMaxBrightness(int brightness) {  //wanghow
        synchronized (mLocks) {
            mMaxBrightness = brightness;
            Slog.d(TAG, "setMaxBrightness brightness=" + brightness);
            if (isScreenOn()) {
                Slog.d(TAG, "setMaxBrightness updateLightsLocked mPowerState=" + mPowerState);
                updateLightsLocked(mPowerState, SCREEN_ON_BIT);
            }
        }
    }
    ...
    SensorEventListener mLightListener = new SensorEventListener() {
        public void onSensorChanged(SensorEvent event) {
            synchronized (mLocks) {
                ...
                if( (value > mHighestLightSensorValue) || FEATURE_AUTO_BRIGHTNESS_DECREASE )
                {
    ...
    private void getDynamicWakeLockLocked(WakeLock wl, boolean isAcquire) {
        ...
        if (isAcquire)	// acquire a WakeLock	
            ...
            mWakeLockBGDbgModeResult += "\nReleased: TAG = " + wl.tag + "(" + wl.total_time + "ms)";
    }
    ...
    public String debugWakeLock() { 
        ...
        synchronized (mLocks) {
            ...
            for (int i=0; i<N; i++) {
                ...
                result += "\nNo." + (i+1) + ": TAG = " + wl.tag + " , FLAG = " + getWakeLockFlagLocked(wl) + "(" + total_time + "ms)";
            }
        }
