...
// ----- MTK begins ----- //
#define NFC_P2PI_BITRATE_DISABLE106 (phNfc_ePassive212 | phNfc_ePassive424 | phNfc_eActive424)
static int s_p2pi_disable_bitrate106 = 0;
// ----- MTK ends ----- //
...
static short get_p2p_mode() {
	...
	// ----- MTK begins ----- //
	if (s_p2pi_disable_bitrate106) {
		return NFC_P2PI_BITRATE_DISABLE106;
	}
	// ----- MTK ends ----- //
    if (value[0]) {
        short mode;
...
static jboolean com_android_nfc_NfcManager_isUiccSecureElementPresence(JNIEnv *e, jobject o) {
	...
	TRACE("******  com_android_nfc_NfcManager_isUiccSecureElementPresense() ******");	
	TRACE("phLibNfc_SE_GetSecureElementList()");
	REENTRANCE_LOCK();
	ret = phLibNfc_SE_GetSecureElementList(se_list, &se_count);
	REENTRANCE_UNLOCK();
...
static void com_android_nfc_NfcManager_doSelectUiccSecureElement(JNIEnv *e, jobject o, jint select) {
	...
	if (isSelected == select) {
		LOGD("[BLAH] UICC SE has the same configure");
		goto clean_and_return;
	}

    TRACE("****** com_android_nfc_NfcManager_doSelectUiccSecureElement ******");
    REENTRANCE_LOCK();
	if (select) {
	    TRACE("phLibNfc_SE_SetMode() as UICC/ActModeVirtual");
    	ret = phLibNfc_SE_SetMode(0xABCDF0, phLibNfc_SE_ActModeVirtual, nfc_jni_se_set_mode_callback,(void *)&cb_data);
	} else {
		TRACE("phLibNfc_SE_SetMode() as UICC/ActModeOff");
		ret = phLibNfc_SE_SetMode(0xABCDF0, phLibNfc_SE_ActModeOff, nfc_jni_se_set_mode_callback,(void *)&cb_data);
	}
    REENTRANCE_UNLOCK();
    if (ret != NFCSTATUS_PENDING) {
        LOGD("phLibNfc_SE_SetMode() returned 0x%04x[%s]", ret, nfc_jni_get_status_name(ret));
...
static void com_android_nfc_NfcManager_doSelectUiccSecureElementInPBTF(JNIEnv *e, jobject o, jint select) {
    ...
    struct nfc_jni_native_data *nat;
    struct nfc_jni_callback_data cb_data;
	uint8_t SESetValue[4] = {0x00, 0x98, 0x00};
	...
	if (isSelected == select) {
		LOGD("[BLAH] UICC SE in PBTF has the same configure");
		goto clean_and_return;
	}
	
    gInputParam.buffer = SESetValue;
    gInputParam.length = 0x04;
    gOutputParam.buffer = Output_Buff;

	if (select) {
		LOGD("[BLAH] Set PBTF mode to UICC");
		SESetValue[3] = 0x0F;
	} else {
		LOGD("[BLAH] Set PBTF mode to None");
		SESetValue[3] = 0x3F;
	}

    TRACE("phLibNfc_Mgt_IoCtl() : set the default PBTF mode SE to UICC");
    REENTRANCE_LOCK();
    ret = phLibNfc_Mgt_IoCtl(gHWRef,NFC_MEM_WRITE,&gInputParam, &gOutputParam,nfc_jni_ioctl_callback, (void *)&cb_data);
    REENTRANCE_UNLOCK();
    if(ret!=NFCSTATUS_PENDING) {
...
static void com_android_nfc_NfcManager_doSelectP2pBitrate106(JNIEnv *e, jobject o, jint select) {
	CONCURRENCY_LOCK();
	if (select) {
		s_p2pi_disable_bitrate106 = 1;
	} else {
		s_p2pi_disable_bitrate106 = 0;
	}
	CONCURRENCY_UNLOCK();
}
// ----- MTK ends ----- //
