...
public class StkAppService extends Service implements Runnable {
        ...
        case SET_UP_MENU:
            ...
            if (isOrangeSupport()) {
                ...
                if (null == appName || appName.isEmpty() || 0 == appName.length()) {
                    ...
                    // TODO: read SIM EF(Elementary File) SUME(Setup Menu Entity) Address 6F54
            ...
            if ((mBrowserSettings!=null)                
                ...
                switch(mBrowserSettings.mode){
                    case LAUNCH_IF_NOT_ALREADY_LAUNCHED : 
                        CatLog.d(this, "[Handle Command][LAUNCH_BROWSER][Should not launch browser]");
                        CatResponseMessage resMsg = new CatResponseMessage(mCurrentCmd);
                        launchBrowser = false;
    ...
    private void processAbnormalNoCallResponse() {
        ...
        CatLog.d(this, "Abnormal No Call Response PROCESS end! cmdName[" + mCurrentCmd.getCmdType().name() + "]");
        CatResponseMessage resMsg = new CatResponseMessage(mCurrentCmd);
        resMsg.setResultCode(ResultCode.TERMINAL_CRNTLY_UNABLE_TO_PROCESS);
        mStkService.onCmdResponse(resMsg);
    }
    ...
    private void processNormalOpenChannelResponse() {
        ...
        if (mCurrentCmd.getCmdType() != null){
            CatLog.d(this, "Normal OpenChannel Response PROCESS end! cmdName[" + mCurrentCmd.getCmdType().name() + "]");    
        }
        CatResponseMessage resMsg = new CatResponseMessage(mCurrentCmd);
        resMsg.setResultCode(ResultCode.OK);
        resMsg.setConfirmation(true);
    ...
    private void processAbnormalOpenChannelResponse() {
        ...
        if (mCurrentCmd.getCmdType() != null){
            CatLog.d(this, "Abnormal OpenChannel Response PROCESS end! cmdName[" + mCurrentCmd.getCmdType().name() + "]");  
        }
        CatResponseMessage resMsg = new CatResponseMessage(mCurrentCmd);
        resMsg.setResultCode(ResultCode.TERMINAL_CRNTLY_UNABLE_TO_PROCESS);
        mStkService.onCmdResponse(resMsg);
    ...
    private boolean canShowTextDialog(TextMessage msg) {
        ...
        CatLog.d(this, "canShowTextDialog? mMenuIsVisible = " + mMenuIsVisible + " mInputIsVisible = " + " mDialogIsVisible = " + mDialogIsVisible); 
        ...
        public void onReceive(Context context, Intent intent) {
            if (TelephonyIntents.ACTION_SIM_STATE_CHANGED.equals(intent.getAction())) {
                ...
                if (IccCard.INTENT_VALUE_ICC_NOT_READY.equals(simState)) {
                    CatLog.d(this, "mSIMStateChangeReceiver() - unReg stk IDLE_MODE_TEXT notification");
                    mNotificationManager.cancel(STK_NOTIFICATION_ID);
                    }
