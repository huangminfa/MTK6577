...
public class BluetoothPbapPath {
    ...
    public static final int PBAP_PATH_TO_ROOT = 2;        // Forward to specified child folder
    ...
    public static String validPath[] = {
       ...
       ,"SIM1/telecom/pb"
       //,"SIM1/telecom/ich", "SIM1/telecom/och", "SIM1/telecom/mch", "SIM1/telecom/cch"
    };
    ...
    public int getPathType(String path, boolean relative) {
        ...
        for(i = 0;i < size;i++) {
            if(path.equalsIgnoreCase(validPath[i]))
                return i;
        }
    ...
    public boolean setPath(int op, String path) {
        ...
        case PBAP_PATH_FORWARD:
            path = formatPath(path);
            if(TextUtils.isEmpty(mCurrentPath) == false)
                path = mCurrentPath + "/" + path;
    ...
    private boolean isValidPath(String path) {
        ...
        for(i = 0;i < size;i++) {
            vp = validPath[i];
            if(length <= vp.length() && 
                path.equalsIgnoreCase(vp.substring(0,length)) &&
