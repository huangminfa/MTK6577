...
int mmi_command_hdlr(char* line){
    ...
    LOGD("mmi_command_hdlr :%s", line);
    
    for(i = 0; i < table_size;i++){
        LOGD("mmi_command_hdlr :%s", mmi_cmd_table[i].cmdName);
        if(strcmp(line_cut, mmi_cmd_table[i].cmdName) == 0){
           strcpy(mmiCmd, line);
           ATOP_t at_op = getAtOp(&line);
           LOGD("The command op is %d; Support op is %d", at_op, mmi_cmd_table[i].opType);
           ...
           if(s_fdAtci_mmi_command > 0){
                ...
                LOGE("The socket connection to framework is not alive");
...
int generic_command_hdlr(char* line){
    ...
    for(i = 0; i < table_size;i++){
        if(strcmp(line_cut, generic_cmd_table[i].cmdName) == 0){
            ATOP_t at_op = getAtOp(&line);
            LOGD("The command op is %d; Support op is %d", at_op, generic_cmd_table[i].opType);
            ...
            if(dataLen >= 0){
               ...
               if(sendLen != dataLen){
                ...
                LOGD("NULL response from command handler function:%s", generic_cmd_table[i].cmdName);
            }
            free(line_cut); 
