...
static EGLBoolean egl_init_drivers_locked() {
    ...
    if (fp) {
        fread(acAppName, 1, sizeof(acAppName), fp);
        fclose(fp);
    } else {
        sprintf(acAppName, "unknown");
    }
    ...
    if (cnx->dso == 0) {
        const char* pSkip = "com.aurorasoftworks.quadrant.ui";
        int len = strlen(pSkip);
        cnx->hooks[GLESv1_INDEX] = &gHooks[GLESv1_INDEX][IMPL_SOFTWARE];
        cnx->hooks[GLESv2_INDEX] = &gHooks[GLESv2_INDEX][IMPL_SOFTWARE];
        if (strncmp(acAppName, pSkip, len) != 0) {
