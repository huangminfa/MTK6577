...
bool AudioYusuVolumeController::setVoiceVolume(float volume, uint32 mode,uint32 routes)  // volume should range 0.0 ~ 1.0
{
    ...
    if(GetDevice == ROUTE_OUT_EARPIECE)
    {
        ...
        if (mAudioHardware->GetFlag_SecondModemPhoneCall()!=0){
            mAudioHardware->pLad->LAD_SetOutputVolume(Lad_Volume,GetDevice);
        }else
#endif
        {
            mAudioHardware->pLad->LAD_SetOutputVolume(Lad_Volume);
        ...
        ||GetDevice == ROUTE_OUT_WIRED_HEADPHONE)  // adjust for analog gain
    {
        ...
        if (mAudioHardware->GetFlag_SecondModemPhoneCall()!=0){
            ...
            //EVDO's volume is adjusted in MD side, AP side only set 0dB line-in path gain.
            Lad_Volume= CalVoicegain(MapVolume,mode,routes); // calculate in_call mode volume
            mAudioHardware->pLad->LAD_SetOutputVolume(Lad_Volume,GetDevice);
            ApplyGainInCall(208/*0dB*/, mode, routes);
            #else
            //6252's volume is adjusted in AP side, MD side only set fixed gain.
...
// ToDo
   ...
   LOGD("!!MT66xx, SetFmVolume(%d)",volume);
   ...
   if(mfmFd > 0 )
   {
      ...
      if(volume != 0){
         uint32_t mute = false;
         mFmMute = false;
         ret = ::ioctl(mfmFd,FM_IOCTL_MUTE,(uint32_t*)&mute);
      }
