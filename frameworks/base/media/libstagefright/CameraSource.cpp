...
status_t CameraSource::initWithCameraAccess(
	...
	if (!mCamera->previewEnabled()) {//Camera return valid memory info after startPreview
		LOGW("Start preview in CameraSource");
	...
	LOGD("Camera Memory Info: VA=%d, Size=%d, Count=%d", (int32_t)memInfo.u4VABase, (int32_t)memInfo.u4MemSize, (int32_t)memInfo.u4MemCount);
#endif
    return OK;
}
...
void CameraSource::startCameraRecording() {
    ...
    if (mCameraFlags & FLAGS_HOT_CAMERA) {
		...
		LOGD("Proxy start recording %d", err);
		if (err == NO_INIT) {
			mCameraFlags &= ~FLAGS_HOT_CAMERA;
			mCamera->lock();
			mCamera->reconnect();
...
void CameraSource::dataCallbackTimestamp(int64_t timestampUs,
    ...
    if (!mStarted || (mNumFramesReceived == 0 && timestampUs < mStartTimeUs)) {
        LOGV("Drop frame at %lld/%lld us, mStarted=%d", timestampUs, mStartTimeUs, mStarted);
    ...
    if (mNumFramesReceived > 0) {
		...
		{
			LOGW("[CameraSource][dataCallbackTimestamp][Warning] current frame timestamp: %lld <= previous frame timestamp: %lld",
