...
public class MiniThumbFile {
    ...
    /**
     * Get the real path for thumbdata.
    ...
    public static String getThumbdataPath(Uri uri) {
        ...
        if (LOG) Log.v(TAG, "getThumbdataPath(" + uri + ") return " + path);
        return path;
    }
    ...
    public synchronized void deactivate() {
    	if (LOG) Log.v(TAG, "deactivate() begin");
        ...
        mChannels.clear();
        if (LOG) Log.v(TAG, "deactivate() end");
    }
    ...
    public synchronized void saveMiniThumbToFile(byte[] data, long id, long magic)
        ...
        try {
            if (data != null) {
                ...
                if (LOG) Log.v(TAG, "saveMiniThumbToFile(" + id + ") flag=1, magic="
    ...
    public synchronized byte[] getMiniThumbFromFile(long id, byte [] data, ThumbResult result) {
        ...
        try {
            ...
            if (size > 1 + 8 + 4 + 8) { // flag, magic, length, check code
                ...
                if (LOG) Log.v(TAG, "getMiniThumbFromFile(" + id + ") flag=" + flag
                ...
                if (size >= 1 + 8 + 4 + 8 + length && data.length >= length) {
                    ...
                    if (newCheck != UNKNOWN_CHECK_CODE && newCheck == check) {
                        ...
                        Log.w(TAG, "getMiniThumbFromFile(" + id + ") wrong check code! newCheck="
        ...
        if (LOG) Log.v(TAG, "getMiniThumbFromFile(" + id + ") return null.");
        return null;
    }
    
    /**
     * create or get thumb result for more detail.
     */
    ...
    //for new cache mechanism enhancement
    ...
    private void removeOldFile(long id) {
        if (LOG) Log.v(TAG, "removeOldFile(" + id + ")");
    ...
    private void removeThumbdataFile(int version, long id) {
        ...
        if (LOG) Log.v(TAG, "removeThumbdataFile(" + version + ", " + id + ")");
    }
}
