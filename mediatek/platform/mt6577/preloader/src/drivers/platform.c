...
void platform_modem_download(void)
{
    ...
    mdelay(200);
	 ...
	 /* MD power key */
    mt_set_gpio_mode(GPIO_52_PWR_KEY, GPIO_52_PWR_KEY_M_GPIO);
    mt_set_gpio_dir(GPIO_52_PWR_KEY, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_52_PWR_KEY, GPIO_OUT_ONE);
	mdelay(200);
...
bool platform_com_wait_forever_check(void)
{
    ...
    /* check download key */
    if (TRUE == mt6577_detect_key(COM_WAIT_KEY)) {
