...
public class AttachmentDownloadService extends Service implements Runnable {
    ...
    /*package*/ class DownloadSet extends TreeSet<DownloadRequest> {
        ...
        private void cancelDownload(DownloadRequest req) {
            ...
            try {
                new EmailServiceProxy(mContext, intent, null).cancelLoadAttachment(req.attachmentId);
            } catch (RemoteException e) {
            ...
            if (attachment != null) {
                ...
                if (statusCode == EmailServiceStatus.MESSAGE_NOT_FOUND) {
                    ...
                    if(statusCode == EmailServiceStatus.SUCCESS && 
                        ...
                        Logging.d(TAG, "endDownload not changed attachment flag to " + attachment.mFlags
    ...
    private class ServiceCallback extends IEmailServiceCallback.Stub {
        public void loadAttachmentStatus(long messageId, long attachmentId, int statusCode,
            ...
            switch (statusCode) {
                ...
                /* 
                 * By default it will continually handle the call back, even the DownloadRequest can't be found.
                 * It will cause the mDownloadSet receive some duplicate endDownload call back.
                 * We change this unreasonable feature.
                 */
                Logging.d(TAG, "loadAttachmentStatus req for " + attachmentId + " can not be found, ignored it");
            }
        }
    ...
    public static void watchdogAlarm() {
        Logging.d("watchdogAlarm in AttachmentDownloadService with sRunningService: " + sRunningService);
        AttachmentDownloadService service = sRunningService;
        if (service != null) {
