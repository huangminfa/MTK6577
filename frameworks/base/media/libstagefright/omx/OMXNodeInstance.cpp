...
OMXNodeInstance::~OMXNodeInstance() {
    ...
    if (mHeap != NULL)
    {
        ...
        if (!sf_pmem_unregister(&rInfo)) {
            LOGE("pmem unregister err: 0x%X, offset: 0x%X !", (unsigned)rInfo.base, rInfo.offset);
        }
...
status_t OMXNodeInstance::useBuffer(
    ...
    //LOGE ("@@ OMX_UseBuffer2 virAddr=0x%x", virAddr);
...
status_t OMXNodeInstance::registerBuffer(
    ...
    if (!sf_pmem_register(&rInfo)) {
        LOGE("pmem register err: 0x%X, offset: 0x%X !", (unsigned)rInfo.base, rInfo.offset);
    }
...
status_t OMXNodeInstance::registerBuffer2(
    ...
    LOGD("registerBuffer2: base = 0x%08X, mMemHeapBase = 0x%08X, size = %d", (unsigned int)memheapinfo.base, (unsigned int)memheapinfo.mMemHeapBase, (unsigned int)memheapinfo.size);
...
#ifndef ANDROID_DEFAULT_CODE
struct UseAndroidNativeBufferParams3 {
    OMX_U32 nSize;
...
status_t OMXNodeInstance::useGraphicBuffer3_l(
    ...
    ver.s.nRevision = 0;
    ver.s.nStep = 0;
    UseAndroidNativeBufferParams3 params = {
        sizeof(UseAndroidNativeBufferParams3), ver, portIndex, bufferMeta,
...
// Morris Yang 20111128 temp workaround for ANativeWindowBuffer [
#ifndef ANDROID_DEFAULT_CODE
    //return useGraphicBuffer3_l(portIndex, graphicBuffer, buffer);
#endif
// ]    

    // See if the newer version of the extension is present.
    OMX_INDEXTYPE index;
// Bruce Hsu 20120329 (our omx don't tell ...Buffer and ...Buffer2 now)
