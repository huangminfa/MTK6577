...
public class DmSessionStateObserver implements SessionStateObserver {
        ...
        if (initiatorName.startsWith("VDM_SCOMO")) {
            ...
            if (type == SessionType.DM) {
                ...
                if (state == SessionState.COMPLETE) {
                    ...
                    DmApplication.getInstance().queueEvent(SessionEventQueue.EVENT_DL_SESSION_COMPLETED);
                    h.sendMessage(h.obtainMessage(IDmPersistentValues.MSG_SCOMO_DL_SESSION_COMPLETED));
                    ...
                    DmApplication.getInstance().queueEvent(SessionEventQueue.EVENT_DL_SESSION_STARTED);
                } else if (state == SessionState.ABORTED) {
                    Log.d(TAG.Session,"+++ DL session ABORTED +++");
                    DmApplication.getInstance().queueEvent(
                        SessionEventQueue.EVENT_DL_SESSION_ABORTED, new Integer(lastError));
                    executeSessionAbort(initiator,lastError,h);
                }
            }
        } else if (initiatorName.startsWith("VDM_FUMO")
                   || initiatorName.startsWith("VDM_LAWMO")) {
            ...
            if (type == SessionType.DM) {
                if (state == SessionState.COMPLETE) {
                    ...
                    h.sendMessage(h.obtainMessage(IDmPersistentValues.MSG_DMSESSIONCOMPLETED,action));
                } else if (state == SessionState.ABORTED) {
                    Log.i(TAG.Session, "DM session aborted, send message to service.");
                    h.sendMessage(h.obtainMessage(IDmPersistentValues.MSG_DMSESSIONABORTED, lastError, 0, initiator));
                }
            } else if (type == SessionType.DL) {
        ...
        } else if (initiatorName.startsWith("Network Inited"))
        {
            ...
            if (type == SessionType.DM) {
                ...
                if (state == SessionState.ABORTED) {
                    ...
                    h.sendMessage(h.obtainMessage(IDmPersistentValues.MSG_DLSESSIONABORTED, lastError, 0, initiator));
    ...
    private void executeSessionAbort(final SessionInitiator initiator, final int lastError, final Handler handler) {

        final boolean isFumo = initiator.getId().startsWith("VDM_FUMO");
        final boolean isScomo = initiator.getId().startsWith("VDM_SCOMO");
        SessionEventQueue.DLAbortState abState = DmApplication.getInstance()
                .analyzeDLAbortState();
        switch (abState.value) {
        case DLAbortState.STATE_NEED_RESUME_NOW:
            Log.d(TAG.Client,
                  "+++ connection re-setup +++, continue DL session...");
        ...
        case DLAbortState.STATE_HAVENOT_TIMEOUT:
            Log.d(TAG.Client,
                  "+++ connection not timeout(5min) +++, waiting...");
                ...
                public void run() {
                    ...
                    if (DmApplication.getInstance().isDMWapConnected()) {
                        ...
                        if (isFumo) {
                            ...
                            handler.sendMessage(handler.obtainMessage(IDmPersistentValues.MSG_SCOMO_DL_SESSION_ABORTED,
                                                lastError, 0, initiator));
                        }
                    }
                }
            ...
            DmApplication.getInstance().cancelAllPendingJobs();
            DmApplication.getInstance().scheduleBatchJobs(batchJob, 10*1000, abState.leftTime);
