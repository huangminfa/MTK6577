...
public class BandModeSIM2 extends Activity implements OnClickListener{
	...
	private void QueryCurrent() {
		...
		if(FeatureOption.MTK_GEMINI_SUPPORT){
		geminiPhone.invokeOemRilRequestStringsGemini(modeString, mResponseHander.obtainMessage(EVENT_QUERY_CURRENT),Phone.GEMINI_SIM_2); 
		}else{
	...
	private void QuerySupport() {
		...
		Log.v(LOG_TAG, "AT String:" + modeString[0]);
		if(FeatureOption.MTK_GEMINI_SUPPORT){
		geminiPhone.invokeOemRilRequestStringsGemini(modeString, mResponseHander.obtainMessage(EVENT_QUERY_SUPPORTED) , Phone.GEMINI_SIM_2); 
		}else{
...
public  void setSupportedMode(int gsmVal) {
	...
	private boolean SetBandMode(int gsm){
		...
		if(FeatureOption.MTK_GEMINI_SUPPORT)
		{
		geminiPhone.invokeOemRilRequestStringsGemini(modeString, 
