...
class AllAppsList {
        ...
        if (matches.size() > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (applicationInfo == null) {
                	if (!optr.equals("OP01")
							&& info.activityInfo.applicationInfo.packageName.equals(
									WIFI_SETTINGPKGNAME)
							&& info.activityInfo.name.equals(
									WIFI_SETTINGCLASSNAME)) {
						if (LauncherLog.DEBUG) {
							LauncherLog.d(TAG,"If the optr isn't 'OP01',  doesn't add the named of wifi's app"
													+ info.activityInfo.name );
						}
						continue;
                	}
    ...
    // remove WIFI from the arrayList,if the model isn't OP01 
    void removeWifiSettings() {

    	for (ApplicationInfo ai : added) {
    		if (ai.componentName.getPackageName().equals(WIFI_SETTINGPKGNAME) &&
    				ai.componentName.getClassName().equals(WIFI_SETTINGCLASSNAME)) {
    			 data.remove(ai);
    			 break;
    		 }
    	 }
