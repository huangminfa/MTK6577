...
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPSwitchCamera_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    ACDK_LOGD("ACDK_CCT_OP_SWITCH_CAMERA\n");
    if (a_u4Argc != 1)
    {
        ACDK_LOGD("Usage: switch <main camera:0, sub camera:1>\n");
        return E_ACDK_IF_API_FAIL;
    }

    MUINT32 u4CameraType = atoi((char *)a_pprArgv[0]);
    MUINT32 u4RetLen = 0;
...
MRESULT mrCCAPAEApplyExpoInfo_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    rAEExpPara.uFlareValue= (UINT8)atoi((char *)a_pprArgv[3]);
    rAEExpPara.uFlareGain = 128*255 / (255 - rAEExpPara.uFlareValue);
    u4FrameRate = 1000000 / rAEExpPara.u4Eposuretime;
    rAEExpPara.uCaptureFlareValue= (UINT8)atoi((char *)a_pprArgv[4]);
    ...
    {// AWB preference color
        ...
        {// CCT estimation
            ...
            {// Rotated X coordinate
                -634,    // i4RotatedXCoordinate[0]
                -486,    // i4RotatedXCoordinate[1]
                -319,    // i4RotatedXCoordinate[2]
                -184,    // i4RotatedXCoordinate[3]
                0,    // i4RotatedXCoordinate[4]
                5    // i4RotatedXCoordinate[5]
            }
        }
};
...
MRESULT mrCCAPAFApply_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    //! 2. Modify the input data, size, and output data, size
    //! ====================================================
    bRet = bSendDataToACDK(ACDK_CCT_V2_OP_AF_APPLY,
	   				       (UINT8*)&sLensPara,
							sizeof(sLensPara),
                            NULL,
                            0,
...
MRESULT mrCCAPFLASHEnable_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    MUINT32 u4RetLen = 0;
    MUINT32 strobeLevel = 12;   // default level is 12

    if (a_u4Argc > 1)   // wrong command
    {
        ACDK_LOGD("Usage: enFlash <Level(Optional):1~32,default=12>\n");
        return E_ACDK_IF_API_FAIL;
    }
    else if(a_u4Argc == 1)  // user assign specific level
    {
        strobeLevel = (MUINT32)atoi((char *)a_pprArgv[0]);
...
/////////////////////////////////////////////////////////////////////////
// Author : Cotta
// Functionality : return AE statistic data of low current strobe and high current strobe
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPStrobeRatioTuning_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    if (!g_bAcdkOpend )
    {
        return E_ACDK_IF_API_FAIL;
    ...
    ACDK_LOGD("ACDK_CCT_OP_STROBE_RATIO_TUNING\n");
    ...
    MUINT32 u4RetLen = 0;
    MUINT32 AE5x5Data[50] = {0};
    MUINT32 strobeLevel = 12;   // default level is 12

    if (a_u4Argc > 1)   // wrong command
    {
        ACDK_LOGD("Usage: strobeRatioTuning <preflash level(Optional):1~32,default=12>\n");
        return E_ACDK_IF_API_FAIL;
    }
    else if(a_u4Argc == 1)  // user assign specific level
    {
    ...
    BOOL bRet = bSendDataToACDK(ACDK_CCT_OP_STROBE_RATIO_TUNING,
                                (UINT8 *)&strobeLevel,
                                sizeof(MUINT32),
                                (UINT8 *)&AE5x5Data,
                                0,
    ...
    else
    {
        for(int i = 0; i < 50; ++i)
        {
            if(i < 25)
            {                
                ACDK_LOGD("low Current AE %d : %u\n",i + 1, AE5x5Data[i]);
            }
            else
            {
                ACDK_LOGD("high Current AE %d : %u\n",i - 24, AE5x5Data[i]);   
            }
        }
    }
...
static const Acdk_CLICmd g_prAcdkCCAPCLICmds [] =
{
    ...
    {"switch",                    "ACDK_CCT_OP_SWITCH_CAMERA, (switch <type>)",     mrCCAPSwitchCamera_Cmd},
    ...
    //Strobe    
    {"enFlash",           "ACDK_CCT_OP_FLASH_ENABLE (enFlash <Level(Optional):1~32,default=12>)", mrCCAPFLASHEnable_Cmd},
    ...
    {"strobeRatioTuning", "ACDK_CCT_OP_STROBE_RATIO_TUNING (strobeRatioTuning <preflash level(Optional):1~32,default=12>)", mrCCAPStrobeRatioTuning_Cmd}, //cotta-- added for strobe high current tuning
