...
#else
#define AAC_EXTR_VEB(a,...)
#define AAC_EXTR_DBG(a,...)
#define AAC_EXTR_WARN(a,...)
#define AAC_EXTR_ERR(a,...)
#endif
		...
		*out_header = U32_AT(tmpBuf);
		...
		if (id != 0)	//copyright ID is presented
		{
			...
			if (iBitrate == 0)	 //bitrate is not known
			{
				...
				LOGE("findADIFHeader -- iBitrate is 0");
				return false;
			}
		...
		else	//copyright ID is not presented
		{
			...
			if (iBitrate == 0)	//bitrate is not known
			{
				LOGE("findADIFHeader--the adif bitrate is 0");
				return false;
				//PVMF_AACPARSER_LOGERROR((0, "AACBitstreamObject::getFileInfo- Misc Error"));
				//return AACBitstreamObject::MISC_ERROR;
			}
         ...
         uint32_t *pBitRate, uint32_t *pProfile, uint32_t *pChannelNum, int32_t *pIsADIF)
{
	...
	{	
		...
		*pSampleFreqIndex = (header>>10)&0x0F;
		//*pSampleRate = AACSampleFreqTable[nSampleFreqIndex];
		...
		*pChannelNum = (header>>6)&0x07;
		...
		{
			*pBitRate = (float)readedDataSize * 8 / frameNum  / 1024 * AACSampleFreqTable[*pSampleFreqIndex];
		}
		else
		{			
			return false;
...
bool AACFrameLenDecoder::initAACDec()
{
    ...
    if (err != MP4AUDEC_SUCCESS) {
        LOGE("MtkAACSource::initAACDec--Failed to initialize MP4 audio decoder");
        return false;
    }
      ...
      mFixedHeader(0)
{
	...
	LOGV("AACExtractor::AACExtractor--nBitRate=%d", nBitRate);
	LOGV("AACExtractor::AACExtractor--samplerate = %d", nSampleRate); 
	LOGV("AACExtractor::AACExtractor--nProfile = %d", nProfile);
	LOGV("AACExtractor::AACExtractor--num channels = %d", nChannelNum);
	LOGV("AACExtractor::AACExtractor--config_info[0]=%x", config_info[0]);
	LOGV("AACExtractor::AACExtractor--config_info[1]=%x", config_info[1]);
...
status_t MtkAACSource::start(MetaData *) {
	...
	if (mMeta->findInt32(kKeySampleRate, &nSampleRate)) 
	{
		...
		AAC_EXTR_DBG("Per Frame Duration =%d,First frame pos =%d",mFrameDurationUs,mFrameDurationUs);
