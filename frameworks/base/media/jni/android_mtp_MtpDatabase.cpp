...
MtpResponseCode MyMtpDatabase::setObjectPropertyValue(MtpObjectHandle handle,
    ...
    //Added for USB Develpment debug, more log for more debuging help
    {    
        LOGE("%s: MTP_RESPONSE_OBJECT_PROP_NOT_SUPPORTED!!\n", __func__);
    //Added for USB Develpment debug, more log for more debuging help
...
MtpResponseCode MyMtpDatabase::getDevicePropertyValue(MtpDeviceProperty property,
    ...
    //Added for USB Develpment debug, more log for more debuging help
    {    
        LOGE("%s: MTP_RESPONSE_DEVICE_PROP_NOT_SUPPORTED!!\n", __func__);
    //Added for USB Develpment debug, more log for more debuging help
...
MtpResponseCode MyMtpDatabase::resetDeviceProperty(MtpDeviceProperty property) {
    ...
    {
        LOGE("%s: property = 0x%x, non-writable!!\n", __func__, property);
        result = MTP_RESPONSE_ACCESS_DENIED;
    }
    else
    {    
        LOGE("%s: property = 0x%x, MTP_RESPONSE_DEVICE_PROP_NOT_SUPPORTED!!\n", __func__, property);
    ...
    if (count > 0) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            switch (type) {
                ...
                default:
                //Added Modification for ALPS00255822, bug from WHQL test
                    LOGE("bad or unsupported data type in MyMtpDatabase::getObjectPropertyList: objectHandles[%d]=0x%x, propertyCodes[%d] = 0x%x, type = 0x%x",i, objectHandles[i], i, propertyCodes[i], type);
                //Added Modification for ALPS00255822, bug from WHQL test
                    break;
            }
        }
...
//Added Modification for ALPS00255822, bug from WHQL test
    ...
    switch (property) {
        ...
        case MTP_PROPERTY_DURATION:
            ...
            result->setFormRange(0x01, 0xFF, 0x02);
            break;
        case MTP_PROPERTY_OBJECT_SIZE:
            result = new MtpProperty(property, MTP_TYPE_UINT64);
            break;
...
int register_android_mtp_MtpDatabase(JNIEnv *env)
{
    ...
    if (method_getObjectBeginIndication == NULL) {
        LOGE("Can't find getObjectBeginIndication");
        return -1;
    }
    ...
    if (method_getObjectEndIndication == NULL) {
        LOGE("Can't find getObjectEndIndication");
        return -1;
    }
    //Added Modification for ALPS00264207, ALPS00248646
