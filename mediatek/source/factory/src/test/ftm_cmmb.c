...
static item_t cmmb_main_items[] = {
	...
	item(ITEM_AUTO_SCAN,   "Auto Scan"),
	item(ITEM_CHNN_LIST,   "Channel List"),
	item(-1, NULL),
};
	...
	while (1) {
		...
		switch (cmmb->cmmb_cmd)
		{
			case CMD_INIT:
				...
				{
					update_iv_info(cmmb, "CMMB driver initialization OK.\n");
					cmmb->cmmb_state = CMMB_IDLE;
				}
				else
					update_iv_info(cmmb, "CMMB driver initialization failed!\n");
			...
			case CMD_AUTO_SCAN:
				channel_count = 0;
				g_cmmb = cmmb;
				update_iv_info(cmmb, "Scanning channels...\n");
				...
				if (errCode == CMMB_S_OK)
				{
					update_iv_info(cmmb, "Channels auto scanning OK.\n");
					cmmb->cmmb_state = CMMB_SCAN_OK;
				}
				else
					update_iv_info(cmmb, "Channels auto scanning failed!\n");
...
*/
	// initialize title and text
	strcpy(szTitle, "CMMB Recording Stream");
	sprintf(szBuf, "Recording stream to %s\n",szFile);
	CmmbProps Props;
	CmmbFtGetProp(&Props);
	// initialize title and text
	FormatSignalInfo(szText, Props.SNR,Props.PRE_BER,Props.RSSI);
//	FormatSignalInfo(szText, propVal1, propVal2, propVal3);
	...
	do {
		...
		{
			CmmbFtStopService();
			sprintf(szBuf, "Record stopped, file is saved to %s\n", szFile);
			FormatSignalInfo(szText, Props.SNR,Props.PRE_BER,Props.RSSI);
			strcat(szText, szBuf);
			recording = false;
		}
...
int ShowServiceListView(void *priv)
{
	...
	// initialize title and text
	strcpy(szTitle, "CMMB Service List");
	strcpy(szText, "Please select a stream to record\n");
...
int ShowChannelListView(void *priv)
{
	...
	// initialize title and text
	strcpy(szTitle, "CMMB Channel List");
	strcpy(szText, "Please select a channel to tune,and wait...\n");
