...
public class DmMailNodeIoHandler implements NodeIoHandler {
    ...
    private String[] item = { "apn", "smtp_server", "smtp_port", "smtp_ssl",
            ...
            for (int i = 0; i < getItem().length; i++) {
                if (uri.getPath().contains(getItem()[i])) {
                    if (mContent[i] != null) {
                        ...
                        Log.i(TAG.NodeIOHandler, "[MailNode] send broadcast intent 'PUSHMAIL_GET_PROFILE' ====>");

                        Log.i(TAG.NodeIOHandler, "[MailNode] blocking here to wait for intent 'PUSHMAIL_PROFILE'...");
                        synchronized (mLock) {
                            while (isDataReady) {
                                try {
                                    //FIXME!!!
                                    //CC procedure will hang if the intent has no response.
                                    //Do we need set a timeout here?
                                    mLock.wait();
                                    Log.i(TAG.NodeIOHandler, "[MailNode] skip waiting when got intent back");
                                    break;
    ...
    class pushMailReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intentAction.equals("android.intent.action.PUSHMAIL_PROFILE")) {
                Log.i(TAG.NodeIOHandler, "[MailNode] received broadcast intent 'PUSHMAIL_PROFILE' <====");
