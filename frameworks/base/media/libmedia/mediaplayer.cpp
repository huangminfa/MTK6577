...
status_t MediaPlayer::start()
{
    ...
    if ( (mPlayer != 0) && ( mCurrentState & ( MEDIA_PLAYER_PREPARED |
		...
		f = fopen(filename, "r");
		if (!f) { *process_name = '\0';  }
		if (!fgets(process_name, sizeof(process_name), f)) { *process_name = '\0'; }
		fclose(f);
		if(!strncasecmp(process_name, "com.android.music", 17)){			
			...
			if (mPlayer != NULL) {
       			err =  mPlayer->setParameter(KEY_PARAMETER_AUDIO_SEEKTABLE, request);
    		}
			LOGV("%s can use seek table! err %d",process_name,err);	
		}else{
			LOGV("Don't notify duration to %s!",process_name);
		}
#endif
...
status_t MediaPlayer::getDuration_l(int *msec)
{
    ...
    if (mPlayer != 0 && isValidState) {
        ...
        //if (mDuration <= 0) //Tina disabled for updating duration
            ret = mPlayer->getDuration(&mDuration);
        if(mDuration < 0){        
           LOGD("getDuration:%d < 0 ",mDuration);
           mDuration = 0;
        }
        if (msec)
            *msec = mDuration;
