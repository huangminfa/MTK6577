...
#define PHONE_MODE_SETTING 2  // for set policy manager parameters command type, defined in AudioIotrl.h
#endif
...
static bool ParaMetersNeedFilter(String8 Pair){
    #ifdef GENERIC_AUDIO
    return true; // always filter with generic audio
    #else
    ...
    : BnAudioPolicyService() , mpAudioPolicyDev(NULL) , mpAudioPolicy(NULL)
{
    ...
    LOGD("AudioPolicyService hw_get_module AUDIO_POLICY_HARDWARE_MODULE_ID");
    rc = hw_get_module(AUDIO_POLICY_HARDWARE_MODULE_ID, &module);
    LOGD("AudioPolicyService hw_get_module AUDIO_POLICY_HARDWARE_MODULE_ID rc = %d",rc);
    if (rc)
        return;

    LOGD("AudioPolicyService audio_policy_dev_open");
    rc = audio_policy_dev_open(module, &mpAudioPolicyDev);
    LOGE_IF(rc, "couldn't open audio policy device (%s)", strerror(-rc));
    if (rc)
        return;

    LOGD("AudioPolicyService create_audio_policy");
    ...
    LOGD("Loaded audio policy from %s (%s)", module->name, module->id);
...
#if defined(MTK_AUDIO) && !defined(ANDROID_DEFAULT_CODE)
    mpAudioPolicy->set_policy_parameters(mpAudioPolicy, PHONE_MODE_SETTING, state,0,0);
#endif
...
bool AudioPolicyService::AudioCommandThread::threadLoop()
{
    nsecs_t waitTime = INT64_MAX;
     ...
     int result = 0;
     result = mtk_AdjustPrio(thread_name);	// ]w\ return 0;  return -1
            ...
            if (mAudioCommands[0]->mTime <= curTime) {
                ...
                case SET_PARAMETERS: {
                     ...
                     if((param.getFloat (String8("SetMasterVolume"), value)) == NO_ERROR){
                         LOGD("SET_PARAMETERS SetMasterVolume value = %f",value);
...
void AudioPolicyService::AudioCommandThread::insertCommand_l(AudioCommand *command, int delayMs)
{
    ...
    if(delayMs == -1)
    {
        ...
        //LOGD("command wait time=%lld ms",ns2ms(command->mTime - systemTime()));
    }//weiguo---->
    else
    {
        ...
        case SET_PARAMETERS: {
            ...
            for (size_t j = 0; j < param.size(); j++) {
               ...
               for (size_t k = 0; k < param2.size(); k++) {
                  ...
                  if (key2 == key) {
                      param2.remove(key2);
                      LOGD("Filtering out parameter %s", key2.string());
                      break;
                  }
               }
            }
