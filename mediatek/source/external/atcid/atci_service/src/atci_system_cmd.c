...
int system_check_fboot(){
    ...
    LOGD("check fboot :%d", ret);
	return ret;
}

static void* restart_routine(void *arg) {
	LOGD("Ready to restarting system after %d seconds!", RESTART_WAITING_TIME);
...
int invoke_restart(){
	...
	if (ret != 0) 
	{
		LOGE("fail to create thread for restarting");
	}
...
int system_fboot_handler(char* cmdline, ATOP_t at_op, char* response){    
    int retval = 0;
        ...
        LOGD("[%s]handle cmdline:%s", __FUNCTION__, cmdline);
