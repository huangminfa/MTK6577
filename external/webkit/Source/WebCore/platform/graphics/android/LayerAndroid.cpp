...
public:
	...
	/// M: Add virtual fucntion to retrieve image URI. @{
	...
	/// @}
	/// M: Add Api to get the image URI string. @{
	const char* getImageURI()const{return mURI;}
	/// @}
protected:
  	/// M: Save the image URI. @{
	const char* mURI;
	/// @}
};
...
public:
	...
	/// M: Add Api to get the image URI string. @{
	const char* bestImageURI()const {return m_checker.getImageURI();}
	/// @}
...
void LayerAndroid::findInner(LayerAndroid::FindState& state) const
{
    ...
    /// M: fix incorrect position offset
    state.setLocation(testX + localBounds.fLeft, testY + localBounds.fTop);
    if (!m_recordingPicture)
        return;
    if (!contentIsScrollable() && !state.drew(m_recordingPicture, localBounds))
        return;
    /// M: Fix for ALPS00359681, The best state is only drew text coordinate. @{
    if (owningLayer() && !hasText())
        return;
    /// @}
...
/// M: Add API to retrieve image URI. @{
const char* LayerAndroid::findBestImageURI(int x, int y, SkPicture* root)const
{
	FindState state(x, y);
    SkRect rootBounds;
    rootBounds.setEmpty();
...
bool LayerAndroid::drawGL()
{
    ...
    /// M: get dirty rectangle of baseLayer for ALPS00321688 @{
    SkRect boundsRect = bounds();
    const IntRect& baseDirtyRect = m_state->getBaseLayerDirtyRect();
    if (boundsRect.intersect(baseDirtyRect.x(), baseDirtyRect.y()
                                , baseDirtyRect.x() + baseDirtyRect.width(), baseDirtyRect.y() + baseDirtyRect.height()))
