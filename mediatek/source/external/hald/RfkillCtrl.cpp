...
int RfkillCtrl::setAllState(int on)
{
	...
	if (fd < 0) {
		  LOGE("Can't open RFKILL control device");
		  return 1;
	}

	if (fcntl(fd, F_SETFL, O_NONBLOCK) < 0) {
		  LOGE("Can't set RFKILL control device to non-blocking");
		  close(fd);
		  return 1;
	}
	...
	while (1) {
		...
		if (len < 0) {
			if (errno == EAGAIN)
				  break;
			LOGE("Reading of RFKILL events failed");
			break;
		}

		if (len != RFKILL_EVENT_SIZE_V1) {
			LOGE("Wrong size of RFKILL event\n");
			continue;
		}
		...
		LOGD("RFKILL id[%d] type[%d] name[%s] blocked[soft:%s hard:%s]",
			event.idx,
			event.type,
			getName(event.idx),
			event.soft ? "y" : "n",
		...
		if(event.soft || event.hard) {
			...
			LOGD("Enable RFKILL interface [%d]  ", event.idx);
		}
	}
