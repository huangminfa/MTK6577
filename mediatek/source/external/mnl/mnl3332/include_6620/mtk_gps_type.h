...
typedef struct
{
  ...
  UINT8     sv_in_view_snr[MTK_GPS_SV_MAX_NUM];
...
typedef struct
{
  ...
  UINT32    fix_interval;   /* fix interval in milliseconds */
                                /* 1000=>1Hz, 200=>5Hz, 2000=>0.5Hz */
  UINT8     Int_LNA_Config; /* for 3326, 1: internal LNA, for 3336,1: high gain*/
  UINT8     Rf_Adc_Type;    /* for MT3332, different RF settings */
  UINT8     Rf_Sig_Src;     /* for MT3332, to check RF/IF source */
  UINT8     Galileo_BW;     /* for MT3332, to config Galileo system bandwidth */
  UINT8     HM_Gain;     /* for MT3332, to config high or middle gain, default middle */
