...
int gps_send_at_command(char * command){
    ...
    {
        LOGE("pas_gps_handler: open sock_fd failed\r\n"); 
        return -1;
    }
    ...
    {
        LOGE("send command to HAL fail:%s\r\n", strerror(errno));
    }else{
        LOGD("send command to HAL successfully !!\n");        
    }
...
void gps_receive_at_result(char* response){   
    ...
    LOGD("Receive GPS AT test result\n");
    left = recvfrom(sock_fd, result, sizeof(result), 0, NULL, NULL);
    if (left < 0){
        LOGE("recv test ack from GPS driver fail: %s\n", strerror(errno));                
    } else {
        LOGD("recv test ack from GPS driver success: %d\n", left);
...
int pas_gps_handler(char* cmdline, ATOP_t at_op, char* response){
    ...
    LOGD("handle cmdline:%s, at_op: %d", cmdline, at_op);
    char test_command[20] = "AT%GPS=";

    switch(at_op){        
        case AT_ACTION_OP:
            //AT%GPS
            LOGD("Action operation: AT%%GPS\n");
