...
static void mpodecoder_close(JNIEnv* env, jobject mpodecoder) {
    ...
    delete mpoDecoder;
    env->SetIntField(mpodecoder,mpoDecoder_nativeInstanceID,NULL);
}
...
static jobject mpodecoder_decodeByteArray(JNIEnv* env, jobject, 
    ...
    LOGE("MpoDecoder:decode byte array:offset=%d, length=%d", offset,length);
    AutoJavaByteArray ar(env, byteArray);
    MpoInputStream* mpoInputStream = new MpoMemoryInputStream(ar.ptr() + offset,
                                                              length, true);
    LOGE("MpoDecoder:got mpoInputStream %d", (int)mpoInputStream);
