...
public class MultipleSimActivity extends PreferenceActivity implements Preference.OnViewChangedListener, Preference.OnPreferenceChangeListener
{
    ...
    private void skipUsIfNeeded()
    {
        ...
        if (this.mSimNumbers == 1 && mTargetClass != null)
        {
            ...
            if(FeatureOption.EVDO_DT_SUPPORT
                    && mTelephonyManager.getPhoneTypeGemini(slotId) == Phone.PHONE_TYPE_CDMA) {
    ...
    void initPreferenceScreen()
    {
        ...
        if (mSimNumbers == simList.size())
        {
            for (int i = 0; i < mSimNumbers; ++i)
            {
                ...
                p.setOnViewChangeListener(this);
    ...
    void initListPreference()
    {
        ...
        for (int i = 0; i < mSimNumbers; ++i)
        {
            ...
            listPref.setOnViewChangeListener(this);
            //Now we use @ as the flag to indicate if need to modify the key
            //Use this must make sure there's only one cotrol use, else will cause issue
            if (mBaseKey != null && mBaseKey.endsWith("@"))
            {
    ...
    private void updateIcon() {
        ListView listView = MultipleSimActivity.this.getListView();
        if (listView != null) {
    ...
    private void customizeForEVDO(Intent intent, int slotId, String pkName, String mTargetClass){
        ...
        if("com.android.phone.GsmUmtsCallForwardOptions".equals(mTargetClass)){
            intent.setClassName(pkName, "com.android.phone.CdmaCallForwardOptions");
            checkToStart(slotId, intent);
        }else if("com.android.phone.GsmUmtsAdditionalCallOptions".equals(mTargetClass)){
            intent.setClassName(pkName, "com.android.phone.CdmaCallWaitingOptions");
            checkToStart(slotId, intent);
        }else if("com.android.phone.FdnSetting2".equals(mTargetClass)
            || "com.android.phone.CallBarring".equals(mTargetClass)
