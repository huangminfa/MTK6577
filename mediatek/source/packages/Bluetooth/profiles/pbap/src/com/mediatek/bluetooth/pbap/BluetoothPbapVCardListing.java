...
import com.android.bluetooth.pbap.BluetoothPbapWriter;
...
public class BluetoothPbapVCardListing {
    ...
    private static final String VCARD_LISTING_BEGIN = 
        "<?xml version=\"1.0\"?>"+
    ...
    private boolean mDirty =  true;     // If not listed before, set to true to prevent get entry request
    private Long[] mIDList = null;  // This is used to keep all IDs of list result sorted by name(only for PB)
    private boolean mSimDirty = true;   // If SIM folder is not listed before, set to true to prevent get entry request
    private Long[] mSimIDList = null;   // This is used to keep all IDs of list result sorted by name(only for sim1\telecom\PB)
    ...
    public BluetoothPbapVCardListing(Context context, String localName, String localNum) {
    ...
    public int list(int type, int order, int searchAttr, String searchVal, int listOffset, int maxListCount) {
        int ownerNotInc = 1;
        int ret = ResponseCodes.OBEX_HTTP_PRECON_FAILED;
        ArrayList<String> result = null;        
        
        printLog("init("+type+","+order+","+searchAttr+","+searchVal+","+listOffset+","+maxListCount+")");
        ...
        case BluetoothPbapPath.FOLDER_TYPE_PB:
            /* Check if owner is included. owner will not be included in search result */
            if(TextUtils.isEmpty(searchVal) == true) {
                ownerNotInc = 0;
            }
            if(mDirty || isSameAttribute(searchAttr, searchVal) != true) {
        ...
        case BluetoothPbapPath.FOLDER_TYPE_SIM1_PB:
            ...
            if(mSimDirty || isSameAttribute(searchAttr, searchVal) != true) {
                mSearchAttrib = searchAttr;
                mSearchValue = searchVal;
        ...
        if(mWriter == null) {
            ...
            for(int i = 0;r && i < result.size();i++) {
                r = mWriter.write("<card handle=\"" + (listOffset+i+ownerNotInc) + ".vcf\" name=\"" + result.get(i)
                        + "\"" + "/>");
            }
            if(r == true) {
    ...
    public long queryPbID(int index){
        ...
        if(mDirty) {
            /* Relisting : set list offset to 1 to prevent owner card is included */
            listPb(true, 1, 0, null);
        }
        if(mIDList != null && mIDList.length > index) {
            return mIDList[index];
        }
        errorLog("ERR] can not found id for index "+index+". mIDList.length = "+((mIDList==null)?0:mIDList.length));
        return -1;
    }
    ...
    public long querySimPbID(int index){
        ...
        errorLog("ERR] can not found id for index "+index+". mSimIDList.length = "+((mSimIDList==null)?0:mSimIDList.length));
        return -1;
    }
    ...
    private String getOwnerName() {
        if(TextUtils.isEmpty(mLocalName) != true ) {
            return mLocalName;
        }else if(TextUtils.isEmpty(mLocalNum) != true ) {
            return mLocalNum;
        }else {
    ...
    private int listPb(boolean relist, int listOffset, int maxListCount, ArrayList<String> result) {
        ...
        Cursor cursor = null;
        boolean ownerInc = (TextUtils.isEmpty(mSearchValue) == true);
        ...
        if(relist == true) {
            String selection = RawContacts.INDICATE_PHONE_SIM + "=" + RawContacts.INDICATE_PHONE;
            String[] projection = null;
            HashSet<Long> idSet = null;
            ArrayList<Long> idList = null;

            mIDList = null;
            if(TextUtils.isEmpty(mSearchValue) != true) {
                if(mSearchAttrib != VCARD_SEARCH_NUMBER) {
                    uri = Uri.withAppendedPath(Contacts.CONTENT_FILTER_URI, Uri.encode(mSearchValue));
            ...
            cursor = resolver.query(uri, projection, selection, null, "upper("+Contacts.DISPLAY_NAME+")");
            if(cursor != null) {
                ...
                printLog("Retrieve data : listOffset="+listOffset+", maxListCount="+maxListCount);
                while(!cursor.isAfterLast()) {
                    if (idSet.add(cursor.getLong(0)) == true ) {
                        // ID is not contained by idSet
                        idList.add(cursor.getLong(0));
                        if(maxListCount > 0 && idList.size() > listOffset ) {
                            printLog("Add one record : "+cursor.getString(1)+",ID:"+cursor.getLong(0));
            ...
            if(mIDList.length <= listOffset) {
                return ResponseCodes.OBEX_HTTP_NOT_FOUND;
            }
            if((mIDList.length-listOffset) < maxListCount) {
                maxListCount = mIDList.length-listOffset;
            }
            if(maxListCount != 0) {
                String idList = genIDList(listOffset, maxListCount);
                if(idList != null) {
                    uri = Contacts.CONTENT_URI;
                    cursor = resolver.query(uri, PB_NAME_PROJECTION, Contacts._ID+" IN "+idList, null, Contacts.DISPLAY_NAME);
    ...
    private int listSimPb(boolean relist, int listOffset, int maxListCount, ArrayList<String> result) {
        ...
        Cursor cursor = null;
        boolean ownerInc = (TextUtils.isEmpty(mSearchValue) == true);
        printLog("[API] listSimPb("+String.valueOf(relist)+","+listOffset+","+maxListCount+")");
        printLog("SearchVal="+mSearchValue+", mSearchAttrib="+mSearchAttrib);
        if(resolver == null) {
            errorLog("[ERR] resolver is null");
            return ResponseCodes.OBEX_HTTP_INTERNAL_ERROR;
        ...
        if(relist == true) {
            ...
            if(mSimIDList.length <= listOffset) {
                return ResponseCodes.OBEX_HTTP_NOT_FOUND;
            }
            if((mSimIDList.length-listOffset) < maxListCount) {
                maxListCount = mSimIDList.length-listOffset;
            }
            if(maxListCount != 0) {
    ...
    /* Return OBEX result code */
    private int listCallLog(int type, int listOffset, int maxListCount, int searchAttrib, String searchVal, ArrayList<String> result) {
        ...
        try {
            String num = null;
            int callSize = 0;
            cursor = mContext.getContentResolver().query(myUri, projection, selection, null, CallLog.Calls.DATE+" DESC");
            if (cursor != null) {
                ...
                if(callSize <= listOffset){
                    ...
                    /*
                    if((callSize-listOffset) < maxListCount) {
                        maxListCount = callSize-listOffset;
                    }
                    */
...
/*
    private int listSimPb(int listOffset, int maxListCount, int searchAttrib, String searchVal, ArrayList<String> result) {
        ListIterator<AdnRecord> iterator= null;
        AdnRecord adn = null;
        String name = null;
        boolean ownerInc = (TextUtils.isEmpty(searchVal) == true);

        if(ownerInc) {
            if(listOffset == 0) {
                result.add(mSimAdn.getOwnerName());
                maxListCount--;
            }
            if(listOffset > 0)
                listOffset--;
        ...
        // update ADN
        if( !mSimAdn.updateAdn() ) return mSimAdn.getLastError();
        // search ADN
        if(!TextUtils.isEmpty(searchVal)) {
            if(searchAttrib != VCARD_SEARCH_NUMBER) {
                mSimAdn.searchAdn(searchVal,false);
            }else{
                mSimAdn.searchAdn(searchVal,true);
            }
        }
        //sort ADN
        mSimAdn.sortAdn();
        // fill result
        if(mSimAdn.getCount() <= listOffset) return ResponseCodes.OBEX_HTTP_NOT_FOUND;
        if( maxListCount > (mSimAdn.getCount()-listOffset)){
            maxListCount = (mSimAdn.getCount()-listOffset);
        }
        iterator = mSimAdn.getAdnList().listIterator(listOffset);        
        while(maxListCount > 0 && iterator.hasNext()){
            maxListCount--;
            adn = iterator.next();
            name = adn.getAlphaTag();
            if(TextUtils.isEmpty(name)) name = adn.getNumber();
            printLog("listSimPb : name="+name);
