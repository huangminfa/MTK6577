...
extern int mt6577_nand_write_page_hwecc (unsigned int logical_addr,
                                         char *buf);
extern int mt6577_nand_read_page_hwecc (unsigned int logical_addr, char *buf);
...
int find_empty_page_from_top(int start_addr)
{
	...
	for(page_offset=0;page_offset<(BLOCK_SIZE/PAGE_SIZE);page_offset++)
	{
		...
		if(!mt6577_nand_read_page_hwecc(current_add, page_readbuf))
		{
	...
	if(page_offset==(BLOCK_SIZE/PAGE_SIZE))
	{
		...
		if(!mt6577_nand_erase(start_addr)) 
...
bool find_mirror_pt_from_bottom(int *start_addr)
{
	...
	for(mpt_locate=(BLOCK_SIZE/PAGE_SIZE);mpt_locate>0;mpt_locate--)
	{
		...
		if(!mt6577_nand_read_page_hwecc(current_start_addr, page_readbuf))
		{
...
int load_exist_part_tab(u8 *buf)
{
   ...
   find:
	...
	for(pt_locate=0;pt_locate<(BLOCK_SIZE/PAGE_SIZE);pt_locate++)
	{
		...
		memset(pmt_spare,0xFF,PT_SIG_SIZE);
		memset(g_nand_spare,0xFF,64);
		if(!mt6577_nand_read_page_hwecc(pt_cur_addr, page_readbuf))
		{
	...
	if(pt_locate==(BLOCK_SIZE/PAGE_SIZE))
	{
		...
		else
		{
			...
			mt6577_nand_read_page_hwecc(mirror_address, page_readbuf);
		}
	}
		...
		*(int *)sig_buf = MPT_SIG;
		...
		if(pageoffset!=0xFFFF)                                                      
		{
			...
			for(i=0;i<2;i++)
			{
				...
				if(!mt6577_nand_write_page_hwecc(current_addr, page_buf))
				{
				...
				else
				{
					...
					//read back verify
					if((!mt6577_nand_read_page_hwecc(current_addr, page_readbuf))||memcmp(page_buf,page_readbuf,PAGE_SIZE))
					{
