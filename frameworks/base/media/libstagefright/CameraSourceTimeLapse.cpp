...
bool CameraSourceTimeLapse::skipFrameAndModifyTimeStamp(int64_t *timestampUs) {
	...
	if (!mReachedStartTime) {
		if (*timestampUs < mStartTimeUs) {
			LOGD("Drop frame before start time %lld/%lld", *timestampUs, mStartTimeUs);
			return true;
		}
		else {
			mReachedStartTime = true;
			LOGD("Reach start time %lld/%lld", *timestampUs, mStartTimeUs);
		}
	}
#endif
		...
		if (mForcePick) {
			if (1 == mForcePickMode) {
				*timestampUs = mLastFrameTimestampUs;
				...
				LOGD("Force pick ts %lld, mode 1", *timestampUs);
				return false;
			} else if (2 == mForcePickMode){
				*timestampUs = mLastFrameTimestampUs + mTimeBetweenTimeLapseVideoFramesUs;
				LOGD("Force pick ts %lld, mode 2", *timestampUs);
				return false;
			}
		}
#endif
