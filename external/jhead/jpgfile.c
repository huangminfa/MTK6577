...
//jpegexif does not protect global variables , add protection - S
#include <pthread.h>
#include <semaphore.h>
...
//jpegexif does not protect global variables , add protection - E
    ...
    for(;;){
        ...
        for (a=0; a<m_size ;a++){
            ...
            if( a == m_size){
                ...
                LOGD("get marker %x, prv %x, after (%d), L:%d!!\n", marker, prev, a, __LINE__);
...
static int ReadJpegStreamSections (ReadMode_t ReadMode)
{
    int a;
    int HaveCom = FALSE;
...
int ReadJpegStream(ReadMode_t ReadMode)
{
    int ret;

    // Scan the JPEG headers.
    printf("ReadJpegStream");
    ret = ReadJpegStreamSections(ReadMode);
    if (!ret){
        LOGE("ReadJpegStream Not JPEG");
...
{
LOGD("[EXIF]other thread is parsing exif, wait for this thread done.");
    pthread_cond_wait(&jpgFilecond , &jpgFileMutex);
LOGD("[EXIF]Wait thread done.");
}
g_u4InUse = 1;
pthread_mutex_unlock(&jpgFileMutex);
//jpegexif does not protect global variables , add protection - E
