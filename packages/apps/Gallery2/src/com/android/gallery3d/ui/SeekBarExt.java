...
//    private Drawable mConvergenceIndicator_red = null;
//    private Drawable mThumb_blue = null;
//    private Drawable mThumb_red = null;
//    private boolean mUseBlueThumb = true;
    private boolean mSqueezed = false;
    private int mConvOffset = 0;
    private int[] mConvArray;
//    private ArrayList<Integer> mMaskedArray;
...
//        int indicatorTop = (intrinsicHeight + measuredHeight) / 2;
//        Rect r = new Rect();
//        progressDrawable.getPadding(r);
//        Log.d(TAG, " progress padding=" + progressDrawable.getPadding(r) + ", rect=" + r);
        /// TODO: change arbitrary value 20 to a proper width for convergence indicator
...
//        Log.d(TAG, " view padding left=" + paddingLeft + ", padding top=" + paddingTop);
        int curProgress = getProgress();
        
        // draw all points from the convergence array, including starting and ending points
        /// TODO: maybe we need a re-entrant lock here for the convergence array...
//        boolean isValueMatch = false;
        ...
        if (canDrawIndicators) {
            ...
            for (int i : arrayToUse) {
                ...
                canvas.save();
                //canvas.translate(paddingLeft + offset - (indicatorW / 2), (measuredHeight + intrinsicHeight) / 2);
                canvas.translate(paddingLeft + offset - (indicatorW / 2), (measuredHeight - intrinsicHeight) / 2 - indicatorH);
//                if (curProgress == mConvArray[i]) {
//                    mConvergenceIndicator_red.draw(canvas);
//                    isValueMatch = true;
//                } else {
...
//                }
        ...
        // use red thumb if needed
        // WARINING: setThumb might cause re-layout, and the progress drawable's bounds will be reset
//        if (isValueMatch && mUseBlueThumb) {
//            setThumb(mThumb_red);
//            Log.w(TAG, " setThumb: RED");
//            Log.w(TAG, " after setThumb(RED): progress h=" + getProgressDrawable().getBounds().height());
//            mUseBlueThumb = false;
//        } else if (!isValueMatch && !mUseBlueThumb) {
//            setThumb(mThumb_blue);
//            Log.w(TAG, " setThumb: BLUE");
//            Log.w(TAG, " after setThumb(BLUE): progress h=" + getProgressDrawable().getBounds().height());
//            mUseBlueThumb = true;
//        }
...
//        if (valueMasks != null && valueMasks.length > 0) {
//            // 1. make sure all mask values are within bounds
//            boolean maskValid = true;
//            for (int mask : valueMasks) {
//                if (mask < 0 || mask >= array.length) {
//                    maskValid = false;
//                    break;
//                }
//            }
//            // 2. filter out mask values if mask array is valid
//            if (maskValid) {
//                mMaskedArray = new int[valueMasks.length];
//                int idx = 0;
//                for (int mask : valueMasks) {
//                    mMaskedArray[idx++] = array[mask];
//                }
//            }
//        }
...
//        mConvergenceIndicator = getResources().getDrawable(R.drawable.ic_s3d_conv_indicator);
//        mConvergenceIndicator_red = getResources().getDrawable(R.drawable.ic_s3d_conv_indicator_red);
//        mThumb_blue = getResources().getDrawable(R.drawable.seekbar_thumb_blue);
//        mThumb_red = getResources().getDrawable(R.drawable.seekbar_thumb_red);
