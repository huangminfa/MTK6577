    ...
    public void onMakeCall(final CallbackArgs args) {
        ...
        switch (args.reason) {
            ...
            case CallOptionHelper.MAKE_CALL_REASON_3G_SERVICE_OFF: {
                ...
                if (null != phoneMgr) {
                    try {
                        if (!phoneMgr.isSimInsert(Phone.GEMINI_SIM_1)
                                && !phoneMgr.isSimInsert(Phone.GEMINI_SIM_2)) {
                            bIsInsertSim = false;
                        }
                    } catch(Exception e) {
                        bIsInsertSim = false;
            ...
            case CallOptionHelper.MAKE_CALL_REASON_ASK: {
                ...
                AlertDialog dialog = SimPickerDialog.create(mContext, title, ((Long)args.args).longValue(), internet, this);
                dialog.setOnDismissListener(this);
                mDialogs[args.reason] = dialog;
                mClicked = false;
                dialog.show();
