...
public class RecentsPanelView extends RelativeLayout implements OnItemClickListener, RecentsCallback,
    ...
    public void show(boolean show, boolean animate,
        ...
        if (show) {
            ...
            if (mRecentsNoApps != null) {
                ...
                Log.d(TAG, "show: mRecentsNoApps = " + mRecentsNoApps
                        + ",noRecentTextView = " + noRecentTextView);
				if (noRecentTextView != null) {
                	if (FeatureOption.MTK_THEMEMANAGER_APP) {
                    	int themeMainColor = mContext.getResources().getThemeMainColor();
                    	if (themeMainColor != 0) {
                        	noRecentTextView.setTextColor(themeMainColor);
                    	} else {
            				themeMainColor = mContext.getResources().getColor(android.R.color.holo_blue_light);
            				noRecentTextView.setTextColor(themeMainColor);
            			}
                	}
					noRecentTextView.setText(mContext.getString(R.string.status_bar_no_recent_apps));
				}
    ...
    public void handleLongPress(
        ...
        mPopupMenu = new PopupMenu(mContext, anchorView == null ? selectedView : anchorView);
        mPopupMenu.getMenuInflater().inflate(R.menu.recent_popup_menu, mPopupMenu.getMenu());
    ...
    /**
     * Dismiss the popup menu when the view has been removed.
     */
    private void dismissPopupMenu() {
        Xlog.d(TAG, "dismissPopupMenu here: mPopupMenu = " + mPopupMenu);
        if (mPopupMenu != null) {
            mPopupMenu.dismiss();
            mPopupMenu = null;
        }
