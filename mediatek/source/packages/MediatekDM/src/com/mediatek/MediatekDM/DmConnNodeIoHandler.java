...
//	private String recordToRead = null;

    private String[] item = { "Cmnet/Name", "Cmnet/apn","Cmwap/Name", "Cmwap/apn", "gprs/Addr",
                              "PortNbr", "csd0/Addr","apn","ProxyAddr","ProxyPort"
                            };
    private String[] contentValue = {null, null, null, null, null, null, null};
    //in device
...
//	Uri table = Telephony.Carriers.CONTENT_URI;
    //in eclipse
    String[] projection = {"apn", "name", "name", "apn", "proxy", "port", "csdnum","apn","proxy","port"};

    Uri table = Uri.parse("content://telephony/carriers");
    //Uri tableGemini = Uri.parse("content://telephony/carriers_gemini");
    ...
    public DmConnNodeIoHandler(Context ctx, Uri treeUri, String mccMnc) {
        ...
        registeredSimId = DmCommomFun.getSimRegisterID(ctx);
...
//						Cursor cur = mContext.getContentResolver().query(table,
//								projection,
//								buildSqlString(mccmnc), null, null);
//						int col = cur.getColumnIndex((String) map.get(item[i]));
//						cur.moveToFirst();
    ...
    protected String buildSqlString(String mccMnc) {
        ...
        if (mcc.equals("460") && (mnc.equals("00") || mnc.equals("02") || mnc.equals("07"))) {
            if (uri.getPath().contains(item[0]) || uri.getPath().contains(item[1])) {
                ...
                return "mcc='460' AND type='default,supl,wap,net,dm' AND (mnc='" + mnc + "')";
//                return "mcc='460' AND (type = '' OR type is null) AND (sourcetype='0') AND (mnc='" + mnc + "')";
            }
