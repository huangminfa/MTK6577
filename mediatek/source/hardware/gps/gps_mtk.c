...
#define  GPS_OPEN     "open"
#define  GPS_CLOSE    "close"
#define  GPS_CN0      "cn0"    
...
#define  ERR(f, ...)  XLOGE("%s: " f, __func__, ##__VA_ARGS__)
#define  WAN(f, ...)  XLOGW("%s: " f, __func__, ##__VA_ARGS__)
#if GPS_DEBUG
#  define DBG(f, ...) XLOGD("%s: " f, __func__, ##__VA_ARGS__)
...
int at_command_stop(){
    ...
    int res = 0;
    int ret = 0;
    res = write(sockfd, "$PMTK817,0*20\r\n", sizeof("$PMTK817,0*20\r\n"));
                    
    if (res > 0){        
        DBG("Command send success!\n");
        ret = 0;                
    } else {
        ERR("Command send fail: %s\n", strerror(errno));
        ret = -1;
    }
...
at_command_get_cno(){
   ...
   int i, res = 0;    
    ...
    if(sockfd != -1){
        while((CN0 == -1)&&(retry < 10)){
            retry++;
            //usleep(200000);
            DBG("retry = %d", retry);    	
            if (read(sockfd, buf, sizeof(buf)) < 0){
                ERR("Read CN0 fail!1111");
            } else {
                ptr=strtok(buf, "\r\n");
                if(ptr == NULL)
                {
                ...
                do 
                {
                    if(strncmp(ptr, "$PMTK", 5) == 0)
                    {
                        ...
                        {
                            DBG("ReadCN_Thread: %s", ptr);
                            //get PMTK817
                            if((!strncmp(ptr, "$PMTK817,2", strlen("$PMTK817,2")))){
                                DBG("Found PMTK817,2");
    ...
    if ((CN0 >= 0) && (CN0 <= 255)){
    	  DBG("** AT Command: READ OK **");
    	  char buff[10];
        sprintf(buff,"%d", CN0); 
        at_command_send_ack(buff, sizeof(buff));       
    }else {
        DBG("** AT Command: READ ERROR **");
        char buff[] = "ERROR";
        at_command_send_ack(buff, sizeof(buff)); 
    }
}
...
{   
    ...
    if (!memcmp(command, GPS_OPEN, 4)){
        if(!at_started){
            at_started = 1;    	
        if (-1 == mt3326_gps_start()){
            ERR("mt3326_gps_start err");
            ret = -1;
        }        
        if (-1 == at_command_start()){
            ERR("at_command_start err");
            ret = -1;
        }               
        if (0 == ret){
            DBG("** AT Command gps test start success **");
            char buff[] = "GPS START OK";
            at_command_send_ack(buff, sizeof(buff));
        } else {
            DBG("** AT Command gps test start fail **");
            char buff[] = "GPS START ERROR";
            at_command_send_ack(buff, sizeof(buff));
        }
        }else{
            DBG("GPS driver is running");            
            char buff[] = "GPS START OK";
            at_command_send_ack(buff, sizeof(buff));            
        }    
    }else if(!memcmp(command, GPS_CLOSE, 5)){
        int ret = 0;        
        if(at_started){
        	  at_started = 0;        
        if (at_command_stop()){
            ERR("at_command_stop err");
            ret = -1;
        }
        if (mt3326_gps_stop()){
            ERR("mt3326_gps_stop err");
            ret = -1;
        }        
        if (0 == ret){
            DBG("** AT Command gps test stop success **");
            char buff[] = "GPS STOPPED";
            at_command_send_ack(buff, sizeof(buff));
    }else {
            char buff[] = "GPS STOP ERROR";
            at_command_send_ack(buff, sizeof(buff));
        }
        }else {
            DBG("GPS driver has stopped");
            char buff[] = "GPS STOPPED";
            at_command_send_ack(buff, sizeof(buff));
        }        	
    }else if(!memcmp(command, GPS_CN0, 3)){
        //read CN0       
        at_command_get_cno();
    } else {
        DBG("** AT Command Parse: Not AT Command **");
        char buff[] = "GPS ERROR";
        at_command_send_ack(buff, sizeof(buff));
    }
}
...
gps_state_thread( void*  arg )
{
	...
	strcpy(un.sun_path, "/data/misc/codec_sock");
	if ((sock_codec = socket(AF_UNIX, SOCK_STREAM, 0)) < 0)
	{	
		ERR("create socket for communicate with codec error, message %s\n", strerror(errno));
		return;
	}	
	unlink("/data/misc/codec_sock");
//	size_codec = offsetof(struct sockaddr_un, sun_path) + strlen(un.sun_path);
	size_codec = sizeof(un.sun_family)+strlen(un.sun_path);
	if(bind(sock_codec, (struct sockaddr *)&un, size_codec) < 0)
	{
		ERR("bind fail, message = %s\n", strerror(errno));
		return;
	}
	if(listen(sock_codec, 5) == -1)
	{
		ERR("listern error, message is %s\n", strerror(errno));
		return;	
	}
	DBG("listen done\n");
	int a = chmod("/data/misc/codec_sock", S_IRUSR|S_IWUSR|S_IXUSR|S_IRGRP|S_IWGRP|S_IXGRP);
	DBG("chmod res = %d\n", a);	
	epoll_register(epoll_fd, sock_codec);
#endif
    // register control file descriptors for polling
    epoll_register( epoll_fd, control_fd );
    epoll_register( epoll_fd, gps_fd );
    epoll_register(epoll_fd, atc_fd);
            ...
            if ((events[ne].events & EPOLLIN) != 0) {
                ...
                else if (fd == gps_fd)
                {
                    ...
                    for (;;) {
                        ...
                        DBG("received %d bytes: %.*s", ret, ret, buff);
                        gps_nmea_end_tag = 0;
                        for (nn = 0; nn < ret; nn++)
                        {
                            if(nn == (ret-1))
...
#if NEED_IPC_WITH_CODEC
				else if(fd == sock_codec)
				{
					client_addr_len = sizeof(un);
					int accept_ret = accept(sock_codec, (struct sockaddr*)&un, &client_addr_len);
					if(accept_ret == -1)
					{	
