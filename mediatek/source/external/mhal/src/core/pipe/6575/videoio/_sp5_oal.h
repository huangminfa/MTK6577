...
#define  SP5_VCodecQueryPyhsicalAddr                  VCodecDrvQueryPyhsicalAddr                               
...
void SP5_VCodecQueryPyhsicalAddr(IN void       *pBuffer_VA,                           
...
void SP5_VCodecWaitISR(VCODEC_OAL_ISR_T *prParam);	
...
void VCodecDrvQueryMemType( IN void            *pBuffer_VA,
		                      IN unsigned int    u4Size,
		                      OUT VCODEC_MEMORY_TYPE_T *peMemType);
void VCodecDrvQueryPyhsicalAddr(IN void       *pBuffer_VA,                           
		                         OUT void     **pBufferOut_PA);
int VCodecDrvSwitchMemType(IN void            *pBuffer_VA,
		                     IN unsigned int    u4Size,
		                     IN VCODEC_MEMORY_TYPE_T eMemType,
		                     OUT void           **pBufferOut_VA);
void VCodecDrvFlushCachedBuffer(IN void         *pBuffer_VA,
		                         IN unsigned int u4Size);		                     
void VCodecDrvInvalidateCachedBuffer(IN void         *pBuffer_VA, 
		                              IN unsigned int 	u4Size);		

void VCodecDrvFlushCachedBufferAll();
void VCodecDrvInvalidateCachedBufferAll(); 
void VCodecDrvFlushInvalidateCacheBufferAll();
void  VCodecDrvMemSet(IN void                *pBuffer_VA, 
		                IN char                cValue, 
		                IN unsigned int        u4Length );    
void  VCodecDrvMemCopy(IN void *            pvDest , 
		                 IN const void *     pvSrc , 
		                 IN unsigned int      u4Length);
void VCodecDrvAssertFail( IN char *ptr, 
			                 IN int i4Line, 
		                     IN int i4Arg);
void VCodecDrvMMAP(VCODEC_OAL_MMAP_T *prParam);
void VCodecDrvUnMMAP(VCODEC_OAL_MMAP_T *prParam	);
void VCodecDrvWaitISR(VCODEC_OAL_ISR_T *prParam);	
void VCodecDrvLockHW(VCODEC_OAL_HW_LOCK_T *prParam);
void VCodecDrvUnLockHW(VCODEC_OAL_HW_LOCK_T *prParam);
void VCodecDrvInitHWLock(VCODEC_OAL_HW_REGISTER_T *prParam);
void VCodecDrvDeInitHWLock(VCODEC_OAL_HW_REGISTER_T *prParam);
void VCodecDrvTraceLog0(IN VCODEC_LOG_GROUP_T eGroup, 
		                  IN VCODEC_LOG_INDEX_T eIndex
						   );
void VCodecDrvTraceLog1(IN VCODEC_LOG_GROUP_T eGroup, 
		                  IN VCODEC_LOG_INDEX_T eIndex, 
						    IN UINT64 arg
							);

void VCodecDrvTraceLog2(IN VCODEC_LOG_GROUP_T eGroup,
		                  IN  VCODEC_LOG_INDEX_T eIndex,
		                  IN  UINT64 arg1,
		                  IN  UINT64 arg2
						   );

void VCodecDrvTraceLog4(IN VCODEC_LOG_GROUP_T eGroup,
		                  IN  VCODEC_LOG_INDEX_T eIndex,
		                  IN  UINT64 arg1,
		                  IN  UINT64 arg2,IN  UINT64 arg3,
		                  IN  UINT64 arg4
...
void VCodecDrvTraceLog8(IN VCODEC_LOG_GROUP_T eGroup,
		                  IN  VCODEC_LOG_INDEX_T eIndex,
		                  IN  UINT64 arg1,
		                  IN  UINT64 arg2,
		                  IN  UINT64 arg3, 
...
VAL_VOID_T VCodecDrvMemAllocAligned(VAL_VOID_T *handle, VAL_UINT32_T size, unsigned int u4AlignSize, VCODEC_MEMORY_TYPE_T cachable, VCODEC_BUFFER_T *pBuf, VAL_MEM_CODEC_T eMemCodec);
VAL_VOID_T VCodecDrvMemFree(VAL_VOID_T *handle, VCODEC_BUFFER_T *pBuf);
VAL_VOID_T VCodecDrvIntMalloc(HANDLE  handle, unsigned int size, unsigned int alignedsize, VCODEC_BUFFER_T *prBuffer_adr);
VAL_VOID_T VCodecDrvIntFree(HANDLE handle, VCODEC_BUFFER_T *prBuffer_adr);
VAL_VOID_T VCodecDrvRegSync(int type, unsigned int v, unsigned int a);


//#include "mach/sync_write.h"

#define VCodecDrvREGSYNC_WriteB(v, a) \
				mt65xx_reg_sync_writeb(v, a);

#define VCodecDrvREGSYNC_WriteW(v, a) \
				mt65xx_reg_sync_writew(v, a);

#define VCodecDrvREGSYNC_WriteL(v, a) \
				mt65xx_reg_sync_writel(v, a);

VAL_VOID_T VCodecDrvRegSyncWriteB(VAL_UINT32_T v, VAL_UINT32_T a);
VAL_VOID_T VCodecDrvRegSyncWriteW(VAL_UINT32_T v, VAL_UINT32_T a);
VAL_VOID_T VCodecDrvRegSyncWriteL(VAL_UINT32_T v, VAL_UINT32_T a);
