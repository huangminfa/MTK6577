...
public class DataConnectionReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (DBG) Log.d(TAG, "Receive " + action);
        if (!TelephonyIntents.ACTION_WIFI_FAILOVER_GPRS_DIALOG.equals(action)) return;

        String shutdownRequest = SystemProperties.get("sys.shutdown.requested");
        if (DBG) Log.d(TAG, "shutdownRequest " + shutdownRequest);
        if (shutdownRequest == null || shutdownRequest.length() == 0 || "0".equals(shutdownRequest)) {
            ...
            if (DBG) Log.d(TAG, "Receive " + action + " but shutdowning, skip");
        }
    }
