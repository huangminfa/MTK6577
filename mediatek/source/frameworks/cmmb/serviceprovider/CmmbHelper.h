...
public:
	...
	void Append(UINT8* buffer, UINT32 bufLen)
	{
		assert(buffer != null);
...
#define DEFAULT_MFS_ROOT_DIR "/data/misc/cmmb"
...
public:
	...
	void open()
	{
		...
		char name[260];
        char* storage = null; 
        storage = getenv("EXTERNAL_STORAGE");
        sprintf(name, "%s/cmmb_test%d.mfs", storage, ++num_);
        if (null == storage)
        {
            SP_LOGE("get SD card fail !!!");
            sprintf(name, "%s/cmmb_test%d.mfs", DEFAULT_MFS_ROOT_DIR, ++num_);
        }

		file_ = fopen(name, "wb");
        SP_LOGE("MFS open file: %s, hd=%d, err=%d", name, file_, errno);
	}
