...
#ifndef ANDROID_DEFAULT_CODE
static  int Yusu_Hardware_Fsync_Flag = 0;
static  pthread_mutex_t Fasyc_Mutex;

// add for warning message
static const int NO_MORE_BUUFER_INTERVAL = 20;

// ----added by HP Cheng for logging mixing and buffer time moment--------------------

//#define LOG_MIXING_BUFFER_TIME

#ifdef LOG_MIXING_BUFFER_TIME
...
//add by weiguo to dump pcm data
    static   const char * gaf_track_pcm = "/sdcard/af_track_pcm";
    static   const char * gaf_track_propty = "af.track.pcm";

    static   const char * gaf_mixer_pcm = "/sdcard/af_mixer_pcm.pcm";
    static   const char * gaf_mixer_propty = "af.mixer.pcm";

    static   const char * gaf_record_pcm = "/sdcard/af_record.pcm";
...
{
    int rc;
    LOGD("load_audio_interface %s ",if_name);
    rc = hw_get_module_by_class(AUDIO_HARDWARE_MODULE_ID, if_name, mod);
    LOGD("hw_get_module_by_class rc = %p ",rc);
    if (rc)
        goto out;

    LOGD("audio_hw_device_open ");
...
//add by chipeng , use for fasync calback function
#ifndef ANDROID_DEFAULT_CODE
static void Yusu_Hardware_Callback(int signo)
{
    if (signo==SIGIO)
    {
        pthread_mutex_lock( &Fasyc_Mutex);
        Yusu_Hardware_Fsync_Flag = 1;
        LOGV("Yusu_Hardware_Callback set Yusu_Hardware_Fsync_Flag = 1");
        pthread_mutex_unlock( &Fasyc_Mutex );
    }
}

//add by chipeng , use for set fsync enable
void AudioFlinger::SetFsync(void)
{
    if(mFd == 0){
    ...
    if(mFd)
    {
        ...
        action.sa_handler = Yusu_Hardware_Callback;
        action.sa_flags = 0;
...
status_t AudioFlinger::SetEMParameter(void *ptr, size_t len)
{
    ...
    mPrimaryHardwareDev->SetEMParameter(mPrimaryHardwareDev,ptr,len);
...
status_t AudioFlinger::GetEMParameter(void *ptr, size_t len)
{
    ...
    AutoMutex lock(mHardwareLock);
    mHardwareStatus = AUDIO_SET_PARAMETER;
    mPrimaryHardwareDev->GetEMParameter(mPrimaryHardwareDev,ptr,len);
    mHardwareStatus = AUDIO_HW_IDLE;
    return NO_ERROR;
}
...
status_t AudioFlinger::SetAudioData(int par1,size_t len,void *ptr)
{
    ...
    AutoMutex lock(mHardwareLock);
    mHardwareStatus = AUDIO_SET_PARAMETER;
    mPrimaryHardwareDev->SetAudioData(mPrimaryHardwareDev,par1,len,ptr);
    mHardwareStatus = AUDIO_HW_IDLE;
    return NO_ERROR;
}
...
status_t AudioFlinger::GetAudioData(int par1,size_t len,void *ptr)
{
    ...
    AutoMutex lock(mHardwareLock);
    mHardwareStatus = AUDIO_SET_PARAMETER;
    mPrimaryHardwareDev->GetAudioData(mPrimaryHardwareDev,par1,len,ptr);
    mHardwareStatus = AUDIO_HW_IDLE;
    return NO_ERROR;
}
...
{
    LOGD("SetAudioCommand par1 = %d,par2 = %d ",parameters1,parameters2);
    AutoMutex lock(mHardwareLock);
    mHardwareStatus = AUDIO_SET_PARAMETER;
    mPrimaryHardwareDev->SetAudioCommand(mPrimaryHardwareDev,parameters1,parameters2);
    mHardwareStatus = AUDIO_HW_IDLE;
    return NO_ERROR;
}
...
{
    LOGD("GetAudioCommand par1 = %d",parameters1);
    int value =0;
    AutoMutex lock(mHardwareLock);
    mHardwareStatus = AUDIO_SET_PARAMETER;
    value =mPrimaryHardwareDev->GetAudioCommand(mPrimaryHardwareDev,parameters1);
    mHardwareStatus = AUDIO_HW_IDLE;
    return value;
}
...
{
    LOGD("+AudioFlinger::SetACFPreviewParameter!! ");
    AutoMutex lock(mHardwareLock);
    mHardwareStatus = AUDIO_SET_PARAMETER;
    mPrimaryHardwareDev->SetACFPreviewParameter(mPrimaryHardwareDev,ptr,len);
    mHardwareStatus = AUDIO_HW_IDLE;
    LOGV("-SetACFPreviewParameter");
    return NO_ERROR;
}

status_t AudioFlinger::SetHCFPreviewParameter(void *ptr, size_t len)
{
    LOGD("+AudioFlinger::SetHCFPreviewParameter!! ");
    AutoMutex lock(mHardwareLock);
    mHardwareStatus = AUDIO_SET_PARAMETER;
    mPrimaryHardwareDev->SetHCFPreviewParameter(mPrimaryHardwareDev,ptr,len);
    mHardwareStatus = AUDIO_HW_IDLE;
    LOGV("-SetHCFPreviewParameter");
    return NO_ERROR;
}
...
{
    AutoMutex lock(mHardwareLock);
    return mPrimaryHardwareDev->xWayPlay_Start(mPrimaryHardwareDev,sample_rate);
}
int AudioFlinger::xWayPlay_Stop()
{
    AutoMutex lock(mHardwareLock);
    return mPrimaryHardwareDev->xWayPlay_Stop(mPrimaryHardwareDev);
}
int AudioFlinger::xWayPlay_Write(void *buffer, int size_bytes)
{
    AutoMutex lock(mHardwareLock);
    return mPrimaryHardwareDev->xWayPlay_Write(mPrimaryHardwareDev,buffer,size_bytes);
}
int AudioFlinger::xWayPlay_GetFreeBufferCount()
{
    AutoMutex lock(mHardwareLock);
    return mPrimaryHardwareDev->xWayPlay_GetFreeBufferCount(mPrimaryHardwareDev);
}
int AudioFlinger::xWayRec_Start(int sample_rate)
{
    AutoMutex lock(mHardwareLock);
    return mPrimaryHardwareDev->xWayRec_Start(mPrimaryHardwareDev,sample_rate);
}
int AudioFlinger::xWayRec_Stop()
{
    AutoMutex lock(mHardwareLock);
    return mPrimaryHardwareDev->xWayRec_Stop(mPrimaryHardwareDev);
}
int AudioFlinger::xWayRec_Read(void *buffer, int size_bytes)
{
    AutoMutex lock(mHardwareLock);
    return mPrimaryHardwareDev->xWayRec_Read(mPrimaryHardwareDev,buffer,size_bytes);
}
#endif
// ----------------------------------------------------------------------------
...
//
    ...
    {
        LOGD("Create /data/misc/srs_processing.cfg");
	//check
        struct stat l_stat;
        int lstresult = lstat("/data/misc", &l_stat);
        LOGD("lstrresult=%d,mode_t=%u,uid_t=%u,gid_t=%u",lstresult, l_stat.st_mode, l_stat.st_uid,l_stat.st_gid);
	    if (lstresult == -1)
	    {
	        LOGE("lstat errro,errno=%d",errno);
	    }
        //
        FILE *pf1 = fopen("/data/misc/srs_processing.cfg", "wb");
        if (pf1  == NULL)
        {
           LOGE("can't open file:data/misc/srs_processing.cfg,error=%d",errno);
        }
        else
        {
            fclose(pf1);
            chmod("/data/misc/srs_processing.cfg", S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH); 
            LOGD("chmod /data/misc/srs_processing.cfg:rw-rw-r--");
        }
    }
#endif
//
...
void AudioFlinger::onFirstRef()
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(audio_interfaces); i++) {
        ...
        LOGI("load_audio_interface %s audio interface from", audio_interfaces[i]);
            ...
            for (size_t i = 0; i < mPlaybackThreads.size(); i++) {
                ...
                if (mPlaybackThreads.keyAt(i) != output) {
                    ...
                    if (sessions & PlaybackThread::TRACK_SESSION) {
                        LOGE("session(%d) on different output threads",*sessionId);
                        lStatus = BAD_VALUE;
                        goto Exit;
                    }
...
status_t AudioFlinger::setStreamVolume(int stream, float value, int output)
{
    ...
    if(stream == android_audio_legacy::AudioSystem::FM)
    {
        LOGD("setStreamVolume FM  value = %f",value);
    ...
    // add for MATV analog input device volume setting
#ifdef MATV_AUDIO_LINEIN_PATH
    // add by chipeng for MATV volume
    if(stream == android_audio_legacy::AudioSystem::MATV)
    {
        LOGD("setStreamVolume MATV value = %f",value);
        int MatvVolume = (AudioSystem::logToLinear(value));
        char Volume[30];
        sprintf(Volume,"SetMatvVolume=%d",MatvVolume);
        String8 Key = String8(Volume);
        mPrimaryHardwareDev->set_parameters (mPrimaryHardwareDev,Key);
    }
#endif
...
status_t AudioFlinger::setStreamMute(int stream, bool muted)
{
    ...
    if(stream == android_audio_legacy::AudioSystem::MATV)
    {
        LOGD("setStreamMute MATV muted=%d",muted);
        char mute_vol[30];
...
{
    Mutex::Autolock _l(mLock);
    LOGV("ThreadBase::setEffectSuspended");
        ...
        const effect_uuid_t *type, bool suspend, int sessionId)
{
    ...
    if (chain != 0) {
        if (type != NULL) {
            LOGV("ThreadBase_chain::setEffectSuspended_l");
            chain->setEffectSuspended_l(type, suspend);
        } else {
            LOGV("ThreadBase_chain::setEffectSuspendedAll_l");
...
void AudioFlinger::ThreadBase::checkSuspendOnAddEffectChain_l(const sp<EffectChain>& chain)
{
    ...
    LOGV("ThreadBase::checkSuspendOnAddEffectChain_l");
...
void AudioFlinger::ThreadBase::updateSuspendedSessions_l(const effect_uuid_t *type,
                                                         ...
                                                         int sessionId)
{
    LOGV("ThreadBase::updateSuspendedSessions_l:suspend=%d,sessionId=%d",suspend,sessionId);
...
{
    Mutex::Autolock _l(mLock);
    LOGV("ThreadBase::checkSuspendOnEffectEnabled");
                                                            ...
                                                            int sessionId)
{
    if (mType != RECORD) {
        ...
        if (sessionId != AUDIO_SESSION_OUTPUT_MIX) {
            LOGV("ThreadBase::checkSuspendOnEffectEnabled_l,setEffectSuspended_l");
    ...
    if (chain != 0) {
        LOGV("ThreadBase::checkSuspendOnEffectEnabled_l,checkSuspendOnEffectEnabled");
...
#ifndef ANDROID_DEFAULT_CODE
			LOGW("Use Resampler with sampleRate  =	%d", sampleRate);
#else
        ...
        for (size_t i = 0; i < mTracks.size(); ++i) {
            ...
            if (t != 0) {
                if (sessionId == t->sessionId() &&
                    ...
                    LOGE("type(%d) and type(%d) on same sessionId(%d) has different strategy ",streamType,t->type(),sessionId);
                    lStatus = BAD_VALUE;
                    goto Exit;
                }
            }
...
void AudioFlinger::PlaybackThread::readOutputParameters()
{
    ...
    //LOGV("readOutputParameters mSampleRate=%u,mChannelCount=%u ,mFrameSize=%u,mFrameCount=%u",mSampleRate,mChannelCount,mFrameSize,mFrameCount);
...
#ifndef ANDROID_DEFAULT_CODE
        /* a2dp will block wirte , so it is impossible to cause underruns . Reason::ALPS00249246*/
        return (uint32_t)((uint32_t)((mFrameCount * 1000) / mSampleRate) * 1000)/2;
#else
...
bool AudioFlinger::MixerThread::threadLoop()
{
    ...
    {
        LOGV("mAudioFlinger->SetFsync()");
        mAudioFlinger->SetFsync();
    }
#endif
    //----------------------------------------------------
    ...
    while (!exitPending())
    {
        ...
        { // scope for mLock
            ...
            //modify by chipeng ,judge for Yusu_Hardware_Fsync_Flag and device setting
            if UNLIKELY((!activeTracks.size() && systemTime() > standbyTime) || mSuspended
#ifndef ANDROID_DEFAULT_CODE
                            || (!activeTracks.size()&&Yusu_Hardware_Fsync_Flag && (mDevice ==android_audio_legacy::AudioSystem::DEVICE_OUT_SPEAKER))
#endif
                         )
                {
                if (!mStandby) {
...
#ifdef LOG_MIXING_BUFFER_TIME
            LOGD(" b4 prepareTracks_l [%lld]", getTimeMs());
#endif
            ////////
            mixerStatus = prepareTracks_l(activeTracks, &tracksToRemove);

#ifdef LOG_MIXING_BUFFER_TIME
            LOGD(" after prepareTracks_l [%lld]", getTimeMs());
#endif
        ...
        if (LIKELY(mixerStatus == MIXER_TRACKS_READY)) {
...
#ifndef ANDROID_DEFAULT_CODE
#ifdef DEBUG_AUDIO_PCM
			dumpPCMData(gaf_mixer_pcm,mMixBuffer,mixBufferSize,gaf_mixer_propty);
#endif
#endif
            ...
            if (sleepTime == 0) {
                if (mixerStatus == MIXER_TRACKS_ENABLED) {
                    ...
                    LOGV("no activetracks ready,sleep %d ms " ,sleepTime/1000);
                } else {
                    sleepTime = idleSleepTime;
                }
                ...
                sleepTime = 0;
                LOGV("write mute data to mixerBuffer");
...
{
    if (mDevice  == android_audio_legacy::AudioSystem::DEVICE_OUT_EARPIECE||
         mDevice  == android_audio_legacy::AudioSystem::DEVICE_OUT_SPEAKER||
         mDevice  == android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADSET ||
         mDevice  == android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADPHONE)
    {
        return true;
    }
    else
...
#ifndef ANDROID_DEFAULT_CODE
    if(count &&(mStandby)&&MixerHardwareOutput(mDevice)){
        // this is little tricky , always promote 0 tracksToRemove
        //LOGV("AudioFlinger MixerThread prepareTracks_l ");
        sp<Track> maint = activeTracks[0].promote();
        Track* const maintrack = maint.get();
        audio_track_cblk_t* maincblk = maintrack->cblk();
        uint32_t mixed_samplerate = maincblk->sampleRate;


        // change hardware samplerate
        if(mixed_samplerate == 48000)
        {
            //LOGV("set Audio asm Hardawre to samplerate 48000");
            mAudioFlinger->setParameters(0,(String8)"AudioSetSampleRate=48000");
        }
        else
        {
            //LOGV("set Audio asm Hardawre to samplerate 44100");
            mixed_samplerate = 44100;
            mAudioFlinger->setParameters(0,(String8)"AudioSetSampleRate=44100");
        }


        for(size_t k =0; k < count ; k++)
        {
            sp<Track> t = activeTracks[k].promote();
            if (t == 0)
            {
                LOGE("prepareTracks_l chck samplerate but no avtive track");
                break;
            }
            Track* const track = t.get();
            mAudioMixer->setActiveTrack(track->name());
            // change mixer default output samplerate2502
            mAudioMixer->setParameter(AudioMixer::RESAMPLE,AudioMixer::MIXER_SAMPLE_RATE,(void *)(&mixed_samplerate));
        }
    }
#endif
    ...
    for (size_t i=0 ; i<count ; i++) {
...
#endif
                LOGW_IF(minFrames > cblk->frameCount,"minFrames(%u) > cblk->frameCount(%u)",minFrames,cblk->frameCount);
            }
        }
                ...
                !track->isPaused() && !track->isTerminated())
        {
            ...
            if (track->isMuted() || track->isPausing() ||
                ...
                //LOGD("typeVolume=%f,trackvolue1=%d,trackvoluer=%d,masterVolume=%f,name=%d",typeVolume, cblk->volume[0],cblk->volume[1],masterVolume,track->name());
            }
            // Delegate volume control to effect in track effect chain if needed
...
//FIX CR:ALPS00032499
                ...
                if (--(track->mRetryCount) <= 0) {
                    LOGV("BUFFER TIMEOUT: remove(%d) from active list on thread %p, u=%08x, s=%08x", track->name(), this, cblk->user, cblk->server);
                    tracksToRemove->add(track);
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        if (t->type() == streamType) {
            ...
            if (t->auxEffectId() > 0)
            {
                ...
                if (effect != 0)
                {
                    //LOGD("clear track audio aux effect temp buffer");
                    effect->clearbuf_l();
                }
            }
...
status_t AudioFlinger::PlaybackThread::Track::getNextBuffer(AudioBufferProvider::Buffer* buffer)
{
     ...
     if (LIKELY(framesReady)) {
        ...
        dumpPCMData(fileName,buffer->raw,buffer->frameCount*cblk->frameSize,gaf_track_propty);
#endif
#endif
        return NO_ERROR;
     }
...
getNextBuffer_exit:
     ...
     buffer->frameCount = 0;
#ifndef ANDROID_DEFAULT_CODE
    if(mNoMoreBufferCount % NO_MORE_BUUFER_INTERVAL == 0)
    {
...
bool AudioFlinger::RecordThread::threadLoop()
{
    ...
    /*
        ...
        LOGW("AudioFlinger::MixerThread::threadLoop set priority = %d",result);
        */
    int result = -1;
    ...
    while (!exitPending()) {
...
#ifdef ADROID_DEFAULT_CODE
                        mStartStopCond.broadcast();
#endif
                    }
#ifndef ADROID_DEFAULT_CODE
                    mStartStopCond.broadcast();
#endif
                    mStandby = false;
                }
        ...
        if (mActiveTrack != 0) {
            ...
            if (LIKELY(mActiveTrack->getNextBuffer(&buffer) == NO_ERROR)) {
                ...
                dumpPCMData(gaf_record_pcm, buffer.raw, (mActiveTrack->mCblk->frameSize)*(buffer.frameCount),gaf_record_propty);
#endif
...
void AudioFlinger::releaseAudioSessionId(int audioSession)
{
    Mutex::Autolock _l(mLock);
    int caller = IPCThreadState::self()->getCallingPid();
    LOGV("releasing %d from %d,tid=%d", audioSession, caller,gettid());
    int num = mAudioSessionRefs.size();
    for (int i = 0; i< num; i++) {
        AudioSessionRef *ref = mAudioSessionRefs.itemAt(i);
        if (ref->sessionid == audioSession && ref->pid == caller) {
...
void AudioFlinger::purgeStaleEffects_l() {
    ...
    LOGV("purging stale effects,tid=%d",gettid());
    ...
    for (size_t i = 0; i < chains.size(); i++) {
        ...
        if (!found) {
            ...
            while (ec->mEffects.size()) {
                ...
                LOGV("purgeStaleEffects_l,removeEffect");
...
void AudioFlinger::ThreadBase::disconnectEffect(const sp<EffectModule>& effect,
    ...
    LOGV("disconnectEffect() %p effect %p,tid=%d", this, effect.get(),gettid());
    // delete the effect module if removing last handle on it
    if (effect->removeHandle(handle) == 0) {
        if (!effect->isPinned() || unpiniflast) {
            removeEffect_l(effect);
            LOGV("disconnectEffect() unregisterEffect+");
            AudioSystem::unregisterEffect(effect->id());
            LOGV("disconnectEffect() unregisterEffect-");
        }
    }
...
//FIX CR:ALPS00054923
            ...
            LOGE("Can't find aux effect,EffectId=%d,error=%d",EffectId,status);
#else
            LOGE("No AuxEffect for Effectid(%d)",EffectId);
            status = BAD_VALUE;
#endif
        }
    }
...
{
    LOGV("disconnect() %p handle %p,tid=%d", this, handle.unsafe_get(),gettid());
    ...
    sp<EffectModule> keep(this);
    {
        ...
        if (handle==keep->mHandles[0] && keep->mHandles.size() > 1)
        {
            ...
            LOGV("increase a refcount for mHandles[1],%p",h_next.get());
            sp<ThreadBase> thread = mThread.promote();
            if (thread != 0) {
...
{
    Mutex::Autolock _l(mLock);
    LOGV("EffectModule::setSuspended");
    mSuspended = suspended;
}
bool AudioFlinger::EffectModule::suspended()
{
    Mutex::Autolock _l(mLock);
    LOGV("EffectModule::suspended");
    return mSuspended;
}
        ...
        const effect_uuid_t *type, bool suspend)
{
    ...
    if (suspend) {
        ...
        if (desc->mRefCount++ == 0) {
            ...
            if (effect != 0) {
                desc->mEffect = effect;
                LOGV("EffectChain::setEffectSuspended_l,effect->setSuspended");
        ...
        if (--desc->mRefCount == 0) {
            ...
            if (desc->mEffect != 0) {
                ...
                if (effect != 0) {
                    ...
                    LOGV("setEffectSuspended_l(),false,effect->controlHandle");        
...
{
    LOGV("EffectChain::getSuspendEligibleEffects");
...
void AudioFlinger::EffectChain::checkSuspendOnEffectEnabled(const sp<EffectModule>& effect,
                                                            bool enabled)
{
    LOGV("EffectChain::checkSuspendOnEffectEnabled");
