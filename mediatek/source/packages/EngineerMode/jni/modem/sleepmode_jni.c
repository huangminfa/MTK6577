...
JNIEXPORT jboolean Java_com_mediatek_engineermode_modem_SleepMode_setSleepMode(
	...
	LOGW("open file successful");
	write(file, Message, sizeof(Message));
	LOGW("write file successful");
	LOGW("After set SleepMode: %d %d %d %d", Message[0], Message[1],
			Message[2], Message[3]);
	close(file);
	...
	if (-1 == fileRead) {
		LOGW("open ccci_uem_rx failed");
		return -1;
	}
	LOGW("open ccci_uem_rx successful");
	LOGW("Source: %d %d %d %d", Message[0], Message[1], Message[2], Message[3]);
	read(fileRead, Message, sizeof(Message));
	LOGW("read file successful");
	LOGW("Recv: %d %d %d %d", Message[0], Message[1], Message[2], Message[3]);
	close(fileRead);
...
JNIEXPORT jboolean Java_com_mediatek_engineermode_modem_SleepMode_setDCM(
	...
	LOGW("write file successful");
	LOGW("After set DCM: %d %d %d %d", Message[0], Message[1], Message[2],
			Message[3]);
	close(file);
