...
public abstract class BaseAccountType extends AccountType {
     ...
     * M:AAS
     */
    protected static EditType buildPhoneType(int type, int resId) {
        return new EditType(type, resId);
    }

    protected static EditType buildPhoneType(int type, String resString) {
        return new EditType(type, resString);
    }

    public static void updatePhoneType(int slotId, DataKind kind) {
        kind.typeList = Lists.newArrayList();
        List<AlphaTag> atList = USIMAas.getUSIMAASListWithoutNullTag(slotId);
        final int specificMax = atList.size() + 2;
        Log.d("UsimAccountType", "slot = " + slotId + " specificMax=" + specificMax);
        kind.typeList.add(buildPhoneType(Anr.NONE_TYPE, R.string.aas_phone_type_none).setSpecificMax(
                specificMax));
        for (AlphaTag tag : atList) {
            int recordIndex = tag.getRecordIndex();
            Log.w("UsimAccountType", "addDataKindPhone, label=" + tag.getAlphaTag());
            kind.typeList.add(buildPhoneType(tag.getRecordIndex(), tag.getAlphaTag()));
        }
        kind.typeList.add(buildPhoneType(Anr.CUSTOM_TYPE, com.android.internal.R.string.phoneTypeCustom)
                .setSpecificMax(specificMax));
        kind.typeOverallMax = specificMax * specificMax;

        kind.fieldList = Lists.newArrayList();
        kind.fieldList.add(new EditField(Phone.NUMBER, R.string.aas_phone_additional, FLAGS_PHONE));
    }
    /*
    ...
    protected DataKind addDataKindPhone(Context context) throws DefinitionException {
        ...
        ///M:AAS
        kind.mAccountType = AccountType.ACCOUNT_TYPE_LOCAL_PHONE;
