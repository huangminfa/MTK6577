...
NativeInputChannel::NativeInputChannel(const sp<InputChannel>& inputChannel) :
    ...
    if (mInputChannel != NULL) {
        if (mInputChannel->getSendPipeFd() < 0) {
            LOGD("NativeInputChannel initialization, sendPipeFd error : %d", mInputChannel->getSendPipeFd());
        } else if (inputChannel->getReceivePipeFd() < 0) {
            LOGD("NativeInputChannel initialization, receivePipeFd error : %d", mInputChannel->getReceivePipeFd());
        }
    }
}
...
static jobjectArray android_view_InputChannel_nativeOpenInputChannelPair(JNIEnv* env,
    ...
    if (serverChannel->getSendPipeFd() < 0) {
        LOGD("server's sendPipeFd error : %d", serverChannel->getSendPipeFd());
    } else if (serverChannel->getReceivePipeFd() < 0) {
        LOGD("server's receivePipeFd error : %d", serverChannel->getReceivePipeFd());
    } else if (clientChannel->getSendPipeFd() < 0) {
        LOGD("client's sendPipeFd error : %d", clientChannel->getSendPipeFd());
    } else if (clientChannel->getReceivePipeFd() < 0) {
        LOGD("client's receivePipeFd error : %d", clientChannel->getReceivePipeFd());
    }
...
static void android_view_InputChannel_nativeTransferTo(JNIEnv* env, jobject obj,
    ...
    if(!nativeInputChannel)
        LOGW("InputChannel checker: nativeInputChannel is NULL in natvieTransferTo");
    else {
        if (nativeInputChannel->getInputChannel()->getSendPipeFd() < 0)
            LOGD("nativeTransferTo: error send fd = %d ", nativeInputChannel->getInputChannel()->getSendPipeFd());
        else if (nativeInputChannel->getInputChannel()->getReceivePipeFd() < 0)
            LOGD("nativeTransferTo: error receive fd = %d ", nativeInputChannel->getInputChannel()->getReceivePipeFd());
    }
...
static void android_view_InputChannel_nativeReadFromParcel(JNIEnv* env, jobject obj,
    ...
    if (parcel) {
        ...
        if (isInitialized) {
            ...
            LOGW("InputChannel checker: fail to initialize in nativeReadFromParcel");
        }
    }
}
...
static void android_view_InputChannel_nativeWriteToParcel(JNIEnv* env, jobject obj,
    ...
    if (parcel) {
        ...
        if (nativeInputChannel) {
            ...
            parcel->writeInt32(0);
            LOGW("InputChannel checker: nativeInputChannel is null in nativeWriteToParcel");
        }
    }
}
