...
void ResetHandler::onEvent(int evt) {
    ...
    switch(evt) {
        case CMD_RESET_START:
            if(isRunningReset) {
                LOGE("Running Reset Now, Skip RESET_START Command");
            } else {
                pthread_mutex_lock(&(mCl->mLock));
                LOGI("====WHOLE-CHIP RESET START!====");
                isRunningReset = true;
        ...
        case CMD_RESET_END:
            if(false == isRunningReset) {
                LOGE("Not Running Reset, Skip RESET_END Command");
            } else {
		...
		LOGI("====WHOLE-CHIP RESET END!  ====");
            }
            break;
        case CMD_UNKNOWN:
            LOGD("Unknown command, Skip it!");
            break;
        default:
            //Do nothing
            break;
