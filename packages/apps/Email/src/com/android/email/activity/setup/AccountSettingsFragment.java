...
*/
    ...
    private final Preference.OnPreferenceChangeListener mSyncOptionsPreferenceChangeListener = new Preference.OnPreferenceChangeListener() {
        public boolean onPreferenceChange(Preference preference, Object newValue) {
            ...
            if (HostAuth.SCHEME_EAS.equals(mAccount.mHostAuthRecv.mProtocol)) {
                acct = new android.accounts.Account(mAccount.mEmailAddress, AccountManagerTypes.TYPE_EXCHANGE);
                isEasAccount = true;
            } else {
                acct = new android.accounts.Account(mAccount.mEmailAddress, AccountManagerTypes.TYPE_POP_IMAP);
                isEasAccount = false;
            }
            if (AccountSettingsFragment.PREFERENCE_SYNC_EMAIL.equals(key)) {
                ContentResolver.setSyncAutomatically(acct, EmailContent.AUTHORITY, (Boolean)newValue);
                if ((Boolean) newValue) {
                ...
                ContentResolver.setSyncAutomatically(acct, CalendarProviderStub.AUTHORITY, (Boolean)newValue);
            } else if (AccountSettingsFragment.PREFERENCE_SYNC_CONTACTS.equals(key)) {
                ContentResolver.setSyncAutomatically(acct, ContactsContract.AUTHORITY, (Boolean)newValue);
            }
