...
public class ListView extends AbsListView {
    ...
    protected void layoutChildren() {
        ...
        if (DBG_LAYOUT || DBG_DATACHANGE) {
            Xlog.d(TAG, "layoutChildren start: mDataChanged = " + mDataChanged + ",mItemCount = "
                    + mItemCount + ",mBlockLayoutRequests = " + mBlockLayoutRequests
        ...
        try {
            ...
            if (DBG_LAYOUT) {
                Xlog.d(TAG, "layoutChildren end: mItemCount = " + mItemCount
                        + ",mBlockLayoutRequests = " + mBlockLayoutRequests + ",this = " + this);
            }
            if (!blockLayoutRequests) {
    ...
    private void setupChild(View child, int position, int y, boolean flowDown, int childrenLeft,
        ...
        if (DBG_LAYOUT) {
            Xlog.d(TAG, "setupChild 1: position = " + position + ",child = " + child + ",y = " + y
                    + ",childrenLeft = " + childrenLeft + ",flowDown = " + flowDown
                    + ",selected = " + selected + ",recycled = " + recycled
                    + ",updateChildSelected = " + updateChildSelected
                    + ",child layout requested = " + child.isLayoutRequested() + ",child width = "
                    + child.getMeasuredWidth() + ",height = " + child.getMeasuredHeight()
                    + ",needToMeasure = " + needToMeasure + ",this = " + this);
        }
        ...
        if (needToMeasure) {
            ...
            if (DBG_LAYOUT) {
                Xlog.d(TAG, "setupChild layout children 2 1: position = " + position + ",child = "
                        + child + ",childHeightSpec = " + childHeightSpec + ",childWidthSpec = "
                        + childWidthSpec + ",this = " + this);
            }
            child.measure(childWidthSpec, childHeightSpec);
        } else {
            if (DBG_LAYOUT) {
                Xlog.d(TAG, "setupChild cleanupLayoutState 2 2: position = " + position
        ...
        if (needToMeasure) {
            ...
            if (DBG_LAYOUT) {
                Xlog.d(TAG, "setupChild layout children 3 1: position = " + position + ",child = "
                        + child + ",childrenLeft = " + childrenLeft + ",childRight = " + childRight
                        + ",childTop = " + childTop + ",childBottom = " + childBottom + ",this = "
                        + this);
            }
            child.layout(childrenLeft, childTop, childRight, childBottom);
        } else {
            if (DBG_LAYOUT) {
                Xlog.d(TAG, "setupChild offset children 3 2: position = " + position + ",child = "
                        + child + ",childrenLeft = " + childrenLeft + ",child left = "
                        + child.getLeft() + ",this = " + this);
            }
