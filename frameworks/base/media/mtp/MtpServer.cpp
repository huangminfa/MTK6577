...
void MtpServer::run() {
    ...
    //Added for USB Develpment debug, more log for more debuging help
        ...
        if (ret < 0) {
            LOGD("request read returned %d, errno: %d\n", ret, errno);
            if (errno == ECANCELED) {
            ...
            {
                LOGE("Try to wait for next command until success!! returned %d, errno: %d", ret, errno);
                //Added Modification for ALPS00255822, bug from WHQL test
                LOGE("EIO!! returned %d, errno: %d", ret, errno);
        ...
        LOGD("operation: %s", MtpDebug::getOperationCodeName(operation));
        mRequest.dump();
        ...
        if (handleRequest()) {
            if (!dataIn && mData.hasData()) {
                ...
                ret = mData.write(fd);
                if (ret < 0) {
                    LOGE("%s, line %d: request write returned %d, errno: %d", __func__, __LINE__, ret, errno);
                    if (errno == ECANCELED) {
                        // return to top of loop and wait for next command
                        continue;
                    }
            ...
            LOGD("sending response %04X", mResponse.getResponseCode());
...
void MtpServer::sendStoreAdded(MtpStorageID id) {
    LOGD("sendStoreAdded %08X\n", id);
    sendEvent(MTP_EVENT_STORE_ADDED, id);
}

void MtpServer::sendStoreRemoved(MtpStorageID id) {
    LOGD("sendStoreRemoved %08X\n", id);
...
bool MtpServer::handleRequest() {
    ...
    switch (operation) {
        ...
        case MTP_OPERATION_GET_OBJECT_PROP_LIST:
            LOGE("got MTP_OPERATION_GET_OBJECT_PROP_LIST command %s, line %d \n", MtpDebug::getOperationCodeName(operation), __LINE__);
        ...
        case MTP_OPERATION_END_EDIT_OBJECT:
            ...
            break;
        default:
            LOGE("got unsupported command %s \n", MtpDebug::getOperationCodeName(operation));
            //Added Modification for ALPS00255822, bug from WHQL test
            sxlog_printf(ANDROID_LOG_ERROR, "MtpServer",
...
MtpResponseCode MtpServer::doGetDeviceInfo() {
    ...
    LOGD("doGetDeviceInfo property_get, ro.sys.usb.mtp.whql.enable, property_get = %s, \n",prop_value);
    if(!(strcmp(prop_value, "1")))
    {
        LOGD("doGetDeviceInfo property_get, WHQL enable!!\n");
        mData.putAUInt16(kWHQLSupportedOperationCodes,
                sizeof(kWHQLSupportedOperationCodes) / sizeof(uint16_t)); // Operations Supported
    }
    else
    {
        LOGD("doGetDeviceInfo property_get, WHQL disable!!\n");
    //Added Modification for ALPS00255822, bug from WHQL test    
    ...
    //ALPS00120037, add log for support debug
    LOGD("doGetDeviceInfo property_get, ro.product.manufacturer, property_get = %s, \n",prop_value);
    //ALPS00120037, add log for support debug
    ...
    string.set(prop_value);
    //ALPS00120037, add log for support debug
    LOGD("doGetDeviceInfo property_get, ro.product.model, property_get = %s, \n",prop_value);
    //ALPS00120037, add log for support debug
    ...
    string.set(prop_value);
    //ALPS00120037, add log for support debug
    LOGD("doGetDeviceInfo property_get, ro.serialno, property_get = %s, \n",prop_value);
    //ALPS00120037, add log for support debug
...
MtpResponseCode MtpServer::doGetObjectPropValue() {
    ...
    LOGD("GetObjectPropValue 0x%x %s\n", handle,
    ...
    {
        LOGD("%s: handle = 0x%x, get the path result\n",  __func__, handle, result);
        return MTP_RESPONSE_INVALID_OBJECT_HANDLE;
    }
    //Added Modification for ALPS00255822, bug from WHQL test
...
MtpResponseCode MtpServer::doSetObjectPropValue() {
    ...
    MtpObjectHandle handle = mRequest.getParameter(1);
    MtpObjectProperty property = mRequest.getParameter(2);
    LOGD("SetObjectPropValue 0x%x %s\n", handle,
            MtpDebug::getObjectPropCodeName(property));
...
MtpResponseCode MtpServer::doGetObjectPropList() {
    ...
    //Added Modification for ALPS00255822, bug from WHQL test
    {    
        LOGE("%s: No storage or bad storage!! \n", __func__);
    //Added Modification for ALPS00255822, bug from WHQL test
        return MTP_RESPONSE_INVALID_OBJECT_HANDLE;
    //Added Modification for ALPS00255822, bug from WHQL test
    }
...
/*   LOGD("GetObjectPropList %d format: %s property: %s group: %d depth: %d\n",
    ...
    LOGD("GetObjectPropList handle: 0x%x, format: %s, property: %s, group: %d, depth: %d\n",
            handle, MtpDebug::getFormatCodeName(format),
            MtpDebug::getObjectPropCodeName(property), groupCode, depth);
    //ALPS00120037, add log for support debug
...
MtpResponseCode MtpServer::doGetObject() {
    ...
    //Added for USB Develpment debug, more log for more debuging help
    LOGD("doGetObject filePath = %s \n", filePath);
    LOGD("%s: mfr.length = %d \n", __func__, mfr.length);
    LOGD("%s: mfr.transaction_id = %d \n", __func__, mfr.transaction_id);
    //Added for USB Develpment debug, more log for more debuging help
    ...
    LOGD("%s: MTP_SEND_FILE_WITH_HEADER returned %d\n", __func__, ret);
    close(mfr.fd);
...
MtpResponseCode MtpServer::doGetPartialObject(MtpOperationCode operation) {
    ...
    //Added for USB Develpment debug, more log for more debuging help
    LOGD("doGetPartialObject filePath = %s \n", filePath);
    //Added for USB Develpment debug, more log for more debuging help
...
MtpResponseCode MtpServer::doSendObject() {
    ...
    if (mSendObjectFileSize - initialData > 0) {
        ...
        LOGD("MTP_RECEIVE_FILE returned %d\n", ret);
    }
    close(mfr.fd);
...
//Added Modification for ALPS00255822, bug from WHQL test
        if (errno == ECANCELED)
        {    
            LOGE("%s: MTP_RECEIVE_FILE return MTP_RESPONSE_TRANSACTION_CANCELLED = 0x%x\n", __func__, MTP_RESPONSE_TRANSACTION_CANCELLED);
            //mSendObjectHandle = kInvalidObjectHandle;
            //mSendObjectFormat = 0;
            //return MTP_RESPONSE_TRANSACTION_CANCELLED;
        ...
        {    
            LOGE("%s: MTP_RECEIVE_FILE return MTP_RESPONSE_INCOMPLETE_TRANSFER = 0x%x\n", __func__, MTP_RESPONSE_INCOMPLETE_TRANSFER);
            result = MTP_RESPONSE_TRANSACTION_CANCELLED;
        }
        else if(ret == -8199)
        {    
            LOGE("%s: MTP_RECEIVE_FILE return MTP_RESPONSE_INCOMPLETE_TRANSFER 2 = 0x%x\n", __func__, MTP_RESPONSE_INCOMPLETE_TRANSFER);
            result = MTP_RESPONSE_TRANSACTION_CANCELLED;
        }
        else
        {    
            LOGE("%s: MTP_RECEIVE_FILE return MTP_RESPONSE_GENERAL_ERROR = 0x%x\n", __func__, MTP_RESPONSE_GENERAL_ERROR);
            result = MTP_RESPONSE_GENERAL_ERROR;
        }
        //Added Modification for ALPS00255822, bug from WHQL test
...
//Added Modification for ALPS00255822, bug from WHQL test
    ...
    if (handle == MTP_PARENT_ROOT)
    {
        ...
        if(handles)
        {
            ...
            //Added Modification for ALPS00255822, bug from WHQL test
            LOGD("%s: Delete handlelist size = %d\n", __func__, length);
            //Added Modification for ALPS00255822, bug from WHQL test
...
//Added Modification for ALPS00255822, bug from WHQL test
    ...
    if (result == MTP_RESPONSE_OK) {
        LOGD("deleting %s", (const char *)filePath);
...
MtpResponseCode MtpServer::doSendPartialObject() {
    ...
    LOGD("receiving partial %s %lld %lld\n", filePath, offset, length);
