...
bool NotifySender::threadLoop()
{
    while(!exitPending())
    {
        ...
        if(p)
        {
            p->mOwner->notify(p->mMsg,p->mExt1,p->mExt2,p->mPlayerID,p->mParcel);
            delete p;
        }
...
status_t NotifySender::sendMessage(sp<IMediaPlayerClient> owner, int msg, int ext1, int ext2,int playerId,const Parcel * obj)
{
    SXLOGD("sendMessage::owner(%p),msg(%d)",owner.get(),msg);
    Mutex::Autolock _l(mLock);
    NotifyMessage * pMsg = new NotifyMessage(owner,msg,ext1,ext2,playerId,obj);
