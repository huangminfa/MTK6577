   ...
   jobject o)
{
	 ...
	 e->SetByteArrayRegion(buf, 0, getNdefMessageLength(),
	    ...
	    LogInformation(" com_android_nfc_NativeNfcTag_doRead %s .", (char *) getNdefMessagebuffer());
	 return buf;
}
   ...
   jobject o, jbyteArray buf)
{
	...
	if(ndefMessageLength==0){
		  LogInformation("com_android_nfc_NativeNfcTag_doWrite: Nothing to write");
		return JNI_FALSE;
	}else{
...
static jboolean com_android_nfc_NativeNfcTag_doNdefFormat(JNIEnv *e, jobject o, jbyteArray key)
{
	...
	result = WNDEFWriteMessageSync (handle, NULL, W_NDEF_ACTION_BIT_FORMAT_ALL);
	if (result == W_ERROR_RF_COMMUNICATION)
		{
		setInitialTag();
		}
...
static jboolean com_android_nfc_NativeNfcTag_doMakeReadonly(JNIEnv *e, jobject o)
{
			...
			if(ndefMessageLength==0){
				result = WNDEFWriteMessageSync (handle, NULL, W_NDEF_ACTION_BIT_LOCK);
