...
static int tvoutIoctl(int code, unsigned int value = 0)
{
    ...
    if (fd >= 0) {
        ...
        if (ret == -1) {
            LOGE("[TVOut] [%s] failed. ioctlCode: %d, errno: %d",
                 __func__, code, errno);
            return -1;
        }
        close(fd);
    } else {
        LOGE("[TVOut] [%s] open TVout failed. errno: %d", __func__, errno);
        return -1;
    }
...
int DoTVoutShowPattern(AudioCmdParam &mAduioCmdParams)
{
   ...
   int id = mAduioCmdParams.param1;
    ...
    if (pattern == NULL) {
        LOGE("[TVOut] pattern can not be used");
        goto End;
    }

    if (pattern->checkID(id) == false) {
        LOGE("[TVOut] pattern id error %d", id);
        delete pattern;
        goto End;
    }
    ...
    if (patInfo.fmt == TVOUT_FMT_RGB565) {
        ...
        LOGE("[TVOut] unsupport pattern");
        delete pattern;
        goto End;
    }

    if (*patBufAddr == NULL) {
        LOGI("allocate pat buffer 0x%x", patBufSize);
        *patBufAddr = (unsigned char*)malloc(patBufSize);
        if (*patBufAddr == NULL) {
            LOGE("[TVOut] JNI buffer can not be allocated");
            delete pattern;
            goto End;
        }
    }

    LOGD("%s() id=%d, patBuf=0x%x, size=0x%x\n", __func__, id, *patBufAddr, patBufSize);
...
int DoTVoutLeavePattern(AudioCmdParam &mAduioCmdParams)
{
    ...
    if (patBufRGB != NULL) {
        LOGI("free RGB pat buffer");
        free(patBufRGB);
        patBufRGB = NULL;
    }

    if (patBufYUV != NULL) {
        LOGI("free YUV pat buffer");
        free(patBufYUV);
        patBufYUV = NULL;
    }
