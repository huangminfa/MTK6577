...
public class WifiP2pService extends IWifiP2pManager.Stub {
    ...
    private static final int P2P_CAP_GO = 9;
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_INVITATION_RECEIVED_EVENT:
                    ...
                    setGroupOwnerCap(groupOwner.deviceAddress);
                    updateDeviceStatus(groupOwner.deviceAddress, WifiP2pDevice.CONNECTED);
                    sendP2pPeersChangedBroadcast();
    ...
    private void setGroupOwnerCap(String deviceAddress) {
        for (WifiP2pDevice d : mPeers.getDeviceList()) {
            if (d.deviceAddress.equals(deviceAddress)) {
                d.groupCapability = P2P_CAP_GO;
                logd("PEER_MESSAGE: setGroupOwnerCap is GO ? " + deviceAddress + " = "+ isGroupOwner(deviceAddress));
                return;
            }
        }
        logd("There is no GO device in the peerlist");
    }
