...
{
	LOGD("ATAudioPlayer constructor");
...
{
	LOGD("ATAudioPlayer::startAudioPlayer in +");
...
{
	LOGD("ATAudioPlayer::stopAudioPlayer in +");
...
{
	LOGD("ATAudioPlayer received message: msg=%d, ext1=%d, ext2=%d", msg, ext1, ext2);
	switch(msg)
	{
	  case MEDIA_PLAYBACK_COMPLETE:
		...
		LOGD("ATAudioPlayer::notify -- audio playback complete");
	  	break;
	  case MEDIA_ERROR:
	  	if (m_MediaPlayerClient.get()!=NULL) {
	  		m_MediaPlayerClient->reset();
...
{
	LOGD("audio cmd handler handles cmdline:%s", cmdline);
  ...
  int actinoID = 0, err = 0;
	...
	switch(at_op){
		...
		case AT_SET_OP:
			...
			LOGD("audio_mp3_handler action ID:%d", actinoID);
			...
			if(actinoID == 0){
				...
				LOGD("audio_mp3_handler before start audio player");
	...
	switch(at_op){
	  case AT_ACTION_OP:
		...
		case AT_TEST_OP:
			...
			LOGD("audio cmd handler get loopback mode:%s", keyValuePairs.string());
...
int audio_volume_control_handler(char* cmdline, ATOP_t at_op, char* response) 
{
	...
	LOGD("audio cmd handler get phone mode:%s", keyValuePairs.string());
	strcpy(sKeyParams, keyValuePairs.string());
  at_tok_start_flag(&pParam, '=');
	...
	switch(at_op){
		...
		case AT_TEST_OP:
			if (mPhoneMode==AUDIO_MODE_IN_CALL || mPhoneMode==AUDIO_MODE_IN_COMMUNICATION || Acoustic_Loopback_On){
				  ...
				  LOGD("audio cmd handler get voice mute:%s", keyValuePairs.string());
	        strcpy(sKeyParams, keyValuePairs.string());
	        pParam = sKeyParams;
          at_tok_start_flag(&pParam, '=');
          ...
          if (!bMusic_FM_Mute) {
              keyValuePairs = AudioSystem::getParameters(0, String8("GetVoiceVolume"));
				      LOGD("audio cmd handler get voice volume:%s", keyValuePairs.string());
				      sscanf(keyValuePairs.string(),"%[a-zA-Z=]%f", sKeyParams, &mVoiceVolume);
				      LOGD("audio cmd handler get voice volume:%f", mVoiceVolume);
          ...
          AudioSystem::getStreamMute(AUDIO_STREAM_MUSIC, &bMusic_FM_Mute);
		      AudioSystem::getStreamVolumeIndex(AUDIO_STREAM_MUSIC, &mStreamVolumeIndex);
		      if(bMusic_FM_Mute) {
	  		      sprintf(response,"\r\n0\r\n\r\nOK\r\n");
		...
		case AT_SET_OP:
			...
			if (mPhoneMode==AUDIO_MODE_IN_CALL || mPhoneMode==AUDIO_MODE_IN_COMMUNICATION || Acoustic_Loopback_On) {
			    ...
			    AudioSystem::setStreamMute(AUDIO_STREAM_FM, true);
				  ...
				  AudioSystem::setStreamVolumeIndex(AUDIO_STREAM_MUSIC, mStreamVolumeValue[actinoID][0]);
				  AudioSystem::setStreamMute(AUDIO_STREAM_FM, false);
				  AudioSystem::setStreamVolumeIndex(AUDIO_STREAM_FM, mStreamVolumeValue[actinoID][1]);
			}
