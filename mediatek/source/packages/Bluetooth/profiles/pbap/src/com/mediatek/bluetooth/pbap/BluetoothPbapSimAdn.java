...
public class BluetoothPbapSimAdn{
    ...
    private static int mErrCode = ResponseCodes.OBEX_HTTP_OK;
    ...
    public class AdnComparator implements Comparator<AdnRecord>{
        ...
        public int compare (AdnRecord adn1, AdnRecord adn2){
            ...
            if(name1 == null) name1 = mEmpty;
            if(name2 == null) name2 = mEmpty;
    ...
    public boolean updateAdn(){
        Cursor cursor = null;
        Uri uri = null;
        int indexColIdx, nameColIdx, numberColIdx, emailsColIdx, anrColIdx, groupColIdx;
        ...
        if(cursor != null){
            ...
            while(!cursor.isAfterLast()){
                ...
                mListAdn.add(new AdnRecord(0,
                                        cursor.getInt(indexColIdx),
                                        cursor.getString(nameColIdx),
                                        cursor.getString(numberColIdx),
                                        cursor.getString(anrColIdx),
                                        emails,
                                        null));
                cursor.moveToNext();
            }
    ...
    public int composeVCard(boolean vcard21, int listOffset, int maxListCount, boolean email, boolean incOwner){
        ...
        builder = new VCardBuilder(vcard21?VCardConfig.VCARD_TYPE_V21_GENERIC:VCardConfig.VCARD_TYPE_V30_GENERIC, "UTF-8");
        if(incOwner){
            if(listOffset == 0){
                if(maxListCount > 0){
    ...
    private String getOwnerRecord(VCardBuilder builder){
        String ownName = null, ownNumber = null;
        TelephonyManager tm = (TelephonyManager)mContext.getSystemService(Context.TELEPHONY_SERVICE);
        int slotId = getDefaultSIM();
