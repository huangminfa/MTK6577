...
#undef LOG_TAG
#define LOG_TAG "PvAacDec"

#define PV_AAC_LOG(fmt, arg...) SXLOGD(fmt, ##arg)
#endif
...
#ifndef ANDROID_DEFAULT_CODE
    PV_AAC_LOG("Audio Object Type = %d", audioObjectType);
                ...
                /*(audioObjectType == MP4AUDIO_TWINVQ)*/) && (status == SUCCESS))
    {
        ...
        if ((pVars->mc_info.audioObjectType != MP4AUDIO_AAC_LC) &&
           ...
           (pVars->mc_info.audioObjectType != MP4AUDIO_LTP) &&
            (pVars->mc_info.audioObjectType != MP4AUDIO_AAC_MAIN))
#else
           (pVars->mc_info.audioObjectType != MP4AUDIO_LTP))
#endif
