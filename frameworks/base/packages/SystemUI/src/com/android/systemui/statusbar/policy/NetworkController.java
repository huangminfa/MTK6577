...
public class NetworkController extends BroadcastReceiver {
    ...
    private final void updateTelephonySignalStrength() {
        if (!hasService()) {
            ...
            if (mSignalStrength == null) {
		        ...
		        Slog.d(TAG, "athens iconLevel="+iconLevel+ " mInetCondition= "+mInetCondition+" IS_CMCC="+IS_CMCC);
                if (isCdma()) {
                    if (isCdmaEri()) {
                    ...
                    // Though mPhone is a Manager, this call is not an IPC
                        ...
                        if (mPhone.isNetworkRoaming()) {
	                        iconList = TelephonyIcons.TELEPHONY_CMCC_SIGNAL_STRENGTH_ROAMING[mInetCondition];
	                    } else {
	                        iconList = TelephonyIcons.TELEPHONY_CMCC_SIGNAL_STRENGTH[mInetCondition];
	                    }
                	} else {
	                    if (mPhone.isNetworkRoaming()) {
	                        iconList = TelephonyIcons.TELEPHONY_SIGNAL_STRENGTH_ROAMING[mInetCondition];
				...
				if (IS_CMCC){
	                mContentDescriptionPhoneSignal = mContext.getString(
	                        AccessibilityContentDescriptions.PHONE_CMCC_SIGNAL_STRENGTH[iconLevel]);
					mDataSignalIconId = TelephonyIcons.CMCC_DATA_SIGNAL_STRENGTH[mInetCondition][iconLevel];
				} else {
	                mContentDescriptionPhoneSignal = mContext.getString(
    ...
    private final void updateDataNetType() {
        if (CHATTY) {
            Slog.d(TAG, "updateDataNetType(): mShowAtLeastThreeGees = " + mShowAtLeastThreeGees);
        }
        if (mIsWimaxEnabled && mWimaxConnected) {
        	...
        	Xlog.d(TAG, "updateDataNetType, DataNetType=" + mDataNetType + ".");
            switch (mDataNetType) {
        ...
        if (!isCdma()) {
            ...
            if (mSimState == IccCard.State.READY || mSimState == IccCard.State.UNKNOWN) {
                if (hasService() && mDataState == TelephonyManager.DATA_CONNECTED) {
                    ...
                    // The data network type will be displayed even if date connection is switched to off according to OP01 spec
                    if (hasService() && IS_CMCC) {
                        iconId = mDataIconList[0];
                        Slog.d(TAG, "updateDataIcon(): for OP01 project data network type is shown even if switch off data connection.");                            
                    }
                }
            } else {
    ...
    private void updateConnectivity(Intent intent) {
        ...
        Slog.d(TAG, "athens updateConnectivity: connectionStatus=" + connectionStatus);
    ...
    void refreshViews() {
        ...
        if (!mHasMobileDataFeature) {
            ...
            if (mDataConnected) {
                ...
                if(IS_CU) {
                	switch (mDataActivity) {
                    case TelephonyManager.DATA_ACTIVITY_IN:
                        mMobileActivityIconId = R.drawable.zzz_stat_sys_signal_in;
                        break;
                    case TelephonyManager.DATA_ACTIVITY_OUT:
                        mMobileActivityIconId = R.drawable.zzz_stat_sys_signal_out;
                        break;
                    case TelephonyManager.DATA_ACTIVITY_INOUT:
                        mMobileActivityIconId = R.drawable.zzz_stat_sys_signal_inout;
                        break;
                    default:
	                ...
	                switch (mDataActivity) {
	                    ...
	                    default:
	                        mMobileActivityIconId = 0;
	                        if (IS_CMCC) {
	                            Slog.d(TAG, "refreshViews(): mDataActivity = " + mDataActivity);
	                            mMobileActivityIconId = R.drawable.stat_sys_signal_not_inout;
	                        }
	                        break;
	                }
                }
                ...
                mMobileActivityIconId = 0;
                Slog.d(TAG, "refreshViews(): mDataConnected = " + mDataConnected + " mDataActivity = " + mDataActivity + ".");
            }
        }
        ...
        if (mAirplaneMode &&
            ...
            if ((isCdma() && isCdmaEri()) || mPhone.isNetworkRoaming()) {
                ...
                if (IS_CMCC
                    ...
                    Slog.d(TAG, "refreshViews(): mDataTypeIconId = " + mDataTypeIconId);                    
        ...
        if (mLastDataTypeIconId != mDataTypeIconId) {
            ...
            Slog.d(TAG, "refreshViews(): mLastDataTypeIconId = " + mLastDataTypeIconId + " N = " + N + ".");
            for (int i=0; i<N; i++) {
