...
failto_alloc_usb_request:
    free_pages((unsigned long)t->buffer, get_order(bufsz));
    kfree(t);
    return NULL;
}
...
free:
        ...
        if (t->urb)
            usb_free_urb(t->urb);
        usb_ep_free_request(transfer->upstream.ep, t->req);
        free_pages((unsigned long)t->buffer, get_order(t->buffer_length));
        kfree(t);
...
static void upstream_second_stage(struct urb *urb) {
    ...
    if ((urb->actual_length % transfer->upstream.ep->maxpacket) == 0)
        req->zero = 1;
    else
        req->zero = 0;
