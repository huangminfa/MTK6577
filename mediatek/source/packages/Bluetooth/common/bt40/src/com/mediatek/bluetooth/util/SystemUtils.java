...
public class SystemUtils {
	...
	public static String getReceivedFilePath( Context context ){
		...
		try {
			...
			if( cursor != null && cursor.moveToFirst() ){
				...
				result = cursor.getString( cursor.getColumnIndex( FM_KEY_PATH ) );
			}
	...
	/**
	 * get the corresponding external-storage-directory ( "/mnt/sdcard" or "/mnt/sdcard/sdcard2" ) for the specified filename.
	 * 
	 * @param filename
	...
	/**
	 * check whether the external storage for the given filename is under "MOUNTED" state (writable).
	 * this function will exclude "UN-MOUNTING" from the original "MOUNTED" state but we need to call startMonitor() before EJECT event.
	 * 
	 * if the given filename is not in any external storage directory => return false
	 * 
	 * @param filename
	 * @return
	 */
	public static boolean isExternalStorageMounted( Context context, String filename ){
		...
		if( Environment.MEDIA_MOUNTED.equals(state) ){
			...
			if( Options.LL_DEBUG ){
				BtLog.d( "SystemUtils.isExternalStorageMounted(): check unmounting[" + storageDirPath + "][" + (!BluetoothReceiver.UNMOUNTING_STORAGE_SET.contains(storageDirPath)) + "]" );
			}
			return (!BluetoothReceiver.UNMOUNTING_STORAGE_SET.contains(storageDirPath));
		}
		return false;
	}
	
	/**
	 * get the writable available size for the given filename
	 * please make sure the directory is already exist.
	 * 
	 * @param filename
	...
	public static long getStorageAvailableSize( Context context, String filename ){
		...
		if( !ff.isDirectory() ){
			...
			ff = ff.getParentFile();
			if( ff == null )	return -1;	// no parent folder
		}
	...
	private static File createNewFileForSaving( int level, String expectedFilePath ){
		...
		if( file.exists() ){
			...
			level++;
			StringBuilder buffer = new StringBuilder(filename.length()).append( basename ).append( '_' ).append( level ).append( extension );
	...
	public static Intent getOpenFileIntent( Context context, String filename, String mimeType ){
		...
		if( filename == null ){
			...
			return MessageActivity.createIntent( context,
					context.getString( R.string.bt_share_openfile_title ),
					context.getString( R.string.bt_share_openfile_message_invalid_param, filename, mimeType ),
					context.getString( R.string.bt_share_openfile_confirm ) );
		}
		...
		if( !file.exists() ){
			...
			return MessageActivity.createIntent( context,
					context.getString( R.string.bt_share_openfile_title ),
					context.getString( R.string.bt_share_openfile_message_resource_not_found, filename ),
					context.getString( R.string.bt_share_openfile_confirm ) );
	        }
		...
		if( !isSupportedDataType( context, path, mimeType ) ){
			...
			return MessageActivity.createIntent( context,
					context.getString( R.string.bt_share_openfile_title ),
					context.getString( R.string.bt_share_openfile_message_unsupported_type, mimeType ),
					context.getString( R.string.bt_share_openfile_confirm ) );
		}
		else {
	...
	/**
	 * Check Bluetooth is allowed to be used under current config
	 * 
	 * Currently not used in app package -> should be used in Settings (BluetoothAdapter.ACTION_REQUEST_ENABLE)
	 */
...
//		boolean isAirplaneSensitive = airplaneModeRadios == null ? true : airplaneModeRadios.contains( Settings.System.RADIO_BLUETOOTH );
//		if( !isAirplaneSensitive )	return true;
//
//		String airplaneModeToggleableRadios = Settings.System.getString(resolver, Settings.System.AIRPLANE_MODE_TOGGLEABLE_RADIOS);
//		boolean isAirplaneToggleable = airplaneModeToggleableRadios == null ? false : airplaneModeToggleableRadios.contains(Settings.System.RADIO_BLUETOOTH);
