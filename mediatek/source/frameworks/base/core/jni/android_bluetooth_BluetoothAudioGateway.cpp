...
void CONVERT_BDADDR2STRING(const MTK_BD_ADDR *source, char *dest)
{
    ...
    sprintf(dest, "%02X:%02X:%02X:%02X:%02X:%02X",
        ...
        source->addr[1],        
        source->addr[0]);        
    LOGI("[BT][HFG][API] CONVERT_BDADDR2STRING : 0x%02X%02X%02X%02X%02X%02X==>%s",    
        source->addr[0],    
        source->addr[1],    
        source->addr[2],    
        source->addr[3],    
...
{    
    ...
    LOGI("[BT][HFG][API] CONVERT_STRING2BDADDR : %s==>0x%02X%02X%02X%02X%02X%02X",    
        source,     
        dest->addr[0],    
        dest->addr[1],    
        dest->addr[2],    
...
#if defined(__BTMTK__) && defined(__BT_HFG_PROFILE__)
    /* in */
    LOGI("[BT][HFG][API] classInitNative");
...
static void initializeNativeDataNative(JNIEnv* env, jobject object) {
    ...
    if (NULL == nat) {
        LOGE("[BT][HFG]%s: out of memory!", __FUNCTION__);
        return;
    }
    ...
    LOGV("[BT][HFG]HF RFCOMM channel = %d.", nat->hf_ag_rfcomm_channel);
    LOGV("[BT][HFG]HS RFCOMM channel = %d.", nat->hs_ag_rfcomm_channel);
...
#if defined(__BTMTK__) && defined(__BT_HFG_PROFILE__)
    native_data_t *nat = (native_data_t *)calloc(1, sizeof(native_data_t));
    LOGI("[BT][HFG][API] initializeNativeDataNative");
    if (NULL == nat) {
        LOGE("[BT][HFG]%s: out of memory!", __FUNCTION__);
        return;
    }
    ...
    if(!nat->lib_handle)
    {
        LOGE("[BT][HFG][ERR] Can not load /system/lib/libmtkbtextadp.so : %s", dlerror());
        return;
    }
...
#if defined(__BTMTK__) && defined(__BT_HFG_PROFILE__)
    native_data_t *nat = get_native_data(env, object);
    LOGI("[BT][HFG][API] cleanupNativeDataNative");
    if (nat) 
    {
        if(nat->lib_handle)
        {
...
static int set_nb(int sk, bool nb) {
    ...
    if (flags < 0) {
        LOGE("[BT][HFG]Can't get socket flags with fcntl(): %s (%d)", 
    ...
    if (status < 0) {
        LOGE("[BT][HFG]Can't set socket to nonblocking mode with fcntl(): %s (%d)",
             strerror(errno), errno);
        close(sk);
        return -1;
    }
...
static int do_accept(JNIEnv* env, jobject object, int ag_fd,
    ...
    if (nsk < 0) {
        LOGE("[BT][HFG]Error on accept from socket fd %d: %s (%d).",
             ag_fd,
             strerror(errno),
             errno);
#if USE_ACCEPT_DIRECTLY==0
    ...
    LOGI("[BT][HFG]Successful accept() on AG socket %d: new socket %d, address %s, RFCOMM channel %d",
         ag_fd,
         nsk,
         addr,
         raddr.rc_channel);
...
{
    int res = -1;
    LOGI("[BT][HFG][FLOW] handleIndication : ind=%lu", ilm->msg_id);
    switch(ilm->msg_id)
    {
    case MSG_ID_BT_HFG_RFCOMM_CONNECTED_IND:
    {
        ...
        LOGI("[BT][HFG][MSG] MSG_ID_BT_HFG_RFCOMM_CONNECTED_IND : address=%s, activeContext=%p", address, nat->activeContext);
    ...
    case MSG_ID_BT_HFG_CONNECT_REQ_IND:
    {
        ...
        nat->activeContext = (HfgChannelContext*)ind_p->user_context;
        CONVERT_BDADDR2STRING(&ind_p->bt_addr, address);
        LOGI("[BT][HFG][MSG] MSG_ID_BT_HFG_CONNECT_REQ_IND : address=%s, activeContext=%p", address, nat->activeContext);
        env->CallVoidMethod(object,
    ...
    case MSG_ID_BT_HFG_DISCONNECTED_IND:
    {
        ...
        CONVERT_BDADDR2STRING(&ind_p->bt_addr, address);
        LOGI("[BT][HFG][MSG] MSG_ID_BT_HFG_DISCONNECTED_IND : address=%s,", address);
        env->CallVoidMethod(object,
    ...
    {
        LOGI("[BT][HFG][MSG] MSG_ID_BT_HFG_SCO_CONNECTED_IND");
        env->CallVoidMethod(object,
    ...
    {
        LOGI("[BT][HFG][MSG] MSG_ID_BT_HFG_SCO_DISCONNECTED_IND");
        env->CallVoidMethod(object,
    ...
    case MSG_ID_BT_HFG_AT_COMMAND_DATA_IND:
    {
        ...
        LOGI("[BT][HFG][MSG] MSG_ID_BT_HFG_AT_COMMAND_DATA_IND : at=%s, size=%d", ind_p->atcmd, ind_p->datasize);
        env->CallVoidMethod(object,
    ...
    default:
        LOGI("[BT][HFG][MSG] Unknown message");
        return -1;
        break;
    }
    return 1;
...
static jboolean waitForHandsfreeIndicationNative(JNIEnv* env, jobject object)
{
    ...
    LOGI("[BT][HFG][API] waitForHandsfreeIndicationNative");
    if(!nat)
    {
        LOGE("[BT][HFG][ERR] nat is NULL");
        return JNI_FALSE;
    }
    ...
    {
        LOGE("[BT][HFG][ERR] nat->hfgservsock == 0. exit");
    }
    LOGI("[BT][HFG]Start select : sockfd=%d", sockfd);
    res = select(sockfd+1, &readfs, NULL, NULL, NULL);
    LOGI("[BT][HFG]Return from select : soresckfd=%d", res);
    if(res > 0)
    {
        res = recvfrom(sockfd, (void*)&ilm, sizeof(ilm_struct), 0, NULL, NULL);
        LOGI("[BT][HFG]Recv HFP indication : %lu", ilm.msg_id);
        if(res < 0)
        {
            LOGE("[BT][HFG][ERR] recvfrom failed : %s, %d", strerror(errno), errno);
        }
        else if(res == sizeof(int))
        {
            int evt = *(int*)&ilm;
            LOGI("[BT][HFG]recv event : %d", evt);
            env->CallVoidMethod(object,
                method_onEvent,
                evt);
        }
    ...
    {
        LOGE("[BT][HFG][ERR] timeout waiting indication");
    }
    else
    {
        LOGE("[BT][HFG][ERR] select failed : %s, %d", strerror(errno), errno);
    }
#endif
...
//    LOGV(__FUNCTION__);
    ...
    if (nat->hf_ag_rfcomm_channel > 0) {
        LOGI("[BT][HFG]Setting HF AG server socket to RFCOMM port %d!", 
             nat->hf_ag_rfcomm_channel);
        struct timeval tv;
        int len = sizeof(tv);
        if (getsockopt(nat->hf_ag_rfcomm_channel, 
                       SOL_SOCKET, SO_RCVTIMEO, &tv, &len) < 0) {
            LOGE("[BT][HFG]getsockopt(%d, SOL_SOCKET, SO_RCVTIMEO): %s (%d)",
                 nat->hf_ag_rfcomm_channel,
                 strerror(errno),
                 errno);
            return JNI_FALSE;
        }
        LOGI("[BT][HFG]Current HF AG server socket RCVTIMEO is (%d(s), %d(us))!", 
        ...
        if (timeout_ms >= 0) {
            ...
            if (setsockopt(nat->hf_ag_rfcomm_channel, 
                ...
                LOGE("[BT][HFG]setsockopt(%d, SOL_SOCKET, SO_RCVTIMEO): %s (%d)",
                     nat->hf_ag_rfcomm_channel,
                     strerror(errno),
                     errno);
                return JNI_FALSE;
            }
            LOGI("[BT][HFG]Changed HF AG server socket RCVTIMEO to (%d(s), %d(us))!", 
                 (int)tv.tv_sec, (int)tv.tv_usec);
        }
    ...
    if (nat->hs_ag_rfcomm_channel > 0) {
        LOGI("[BT][HFG]Setting HS AG server socket to RFCOMM port %d!", 
             nat->hs_ag_rfcomm_channel);
        cnt++;
        FD_SET(nat->hs_ag_rfcomm_sock, &rset);
    }
    if (cnt == 0) {
        LOGE("[BT][HFG]Neither HF nor HS listening sockets are open!");
        return JNI_FALSE;
    }
    ...
    if (timeout_ms > 0) {
        ...
        LOGI("[BT][HFG]Remaining time %ldms", (long)remaining);
        env->SetIntField(object, field_mTimeoutRemainingMs,
                         remaining);
    }

    LOGI("[BT][HFG]listening select() returned %d", n);

    if (n <= 0) {
        if (n < 0)  {
            LOGE("[BT][HFG]listening select() on RFCOMM sockets: %s (%d)",
                 strerror(errno),
                 errno);
        }
        return JNI_FALSE;
    ...
    int cnt = 0;
    if (nat->hf_ag_rfcomm_channel > 0) {
//        LOGI("[BT][HFG]Setting HF AG server socket %d to RFCOMM port %d!", 
...
//             nat->hf_ag_rfcomm_channel);
    ...
    if (nat->hs_ag_rfcomm_channel > 0) {
//        LOGI("[BT][HFG]Setting HS AG server socket %d to RFCOMM port %d!", 
...
//             nat->hs_ag_rfcomm_channel);
    ...
    if (n <= 0) {
        if (n < 0)  {
            LOGE("[BT][HFG]listening poll() on RFCOMM sockets: %s (%d)",
                 strerror(errno),
                 errno);
        }
        else {
            env->SetIntField(object, field_mTimeoutRemainingMs, 0);
//            LOGI("[BT][HFG]listening poll() on RFCOMM socket timed out");
        }
        return JNI_FALSE;
    }

    //LOGI("[BT][HFG]listening poll() on RFCOMM socket returned %d", n);
    int err = 0;
    for (cnt = 0; cnt < (int)(sizeof(fds)/sizeof(fds[0])); cnt++) {
        //LOGI("[BT][HFG]Poll on fd %d revent = %d.", fds[cnt].fd, fds[cnt].revents);
        if (fds[cnt].fd == nat->hf_ag_rfcomm_sock) {
            if (fds[cnt].revents & (POLLIN | POLLPRI | POLLOUT)) {
                LOGI("[BT][HFG]Accepting HF connection.\n");
            ...
            if (fds[cnt].revents & (POLLIN | POLLPRI | POLLOUT)) {
                LOGI("[BT][HFG]Accepting HS connection.\n");
                err += do_accept(env, object, fds[cnt].fd, 
                               field_mConnectingHeadsetSocketFd,
                               field_mConnectingHeadsetAddress,
                               field_mConnectingHeadsetRfcommChannel);
    ...
    if (n != 0) {
        LOGI("[BT][HFG]Bogus poll(): %d fake pollfd entrie(s)!", n);
        return JNI_FALSE;
    }
...
static jboolean setUpListeningSocketsNative(JNIEnv* env, jobject object) {
    ...
    LOGI("[BT][HFG][API] setUpListeningSocketsNative");
    ...
    LOGI("[BT][HFG]nat->hfgservsock==%d", nat->hfgservsock);
    if (nat->hfgservsock < 0)
    {
        LOGE("[BT][HFG][ERR] create hfg server socket failed : %s, errno=%d", strerror(errno), errno);
        return JNI_FALSE;
    }
    if (bind (nat->hfgservsock, (struct sockaddr *) &hfgname, hfgnamelen) < 0)
    {
        LOGE("[BT][HFG][ERR] bind hfg service socket failed : %s, errno=%d", strerror(errno), errno);
        goto exit;
    }
    else
    {
        ...
        {
            LOGI("[BT][HFG]getsockname failed : %s, errno=%d", strerror(errno), errno);
        }
        else
        {
            LOGI("[BT][HFG]Auto bind HFG server : len=%d, addr=%s", hfgnamelen, &hfgname.sun_path[1]);
        }
    }
#else
    ...
    if(nat->hfgservsock < 0)
    {
    ...
    else
   	{
        hfgnamelen = sizeof(hfgname.sun_path);
        hfgname.sun_path[0] = '\0';
	LOGI("[BT][HFG]create hfg server socket success");
        if (getsockname(nat->hfgservsock, (sockaddr*)&hfgname, &hfgnamelen) < 0)
        {
            LOGI("[BT][HFG]getsockname failed : %s, errno=%d", strerror(errno), errno);
        }
    ...
    LOGI("[BT][HFG]nat->hfgsock==%d", nat->hfgsock);
    if (nat->hfgsock < 0)
    {
        LOGE("[BT][HFG][ERR] create hfg api socket failed : %s, errno=%d", strerror(errno), errno);
        return JNI_FALSE;
    }
    if (bind (nat->hfgsock, (struct sockaddr *) &hfgname, hfgnamelen) < 0)
    {
        LOGE("[BT][HFG][ERR] bind hfg api socket failed : %s, errno=%d", strerror(errno), errno);
        goto exit;
    }
    else
    {
        ...
        {
            LOGE("[BT][HFG][ERR] getsockname failed : %s, errno=%d", strerror(errno), errno);
        }
        else
        {
            LOGI("[BT][HFG]Auto bind HFG server : len=%d, addr=%s", hfgnamelen, &hfgname.sun_path[1]);
    ...
    {
        LOGE("[BT][HFG][ERR] connect to /data/btserv failed : %s, errno=%d", strerror(errno), errno);
        goto exit;
    }
    ...
    #endif
    {
        LOGI("[BT][HFG]pContext[HSP_CONTEXT] = %p, hfgContext=%p", nat->pContext[HSP_CONTEXT], nat->pContext[HSP_CONTEXT]->hfgContext);
        #if defined(EXT_DYNAMIC_LOADING)
        ...
        #endif            
        {
            LOGE("[BT][HFG][ERR] btmtk_hfg_register for HFP returns failed");
            #if defined(EXT_DYNAMIC_LOADING)
        ...
        LOGI("[BT][HFG]pContext[HFP_CONTEXT] = %p, hfgContext=%p", nat->pContext[HFP_CONTEXT], nat->pContext[HFP_CONTEXT]->hfgContext);
    }
    else
    {
        LOGE("[BT][HFG][ERR] btmtk_hfg_register for HSP returns failed");
        goto exit;
    }
    return JNI_TRUE;
exit:
...
static int setup_listening_socket(int dev, int channel) {
    ...
    if (sk < 0) {
        LOGE("[BT][HFG]Can't create RFCOMM socket");
        return -1;
    }
	...
	if (lm && setsockopt(sk, SOL_RFCOMM, RFCOMM_LM, &lm, sizeof(lm)) < 0) {
		LOGE("[BT][HFG]Can't set RFCOMM link mode");
		close(sk);
		return -1;
	}
	...
	if (bind(sk, (struct sockaddr *)&laddr, sizeof(laddr)) < 0) {
		LOGE("[BT][HFG]Can't bind RFCOMM socket");
		close(sk);
		return -1;
	}
...
static void tearDownListeningSocketsNative(JNIEnv *env, jobject object) {
    ...
    if (nat->hf_ag_rfcomm_sock > 0) {
        if (close(nat->hf_ag_rfcomm_sock) < 0) {
            LOGE("[BT][HFG]Could not close HF server socket: %s (%d)\n",
                 strerror(errno), errno);
        }
        nat->hf_ag_rfcomm_sock = -1;
    }
    if (nat->hs_ag_rfcomm_sock > 0) {
        if (close(nat->hs_ag_rfcomm_sock) < 0) {
            LOGE("[BT][HFG]Could not close HS server socket: %s (%d)\n",
                 strerror(errno), errno);
        }
        nat->hs_ag_rfcomm_sock = -1;
    }
...
#if defined(__BTMTK__) && defined(__BT_HFG_PROFILE__)
    native_data_t *nat = get_native_data(env, object);
    LOGI("[BT][HFG][API] tearDownListeningSocketsNative");
    // Deregister HFG
    for(int i = 0;i < NUM_OF_CONTEXT;i++)
    {
        if(nat->pContext[i])
        {
            ...
            #endif
            LOGI("[BT][HFG]deregister %p context : return=%d", nat->pContext[i], status);
            //nat->pContext[i]= NULL;
        }
    }
    // Close sockets
    if (nat->hfgservsock)
    {
        LOGI("[BT][HFG]Close hfg serv socket");
        close(nat->hfgservsock);
        nat->hfgservsock = 0;
    }
    if (nat->hfgsock)
    {
        LOGI("[BT][HFG]Close hfg api socket");
        close(nat->hfgsock);
        nat->hfgsock = 0;
    }
#endif
...
static jint waitForAsyncConnectNative(JNIEnv *env, jobject obj, jstring address, jint timeout_ms, jint type)
{
    ...
    LOGI("[BT][HFG][API] waitForAsyncConnectNative : type=%d", type);
    if(nat)
    {
        BtStatus status;
        MTK_BD_ADDR bdaddr;
        ...
        {
            LOGE("[BT][HFG][ERR] activeContext is not NULL : %p", nat->activeContext);
            return -1;
        }
        ...
        {
            LOGE("[BT][HFG][ERR] btmtk_hfg_create_service_link returns %d", status);
        }
    }
    else
    {
...
{
    LOGI("[BT][HFG][API] disconnectNative");
#if defined(__BTMTK__) && defined(__BT_HFG_PROFILE__)
    native_data_t *nat = get_native_data(env, obj);
    if(nat)
    {
        ...
        if(nat->activeContext == NULL)
        {
            LOGE("[BT][HFG][ERR] activeContext is NULL");
            return;
        }        
        #if defined(EXT_DYNAMIC_LOADING)
            ...
            if(buffer)
            {
                ...
                while(len > 0){
                    ...
                    if( (total+at_len) > (MAX_AT_STRING_LEN-1) || len <= 0){
                        ...
                        #endif
                        LOGI("1. at_len=%d, total=%d, len=%d", at_len, total, len);
                        total = at_len;
                        ptr1 = ptr2;
                        LOGI("2. at_len=%d, total=%d, len=%d", at_len, total, len);
                    }else{
                        total += at_len;
                        LOGI("3. at_len=%d, total=%d, len=%d", at_len, total, len);
                    }
                    ptr2 += at_len;
                }
                free(buffer);
                ...
                {
                    LOGE("[BT][HFG][ERR] sendURCNative returns %d", status);
                }
            }
            else
            {
                LOGE("[BT][HFG][ERR] Out of memory");
            }
...
static jboolean connectNative(JNIEnv *env, jobject object) 
{
    ...
    if(nat)
    {
        if(nat->activeContext != NULL)
        {
            ...
            if(status != BT_STATUS_PENDING)
            {
                LOGE("[BT][HFG][ERR] btmtk_hfg_create_audio_link returns %d", status);
            }
        }
        else
        {
...
static void closeNative(JNIEnv *env, jobject object) 
{
    ...
    if(nat)
    {
        if(nat->activeContext != NULL)
        {
            ...
            if(status != BT_STATUS_PENDING)
            {
                LOGE("[BT][HFG][ERR] btmtk_hfg_disconnect_audio_link returns %d", status);
            }
        }
        else
        {
...
#if defined(__BTMTK__) && defined(__BT_HFG_PROFILE__)
    BtStatus status = BT_STATUS_FAILED;
    LOGI("[BT][HFG][API] acceptConnectionNative");
    native_data_t *nat = get_native_data(env, object);
    if(nat)
    {
        if(nat->activeContext != NULL)
        {
            ...
            if(status != BT_STATUS_PENDING)
            {
                LOGE("[BT][HFG][ERR] btmtk_hfg_accept_connect returns %d", status);
            }
        }
        else
        {
...
#if defined(__BTMTK__) && defined(__BT_HFG_PROFILE__)
    BtStatus status = BT_STATUS_FAILED;
    LOGI("[BT][HFG][API] rejectConnectionNative");
    native_data_t *nat = get_native_data(env, object);
    if(nat)
    {
        if(nat->activeContext != NULL)
        {
            ...
            if(status != BT_STATUS_PENDING)
            {
                LOGE("[BT][HFG][ERR] btmtk_hfg_reject_connect returns %d", status);
            }
        }
        else
        {
...
static void setEventNative(JNIEnv *env, jobject object, jint evt)
{
    ...
    LOGI("[BT][HFG][API] setEventNative(%d)", evt);
    if(nat)
    {
        sockaddr_un name;
        socklen_t   namelen;
        ...
        LOGI("[BT][HFG]nat->hfgservsock : name=%s, size=%d", &name.sun_path[1], namelen);
        if(ret == 0 && namelen > 0)
        {
            ret = sendto(nat->hfgsock, &evt, sizeof(int), 0, (struct sockaddr*)&name, namelen);
            if(ret < 0)
            {
                LOGE("[BT][HFG][ERR] sendto servsock itself failed : %s, errno=%d", strerror(errno), errno);
            }
        }
        else
        {
