...
#define JPEG_PROG_LIMITATION_SIZE (4096*4096)
...
public:
	...
	virtual size_t read(void* buffer, size_t size){
        ...
        if( read_start <= hwInputBufSize && read_end <= hwInputBufSize){
		  ...
		  real_size_2 = srcStream->read(buffer, size_2);
		  total_read_size += real_size_2 ;
		  //SkDebugf("SkJpgStream::read(HW+Stream), size_2 %x, real_size %x(%x), total_size %x!!\n", size_2, real_size_2, srcStream->getLength(),total_read_size);  
...
public:
    ...
    void* alloc(size_t size) 
    {
        ...
        {
            fAddr = NULL;
            SkDebugf("------- mmap failed for ashmem size=%d err=%d\n", size, errno);
            close(fFD);
            return NULL;
        }
...
bool SkJPEGImageDecoder::onDecode(SkStream* stream, SkBitmap* bm, Mode mode) {
    ...
    tSrc = (uint8_t*)tAllocator.alloc(sLength);
                ...
                if(rSize == MAX_HEADER_SIZE)
                {
                    ...
                    do
                    {
                        if(sLength <= rSize + 16)
                        {
                            ...
                            tmpBuffer = (uint8_t*)tmpAllocator.alloc(rSize);
...
//#if MTK_JPEG_HW_DECODER
    ...
    fInitRegion = true;
    fSrc = (uint8_t*)fAllocator.alloc(sLength);
    fSize = 0;
    if (fSrc != NULL) 
    {
            ...
            if(fSize == MAX_HEADER_SIZE)
            {
                ...
                do
                {
                    if(sLength <= fSize + 16)
                    {
                        ...
                        tmpBuffer = (uint8_t*)tmpAllocator.alloc(sLength);
...
#if MTK_JPEG_HW_DECODER
    if(width * height > HW_RESIZE_MAX_PIXELS)
    {
        fFirstTileDone = true;
        fUseHWResizer = false;
        XLOGW("It is too large pixels (%d) to use hw resizer! Use sw sampler ", width * height);
    }
...
if(!fFirstTileDone || fUseHWResizer)
{
    ...
    uint8_t* hwBuffer = (uint8_t*)srcStorage.alloc(width * height * bpp + 4);
