...
import android.view.GLES20Canvas.InvalidDrawException;
...
public final class ViewRootImpl extends Handler implements ViewParent,
    ...
    private static boolean mAppLaunchTimeLog = false;
    private static boolean mEnableViewRootImplLog = false;
	...
	public String toString() {
		if (mStringNameCache == null || mLastTitle != mWindowAttributes.getTitle()) {
			mLastTitle = mWindowAttributes.getTitle();
            mStringNameCache = "ViewRoot{" + Integer.toHexString(System.identityHashCode(this))
    ...
    public void setView(View view, WindowManager.LayoutParams attrs, View panelParentView) {
        synchronized (this) {
            if (mView == null) {
                ...
                if (DEBUG_LAYOUT) Log.d(TAG, this + " WindowLayout in setView:" + attrs);
                ...
                if (DEBUG_LAYOUT) Log.v(TAG, this + " Added window " + mWindow);
    ...
    void handleAppVisibility(boolean visible) {
    	if (LOCAL_LOGV) {
    		Log.d(TAG, this + " handleAppVisibility visible=" + visible +
    				", mAppVisible=" + mAppVisible);
    	}
        if (mAppVisible != visible) {
            mAppVisible = visible;
            mAttachInfo.mForceVisibilityChange = true;
            scheduleTraversals();
        }
    }
    ...
    public void requestLayout() {
        if (DEBUG_LAYOUT) {
        	Log.d(TAG, this + " requestLayout, mLayoutRequested = true");
        }
        checkThread();
    ...
    public void invalidateChild(View child, Rect dirty) {
        checkThread();
        if (DEBUG_DRAW)
			Log.v(TAG, this + " invalidateChild child is: " + child  + ", dirty is: "+ dirty + ", mCurScrollY is: " + 
					mCurScrollY + ", mDirty before union is: " + mDirty);
        if (dirty == null) {
        ...
        mDirty.union(dirty);
        if (DEBUG_DRAW)
			Log.v(TAG, this + " invalidateChild: mDirty after union is: " + mDirty + ", mWillDrawSoon is: " + mWillDrawSoon);
        if (!mWillDrawSoon) {
            scheduleTraversals();
        }
    }
    ...
    void setStopped(boolean stopped) {
        if (DEBUG_LAYOUT) {
        	Log.d(TAG, this + " setStopped, stopped=" + stopped);
        }
        if (mStopped != stopped) {
            mStopped = stopped;
            if (!stopped) {
    ...
    public boolean getChildVisibleRect(View child, Rect r, android.graphics.Point offset) {
        ...
        if (DEBUG_DRAW) {
		    Log.v(TAG, this + " getChildVisibleRect: child: " + child + ", r is: " + r + ", offset is: " + offset);
		}
        // Note: don't apply scroll offset, because we want to know its
        // visibility in the virtual canvas being given to the view hierarchy.
        boolean isInterSeted = r.intersect(0, 0, mWidth, mHeight);
		if (DEBUG_DRAW) {
		    Log.v(TAG, this + " getChildVisibleRect: updated r is: " + r);
		}
		return isInterSeted;
    }
    ...
    private void performTraversals() {
    	logAppLaunchTime(TAG, this + " performTraversals enter=====");
        ...
        boolean viewVisibilityChanged = mViewVisibility != viewVisibility
                || mNewSurfaceNeeded
                || (mAttachInfo.mForceVisibilityChange && mView != null && mView.getVisibility() == View.VISIBLE);
        ...
        if (mFirst) {
            ...
            logAppLaunchTime(TAG, this + " dispatchAttachToWindow +");
        ...
        if (LOCAL_LOGV) {
			Log.v(TAG, this + " ViewRoot performTraversals() enter, viewVisibility=" + viewVisibility +
					", viewVisibilityChanged=" + viewVisibilityChanged +
					", frame=" + frame.toShortString() +
					", mFirst=" + mFirst +
					", mLayoutRequested=" + mLayoutRequested +
					", fullRedrawNeeded=" + fullRedrawNeeded +
					", desiredWindowWidth=" + desiredWindowWidth +
					", desiredWindowHeight=" + desiredWindowHeight +
					", mWidth=" + mWidth +
					", mHeight=" + mHeight +
					", mStopped=" + mStopped +
					", mSoftInputMode=0X" + Integer.toHexString(mSoftInputMode) +
					", mAttachInfo.mContentInsets=" + mAttachInfo.mContentInsets +
                    ", mAttachInfo.mForceVisibilityChange=" + mAttachInfo.mForceVisibilityChange);
		}

        mAttachInfo.mForceVisibilityChange = false;
        ...
        if (mLayoutRequested && !mStopped) {
            ...
            if (mFirst) {
                ...
                if (!mPendingContentInsets.equals(mAttachInfo.mContentInsets)) {
                    insetsChanged = true;
                    if (DEBUG_LAYOUT)
						Log.v(TAG, this + " Content insets changing to: "
								+ mAttachInfo.mContentInsets.toShortString());
                }
                if (!mPendingVisibleInsets.equals(mAttachInfo.mVisibleInsets)) {
                    mAttachInfo.mVisibleInsets.set(mPendingVisibleInsets);
                    if (DEBUG_LAYOUT)
						Log.v(TAG, this + " Visible insets changing to: "
								+ mAttachInfo.mVisibleInsets.toShortString());
                }
            ...
            if (DEBUG_LAYOUT)
				Log.v(TAG, this + " ViewRoot measure+ desiredWindowWidth=" + desiredWindowWidth +
						", desiredWindowHeight=" + desiredWindowHeight +
						", lp.width=" + lp.width +
						", lp.height=" + lp.height);
            ...
            if (lp.width == ViewGroup.LayoutParams.WRAP_CONTENT) {
                ...
                if (DEBUG_DIALOG) Log.v(TAG, this + " DialogWindow " + mView + ": baseSize=" + baseSize);
                if (baseSize != 0 && desiredWindowWidth > baseSize) {
                    childWidthMeasureSpec = getRootMeasureSpec(baseSize, lp.width);
                    childHeightMeasureSpec = getRootMeasureSpec(desiredWindowHeight, lp.height);
                    host.measure(childWidthMeasureSpec, childHeightMeasureSpec);
                    if (DEBUG_DIALOG) Log.v(TAG, this + " DialogWindow " + mView + ": measured ("
                    ...
                    if ((host.getMeasuredWidthAndState()&View.MEASURED_STATE_TOO_SMALL) == 0) {
                        ...
                        if (DEBUG_DIALOG) Log.v(TAG, this + " DialogWindow " + mView + ": next baseSize="
                                + baseSize);
                        childWidthMeasureSpec = getRootMeasureSpec(baseSize, lp.width);
                        host.measure(childWidthMeasureSpec, childHeightMeasureSpec);
                        if (DEBUG_DIALOG) Log.v(TAG, this + " DialogWindow " + mView + ": measured ("
                                + host.getMeasuredWidth() + "," + host.getMeasuredHeight() + ")");
                        if ((host.getMeasuredWidthAndState()&View.MEASURED_STATE_TOO_SMALL) == 0) {
                            if (DEBUG_DIALOG) Log.v(TAG, this + " DialogWindow measure Good!");
                            goodMeasure = true;
                        }
                    }
                }
            ...
            if (!goodMeasure) {
                ...
                childHeightMeasureSpec = getRootMeasureSpec(desiredWindowHeight, lp.height);
                logAppLaunchTime(TAG, this + " host measure +");
                host.measure(childWidthMeasureSpec, childHeightMeasureSpec);
                logAppLaunchTime(TAG, this + " host measure -");
            ...
            if (DEBUG_LAYOUT)
				Log.v(TAG, this + " ViewRoot measure- host.getMeasuredWidth()=" + host.getMeasuredWidth() + 
						", host.getMeasuredHeight()=" + host.getMeasuredHeight() +
						", windowSizeMayChange=" + windowSizeMayChange);
        ...
        if (mFirst || attachInfo.mViewVisibilityChanged) {
            ...
            if (DEBUG_INPUT_RESIZE) {
				Log.v(TAG, this + " ViewRoot resizeMode+ resizeMode=0X" + Integer.toHexString(resizeMode) + ", lp.softInputMode=0X" + Integer.toHexString(lp.softInputMode));
			}
        ...
        if (DEBUG_LAYOUT) {
			Log.v(TAG, this + " ViewRoot adjustSize+ mFirst=" + mFirst +
					", windowShouldResize=" + windowShouldResize +
        ...
        if (mFirst || windowShouldResize || insetsChanged ||
            ...
            try {
                ...
                if (DEBUG_LAYOUT) {
                    Log.i(TAG, this + " host=w:" + host.getMeasuredWidth() + ", h:" +
                            host.getMeasuredHeight() + ", params=" + params);
                }

                final int surfaceGenerationId = mSurface.getGenerationId();
                logAppLaunchTime(TAG, this + " relayoutWindow +");
                ...
                if (mPendingConfiguration.seq != 0) {
                    if (DEBUG_CONFIGURATION) Log.v(TAG, this + " Visible with new config: "
                ...
                if (contentInsetsChanged) {
                    ...
                    host.fitSystemWindows(mAttachInfo.mContentInsets);
                    if (DEBUG_LAYOUT) Log.v(TAG, this + " Content insets changing to: "
                            + mAttachInfo.mContentInsets);
                }
                if (visibleInsetsChanged) {
                    mAttachInfo.mVisibleInsets.set(mPendingVisibleInsets);
                    if (DEBUG_LAYOUT) Log.v(TAG, this + " Visible insets changing to: "
                            + mAttachInfo.mVisibleInsets);
                }
                ...
                if (!hadSurface) {
                    if (mSurface.isValid()) {
                        ...
                        logAppLaunchTime(TAG, this + " HWUI render initialize +");
                        if (mAttachInfo.mHardwareRenderer != null) {
                            try {
            ...
            if (mAttachInfo.mHardwareRenderer != null &&
                ...
                if (hwInitialized || windowShouldResize ||
                	...
                	logAppLaunchTime(TAG, this + " HWUI render setup +");
            ...
            if (DEBUG_LAYOUT) {
                Log.d(TAG, this + " ViewRoot remeasure+, mStopped=" + mStopped);
            }

            if (!mStopped) {
                boolean focusChangedDueToTouchMode = ensureTouchModeLocally(
                        (relayoutResult&WindowManagerImpl.RELAYOUT_RES_IN_TOUCH_MODE) != 0);
                if (DEBUG_LAYOUT) Log.v(TAG, this + " ViewRoot remeasure" +
                		", focusChangeDueToTouchMode=" + focusChangedDueToTouchMode +
                		", mWidth=" + mWidth +
                		", measuredWidth=" + host.getMeasuredWidth() +
                        ", mHeight=" + mHeight +
                        ", measuredHeight=" + host.getMeasuredHeight() +
                        ", coveredInsetsChanged=" + contentInsetsChanged +
                        ", mActionBarMeasureRequred=" + mAttachInfo.mActionBarMeasureRequired);
                if (focusChangedDueToTouchMode || mWidth != host.getMeasuredWidth()
                        || mHeight != host.getMeasuredHeight() || contentInsetsChanged
                        || mAttachInfo.mActionBarMeasureRequired) {
                    mAttachInfo.mActionBarMeasureRequired = false;
                    childWidthMeasureSpec = getRootMeasureSpec(mWidth, lp.width);
                    childHeightMeasureSpec = getRootMeasureSpec(mHeight, lp.height);
    
                    if (DEBUG_LAYOUT) Log.v(TAG, this + " Ooops, something changed! need to measure again");
                    ...
                    if (measureAgain) {
                        if (DEBUG_LAYOUT) Log.v(TAG,
                                this + " And hey let's measure once more: width=" + width
                                + " height=" + height);
                        logAppLaunchTime(TAG, this + " host measure +");
                        host.measure(childWidthMeasureSpec, childHeightMeasureSpec);
                        logAppLaunchTime(TAG, this + " host measure -");
            ...
            if (DEBUG_LAYOUT) {
                Log.v(TAG, this + " ViewRoot remeasure- host.getMeasuredWidth()=" + host.getMeasuredWidth() + 
						", host.getMeasuredHeight()=" + host.getMeasuredHeight());
            }
        }
        
        if (DEBUG_LAYOUT) {
			Log.v(TAG, this + " ViewRoot adjustSize- " +
					", frame=" + frame +
        ...
        if (DEBUG_LAYOUT) {
			Log.v(TAG, this + " ViewRoot layout+ " +
					", mLayoutRequested=" + mLayoutRequested +
                    ", mStopped=" + mStopped);
		}
        ...
        if (didLayout) {
            ...
            if (DEBUG_ORIENTATION || DEBUG_LAYOUT) Log.v(
                TAG, this + " Laying out " + host + " to (" +
                host.getMeasuredWidth() + ", " + host.getMeasuredHeight() + ")");
            long startTime = 0L;
            if (ViewDebug.DEBUG_PROFILE_LAYOUT) {
                startTime = SystemClock.elapsedRealtime();
            }
            logAppLaunchTime(TAG, this + " host layout +");
        ...
        if (DEBUG_LAYOUT) {
			Log.v(TAG, this + " ViewRoot layout- " +
					", host.mLeft=" + host.mLeft +
					", host.mRight=" + host.mRight +
					", host.mTop=" + host.mTop +
					", host.mBottom=" + host.mBottom);
		}
        ...
        if (mFirst) {
            ...
            if (DEBUG_INPUT_RESIZE) Log.v(TAG, this + " First: mView.hasFocus()="
                    + mView.hasFocus());
            if (mView != null) {
                if (!mView.hasFocus()) {
                    mView.requestFocus(View.FOCUS_FORWARD);
                    mFocusedView = mRealFocusedView = mView.findFocus();
                    if (DEBUG_INPUT_RESIZE) Log.v(TAG, this + " First: requested focused view="
                            + mFocusedView);
                } else {
                    mRealFocusedView = mView.findFocus();
                    if (DEBUG_INPUT_RESIZE) Log.v(TAG, this + " First: existing focused view="
                            + mRealFocusedView);
                }
            }
        }
        ...
        if (!cancelDraw && !newSurface) {
            ...
            if (DEBUG_DRAW) {
            	Log.d(TAG, this + " ViewRoot draw FINISHED DRAWING" +
        ...
        logAppLaunchTime(TAG, this + " performTraversal exit=====");
        if (DEBUG_DRAW) {
        	Log.d(TAG, this + " ViewRoot draw-");
        }
    }
    ...
    private void draw(boolean fullRedrawNeeded) {
        ...
        if (mCurScrollY != yoff) {
            mCurScrollY = yoff;
            fullRedrawNeeded = true;
            mAttachInfo.mTreeObserver.dispatchOnViewRootScrollChanged();
        }
        ...
        if (mAttachInfo.mHardwareRenderer != null && mAttachInfo.mHardwareRenderer.isEnabled()) {
            if (!dirty.isEmpty() || mIsAnimating) {
                ...
                if (DEBUG_DRAW) {
                    Log.d(TAG, this + " HWUI draw() " +
                    		", mPreviousDirty=" + mPreviousDirty +
                    		", mCurrentDirty=" + mCurrentDirty +
                    		", currentDirty=" + currentDirty);
                }
                
                logAppLaunchTime(TAG, this + " HWUI draw +");
                try {
        ...
        if (!dirty.isEmpty() || mIsAnimating) {
            ...
            try {
                ...
                if (DEBUG_DRAW) {
                    Log.d(TAG, this + " draw(), after lockCanvas dirty = " + dirty.toShortString());
                }
            ...
            try {
                if (!dirty.isEmpty() || mIsAnimating) {
                    ...
                    if (DEBUG_ORIENTATION || DEBUG_DRAW) {
                        Log.v(TAG, this + " Surface " + surface + " drawing to bitmap w="
                    ...
                    try {
                        ...
                        logAppLaunchTime(TAG, this + " draw +");
                        mView.draw(canvas);
                        logAppLaunchTime(TAG, this + " draw -");
                    } finally {
        ...
        if (LOCAL_LOGV) {
            Log.v(TAG, this + " Surface " + surface + " unlockCanvasAndPost");
        }
    ...
    boolean scrollToRectOrFocus(Rect rectangle, boolean immediate) {
        ...
        if (vi.left > ci.left || vi.top > ci.top
            ...
            if (DEBUG_INPUT_RESIZE) Log.v(TAG, this + " Eval scroll: focus=" + focus
            ...
            if (focus == mLastScrolledFocus && !mScrollMayChange
                ...
                // as they are.
                if (DEBUG_INPUT_RESIZE) Log.v(TAG, this + " Keeping scroll y="
                ...
                if (DEBUG_INPUT_RESIZE) Log.v(TAG, this + " Need to scroll?");
                // Try to find the rectangle from the focus view.
                if (focus.getGlobalVisibleRect(mVisRect, null)) {
                    if (DEBUG_INPUT_RESIZE) Log.v(TAG, this + " Root w="
                            + mView.getWidth() + " h=" + mView.getHeight()
                            + " ci=" + ci.toShortString()
                            + " vi=" + vi.toShortString());
                    if (rectangle == null) {
                        focus.getFocusedRect(mTempRect);
                        if (DEBUG_INPUT_RESIZE) Log.v(TAG, this + " Focus " + focus
                        ...
                        if (DEBUG_INPUT_RESIZE) Log.v(TAG,
                                this + " Focus in window: focusRect="
                                + mTempRect.toShortString()
                                + " visRect=" + mVisRect.toShortString());
                    } else {
                        mTempRect.set(rectangle);
                        if (DEBUG_INPUT_RESIZE) Log.v(TAG,
                                this + " Request scroll to rect: "
                                + mTempRect.toShortString()
                                + " visRect=" + mVisRect.toShortString());
                    }
                    if (mTempRect.intersect(mVisRect)) {
                        if (DEBUG_INPUT_RESIZE) Log.v(TAG,
                                this + " Focus window visible rect: "
                                + mTempRect.toShortString());
                        if (mTempRect.height() >
        ...
        if (scrollY != mScrollY) {
            if (DEBUG_INPUT_RESIZE) Log.v(TAG, this + "Pan scroll changed: old="
    ...
    public void requestChildFocus(View child, View focused) {
        ...
        if (DEBUG_INPUT_RESIZE) Log.v(TAG, this + " Request child focus: focus now "
                + mFocusedView);
    }
    ...
    public void clearChildFocus(View child) {
        ...
        if (DEBUG_INPUT_RESIZE) Log.v(TAG, this + " Clearing child focus");
    ...
    void updateConfiguration(Configuration config, boolean force) {
        if (DEBUG_CONFIGURATION) Log.v(TAG,
                this + " Applying new config to window "
                + mWindowAttributes.getTitle()
                + ": " + config);
    ...
    public void handleMessage(Message msg) {
    	if (DEBUG_LAYOUT) {
    		Log.v(TAG, this + " handleMessage " + getMessageName(msg));
    	}
        switch (msg.what) {
        ...
        case RESIZED:
            ...
            if (DEBUG_LAYOUT) {
            	Log.v(TAG, " message RESIZE fall through to RESIZE_REPORT, msg.arg1 is: " + msg.arg1 + 
            			", msg.arg2 is: " + msg.arg2 + ", mWinFrame is: " + mWinFrame.toShortString() +
            			", mPendingContentInsets is: " + mPendingContentInsets + ", ri.coveredInsets is: " + ri.coveredInsets.flattenToString() +
            			", mPendingVisibleInsets is: " + mPendingVisibleInsets + ", ri.visibleInsets is: " + ri.visibleInsets.flattenToString());
            }
            // fall through...
        case RESIZED_REPORT:
            if (mAdded) {
    ...
    private void startInputEvent(InputQueue.FinishedCallback finishedCallback) {
        if (mFinishedCallback != null) {
            Slog.w(TAG, this + " Received a new input event from the input queue but there is "
    ...
    private void finishInputEvent(InputEvent event, boolean handled) {
        if (LOCAL_LOGV) Log.v(TAG, this + " Telling window manager input event is finished");
        ...
        if (mFinishedCallback == null) {
            Slog.w(TAG, this + " Attempted to tell the input queue that the current input event "
    ...
    boolean ensureTouchMode(boolean inTouchMode) {
        if (DBG) Log.d(TAG, this + " ensureTouchMode(" + inTouchMode + "), current "
    ...
    private boolean ensureTouchModeLocally(boolean inTouchMode) {
        if (DBG) Log.d(TAG, this + " ensureTouchModeLocally(" + inTouchMode + "), current "
                + "touch mode is " + mAttachInfo.mInTouchMode);
    ...
    private void finishMotionEvent(MotionEvent event, boolean sendDone, boolean handled) {
        event.recycle();
        mMotionEventStatus = MOTION_DISPATCH_STATE_FINISHED;
        if (sendDone) {
        ...
        if (LOCAL_LOGV || WATCH_POINTER) {
            if ((event.getSource() & InputDevice.SOURCE_CLASS_POINTER) != 0) {
                Log.i(TAG, this + " finishMotionEvent Done dispatching!, event is: " + event + ", sendDone is: " + sendDone +
                		", handled is: " + handled);
            }
        }
    }
    ...
    private void deliverTrackballEvent(MotionEvent event, boolean sendDone) {
        ...
        if (DEBUG_TRACKBALL) Log.v(TAG, this + " deliverTrackballEvent Motion event:" + event + ", sendDone is: " + sendDone);
        ...
        if (DEBUG_TRACKBALL) Log.v(TAG, this + " TB X=" + x.position + " step="
        ...
        if (keycode != 0) {
            ...
            if (DEBUG_TRACKBALL) Log.v(TAG, this + " Move: movement=" + movement
                    + " accelMovement=" + accelMovement
                    + " accel=" + accel);
            if (accelMovement > movement) {
                if (DEBUG_TRACKBALL) Log.v(TAG, this + " Delivering fake DPAD: "
                        + keycode);
                movement--;
    ...
    private void deliverGenericMotionEvent(MotionEvent event, boolean sendDone) {
        ...
        if (LOCAL_LOGV) 
        	Log.v(TAG, this + " deliverGenericMotionEvent Motion event:" + event + ", sendDone is: " + sendDone);
    ...
    private void deliverKeyEvent(KeyEvent event, boolean sendDone) {
        ...
        if (LOCAL_LOGV) Log.v(TAG, this + " Dispatching key " + event + " to " + mView + ", sendDone is: " + sendDone);
        ...
        if (mLastWasImTarget) {
            ...
            if (imm != null) {
                ...
                if (DEBUG_IMF) Log.v(TAG, this + " Sending key event to IME: seq="
                        + seq + " event=" + event);
    ...
    private void handleFinishedEvent(int seq, boolean handled) {
        ...
        if (DEBUG_IMF) Log.v(TAG, this + " IME finished event: seq=" + seq
    ...
    private void finishKeyEvent(KeyEvent event, boolean sendDone, boolean handled) {
        mKeyEventStatus = KEY_DISPATCH_STATE_FINISHED;
        if (sendDone) {
            finishInputEvent(event, handled);
        }
    }
        ...
        if (mView != null && mAdded) {
            ...
            if (what == DragEvent.ACTION_DRAG_EXITED) {
                ...
                if (what == DragEvent.ACTION_DROP) {
                    ...
                    try {
                        Log.i(TAG, this + " Reporting drop result: " + result);
    ...
    private int relayoutWindow(WindowManager.LayoutParams params, int viewVisibility,
        ...
        if (DEBUG_LAYOUT) {
            if (params != null) {
                if (DBG) Log.d(TAG, this + " relayoutWindow WindowLayout in layoutWindow:" + params);
            }
        }
        mPendingConfiguration.seq = 0;
        //Log.d(TAG, ">>>>>> CALLING relayout");
        if (DEBUG_LAYOUT) {
    		Log.v(TAG, this + " ViewRoot relayoutWindow+ viewVisibility=" + viewVisibility + 
    				", insetsPending=" + insetsPending +
    				", mSeq=" + mSeq + 
    				", width=" + (mView.getMeasuredWidth() * appScale + 0.5f) +
    				", height=" + (mView.getMeasuredHeight() * appScale + 0.5f) +
    				", mWinFrame=" + mWinFrame +
    				", mPendingContentInsets=" + mPendingContentInsets +
    				", mPendingVisibleInsets=" + mPendingVisibleInsets +
    				", mPendingConfiguration=" + mPendingConfiguration +
    				", mSurface=" + mSurface);
    	}
        ...
        if (DEBUG_LAYOUT) {
            if (params != null) {
        	    Log.d(TAG, this + " >>>>>> before relayout, params.type is: "  + params.type + ", mOrigWindowType is: " + mOrigWindowType);
            } else {
                Log.d(TAG, this + " >>>>>> before relayout, params.type is null");
            }
        }
        ...
        if (DEBUG_LAYOUT) {
            if (params != null) {
        	    Log.d(TAG, this + " >>>>>> after relayout, params.type is: "  + params.type + ", mOrigWindowType is: " + mOrigWindowType);
            } else {
                Log.d(TAG, this + " >>>>>> after relayout, params.type is null");
            }
        }
        
        if (DEBUG_LAYOUT) {
    		Log.v(TAG, this + " ViewRoot relayoutWindow- mWinFrame=" + mWinFrame +
    				", mPendingContentInsets=" + mPendingContentInsets +
    				", mPendingVisibleInsets=" + mPendingVisibleInsets +
    				", mPendingConfiguration=" + mPendingConfiguration +
    				", mSurface=" + mSurface);
    ...
    void doDie() {
        checkThread();
        if (LOCAL_LOGV) Log.v(TAG, this + " DIE in " + this + " of " + mSurface);
        synchronized (this) {
            if (mAdded) {
                mAdded = false;
    ...
    public void dispatchResized(int w, int h, Rect coveredInsets,
        ...
        if (DEBUG_LAYOUT) Log.v(TAG, this + " Resizing: w=" + w
    ...
    private final InputHandler mInputHandler = new InputHandler() {
        public void handleKey(KeyEvent event, InputQueue.FinishedCallback finishedCallback) {
            ...
            mKeyEventStatus = KEY_DISPATCH_STATE_STARTED;
            startInputEvent(finishedCallback);
            dispatchKey(event, true);
        }

        public void handleMotion(MotionEvent event, InputQueue.FinishedCallback finishedCallback) {
        	if (mAppLaunchTimeLog && MotionEvent.ACTION_DOWN == event.getAction()) {
        		Log.i(TAG, "[AppLaunch] " + "handleMotion() action_down event coming.");
        	}
        	mCurrentMotion = event;
        	mMotionEventStatus = MOTION_DISPATCH_STATE_STARTED;
            startInputEvent(finishedCallback);
    ...
    private void enqueueInputEvent(Message msg, long when) {
        ...
        if (msg.obj instanceof KeyEvent) {
            mKeyEventStatus = KEY_DISPATCH_STATE_ENQUEUE_EVENT;
        } else {
            mMotionEventStatus = MOTION_DISPATCH_STATE_ENQUEUE_EVENT;
        }    
    ...
    private void dispatchKey(KeyEvent event, boolean sendDone) {
        ...
        if (LOCAL_LOGV) Log.v(
            TAG, this + " dispatchKey: " + event + " to " + mView);

        mKeyEventStatus = KEY_DISPATCH_STATE_DISPATCH_KEY;
        enqueueInputEvent(msg, event.getEventTime());
    }
    
    private void dispatchMotion(MotionEvent event, boolean sendDone) {
    	if (LOCAL_LOGV) Log.v(
                TAG, this + " dispatchMotion: " + event + " to " + mView + ", sendDone is: " + sendDone);
    ...
    private void dispatchPointer(MotionEvent event, boolean sendDone) {
        mMotionEventStatus = MOTION_DISPATCH_STATE_ENQUEUE_POINTER;
    	if (LOCAL_LOGV) 
    		Log.v(TAG, this + " dispatchPointer: " + event + " to " + mView + ", sendDone is: " + sendDone);
    ...
    private void dispatchTrackball(MotionEvent event, boolean sendDone) {
        mMotionEventStatus = MOTION_DISPATCH_STATE_ENQUEUE_TRACKBALL;
    	if (LOCAL_LOGV) 
    		Log.v(TAG, this + " dispatchTrackball: " + event + " to " + mView + ", sendDone is: " + sendDone);
    ...
    private void dispatchGenericMotion(MotionEvent event, boolean sendDone) {
        mMotionEventStatus = MOTION_DISPATCH_STATE_ENQUEUE_GENERIC;
    	if (LOCAL_LOGV) 
    		Log.v(TAG, this + " dispatchGenericMotion: " + event + " to " + mView + ", sendDone is: " + sendDone);
    ...
    public void dispatchAppVisibility(boolean visible) {
    	if (LOCAL_LOGV) 
    		Log.v(TAG, this + " dispatchAppVisibility visible is: " + visible); 
    ...
    public void windowFocusChanged(boolean hasFocus, boolean inTouchMode) {
    	if (LOCAL_LOGV) 
    		Log.v(TAG, this + " windowFocusChanged hasFocus is: " + hasFocus + ", inTouchMode is: " + inTouchMode);
    ...
    public void dispatchCloseSystemDialogs(String reason) {
    	if (LOCAL_LOGV) 
    		Log.v(TAG, this + " dispatchCloseSystemDialogs reason is: " + reason);
        Message msg = Message.obtain();
    ...
    public void dispatchDragEvent(DragEvent event) {
        ...
        if (LOCAL_LOGV) 
    		Log.v(TAG, this + " dispatchDragEvent " + event + ", what is: " + what);
    ...
    public void dispatchSystemUiVisibilityChanged(int seq, int globalVisibility,
    	...
    	if (LOCAL_LOGV) 
    		Log.v(TAG, this + " dispatchSystemUiVisibilityChanged seq is: " + seq + ", globalVisibility is: " + globalVisibility +
    				", localValue is: " + localValue + ", localChanges is: " + localChanges);
    ...
    private void sendAccessibilityEvents() {
    	if (LOCAL_LOGV) 
    		Log.v(TAG, this + " sendAccessibilityEvents  is AccessibilityManager enalbed ? " + mAccessibilityManager.isEnabled());
        if (!mAccessibilityManager.isEnabled()) {
            return;
        }
    ...
    static class W extends IWindow.Stub {
        ...
        public void dispatchDumpWindowInfo(){
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                viewAncestor.dispatchDumpWindowInfo();
            }
        }
    }
    ...
    /*
     *  Call this to enable ViewRootImpl log by
     *  checking system property "debug.viewroot.enable"
     *  1 : Enable
     *  0 : Disable
     */
    private static boolean checkViewRootImplLogProperty() {
    	return (1 == SystemProperties.getInt("debug.viewroot.enable", 0)) ? true : false;
    }
...
//        DBG = enable;
     ...
     */
    public void dumpInputDispatchingStatus() {
        Log.i(TAG, "Dispatch " + mCurrentKeyEvent + " status is " + mKeyEventStatus);
        Log.i(TAG, "Dispatch " + mCurrentMotion + " status is " + mMotionEventStatus);
    }
    
    private static final String KEY_DISPATCH_STATE_STARTED = "1: Start event from input";
    private static final String KEY_DISPATCH_STATE_DISPATCH_KEY = "2: Dispach key event";
    private static final String KEY_DISPATCH_STATE_ENQUEUE_EVENT = "3: Enqueue input event";
    private static final String KEY_DISPATCH_STATE_BEGIN_DELIEVER = "4: Begin deliver event";
    private static final String KEY_DISPATCH_STATE_DELIEVER_TO_IMM = "5: Deliver key to imm";
    private static final String KEY_DISPATCH_STATE_DELIEVER_TO_VIEW = "6: Deliver key to view hierarchy";
    private static final String KEY_DISPATCH_STATE_FINISHED = "7: Finish handle key event";
    
    private static final String MOTION_DISPATCH_STATE_STARTED = "1: Start event from input";
    private static final String MOTION_DISPATCH_STATE_ENQUEUE_POINTER = "2 0: Dispach pointer event";
    private static final String MOTION_DISPATCH_STATE_ENQUEUE_TRACKBALL = "2 1: Dispach trackball event";
    private static final String MOTION_DISPATCH_STATE_ENQUEUE_GENERIC = "2 2: Dispach generic event";
    private static final String MOTION_DISPATCH_STATE_ENQUEUE_EVENT = "3: Enqueue motion event";
    private static final String MOTION_DISPATCH_STATE_DELIEVER_POINTER = "4 0: Deliver pointer event";
    private static final String MOTION_DISPATCH_STATE_DELIEVER_TRACKBALL = "4 1: Deliver pointer event";
    private static final String MOTION_DISPATCH_STATE_DELIEVER_GENERIC = "4 2: Deliver pointer event";   
    private static final String MOTION_DISPATCH_STATE_FINISHED = "5: Finish handle motion event";
