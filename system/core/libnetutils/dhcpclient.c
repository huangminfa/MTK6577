...
int dhcp_init_ifc(const char *ifname)
{
    ...
    struct pollfd pfd;
    unsigned int state;
    unsigned int timeout, lefttime;
    int if_index;
    
    unsigned int ntokens;
	struct timeval poll_time, recv_time;
    ...
    for (;;) {
        ...
        LOGD("poll start, lefttime = '%d'", lefttime);
        r = poll(&pfd, 1, lefttime);
        ...
        if (r < 0) {
            ...
            ntokens = (recv_time.tv_sec - poll_time.tv_sec)*1000 +
			(recv_time.tv_usec-poll_time.tv_usec)/1000;
			if(lefttime > ntokens){
				lefttime -= ntokens;
				continue;	
			}else{
				LOGD("lefttime is less than recv time !");
            	lefttime =0 ;
            }
        }
        ...
        if (!valid_reply) {
			...
			if(lefttime > ntokens){
				...
				LOGD("invalid reply, lefttime is less than recv time !");
            	lefttime =0 ;
            }
        }
