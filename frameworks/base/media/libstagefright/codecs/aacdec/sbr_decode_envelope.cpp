...
SBR_ERROR sbr_check_envelope(SBR_FRAME_DATA * hFrameData)
{
    Int32 i;
    Int32 nScaleFactors = hFrameData->nScaleFactors;
    Int32 *iEnvelope    = hFrameData->iEnvelope_man;
        Int32 sbr_max_energy =
          (hFrameData->ampRes == SBR_AMP_RES_3_0) ? SBR_MAX_ENERGY : (SBR_MAX_ENERGY << 1);
        ...
        */
        for (i = 0; i < nScaleFactors; i++) {
          if (iEnvelope[i] > sbr_max_energy) {
                return (SBRDEC_TOO_MANY_SBR_ENVELOPES);
          }
        }
