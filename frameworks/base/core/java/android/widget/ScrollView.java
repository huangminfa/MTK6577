...
public class ScrollView extends FrameLayout {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mIsBeingDragged) {
                    ...
                    int realDeltaY = calcRealOverScrollDist(deltaY);
                    mLastMotionY = y;
                    ...
                    if (overScrollBy(0, realDeltaY, 0, mScrollY,
                    ...
                    if (canOverscroll) {
                        final int pulledToY = oldY + realDeltaY;
                        if (pulledToY < 0) {
                            mEdgeGlowTop.onPull((float) realDeltaY / getHeight());
                            if (!mEdgeGlowBottom.isFinished()) {
                                mEdgeGlowBottom.onRelease();
                            }
                        } else if (pulledToY > range) {
                            mEdgeGlowBottom.onPull((float) realDeltaY / getHeight());
    ...
    private int calcRealOverScrollDist(int dist) {
        ...
        } else if (mScrollY >= getScrollRange()){
            return (int)(dist * (1 - Math.abs(mScrollY - getScrollRange()) * 1.0f / mOverscrollDistance));
        } else {
            return dist;
        }
    }
