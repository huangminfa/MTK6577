...
//MTK-END [mtk80950][120810][ALPS00XXXXXX] add UTK
    ...
    /* Add for Gemini CDMAPhone initialization */
    private int mySimId;
    ...
    public CDMAPhone(Context context, CommandsInterface ci, PhoneNotifier notifier) {
		...
		this(context, ci, notifier, false, Phone.GEMINI_SIM_2);
		/*Set GSM as Phone1 and CDMA as Phone2 */
    }

    public CDMAPhone(Context context, CommandsInterface ci, PhoneNotifier notifier,
            boolean unitTestMode) {
			this(context, ci, notifier, unitTestMode, Phone.GEMINI_SIM_2);
	}
    ...
    public CDMAPhone(Context context, CommandsInterface ci, PhoneNotifier notifier,
        ...
        if(FeatureOption.EVDO_DT_VIA_SUPPORT == true){
            mViaGpsProcess = new ViaGpsProcess(context, ci,mySimId);
        }
        //VIA-END VIA AGPS
        initSstIcc();
...
//MTK-START [mtk80950][120810][ALPS00XXXXXX] add UTK
        ...
        if (FeatureOption.EVDO_DT_VIA_SUPPORT) {
            ...
            mRuimCard = new RuimCard(this, LOG_TAG, DBG);
            mIccRecords = mRuimRecords;
            mIccCard = mRuimCard;
        }
//MTK-END [mtk80950][120810][ALPS00XXXXXX] add UTK    
        ...
        //MTK-START [mtk04070][111117][ALPS00093395]Remove second parameter - mSMS
        mRuimSmsInterfaceManager = new RuimSmsInterfaceManager(this, mSMS);
        //MTK-END [mtk04070][111117][ALPS00093395]Remove second parameter - mSMS
    ...
    //MTK-START [mtk80950][120810][ALPS00XXXXXX] add UTK        
        // add utk from 2.3
        if (FeatureOption.EVDO_DT_VIA_SUPPORT) {
            mUtkService = CatService.getInstance(mCM, mRuimRecords, mContext,
                (RuimFileHandler)mIccFileHandler, mRuimCard);
        }
    //MTK-END [mtk80950][120810][ALPS00XXXXXX] add UTK    
        ...
        if (getMySimId() == Phone.GEMINI_SIM_1) {
...
//MTK-END [mtk80950][120810][ALPS00XXXXXX] add UTK    
	...
	//via support start
        if (!mIsPhoneInEcmState) {
            if (DBG) {
                Log.e(LOG_TAG, "handleTimerInEmergencyCallbackMode, unsupported EcmState " + mIsPhoneInEcmState);
            }    
            return;
        }
        //via support end
    ...
    public void requestSwitchHPF (boolean enableHPF, Message response) {
        Log.d(LOG_TAG, "switch HPF to " + enableHPF);
        mCM.requestSwitchHPF(enableHPF, response);
    }

    public void setAvoidSYS (boolean avoidSYS, Message response) {
        Log.d(LOG_TAG, "avoid sys " + avoidSYS);
        mCM.setAvoidSYS(avoidSYS, response);
    }

    public void getAvoidSYSList (Message response) {
        Log.d(LOG_TAG, "get avoid sys list");
        mCM.getAvoidSYSList(response);
    }
    public void queryCDMANetworkInfo (Message response){
        Log.d(LOG_TAG, "query CDMA network infor");
        mCM.queryCDMANetworkInfo(response);
    }
