...
int SkPaint::getTextWidths(const void* textData, size_t byteLength,
                           SkScalar widths[], SkFontID fontID, SkRect bounds[]) const {
    if (0 == byteLength) {
        return 0;
    }
    ...
    SkAutoGlyphCache    autoCache(*this, NULL);
    SkGlyphCache*       cache = autoCache.getCache();
    SkMeasureCacheByFontProc  glyphCacheProc;
    glyphCacheProc = this->getMeasureCacheProcByFontID(kForward_TextBufferDirection, NULL != bounds);
    ...
    if (this->isDevKernText()) {
        ...
        if (scale) {
            while (text < stop) {
                const SkGlyph& g = glyphCacheProc(cache, &text, fontID);
                if (widths) {
                    SkFixed  adjust = autokern.adjust(g);
