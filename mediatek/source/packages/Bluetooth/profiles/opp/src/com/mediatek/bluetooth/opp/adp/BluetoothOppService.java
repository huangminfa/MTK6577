...
public class BluetoothOppService extends Service {
	...
	public static void sendStateChangedBroadcast( Context context, BluetoothShareTask task, boolean isConnect ){

		Intent intent = new Intent(BluetoothProfileManager.ACTION_STATE_CHANGED) ;
		if( task.isOppcTask() ){

			intent.putExtra(BluetoothProfileManager.EXTRA_PROFILE, BluetoothProfileManager.Profile.Bluetooth_OPP_Client);
		}
		else {
			intent.putExtra(BluetoothProfileManager.EXTRA_PROFILE, BluetoothProfileManager.Profile.Bluetooth_OPP_Server);
		}
		if( isConnect ){
		...
		intent.putExtra(BluetoothDevice.EXTRA_DEVICE, BluetoothAdapter.getDefaultAdapter().getRemoteDevice( task.getPeerAddr() ));
...
class BluetoothOppClientImpl extends IBluetoothOpp.Stub {
	...
	private OppManager manager;

	public BluetoothOppClientImpl( Context context ){

		this.context = context;
		this.manager = OppManager.getInstance(this.context);
	}

	public void disconnect( BluetoothDevice device ) throws RemoteException {

		this.manager.oppAbortDeviceTasks( device.getAddress() );
	}

	public BluetoothDevice getConnectedDevice() throws RemoteException {

		Uri uri = this.manager.oppcGetCurrentTask();
		return this.manager.oppQueryTaskDevice(uri);
	}

	public int getState() throws RemoteException {

		if( this.manager.oppcGetCurrentTask() != null ){
...
class BluetoothOppServerImpl extends IBluetoothOpp.Stub {
	...
	public BluetoothDevice getConnectedDevice() throws RemoteException {
		...
		Uri uri = this.manager.oppsGetCurrentTask();
		return this.manager.oppQueryTaskDevice(uri);
	}

	public int getState() throws RemoteException {

		if( this.manager.oppsGetCurrentTask() != null ){
