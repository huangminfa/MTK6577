...
//  support overflow menu in text view
...
//                break;
    ...
    public void setHorizontallyScrolling(boolean whether) {
        if (!isEditToolbarReadMode && mHorizontallyScrolling != whether) { 
    ...
    private void showError() {
        ...
        // Listen for ViewRoot's mCurScrollY change. If it changes, the Error PopUpWindow should update position
        mPopup.registerForViewRootScrollChanged(this, xoff, yoff);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // convert image id to drawable 
        BackgroundImageSpan.convert(getText(), getContext());
        ...
        */
        ...
        if (drawCursor && !isEditToolbarReadMode) {
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_DPAD_CENTER:
                if (!isEditToolbarReadMode && event.hasNoModifiers()) { 
                    /*
                    ...
                    if (!hasOnClickListeners()) {
                        if (mMovement != null && mText instanceof Editable
                            ...
                            InputMethodManager imm = InputMethodManager.peekInstance();
                            viewClicked(imm);
                            if (imm != null && mSoftInputShownOnFocus && isEnableShowSoftInput()) {
    ...
    protected void makeNewLayout(int wantWidth, int hintWidth,
        ...
        if (mEllipsize == TextUtils.TruncateAt.MARQUEE) {
            if (!compressText(ellipsisWidth)) {
                final int width = mLayoutParams.width;
    ...
    /**
      ...
      * @hide
      */
    public boolean isSelectionActionModeOverflowMenuShowing() {
        ...
        final boolean touchIsFinished = (action == MotionEvent.ACTION_UP) &&
         ...
         if ((mMovement != null || onCheckIsTextEditor()) && isEnabled()
            ...
            if (touchIsFinished && (isTextEditable() || mTextIsSelectable)) {
                ...
                viewClicked(imm);
                if (!isEditToolbarReadMode && !mTextIsSelectable && mSoftInputShownOnFocus && isEnableShowSoftInput()) {
                    handled |= imm != null && imm.showSoftInput(this, 0);
                }

                boolean selectAllGotFocus = mSelectAllOnFocus && didTouchFocusSelect();
                hideControllers();
                if (!isEditToolbarReadMode && !selectAllGotFocus && mText.length() > 0) {
                    if (mSpellChecker != null) {
    ...
    private boolean shouldBlink() {
        if (isEditToolbarReadMode || !isFocused()) return false; 
    ...
    private boolean startSelectionActionMode() {
        ...
        if (!willExtract || isEditToolbarReadMode) {
            ActionMode.Callback actionModeCallback = new SelectionActionModeCallback();
            mSelectionActionMode = startActionMode(actionModeCallback);
        }

        final boolean selectionStarted = mSelectionActionMode != null || willExtract;
        if (selectionStarted && !mTextIsSelectable && mSoftInputShownOnFocus && !isEditToolbarReadMode) {
            // Show the IME to be able to replace text, except when selecting non editable text.
            final InputMethodManager imm = InputMethodManager.peekInstance();
            if (imm != null && isEnableShowSoftInput()) {
    ...
    private class SelectionActionModeCallback implements ActionMode.Callback {
        ...
        public boolean onCreateActionMode(ActionMode mode, Menu menu) {
            ...
            if (!isEditToolbarReadMode && canCut()) { 
            ...
            if (!isEditToolbarReadMode && canPaste()) {  
    ...
    /**
     * Set showing the soft Input is enable or not
     * @hide
    */
    protected void setEnableShowSoftInput(boolean enableShowSoftInput) {
        mEnableShowSoftInput = enableShowSoftInput;
    }

    /**
     * Check showing the soft Input is enable or not
     * @hide
    */
    protected boolean isEnableShowSoftInput() {
        return mEnableShowSoftInput;
    }
    
    /**
     * Set browsering the url choice dialog or not. Only for MMS.
     * @hide
    */
    ...
    /**
     * Check browsering the url choice dialog or not. Only for MMS.
     * @hide
    */
    ...
    /**
     * Set highlight color from current skin.
     * 
    */
    private void setThemeHighlightColor(int color) {
        int themeHlColor = mContext.getResources().getThemeHighlightColor();
        if (color == HIGHLIGHT_COLOR_DEFAULT && themeHlColor != 0) {    //default style highlight color
    ...
    private boolean mEnableShowSoftInput = true;
    private boolean mEnableShowUrlDialog = false;
    /**
     * 
     * @hide
     */
    protected boolean isSuggestionUnderLineRefreshFlag =false;
    
    /**
     * define the default highlight color to a constant
     */
