...
public abstract class DefaultBulkInsertHelper {
    ...
    public DefaultBulkInsertHelper(){
        ...
        this.buffer = new ArrayList<ContentValues>(this.bulkLimit);
        this.parcelSize = 0;
    }
    
    public DefaultBulkInsertHelper(int bulkLimit){
    ...
    public boolean append(MeasuredContentValues measuredValues){
    	if (null == measuredValues){
            statusCode = SC_APPENDED;
    		return true;
    	}
        if (measuredValues.measure() > MAX_PARCEL_SIZE){
            // Single ContentValues already exceeds parcel size limit
            this.statusCode = SC_SINGLE_VALUE_EXCEED;
            return false;
        } else if (parcelSize + measuredValues.measure() > MAX_PARCEL_SIZE){
            // Buffer exceeds parcel size limit, bulk insert current 
            // ContentValues array, and free buffer for new ContentValues
            if (!execute()){
                return false;
            };
        } else if (buffer.size() >= bulkLimit) {
            // Insert 'bulkLimit' records at most at one time, bulk insert current 
            // ContentValues array, and free buffer for new ContentValues
            if(!execute()){
                return false;
            };
        }
        parcelSize += measuredValues.measure();
        buffer.add(measuredValues.getValues());
        statusCode = SC_APPENDED;
        return true;
    }
    ...
    public boolean execute(){
        if (buffer.size() > 0){
            ContentValues[] values = new ContentValues[buffer.size()];
            buffer.toArray(values);
            // Reset
            clear();
            // Execute insert
            if (onExecute(values)){
                statusCode = SC_EXECUTED;
                return true;
            } else {
                statusCode = SC_FAILED;
                return false;
            }
        } else {
            statusCode = SC_BUFFER_EMPTY;
            return true;
        }
    }
    
    public boolean isStatusOK(){
        return ((this.statusCode & SC_OK) != 0);
    }
