...
public class BluetoothSimapSettings 
	...
	//add by mtk81225
	private final static int MAX_SIM_NUM = 2;
	...
	public void handleMessage(Message msg) {
		...
		case BluetoothSimap.BT_SIMAPUI_DISABLED:
		    ...
		    mSimapServerEnable.setChecked(false);
		    mSimapServerEnable.setEnabled(true);
		    mSimapServerEnable.setSummary(R.string.bluetooth_simap_server_enable_summary);
		    break;
	...
	public void handleCheckSimState(){
		...
		if(simNum == MAX_SIM_NUM){
			...
			mSimapServerSimIndex.setValueIndex((simIndex-1));
			mSimapServerSimIndex.setSummary(simIndex == 1 ?
									R.string.bluetooth_simap_server_sim_index_summary_sim1:
			...
			for(i = 0; i < MAX_SIM_NUM; i ++){
				if(slot[i] == 1){
					//sim is exist
					try {
						mSimapService.selectSIM(i+1);
						Log.i(TAG, "simIndex = " + (i+1));
					}catch (RemoteException e) {
						Log.e(TAG, "getSIMIndex error");
					}
					mSimapServerSimIndex.setValueIndex(i);
					mSimapServerSimIndex.setSummary(i == 0 ?
													R.string.bluetooth_simap_server_sim_index_summary_sim1:
													R.string.bluetooth_simap_server_sim_index_summary_sim2);	
		...
		if (KEY_SIMAP_SERVER_ENABLE.equals(key)) {
			...
			if (KEY_SIMAP_SERVER_SIM_INDEX.equals(key)) {
				try {
					...
					mSimapServerSimIndex.setSummary(value == 1 ?
													R.string.bluetooth_simap_server_sim_index_summary_sim1:
													R.string.bluetooth_simap_server_sim_index_summary_sim2);			
