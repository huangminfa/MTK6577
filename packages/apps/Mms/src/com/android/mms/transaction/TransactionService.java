...
public class TransactionService extends Service implements Observer {
    ...
    public void onCreate() {
        ...
        Xlog.d(MmsApp.TXN_TAG, "Sticky Intent would be received:" + (mFWStickyIntent!=null?"true":"false"));
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        //add this case for read report.    
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                launchTransactionGemini(startId, intent.getIntExtra("simId",-1), args);
            } else {
                args = new TransactionBundle(intent.getExtras());
                launchTransaction(startId, args, noNetwork);
            }
            Xlog.d(MmsApp.TXN_TAG,"transaction type:"+args.getTransactionType()+",uri:"+args.getUri());
        } else {
    ...
    */
    ...
    private void stopSelfIfIdle(int startId) {
        ...
        synchronized (mProcessing) {
            if (mProcessing.isEmpty() && mPending.isEmpty() && mNeedWait == false/*avoid incorrectly stop service*/) {
                ...
                Xlog.d(MmsApp.TXN_TAG, "stop TransactionService.");
                sServiceAlive = false;
                stopSelf(startId);
            }
        }
    ...
    private void launchTransaction(int serviceId, TransactionBundle txnBundle, boolean noNetwork) {
        // if transaction type is notification, we must let it check network later(in it's run method).
        // in NotificationTransaction's run method, will check auto download setting and try to retrieve.
        // if returned here, the notification can not be scanned, because it's error status is no error.
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case EVENT_DATA_STATE_CHANGED:
                    ...
                    if (info == null) {
                        Xlog.d(MmsApp.TXN_TAG, "NetworkInfo == null.");
                        return;
                    }
                    int slotOfInfo = info.getSimId();
                    
                    Xlog.d(MmsApp.TXN_TAG, "Newwork info,reason: " + info.getReason() 
                                            + ",state:" + info.getState()
                    ...
                    if (!info.isConnected()) {
                        ...
                        if (FeatureOption.MTK_GEMINI_SUPPORT && pendingSize != 0) {
                            Transaction trxn = null;
                            synchronized (mProcessing) {
                                trxn = mPending.get(0);
                            }
                            int slotId = SIMInfo.getSlotById(getApplicationContext(), trxn.mSimId);
                            if (slotId != slotOfInfo) {
                                Xlog.d(MmsApp.TXN_TAG, "the connected slot not the one needed.");
                                return;
                            }
                        }
                    }

                    if (Phone.REASON_VOICE_CALL_ENDED.equals(info.getReason())){
                        if (0 != pendingSize){
                            Transaction trxn = null;
                            synchronized (mProcessing) {
                                trxn = mPending.get(0);
                    ...
                    if (FeatureOption.MTK_GEMINI_SUPPORT) {
                        if (SMART) {
                            if (Phone.GEMINI_SIM_1 == slotOfInfo || Phone.GEMINI_SIM_2 == slotOfInfo) {
                    ...
                    if (FeatureOption.MTK_GEMINI_SUPPORT) {
                        if (SMART) {
                            if (Phone.GEMINI_SIM_1 == slotOfInfo || Phone.GEMINI_SIM_2 == slotOfInfo) {
                                ...
                                if (null == si) {
                                    Xlog.e(MmsApp.TXN_TAG, "TransactionService:SIMInfo is null for slot " + slotOfInfo);
                                    return;
                                }
                                int simId = (int)si.mSimId;
        ...
        private boolean processTransaction(Transaction transaction) throws IOException {
            ...
            int requestResult = Phone.APN_REQUEST_FAILED;
            ...
            if (requestResult == Phone.APN_ALREADY_ACTIVE) {
                Xlog.d(MmsApp.TXN_TAG, "request ok, renew connection.");
    ...
    private void setTransactionFail(Transaction txn, int failType) {
        ...
        if (null != uri) {
            ...
            Xlog.e(MmsApp.TXN_TAG, "set Transaction Fail. uri is null.");
            return;
        }
