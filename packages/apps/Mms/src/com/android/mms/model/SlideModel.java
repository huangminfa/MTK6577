...
public class SlideModel extends Model implements List<MediaModel>, EventListener {
    ...
    private void internalAdd(MediaModel media) throws IllegalStateException {
        Xlog.i(TAG, "add media.");
        if (media == null) {
            Xlog.i(TAG, "internal add media. Media is null");
            Thread.dumpStack();
    ...
    private void internalAddOrReplace(MediaModel old, MediaModel media) {
        ...
        if (old == null) {
            ...
            /**
             * For fix bug [ALPS234350] [Root cause]: when replace an old media, the old media does not exist in
             * history. So, there is an exception occurred. The up is not the real root cause which has not been found.
             * [Solution]: there is no an effective way to solve this CR. This solution is a temporary solution.  
             */
            int mOldMediaIndex = mMedia.indexOf(old);
            if (mOldMediaIndex == -1) {
                Xlog.i(TAG, "There may be an exception for the old media not exist in history.");
                mMedia.add(media);
                increaseSlideSize(addSize);
                increaseMessageSize(addSize);
            } else {
