...
public:
	...
	void setupOutput();
	int doKmsgCat(const char *before_s, const char *fname, int fileSize, int shm_id);
	int buffer_write(int fd, const char *buffer, int len);
	int buffer_flush(int fd);
	void KmsgCat_exit(int ret);
...
bool isSdAlmostFull(char *filename);
unsigned long long getAvailSizeSD(char *mountpoint, int retryNum);
