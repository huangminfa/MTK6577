...
public class GalleryUtils {
    ...
    public static void showOnMap(Context context, double latitude, double longitude) {
        try {
            ...
            // Added by Mediatek: work around for Google map UI issue
            // the q=(lat, lng) syntax may result Google map show incomplete string:
            // "searching for (xx.xxx". Changing syntax to q=(lat lng), Google map will
            // show complete string "searching for (xx.xxx yy.yyy)"
            // This seems to be bug of Google Map, but we have to avoid for first solution,
            // so the following uri is changed to this syntax
