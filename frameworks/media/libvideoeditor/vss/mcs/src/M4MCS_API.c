                     ...
                     M4OSA_Void *pTempFile )
{
    ...
    * Reset reader related variables */
    ...
    pC->pReaderAudioStream = M4OSA_NULL;
#ifndef ANDROID_DEFAULT_CODE
    LOGD("M4MCS_open():InputFileType=%d",InputFileType);
#endif
    /*******************************************************/
        ...
        * In case we found a BIFS stream or something else...*/
        ...
        if( M4NO_ERROR == err ) /**< One stream found */
        {
...
#ifndef ANDROID_DEFAULT_CODE            
            LOGD("M4MCS_open():find Video Stream:m_streamType=%d (0 mpeg4/1 h263/9 avc)", pStreamHandler->m_streamType);
#endif            
            ...
            * Found the first audio stream */
            ...
            {
#ifndef ANDROID_DEFAULT_CODE            	
             LOGD("M4MCS_open():find Audio Stream:m_streamType=%d (aac/4, mp3/5)", pStreamHandler->m_streamType);
#endif             
    ...
    * Check state automaton */
    ...
    switch( pC->State )
    {
        case M4MCS_kState_READY:
            *pProgress = 0;
#ifndef ANDROID_DEFAULT_CODE            
		LOGD("M4MCS_step:M4MCS_kState_READY,*pProgress=%d",*pProgress);
#endif		
...
#ifndef ANDROID_DEFAULT_CODE                
		   if(*pProgress > 99)
		   	LOGD("M4MCS_step:M4MCS_kState_BEGINVIDEOJUMP,*pProgress=%d, EncodingVideoFormat=%d",*pProgress,pC->EncodingVideoFormat);
#endif		   	
                return err;
            }
            break;
...
#ifndef ANDROID_DEFAULT_CODE
    LOGD("M4MCS_pause(): returning M4NO_ERROR");
#endif    
    return M4NO_ERROR;
}
...
#ifndef ANDROID_DEFAULT_CODE
    LOGD("M4MCS_resume(): returning M4NO_ERROR");
#endif    
    return M4NO_ERROR;
}
    ...
    if( M4OSA_NULL != pC->pWriterContext ) /* happens in state _SET */
    {
        ...
        if( pC->novideo != M4OSA_TRUE )
        {
                ...
                && (M4ENCODER_kNULL == pC->EncodingVideoFormat) )
            {
                ...
                {
#ifndef ANDROID_DEFAULT_CODE                	
                    LOGV("Decoder specific info size = %d",
                        pC->m_pInstance->m_decoderSpecificInfoSize);
#endif	                        
                    ...
                    {
#ifndef ANDROID_DEFAULT_CODE                    	
                        LOGE("instance->m_pFinalDSI: allocation error");
#endif                        
                        return M4ERR_ALLOC;
                    }
...
#ifndef ANDROID_DEFAULT_CODE                    
                LOGV("calling set option M4WRITER_kDSI");
#endif                
                err = pC->pWriterGlobalFcts->pFctSetOption(pC->pWriterContext,
                    M4WRITER_kDSI, &streamHeader);
                M4OSA_TRACE1_0("set option done");
...
#ifndef ANDROID_DEFAULT_CODE
            {
                ...
                LOGI("set duration as %d, clip=%d, begin=%d, end=%d", 
...
#ifndef ANDROID_DEFAULT_CODE
    LOGD("M4MCS_close(): returning M4NO_ERROR");
#endif    
    return err;
}
                                ...
                                M4MCS_OutputParams *pParams )
{
    ...
    M4OSA_TRACE2_2(
        ...
        pContext, pParams);
#ifndef ANDROID_DEFAULT_CODE        
	LOGD("==========================M4MCS_setOutputParams========================== \n");
	LOGD("M4MCS_setOutputParams: OutputFileType=%d,OutputVideoFormat=%d,OutputVideoFrameRate=%d, pC->InputFileProperties.VideoStreamType=%d",
   	pParams->OutputFileType,pParams->OutputVideoFormat,pParams->OutputVideoFrameRate, pC->InputFileProperties.VideoStreamType);

	LOGD("M4MCS_setOutputParams: outputVideoProfile=%d,outputVideoLevel=%d,OutputAudioFormat=%d ",
	   							pParams->outputVideoProfile,pParams->outputVideoLevel,pParams->OutputAudioFormat );

	LOGD("M4MCS_setOutputParams: MediaRendering=%d,nbEffects=%d,OutputVideoFrameSize=%d ",
...
#ifndef ANDROID_DEFAULT_CODE            
		LOGD("M4MCS_setOutputParams():pParams->pEffects[%d].AudioEffectType=%d",j,pParams->pEffects[j].AudioEffectType);
#endif		
...
#ifndef ANDROID_DEFAULT_CODE    
 LOGD("=============================M4MCS_setEncodingParams=============================");
 LOGD("M4MCS_setEncodingParams:uiVideoBitrate=%d,uiAudioBitrate=%d,uiBeginCutTime=%d,uiEndCutTime=%d",
		pRates->OutputVideoBitrate,pRates->OutputAudioBitrate,pRates->BeginCutTime,pRates->EndCutTime);
 
 LOGD("M4MCS_setEncodingParams:uiClipDuration=%d,OutputVideoTimescale=%d, OutputFileSize=%d",
...
M4OSA_ERR M4MCS_checkParamsAndStart( M4MCS_Context pContext )
{
    ...
    M4OSA_ERR err;
#ifndef ANDROID_DEFAULT_CODE    
   LOGD("M4MCS_checkParamsAndStart:");
#endif
    /**
    * Check input parameters */
    M4OSA_DEBUG_IF2((M4OSA_NULL == pContext), M4ERR_PARAMETER,
...
#ifndef ANDROID_DEFAULT_CODE        
	  LOGD("M4MCS_intStepSet:pC->bH264Trim = M4OSA_TRUE;");
#endif	  
    }
...
static M4OSA_ERR M4MCS_intPrepareVideoEncoder( M4MCS_InternalContext *pC )
{
    ...
    if( pC->novideo )
        return M4NO_ERROR;
#ifndef ANDROID_DEFAULT_CODE	
    LOGD("M4MCS_intPrepareVideoEncoder:EncodingVideoFormat=%d,uiBeginCutTime=%d",
	...
	LOGD("M4MCS_intPrepareVideoEncoder: :after M4MCS_intLimitBitratePerCodecProfileLevel,Bitrate=%d ",EncParams.Bitrate);
    /**
    * Create video encoder */
        ...
        //    && (M4VIDEOEDITING_kH264 == pC->InputFileProperties.VideoStreamType))
    {
        ...
        LOGD("M4MCS_intPrepareVideoEncoder:mcs encoder open profile :%d, level %d",
...
#ifndef ANDROID_DEFAULT_CODE    
    LOGD("M4MCS_intPrepareVideoEncoder: M4MCS_kEncoderRunning");
#endif	
    ...
    * Compare video input size and video output size to check if resize is needed */
    ...
    {
#ifndef ANDROID_DEFAULT_CODE
	LOGD("M4MCS_intPrepareVideoEncoder: need resize (W %d,H %d)-> (W %d,H %d)",
...
static M4OSA_ERR M4MCS_intPrepareAudioProcessing( M4MCS_InternalContext *pC )
{
    ...
    * Allocate output buffer for the audio decoder */
    ...
    LOGD("pC->InputFileProperties.AudioStreamType %d pC->InputFileProperties.uiDecodedPcmSize %d",pC->InputFileProperties.AudioStreamType,pC->InputFileProperties.uiDecodedPcmSize);
#endif
...
static M4OSA_ERR M4MCS_intPrepareWriter( M4MCS_InternalContext *pC )
{
    ...
    * If there is a video input, allocate and fill the video stream structures for the writer */
    if( pC->novideo == M4OSA_FALSE )
    {
        ...
        * Update Encoder Header properties for Video stream if needed */
        ...
        {
#ifndef ANDROID_DEFAULT_CODE        	
            LOGD("M4MCS_intPrepareWriter:M4WRITER_kMUL_PPS_SPS");
#endif            
...
#ifndef ANDROID_DEFAULT_CODE
    LOGD("M4MCS_intPrepareWriter:W=%d,H=%d,EncodingVideoFormat=%d,EncodingVideoFramerate=%d,uiMaxFileSize=%d\n\
    ...
    * Return with no error */
#ifndef ANDROID_DEFAULT_CODE    
    LOGD("M4MCS_intPrepareWriter(): returning M4NO_ERROR");
#endif    
    return M4NO_ERROR;
}
...
static M4OSA_ERR M4MCS_intPrepareAudioBeginCut( M4MCS_InternalContext *pC )
{
    ...
    * In that case we need to slice the first AU to keep the 20 ms cut precision */
	...
	LOGD("M4MCS_intPrepareAudioBeginCut(): returning M4NO_ERROR pC->ReaderAudioAU.m_CTS %f pC->iAudioCtsOffset %d",pC->ReaderAudioAU.m_CTS,pC->iAudioCtsOffset);
#endif
    /**
    * Return with no error */
            ...
            && (uiAudioStepCount < 1)) )
        {
            ...
            * Check for end cut */
			...
			if((pC->dAuDecCurrentCts+pC->m_audioAUDuration)> pC->uiEndCutTime){
				LOGD("M4MCS_intStepEncoding pC->AudioState = M4MCS_kStreamState_FINISHED %f",pC->dAuDecCurrentCts);
				pC->AudioState = M4MCS_kStreamState_FINISHED;
                break;
			}
#else
...
#ifndef ANDROID_DEFAULT_CODE        
	  LOGD("M4MCS_intStepBeginVideoJump:H264MCS_ProcessSPS_PPS");
#endif	  
...
static M4OSA_ERR M4MCS_intAudioNullEncoding( M4MCS_InternalContext *pC )
{
    ...
    * Write it to the output file */
	...
	if( M4WAR_NO_MORE_AU == err2 )
	{
		/* The audio transcoding is finished */
		pC->AudioState = M4MCS_kStreamState_FINISHED;
		LOGD( "M4MCS_intAudioNullEncoding(): \
...
#ifndef ANDROID_DEFAULT_CODE            
		LOGE("M4MCS_intGetInputClipProperties:videoRotationDegrees=%d", pC->pReaderVideoStream->videoRotationDegrees);
#endif		
    }
    else
    {
    ...
    if( M4OSA_NULL != pC->pReaderAudioStream )
    {
        ...
        * Bugfix P4ME00001128: With some IMTC files, the AMR bit rate is 0 kbps according
		...
		if( pC->InputFileProperties.VideoStreamType == M4VIDEOEDITING_kNoneVideo ){
			...
			{
				LOGD("Video Editor support 8KHz ~ 48KHz only! nowe samplerate %d",pC->InputFileProperties.uiSamplingFrequency);
				pC->bUnsupportedAudioFound=M4OSA_TRUE;
...
#ifndef ANDROID_DEFAULT_CODE
    LOGI("duration v=%d, a=%d", pC->InputFileProperties.uiClipVideoDuration, 
            pC->InputFileProperties.uiClipAudioDuration);
#endif
...
#ifndef ANDROID_DEFAULT_CODE
    LOGD("M4MCS_intCheckAndGetCodecProperties :start");
#endif    
    ...
    if ( err == M4WAR_INFO_FORMAT_CHANGE ) {
        ...
        if (M4DA_StreamTypeAudioAac ==
			...
			switch( iAacType2 )
            {
			 ...
			 LOGD( "M4MCS_intGetInputClipProperties: AAC aSBRPresent=%d, aPSPresent=%d",
