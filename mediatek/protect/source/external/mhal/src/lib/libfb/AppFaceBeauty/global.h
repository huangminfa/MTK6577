...
int Map_extractor(unsigned char* r_data, unsigned char* g_data, unsigned char* b_data, int width, int height, int FD_y, int FD_x
				  ...
				  ,int sub_box_size, int sub_box_height, int sub_box_width, unsigned char* skin_block, unsigned char* AvgRGB_block
...
void template_drawing(int center_group, int face_pose, int sub_box_size, int sub_box_width, int sub_box_height
					  , int face_boundary_portion[6], unsigned char* new_GroupMask1, unsigned char* new_GroupMask, unsigned char* Map_temp
					  , int* left, int* right);
void template_matching(int Label_num, int Label_num1, int relaible_block_number, int relaible_block_number2, int sub_box_size, int sub_box_width, 
					   int sub_box_height, unsigned char* GroupMask, unsigned char* GroupMask_o, unsigned char* AvgRGB_block, unsigned char* new_GroupMask1,
					   int center_group, int top_place, int bottom_place, int avg_Cb, int avg_Cr, unsigned char* Map_temp, unsigned char* working_buffer);
void inner_block_filling(int sub_box_size, int sub_box_width, int sub_box_height, int bottom_place, unsigned char* SubSkinColorMask, int face_pose,
						 unsigned char* Map_temp);
void block_to_pixel(unsigned char* in_block,int sub_box_height,int sub_box_width,int block_size,unsigned char* Map_temp);
int Grouping_and_ExtractReliableGroup(unsigned char* SubSkinColorMask,unsigned char* GroupMask, int sub_box_height,int sub_box_width,
					  unsigned char* AvgRGB_block, int face_boundary_portion[6],int face_pose, int mode, unsigned char* new_SubSkinColorMask
					  ,int* relaible_block_number, int* top_place, int* bottom_place, int* avg_Cb, int* avg_Cr, int* GroupMask_tmp, unsigned char* working_buffer);
void Face_Boundary_Reconstruction(unsigned char* GroupMask, int sub_box_height, int sub_box_width, int center_group, int face_boundary_portion[6], 
...
void watershed_expansion(unsigned char* edge_power_all,unsigned char* SubSkinColorMask,
						 int sub_box_height,int sub_box_width,int sub_box_size,int ori_box_height,int ori_box_width,unsigned char* skin_tone_map1, int middle_edge, unsigned char* SubSkinColorMask_boundary,int use_texture_only, unsigned char* expansion_working_buffer);
#if(local_skin_tone_en==1)
	void alpha_map_func(unsigned char* SkinColorMask,unsigned char* edge_power_all, int input_img_height, int input_img_width,unsigned char* SubSkinColorMask_boundary
					,int sub_box_height,int sub_box_width,int sub_box_size,int min_edge, int alpha_constant,unsigned char* Skin_block);
#else
	void alpha_map_func(unsigned char* SkinColorMask,unsigned char* edge_power_all, int input_img_height, int input_img_width,unsigned char* SubSkinColorMask_boundary
					,int sub_box_height,int sub_box_width,int sub_box_size,int min_edge, int alpha_constant);
#endif
	void Gaussian_filter(unsigned char* input,unsigned char* output,double theta,int box_size, int input_img_height, int input_img_width, int average_only);
