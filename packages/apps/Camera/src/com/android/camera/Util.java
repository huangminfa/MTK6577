...
public class Util {
    ...
    //Mediatek modify begin
    private static boolean sIsMAVSupport = false;
    private static boolean sIsPANORAMASupport = false;
    private static boolean sIsVideoCameraSupport = false;
    public static final boolean CMCC;
    public static final boolean CU;
    private static Uri sLastUri = null;
    static {
        String operator = SystemProperties.get("ro.operator.optr");
        if ("OP01".equals(operator)) {
            CMCC = true;
            CU = false;
        } else if ("OP02".equals(operator)) {
            CU = true;
            CMCC = false;
        } else {
            CU = false;
    ...
    public static boolean STEREO3D_MODE = false;
    public static void setStereo3DMode(String stereo3DMode) {
        STEREO3D_MODE = CameraSettings.STEREO3D_ENABLE.equals(stereo3DMode);
        Log.v(TAG, "STEREO3D_MODE=" + STEREO3D_MODE);
    }
    private static boolean s3DMode = false;
    public static boolean getS3DMode() {
        Log.v("Test", "s3DMode=" + s3DMode);
        return s3DMode;
    }
    public static boolean switch3DMode() {
        s3DMode = !s3DMode;
        return s3DMode;
    }
    public static boolean reset3DMode() {
        s3DMode = false;
        return s3DMode;
    }
...
//Mediatek modify begin
    public static void setModeSupport(Parameters parameter) {
        ...
        // single3d & panorama3d mode check
        if (getS3DMode()) {
            sIsMAVSupport = false;
        } else {
            sIsVideoCameraSupport = true;
        }
    }
    ...
    public static boolean isVideoCameraSupport() {
        return sIsVideoCameraSupport;
    }
//Mediatek modify end
    ...
    public static boolean isUriValid(Uri uri, ContentResolver resolver) {
        ...
        // we will check here to avoid thumbnail cannot be updated when recording a video in videowallpaper.
        if (sLastUri != null && !uri.equals(sLastUri)) return false;
        sLastUri = null;
        ...
        return getMountPointChange(uri, resolver);
    }

    private static boolean getMountPointChange(Uri uri, ContentResolver resolver) {
        ...
        Log.v(TAG, "path =" + path + "; Storage.MOUNT_POINT =" + Storage.MOUNT_POINT);
        return path.startsWith(Storage.MOUNT_POINT + File.separator);
    }
    ...
    //Mediatek modify begin
    public static boolean hasItem(String value, String[] items) {
    ...
    public static boolean hasItem(String value, List<String> items) {
        if (items == null || value == null) return false;
        for (String item : items) {
            if (value.equals(item)) return true;
        }
    ...
    public static boolean isSupported(String value, String[] supported) {
