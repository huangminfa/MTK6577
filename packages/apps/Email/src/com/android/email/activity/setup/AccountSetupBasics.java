...
public class AccountSetupBasics extends AccountSetupActivity
    ...
    public final static boolean ENTER_DEBUG_SCREEN = Build.TYPE.equals("eng") ? true : false;;

    // This static variable is only used in the case: 
    // Contact -> add a new contact -> add a new account -> select "corporate" -> Email.AccountSetupBasics.
    ...
    private static String TAG = "AccountSetupBasics";
    ...
    public static void actionNewAccount(Activity fromActivity) {
        ...
        // Add flag FLAG_ACTIVITY_CLEAR_TOP to avoid duplicate activities
...
//        i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    ...
    /**
     * This method is specially designed for adding account from Settings app, and this design is ugly.
     ...
     * This design is to avoid a) the transparent activity com.android.settings.account.AddAccountSettings 
     * is not finished in some cases. b) Can build two accounts in the same time, one from Settings, another from Email.
     ...
     */
    public static void addAccountFromExtenal(Context context, boolean isEasAccount, AccountAuthenticatorResponse response) {
        Intent i = new Intent(context, AccountSetupBasics.class);
        i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        if (isEasAccount) {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (flowMode == SetupData.FLOW_MODE_ACCOUNT_MANAGER_EAS || flowMode == SETTINGS_APP_ADD_EAS_ACCOUNT) {
    ...
    public void onResume() {
        super.onResume();
        int flowMode = getIntent().getIntExtra(EXTRA_FLOW_MODE, SetupData.FLOW_MODE_UNSPECIFIED);
        // If the return value of getCallingActivity() is null, the caller does not use startActivityForResult().
        // Otherwise, caller use startActivityForResult(), this situation could only happen in the case: 
        // Contact -> add a new contact -> add a new account -> select "corporate" -> Email.AccountSetupBasics.
        if (getCallingActivity() == null && (flowMode & (1 << 4)) != 0) {
            Logging.d("Add account from AuthenticatorService, will restart Email app using flag NEW_TASK and CLEAR_TOP!");
            if (flowMode == SETTINGS_APP_ADD_EAS_ACCOUNT) {
                AccountAuthenticatorResponse reponse = getIntent().getParcelableExtra(
                        AccountManager.KEY_ACCOUNT_AUTHENTICATOR_RESPONSE);
                addAccountFromExtenal(this, true, reponse);
                finish();
                return;
            } else if (flowMode == SETTINGS_APP_ADD_POPIMAP_ACCOUNT) {
                AccountAuthenticatorResponse reponse = getIntent().getParcelableExtra(
                        AccountManager.KEY_ACCOUNT_AUTHENTICATOR_RESPONSE);
                addAccountFromExtenal(this, false, reponse);
                finish();
                return;
            }
        }
        // SETTINGS_APP_ADD_EAS_ACCOUNT is just a temporary variable, so we need to covert it to normal.
        if (flowMode == SETTINGS_APP_ADD_EAS_ACCOUNT) {
    ...
    // following code such as onStart, onStop and mPostDrawListener is only used
    // to test Email launch performance. 
    ...
    private OnPostDrawListener mPostDrawListener = new OnPostDrawListener() {

        public boolean onPostDraw() {
            Xlog.i("AppLaunch", "[AppLaunch] email onPostDraw");
            return true;
        }
    };
