...
bool mt6577_detect_dl_keys (void)
{
    mt6577_kpd_gpio_set();
    if (mt6577_detect_key (KPD_DL_KEY1) &&
        mt6577_detect_key (KPD_DL_KEY2) && mt6577_detect_key (KPD_DL_KEY3))
    {
...
void mt6577_kpd_gpios(unsigned int ROW_REG[], unsigned int COL_REG[])
{
	int i;
	for(i = 0; i< 8; i++)
	{
...
void mt6577_kpd_gpio_set(void)
{
	...
	mt6577_kpd_gpios(ROW_REG, COL_REG);
