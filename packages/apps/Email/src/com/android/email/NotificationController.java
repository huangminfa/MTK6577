...
public class NotificationController {
    ...
    private Notification createAccountNotification(Account account, String ticker,
        ...
        if (enableAudio) {
            Logging.d(Logging.LOG_TAG, "[Notification] createAccountNotification for account: " + account.mId);
    ...
    void setupSoundAndVibration(Notification.Builder builder, Account account) {
        ...
        Logging.d(Logging.LOG_TAG, "[Notification] setupSoundAndVibration default flag " + defaults);
    ...
    /**
     * Returns a notification ID for email sending failed notifications for the given account account.
     */
    ...
    private static class UnreadMessagesCountObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            ...
            if (mUnreadNumOfAllInbox != unreadCount) {
                ...
                i.putExtra(Intent.MTK_EXTRA_UNREAD_NUMBER, mUnreadNumOfAllInbox);
                i.putExtra(Intent.MTK_EXTRA_UNREAD_COMPONENT, new ComponentName("com.android.email",
                                "com.android.email.activity.Welcome"));
                mContext.sendBroadcast(i);
                Log.d(Logging.LOG_TAG, "!!!Unread count changed: send broadcast to laucher : unreadCount = "
                        + mUnreadNumOfAllInbox);

                android.provider.Settings.System.putInt(mContext
                        .getContentResolver(), "com_android_email_mtk_unread",mUnreadNumOfAllInbox);
                // test begin. Must be removed in the future.
                try {
                    int result = android.provider.Settings.System.getInt(
                            mContext.getContentResolver(),
                            "com_android_email_mtk_unread");
                    Log.d(Logging.LOG_TAG,
                            "!!!Unread count changed: read result from settings: "
                                    + result);
                } catch (Exception e) {
                    Log.w(Logging.LOG_TAG,
                            "!!![Exception]Unread count changed: read result from settings exception");
                }
               // test end.
            }
        }
        ...
        public void onChange(boolean selfChange) {
            ...
            try {
                ...
                if (newMessageCount == 0) {
                    ...
                    if (n != null) {
                        Logging.d(Logging.LOG_TAG, "[Notification] notify default flag " + n.defaults);
                        // Make the notification visible
                        sInstance.mNotificationManager.notify(
                                sInstance.getNewMessageNotificationId(mAccountId), n);
                    }
                    NotificationPlus notiPlus = sInstance.createNewMessageNotificationPlus(
                            mAccountId, mMailboxId, newMessageId,
                            newMessageCount, unreadCount);
                    if (notiPlus == null) {
     ...
     */
    NotificationPlus createNewMessageNotificationPlus(long accountId, long mailboxId, long messageId,
            int unseenMessageCount, int unreadCount) {
        final Account account = Account.restoreAccountWithId(mContext, accountId);
        if (account == null) {
            return null;
        ...
        final String title = String.format(mContext
                .getString(R.string.notification_multiple_new_messages_fmt),
                unseenMessageCount);
        ...
        .setMessage(text)
        .setPositiveButton(mContext.getString(android.R.string.ok), pending)
        .create();
