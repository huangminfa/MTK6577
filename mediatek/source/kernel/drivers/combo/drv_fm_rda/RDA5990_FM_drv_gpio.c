...
#define FM_DEV_MAJOR	193 //0 -- dynamic allocation; xxx -- static allocation
...
static uint16_t RDAFM_CHIP_ID = 0x0;
...
static int RDAFM_GetChipID(uint8_t chipAddr, uint16_t *pChipID)
{
	...
	if(err < 0){
		FM_ALERT("RDAFM_GetChipID: reset FM chip failed!\n");
		return -1;
	}
	...
	if (err == 0)
	{
		...
		{
			FM_ALERT("RDAFM_GetChipID: get chip ID failed! get value = %04X\n", val);
			ret = -1;
		}
	...
	{
		FM_ALERT("RDAFM_GetChipID: get chip ID failed!\n");
		ret = -1;
	}
...
static int RDAFM_write(uint8_t chipAddr, uint8_t regAddr, uint16_t val)
{
	...
	ret = rda_gpio_i2c_write_1_addr_2_data(chipAddr, regAddr, val);
	if (ret < 0)
	{
		...
		uint16_t space)
{
	...
	//mute FM
	ret = RDAFM_enable_hmute(chipAddr);
	if (ret < 0)
	{
		FM_ALERT("RDAFM_enable_hmute failed!\n"); 
		return false;
	}

	//set seekth
	tRegValue = 0;
	ret = RDAFM_read(chipAddr, 0x05, &tRegValue);
	if (ret < 0)
	{
		FM_ALERT("RDAFM_read failed!\n"); 
		return false;
	}
	tRegValue &= (~(0x7f<<8));
	tRegValue |= ((FM_SEEK_THRESHOLD & 0x7f) << 8);
	ret = RDAFM_write(chipAddr, 0x05, tRegValue);
	if (ret < 0)
	{
		FM_ALERT("RDAFM_write failed!\n"); 
		return false;
	}
	...
	do {
		...
		tRegValue = 0;
		ret = RDAFM_read(chipAddr, 0x03, &tRegValue);
		if (ret < 0)
		{
			FM_ALERT("RDAFM_read failed!\n"); 
			return false;
		...
		if(isTrueStation){
			FM_DEBUG("channel %d is a true station.", (channel - 5));
			channel = channel - 5;
			...
			FM_DEBUG("channel %d is not a true station.", (channel - 5));
		}
	...
	*pFreq = 0;
	...
	//clear FM mute
	ret = RDAFM_clear_hmute(chipAddr);
	if (ret < 0)
	{
		FM_ALERT("RDAFM_clear_hmute failed!\n"); 
		return false;
	}
...
static int fm_setup_cdev(struct fm *fm)
{
	...
	if(fm_dev_major){
		...
		if (err) {
			FM_ALERT("register dev_t failed\n");
			return -1;
		}
		FM_ALERT("register device %s:%d:%d\n", FM_NAME,
		...
		case FM_IOCTL_RDS_SUPPORT:
			{
				...
				FM_DEBUG("RDA FM don't support RDS!\n");
...
static int fm_init(void)
{
	...
	if(ret < 0){
		...
		ret = rda_fm_power_on();
		if(ret < 0){
			FM_ALERT("%s() enable the FM chip in combo failed!\n", __func__);
			err = -ENOMEM;
			goto ERR_EXIT;
		}
		msleep(2*FM_SLEEP_TIME);
		ret = RDAFM_GetChipID(fm->chipAddr, &RDAFM_CHIP_ID);
		if(ret < 0){
			FM_ALERT("%s() Get the FM chip ID in combo failed!\n", __func__);
			err = -ENOMEM;
			goto ERR_EXIT;
		}else{
		...
		if(ret < 0){
			FM_ALERT("%s() power off the FM chip in combo failed!\n", __func__);
			err = -ENOMEM;
			goto ERR_EXIT;
		}
		FM_ALERT("%s() power off the FM chip in combo success!\n", __func__);
	}else{
...
static int fm_powerup(struct fm *fm, struct fm_tune_parm *parm)
{
	...
	if(fm->chip_id == ID_RDA5820NS){
		ret = rda_fm_power_on();
		if(ret < 0){
			FM_ALERT("%s() power on the FM chip in combo failed!\n", __func__);
			return -EPERM;
		}
		msleep(2*FM_SLEEP_TIME);
	}
	...
	//Reset RDA FM
	tRegValue = 0x0002;
	ret = RDAFM_write(fm->chipAddr, 0x02, tRegValue);
	if(ret < 0){
		FM_ALERT("%s() RDAFM_write failed!\n", __func__);
		return -EPERM;
	}
	msleep(2*FM_SLEEP_TIME);
...
static int fm_powerdown(struct fm *fm)
{
	...
	ret = RDAFM_read(fm->chipAddr, 0x02, &tRegValue);
	if(ret < 0){
		FM_ALERT("%s() RDAFM_read failed!\n", __func__);
		return -EPERM;
	}
	tRegValue &= (~(1 << 0));
	ret = RDAFM_write(fm->chipAddr, 0x02, tRegValue);
	...
	if(fm->chip_id == ID_RDA5820NS){
		ret = rda_fm_power_off();
		if(ret < 0){
			FM_ALERT("%s() rda_fm_power_off failed!\n", __func__);
			return -EPERM;
		}
	}
...
static int fm_seek(struct fm *fm, struct fm_seek_parm *parm)
{
	...
	if (!fm->powerup)
	{
		FM_ALERT("%s() FM is power off, cann't seek!\n", __func__);
		parm->err = FM_BADSTATUS;
		return -EPERM;
	}
	...
	ret = RDAFM_enable_hmute(fm->chipAddr);
	if(ret < 0){
		FM_ALERT("%s() RDAFM_enable_hmute failed!\n", __func__);
		return -EPERM;
	}
	...
	ret = RDAFM_clear_hmute(fm->chipAddr);
	if(ret < 0){
		FM_ALERT("%s() RDAFM_clear_hmute failed!\n", __func__);
		return -EPERM;
	}
...
static int  fm_scan(struct fm *fm, struct fm_scan_parm *parm)
{
	...
	ret = RDAFM_read(fm->chipAddr, 0x03, &tRegValue);
	if(ret < 0){
		FM_ALERT("%s() RDAFM_read failed!\n", __func__);
		return -EPERM;
	}
	...
	//set space and band
	ret = RDAFM_write(fm->chipAddr, 0x03, tRegValue);
	if(ret < 0){
		FM_ALERT("%s() RDAFM_write failed!\n", __func__);
		return -EPERM;
	}
...
static void platform_fm_release(struct device *dev)
{
	return;
}
