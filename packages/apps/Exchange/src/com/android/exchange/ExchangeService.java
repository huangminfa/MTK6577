...
public class ExchangeService extends Service implements Runnable {
    ...
    // The base wake lock id for EasSyncService alarm, for each mailbox, its wake lock id = ALARM_WAKELOCK_ID - mailboxId  
    ...
    private final IEmailService.Stub mBinder = new IEmailService.Stub() {
        ...
        public void sendMeetingResponse(long messageId, int response) throws RemoteException {
            Logging.v(TAG, "sendMeetingResponse for messageId: " + messageId + " and response: " + response);
                    ...
                    for (Account account : mAccountList) {
                        ...
                        if (!currentAccounts.contains(account.mId) && !accountIncomplete) {
                            ...
                            if (deletedAccount != null) {
                                ...
                                // Stop ExchangeService since no EAS account exists 
                            ...
                            if (account.mSyncInterval != updatedAccount.mSyncInterval
                                ...
                                // If user changed the SyncInterval settings, we will stop all current syncs; 
                                // and the appropriate ones will restart. 
                                // But if only SyncLookback changed, the following code will not stop all current syncs. 
                                if (account.mSyncInterval != updatedAccount.mSyncInterval) {
    ...
    private void stopAccountSyncs(long acctId, boolean includeAccountMailbox, boolean isSaveReason) {
    ...
    static public void setEasSyncAlarm(long id, long millis) {
        log("ExchangeService"+"-setEasSyncAlarm");
        ExchangeService exchangeService = INSTANCE;
        exchangeService.setAlarm(id, millis, true);
   }

    static public void clearEasSyncAlarm(long id) {
    ...
    static public void acquireEasWakeLock(long id) {
        ExchangeService exchangeService = INSTANCE;
        if (exchangeService != null) {
            exchangeService.acquireWakeLock(id);
        }
    ...
    static public void releaseEasWakeLock(long id) {
        ExchangeService exchangeService = INSTANCE;
        if (exchangeService != null) {
            exchangeService.releaseWakeLock(id);
        }
    ...
    static public void notifyEasSyncService(long id) {
        ExchangeService exchangeService = INSTANCE;
    ...
    private void stopServiceThreads() {
        synchronized (sSyncLock) {
            ...
            for (Long mailboxId : toStop) {
                ...
                if (svc != null) {
                    log("Stopping " + (svc.mAccount != null ? svc.mAccount.mDisplayName : "Account is null. ") + '/'
                        + (svc.mMailbox != null ? svc.mMailbox.mDisplayName : " Mailbox is null. "));
                    EmailAsyncTask.runAsyncParallel(new Runnable(){
    ...
    private long checkMailboxes () {
        ...
        try {
            while (c.moveToNext()) {
                ...
                if (service == null) {
                    Logging.v(TAG, "Syncing thread for mailbox " + c.getString(
                            Mailbox.CONTENT_DISPLAY_NAME_COLUMN) + " mailbox id:" + mailboxId + " is not started");
        ...
        if (mInPushMode) {
            ...
            log("we set ExchangeService nextwait to ONE_DAY_MINUTES + 1 to stop ExchangeService alarm");
        }
        return nextWait;
    }
    ...
    static public void fetchMailRequest(Request req) {
        ExchangeService exchangeService = INSTANCE;
        if (exchangeService == null) return;
        Message msg = Message.restoreMessageWithId(exchangeService, req.mMessageId);
        if (msg == null) {
    ...
    public void sendMessages(long accountId, long mailboxId, Uri uri, 
        ...
        if (exchangeService == null) {
            try {
                ...
                Context context = getContext();
                context.startService(new Intent(context, ExchangeService.class));
                //The ExchangeService need sometime to start it. Not wait it just return failed callback.
                callback.sendCallback(EmailExternalConstants.RESULT_FAIL, accountId, EmailExternalConstants.TYPE_SEND);
        ...
        if (mailbox != null) {
            ...
            AbstractSyncService service = new EmailExternalEas(exchangeService, mailbox, uri, callback, saveToSent);
            startServiceThread(service, mailbox);
        } else {
            try {
                callback.sendCallback(EmailExternalConstants.RESULT_FAIL, accountId, EmailExternalConstants.TYPE_SEND);
    ...
    static public int pingStatus(long mailboxId) {
        ...
        Logging.d(TAG, "Error occurred since last running for mailboxId: " + mailboxId
                + " with error: " + error + " and holdTime: " + (error != null ? error.holdEndTime : 0));
        if (error != null) {
            if (error.fatal) {
