 ...
 * <p>Package: com.mediatek.apst.target.data.proxy.message
 * <p>
 * <p>Description: 
 * <p>Transfer whole MMS information to PC for backup
 * <p>
...
public class FastMmsBackupCursorParser extends FastCursorParser {
    ...
    public FastMmsBackupCursorParser(Cursor c, IRawBlockConsumer consumer,
...
/*
        ...
        c_addr = mMessageProxy.getContentResolver().query(uri_addr, null, null,
                null, null);  
        TargetAddress mmsAddress;
        if (null != c_addr) {            
            int c_count = c_addr.getCount();
            if (c_count != 0) {
                c_addr.moveToFirst();
                if (mms.getBox() == Mms.BOX_SENT && c_count > 1) {
                    c_addr.moveToNext();
                }
...
*/
        Cursor c_part = mMessageProxy.getContentResolver().query(
        ...
        if (null != c_part) {
            while (c_part.moveToNext()) {
                MmsPart part = MmsContent.cursorToMmsPart(c_part);
                if (null != part.getDataPath()) {
                    ...
                    try {
                        ...
                        if (null != is && is.available() != 0) {
                            ...
                            byte[] buffer_temp = new byte[256];
                            int len = is.read(buffer_temp);
                            while (len != -1) {
                                baos.write(buffer_temp, 0, len);
                                len = is.read(buffer_temp);
                            }
