...
public class DmPim1NodeIoHandler implements NodeIoHandler {
    ...
    private String[] mItem = {"Addr", "ConnProfile", "AddressBookURI", "CalendarURI"};
//	private String mBase = "content://com.scan.cmpim/DevDetail/Ext/Conf/DataSync/";
    private String[] mProjection = {"PIM_SERVER","PIM_APN","PIM_CONTACT","PIM_CALENDAR"};
            ...
            for (int i = 0; i < mItem.length; i++) {
                if (mUri.getPath().contains(mItem[i])) {
                    if ((String)mMap.get(mItem[i]) != null) {
                        ...
                        try {
                            recordToRead = Settings.System.getString(mContext.getContentResolver(), dbKey);
                        } catch (Exception e) {
                            e.printStackTrace();
