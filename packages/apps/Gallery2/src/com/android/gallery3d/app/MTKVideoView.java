    ...
    private void initialize() {
        mPreparedListener = new MediaPlayer.OnPreparedListener() {
            public void onPrepared(MediaPlayer mp) {
                ...
                if (LOG) MtkLog.v(TAG, "mPreparedListener.onPrepared() mCanPause=" + mCanPause);
        ...
        mErrorListener = new MediaPlayer.OnErrorListener() {
            public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
                ...
                if (LOG) Log.v(TAG, "onError() mSeekWhenPrepared=" + mSeekWhenPrepared + ", mDuration=" + mDuration);
        ...
        mSHCallback = new SurfaceHolder.Callback() {
            ...
            public void surfaceCreated(SurfaceHolder holder) {
                if (LOG) Log.v(TAG, "surfaceCreated(" + holder + ")");
...
//            }
        ...
        if (LOG) MtkLog.v(TAG, "onMeasure() mNeedWaitLayout=" + mNeedWaitLayout);
        if (mNeedWaitLayout) {//when OnMeasure ok, start video.
        	mNeedWaitLayout = false;
            mHander.sendEmptyMessage(MSG_LAYOUT_READY);
        }
    }
...
//    }
    ...
    public void setOnVideoSizeChangedListener(OnVideoSizeChangedListener l) {
        ...
        if (LOG) Log.i(TAG, "setOnVideoSizeChangedListener(" + l + ")");
    }
    ...
    public int getDuration() {
        boolean inPlaybackState = isInPlaybackState();
        if (LOG) Log.v(TAG, "getDuration() mDuration=" + mDuration + ", inPlaybackState=" + inPlaybackState);
        if (inPlaybackState) {
            if (mDuration > 0) {
                return mDuration;
            }
    ...
    public void clearDuration() {
        if (LOG) MtkLog.v(TAG, "clearDuration() mDuration=" + mDuration);
        mDuration = -1;
    }
