...
/**
 * Note: should in the same process with app and service.
 * If service is moved to other process, this class should be modified.
 */
public class EBManager implements onClientEBMListener {
    ...
    private final PriorityQueue<Profile> mCacheQueue =
        new PriorityQueue<Profile>(MAX_CAPABILITY, Profile.getComparator());
    private Context mContext;
    private NotificationManager mNotificationManager;
    private ContentResolver mContentResolver;
    private EBListener mOnEBListener;
    private boolean mLoad;
    
    private static final String[] EB_PROJECTION = new String[]{
        EB.Broadcast._ID,
        EB.Broadcast.NET_LEVEL,
        EB.Broadcast.NET_ID,
        EB.Broadcast.MSG_ID,
    ...
    private int[] ICONS = {
            R.drawable.ebm_level1,
            R.drawable.ebm_level2,
            R.drawable.ebm_level3,
            R.drawable.ebm_level4
    };
    ...
    private void loadProfiles() {
        if (mLoad) return;
        Cursor cursor = mContentResolver.query(EB.Broadcast.CONTENT_URI,
                EB_PROJECTION, null, null, null);
        if (cursor != null) {
            //here don't check max count for we control the insert.
            while(cursor.moveToNext()) {
                Profile profile = new Profile();
                profile._id = cursor.getLong(COL_ROW_ID);
                profile.net_level = cursor.getInt(COL_NET_LEVEL);
                profile.net_id = cursor.getInt(COL_NET_ID);
                profile.msg_id = cursor.getInt(COL_MSG_ID);
                profile.priority = cursor.getLong(COL_ADD_TIME);
                mCacheQueue.add(profile);
            }
            if (LOG) Log.v(TAG, "loadProfiles() mCacheQueue.size()=" + mCacheQueue.size());
            cursor.close();
        }
        mLoad = true;
    }
...
//    }
    ...
    public synchronized void EBMCallback(CMMBEmergencyMessage msg) {
//      testMsg(msg);
        if (msg == null || !isFormal(msg.msg_level)) {
            if (LOG) Log.v(TAG, "EBMCallback() not formal " + msg);
            return;
        }
        loadProfiles();
        ...
        if (!mCacheQueue.contains(profile)) {
            if (mCacheQueue.size() < MAX_CAPABILITY) {
                ...
                if (deleteDB(mContentResolver, old._id)) {
                    mNotificationManager.cancel((int)old._id);
                    mCacheQueue.poll();//drop it
                    add(msg, profile);
                }
            }//end < MAX_CAPABILITY
        }//end contains
        if (LOG) Log.v(TAG, "EBMCallback() size=" + mCacheQueue.size() + ", " + profile);
    }
    ...
    private void checkNotify(CMMBEmergencyMessage msg, Profile profile, Uri data) {
        ...
        notification.icon = ICONS[formalLevel - 1];
        ...
        mNotificationManager.notify((int)profile._id, notification);//need check
        
        if (ModeSwitchManager.isInForground()) {
            if (formalLevel < 3) {// 1,2 will show detail, 3,4 just notify
                goToMessageDetails(data);
            } else {//notify other listener
                if (mListeners != null) {
                    for(EBListener listener : mListeners) {
                        if (listener != null) {
        ...
        if (LOG) Log.w(TAG, "checkNotify(" + profile + ", " + data + ") level=" + msg.msg_level
                + ", front=" + ModeSwitchManager.isInForground());
    }
    ...
    private Uri insertDB(CMMBEmergencyMessage msg, long receiveTime) {
        ...
        if (LOG) Log.v(TAG, "insertDB(" + msg + ", " + receiveTime + ") return " + newUri);
        return newUri;
    }
    ...
    private long getID(Uri uri) {
        ...
        if (LOG) Log.v(TAG, "getID(" + uri + ") return " + id);
        return id;
    }
    ...
    private void remove(long id) {
        ...
        if (LOG) Log.v(TAG, "remove(" + id + ")");
    }
    
    private static boolean deleteDB(ContentResolver cr, long id) {
        int result = cr.delete(EB.Broadcast.CONTENT_URI,
                EB.Broadcast._ID + "=? ", new String[]{String.valueOf(id)});
        if (result != 1) {
            Log.e(TAG, "deleteDB(" + id + ") result=" + result);
        }
        if (LOG) Log.v(TAG, "deleteDB(" + id + ") result=" + result);
        return result == 1;
    }
    
    private void clear() {
        if (LOG) Log.v(TAG, "clear() mCacheQueue.size()=" + mCacheQueue.size());
    ...
    private static void clearDB(ContentResolver cr) {
        ...
        if (LOG) Log.v(TAG, "clearDB() result=" + result);
    }
    
    private static EBManager mManager;
    private static ArrayList<EBListener> mListeners;
    /**
     * Singlton mode. 
     * Return the instance of EBMananger. 
     * @param context
     * @return
     */
    /*package*/ static EBManager getInstance(Context context) {
        if (mManager == null) {
            mManager = new EBManager(context);
        }
        return mManager;
    }
     ...
     * @param id
     */
    public synchronized static void delete(ContentResolver cr, long id) {
        if (mManager != null) {
            mManager.remove(id);
        } else {
            deleteDB(cr, id);
     ...
     * @param cr
     */
    public synchronized static void clear(ContentResolver cr) {
        if (mManager != null) {
            mManager.clear();
        } else {
            clearDB(cr);
    ...
    public static void cancelNotification() {
        if (mManager != null) {
            mManager.mNotificationManager.cancelAll();
        }
    }
    
    /**
     * Cancel one notification of specified EB message.
     * @param id the row id of EB message.
     */
    public static void cancelNotification(int id) {
        if (mManager != null) {
            mManager.mNotificationManager.cancel(id);
        }
    }
    ...
    public static boolean addEBListener(EBListener listener) {
        if (mListeners == null) {
            mListeners = new ArrayList<EBListener>();
        }
    ...
    /**
     * Profile of Emergency Broadcast.
     * net_level, net_id and msg_id will be identity.
     * If these three members are same, we think profiles are the same.
     *
     */
    private static class Profile {
        /*package*/ long _id;
        /*package*/ int net_level;
        /*package*/ int net_id;
        /*package*/ int msg_id;
        ...
        /*package*/ static Profile copy(CMMBEmergencyMessage em) {
            Profile profile = new Profile();
            profile.net_level = em.net_level;
            profile.net_id = em.net_id;
            profile.msg_id = em.msg_id;
//            profile.priority = getTime(em.msg_date, em.msg_time);//need check
            profile.priority = System.currentTimeMillis();
            return profile;
        }
        
        private static Calendar cal = new GregorianCalendar(TimeZone.getTimeZone("GMT+08:00")); 
        private static long getTime(int date, int time) {
            ...
            if (LOG) Log.v(TAG, "getTime(" + date + ", " + time + ") "
                    + String.format("%d-%d-%d %d:%d:%d", year, month, day, hour, minute, second)
                    + " result=" + result);
            return result;
        }
        ...
        public boolean equals(Object o) {
            ...
            return (net_level == cmp.net_level
                    && net_id == cmp.net_id
                    && msg_id == cmp.msg_id);
        }
        ...
        public String toString() {
            return new StringBuilder().append("(net_level=")
                .append(net_level)
                .append(", net_id=")
                .append(net_id)
                .append(", msg_id=")
        ...
        static Comparator<Profile> getComparator() {
            return new Comparator<Profile>() {
                ...
                public int compare(Profile r1, Profile r2) {
                    return (r1.priority == r2.priority) ? 0 :
                        ((r1.priority < r2.priority) ? -1 : 1);
                }
