...
//                RingtoneManager.TYPE_ALARM);
		Uri alert = Alarms.getAlarmAlertUri(this);
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
            updateAlarmVolumeAndSetting(AudioManager.ADJUST_LOWER);
            return true;
        } else if (keyCode == KeyEvent.KEYCODE_VOLUME_UP) {
            updateAlarmVolumeAndSetting(AudioManager.ADJUST_RAISE);
            return true;
        } else {
    ...
    private void updateAlarmVolumeAndSetting(int direction) {
        final AudioManager audioManager = (AudioManager) getSystemService(Context.AUDIO_SERVICE);
        switch (direction) {
        case AudioManager.ADJUST_LOWER:
            audioManager.adjustStreamVolume(AudioManager.STREAM_ALARM,direction, AudioManager.FLAG_SHOW_UI|AudioManager.FLAG_PLAY_SOUND);
            break;
        case AudioManager.ADJUST_RAISE:
            audioManager.adjustStreamVolume(AudioManager.STREAM_ALARM,direction, AudioManager.FLAG_SHOW_UI|AudioManager.FLAG_PLAY_SOUND);
            break;
        }
        int volumeValue = audioManager.getStreamVolume(AudioManager.STREAM_ALARM);
        PreferenceManager.getDefaultSharedPreferences(this).edit().putString("volume_button_setting",
				                  String.valueOf(volumeValue)).commit();
    }
}
