...
public class NfcSoftwareStack extends Activity implements OnClickListener {
	...
	private ArrayList<ModeMap> registerNotificationArray =
		new ArrayList<ModeMap>();
	private ArrayList<ModeMap> discoveryNotificationArray =
		new ArrayList<ModeMap>();
		...
		if (arg0 == mBtnSet) {
			...
			try {
				...
				if (NfcCommonDef.ERROR_OK == checkReslut) {
					...
					Toast.makeText(getApplicationContext(),
						"Response is null!", Toast.LENGTH_LONG).show();
				} else {
					Toast
						.makeText(getApplicationContext(),
							"Response status is: " + checkReslut,
			...
			for (ModeMap m : registerNotificationArray) {
				m.chkBox.setChecked(shouldSelect);
			}
	...
	private int checkSecureElementSupport(int secureEleMask) throws Exception {
		...
		try {
			...
			Elog.e(TAG, "NfcNativeCallClass.checkSecureElementSupport(req) EXP!"
				+ e.getMessage());
			throw e;
		}
		if (resp == null) {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					Elog.e(TAG, "sendScanCommand(req) EXP!" + e.getMessage());
					mHandler.sendEmptyMessage(EVENT_OP_EXCEPTION);
				}
	...
	private int sendScanCommand() throws Exception {
		...
		Elog.d(TAG, String.format("REQ: dis_type, duration,%d,%d", req.dis_type,
			req.duration));
		...
		try {
			...
			if (resp == null) {
				Elog
					.e(TAG,
						"NfcNativeCallClass.getSettings(req) (resp == null)");
				return NfcCommonDef.ERROR_ERROR;
			}
			if (resp.status != 0) {
				Elog.e(TAG,
			...
			Elog.e(TAG, "NfcNativeCallClass.sendDiscoveryNotif(req) EXP!"
				+ e.getMessage());
			throw e;
		}
	...
	private void onScanOK() {
		...
		if (resp.target instanceof NfcNativeCallClass.nfc_none_det_response) {
			...
			NfcRespMap.getInst().put(NfcRespMap.KEY_SS_P2P_TARGET_DECT,
				resp.target);
			intent.setClassName(this,
				"com.mediatek.engineermode.nfc.NfcSoftwareStackP2PInitDect"); // no support
			this.startActivity(intent);
			// mHandler.sendEmptyMessage(EVENT_UNDER_CONSTRUCT);
		} else {
	...
	private void initUI() {
		...
		registerNotificationArray.add(new ModeMap(
			(CheckBox) findViewById(R.id.NFC_Settings_Result_Support_MifareUL),
			0));
		registerNotificationArray
			.add(new ModeMap(
		...
		registerNotificationArray
			.add(new ModeMap(
				...
				4));
		registerNotificationArray.add(new ModeMap(
			(CheckBox) findViewById(R.id.NFC_Settings_Result_Support_NFC), 5));
		registerNotificationArray
			.add(new ModeMap(
		...
		registerNotificationArray.add(new ModeMap(
			...
			7));
		// =========================
		discoveryNotificationArray.add(new ModeMap(
		...
		discoveryNotificationArray.add(new ModeMap(
			(CheckBox) findViewById(R.id.NFC_SoftwareStack_DN_felica424), 3));
		discoveryNotificationArray.add(new ModeMap(
			(CheckBox) findViewById(R.id.NFC_SoftwareStack_DN_ISO15693), 4));
		discoveryNotificationArray.add(new ModeMap(
			(CheckBox) findViewById(R.id.NFC_SoftwareStack_DN_NFCActive), 5));
		discoveryNotificationArray.add(new ModeMap(
			(CheckBox) findViewById(R.id.NFC_SoftwareStack_DN_DCE), 6));
		discoveryNotificationArray.add(new ModeMap(
			(CheckBox) findViewById(R.id.NFC_SoftwareStack_DN_DisableP2P), 7));
	...
	private int getValFromDisNotiBox() {
		int val = 0;
		for (ModeMap m : discoveryNotificationArray) {
			if (m.chkBox.isChecked()) {
				val |= (1 << m.bit);
			}
		}
