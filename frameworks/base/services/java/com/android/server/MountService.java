...
class MountService extends IMountService.Stub
    ...
    private static final int MTP_RESERVE_SPACE = 10485760;//This number is by MTP request
    ...
    private static final Object OMADMSyncLock = new Object(); 
    //
    private static final Object TurnOnUsbSyncLock = new Object(); 
    private static final Object CDRomShareLock = new Object(); 

    private static final int UNMOUNT_WAIT      = 1500;//wait APP handle MEDIA_EJECT
    ...
    private boolean                               mIsTurnOnUsb = false;
    ...
    //flag indicate whether we have already send Intent.ACTION_MEDIA_EJECT
    //private boolean 							  mAlreadySendIntent = false;
    ...
    class UmsEnableCallBack extends UnmountCallBack {
        ...
        @Override
        void handleFinished() {
            if(mIsUsbConnected == false ) {
                updatePublicVolumeState(this.path, Environment.MEDIA_CHECKING);
                sendStorageIntent(Intent.ACTION_MEDIA_CHECKING, path);
                updatePublicVolumeState(this.path, Environment.MEDIA_MOUNTED);
                sendStorageIntent(Intent.ACTION_MEDIA_MOUNTED, path);
            } else {
                synchronized (TurnOnUsbSyncLock) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case H_UNMOUNT_PM_UPDATE: {
                    ...
                    sendStorageIntent(Intent.ACTION_MEDIA_EJECT, ucb.path);
                    if (FeatureOption.MTK_2SDCARD_SWAP && mUnmountSwap && !ucb.path.equals("/mnt/sdcard2")) {
                        sendStorageIntent(Intent.ACTION_MEDIA_EJECT, "/mnt/sdcard2");
                    } 
                    SystemClock.sleep(UNMOUNT_WAIT);
                        
                    //before killing processes, send media_eject intent first
                    /*if(!mAlreadySendIntent){
                    	mAlreadySendIntent = true;
                    	sendStorageIntent(Intent.ACTION_MEDIA_EJECT, ucb.path);
                    ...
                    if (!mUpdatingStatus) {
                        ...
                        } else if ((mUnmountPrimary == false) || FeatureOption.MTK_SHARED_SDCARD) {
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_BOOT_COMPLETED)) {
                Slog.d(TAG, "MountService BOOT_COMPLETE");
                mBooted = true;
                mShutdownRet = StorageResultCode.OperationSucceeded;
                ...
                new Thread() {
                    ...
                    public void run() {
                        ...
                        try {
                            try {
                                //mIsUsbConnected = doGetShareMethodAvailable("ums");
                                Slog.d(TAG, "updating vaild ums " + Boolean.toString(mIsUsbConnected));
                            } catch (Exception ex) {
                        ...
                        updateDefaultpath();
                        Slog.d(TAG, "BOOT_COMPLETE  mSetDefaultEnable = true");
                ...
                new Thread() {
                    ...
                    public void run() {
                    	...
                    	try {
                            ...
                            Slog.e(TAG, "Error get sd state while IPO", e);
                        }
                        ...
                        updateDefaultpath();
                        Slog.d(TAG, "BOOT_IPO  mSetDefaultEnable = true");
                        mSetDefaultEnable = true;
                        mUnmountPrimary = false;
                        mMountAll = false;
                    }
            ...
            } else if (action.equals(OMADM_USB_ENABLE)) {
            	Slog.e(TAG, "USB enable");
				new Thread() {
                    public void run() {
						synchronized (OMADMSyncLock) {
							try {
						...
						synchronized (OMADMSyncLock) {
                            ...
                            for (int i = 0; i < length; i++) {
                                ...
                                try {
								    ...
								    if (StorageResultCode.OperationSucceeded == Ret) Slog.e(TAG, "SD format Succeed!");
								    else Slog.e(TAG, "SD format Failed!");
	                            } catch (Exception ex) {
			...
			} else if(action.equals("android.action.AMS_SYSTEM_READY")) {  //power off alarm
                Slog.v(TAG, "receive intent - android.action.AMS_SYSTEM_READY");
                new Thread() {
                    public void run() {
                        try {
                            String[] paths;
                        ...
                        updateDefaultpath();
                        Slog.d(TAG, "AMS_SYSTEM_READY  mSetDefaultEnable = true");
                        mSetDefaultEnable = true;
                    }
                }.start();//power off alarm
            } else if (action.equals(Intent.ACTION_LOCALE_CHANGED)) {
                new Thread() {
                    public void run() {                  
                        synchronized (mVolumeStates) {                       
                            Slog.i(TAG, "Intent.ACTION_LOCALE_CHANGED");
                            //clear mPrimaryVolume before re-read list
                            mPrimaryVolume = null ;
                            readStorageList(resources);
                            doShareSDVolumeUpdate();
                            if (FeatureOption.MTK_2SDCARD_SWAP) {
            ...
            } else if (action.equals(Intent.ACTION_PRE_BOOT_COMPLETED)){
                try {  
				    Runtime.getRuntime().exec("rm -r /mnt/sdcard/sdcard2/");
			    } catch (IOException e) {
					Slog.i(TAG, "delete /mnt/sdcard/sdcard2/ fail"+ e);
			    }
            }
        }
    };
    ...
    private void updatePublicVolumeState(String path, String state) {
        ...
        if (path.equals(mExternalStoragePath)) {
            ...
            if (!mEmulateExternalStorage) {
                ...
                } else if (Environment.MEDIA_MOUNTED.equals(state) && path.equals(Environment.getExternalStorageDirectory().getPath())) {
    ...
    private void notifyVolumeStateChange(String label, String path, int oldState, int newState) {
        ...
        if (newState == VolumeState.Init) {
            ...
            if (!vs.equals(
                ...
                updatePublicVolumeState(path, Environment.MEDIA_UNMOUNTED);
                if (mSetDefaultEnable && !mIsTurnOnUsb && !mMountSwap && !mUnmountSwap) {
            ...
            //before killing processes, send media_eject intent first
        	//if (mAlreadySendIntent == true){
        	//	action =null;
        	//	mAlreadySendIntent = false;
        	//}
        	//else {
            //    action = Intent.ACTION_MEDIA_EJECT;
        	//}
    ...
    private int doMountVolume(String path) {
        ...
        try {
            ...
            mConnector.doCommand(String.format(Locale.ENGLISH, "volume mount %s%s", path, arg));
        } catch (NativeDaemonConnectorException e) {
            /*
    ...
    private int doUnmountVolume(String path, boolean force, boolean removeEncryption) {
        ...
        try {
            ...
            mConnector.doCommand(String.format(Locale.ENGLISH, "volume unmount %s%s", path, arg));
    ...
    private int doFormatVolume(String path) {
        try {
            String cmd = String.format(Locale.ENGLISH, "volume format %s", path);
    ...
    private boolean doGetVolumeShared(String path, String method) {
        String cmd = String.format(Locale.ENGLISH, "volume shared %s %s", path, method);
    ...
    private void notifyShareAvailabilityChange(final boolean avail) {
        Slog.e(TAG, "notifyShareAvailabilityChange avail="+avail);
        synchronized (mListeners) {
            mUmsAvailable = avail;
            for (int i = mListeners.size() -1; i >= 0; i--) {
                MountServiceBinderListener bl = mListeners.get(i);
        ...
        final String path = getUMSPath();
        if (avail == false && (getVolumeState(path).equals(Environment.MEDIA_SHARED) || mIsTurnOnUsb == true)) {
            /*
    ...
    public MountService(Context context) {
        ...
        mEmulateExternalStorage = Environment.isSomeStorageEmulated();
        ...
        filter.addAction("android.action.AMS_SYSTEM_READY");//power off alarm
        filter.addAction(OMADM_USB_ENABLE);
        filter.addAction(OMADM_USB_DISABLE);
        filter.addAction(OMADM_SD_FORMAT);  
    ...
    public void shutdown(final IMountShutdownObserver observer) {
        ...
        synchronized (mVolumeStates) {
        	//if mnt/sdcard is mounted while shutdown
    ...
    public boolean isUsbMassStorageConnected() {
        //ICS default don't need get this value from vold
        //use USB intent instead, so no need waitForReady()
        //waitForReady();
    ...
    public void setUsbMassStorageEnabled(boolean enable) {
        ...
        String mState = Environment.getExternalStorageState();
        if (mState.equals(Environment.MEDIA_MOUNTED) && !FeatureOption.MTK_SHARED_SDCARD) {
            mUnmountPrimary = true;
        }
    ...
    public boolean isUsbMassStorageEnabled() {
        waitForReady();
        return doGetVolumeShared(getUMSPath(), "ums");
    }
    ...
    public int mountVolume(String path) {
        ...
        if (FeatureOption.MTK_2SDCARD_SWAP && isExternalStorage(path) && !path.equals("/mnt/sdcard") && !FeatureOption.MTK_SWAP_STATIC_MODE) {
            mMountSwap = true;
            sendStorageIntent(Intent.ACTION_MEDIA_EJECT, 
    ...
    public void unmountVolume(String path, boolean force, boolean removeEncryption) {
        ...
        if (FeatureOption.MTK_2SDCARD_SWAP && isExternalStorage(path) && !FeatureOption.MTK_SWAP_STATIC_MODE) {
            mUnmountSwap = true;
	...
	public int getMediaProcessStorageUser(String path) {
		   validatePermission(android.Manifest.permission.MOUNT_UNMOUNT_FILESYSTEMS);
		   waitForReady();
		   int pid = -1;
		   try {
			   ...
			   for (int i = 0; i < r.length; i++) {
				   String []tok = r[i].split(" ");
				   if (tok[1].equals("android.process.media")){
				   	   try {
                           pid = Integer.parseInt(tok[0]);
                       } catch (NumberFormatException nfe) {
                           Slog.e(TAG, String.format("Error parsing pid %s", tok[0]));
                           return -1;
                       } 
    ...
    public int createSecureContainer(String id, int sizeMb, String fstype,
        ...
        if(FeatureOption.MTK_2SDCARD_SWAP && 
            getVolumeState(Environment.getExternalStorageDirectory().getPath()).equals(Environment.MEDIA_MOUNTED)) {
            if(!isSDExist()) {
                Slog.e(TAG, "External SD not exist, createSecureContainer fail!");
                return StorageResultCode.OperationFailedInternalError;
            }
        }
        ...
        int rc = StorageResultCode.OperationSucceeded;
        String cmd = String.format(Locale.ENGLISH, "asec create %s %d %s %s %d", id, sizeMb, fstype, key, ownerUid);
        try {
            mConnector.doCommand(cmd);
        } catch (NativeDaemonConnectorException e) {
            rc = StorageResultCode.OperationFailedInternalError;
    ...
    public int finalizeSecureContainer(String id) {
        ...
        int rc = StorageResultCode.OperationSucceeded;
        try {
            mConnector.doCommand(String.format(Locale.ENGLISH, "asec finalize %s", id));
            /*
    ...
    public int destroySecureContainer(String id, boolean force) {
        ...
        int rc = StorageResultCode.OperationSucceeded;
        try {
            mConnector.doCommand(String.format(Locale.ENGLISH, "asec destroy %s%s", id, (force ? " force" : "")));
        } catch (NativeDaemonConnectorException e) {
            int code = e.getCode();
    ...
    public int mountSecureContainer(String id, String key, int ownerUid) {
        ...
        int rc = StorageResultCode.OperationSucceeded;
        String cmd = String.format(Locale.ENGLISH, "asec mount %s %s %d", id, key, ownerUid);
        try {
            mConnector.doCommand(cmd);
        } catch (NativeDaemonConnectorException e) {
            int code = e.getCode();
    ...
    public int unmountSecureContainer(String id, boolean force) {
        ...
        int rc = StorageResultCode.OperationSucceeded;
        String cmd = String.format(Locale.ENGLISH, "asec unmount %s%s", id, (force ? " force" : ""));
        try {
            mConnector.doCommand(cmd);
        } catch (NativeDaemonConnectorException e) {
            int code = e.getCode();
    ...
    public int renameSecureContainer(String oldId, String newId) {
        ...
        int rc = StorageResultCode.OperationSucceeded;
        String cmd = String.format(Locale.ENGLISH, "asec rename %s %s", oldId, newId);
        try {
            mConnector.doCommand(cmd);
        } catch (NativeDaemonConnectorException e) {
            rc = StorageResultCode.OperationFailedInternalError;
    ...
    public String getSecureContainerPath(String id) {
        ...
        try {
            ArrayList<String> rsp = mConnector.doCommand(String.format(Locale.ENGLISH, "asec path %s", id));
    ...
    public String getSecureContainerFilesystemPath(String id) {
        ...
        try {
            ArrayList<String> rsp = mConnector.doCommand(String.format(Locale.ENGLISH, "asec fspath %s", id));
            String []tok = rsp.get(0).split(" ");
            int code = Integer.parseInt(tok[0]);
            if (code != VoldResponseCode.AsecPathResult) {
                throw new IllegalStateException(String.format("Unexpected response code %d", code));
    ...
    public String getMountedObbPath(String filename) {
        ...
        try {
            ArrayList<String> rsp = mConnector.doCommand(String.format(Locale.ENGLISH, "obb path %s", filename));
            String []tok = rsp.get(0).split(" ");
            int code = Integer.parseInt(tok[0]);
            if (code != VoldResponseCode.AsecPathResult) {
                throw new IllegalStateException(String.format("Unexpected response code %d", code));
    ...
    public int getEncryptionState() {
        ...
        if(FeatureOption.MTK_EMMC_SUPPORT) {
        	Slog.w(TAG, "getEncryptionState MTK_EMMC_SUPPORT");
        	return ENCRYPTION_STATE_NONE;
        }
    ...
    public int encryptStorage(String password) {
        ...
        try {
            mConnector.doCommand(String.format(Locale.ENGLISH, "cryptfs enablecrypto inplace %s", password));
        } catch (NativeDaemonConnectorException e) {
            // Encryption failed
            return e.getCode();
        }
    ...
    class MountObbAction extends ObbAction {
        ...
        public void handleExecute() throws IOException, RemoteException {
            ...
            int rc = StorageResultCode.OperationSucceeded;
            String cmd = String.format(Locale.ENGLISH, "obb mount %s %s %d", mObbState.filename, hashedKey,
                    mObbState.callerUid);
            try {
                mConnector.doCommand(cmd);
            } catch (NativeDaemonConnectorException e) {
    ...
    class UnmountObbAction extends ObbAction {
        ...
        public void handleExecute() throws IOException {
            ...
            int rc = StorageResultCode.OperationSucceeded;
            String cmd = String.format(Locale.ENGLISH, "obb unmount %s%s", mObbState.filename,
                    (mForceUnmount ? " force" : ""));
            try {
                mConnector.doCommand(cmd);
            } catch (NativeDaemonConnectorException e) {
        ...
        new Thread() {
            public void run() {
                if("yes".equals(SystemProperties.get("sys.usb.mtk_bicr_support")) 
                    ...
                    waitForReady();
                    synchronized(CDRomShareLock) {
                        int state = doGetCDRomState();
                        Slog.i(TAG, "CDRom status="+state);

                        if((state==CDRomState.Shared && doShare==false) || (state==CDRomState.Unshared && doShare==true)) {
    ...
    private boolean doGetSDExist() {
        ...
        for (String line : rsp) {
            ...
            int code;
            try {
                code = Integer.parseInt(tok[1]);
            } catch (NumberFormatException nfe) {
                Slog.e(TAG, String.format("Error parsing code %s", tok[1]));
                return false;
            }
            if (code == 1) {
                return true;
    ...
    private void doShareSDVolumeUpdate() {
        if (FeatureOption.MTK_SHARED_SDCARD) {
		    mPrimaryVolume = new StorageVolume(
                ...
                mContext.getString(com.android.internal.R.string.storage_internal), //mPrimaryVolume.getDescription(), 
                mPrimaryVolume.isRemovable(), 
                true,//emulated
                MTP_RESERVE_SPACE,//mPrimaryVolume.getMtpReserveSpace(),  by MTP request
    ...
    private String getUMSPath() {
        ...
        "/mnt/sdcard2" : Environment.getExternalStorageDirectory().getPath();
    }

    private void doSDSwapVolumeUpdate() {
        Slog.i(TAG, "swap_volume_attribute");
        //for sd swap featuer
        //if SD exist, need to change info of primary volume, such as description, removable
        if (FeatureOption.MTK_2SDCARD_SWAP && (isSDExist() || mUnmountSwap)) {
            StorageVolume tVolume;
            StorageVolume pVolume;
            StorageVolume volume;
            synchronized (mVolumeMap) {
                volume = mVolumeMap.get("/mnt/sdcard2");
                if (volume==null) {
                    return;
                }
                ...
                pVolume = new StorageVolume(
                    ...
                    volume.isRemovable(),
                    volume.isEmulated(),
                    volume.getMtpReserveSpace(),
                    volume.allowMassStorage(),
                    volume.getMaxFileSize());

                tVolume = new StorageVolume(
                    volume.getPath(),
                    mPrimaryVolume.getDescription(),
                    mPrimaryVolume.isRemovable(),
                    mPrimaryVolume.isEmulated(),
                    mPrimaryVolume.getMtpReserveSpace(),
                    mPrimaryVolume.allowMassStorage(),
                    mPrimaryVolume.getMaxFileSize());

                mPrimaryVolume = pVolume;
                mVolumeMap.put("/mnt/sdcard", pVolume);
                mVolumeMap.put("/mnt/sdcard2", tVolume);
            }
            ...
            synchronized (mVolumes) {
                ...
                if (length > 0) {
                    mVolumes.set(0, mPrimaryVolume);
                    for (int i = 0; i < length; i++) {
                        if (mVolumes.get(i).getPath().equals("/mnt/sdcard2")) {
    ...
    private void sendSDSwapIntent() {
        ...
        intent.putExtra(SD_EXIST, isSDExist());
    ...
    private void updateDefaultpathForSwap(boolean mount, String path) {
        ...
        String storage1 = Environment.getExternalStorageDirectory().getPath();
        String storage2 = "/mnt/sdcard2";
        if (mount && storage2.equals(path)) {
            ...
            StorageManager.setDefaultPath(storage2);
        } else if (!mount) {
            //Two case if swap enable when unmount
            String defaultPath = StorageManager.getDefaultPath();
            if (storage1.equals(defaultPath)) {
            ...
            } else if (storage2.equals(defaultPath)) {
                ...
                StorageManager.setDefaultPath(storage1);
            }
        }
    }
}
