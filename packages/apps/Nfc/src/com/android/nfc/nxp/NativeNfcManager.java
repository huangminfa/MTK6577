...
import java.io.File;
/// M: import the FeatureOption class 
...
public class NativeNfcManager implements DeviceHost {
    ...
    /// M: select firmware path according to feature option
    private static final String NFC_CONTROLLER_FIRMWARE_FILE_NAME = FeatureOption.MTK_NFC_FW_NXP ? "/vendor/firmware/libpn544_fw.so" : (FeatureOption.MTK_NFC_FW_INSIDE ? "Inside.path" : "MtkInhouse.path");
	...
	public native boolean doIsUiccSecureElementPresence();

	public native void doSelectUiccSecureElement(int select);

	public native void doSelectUiccSecureElementInPBTF(int select);
	
	@Override
	public void selectUiccSecureElement(int select) {
		if (doIsUiccSecureElementPresence()) {
			doSelectUiccSecureElement(select);
			}
	}

	@Override
	public void selectUiccSecureElementInPBTF(int select) {
		if (doIsUiccSecureElementPresence()) {
			doSelectUiccSecureElementInPBTF(select);
			}
	}	

	@Override
	public native void doSelectP2pBitrate106(int select);
	// ----- MTK ends ----- //
