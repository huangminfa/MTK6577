...
public class OmaReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (LOG) MtkLog.v(TAG, "onReceive(" + intent.getAction() + ")");
        mContext = context;
    ...
    private void sendRtspCapability() {
        ...
        if (LOG) MtkLog.v(TAG, "sendRtspCapability()...");
    }
    
    private void saveRtspSetting(Bundle extras) {
        if (LOG) MtkLog.v(TAG, "saveRtspSetting(" + extras + ")");
        if (extras != null) {
    ...
    private boolean writeSetting(Bundle extras) {
        if (LOG) MtkLog.v(TAG, "writeSetting(" + extras + ")");
        String to_proxy = "('" + catString(extras.getStringArrayList(KEY_TO_PROXY), "','") + "')";
        ...
        if (LOG) MtkLog.v(TAG, "writeSetting() count=" + count);
    ...
    private String catString(ArrayList<String> list, String seperator) {
        ...
        if (LOG) MtkLog.v(TAG, "catString() return " + elements);
        return elements;
    }
