...
public class CropView extends GLView {
    ...
    private int mFirstImageWidth = SIZE_UNKNOWN;
    private int mFirstImageHeight = SIZE_UNKNOWN;
    private int mSecondImageWidth = SIZE_UNKNOWN;
    private int mSecondImageHeight = SIZE_UNKNOWN;
    ...
    private boolean setImageViewPosition(int centerX, int centerY, float scale) {
        ...
        changed = setTileViewPosition(mImageView, mImageWidth, mImageHeight, 
                                      centerX, centerY, scale);
        if (mIsStereoDisplaySupported) {
            changed =
                setTileViewPosition(mFirstTileView, mFirstImageWidth,
                    mFirstImageHeight, centerX, centerY, scale) || changed;
            changed =
                setTileViewPosition(mSecondTileView, mSecondImageWidth, 
                    mSecondImageHeight, centerX, centerY, scale) || changed;
        }
        return changed;
    }

    private boolean setTileViewPosition(TileImageView t, int width, int height,
                                        int centerX, int centerY, float scale) {
        int inverseX = width - centerX;
        int inverseY = height - centerY;
        int rotation = mImageRotation;
        switch (rotation) {
            case 0: return t.setPosition(centerX, centerY, scale, 0);
            case 90: return t.setPosition(centerY, inverseX, scale, 90);
    ...
    private class HighlightRectangle extends GLView {
        ...
        private void validateRect(RectF rect) {
            if (rect.left < 0.0f) rect.left = 0.0f;
            if (rect.right > 1.0f) rect.right = 1.0f;
            if (rect.top < 0.0f) rect.top = 0.0f;
            if (rect.bottom > 1.0f) rect.bottom = 1.0f;
        }
    ...
    public void setDataModel(TileImageView.Model dataModel, int rotation) {
        ...
        if (mIsStereoDisplaySupported) {
            mFirstTileView.setModel(dataModel);
            mSecondTileView.setModel(dataModel);
            if (((rotation / 90) & 0x01) != 0) {
                mFirstImageWidth = dataModel.getImageHeight(StereoHelper.STEREO_INDEX_FIRST);
                mFirstImageHeight = dataModel.getImageWidth(StereoHelper.STEREO_INDEX_FIRST);
                mSecondImageWidth = dataModel.getImageHeight(StereoHelper.STEREO_INDEX_SECOND);
                mSecondImageHeight = dataModel.getImageWidth(StereoHelper.STEREO_INDEX_SECOND);
            } else {
                mFirstImageWidth = dataModel.getImageWidth(StereoHelper.STEREO_INDEX_FIRST);
                mFirstImageHeight = dataModel.getImageHeight(StereoHelper.STEREO_INDEX_FIRST);
                mSecondImageWidth = dataModel.getImageWidth(StereoHelper.STEREO_INDEX_SECOND);
                mSecondImageHeight = dataModel.getImageHeight(StereoHelper.STEREO_INDEX_SECOND);
            }
        }
        mAnimation.initialize();
    }
    ...
    public void setStereoMode(boolean stereoMode) {
        ...
        getGLRoot().setStereoMode(mStereoMode);
    }
