        ...
        private String executeSensitiveCommand(String command, String sensitiveReplacement)
            ...
            try {
                ...
                if (response.length() > 1 && response.charAt(0) == '-') {
                    Log.w(Logging.LOG_TAG, "Email send command [" + command + "] receive Server response [" + response+"]" );
                    throw new MessagingException(MessagingException.UNSPECIFIED_EXCEPTION,response);
                } else if ( null == response ){
                    Log.w(Logging.LOG_TAG, "Email send command [" + command + "] receive Server response is null" );
                } else if (null != response && 0 == response.length()){
                    Log.w(Logging.LOG_TAG, "Email send command [" + command + "] receive Server response is 0 byte" );
                    throw new MessagingException(MessagingException.UNSPECIFIED_EXCEPTION, response);
                }
                return response;
            } catch(IOException e) {
