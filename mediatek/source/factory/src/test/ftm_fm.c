...
int fm_entry(struct ftm_param *param, void *priv)
{
	...
	for(i=0; i<8; i++){
    	...
    	if(headset_state > 0){
            ...
            sprintf(fmr->info, "%s", "Please insert headset before do FM test!\n"); 
            iv->redraw(iv);
...
static void *fm_update_thread(void *priv)
{
    ...
    if (g_fm_fd < 0)
    {
        ...
        sprintf(fmr->info, "%s", "FM open failed\n"); 
        iv->redraw(iv);
        goto out;
    }
 ...
 #endif
        g_fm_fd = -1;
        sprintf(fmr->info, "%s", "FM power on failed\n"); 
        iv->redraw(iv);
        goto out;
    }
 ...
 #endif
        g_fm_fd = -1;
        sprintf(fmr->info, "%s", "FM mute after on failed\n"); 
        iv->redraw(iv);
        goto out;
    }
    ...
    sprintf(fmr->info, "%s", "FM power on successfully\n");
    iv->redraw(iv);
        ...
        if(g_setfreq)
        {
         ...
         if(!FMAudioEnable)
         {
           ...
           ret = fm_tune(&parm_tune) ? 0 : 1;
            ...
            if (ret)
            {
                sprintf(fmr->info+strlen(fmr->info), "Failed\n");
                iv->redraw(iv);
                printf("[FM] FM_IOCTL_TUNE failed:%d:%d\n", ret, parm_tune.err);
            }
            else
            {
                sprintf(fmr->info+strlen(fmr->info), "Successfully\n");
                iv->redraw(iv);
#if (defined MT5192_FM || defined MT5193_FM)
               rssi = fm_getrssi();
               ret = 0;
                ...
                {
                     sprintf(fmr->info+strlen(fmr->info), "RSSI: %d (dBm)\n", rssi);
                     iv->redraw(iv);
                }
            }
        }
