...
static void renderText(OpenGLRenderer* renderer, const jchar* text, int count,
    ...
    const jfloat* xOffset = value->getXOffset();
    const jfloat* yOffset = value->getYOffset();
    if (xOffset == NULL || yOffset == NULL) {
        renderer->drawText((const char*) glyphs, bytesCount, glyphsCount, x, y, paint);
    } else {
        renderer->drawText((const char*) glyphs, xOffset, yOffset, bytesCount, glyphsCount, x, y, paint);
    }
    /** @} */
#else
...
static bool android_view_GLES20Canvas_drawDisplayList(JNIEnv* env,
    ...
    uint32_t err = 0;
    bool redraw = renderer->drawDisplayList(displayList, width, height, bounds, err);
    if (err != 0)
    {
        LOGE("Fail to drawDisplayList: 0x%x", err);
        renderer->outputDisplayList(displayList);
#ifndef MTK_BSP_PACKAGE
        jniThrowException(env, "android/view/GLES20Canvas$InvalidDrawException", "Invalid DisplayList");
#endif
    }
