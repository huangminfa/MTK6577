...
class TelephonyRegistry extends ITelephonyRegistry.Stub {
    ...
    TelephonyRegistry(Context context, boolean isGemini) {
        this(context);
        if(isGemini){
        mySimId = Phone.GEMINI_SIM_2; // Overwrite mySimId when Gemini enabled
        }else{
            mySimId = Phone.GEMINI_SIM_1; // Overwrite mySimId when Gemini enabled
        }
    }
    //MTK-END [mtk04070][111128][ALPS00093395]Added for Gemini
    ...
    private void broadcastServiceStateChanged(ServiceState state) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            intent.putExtra(Phone.GEMINI_SIM_ID_KEY, mySimId);
        }
    ...
    private void notifyDataConnectionChanged(Intent intent) {

        Phone.DataState phoneState;
        int state;
        ...
        phoneState = Enum.valueOf(Phone.DataState.class,
                        intent.getStringExtra(Phone.STATE_KEY));
        state = DefaultPhoneNotifier.convertDataState(phoneState);
        
        isDataConnectivityPossible = !intent.getBooleanExtra(Phone.NETWORK_UNAVAILABLE_KEY,false);
        reason = intent.getStringExtra(Phone.STATE_CHANGE_REASON_KEY);
        apn = intent.getStringExtra(Phone.DATA_APN_KEY);
        apnType = intent.getStringExtra(Phone.DATA_APN_TYPE_KEY);
        linkProperties = intent.getParcelableExtra(
                                    Phone.DATA_LINK_PROPERTIES_KEY);
        linkCapabilities = intent.getParcelableExtra(
                                    Phone.DATA_LINK_CAPABILITIES_KEY);
        roaming = intent.getBooleanExtra(Phone.DATA_NETWORK_ROAMING_KEY, false);
        ...
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            networkType = ((telephony!=null) ? telephony.getNetworkTypeGemini(simId) : TelephonyManager.NETWORK_TYPE_UNKNOWN);
        }
        
        if (DBG) {
            Slog.i(TAG, " notifyDataConnectionChanged: state=" + state + " isDataConnectivityPossible="
                + isDataConnectivityPossible + " reason='" + reason
                + "' apn='" + apn + "' apnType=" + apnType + " networkType=" + networkType);
        }
        synchronized (mRecords) {
    ...
    private BroadcastReceiver mDataConnChangeReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            if (TelephonyIntents.ACTION_ANY_DATA_CONNECTION_STATE_CHANGED_MOBILE.equals(intent.getAction())) {
                ...
                notifyDataConnectionChanged(intent);
            }
        }        
    };
