...
#undef LOG_TAG
#endif
#define LOG_TAG "HeadphoneCompFltCustParam"
...
void getDefaultHeadphoneCompFltParam(AUDIO_ACF_CUSTOM_PARAM_STRUCT *audioParam)
{
    memcpy((void*)audioParam, (void*)&(audio_hcf_custom_default), sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT));
}

int  GetHeadphoneCompFltCustParamFromNV(AUDIO_ACF_CUSTOM_PARAM_STRUCT *audioParam)
{
    int result = 0;
    ...
    LOGV("audio_nvram_fd.iFileDesc=%d, audio_nvram_fd.ifile_lid=%d",audio_nvram_fd.iFileDesc, audio_nvram_fd.ifile_lid);
    result = read(audio_nvram_fd.iFileDesc, audioParam, rec_size*rec_num);
    LOGV("result=%d, &audioParam=0x%x",result, &audioParam);
