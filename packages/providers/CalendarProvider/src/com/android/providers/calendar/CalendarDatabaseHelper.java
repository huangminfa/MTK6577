...
/* package */ class CalendarDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String LAST_SYNCED_EVENT_COLUMNS =
            ...
            Events.LUNAR_RRULE + "," +
            ///@}
            ///M: add CREATE_TIME & MODIFY_TIME columns. @{
            Events.CREATE_TIME +"," +
            Events.MODIFY_TIME;
            ///@}
    ...
    private void createEventsTable(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + Tables.EVENTS + " (" +
                ...
                ///M: add CREATE_TIME & MODIFY_TIME columns. @{
                CalendarContract.Events.CREATE_TIME + " INTEGER," +
                CalendarContract.Events.MODIFY_TIME + " INTEGER," +
                /// @}
                ///M: add IS_LUNAR/LUNAR_RRULE Columns for lunar calendar. @{
                CalendarContract.Events.IS_LUNAR + " INTEGER NOT NULL DEFAULT 0," +
                CalendarContract.Events.LUNAR_RRULE + " TEXT" +
                /// @}
                ");");
    ...
    private void createCalendarsTable(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + Tables.CALENDARS + " (" +
                ...
                ");");
        ///M: add PC Sync account in Calendars table.
    ...
    private static void createEventsView(SQLiteDatabase db) {
        ...
        String eventsSelect = "SELECT "
                ...
                ///M:add CREATE_TIME/CREATE_TIME. @{
                + CalendarContract.Events.CREATE_TIME + ","
                + CalendarContract.Events.MODIFY_TIME + ","
                ///@}
                ///M: add IS_LUNAR/LUNAR_RRULE Columns for lunar calendar. @{
                + CalendarContract.Events.IS_LUNAR + ","
                + CalendarContract.Events.LUNAR_RRULE + ","
                /// @}
    ...
    ///M: add createPCSyncAccount() function. @{
    /**
     * Create a PC sync account in Calendars table.
     * @param db The database
     */
    private void createPCSyncAccount(SQLiteDatabase db) {
        String pcSync = "PC Sync";
        String pcSyncColor = "-9215145";
        String accessLevel = "700";
        String syncEvents = "1";
    ...
    ///M: upgrade database to 309--MTK database version. @{
    @VisibleForTesting
    void upgradeToVersion309(SQLiteDatabase db) {
        /*
         * Changes from version 308 to 309:
         * - add isLunar table to db
         * - add lunarRrule to Events table
         * The old solution is add isLunar,lunarRrule columns in upgradeToVersion308();
         * So, here we should judge if the columns is already exist in database.
         * (If the columns already exist in db, and we add them again, it will throw SqlException.)
         * 
         * Note: we try to get the Events table columns by 
         * [ Cursor cursor = db.rawQuery("PRAGMA table_info(Events)", null); ]
         * but failed, so we use a common query to get a cursor to judge the columns.
         */

        Cursor cursor = db.rawQuery("select * from Events where _id=0", null);
        int isLunarIndex = cursor.getColumnIndex(CalendarContract.Events.IS_LUNAR);
        int lunarRruleIndex = cursor.getColumnIndex(CalendarContract.Events.LUNAR_RRULE);
        cursor.close();
        Log.i(TAG, "upgradeToVersion309, isLunarIndex=" + isLunarIndex + "lunarRruleIndex=" + lunarRruleIndex);
        ///M:#Lunar# add IS_LUNAR/LUNAR_RRULE Columns for lunar calendar. @{
        if(isLunarIndex == -1) {
            db.execSQL("ALTER TABLE Events ADD COLUMN isLunar INTEGER NOT NULL DEFAULT 0;");
        }
        if(lunarRruleIndex == -1) {
            db.execSQL("ALTER TABLE Events ADD COLUMN lunarRrule TEXT;");
        }
        ///@}
    }
    ///@}
