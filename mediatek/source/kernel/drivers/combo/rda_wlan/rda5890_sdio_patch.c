...
* RDA5990E/F1足?t21???米?¯:
*     1. 1?芍a?～2??邦??豕??‘??㏒?∩車米?芍¯?㏒
*     2. 802.1X米?EAPOL那y?Y～邦那1車?℅?D??迄㊣??迄?那﹞⊿?赤?㏒
*     3. ?‘??o赤??D??DMPLL???那D?2???
*     4. 那y?Y～邦?DPM?谷?邦?a1
*     5. ACK????～邦?DPM?谷?邦?a1
*     6. ????(SSID)米?AP芍??車
*     7. 辰t2?SSID米?AP芍??車
*     8. ?‘???角1?米??車??
*     9. 2??邦芍??車?∫3?DSSS-OFDM㏒?PBCC㏒?Channel Agility米豕????米?AP㏒?辰?～?㊣赤???a
*        ??足???那??y?迆谷“?豕㏒??y?迆芍??車
*     10.2??邦芍??車℅谷2ㄓTKIP?車?邦﹞?那?key index2??a1米?AP㏒?辰?～?㊣赤???a豕??∟﹞?那?那?
*        WPA/WPA2米?AP?3辰???那㊣??芍??車2?谷??﹞車谷?¯㏒?????辰??㊣??那??y?迆??豕?IP米??﹞米?
*        ℅邦那???豕?2?米?㏒???1y辰???那㊣????o車(2?D豕辰a?????﹞車谷?¯)?赤?邦?y3㏒芍??車
*     11.車??‘Wlan﹞⊿?赤℅??‘?迄?那????
*     12.車??‘BT/WIFI12∩??迆??
*
* 21??那㊣??: 
...
const u32 wifi_notch_data[][2] =
{
    ...
    {0x00100a14, 0x057c71de},   
    {0x00100a18, 0x50090044},   
    {0x00100a1c, 0x10000000},   
    //ch 7
    ...
    {0x00100a94, 0x057c7e22},   
    {0x00100a98, 0x50090044},   
    {0x00100a9c, 0x10000000},   
    //ch 9
...
int rda5890_write_sdio32_polling(struct rda5890_private *priv,const unsigned int (*data)[2], unsigned int size)
{
	...
	{
		ret = rda5890_set_core_init_polling(priv, (const unsigned int (*)[2])&data[8*index][0], 8);
        if(ret < 0)
        goto err;
	}
	...
	if(count%8 > 0)
	{
		ret = rda5890_set_core_init_polling(priv, (const unsigned int (*)[2])&data[8*index][0], count%8);
        if(ret < 0)
        goto err;
	}	
err:
...
int rda5890_write_sdio32(struct rda5890_private *priv, const unsigned int (*data)[2], unsigned int size)
{
	...
	for(index = 0; index < count/8; index ++)  //each time write five init data
	{
		ret = rda5890_set_core_init(priv, (const unsigned int (*)[2])data[8*index], 8);
        if(ret < 0)
            goto err;
	}
	...
	if(count%8 > 0)
	{
		ret = rda5890_set_core_init(priv, (const unsigned int (*)[2])data[8*index], count%8);
        if(ret < 0)
            goto err;
	}	
err:
...
int rda5890_write_sdio8_polling(struct rda5890_private *priv, const unsigned char (*data)[2], unsigned int size)
{
	...
	for(index = 0; index < count/8; index ++)  //each time write five init data
	{
		ret = rda5890_set_core_patch_polling(priv, (const unsigned char (*)[2])data[8*index], 8);
	    if(ret < 0)
	        goto err;
	}
	...
	if(count%8 > 0)
	{
		ret = rda5890_set_core_patch_polling(priv, (const unsigned char (*)[2])data[8*index], count%8);
	    if(ret < 0)
	        goto err;
	}	
err:
...
int rda5890_write_sdio8(struct rda5890_private *priv ,const unsigned char (*data)[2], unsigned int size)
{
	...
	for(index = 0; index < count/8; index ++)  //each time write five init data
	{
		ret = rda5890_set_core_patch(priv, (const unsigned char (*)[2])data[8*index], 8);
	  if(ret < 0)
	      goto err;
	}
	...
	if(count%8 > 0)
	{
		ret = rda5890_set_core_patch(priv, (const unsigned char (*)[2])data[8*index], count%8);
	  if(ret < 0)
	      goto err;
	}	
err:
...
static const u32 rda5990_assoc_power_save_data32[][2] =
{
    {  0x5000050c, 0x00008000 }// for association power save
};
...
static const unsigned int wifi_test_mode_rx_notch_32[][2] =
{
	//item:notch_filter_5
...
static const unsigned int wifi_test_mode_agc_patch32[][2] =
{
...
static const unsigned int rda5990_test_mode_digital32[][2] =
{ 
