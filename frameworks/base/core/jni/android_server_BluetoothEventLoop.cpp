...
static void initializeNativeDataNative(JNIEnv* env, jobject object) {
    ...
    LOGI("android_server_BluetoothEventLoop.cpp: initializeNativeDataNative");
    if (NULL == nat) {
        LOGE("%s: out of memory!", __FUNCTION__);
        return;
    }
...
static jboolean setUpEventLoop(native_data_t *nat) {
    LOGV("[JNI][API] setUpEventLoop");

    LOGI("[JNI][API] setUpEventLoop(0x%X) : gapSock=%d", nat, nat->gapSock);
    if (nat != NULL && nat->gapSock < 0) {
        ...
        if (nat->gapSock < 0)
        {
            LOGE("[JNI][ERR] create gap socket failed : %s, errno=%d", strerror(errno), errno);
            return JNI_FALSE;
        }
...
btmtk_sdp_req_struct *btmtk_util_find_sdp_request(native_data_t *nat, bt_addr_struct *addr)
{
    ...
    LOGI("[GAP] btmtk_util_find_sdp_request search 0x%X:0x%X:0x%X", addr->lap, addr->uap, addr->nap);
    while (ptr)
    {
        LOGI("[GAP] btmtk_util_find_sdp_request found 0x%X:0x%X:0x%X", ptr->addr.lap, ptr->addr.uap, ptr->addr.nap);
...
void btmtk_util_update_adapter_property_discovering(JNIEnv *env, native_data_t *nat, bool is_discovering)
{
    ...
    LOGI("[GAP] btmtk_util_update_adapter_property_discovering: is_discovering = %d", is_discovering);
        ...
        if (nat->paired_cache_no)
        {
            ...
            for (i = 0; i < nat->paired_cache_no; i++)
            {
                ...
                if(value.array_val[i])
                {
                    ...
                    LOGI("[GAP] Set Devices: %s", value.array_val[i]);  
                }
            }
        }
...
void btmtk_util_update_device_property_trusted(JNIEnv *env, native_data_t *nat, bt_addr_struct *addr, bool trusted)
{
    ...
    LOGI("[GAP] btmtk_util_update_device_property_trusted: trusted = %d", trusted);
...
void btmtk_util_update_device_property_paired(JNIEnv *env, native_data_t *nat, bt_addr_struct *addr, bool paired)
{
    jclass stringClass = NULL;
    jobjectArray str_array = NULL;
    Properties prop;
    ...
    LOGI("[GAP] btmtk_util_update_device_property_paired: paired = %d", paired);
    if ((p_entry = btmtk_paired_dev_cache_find(nat, addr)) != NULL)
    {
...
void btmtk_util_update_device_property_name(JNIEnv *env, native_data_t *nat, bt_addr_struct *addr, char *devicename, int namelen)
{
    ...
    LOGI("[GAP] btmtk_util_update_device_property_name: name = %s", devicename);
...
void btmtk_inquired_dev_cache_add(native_data_t *nat, btmtk_device_entry_struct *entry)
{
    int i;
    btmtk_device_entry_struct *ptr = NULL;
	if (entry == NULL)
	{
		LOGE("[GAP] btmtk_inquired_dev_cache_add: entry is null ");
		return;
	}

    LOGI("[GAP] btmtk_inquired_dev_cache_add: addr=0x%X:0x%X:0x%X", entry->addr.lap, entry->addr.uap, entry->addr.nap);

    for (i = 0; i < nat->inquired_cache_no; i++)
    {
        if (btmtk_util_equal_bdaddr(&nat->inquired_dev_cache[i].addr, &entry->addr))
        {
            LOGI("[GAP] btmtk_inquired_dev_cache_add: already paired");
    ...
    if (!ptr)
    {
        ...
        {
            LOGE("[GAP] btmtk_inquired_dev_cache_add: exceed %d entries", BTBM_ADP_MAX_INQUIRY_NO);
            return;
        }
        ptr = &nat->inquired_dev_cache[nat->inquired_cache_no++];
    }
    LOGI("[GAP] btmtk_inquired_dev_cache_add: %d device inquired", nat->inquired_cache_no);
...
void btmtk_inquired_dev_cache_reset(native_data_t *nat)
{
    ...
    LOGI("[GAP] btmtk_inquired_dev_cache_reset");
    ...
    for (i = 0; i < count; i++)
    {
        ...
        {
            LOGI("[GAP] the device is in bonding");
...
void btmtk_host_cache_write(native_data_t *nat)
{
    ...
    LOGI("[GAP] btmtk_host_cache_write");
    if ((fd = open(BTMTK_HOST_CACHE_PATH, O_WRONLY | O_CREAT, 0666)) < 0)
    {
        LOGE("[GAP] btmtk_host_cache_write open error %d: %s", errno, strerror(errno));
        return;
    }
	len = write(fd, &nat->host_cache, size);
	if (len < 0 || len != size) 
    {
        LOGE("[GAP] btmtk_host_cache_write %d byte error %d: %s", len, errno, strerror(errno));
	}
    close(fd);
}
...
void btmtk_host_cache_init(native_data_t *nat)
{
    ...
    LOGI("[GAP] btmtk_host_cache_init");
        ...
        (fd = open(BTMTK_HOST_CACHE_PATH, O_RDONLY)) < 0)
    {
        ...
        property_get("ro.operator.optr", op, "");
        ...
        LOGE("[GAP] btmtk_host_cache_init open error %d: %s", errno, strerror(errno));
    }
    else
    {
        int read_byte = read(fd, &nat->host_cache, size), i;
        if (read_byte < 0) 
        {
            LOGE("[GAP] btmtk_host_cache_init read error %d: %s", errno, strerror(errno));
        } 
...
void btmtk_paired_dev_cache_write(native_data_t *nat)
{
    ...
    LOGI("[GAP] btmtk_paired_dev_cache_write");
    if ((fd = open(BTMTK_DEV_CACHE_PATH, O_WRONLY | O_CREAT, 0666)) < 0)
    {
        LOGE("[GAP] btmtk_paired_dev_cache_write open error %d: %s", errno, strerror(errno));
        return;
    }
	len = write(fd, &nat->paired_dev_cache, size);
	if (len < 0 || len != size) 
    {
        LOGE("[GAP] btmtk_paired_dev_cache_write %d byte error %d: %s", len, errno, strerror(errno));
	}
    close(fd);
}
...
void btmtk_paired_dev_cache_del(native_data_t *nat, bt_addr_struct *addr)
{
    ...
    LOGI("[GAP] btmtk_paired_dev_cache_del: addr=0x%X:0x%X:0x%X", addr->lap, addr->uap, addr->nap);
    for (i = 0; i < nat->paired_cache_no; i++)
    {
        if (btmtk_util_equal_bdaddr(&nat->paired_dev_cache[i].addr, addr))
        {
            LOGI("[GAP] btmtk_paired_dev_cache_del: found");
            idx = i;
            break;
        }
    }
    ...
    LOGI("[GAP] btmtk_paired_dev_cache_del: %d device paired", nat->paired_cache_no);
    btmtk_paired_dev_cache_write(nat);
}
...
void btmtk_paired_dev_cache_add(native_data_t *nat, btmtk_device_entry_struct *entry)
{
    ...
    LOGI("[GAP] btmtk_paired_dev_cache_add: addr=0x%X:0x%X:0x%X", entry->addr.lap, entry->addr.uap, entry->addr.nap);
    entry->paired = TRUE;
    for (i = 0; i < nat->paired_cache_no; i++)
    {
        if (btmtk_util_equal_bdaddr(&nat->paired_dev_cache[i].addr, &entry->addr))
        {
            LOGI("[GAP] btmtk_paired_dev_cache_add: already paired");
    ...
    if (!ptr)
    {
        ...
        {
            LOGE("[GAP] btmtk_paired_dev_cache_add: exceed %d entries", BTBM_ADP_MAX_PAIRED_LIST_NO);
            return;
        }
        ptr = &nat->paired_dev_cache[nat->paired_cache_no++];
    }
    LOGI("[GAP] btmtk_paired_dev_cache_add: %d device paired", nat->paired_cache_no);
    memcpy(ptr, entry, sizeof(btmtk_device_entry_struct));
    btmtk_paired_dev_cache_write(nat);
}
...
void btmtk_paired_dev_cache_init(native_data_t *nat)
{
    ...
    LOGI("[GAP] btmtk_paired_dev_cache_init");
        ...
        (fd = open(BTMTK_DEV_CACHE_PATH, O_RDONLY)) < 0)
    {
        ...
        LOGE("[GAP] btmtk_paired_dev_cache_init open error %d: %s", errno, strerror(errno));
    }
    else
    {
        int read_byte = read(fd, nat->paired_dev_cache, size), i;
        if (read_byte < 0) 
        {
            LOGE("[GAP] btmtk_paired_dev_cache_init read error %d: %s", errno, strerror(errno));
        } 
        else if (read_byte != size) 
        {
            LOGE("[GAP] btmtk_paired_dev_cache_init read error(%d bytes read) %d: %s", read_byte, errno, strerror(errno));
        }
        close(fd);
...
static void btmtk_gap_handle_pair_ind(native_data_t *nat, JNIEnv *env, bt_addr_struct *bd_addr, kal_uint32 cod, kal_uint8 name_len, char *name)
{
    jobjectArray str_array = NULL;
    jclass stringClass = NULL;
    Properties prop;
    ...
    LOGI("[GAP] remote initiate pairing");
    ...
    name = (name_len) ? name : (char *)BTMTK_ANDROID_DEFAULT_REMOTE_NAME;
    strncpy(inquired_entry.name, name, BTBM_ADP_MAX_NAME_LEN);
    if (strlen(name) >= BTBM_ADP_MAX_NAME_LEN)
    {
	    inquired_entry.name[BTBM_ADP_MAX_NAME_LEN-1]='\0';
    }
    inquired_entry.cod = cod;
    inquired_entry.legacy_pairing = true;
    btmtk_inquired_dev_cache_add(nat, &inquired_entry);
...
static void btmtk_gap_handle_link_state_ind(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_LINK_STATE_IND linkno=%d addr=%s", ind_p->current_number, remote_device_path);
...
static void btmtk_gap_handle_discovery_result_ind(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_DISCOVERY_RESULT_IND addr=0x%X:0x%X:0x%X", ind_p->bd_addr.lap, ind_p->bd_addr.uap, ind_p->bd_addr.nap);
        ...
        for (i = 0; i < BTBM_ADP_MAX_SDAP_UUID_NO && idx < 32 * 6; idx++)
        {
            if (idx < 32)
            {
                if ((ind_p->service_list1 >> idx) & 0x1)
                {
                    ...
                    LOGI("[GAP] entry.uuid[%d] = 0x%x", i, entry.sdp.uuid[i]);
                    i++;
                }
            }
            else if (idx < 64)
	...
	{	
    	...
    	for (int i = 0; i < entry.sdp.uuid_no; i++)
    	{
			...
			if(value.array_val[i])
			{
				...
				LOGI("[GAP] btmtk_gap_handle_discovery_result_ind UUID: %s", value.array_val[i]);  
        	}
     	}
...
static void btmtk_gap_handle_discovery_update_ind(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_DISCOVERY_UPDATE_IND addr=0x%X:0x%X:0x%X", ind_p->bd_addr.lap, ind_p->bd_addr.uap, ind_p->bd_addr.nap);
...
{
    LOGI("[GAP] MSG_ID_BT_BM_DISCOVERY_CNF");
...
static void btmtk_gap_handle_delete_trust_cnf(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_DELETE_TRUST_CNF result=%d, addr=%s", cnf_p->result, object_path);
	...
	btmtk_device_entry_struct *paired_entry = btmtk_paired_dev_cache_find(nat, &cnf_p->bd_addr);;
        ...
        if (inquired_entry)
        {
            inquired_entry->paired = false;
            inquired_entry->trusted = false;
        }
	else if (paired_entry) 
	{
	    paired_entry->paired = false;
  	...
  	btmtk_paired_dev_cache_del(nat, &cnf_p->bd_addr);
        ...
        btmtk_util_update_device_property_paired(env, nat, &cnf_p->bd_addr, FALSE);
        /* Handler */
        env->CallVoidMethod(nat->me,
...
static void btmtk_gap_handle_pin_code_ind(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    btmtk_util_convert_bdaddr2objpath(object_path, &ind_p->bd_addr);
    LOGI("[GAP] MSG_ID_BT_BM_PIN_CODE_IND addr=%s", object_path);
...
static void btmtk_gap_handle_bonding_result_ind(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_BONDING_RESULT_IND result=%d", ind_p->result);
    ...
    if (req!= NULL && req->type == BTMTK_SDP_CREATE_PAIRED_DEVICE)
    {
		...
		if(ind_p->result == BTBM_ADP_SUCCESS)
    	{
            ...
            /* wait for SDP result */
            LOGI("[GAP] btmtk_gap_handle_bonding_result_ind wait for SDP result");
            entry.sdp.uuid_no = 0;
    ...
    else
    {
        ...
        if(ind_p->result == BTBM_ADP_SUCCESS)
        {
            ...
            {
                LOGE("[GAP] cannot find addr=0x%X:0x%X:0x%X in inquired cache", ind_p->bd_addr.lap, ind_p->bd_addr.uap, ind_p->bd_addr.nap);
            }
            entry.cod = ind_p->cod;
            btmtk_paired_dev_cache_add(nat, &entry);
...
static void btmtk_gap_handle_bonding_cnf(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_BONDING_CNF result=%d", cnf_p->result);
...
static void btmtk_gap_handle_security_user_confirm_ind(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    number = atoi((const char*)ind_p->numeric);
    LOGI("[GAP] MSG_ID_BT_BM_SECURITY_USER_CONFIRM_IND : number=%d, display=%d", number, ind_p->display_numeric);
    btmtk_util_convert_bdaddr2objpath(object_path, &ind_p->bd_addr);
...
static void btmtk_gap_handle_security_passkey_entry_ind(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_SECURITY_PASSKEY_ENTRY_IND");
    btmtk_util_convert_bdaddr2objpath(object_path, &ind_p->bd_addr);
...
static void btmtk_gap_handle_security_passkey_notify_ind(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_SECURITY_PASSKEY_NOTIFY_IND : passkey=%d", ind_p->passkey);
    btmtk_util_convert_bdaddr2objpath(object_path, &ind_p->bd_addr);
...
//    char hash[] = {0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0xF, 0xE, 0xD, 0xC, 0xB, 0xA, 0x0, 0x9, 0x8, 0x7};
    ...
    LOGI("[GAP] MSG_ID_BT_BM_SECURITY_OOB_DATA_IND");
...
void btmtk_gap_handle_service_search_result_ind(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_SERVICE_SEARCH_RESULT_IND : uuid=0x%X", ind_p->uuid);
    ...
    if (req && req->uuid_no < BTBM_ADP_MAX_SDAP_UUID_NO)
    {
        ...
        LOGI("[GAP] MSG_ID_BT_BM_SERVICE_SEARCH_RESULT_IND : uuid_no=%d", req->uuid_no);
    }
}
...
void btmtk_gap_handle_service_search_cnf(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_SERVICE_SEARCH_CNF: addr=0x%X:0x%X:0x%X, result=%d, searched=%d", 
    ...
    if (req == NULL)
    {
        LOGE("[GAP] btmtk_gap_handle_service_search_cnf no sdp request found");
        return;
    }
    LOGI("[GAP] btmtk_gap_handle_service_search_cnf sdp req type=%d, uuid_no=%d", req->type, req->uuid_no);
    ...
    if (req->uuid_no)
    {
		...
		else if (req->uuid[i] == 0x2002)
		{
		    memcpy((char*)uuid128, SDP_SECURE, 16);
                ...
                btmtk_util_convert_uuid128_2_string(value.array_val[i], (char*)uuid128);
                LOGI("[GAP] btmtk_gap_handle_service_search_cnf UUID: %s", value.array_val[i]);  
            }
        }
    ...
    case BTMTK_SDP_CREATE_DEVICE:
    {
        ...
        LOGI("[GAP] btmtk_gap_handle_service_search_cnf: BTMTK_SDP_CREATE_DEVICE");
    ...
    case BTMTK_SDP_CREATE_PAIRED_DEVICE:
    {
        ...
        LOGI("[GAP] btmtk_gap_handle_service_search_cnf: BTMTK_SDP_CREATE_PAIRED_DEVICE");
        /* Update adapter property */
        btmtk_util_update_adapter_property_device(env, nat);
...
#endif
		btmtk_device_entry_struct *inquired_entry = btmtk_inquired_dev_cache_find(nat, &cnf_p->bd_addr);
		if (inquired_entry && inquired_entry->pairing == TRUE)
        {
    ...
    {
        bool result;
        LOGI("[GAP] btmtk_gap_handle_service_search_cnf: BTMTK_SDP_DISCOVERY");
    ...
    default:
        LOGE("[GAP] btmtk_gap_handle_service_search_result_ind() req->type=%d", req->type);
        break;
    }
    btmtk_util_list_remove((btmtk_list_header_struct **)&nat->requests, (btmtk_list_header_struct *)req);
}
...
void btmtk_gap_handle_service_search_raw_result(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_SEARCH_RAW_RESULT : addr=0x%X:0x%X:0x%X, len=%d", 
    ...
    if (req->type == BTMTK_SDP_DISCOVERY && req->app_uuid_no && cnf_p->len == 4)
    {
        btmtk_device_entry_struct *p_entry = btmtk_paired_dev_cache_find(nat, &cnf_p->bd_addr);
        btmtk_device_entry_struct *i_entry = btmtk_inquired_dev_cache_find(nat, &cnf_p->bd_addr);
        LOGI("[GAP] Service record handle = 0x%2x %2x %2x %2x", cnf_p->data[0], cnf_p->data[1], cnf_p->data[2], cnf_p->data[3]);
        if (p_entry)
        {
            int i;
            ...
            {
                LOGI("[GAP] Add %dth app_uuid to paired cache: result=0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x", 
        ...
        if (i_entry)
        {
            ...
            {
                LOGI("[GAP] Add app_uuid to inquiry cache");
...
void btmtk_gap_handle_service_search_raw_cnf(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] MSG_ID_BT_BM_SEARCH_RAW_CNF : result=%d", cnf_p->result);
...
void btmtk_hdp_handle_connect_cnf(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] btmtk_hdp_handle_connect_cnf : result=%d", cnf->result);
...
void btmtk_hdp_handle_disconnect_cnf(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] btmtk_hdp_handle_disconnect_cnf : result=%d", cnf->result);
...
void btmtk_hdp_handle_channel_opened_ind(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] btmtk_hdp_handle_channel_opened_ind ");
...
void btmtk_hdp_handle_channel_closed_ind(native_data_t *nat, JNIEnv *env, ilm_struct *ilm)
{
    ...
    LOGI("[GAP] btmtk_hdp_handle_disconnect_ind");
	btmtk_util_convert_bdaddr2objpath(device_path, &ind->bdaddr);
	...
	{
		LOGI("[GAP] btmtk_hdp_handle_disconnect_ind: invalid parms");
	}
}
...
void btmtk_handle_reset_ind(native_data_t *nat, JNIEnv *env, ilm_struct *rec_ilm)
{
    ...
    LOGI("[GAP] btmtk_handle_reset_ind");
...
*/
  ...
  /*      memset(&ilm, 0x0, sizeof(ilm_struct));
	...
	if (strlen(name) >= BTBM_ADP_MAX_NAME_LEN)
	{
        ...
        LOGI("[GAP] btmtk_gap_set_local_name : name=%s, len=%d", name, msg_p1->name_len);
...
*/
        /* Set scanenable */
    /*    LOGI("[GAP] btmtk_gap_set_scanable_mode: mode=0x%X", nat->host_cache.scan_mode);
        memset(&ilm, 0x0, sizeof(ilm_struct));
...
static bool msg_handler(native_data_t *nat, ilm_struct *ilm) {
    ...
    LOGI("[GAP] receive message=%d", ilm->msg_id);
    ...
    case MSG_ID_BT_BM_SECURITY_KEYPRESS_NOTIFY_IND:
        LOGW("MSG_ID_BT_BM_SECURITY_KEYPRESS_NOTIFY_IND handle not implemented yet");
        break;
    case MSG_ID_BT_POWERON_CNF:
		LOGI("[GAP] MSG_ID_BT_POWERON_CNF");
		break;
    case MSG_ID_BT_POWEROFF_CNF:
		LOGI("[GAP] MSG_ID_BT_POWEROFF_CNF");
		break;
    case MSG_ID_BT_BM_DISCOVERY_CANCEL_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_DISCOVERY_CANCEL_CNF");
		break;
    case MSG_ID_BT_BM_WRITE_LOCAL_NAME_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_WRITE_LOCAL_NAME_CNF");
		break;
    case MSG_ID_BT_BM_READ_LOCAL_NAME_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_READ_LOCAL_NAME_CNF");
		break;
    case MSG_ID_BT_BM_READ_REMOTE_NAME_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_READ_REMOTE_NAME_CNF");
		break;
    case MSG_ID_BT_BM_READ_REMOTE_NAME_CANCEL_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_READ_REMOTE_NAME_CANCEL_CNF");
		break;
    case MSG_ID_BT_BM_WRITE_SCANENABLE_MODE_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_WRITE_SCANENABLE_MODE_CNF");
		break;
    case MSG_ID_BT_BM_READ_SCANENABLE_MODE_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_READ_SCANENABLE_MODE_CNF");
		break;
    case MSG_ID_BT_BM_READ_LOCAL_ADDR_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_READ_LOCAL_ADDR_CNF");
		break;
    case MSG_ID_BT_BM_READ_LOCAL_COD_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_READ_LOCAL_COD_CNF");
		break;
    case MSG_ID_BT_BM_WRITE_LOCAL_COD_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_WRITE_LOCAL_COD_CNF");
		break;
    case MSG_ID_BT_BM_READ_PROPERTY_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_READ_PROPERTY_CNF");
		break;
    case MSG_ID_BT_BM_WRITE_AUTHENTICATION_MODE_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_WRITE_AUTHENTICATION_MODE_CNF");
		break;
    case MSG_ID_BT_BM_BLOCK_ACTIVE_LINK_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_BLOCK_ACTIVE_LINK_CNF");
		break;
    case MSG_ID_BT_BM_BLOCK_LIST_UPDATE_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_BLOCK_LIST_UPDATE_CNF");
		break;
    case MSG_ID_BT_BM_DELETE_TRUST_ALL_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_DELETE_TRUST_ALL_CNF");
		break;
    case MSG_ID_BT_BM_BONDING_CANCEL_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_BONDING_CANCEL_CNF");
		break;
    case MSG_ID_BT_BM_PAIRING_CANCEL_CNF:
		LOGI("[GAP] MSG_ID_BT_BM_PAIRING_CANCEL_CNF");
		break;
...
static bool event_handler(native_data_t *nat, ev_struct *event) {
    ...
    nat->vm->GetEnv((void**)&env, nat->envVer);
    LOGI("[GAP] receive event=%d", event->msg_id);
    env->PushLocalFrame(EVENT_LOOP_REFS);
    switch (event->msg_id)
    {
    ...
    case ANDROID_EV_GAP_LOCAL_NAME_CHANGE:
    {
        ...
        LOGI("ANDROID_EV_GAP_LOCAL_NAME_CHANGE : name=%s , len = %d", ind_p->name, ind_p->name_len);
    ...
    case ANDROID_EV_GAP_SCAN_MODE_CHANGE:
    {
        ...
        LOGI("ANDROID_EV_GAP_SCAN_MODE_CHANGE : mode = 0x%x", mode);
    ...
    case ANDROID_EV_GAP_SCAN_MODE_TIMEOUT_CHANGE:
    {
        ...
        LOGI("ANDROID_EV_GAP_SCAN_MODE_TIMEOUT_CHANGE : timeout = %d", *(int *)event->ilm_data);
	...
	case ANDROID_EV_GAP_PAIRED_DEVICE_RENAME:
    {
		...
		LOGI("[GAP] receive event ANDROID_EV_GAP_PAIRED_DEVICE_RENAME : name=%s , len = %d",
    ...
    case ANDROID_EV_SDP_DEVICE_CREATE:
    {
        ...
        LOGI("[GAP] receive event ANDROID_EV_SDP_DEVICE_CREATE 0x%X:0x%X:0x%X", addr->lap, addr->uap, addr->nap);
    ...
    case ANDROID_EV_SDP_PAIRED_DEVICE_CREATE:
    {
        ...
        LOGI("[GAP] receive event ANDROID_EV_SDP_PAIRED_DEVICE_CREATE 0x%X:0x%X:0x%X", addr->lap, addr->uap, addr->nap);
    ...
    case ANDROID_EV_SDP_DISCOVER:
    {
        ...
        LOGI("[GAP] receive event ANDROID_EV_SDP_DEVICE_CREATE 0x%X:0x%X:0x%X", ptr->addr.lap, ptr->addr.uap, ptr->addr.nap);
        LOGI("[GAP] pattern (%d) %2x:%2x:%2x:%2x", 
    ...
    case ANDROID_EV_DUN_PPPD_START:
    {
        ...
        if (ret < 0)
        {
            LOGE("[GAP] failed to start ppp daemon process: %d", ret);
        }
        break;
    }
    ...
    while (1) {
        ...
        for (int i = 0; i < nat->pollMemberCount; i++) {
            ...
            else 
            {
                ...
                LOGI("[MSG] nat->pollData[i].fd data ready : revents = 0x%X", nat->pollData[i].revents);
                while ((ret = recv(nat->pollData[i].fd, (void*)&ilm, sizeof(ilm_struct), MSG_DONTWAIT)) != -1) 
                {
                    LOGI("[MSG] msg %d received : size=%d", ilm.msg_id, ret);
					if (nat->service_nat->state != BTMTK_POWER_STATE_ON)
					{
                        LOGI("[MSG] Wrong state %d: discard msg", nat->service_nat->state);
				    }
	 ...
	 /*
        ...
        */
        LOGI("[MSG] Start polling");
        poll(nat->pollData, nat->pollMemberCount, -1);
        LOGI("[MSG] Polling returned");
    }
}
#endif
...
#ifdef __BTMTK__
    LOGV("[JNI][API] startEventLoopNative");
    event_loop_native_data_t *nat = get_native_data(env, object);
