...
#include "audio_custom_exp.h"		//include to see if we need to ENABLE_AUDIO_TRACK_LOUDNESS
...
void AudioTrack::start()
{
    ...
    LOGD("audiotrack %p start", this);///, mCblkMemory);
    if (t != 0) {
        if (t->exitPending()) {
...
status_t AudioTrack::obtainBuffer(Buffer* audioBuffer, int32_t waitCount)
{
	...
	if (cblktemp == 0){
		LOGE("obtainBuffer cblk is null %p", this);
		return DEAD_OBJECT;
	}
    ...
    uint32_t waitTimeMs = (waitCount < 0) ? cblk->bufferTimeoutMs : afLatency+2;
    ...
    if (framesAvail == 0) {
        ...
        while (framesAvail == 0) {
            active = mActive;
            if (UNLIKELY(!active)) {
                LOGD("%p, Not active and NO_MORE_BUFFERS", this);
                cblk->lock.unlock();
            ...
            LOGD_IF(cblk->flags & CBLK_DISABLED_MSK, "obtainBuffer track %p, ative=%d, user=%08x, server=%08x, loopend=%08x",
...
create_new_track:
    ...
    uint64_t bufferEnd =(uint64_t)(cblk->userBase + cblk->frameCount);

    if ( (uint64_t)(u + framesReq) > bufferEnd) {
        framesAvail = (uint32_t)(bufferEnd - u);        
...
ssize_t AudioTrack::write(const void* buffer, size_t userSize)
{
    ...
    LOGV("audiotrack %p: write %d bytes, mActive=%d", this, userSize, mActive);
...
bool AudioTrack::processAudioBuffer(const sp<AudioTrackThread>& thread)
{
    ...
    if (mActive && (cblk->framesAvailable() == cblk->frameCount)) {
        LOGV("Underrun %p, user: %x, server: %x, flowControlFlag %d", this, mCblk->user, mCblk->server, mCblk->flags);
...
status_t AudioTrack::AudioTrackThread::readyToRun()
{
    ...
    sched_p.sched_priority = RTPM_PRIO_AUDIOTRACK_THREAD;
        if(0 != sched_setscheduler(0, SCHED_RR, &sched_p)) {
            SXLOGV("pid [%d] sched_setscheduler not success: %d", getpid(), errno);
        }
        else {
            sched_p.sched_priority = RTPM_PRIO_AUDIOTRACK_THREAD;
            sched_getparam(0, &sched_p);
...
uint32_t audio_track_cblk_t::stepUser(uint32_t frameCount)
{
    ...
    if ((u >= userBase + this->frameCount)&& (userBase < (UINT_MAX - this->frameCount))) {
        userBase += this->frameCount;
    }
    else if(u >= (UINT_MAX - this->frameCount)){
        Mutex::Autolock _l(lock);
        uint32_t tempstep = this->frameCount * 2;
        userBase   += tempstep;
        serverBase += tempstep;
        this->user += tempstep;
        this->server += tempstep;
        u += tempstep;
        SXLOGD("track_cblk stepUser, reset");
    }
    ...
    ///LOGV("track_cblk %p stepUser, u %p, s %p, %d, %d", this, this->user, this->server, flags, bufferTimeoutMs);
    return u;
}
